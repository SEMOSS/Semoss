...
r <- data.frame(item=character(), fuzzyMatch=character(),similarity=numeric(),method = character(),stringsAsFactors=FALSE)
...
r<-r[order(r$item,r$fuzzyMatch,-r$similarity,r$method),]
return(r)
}