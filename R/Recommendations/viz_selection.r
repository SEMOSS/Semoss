...
get_viz_choices<-function(fileroot,df1,choice=''){
	...
	if(file.exists(filename)){
		...
		out<-paste0('Historical data ',filename,' does not exist')
	}
	gc()
	return(out)
}

get_viz_choices_helper<-function(df,df1){
	library(plyr)
	library(data.table)
...
build_viz<-function(choice,o,tbl,dt,q){			
	o<-o[o$chart==choice,]
	if(nrow(o)==1){
		tbl1<-merge(tbl,o)
		...
		q1<-paste0('Parameter ',choice,' does not match any of the available visualizations')
	}
	gc()
	return(q1)
}		
