...
best_match_lessthan<-function(col1,col2,maxdist=0,method="jw",p=0.1){
...
blend<-function(src,srccol,trg,trgcol,link,jointype="inner"){
	...
	#r<-r[,ifelse(!(colnames(r) %in% c(colnames(link)[1],colnames(link)[2],"i.and.d")))]
	#list <- c("i.and.d","col1","col2","dist.y","dist.x")
	#r<-r[,colnames(r) %in% list]
	#	r<-r[,colnames(r) %in% c("i.and.d","col1","col2","dist.y","dist.x")]
# 
	gc()
	return(r)
