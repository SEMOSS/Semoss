...
class AzureOpenAiClient(BaseClient):
  ...
  def __init__(self, 
    ...
    if (endpoint==None):
      raise Exception("Must define the Azure endpoint")
    if (api_key==None):
      raise Exception("Must define the Azure API Key")
  ...
  def ask(self, 
    ...
    if self.chat_type == 'chat-completion':
      ...
      if 'full_prompt' not in kwargs.keys():
        ...
        responses = openai.ChatCompletion.create(engine=self.model_name, stream = True, **kwargs)
        ...
        kwargs['messages'] = message_payload
        completion = openai.ChatCompletion.create(engine=self.model_name, **kwargs)
      ...
      responses = openai.Completion.create(engine=self.model_name, prompt=question, stream = True, **kwargs)
      for chunk in responses:
        if chunk.choices:
          if len(chunk.choices) > 0:
