...
from genai_client.remote_client_manager.remote_client_manager import RemoteClientManager
...
class ImageGenClient(AbstractImageGenerationClient):
    ...
    async def generate_image_remote(
        ...
        remote_client_manager = RemoteClientManager(uri=self.endpoint)
        request = {
            "prompt": prompt,
        ...
        try:
            response_data, base64_uncompressed = (
                await remote_client_manager.gaas_request(request)
            )
    ...
    # LEGACY CODE FOR GENERATING IMAGES LOCALLY ON SEMOSS
