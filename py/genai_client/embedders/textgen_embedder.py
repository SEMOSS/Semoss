...
class TextEmbeddingsInference(LocalEmbedder):
    ...
    def __init__(self, endpoint: str, model_name: str, **kwargs) -> None:
        self.model_name = model_name
        assert self.model_name != None

        self.batch_size = kwargs.pop(TextEmbeddingsInference.BATCH_SIZE, 32)

        self.embedder = self.get_embedder(endpoint=endpoint)
            ...
            def encode(self, sentences: List[str], **kwargs):
                ...
                for sublist in sentence_sublists:
                    ...
                    response = requests.post(
                        self.endpoint, json={"inputs": sublist, "truncate": True}
                    )
                    ...
                    if isinstance(result, list):
                        ...
                        raise RuntimeError("Unable to get vector embeddings.")
