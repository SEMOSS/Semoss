...
class Trainer:
    ...
    def load_lora(self, lora_name, replace_model=True):
        ...
        if hasattr(self.model, "load_adapter"):
            ...
            self.model = peft.PeftModel.from_pretrained(
                self.model, lora_name, adapter_name=lora_name
            )
    ...
    def tokenize_training_text(
        self, training_text, max_seq_length, separator="\n\n\n", **kwargs
    ):
        ...
        training_dataset = training_dataset.shuffle().map(
            lambda x: self.tokenize_sample(x, max_seq_length), batched=False
        )
