...
class PromptGuard:
        ...
        **kwargs,
        ...
        if pipe is None:
            ...
            model = AutoModelForCausalLM.from_pretrained(
                model_name, torch_dtype=dtype, device_map={"": 0}, **kwargs
            )
        ...
        self.parser = JsonSchemaParser(
            json.loads(
                """{'title': 'AnswerFormat', 'type': 'object', 'properties': {'category': {'title': 'Category', 'type': 'string'}, 'unsafe': {'title': 'Unsafe', 'type': 'boolean'}}, 'required': ['category', 'unsafe']}""".replace(
                    "'", '"'
                )
            )
        )
        ...
        self.prefix_function = build_transformers_prefix_allowed_tokens_fn(
            self.pipe.tokenizer, self.parser
        )
    ...
    def get_output(self, prompt=None, filter_list=None, **kwargs):
        ...
        output_dict = self.pipe(
            final_prompt, prefix_allowed_tokens_fn=self.prefix_function, **kwargs
        )
