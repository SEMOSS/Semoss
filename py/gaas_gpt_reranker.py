...
class Reranker:
  ...
  def __init__(self, reranker_name='BAAI/bge-reranker-base', **kwargs):
    ...
    self.reranker_model = AutoModelForSequenceClassification.from_pretrained(self.reranker_gaas_model)
  ...
  def model(self,
        ...
        **kwargs):
    tok_args = {"padding":True, "truncation":True, 'return_tensors':'pt', 'max_length':512}
    pair = input
    if 'tok_args' in kwargs:
      tok_args = kwargs.pop('tok_args')
    with torch.no_grad():
      inputs = self.reranker_tok(pair, **tok_args)
      scores = list(self.reranker_model(**inputs, return_dict=True).logits.view(-1, ).float().detach().numpy())
      return scores[0]
