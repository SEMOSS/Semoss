...
class LoraGPT():
  ...
  def load_base(self, llama_model='decapoda-research/llama-7b-hf'):
    ...
    self.model = AutoModelForCausalLM.from_pretrained(llama_model,load_in_8bit=True, device_map="auto")
  ...
  def init(self, llama_model='decapoda-research/llama-7b-hf', alpaca_model='tloen/alpaca-lora-7b'):
    from peft import PeftModel
    from transformers import LlamaTokenizer, LlamaForCausalLM, GenerationConfig, AutoModelForCausalLM
    ...
    self.model = AutoModelForCausalLM.from_pretrained(llama_model,load_in_8bit=True, device_map="auto")
    #this for dolly, alpaca 13b
    self.model = PeftModel.from_pretrained(self.model, alpaca_model,device_map={'': 0})
    #model = PeftModel.from_pretrained(model, alpaca_model)
    return self.model, self.tokenizer
