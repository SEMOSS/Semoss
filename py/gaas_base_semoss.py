...
class Gaas_Base_Semoss():
  ...
  def __init__(self, data_file = None, input_column=None, output_column=None, base_model="t5-base", model_output_dir=None, **kwargs):
  ...
  def tokenize_text(self):
    ...
    for sent in self.sentences:
      encoded_dict = self.tokenizer.encode_plus(sent, add_special_tokens=True, max_length=max_len, pad_to_max_length=True, return_attention_mask=True, return_tensors="pt")
  ...
  def get_scheduler(self, epochs=5, optimizer=None):
    ...
    total_steps = len(self.train_loader) * epochs # why does this not consider batch size
  ...
  def predict_value(self, text):
    sent_tokens = self.tokenizer(text, max_length=250, padding="max_length", truncation=True, add_special_tokens=True, return_attention_mask=True, return_token_type_ids=True, return_tensors='pt')
    inputs, masks, token_type_ids = sent_tokens['input_ids'], sent_tokens['attention_mask'], sent_tokens['token_type_ids']
