...
class FaissFrame():
    ...
    def add_rows(self, row_list:List) -> None:
        ...
        try:
            if self.ds is None:
                init_dict = {key: [row[key] for row in row_list] for key in row_list[0].keys()}
    ...
    def remove_rows(self, indices: List[int]) -> None:
        ...
        if self.ds is not None:
            self.ds = self.ds.select([i for i in range(len(self.ds)) if i not in indices])


    
    def search(self, search_embeddings:List[float], k:int = 5) -> List[Dict[str, Any]]:
        """
        ...
        scores, retrieved_examples = self.ds.get_nearest_examples(
            FaissFrame.EMBEDDINGS_COLUMN, 
            np.array(search_embeddings), 
            k=k
        )
        
        search_results = [
            {'Score': scores[i], **{col: retrieved_examples[col][i] for col in retrieved_examples.keys()}}
            for i in range(k)
        ]
