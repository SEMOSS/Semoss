...
class TableUtil:
    def __init__(self, target_folder=None):
        ...
        self.structure_recognizer = ggm.ModelEngine(
            local=True,
            engine_id="microsoft/table-transformer-structure-recognition",
            pipeline_type="object-detection",
        )
        self.target_folder = target_folder
        self.ocr = PaddleOCR(use_angle_cls=True, lang="en")

        # self.detector = pipeline("object-detection", model="microsoft/table-transformer-detection", device=device) s
        self.detector = ggm.ModelEngine(
            pipeline_type="object-detection",
            engine_id="microsoft/table-transformer-detection",
            local=True,
        )
        self.top = 50
        self.bottom = 90
    ...
    def parse_table_to_csv_ms(self, pil_image=None, target_folder=None):
        ...
        for row in rows:
            ...
            cell_coordinates.append(
                {"row": row["box"], "cells": row_cells, "cell_count": len(row_cells)}
            )
        ...
        for row in cell_coordinates:
            ...
            for column in row["cells"]:
                ...
                print(data_output)
                if (
                    data_output is not None
                    and data_output[0] is not None
                    and data_output[0][0] is not None
                    and data_output[0][0][1] is not None
                    and data_output[0][0][1][0] is not None
                ):
