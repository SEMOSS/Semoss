...
class SEMOSSDeepEvalAdapter(DeepEvalBaseLLM):
    def __init__(
        ...
        **params
        ...
        if access_key is not None and secret_key is not None:
            ...
            self.server_connection = ServerClient(
                access_key=self.loginKeys["accessKey"],
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            self.model_name, load_in_8bit=True, local_files_only=True, device_map="cuda"
        )
        self.tokenizer = AutoTokenizer.from_pretrained(
            self.model_name, device=self.model.device, use_fast=False
        )
        self.pipe = pipeline(
            "text-generation",
            model=self.model,
    ...
    def generate(self, prompt: str, schema: BaseModel) -> BaseModel:
        ...
        prefix_function = build_transformers_prefix_allowed_tokens_fn(
            self.pipe.tokenizer, parser
        )
        self.prompt_response = self.pipe(
            prompt, prefix_allowed_tokens_fn=prefix_function
        )

        print(self.prompt_response)
        output = self.prompt_response[0]["generated_text"][len(prompt) :]
        str_output = json.dumps(output, ensure_ascii=False)
