...
class PyFrame:
    ...
    def merge_match_results(this, col_name, link_frame):
        ...
        for index, old_value in enumerate(old_values):
            ...
            frame.replace(
                {col_name: old_value}, {col_name: new_value}, regex=False, inplace=True
            )
        return frame
    ...
    def replace_val2(
        ...
        if not inplace:
            this.makeCopy()
        nf = frame.replace(
            {col_name: cur_value}, {new_col: new_value}, regex=regx, inplace=inplace
        )
        print("replacing inplace")
    ...
    def regex_replace_val(this, col_name, old_value, new_value, inplace=True):
        ...
        frame.replace(
            {col_name: str(old_value)},
            {col_name: str(new_value)},
            regex=True,
            inplace=inplace,
        )
    ...
    def split_unpivot(this, col_name, delim, var="variable", inplace=True):
        ...
        output = pd.melt(
            frame, id_vars=idcols, value_vars=valcols, var_name=var, value_name=col_name
    ...
    def stat(this, col_name):
        frame = this.cache["data"]
        return pd.DataFrame(frame[col_name].describe(include=np.number)).to_dict(
            orient="index"
        )
    ...
    def collapse(this, group_col, agg_col, delim: object = "", other_cols="assign"):
        frame = this.cache["data"]
        res_frame = (
            frame.groupby(group_col)[agg_col]
            .apply(lambda x: delim.join(x.astype(str)))
            .reset_index()
        )
    ...
    def date_difference_columns(
        this, date_column1, date_column2, unit_of_measure, output_column
    ):
        frame = this.cache["data"]
    ...
    def date_difference_constant(
        this, date_column, date_constant, direction_bool, unit_of_measure, output_column
    ):
    ...
    def sig_fig_round(this, num, sf=2):
        ...
        return (
            round(num, sf - int(math.floor(math.log10(abs(num)))) - 1)
            if num != 0
            else 0
        )

    def discretize_column(
        this, col, new_col, breaks=None, labels=False, num_digits=None
    ):
        frame = this.cache["data"]
        ...
        # Just num_digits passed, round to num_digits sig figs
            ...
            frame[new_col] = pd.cut(
                frame[col], sig_fig_rounded_bins, include_lowest=True
            ).astype(str)
    ...
    def column_like(this, filterKey, col, searchstr):
        ...
        output = this.cache[final_key].ravel()[
            np.flatnonzero(
                np.char.find(this.cache[final_key + "__u"], str(searchstr).upper())
                != -1
            )
        ]
        return list(output)
    ...
    def column_not_like(this, filterKey, col, searchstr):
        final_key = this.idxColFilter(filterKey, col)
        output = this.cache[final_key].flatten()[
            np.flatnonzero(
                np.char.find(this.cache[final_key + "__u"], str(searchstr).upper())
                == -1
            )
    ...
    def hasFrameChanged(this):
        ...
        if len(shared_keys) > 0:
            modified = {
                o: (cur_meta[o], prev_meta[o])
                for o in shared_keys
                if cur_meta[o] != prev_meta[o]
            }
            if len(modified) > 0:
                cur_meta = this.cache["data"].dtypes.to_dict()
                this.cache["meta"] = cur_meta
