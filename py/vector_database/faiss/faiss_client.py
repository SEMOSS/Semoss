...
class FAISSSearcher():
        ...
        if name == 'encoded_vectors' or value != None:
            if name in ['ds']:
                if not isinstance(value, (pd.DataFrame, Dataset)):
                    raise TypeError(
                        f"{name} must be a pd.DataFrame or Dataset")
    ...
    def load_dataset_and_vectors(
        ...
        '''
        Utility method to load stored datasets and vectors.
        ...
        '''
        # Loading the dataset
        loaded_dataset = self._load_dataset(dataset_location=dataset_location)
        assert isinstance(loaded_dataset, Dataset)
        self.ds = loaded_dataset

        # Check if the dataset has an 'Embedding' column, parse it into a numpy array and load it into the object
        if 'Embedding' in loaded_dataset.features:
            vectors = np.array([list(map(float, emb.split(',')))
                               for emb in loaded_dataset['Embedding']], dtype=np.float32)
            self.encoded_vectors = vectors
            self.vector_dimensions = vectors.shape
        else:
            raise ValueError("Embedding column not found in the dataset")
    ...
    def _load_dataset(
        ...
        if (dataset_location.endswith('.csv')):
            try:
                ...
                for encoding in ENCODING_OPTIONS:
                    try:
                        temp_df = pd.read_csv(
                            dataset_location, encoding=encoding)
        ...
        extracted_with_cfg = all(col in dataset_columns for col in [
                                 'Source', 'Divider', 'Part', 'Tokens', 'Content'])
        if isinstance(loaded_dataset, Dataset) and extracted_with_cfg:
            ...
            try:
                loaded_dataset = loaded_dataset.cast(
                    new_features, keep_in_memory=True)
    ...
    def add_document(
        ...
        separator(`str`):
            The character to use as a delimiter between columns for the concatenated column that the embeddings will be created from
        ...
        assert {os.path.basename(os.path.dirname(path))
                for path in documentFileLocation} == {'indexed_files'}
        ...
        for document in documentFileLocation:
            ...
            new_file_path = os.path.join(
                directory,
                file_name_without_extension + '_dataset_vectors' + new_file_extension
            )
            ...
            if (dataset.num_rows > 0):
                ...
                if keyword_search_params is not None and keyword_search_params.pop('keywordSearch', None) is True:
                ...
                # Checking that this is a 2D array
                ...
                dataset = dataset.add_column("Embedding", vectors_str)
                ...
                createDocumentsResponse['createdDocuments'].append(
                    new_file_path)

                # TODO need to update the flow for how we instantiate
                if np.any(self.encoded_vectors) is None:
        ...
        master_indexClass_files, corrupted_file_sets = self.create_master_file(
            path_to_files=os.path.dirname(
                os.path.dirname(documentFileLocation[0]))
        )
    ...
    def _validateEmbeddingFiles(
        ...
        # List all source document files in the directory (e.g., PDFs)
        ...
        for full_source_path in source_documents:
            ...
            base_filename = os.path.splitext(source_file_name)[0]

            # Get the file names for the combined dataset
            combined_file_name = base_filename + "_dataset_vectors.pkl"
            full_combined_path = os.path.join(
                indexed_files_path, combined_file_name)

            # If the combined file path exists, attempt to load it
            if os.path.exists(full_combined_path):
                try:
                    # Try to load the combined dataset
                    with open(full_combined_path, "rb") as file:
                        dataset = pickle.load(file)
                except Exception as e:
                    corrupted_file_sets.append(
                        (full_combined_path, full_source_path)
                    )
                    corrupted_docs[full_source_path] = f"Couldn't load the combined file: {e}"
                    continue

                try:
                    # Extract embeddings from the loaded dataset
                    vectors = np.array([list(map(float, emb.split(',')))
                                       for emb in dataset['Embedding']], dtype=np.float32)
                except Exception as e:
                    corrupted_file_sets.append(
                        (full_combined_path, full_source_path)
                    )
                    corrupted_docs[
                        full_source_path] = f"Couldn't extract embeddings from the combined file: {e}"
                    continue
        ...
        if len(valid_datasets_and_vectors) > 0:
            ...
            # Save the combined dataset and vectors into a single file
            combined_file_path = os.path.join(
                path_to_files, "master_dataset_vectors.pkl")
            with open(combined_file_path, "wb") as file:
                pickle.dump(self.ds, file)
            created_documents.append(combined_file_path)
    ...
    def do_rerank(self,
        ...
        for _, row in samples_df.iterrows():
            ...
            self.class_logger.info(f"Row to pick {int(row['ann'])}", extra={
                                   "stack": "BACKEND"})
            self.class_logger.info(f"[{str(data_row['Content'])}]", extra={
                                   "stack": "BACKEND"})
        ...
        if reranker_call_success:
            new_output = sorted(
                final_output, key=lambda x: x['Sim'], reverse=True)
        else:
