...
class Interrogator:
    ...
    def load_model_cache(
        self, model_path=None, model_loader=AutoModelForCausalLM, **kwargs
    ):
        assert model_path is not None
        self.model = model_loader.from_pretrained(model_path, **kwargs)
    ...
    def serialize_model(self):
        ...
        if self.model_name is not None and self.model is not None:
            path = Path(
                hub_api.try_to_load_from_cache(
    ...
    def check_if_snapshot_available(self, repo_id=None, snapshot="main"):
        config_file = hub_api.try_to_load_from_cache(
            repo_id=repo_id, revision=snapshot, filename="config.json"
        )
