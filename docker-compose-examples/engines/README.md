...
```bash
docker compose -f semoss-weaviate.yml up -d      # or semoss-chroma.yml / -opensearch.yml / -pgvector.yml
...
`semoss-net`, so a dockerized SEMOSS reaches a vector DB **by its container name**
...
- All credentials here (`test-key`, `admin` / `Str0ngVectorP@ss1`,
  `pgvector` / `pgvector`) are local-dev defaults - change them before using any
  of this beyond your machine.
