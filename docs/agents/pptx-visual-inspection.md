...
**PPTX Reviewer** is a built-in system agent with workspace ID `pptx-reviewer`, packaged in `project/platform__pptx-reviewer.smss` and `project/platform__pptx-reviewer/`. Like the App Building Agent, `ProjectWatcher` catalogs it globally with the `SYSTEM` and `Workspace_Project` tags, and `SystemAgentSeeder` provisions its workspace at startup when ModelInferenceLogsDatabase is enabled. It has no owner and cannot be edited or deleted through workspace APIs. Its prompt (also documented in [pptx-reviewer-prompt.txt](pptx-reviewer-prompt.txt)) and configuration are restored from code on every startup.

The seeded configuration enables default tools, disables WriteFile, EditFile, MultiEdit, MoveFile, DeleteFile, BashCommand, ExecuteNodeCode and TodoWrite, and sets `tool_policy.result_tool` to `"InspectPptx"`. The Semoss harness persists that tool result verbatim as the final response and ends the run without another reviewer-model call or reflection. The result tool must be invoked alone in a batch. The reviewer has no MCP projects, skills, hooks or subagents. Its budgets are 3 tool rounds, 0 reflections and 900 seconds, with further delegation disabled. No installation-specific model IDs are shipped; use `InspectPptx.engine` or `PPTX_VISION_MODEL_ID` for the vision model. Create an editable copy of this agent if you need a pinned model or tool default.

Attach the system reviewer to the authoring agent's `CONFIG_JSON.subagents` array as `{"workspaceId":"pptx-reviewer"}`. For an existing author configured with a manually created reviewer, replace that reviewer's workspace reference with `pptx-reviewer`; existing user workspaces are not migrated or deleted at startup. The named tool alias is derived from the agent name (`agent_pptx_reviewer`). Named subagent calls currently take a `prompt` string and `inherit_parent_workdir`; send the task details in the prompt and set `inherit_parent_workdir: true` so the tool reads the author's saved file.

For the dedicated-reviewer workflow, add `InspectPptx` to the author's `tool_policy.default_tools.disabled` list, while leaving it enabled on the reviewer. This prevents the author from first calling the inspection service with its own text-only model. The system reviewer inherits the parent's tool-calling model; a separate vision engine is selected by the explicit `engine` argument or the deployment default. An editable copy can also set its own model and tool default. Include an exact requested vision ID in the delegation prompt; the reviewer forwards it unchanged.
...
1. **Save and validate.** Hash the saved PPTX and generator; verify requested count and structural validity. Until a build validates, structural failures are repaired within `repair_turns` rounds counted from the first failure; once a validated deck exists, a failed rebuild delivers that deck instead. An edit is checked only for errors it introduced: errors its input snapshot already had do not block it. A successful save sends advisory warnings directly to review before another author request.
