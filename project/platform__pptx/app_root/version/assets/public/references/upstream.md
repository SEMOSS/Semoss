## Upstream guidance (SEMOSS availability notes apply)
...
> SEMOSS: the Python/LibreOffice workflows below are unavailable through the agent tools. Use the Node authoring and validation workflow above.
...
> SEMOSS: native PptxGenJS authoring is available for any slide. The guarded `deck.create` path adds protections. All npm-install fallbacks below are unavailable; use `deck.validate` in place of Python validation.
...
- **Shape `w`/`h` must be ≥ 0** — pptxgenjs writes negative sizes into the XML verbatim and PowerPoint refuses to open the file (LibreOffice hides the fault). For a line that runs up or to the left, give positive `w`/`h` and set `flipV: true` / `flipH: true`.
...
> SEMOSS: use JS validation plus the design review above. The command-based QA and rendering steps below are unavailable here. A structural pass alone does not complete the review.
...
> SEMOSS: `markitdown` is unavailable. Review all requested content in the authored slides and notes, including native calls and any JSON spec.
...
> SEMOSS: use `deck.validate(outPath, { slides: requestedCount, strictCanvas: false })`. Review its structural errors and layout warnings. The upstream Python workflow is unavailable through the agent tools.
