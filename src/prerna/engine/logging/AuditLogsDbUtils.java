...
public class AuditLogsDbUtils {
	...
	private static void executeInitDatabaseSchema(IRDBMSEngine engine, Connection conn,
		...
		// Handle partitioning first using the new partition manager
		boolean enablePartitioning = Utility.isAuditLogsDatabasePartitioningEnabled();

		if (enablePartitioning && queryUtil.supportsTablePartitioning()) {
			AuditLogPartitionManager partitionManager = new AuditLogPartitionManager(engine, conn);
			partitionManager.setupPartitioning(dbSchema);
		}
		...
		for (Pair<String, List<Pair<String, String>>> tableSchema : dbSchema) {
			...
			// Skip AUDIT_LOGS if we already handled it with partitioning
			if (enablePartitioning && queryUtil.supportsTablePartitioning() && "AUDIT_LOGS".equals(tableName)) {
				continue; // Already handled by partition manager
			}

			// Regular table creation for other tables
