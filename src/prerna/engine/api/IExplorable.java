...
public interface IExplorable {
   ...
   *
   * @param concept The concept URI Assumes the concept URI is the conceptual URI
   * @param conceptualNames Boolean to determine if the return should be the properties conceptual
  ...
  //
  //	// REFAC: Change this to engine - this should be local master
  //	String getPropertyPhysicalUriFromConceptualUri(String conceptualURI, String
  // parentConceptualUri);
  //
  //	/**
  ...
  /**
   * Get the datatypes for the uris from the associated owl file If varargs param is empty, it will
  ...
  /**
   * Get the list of selectors for a given concept/table in TABLE__COLUMN format This will include
   * the TABLE if it contains data (i.e. RDF/Graph but not RDBMS) If you only want the properties,
   * please refer to {@link #getPropertyPixelSelectors(String)}
   *
  ...
  /**
   * Get the list of property selectors for a given concept/table in TABLE format If you want the
   * selectors including the concept (assuming it has data), please refer to {@link
   * #getPixelSelectors(String)}
   *
   * @param conceptPixelName
   * @return
   */
  ...
  /**
   * Get the physical URI based on the pixel selector The pixel selector input will be in
   * TABLE__COLUMN format
   *
   * @param pixelSelector
   * @return
   */
   ...
   *
   * <p>We cannot use this cause of the fact that we have not updated the OWL triples for a RDF
   * engine for the properties to contain the Concept in the URL (which would make it unique)
  ...
  /**
   * Get the primary key for table This is for legacy pixels where we use TABLE without specifying
   * the column in RDBMS engines
   *
   * @param physicalUri
   * @return
   */
