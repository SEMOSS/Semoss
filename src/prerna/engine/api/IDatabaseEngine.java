...
/**
 * This interface standardizes the functionality of all engines. All engines must implement this
 * interface so that they can be used without first recognizing what specific engine class it is. A
 * lot of different classes call on IEngine to refer to a specific engine, including, most notably,
...
public interface IDatabaseEngine extends IExplorable, IEngine {
  ...
  /**
   * Runs the passed string query against the engine. The query passed must be in the structure that
   * the specific engine implementation requires
   *
  ...
  /**
   * Runs the passed string query against the engine as an insert query. Query must be in the
   * structure that the specific engine implementation requires
   *
  ...
  /**
   * Commit the database. Commits the active transaction. This operation ends the active
   * transaction. Saves the db to a file
   */
  void commit();

  /**
   * Gets the type of the database. The database type is often used to determine what API to use
   * while running queries against the database.
   *
