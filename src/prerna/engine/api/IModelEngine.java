...
public interface IModelEngine extends IEngine {
	 ...
	 *
	 * @param question
	 *            The question being asked to the LLM
	 * @param context
	 *            (Optional) The context passed in by the user
	 * @param insight
	 *            The insight from where the call is being made. The insight holds
	 *            user credentials, project information and conversation history
	 *            tied to the insightId
	 * @param parameters
	 *            Additional parameters such as temperature, top_k, max_new_tokens
	 *            etc
	 ...
	 *            (Optional) The context passed in by the user
	 * @param room
	 *            The room from where the call is being made. The room holder the
	 *            insight and user, along with other room properties like tools, vec
	 *            dbs, system prompts.
	 * @param parameters
	 *            Additional parameters such as temperature, top_k, max_new_tokens
	 *            etc
	...
	/**
	 * Passes the string task along with other parameters such as context and
	 * temperature to the python client
	 *
	 * @param task
	 *            The task that the LLM is being asked to generate steps for
	 * @param context
	 *            (Optional) The context passed in by the user
	 * @param insight
	 *            The insight from where the call is being made. The insight holds
	 ...
	 *            etc
	 * @return creates a map response with the following keys - response : The
	 *         response from the LLM in the form of an array of strings where each
	 *         index represents an individual step - messageId : The unique
	 *         identifier of a message (the user's input and the model response) -
	 *         roomId: The insightId that the runPixel endpoint is being called from
	 */
	InstructModelEngineResponse instruct(String task, String context, List<Map<String, Object>> projectData,
