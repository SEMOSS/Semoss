...
public abstract class AbstractRUserConnection implements IRUserConnection {
              ...
              executor.submit(
                  new Callable<REXP>() {
                    ...
                    public REXP call() throws Exception {
                      REXP rexp =
                          rcon.eval(
                              rScript); // fails here .. if you wrapped this.. all is well I feel..
                      if (recoveryEnabled) {
  ...
  private void voidEval(
      String rScript, long healthTimeout, TimeUnit healthTimeoutUnit, boolean retry) {
    if (isHealthy(healthTimeout, healthTimeoutUnit)) {
      if (rScript.length() > 500) {
        logger.info("Running R: " + rScript.substring(0, 500) + "...");
        logger.debug("Running R: " + rScript);
      ...
      try {
        synchronized (rconMonitor) {
          ...
          try {
            ...
            logger.error(Constants.STACKTRACE, e);
            throw new IllegalArgumentException(
                "Timout occurred when running R script = " + rScript);
          } catch (ExecutionException e) {
            logger.error(Constants.STACKTRACE, e);
  ...
  public RSession detach() {
    if (isHealthy()) {
      ...
      try {
        synchronized (rconMonitor) {
          Future<RSession> future =
              executor.submit(
                  new Callable<RSession>() {
                    @Override
  ...
  private void saveImage() throws RserveException {
    ...
    if (!new File(rDataFile).getParentFile().exists())
      throw new IllegalArgumentException(
          "Cannot save workspace image, as the RData file folder is not defined.");
    synchronized (rconMonitor) {
  ...
  private void loadImage() throws RserveException {
    ...
    if (!new File(rDataFile).exists())
      throw new IllegalArgumentException(
          "Cannot load workspace image, as the RData file is not defined.");
    synchronized (rconMonitor) {
