...
public class AWSNativeBlobStorageEngine extends AbstractStorageEngine {
    ...
    try {
      ...
      for (S3Object object : listObjectsV2Response.contents()) {
        ...
        HeadObjectRequest headRequest =
            HeadObjectRequest.builder().bucket(this.bucket).key(object.key()).build();
  ...
  public void syncLocalToStorage(String localPath, String storagePath, Map<String, Object> metadata)
    ...
    try {
      ...
      Path localBasePath =
          Files.isDirectory(localFilePath) ? localFilePath : localFilePath.getParent();
    ...
    classLogger.info(
        uploadedFiles.isEmpty()
            ...
            : "Sucessfully uploaded:" + uploadedFiles);
    classLogger.info(
        found
            ? "Sync complted successfully for:" + storagePath
  ...
  public void syncStorageToLocal(String storagePath, String localPath) throws Exception {
    ...
    for (S3Object s3Object : listObjectsResponse.contents()) {
      ...
      if (Files.isDirectory(localFilePath)) continue;
      ...
      try {
        ...
        if (fileExists) {
          ...
          shouldDownload =
              cloudFileSize != localFileSize || cloudModifiedTime > localModifiedTime.toMillis();
        }
        ...
        if (shouldDownload) {
          ...
          classLogger.info(
              fileExists
                  ? "Updated file: " + localFilePath
                  : "Downloaded new file: " + localFilePath);
        }
        found = true;
      } catch (Exception e) {
        failedFiles.add(key);
    ...
    Files.walk(localDirectory)
        .filter(Files::isRegularFile)
        .filter(localFile -> !cloudFiles.contains(localFile.toString()))
        .forEach(
            localFile -> {
              try {
    ...
    classLogger.info(
        found
            ? "Sync completed successfully for: " + storagePath
            : "No files found to sync for: " + storagePath);
  }

  @Override
  public void copyToStorage(
      String localFilePath, String storageFolderPath, Map<String, Object> metadata)
      throws Exception {
    ...
    classLogger.info(
        uploadedFiles.isEmpty()
            ? "No files were uploaded."
            : "Successfully uploaded files: " + uploadedFiles);
    classLogger.info(
        found
            ? "Copy completed successfully for: " + storageFolderPath
  ...
  public void copyToLocal(String storageFilePath, String localFolderPath) throws Exception {
    ...
    for (String s3FolderPath : paths) {
      ...
      do {
        ...
        for (S3Object s3Object : response.contents()) {
          ...
          if (key.endsWith("/") && s3Object.size() == 0) continue;

          Path localFilePath =
              localDirectory.resolve(key.substring(prefix.length()).replace("/", File.separator));
    ...
    classLogger.info(
        found
            ? "Copy completed successfully for: " + storageFilePath
  ...
  public void deleteFromStorage(String storagePath) throws Exception {
    ...
    if (responseListObjects == null
        || responseListObjects.contents() == null
        || responseListObjects.contents().isEmpty()) {
      classLogger.warn(
          storagePath.isEmpty()
              ? "No files found in bucket: " + this.bucket
    ...
    for (S3Object object : responseListObjects.contents()) {
      ...
      if (storagePath.isEmpty()
          || objectKey.equals(storagePath)
          || objectKey.startsWith(storagePath + "/")) {
    ...
    classLogger.info(
        deletedFiles.isEmpty()
            ...
            : "Successfully deleted files: " + deletedFiles);
  ...
  public void deleteFromStorage(String storagePath, boolean leaveFolderStructure) throws Exception {
    ...
    classLogger.info(
        hasFilesToDelete
            ? "Deletion process completed for: " + storagePath
  ...
  public void deleteFolderFromStorage(String storageFolderPath) throws Exception {
    ...
    classLogger.info(
        storageFolderPath.isEmpty()
            ? "Folder path is empty. Deleting all files in bucket: " + this.bucket
        ...
        retryOperation(
            () -> {
              DeleteObjectRequest deleteRequest =
                  DeleteObjectRequest.builder().bucket(this.bucket).key(objectKey).build();
    ...
    classLogger.info(
        folderExists
            ? "Successfully deleted folder: " + storageFolderPath
            : "No files found in directory: " + storageFolderPath);
  }

  private String uploadFileToS3(
      String storagePath, Path filePath, Path basePath, Map<String, Object> metadata)
      throws Exception {
    ...
    String fileKey =
        normalizedStoragePath + (normalizedStoragePath.endsWith("/") ? "" : "/") + relativePath;

    try {
      HeadObjectRequest headRequest =
          HeadObjectRequest.builder().bucket(this.bucket).key(fileKey).build();
    ...
    retryOperation(
        () -> {
          PutObjectRequest putRequest =
              PutObjectRequest.builder().bucket(this.bucket).key(fileKey).metadata(metaMap).build();
  ...
  private void downloadedFile(String bucketName, String key, Path destinationPath)
      throws IOException {
  ...
  private void preserveFolderStructure(List<String> deletedFiles) {
    ...
    for (String folderPath : folderPaths) {
      PutObjectRequest request =
          PutObjectRequest.builder().bucket(this.bucket).key(folderPath).build();
  ...
  private void deleteEmptyBlobsFromS3(String storagePrefix) {
    ...
    for (S3Object obj : response.contents()) {
      if (obj.size() == 0) {
        DeleteObjectRequest deleteRequest =
            DeleteObjectRequest.builder().bucket(this.bucket).key(obj.key()).build();
        this.client.deleteObject(deleteRequest);
    ...
    for (String fileKey : failedFiles) {
      try {
        retryOperation(
            () -> {
              DeleteObjectRequest deleteRequest =
                  DeleteObjectRequest.builder().bucket(this.bucket).key(fileKey).build();
              this.client.deleteObject(deleteRequest);
  ...
  private String uploadFile(
      Path rootPath, Path file, String storageFolderPath, Map<String, Object> metadata)
      throws IOException {
    ...
    PutObjectRequest.Builder putBuilder =
        PutObjectRequest.builder().bucket(this.bucket).key(fileKey);
  ...
  private ListObjectsV2Response s3ListObjectResponse(String storagePrefix) {
    ListObjectsV2Request listObjects =
        ListObjectsV2Request.builder().bucket(this.bucket).prefix(storagePrefix).build();
