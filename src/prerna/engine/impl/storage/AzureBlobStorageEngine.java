...
public class AzureBlobStorageEngine extends AbstractRCloneStorageEngine {
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    this.generateDynamicSAS =
        Boolean.parseBoolean(smssProp.getProperty(AZ_GENERATE_DYNAMIC_SAS, "false"));
  ...
  public String getDynamicSAS(String containerName) {
    ...
    try {
      ...
      //			if(!container.exists()) {
      //				classLogger.info("Created service access signature for container that does not yet
      // exist");
      //				container.create();
      //			}
      ...
      retString =
          container.getUri()
              + "?"
              + container.generateSharedAccessSignature(getSASConstraints(), null);
    } catch (URISyntaxException use) {
      classLogger.error(Constants.STACKTRACE, use);
  ...
  private SharedAccessBlobPolicy getSASConstraints() {
    ...
    EnumSet<SharedAccessBlobPermissions> permSet =
        EnumSet.noneOf(SharedAccessBlobPermissions.class);
  ...
  public String createRCloneConfig() throws IOException, InterruptedException {
    if (this.generateDynamicSAS) {
      classLogger.warn(
          "Calling creation of rclone without passing in the container name to generate a SAS");
      classLogger.warn(
          "Calling creation of rclone without passing in the container name to generate a SAS");
      classLogger.warn(
          "Calling creation of rclone without passing in the container name to generate a SAS");
  ...
  public String createRCloneConfig(String containerName) throws IOException, InterruptedException {
    ...
    if (this.generateDynamicSAS) {
      ...
      runRcloneProcess(
          rcloneConfig, RCLONE, "config", "create", rcloneConfig, PROVIDER, "sas_url", sasUrl);
    } else if (this.useMsi) {
      runRcloneProcess(
          rcloneConfig, RCLONE, "config", "create", rcloneConfig, PROVIDER, "use_msi", "true");
      ...
      runRcloneProcess(
          rcloneConfig, RCLONE, "config", "create", rcloneConfig, PROVIDER, "env_auth", "true");
    }
  ...
  // engine.listDetails("08e03a5f-9b8d-4f24-a3f7-ba6959f2c5c0/version");
  //			System.out.println(list);
  //		}
  //		engine.close();
  //	}
