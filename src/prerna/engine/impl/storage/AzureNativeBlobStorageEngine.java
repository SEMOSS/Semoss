...
public class AzureNativeBlobStorageEngine extends AbstractStorageEngine {
  private static final Logger classLogger =
      LogManager.getLogger(AzureNativeBlobStorageEngine.class);
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    if (this.connectionString == null || this.connectionString.isEmpty()) {
      classLogger.error(
          "Azure Blob connection string is missing, cannot initialize Azure Blob client.");
          ...
          .forEach(
              file -> {
                try {
                  uploadedFiles.add(
                      uploadFileToBlob(
                          file, localFilePath, containerClient, blobDirectory, metadata));
                } catch (Exception e) {
  ...
  public void syncStorageToLocal(String storagePath, String localPath) throws Exception {
    ...
    for (BlobItem blobItem :
        containerClient.listBlobs(
            new ListBlobsOptions().setPrefix(blobDirectory.isEmpty() ? null : blobDirectory),
            null)) {
      BlobClient blobClient = containerClient.getBlobClient(blobItem.getName());
      ...
      Path localFilePath =
          localDirectory.resolve(
              blobItem.getName().startsWith(blobDirectory)
                  ? blobItem
                      .getName()
                      .substring(blobDirectory.length())
                      .replace("/", File.separator)
                  : blobItem.getName().replace("/", File.separator));
      ...
      try {
        ...
        if (!shouldDownload) {
          ...
          shouldDownload =
              properties.getBlobSize() != localFileSize
                  || properties.getLastModified().toInstant().toEpochMilli()
                      > localModifiedTime.toMillis();
        }
        if (shouldDownload) {
        ...
        failedFiles.add(blobItem.getName());
  ...
  public void copyToStorage(
    ...
    for (Path filePath : paths) {
      ...
      if (Files.isDirectory(filePath)) {
        try (Stream<Path> fileStream = Files.walk(filePath).filter(Files::isRegularFile)) {
          fileStream.forEach(
              file -> {
                try {
                  uploadedFiles.add(
                      uploadFile(containerClient, filePath, file, blobDirectory, metadata));
                } catch (Exception e) {
                  failedFiles.add(file.toString());
                  classLogger.error("Failed to upload file: " + file, e);
                  rollbackUploads(containerClient, failedFiles);
        ...
        try {
          uploadedFiles.add(
              uploadFile(containerClient, filePath.getParent(), filePath, blobDirectory, metadata));
          found = true;
        } catch (Exception e) {
          failedFiles.add(filePath.toString());
  ...
  public void copyToLocal(String storageFilePath, String localFolderPath) throws Exception {
    ...
    for (String path : paths) {
      Iterable<BlobItem> getBlobItems =
          paths.isEmpty()
              ? containerClient.listBlobs()
      ...
      for (BlobItem blobItem : getBlobItems) {
        ...
        Path localFilePath =
            localDirectory.resolve(blobName.substring(path.length()).replace("/", File.separator));
        try {
          Files.createDirectories(localFilePath.getParent());
          retryOperation(
              () -> blobClient.downloadToFile(localFilePath.toString(), true),
  ...
  public void deleteFromStorage(String storagePath) throws Exception {
    ...
    for (BlobItem blobItem : blobItems) {
      ...
      if (blobDirectory.isEmpty()
          || (!blobName.equals(blobDirectory)
              && !blobName.substring(blobDirectory.length()).contains("/"))) {
    ...
    if (!hasFilesToDelete) {
      classLogger.warn(
          blobDirectory.isEmpty()
              ? "No files found in container: " + containerName
  ...
  public void deleteFolderFromStorage(String storageFolderPath) throws Exception {
    ...
    classLogger.info(
        blobDirectory.isEmpty()
            ? "Blob directory is empty. Deleting all files in container: " + containerName
            : "Deleting folder: " + blobDirectory);
  ...
  private String uploadFileToBlob(
      Path file,
      Path basePath,
      BlobContainerClient containerClient,
  ...
  private void preserveFolderStructure(
      BlobContainerClient containerClient, List<String> deletedFiles) {
  ...
  private void syncStorageDeletion(
      BlobContainerClient containerClient, String blobDirectory, Path localBasePath) {
    ...
    for (BlobItem blobItem : blobItems) {
      ...
      Path localFilePath =
          localBasePath.resolve(
              blobName.replaceFirst(blobDirectory, "").replace("/", File.separator));
