...
public class iGraphUtilities {
	 ...
	 * @param logger
	 */
	public static void synchronizeGraphToR(TinkerFrame graph, AbstractRJavaTranslator rJavaTranslator, String graphName,
			String wd, Logger logger) {
		try {
	 ...
	 * @param nodeList
	 */
	public static void removeNodeFromR(String graphName, AbstractRJavaTranslator rJavaTranslator, String type,
			List<Object> nodeList, Logger logger) {
		for (int nodeIndex = 0; nodeIndex < nodeList.size(); nodeIndex++) {
			...
			try {
				...
				// implementation
				rJavaTranslator.executeEmptyR(graphName + " <- delete_vertices(" + graphName + ", V(" + graphName
						+ ")[vertex_attr(" + graphName + ", \"" + TinkerFrame.TINKER_ID + "\") == \"" + name + "\"])");
			} catch (Exception ex) {
				logger.error(Constants.STACKTRACE, ex);
