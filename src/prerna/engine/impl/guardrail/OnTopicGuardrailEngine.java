 ...
 * pre-loaded with example on-topic prompts/documents. A prompt passes if its
...
public class OnTopicGuardrailEngine extends AbstractGuardrailReactorFunctionEngine {
	...
	/** Key name used by all vector DB backends for the similarity score. */
	...
	public void open(Properties smssProp) throws Exception {
		...
		this.functionDescription = "Checks whether the prompt is on-topic by comparing it against a vector database "
				+ "of known on-topic examples. Rejects the prompt if its best similarity score is below the threshold.";
		...
		this.parameters.add(new FunctionParameter("threshold", "Double",
				"Minimum similarity score (0–1 for cosine similarity) for the prompt to be considered on-topic. "
						+ "Default is " + this.defaultThreshold));
	...
	public GuardrailNounMetadata execute(NounStore ns, GenRowStruct curRow) {
		...
		Insight queryInsight = new Insight();
		...
		boolean pass = bestScore >= threshold;
