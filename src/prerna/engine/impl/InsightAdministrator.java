...
public class InsightAdministrator {
	...
	private static final Logger classLogger = LogManager.getLogger(InsightAdministrator.class);
	...
	public String addInsight(String insightName, String layout, Collection<String> pixelRecipeToSave, boolean global,
			...
			String schemaName) {
		return addInsight(insightName, layout, pixelRecipeToSave.toArray(new String[] {}), global, cacheable,
				cacheMinutes, cacheCron, cachedOn, cacheEncrypt, schemaName);
	}

	public String addInsight(final String insightId, String insightName, String layout,
			Collection<String> pixelRecipeToSave, boolean global, boolean cacheable, int cacheMinutes, String cacheCron,
			ZonedDateTime cachedOn, boolean cacheEncrypt, String schemaName) {
		return addInsight(insightId, insightName, layout, pixelRecipeToSave.toArray(new String[] {}), global, cacheable,
				cacheMinutes, cacheCron, cachedOn, cacheEncrypt, schemaName);
	}
	...
	public String addInsight(String insightName, String layout, String[] pixelRecipeToSave, boolean global,
		...
		return addInsight(newId, insightName, layout, pixelRecipeToSave, global, cacheable, cacheMinutes, cacheCron,
	...
	public String addInsight(String insightId, String insightName, String layout, String[] pixelRecipeToSave,
			boolean global, boolean cacheable, int cacheMinutes, String cacheCron, ZonedDateTime cachedOn,
			boolean cacheEncrypt, String schemaName) {
		classLogger.info("Adding new question with insight id :::: " + Utility.cleanLogString(insightId));
		classLogger.info("Adding new question with name :::: " + Utility.cleanLogString(insightName));
		classLogger.info("Adding new question with layout :::: " + Utility.cleanLogString(layout));
		classLogger.info(
				"Adding new question with recipe :::: " + Utility.cleanLogString(Arrays.toString(pixelRecipeToSave)));
	...
	public String batchInsight(PreparedStatement ps, String insightId, String insightName, String layout,
			List<String> pixelRecipeToSave, boolean global, boolean cacheable, int cacheMinutes, String cacheCron,
			LocalDateTime cachedOn, boolean cacheEncrypt, String schemaName) {
		return batchInsight(ps, insightId, insightName, layout, pixelRecipeToSave.toArray(new String[] {}), global,
	 ...
	 */
	public String batchInsight(PreparedStatement ps, String insightId, String insightName, String layout,
			String[] pixelRecipeToSave, boolean global, boolean cacheable, int cacheMinutes, String cacheCron,
			LocalDateTime cachedOn, boolean cacheEncrypt, String schemaName) {
		classLogger.info("Adding new question with insight id :::: " + Utility.cleanLogString(insightId));
		classLogger.info("Adding new question with name :::: " + Utility.cleanLogString(insightName));
		classLogger.info("Adding new question with layout :::: " + Utility.cleanLogString(layout));
	...
	public void updateInsightDescription(String insightId, String description) {
		...
		PreparedStatement ps = null;
		try {
			ps = this.insightEngine
					.getPreparedStatement("UPDATE INSIGHTMETA SET METAVALUE=? WHERE METAKEY=? AND INSIGHTID=?");
			...
			if (ps.getUpdateCount() == 0) {
				...
				insertStatement = insightEngine.getPreparedStatement(
	...
	public void updateInsight(String existingRdbmsId, String insightName, String layout, String[] pixelRecipeToSave,
			boolean global, boolean cacheable, int cacheMinutes, String cacheCron, ZonedDateTime cachedOn,
			boolean cacheEncrypt, String schemaName) {
		classLogger.info("Modifying insight id :::: " + Utility.cleanLogString(existingRdbmsId));
		classLogger.info("Adding new question with name :::: " + Utility.cleanLogString(insightName));
		classLogger.info("Adding new question with layout :::: " + Utility.cleanLogString(layout));
		classLogger.info(
				"Adding new question with recipe :::: " + Utility.cleanLogString(Arrays.toString(pixelRecipeToSave)));

		String query = "UPDATE " + TABLE_NAME + " SET " + QUESTION_NAME_COL + "=?, " + QUESTION_LAYOUT_COL + "=?, "
				+ HIDDEN_INSIGHT_COL + "=?, " + CACHEABLE_COL + "=?, " + CACHE_MINUTES_COL + "=?, " + CACHE_CRON_COL
				+ "=?, " + CACHED_ON_COL + "=?, " + CACHE_ENCRYPT_COL + "=?, " + QUESTION_PKQL_COL + "=?, "
				+ SCHEMA_NAME_COL + "=? WHERE " + QUESTION_ID_COL + "=?";
	...
	public void updateInsight(String existingRdbmsId, String insightName, String layout,
			Collection<String> pixelRecipeToSave, boolean global, boolean cacheable, int cacheMinutes, String cacheCron,
			ZonedDateTime cachedOn, boolean cacheEncrypt, String schemaName) {
		updateInsight(existingRdbmsId, insightName, layout, pixelRecipeToSave.toArray(new String[] {}), global,
				cacheable, cacheMinutes, cacheCron, cachedOn, cacheEncrypt, schemaName);
	}

	public void updateInsightName(String existingRdbmsId, String insightName) {
		classLogger.info("Modifying insight id :::: " + existingRdbmsId);
		classLogger.info("Updating question name to :::: " + insightName);

		String query = "UPDATE " + TABLE_NAME + " SET " + QUESTION_NAME_COL + "=? WHERE " + QUESTION_ID_COL + "=?";
	...
	public void updateInsightSchemaName(String existingRdbmsId, String schemaName) {
		classLogger.info("Modifying insight id :::: " + existingRdbmsId);
		classLogger.info("Updating insight schema name to :::: " + schemaName);

		String query = "UPDATE " + TABLE_NAME + " SET " + SCHEMA_NAME_COL + "=? WHERE " + QUESTION_ID_COL + "=?";
	...
	public void updateInsightCache(String existingRdbmsId, boolean cacheable, int cacheMinutes, String cacheCron,
			LocalDateTime cachedOn, boolean cacheEncrypt) {
		classLogger.info("Modifying insight id :::: " + existingRdbmsId);
		classLogger.info("Updating question cache :::: " + cacheable);
		classLogger.info("Updating question cache minutes :::: " + cacheMinutes);
		classLogger.info("Updating question cache encrypt :::: " + cacheEncrypt);

		String query = "UPDATE " + TABLE_NAME + " SET " + CACHEABLE_COL + "=?, " + CACHE_MINUTES_COL + "=?, "
				+ CACHE_CRON_COL + "=?, " + CACHED_ON_COL + "=?, " + CACHE_ENCRYPT_COL + "=? WHERE " + QUESTION_ID_COL
				+ "=?";
	...
	public void updateInsightCachedOn(String existingRdbmsId, ZonedDateTime cachedOn) {
		classLogger.info("Modifying insight id :::: " + Utility.cleanLogString(existingRdbmsId));
		classLogger.info("Updating question cache date :::: " + cachedOn);

		String query = "UPDATE " + TABLE_NAME + " SET " + CACHED_ON_COL + "=? WHERE " + QUESTION_ID_COL + "=?";
	...
	public void updateInsightGlobal(String existingRdbmsId, boolean isGlobal) {
		...
		String query = "UPDATE " + TABLE_NAME + " SET " + HIDDEN_INSIGHT_COL + "=? WHERE " + QUESTION_ID_COL + "=?";
	...
	public void batchInsightMetadata(PreparedStatement insertPs, String insightId, String metaKey, String metaValue,
			int order) {
		try {
	...
	public void dropInsight(String... insightIDs) {
		...
		classLogger.info("Running drop query :::: " + Utility.cleanLogString(deleteQuery));
		PreparedStatement ps = null;
		try {
	...
	public PreparedStatement getAddInsightPreparedStatement() {
		PreparedStatement ps = null;
		try {
			ps = insightEngine.bulkInsertPreparedStatement(new String[] { TABLE_NAME, QUESTION_ID_COL,
					QUESTION_NAME_COL, QUESTION_LAYOUT_COL, HIDDEN_INSIGHT_COL, CACHEABLE_COL, CACHE_MINUTES_COL,
					CACHE_CRON_COL, CACHED_ON_COL, CACHE_ENCRYPT_COL, QUESTION_PKQL_COL, SCHEMA_NAME_COL });
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
			throw new IllegalArgumentException(
					"Error occurred generating the prepared statement to insert the insight");
		}
	...
	public PreparedStatement getAddInsightMetaPreparedStatement() {
		PreparedStatement ps = null;
		try {
			ps = insightEngine.bulkInsertPreparedStatement(
