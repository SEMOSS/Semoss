...
public class InMemorySesameEngine extends AbstractDatabaseEngine implements IRDFDatabase {
  ...
  /**
   * Processes the passed ASK SPARQL query against the engine. The query must be in the structure of
   * an ASK query and the result will be a boolean indicating whether or not the data store
   * connected to the engine has triples matching the pattern of the ASK query.
   *
   * @param query the ASK SPARQL query to be run against the engine
   * @return true if the data store connected to the engine contains triples that match the pattern
   *     of the query and false if it does not.
   */
  //	@Override
