 ...
 * wrapper processes CONSTRUCT statements.
 */
@Deprecated
public class SesameJenaConstructWrapper extends AbstractWrapper {
        ...
        try {
          ...
          if (!myObject.toString().equalsIgnoreCase("null")) {
            org.apache.jena.rdf.model.Statement stmt =
                (org.apache.jena.rdf.model.Statement) myObject;
  ...
  //		SesameJenaConstructWrapper sjcw = new SesameJenaConstructWrapper();
  // //(SesameJenaSelectWrapper) engine.execSelectQuery("SELECT ?S ?P ?O WHERE {{?S ?P ?O}.} LIMIT
  // 1");
  //		sjcw.setEngine(engine);
  //		sjcw.setEngineType(engine.getEngineType());
  //		sjcw.setQuery("CONSTRUCT {?subject ?predicate ?object} WHERE {{?subject ?predicate
  // ?object.}}");
  //
  //		sjcw.execute();
  //
