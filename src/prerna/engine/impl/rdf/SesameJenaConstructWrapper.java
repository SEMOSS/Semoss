...
public class SesameJenaConstructWrapper extends AbstractWrapper {
	 ...
	 *
	 * @param databaseType
	 *            Enum - The type engine that this is being set to.
	 */
	...
	// //(SesameJenaSelectWrapper) engine.execSelectQuery("SELECT ?S ?P ?O WHERE
	// {{?S ?P ?O}.} LIMIT
	// 1");
	// sjcw.setEngine(engine);
	// sjcw.setEngineType(engine.getEngineType());
	// sjcw.setQuery("CONSTRUCT {?subject ?predicate ?object} WHERE {{?subject
	// ?predicate
	// ?object.}}");
	//
	// sjcw.execute();
