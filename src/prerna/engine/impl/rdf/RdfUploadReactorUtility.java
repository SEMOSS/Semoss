...
public class RdfUploadReactorUtility {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(RdfUploadReactorUtility.class.getName());
  ...
  public static void loadMetadataIntoEngine(IRDFDatabase engine, WriteOWLEngine owlEngine) {
    ...
    for (String relation : hash.keySet()) {
      allStatementInserts.add(
          new Object[] {hash.get(relation), RDFS.SUBPROPERTYOF + "", object, true});
    }
  ...
  public static void createRelationship(
    ...
    allStatementInserts.add(
        new Object[] {
          instanceRelURI,
          RDFS.LABEL,
          instanceSubjectName + Constants.RELATION_URI_CONCATENATOR + instanceObjectName,
          false
        });
   ...
   * @param allStatementInserts
   */
  private static void insertCurrentDate(
      String propInstanceURI,
      String basePropURI,
      String subjectNodeURI,
      List<Object[]> allStatementInserts) {
   ...
   * @param allStatementInserts
   */
  private static void insertCurrentUser(
      String propURI,
      String basePropURI,
      String subjectNodeURI,
      List<Object[]> allStatementInserts) {
  ...
  public static void deleteAllTriples(IDatabaseEngine engine) {
    ...
    classLogger.info(
        "Starting to delete all triples from database "
    ...
    classLogger.info(
        "Done deleting all triples from database "
            + engine.getEngineName()
            + "_"
            + engine.getEngineId());
