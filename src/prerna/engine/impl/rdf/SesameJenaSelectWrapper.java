...
/**
 * The wrapper helps takes care of selection of the type of engine you are using (Jena/Sesame). This
 * wrapper processes SELECT statements.
 */
@Deprecated
public class SesameJenaSelectWrapper extends AbstractWrapper {
  ...
  private SesameJenaSelectStatement getSJSSfromBinding(BindingSet bs) {
    ...
    for (int colIndex = 0; colIndex < var.length; colIndex++) {
        ...
        if (val != null && val instanceof Literal) {
          if (QueryEvaluationUtil.isStringLiteral((Value) val)) {
            ...
            dateStr =
                (val.toString())
                    .substring(
                        (val.toString()).indexOf("\"") + 1, (val.toString()).lastIndexOf("\""));
          } else {
      ...
      else if (weightVal != null) sjss.setVar(var[colIndex], weightVal);
      else if (dateStr != null) sjss.setVar(var[colIndex], dateStr);
      else if (stringVal != null) sjss.setVar(var[colIndex], stringVal);
      else if (val == null) {
  ...
  /**
   * Method BVnext. Returns full URIs instead of just the instances of the select statement. This
   * contains a check so that property values are only sent once.
   *
    ...
    try {
      if (databaseType == IDatabaseEngine.DATABASE_TYPE.SESAME) {
        ...
        for (int colIndex = 0; colIndex < var.length; colIndex++) {
          ...
          String value = bs.getValue(var[colIndex]) + "";
          if (weightVal == null && val != null) retSt.setVar(var[colIndex], value);
          else if (weightVal != null) retSt.setVar(var[colIndex], weightVal);
          logger.debug("Binding Name " + var[colIndex]);
          logger.debug("Binding Value " + value);
        }
        ...
        String output =
            Utility.retrieveResult(remoteWrapperProxy.getRemoteAPI() + "/bvnext", params);
        Gson gson = new Gson();
        retSt =
            gson.fromJson(
                output,
                SesameJenaSelectStatement.class); // cleans up automatically at the remote end
      }
    } catch (RuntimeException ex) {
      logger.error(Constants.STACKTRACE, ex);
    } catch (QueryEvaluationException e) {
  ...
  //		SesameJenaSelectWrapper sjcw = new SesameJenaSelectWrapper(); //(SesameJenaSelectWrapper)
  // engine.execSelectQuery("SELECT ?S ?P ?O WHERE {{?S ?P ?O}.} LIMIT 1");
