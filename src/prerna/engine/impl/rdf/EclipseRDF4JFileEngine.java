...
/**
 * References the RDF source and uses the Eclipse RDF4J API to query a database stored in an RDF
 * file
 */
public class EclipseRDF4JFileEngine extends AbstractDatabaseEngine implements IRDFDatabase {
  ...
  private void loadFile() throws IOException, RDFParseException, RepositoryException {
    if (this.filePath != null) {
      ...
      if (!(file.exists() && file.isFile())) {
        classLogger.warn(
            "Calling open for RDFFileSesameEngine with file path set but file does not exist");
      } else {
  ...
  //		classLogger.info("Exporting database " + SmssUtilities.getUniqueName(this.engineName,
  // this.engineId));
  ...
  public void commit() {
    try {
      rc.commit();
      classLogger.info(
          "Exporting database " + SmssUtilities.getUniqueName(this.engineName, this.engineId));
      //			RDFXMLWriter rdfWriter = null;
      //			try (OutputStreamWriter writer = new OutputStreamWriter(
      //					new FileOutputStream(Utility.normalizePath(filePath)), StandardCharsets.UTF_8)){
      //				rdfWriter = new RDFXMLWriter(writer);
