...
/**
 * References the RDF source and uses the Eclipse RDF4J API to query a database
 * stored in an RDF file
 */
public class EclipseRDF4JFileEngine extends AbstractDatabaseEngine implements IRDFDatabase {
	 ...
	 *            engine should be instantiated. Dependent on what type of engine is
	 *            being instantiated.
	 * @throws Exception
	 */
	@Override
	public void open(Properties smssProp) throws Exception {
		...
		String dataDir = EngineUtility.getSpecificEngineBaseFolder(this.getCatalogType(), this.getEngineId(),
				this.getEngineName());
		dataDir += "/data";
		String indexes = "spoc,psoc,ospc";
		this.myRepository = new SailRepository(
				new SchemaCachingRDFSInferencer(new NativeStore(new File(dataDir), indexes)));
		...
		if (this.filePath != null) {
			...
			if (!(file.exists() && file.isFile())) {
				...
				if (rdfFileType.equalsIgnoreCase("RDF/XML"))
					...
					throw new IOException("Unable to load RDF file type = " + rdfFileType);
			}
		}
	}
	 ...
	 *
	 * @param type
	 *            The full URI of the node type that we want to get the instances of
	...
	/**
	 * Handles the transaction for a single triple to be added/removed from the
	 * dataset
	 *
	 * @param args
	 *            array contains the following subject String - RDF Subject
	 *            predicate String - RDF Predicate object Object - RDF Object
	 *            concept boolean - True if the statement is a concept (URI), False
	 *            if it is a property (Literal)
	 * @param add
	 *            if we are adding or removing the triple
	 */
	...
	// SmssUtilities.getUniqueName(this.engineName,
	// this.engineId));
	// RDFXMLWriter rdfWriter = null;
	// try (OutputStreamWriter writer = new OutputStreamWriter(
	// new FileOutputStream(Utility.normalizePath(filePath)),
