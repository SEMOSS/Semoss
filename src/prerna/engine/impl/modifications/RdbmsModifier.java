...
public class RdbmsModifier implements IEngineModifier {
  ...
  public void addProperty(String tableName, String newColumn, String dataType) throws Exception {
    if (queryUtil.allowAddColumn()) {
      ...
      // we need to drop the existing table (we will rename it in case the last operation fails and
      // only drop at end)
  ...
  @Override
  public void editProperty(String tableName, String newColumn, String newDataType)
      throws Exception {
    if (queryUtil.allowAddColumn()) {
  ...
  @Override
  public void addIndex(String tableName, String columnName, String indexName, boolean addIfExists)
      throws Exception {
    ...
    if (addIfExists) {
      ...
      String existingIndicesQuery =
          queryUtil.allIndexForTableQuery(tableName, this.database, this.schmea);
      IRawSelectWrapper wrapper =
          WrapperManager.getInstance().getRawWrapper(engine, existingIndicesQuery);
      while (wrapper.hasNext()) {
        ...
        if (values[1].toString().equalsIgnoreCase(tableName)) {
          throw new IllegalArgumentException(
              "Index already exists on the column with the name '" + values[1] + "'");
        }
      }
  ...
  @Override
  public void renameProperty(String existingConcept, String existingColumn, String newColumn)
      throws Exception {
