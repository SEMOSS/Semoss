...
public class PineConeVectorDatabaseEngine extends AbstractVectorDatabaseEngine {
  ...
  private static final Logger classLogger =
      LoggerFactory.getLogger(PineConeVectorDatabaseEngine.class);
  ...
  public void removeDocument(List<String> fileNames, Map<String, Object> parameters)
    ...
    for (int fileIndex = 0; fileIndex < sourceNames.size(); fileIndex++) {
      ...
      while (firstExecution || paginationToken != null) {
        ...
        String idListResponse =
            HttpHelperUtility.getRequest(listVectorsUrl, headersMap, null, null, null);
        Map<String, Object> responseMap =
            gson.fromJson(idListResponse, new TypeToken<Map<String, Object>>() {}.getType());
  ...
  private void executeDelete(
      String url, Map<String, String> headersMap, List<Map<String, String>> listReturnVector) {
    ...
    HttpHelperUtility.postRequestStringBody(
        url, headersMap, deleteJson.toString(), ContentType.APPLICATION_JSON, null, null, null);
  }
  ...
  public List<Map<String, Object>> nearestNeighborCall(
    ...
    String nearestNeigborResponse =
        HttpHelperUtility.postRequestStringBody(
            url, headersMap, queryJson.toString(), ContentType.APPLICATION_JSON, null, null, null);
    ...
    for (int i = 0; i < matches.size(); i++) {
      ...
      resultMap.put(
          VectorDatabaseCSVTable.CONTENT, metadataMap.get(VectorDatabaseCSVTable.CONTENT));
      resultMap.put(
          VectorDatabaseCSVTable.DIVIDER, metadataMap.get(VectorDatabaseCSVTable.DIVIDER));
      resultMap.put(
          VectorDatabaseCSVTable.MODALITY, metadataMap.get(VectorDatabaseCSVTable.MODALITY));
  ...
  public List<Map<String, Object>> listDocuments(Map<String, Object> parameters) {
    /**
     * Pinecone's API is not useful https://docs.pinecone.io/reference/api/2025-01/data-plane/list
     *
     * <p>dont have a way to fetch results w/o first listing the index's and after listing the
     * index's the fetch cant limit the return so will also get back the vectors
     */
    String indexClass = this.defaultIndexClass;
  ...
  public List<Map<String, Object>> listAllRecords(Map<String, Object> parameters) {
     ...
     * Pinecone's API is not useful https://docs.pinecone.io/reference/api/2025-01/data-plane/list
     *
     * <p>don't have a way to fetch results w/o first listing the index's and after listing the
     * index's the fetch can't limit the return so will also get back the vectors
     */
    Gson gson = new GsonBuilder().create();
  ...
  private List<Map<String, Object>> fetchAllValues(StringBuilder ids) {
    ...
    for (String uid : vectorsJson.keySet()) {
      ...
      recordMap.put(
          VectorDatabaseCSVTable.SOURCE, metadata.get(VectorDatabaseCSVTable.SOURCE).getAsString());
      recordMap.put(
          VectorDatabaseCSVTable.MODALITY,
          metadata.get(VectorDatabaseCSVTable.MODALITY).getAsString());
      recordMap.put(
          VectorDatabaseCSVTable.DIVIDER,
          metadata.get(VectorDatabaseCSVTable.DIVIDER).getAsString());
      recordMap.put(
          VectorDatabaseCSVTable.PART, metadata.get(VectorDatabaseCSVTable.PART).getAsString());
      recordMap.put(
          VectorDatabaseCSVTable.TOKENS, metadata.get(VectorDatabaseCSVTable.TOKENS).getAsInt());
      recordMap.put(
          VectorDatabaseCSVTable.CONTENT,
          metadata.get(VectorDatabaseCSVTable.CONTENT).getAsString());
