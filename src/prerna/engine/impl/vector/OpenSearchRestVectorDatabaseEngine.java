...
public class OpenSearchRestVectorDatabaseEngine extends AbstractVectorDatabaseEngine {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(OpenSearchRestVectorDatabaseEngine.class);
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    externallyManagedIndex =
        Boolean.parseBoolean(
            StringUtils.trimToNull(this.smssProp.getProperty(EXTERNALLY_MANAGED_INDEX)));
    ...
    if (queryLimitInput != null && !(queryLimitInput = queryLimitInput.trim()).isEmpty()) {
      try {
        ...
        classLogger.warn(
            "Invalid string value for query_limit value '"
                + queryLimitInput
                + "'. Must be an integer value");
        classLogger.error(Constants.STACKTRACE, e);
      }
    ...
    if (batchLimitInput != null && !(batchLimitInput = batchLimitInput.trim()).isEmpty()) {
      try {
        ...
        classLogger.warn(
            "Invalid string value for batch_limit value '"
                + queryLimitInput
                + "'. Must be an integer value");
        classLogger.error(Constants.STACKTRACE, e);
      }
    ...
    listDocumentsQuery =
        (String)
            ObjectUtils.firstNonNull(
                StringUtils.trimToNull(this.smssProp.getProperty(LIST_DOCUMENTS_QUERY)),
                DEFAULT_LIST_DOCUMENTS_QUERY);
    listDocumentsResultsPath =
        (String)
            ObjectUtils.firstNonNull(
  ...
  public List<FileEmbeddingStatus> addEmbeddings(
    ...
    String response =
        HttpHelperUtility.postRequestStringBody(
            url, headersMap, bulkRequest, ContentType.APPLICATION_JSON, null, null, null);
    ...
    classLogger.info(
        "Inserted "
            + insertions.intValue()
            + " bulk inserts (create index + record value) into open search index "
            + this.indexName);
    ...
    if (errors) {
      classLogger.warn(
          "There were errors with some of the bulk insertions in the open search index "
              + this.indexName);
    } else {
      classLogger.info(
          "All records inserted successfully into OpenSearchRest index '{}'", this.indexName);
    }
  ...
  public void removeDocument(List<String> fileNames, Map<String, Object> parameters)
    ...
    if (externallyManagedIndex) {
      throw new NotImplementedException(
          "Document management features are disabled for this instance");
    }
  ...
  public List<Map<String, Object>> nearestNeighborCall(
    ...
    if (nearestNeighborQuery.contains("${VECTOR}")) {
      ...
      vectorString =
          gson.toJson(convertListNumToJsonArray(embeddingsResponse.getResponse().get(0)));
    }
  ...
  public List<Map<String, Object>> listDocuments(Map<String, Object> parameters) {
    ...
    do {
      ...
      List<IQueryFilter> pageFilters = new ArrayList<>(filters);
      if (searchAfter != null) {
        IQueryFilter sourceFilter =
            SimpleQueryFilter.makeColToValFilter(
                VectorDatabaseCSVTable.SOURCE, ">", "${SOURCE_OFFSET}");
    ...
    pageProcessing:
    {
      do {
        ...
        for (Object o : pageResult) {
          ...
          // if we can form a chunk_offset-specific search_after then we want to add all results
      ...
      JsonObject filterBool = new JsonObject();
      {
        ...
        for (IQueryFilter queryFilter : filters) {
          RestVectorQueryFilterTranslationHelper.processFilter(
              queryFilter, filter, should, must_not);
        }
  ...
  protected String getSearchResponse(String searchBody) {
    ...
    headersMap.put(HttpHeaders.AUTHORIZATION, "Basic " + getCredsBase64Encoded());
    headersMap.put(HttpHeaders.CONTENT_TYPE, "application/json");
    return HttpHelperUtility.postRequestStringBody(
        url, headersMap, searchBody, ContentType.APPLICATION_JSON, null, null, null);
  }
