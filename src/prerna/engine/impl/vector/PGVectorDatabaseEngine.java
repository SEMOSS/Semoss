...
public class PGVectorDatabaseEngine extends RDBMSNativeEngine implements IVectorDatabaseEngine {
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    if (this.vectorTableName == null
        || (this.vectorTableName = this.vectorTableName.trim()).isEmpty()) {
    ...
    if (this.smssProp.containsKey(Constants.DEFAULT_CHUNK_UNIT)) {
      this.defaultChunkUnit =
          this.smssProp.getProperty(Constants.DEFAULT_CHUNK_UNIT).toLowerCase().trim();
      if (!this.defaultChunkUnit.equals("tokens") && !this.defaultChunkUnit.equals("characters")) {
        throw new IllegalArgumentException(
            "DEFAULT_CHUNK_UNIT should be either 'tokens' or 'characters'");
      }
    }

    //		this.defaultExtractionMethod = this.smssProp.getProperty(Constants.EXTRACTION_METHOD,
    // "None");
    ...
    if (this.smssProp.containsKey(Constants.CUSTOM_DOCUMENT_PROCESSOR)) {
      this.customDocumentProcessor =
          Boolean.parseBoolean(this.smssProp.getProperty(Constants.CUSTOM_DOCUMENT_PROCESSOR));
    }
    if (this.smssProp.containsKey(Constants.CUSTOM_DOCUMENT_PROCESSOR_FUNCTION_ID)) {
      this.customDocumentProcessorFunctionID =
          this.smssProp.getProperty(Constants.CUSTOM_DOCUMENT_PROCESSOR_FUNCTION_ID);
    }
    ...
    // second layer - This holds all the different "tables". The reason we want this is to easily
    // and quickly grab the sub folders
  ...
  protected void verifyModelProps() {
    ...
    // This is to get the Model Name and Max Token for an encoder -- we need this to verify chunks
    // aren't getting truncated
    this.embedderEngineId = this.smssProp.getProperty(Constants.EMBEDDER_ENGINE_ID);
    if (this.embedderEngineId == null
        || (this.embedderEngineId = this.embedderEngineId.trim()).isEmpty()) {
      ...
      if (this.embedderEngineId == null
          || (this.embedderEngineId = this.embedderEngineId.trim()).isEmpty()) {
        throw new IllegalArgumentException(
            "Must define the embedder engine id for this vector database using "
    ...
    if (modelProperties.isEmpty() || !modelProperties.containsKey(Constants.MODEL)) {
      throw new IllegalArgumentException(
          "Embedder engine exists but does not contain key " + Constants.MODEL);
    }

    this.smssProp.put(Constants.MODEL, modelProperties.getProperty(Constants.MODEL));
    this.smssProp.put(
        IModelEngine.MODEL_TYPE, modelProperties.getProperty(IModelEngine.MODEL_TYPE));
  ...
  public List<FileEmbeddingStatus> addEmbeddings(
      List<String> vectorCsvFiles, Insight insight, Map<String, Object> parameters)
      throws Exception {
    List<FileEmbeddingStatus> fileStatusList = new ArrayList<>();
    for (String vectorCsvFile : vectorCsvFiles) {
      VectorDatabaseCSVTable vectorCsvTable =
          VectorDatabaseCSVTable.initCSVTable(new File(vectorCsvFile));
  ...
  @Override
  public List<FileEmbeddingStatus> addEmbeddings(
      String vectorCsvFile, Insight insight, Map<String, Object> parameters) throws Exception {
    VectorDatabaseCSVTable vectorCsvTable =
        VectorDatabaseCSVTable.initCSVTable(new File(vectorCsvFile));
  ...
  @Override
  public List<FileEmbeddingStatus> addEmbeddingFiles(
      List<File> vectorCsvFiles, Insight insight, Map<String, Object> parameters) throws Exception {
    List<FileEmbeddingStatus> fileStatusList = new ArrayList<>();
  ...
  @Override
  public List<FileEmbeddingStatus> addEmbeddingFile(
      File vectorCsvFile, Insight insight, Map<String, Object> parameters) throws Exception {
    VectorDatabaseCSVTable vectorCsvTable = VectorDatabaseCSVTable.initCSVTable(vectorCsvFile);
    return addEmbeddings(vectorCsvTable, insight, parameters);
  }
  ...
  public List<FileEmbeddingStatus> addEmbeddings(
    ...
    if (parameters != null && parameters.containsKey(AbstractVectorDatabaseEngine.METADATA)) {
      Map<String, Map<String, Object>> metadata =
          (Map<String, Map<String, Object>>) parameters.get(AbstractVectorDatabaseEngine.METADATA);
      if (!metadata.isEmpty()) {
        String tempMetadataFile =
            insight.getInsightFolder() + "/metadata" + Utility.getRandomString(6) + ".csv";
        VectorDatabaseMetadataCSVWriter writer =
            new VectorDatabaseMetadataCSVWriter(tempMetadataFile);
   ...
   */
  private void updateInsertCounts(
      int[] results, VectorDatabaseCSVTable table, Map<String, Integer> fileInsertedCountMap) {
    ...
    try {
      ...
      for (VectorDatabaseMetadataCSVRow row : metadataTable.getRows()) {
        ...
        if (row.getDateValue() == null) {
          ...
          ps.setDate(
              index++, java.sql.Date.valueOf(row.getDateValue().getZonedDateTime().toLocalDate()));
        }
        if (row.getTimestampValue() == null) {
          ps.setNull(index++, java.sql.Types.TIMESTAMP);
        } else {
          ps.setTimestamp(
              index++, java.sql.Timestamp.from(row.getDateValue().getZonedDateTime().toInstant()));
        }
  ...
  public List<Map<String, Object>> nearestNeighborCall(
    ...
    if (parameters.containsKey(AbstractVectorDatabaseEngine.FILTERS_KEY)) {
      filters =
          PGVectorQueryFitlerTranslationHelper.convertFilters(
    ...
    if (parameters.containsKey(AbstractVectorDatabaseEngine.METADATA_FILTERS_KEY)) {
      metaFilters =
          PGVectorQueryMetaFitlerTranslationHelper.convertFilters(
              (List<IQueryFilter>)
                  parameters.get(AbstractVectorDatabaseEngine.METADATA_FILTERS_KEY),
              this.vectorTableMetadataName);
    }
    ...
    EmbeddingsModelEngineResponse embeddingsResponse =
        engine.embeddings(Arrays.asList(new String[] {searchStatement}), insight, null);
    ...
    qs.addSelector(
        new QueryColumnSelector(
            tablePrefix + VectorDatabaseCSVTable.SOURCE, VectorDatabaseCSVTable.SOURCE));
    qs.addSelector(
        new QueryColumnSelector(
            tablePrefix + VectorDatabaseCSVTable.MODALITY, VectorDatabaseCSVTable.MODALITY));
    qs.addSelector(
        new QueryColumnSelector(
            tablePrefix + VectorDatabaseCSVTable.DIVIDER, VectorDatabaseCSVTable.DIVIDER));
    qs.addSelector(
        new QueryColumnSelector(
            tablePrefix + VectorDatabaseCSVTable.PART, VectorDatabaseCSVTable.PART));
    qs.addSelector(
        new QueryColumnSelector(
            tablePrefix + VectorDatabaseCSVTable.TOKENS, VectorDatabaseCSVTable.TOKENS));
    qs.addSelector(
        new QueryColumnSelector(
            tablePrefix + VectorDatabaseCSVTable.CONTENT, VectorDatabaseCSVTable.CONTENT));
    ...
    if ("Cosine Similarity".equalsIgnoreCase(distanceMethod)) {
      ...
      // The POWER function is used to square the distance to avoid the computational cost of square
      // roots
      // This also ensures all distance values are non-negative, which is important for optimization
      qs.addSelector(
          new QueryOpaqueSelector(
              "POWER((EMBEDDING <-> '" + embeddingsResponse.getResponse().get(0) + "'),2)",
              "Score"));
  ...
  public List<Map<String, Object>> listDocuments(Map<String, Object> parameters) {
    ...
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(
        new QueryColumnSelector(tablePrefix + VectorDatabaseCSVTable.SOURCE, "fileName"));
  ...
  /**
   * This method is meant to be overriden so that we dont need to copy/paste the startServer code
  ...
  private synchronized void startServer(int port) {
    ...
    if (port < 0) {
      ...
      if (forcePort != null && !(forcePort = forcePort.trim()).isEmpty()) {
        try {
          ...
          // ignore
          classLogger.warn(
              "Vector Database " + this.engineName + " has an invalid FORCE_PORT value");
        }
      }
    }
    ...
    try {
      ...
      if (cpwToInit != null) {
        classLogger.warn(
            "Able to start the python process for the vector database "
                + SmssUtilities.getUniqueName(this.engineName, this.engineId)
  ...
  @Override
  public List<FileEmbeddingStatus> addDocument(
      List<String> filePaths, Map<String, Object> parameters) throws Exception {
    List<FileEmbeddingStatus> fileStatusList = new ArrayList<>();
    if (!modelPropsLoaded) {
      verifyModelProps();
    }
    ...
    if (parameters.containsKey(VectorDatabaseParamOptionsEnum.CONTENT_LENGTH.getKey())) {
      chunkMaxTokenLength =
          (int) parameters.get(VectorDatabaseParamOptionsEnum.CONTENT_LENGTH.getKey());
    }
    ...
    if (parameters.containsKey(VectorDatabaseParamOptionsEnum.CONTENT_OVERLAP.getKey())) {
      tokenOverlapBetweenChunks =
          (int) parameters.get(VectorDatabaseParamOptionsEnum.CONTENT_OVERLAP.getKey());
    }
    ...
    // parameters.get(VectorDatabaseParamOptionsEnum.EXTRACTION_METHOD.getKey());
    //		}
    ...
    File indexFilesFolder =
        new File(
            this.schemaFolder + "/" + indexClass, AbstractVectorDatabaseEngine.INDEXED_FOLDER_NAME);
    ...
    try {
      ...
      File documentDir =
          new File(indexDirectory, AbstractVectorDatabaseEngine.DOCUMENTS_FOLDER_NAME);
      ...
      List<String> filesToCopyToCloud =
          new ArrayList<>(); // create a list to store all the net new files so we can push them to
      // the cloud
      String chunkingStrategy =
          PyUtils.determineStringType(parameters.getOrDefault("chunkingStrategy", "ALL"));
      ...
      for (String fileName : filePaths) {
        ...
        try {
          ...
          classLogger.error(Constants.STACKTRACE, e);
          throw new IllegalArgumentException(
              "Unable to remove previously created file for "
                  + destinationFile.getName()
                  + " or move it to the document directory");
        }
      ...
      for (File document : fileToExtractFrom) {
        ...
        File extractedFile =
            new File(indexFilesFolder.getAbsolutePath() + "/" + documentName + ".csv");
        ...
        try {
          ...
          if (docLower.endsWith(".csv")) {
            classLogger.info(
                "You are attempting to load in a structured table for "
                    + documentName
            ...
            if (this.customDocumentProcessor) {
              ...
              IFunctionEngine functionEngine =
                  Utility.getFunctionEngine(this.customDocumentProcessorFunctionID);
              ...
              ICustomEmbeddingsFunctionEngine customEmbeddings =
                  (ICustomEmbeddingsFunctionEngine) functionEngine;
              if (customEmbeddings.canProcessDocument(document)) {
                rowsCreated =
                    customEmbeddings.processDocument(
                        extractedFile.getAbsolutePath(), document, parameters);
                processed = true;
              }
            }
            ...
            if (!processed) {
              rowsCreated =
                  VectorDatabaseUtils.convertFilesToCSV(extractedFile.getAbsolutePath(), document);
            }
            ...
            splitTextCommand
                .append("vector_database.split_text(csv_file_location = '")
      ...
      if (extractedFiles.size() > 0) {
        ...
        if (ClusterUtil.IS_CLUSTER) {
          ...
          Thread copyFilesToCloudThread =
              new Thread(
                  new CopyFilesToEngineRunner(
                      this.engineId,
                      this.getCatalogType(),
  ...
  @Override
  public List<Map<String, Object>> nearestNeighbor(
      Insight insight, String searchStatement, Number limit, Map<String, Object> parameters) {
    if (parameters == null) {
    ...
    List<Map<String, Object>> vectorSearchResponse =
        nearestNeighborCall(insight, searchStatement, limit, parameters);
  ...
  public String getDocumentsFilesPath(String indexClass) {
    ...
    if (!this.indexClasses.contains(indexClass)) {
      throw new IllegalArgumentException(
          "Unable to retieve document csv from a directory that does not exist");
    }
    return Utility.normalizePath(
        this.schemaFolder.getAbsolutePath()
            + "/"
            + indexClass
            + "/"
