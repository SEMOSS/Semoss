...
public class ElasticSearchRestVectorDatabaseEngine extends AbstractVectorDatabaseEngine {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(ElasticSearchRestVectorDatabaseEngine.class);
  ...
  public List<FileEmbeddingStatus> addEmbeddings(
    ...
    classLogger.info(
        "Inserted "
            + insertions.intValue()
            + " bulk inserts (create index + record value) into elastic search index "
            + this.indexName);
    ...
    if (errors) {
      classLogger.warn(
          "There were errors with some of the bulk insertions in the elastic search index "
              + this.indexName);
    } else {
      classLogger.info(
          "All records inserted successfully into Elasticsearch index '{}'", this.indexName);
    }
    ...
    search.addProperty("size", limit);
    {
      JsonObject query = new JsonObject();
      {
        if (!parameters.containsKey("filters")) {
          ...
          {
            knn.add(
                "query_vector", convertListNumToJsonArray(embeddingsResponse.getResponse().get(0)));
                  ...
                  JsonObject filterParent = new JsonObject();
                  {
                    JsonObject filterBool = new JsonObject();
                    {
                      ...
                      List<IQueryFilter> filters =
                          (List<IQueryFilter>) parameters.remove("filters");
  ...
  public List<Map<String, Object>> listDocuments(Map<String, Object> parameters) {
    ...
    String response =
        HttpHelperUtility.postRequestStringBody(
            url, headersMap, search.toString(), ContentType.APPLICATION_JSON, null, null, null);
    JsonObject responseJson = JsonParser.parseString(response).getAsJsonObject();
    JsonArray bucketsArr =
        responseJson
            .getAsJsonObject("aggregations")
            .getAsJsonObject(UNIQUE_SOURCES)
  ...
  public List<Map<String, Object>> listAllRecords(Map<String, Object> parameters) {
    ...
    for (JsonElement e : hits) {
      ...
      thisDocument.put(
          VectorDatabaseCSVTable.SOURCE, fields.get(VectorDatabaseCSVTable.SOURCE).getAsString());
      thisDocument.put(
          VectorDatabaseCSVTable.MODALITY,
          fields.get(VectorDatabaseCSVTable.MODALITY).getAsString());
      thisDocument.put(
          VectorDatabaseCSVTable.DIVIDER, fields.get(VectorDatabaseCSVTable.DIVIDER).getAsString());
      thisDocument.put(
          VectorDatabaseCSVTable.PART, fields.get(VectorDatabaseCSVTable.PART).getAsString());
      thisDocument.put(
          VectorDatabaseCSVTable.TOKENS, fields.get(VectorDatabaseCSVTable.TOKENS).getAsLong());
      thisDocument.put(
          VectorDatabaseCSVTable.CONTENT, fields.get(VectorDatabaseCSVTable.CONTENT).getAsString());
    }
  ...
  private String getCredsBase64Encoded() {
    ...
    if (this.authorizationMethod.equals("API_KEY")) {
      encoding =
          "ApiKey "
              + Base64.getEncoder().encodeToString((this.apiKeyId + ":" + this.apiKey).getBytes());
