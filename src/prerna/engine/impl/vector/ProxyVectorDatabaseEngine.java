...
public class ProxyVectorDatabaseEngine extends AbstractVectorDatabaseEngine {
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    if (secretStore != null) {
      ...
      if (engineSecrets == null || engineSecrets.isEmpty()) {
        classLogger.info(
            "No secrets found for " + SmssUtilities.getUniqueName(this.engineName, this.engineId));
      } else {
        classLogger.info(
            "Successfully pulled secrets for "
    ...
    if (targetInterceptorType == null
        || (targetInterceptorType = targetInterceptorType.trim()).isEmpty()) {
    ...
    if (targetParametersStr != null
        && !(targetParametersStr = targetParametersStr.trim()).isEmpty()) {
      this.targetParameters = new Gson().fromJson(targetParametersStr, Object[].class);
    }

    this.smssProp.put(
        Constants.EMBEDDER_ENGINE_ID, targetEngine.getSmssProp().get(Constants.EMBEDDER_ENGINE_ID));

    try {
      targetInterceptor =
          AbstractInterceptor.buildInterceptor(
              targetInterceptorType, this, targetEngine, targetParameters);
    } catch (ClassNotFoundException | ClassCastException e) {
      throw new IllegalArgumentException(
          "Unable to create interceptor for proxy vector database engine", e);
    }
  ...
  public void addEmbedding(
      ...
      throws Exception {
    proxy.addEmbedding(
        embedding, source, modality, divider, part, tokens, content, additionalMetadata);
  }
  ...
  protected List<Map<String, Object>> nearestNeighborCall(
    ...
    if (proxy instanceof AbstractVectorDatabaseEngine) {
      return ((AbstractVectorDatabaseEngine) proxy)
