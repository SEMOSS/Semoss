...
public final class PGVectorQueryFitlerTranslationHelper {
  ...
  private static SimpleQueryFilter convertSimpleQueryFilter(
    ...
    if (origL.getNounType() == PixelDataType.COLUMN) {
      ...
      newL =
          new NounMetadata(
              convertSelector((IQuerySelector) origL.getValue(), tableName), PixelDataType.COLUMN);
    }
    ...
    if (origR.getNounType() == PixelDataType.COLUMN) {
      ...
      newR =
          new NounMetadata(
              convertSelector((IQuerySelector) origR.getValue(), tableName), PixelDataType.COLUMN);
    }
    // Not going to handle a subquery against the pgvector at this point..
  ...
  private static BetweenQueryFilter convertBetweenQueryFilter(
      BetweenQueryFilter queryFilter, String tableName) {
  ...
  private static IQuerySelector convertIfElseSelector(QueryIfSelector selector, String tableName) {
    ...
    if (antecedent != null) selector.setAntecedent(convertSelector(antecedent, tableName));

    return selector;
  }

  private static IQuerySelector convertColumnSelector(
      QueryColumnSelector selector, String tableName) {
  ...
  private static IQuerySelector convertArithmeticSelector(
      QueryArithmeticSelector selector, String tableName) {
  ...
  private static IQuerySelector convertFunctionSelector(
      QueryFunctionSelector selector, String tableName) {
