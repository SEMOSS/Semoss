...
public class AwsS3VectorDatabaseEngine extends AbstractVectorDatabaseEngine {
	...
	private String awsS3VectorUrl= null;
		...
		public void removeDocument(List<String> fileNames, Map<String, Object> parameters) throws Exception {
		    ...
		    // Step 1: Fetch all vectors from the index
		    ...
		    listRequest.addProperty("returnMetadata", false);
		    listRequest.addProperty("returnData", false);
		    listRequest.addProperty("maxResults", 1000); // you can increase if needed
		    ...
		    JsonArray allVectors = responseJson.getAsJsonArray("vectors");
		    ...
		    for (String fileName : fileNames) {
		        ...
		        String normalizedName = baseName.replace(" ", "_")
		                                        .replaceAll("[^a-zA-Z0-9_]", "")
		                                        .toLowerCase();

		        for (JsonElement element : allVectors) {
		            JsonObject vector = element.getAsJsonObject();
		            String key = vector.get("key").getAsString().toLowerCase();
		    ...
		    System.out.println(keysToDelete.toString());
		    // Step 2: Send DELETE API request
		    if (!keysToDelete.isEmpty()) {
		        ...
		        if (deleteResponse == null || deleteResponse.trim().isEmpty()) {
		        ...
		        if (matchingFiles != null) {
		            for (File file : matchingFiles) {
		                if (file.exists()) {
		                    ...
		                    filesToRemoveFromCloud.add(file.getAbsolutePath());
		                }
		            }
		        }
		    }
	    ...
	    do {
	        ...
	        if (vectors != null) {
	            for (JsonElement ele : vectors) {
	                ...
	                document.put("AWS_S3_Key", key); // or use `this.ID` or `VECTOR_ID`, per your schema
	...
	protected List<Map<String, Object>> nearestNeighborCall(Insight insight, String searchStatement, Number limit,
		  ...
		  if (insight == null) {
		    ...
		    JsonObject genreEq = new JsonObject();
		    genreEq.addProperty("$eq", "IndiaPolicy.pdf");

		    filters.add("Source", genreEq);

		    // Build request body for /QueryVectors
		    JsonObject requestBody = new JsonObject();
		    requestBody.add("filter", filters);
	        requestBody.addProperty("indexName", this.indexName);
	        requestBody.addProperty("vectorBucketName", this.vectorBucketName);
		    ...
		    queryVectorObj.add("values", queryVectorArray);
		    requestBody.add("queryVector", queryVectorArray);
		    
		    System.out.println(requestBody.toString());

		    // Optional filter (if metadata filters provided)
		    if (parameters.containsKey(AbstractVectorDatabaseEngine.METADATA_FILTERS_KEY)) {
		        ...
		        JsonObject filter = filterBuilder.processAwsFilter(metaFilters); // <-- You need to implement this class
