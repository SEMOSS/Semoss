...
public class AwsS3VectorDatabaseEngine extends AbstractVectorDatabaseEngine {
	...
	private static final String VECTOR_BUCKET_NAME = "VECTOR_BUCKET_NAME";
	private static final String NON_FILTERABLE_KEYS = "NON_FILTERABLE_KEYS";
	...
	public void open(Properties smssProp) throws Exception {
		...
		if (this.distanceMetric == null
				|| !(this.distanceMetric = this.distanceMetric.trim()).matches("(?i)(cosine|euclidean)")) {
		...
		if (metadataKeys != null && !metadataKeys.trim().isEmpty()) {
			nonFilterableKeys = Arrays.stream(metadataKeys.split(",")).map(String::trim).collect(Collectors.toList());
		}
	...
	public List<FileEmbeddingStatus> addEmbeddings(VectorDatabaseCSVTable vectorCsvTable, Insight insight,
		...
		// Send POST request
		String response = HttpHelperUtility.postRequestStringBody(url,
				generateHeaders(getRequest.toString(), PUT_VECTORS_ENDPOINT), getRequest.toString(),
		...
		classLogger.info(
				"Successfully inserted " + vectorList.size() + " vectors into AWS S3 vector index " + this.indexName);
	...
	public void removeDocument(List<String> fileNames, Map<String, Object> parameters) throws Exception {
		...
		String listResponse = HttpHelperUtility.postRequestStringBody(listUrl,
				generateHeaders(listRequest.toString(), LIST_VECTORS_ENDPOINT), listRequest.toString(),
				ContentType.APPLICATION_JSON, null, null, null);
		...
		if (!keysToDelete.isEmpty()) {
			...
			String deleteResponse = HttpHelperUtility.postRequestStringBody(deleteUrl,
					generateHeaders(deleteRequest.toString(), DELETE_VECTORS_ENDPOINT), deleteRequest.toString(),
					ContentType.APPLICATION_JSON, null, null, null);
	...
	protected List<Map<String, Object>> nearestNeighborCall(Insight insight, String searchStatement, Number limit,
		...
		EmbeddingsModelEngineResponse embeddingsResponse = engine.embeddings(Arrays.asList(searchStatement), insight,
				null);
		...
		if (parameters.containsKey(AbstractVectorDatabaseEngine.FILTERS_KEY)) {
			...
			IQueryFilter combinedFilter = filters.size() == 1 ? filters.get(0) : new AndQueryFilter(filters);

			StringBuilder filter = filterBuilder.processAwsFilter(combinedFilter);

			// Parse the filter string into a JsonObject
			JsonObject filterObject = JsonParser.parseString(filter.toString()).getAsJsonObject();
		...
		if (parameters.containsKey(AbstractVectorDatabaseEngine.METADATA_FILTERS_KEY)) {
			List<IQueryFilter> metaFilters = (List<IQueryFilter>) parameters
					.get(AbstractVectorDatabaseEngine.METADATA_FILTERS_KEY);
		...
		try {
			response = HttpHelperUtility.postRequestStringBody(url,
					generateHeaders(requestBody.toString(), QUERY_VECTORS_ENDPOINT), requestBody.toString(),
					ContentType.APPLICATION_JSON, null, null, null);
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
		}
					...
					for (JsonElement ele : vectors) {
						...
						if (metadata != null) {
							document.put(VectorDatabaseCSVTable.SOURCE,
									getJsonValue(metadata.get(VectorDatabaseCSVTable.SOURCE)));
							document.put(VectorDatabaseCSVTable.MODALITY,
									getJsonValue(metadata.get(VectorDatabaseCSVTable.MODALITY)));
							document.put(VectorDatabaseCSVTable.DIVIDER,
									getJsonValue(metadata.get(VectorDatabaseCSVTable.DIVIDER)));
							document.put(VectorDatabaseCSVTable.PART,
									getJsonValue(metadata.get(VectorDatabaseCSVTable.PART)));
							document.put(VectorDatabaseCSVTable.TOKENS,
									getJsonValue(metadata.get(VectorDatabaseCSVTable.TOKENS)));
							document.put(VectorDatabaseCSVTable.CONTENT,
									getJsonValue(metadata.get(VectorDatabaseCSVTable.CONTENT)));
						}
	...
	private void createIndexInAwsS3() throws Exception {
		...
		HttpHelperUtility.postRequestStringBody(url, generateHeaders(request.toString(), CREATE_INDEX_ENDPOINT),
				request.toString(), ContentType.APPLICATION_JSON, null, null, null);
	...
	private void getOrCreateIndexIfNotExists() throws Exception {
		...
		try {
			HttpHelperUtility.postRequestStringBody(url, generateHeaders(getRequest.toString(), GET_INDEX_ENDPOINT),
					getRequest.toString(), ContentType.APPLICATION_JSON, null, null, null);
	...
	/**
	 * Generates AWS SigV4 signed headers for making a POST request to a custom AWS
	 * S3-compatible vector endpoint.
	 *
	 * @param body
	...
	private Map<String, String> generateHeaders(String body, String AWS_S3_ENDPOINT) throws Exception {
		...
		SdkHttpFullRequest unsignedRequest = SdkHttpFullRequest.builder().method(SdkHttpMethod.POST).uri(uri)
				.encodedPath(AWS_S3_ENDPOINT).putHeader("Content-Type", "application/json")
				.putHeader("Host", uri.getHost())
				.contentStreamProvider(() -> new ByteArrayInputStream(body.getBytes(StandardCharsets.UTF_8))).build();

		SdkHttpFullRequest signedRequest = signer.sign(unsignedRequest,
				Aws4SignerParams.builder().awsCredentials(awsCreds).signingName("s3vectors") // AWS service name
						.signingRegion(Region.US_EAST_1).build());
