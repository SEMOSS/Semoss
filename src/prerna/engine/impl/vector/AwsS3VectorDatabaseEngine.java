...
public class AwsS3VectorDatabaseEngine extends AbstractVectorDatabaseEngine {
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    if (this.vectorBucketName == null
        || (this.vectorBucketName = this.vectorBucketName.trim()).isEmpty()) {
    ...
    if (metadataKeys != null && !metadataKeys.trim().isEmpty()) {
      nonFilterableKeys =
          Arrays.stream(metadataKeys.split(",")).map(String::trim).collect(Collectors.toList());
    }
  ...
  public List<FileEmbeddingStatus> addEmbeddings(
    ...
    classLogger.info(
        "Successfully inserted "
            + vectorList.size()
            + " vectors into AWS S3 vector index "
            + this.indexName);
  ...
  protected List<Map<String, Object>> nearestNeighborCall(
    ...
    EmbeddingsModelEngineResponse embeddingsResponse =
        engine.embeddings(Arrays.asList(searchStatement), insight, null);
    ...
    AwsVectorQueryFilterTranslationHelper filterBuilder =
        new AwsVectorQueryFilterTranslationHelper();
    if (parameters.containsKey(AbstractVectorDatabaseEngine.FILTERS_KEY)) {
      ...
      IQueryFilter combinedFilter =
          filters.size() == 1 ? filters.get(0) : new AndQueryFilter(filters);
    ...
    if (parameters.containsKey(AbstractVectorDatabaseEngine.METADATA_FILTERS_KEY)) {
      ...
      IQueryFilter combinedMetaFilter =
          metaFilters.size() == 1 ? metaFilters.get(0) : new AndQueryFilter(metaFilters);
  ...
  public List<Map<String, Object>> listDocuments(Map<String, Object> parameters) {
    ...
    try {
      response =
          HttpHelperUtility.postRequestStringBody(
              url,
              generateHeaders(requestBody.toString(), LIST_VECTORS_ENDPOINT),
              requestBody.toString(),
              ContentType.APPLICATION_JSON,
              null,
              null,
      ...
      try {
        ...
        if (vectors != null) {
          for (JsonElement ele : vectors) {
            ...
            JsonObject metadata =
                vectorObj.has("metadata") && vectorObj.get("metadata").isJsonObject()
        ...
        nextToken =
            responseJson.has("nextToken") && !responseJson.get("nextToken").isJsonNull()
  ...
  /**
   * Generates AWS SigV4 signed headers for making a POST request to a custom AWS S3-compatible
   * vector endpoint.
   *
   * @param body
   ...
   * @throws Exception
   */
  private Map<String, String> generateHeaders(String body, String AWS_S3_ENDPOINT)
      throws Exception {
    ...
    SdkHttpFullRequest unsignedRequest =
