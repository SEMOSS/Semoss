...
public class OpenSearchDocumentSubsetInterceptor extends AbstractDocumentSubsetInterceptor {
	...
	public static final Set<String> INTERCEPTED_METHOD_NAMES = Sets.newHashSet("listAllRecords", "listDocuments",
			"nearestNeighbor", "removeDocument", "addDocument", "getDocumentsFilesPath");

	public OpenSearchDocumentSubsetInterceptor(IVectorDatabaseEngine proxyEngine, IVectorDatabaseEngine targetEngine,
	...
	/**
	 * Persist the filtered documents subset to disk, in the proxy's .smss Updates
	 * the TARGET_PARAMETERS field, which is a JSON array
	 */
	private void writeBackDocumentSubset(AbstractEngine proxyEngine, Set<String> subset) throws IOException {
		...
		// Step 2: Convert the current subset to a pretty JSON array string for the
		// field
		Gson gson = new Gson();
	...
	public String getDocumentsFilesPath(AbstractVectorDatabaseEngine proxyEngine, String indexClass) {
		...
		String engineDir = EngineUtility.getSpecificEngineAssetsFolder(IEngine.CATALOG_TYPE.VECTOR,
				proxyEngine.getEngineId(), proxyEngine.getEngineName());
		...
		return Utility.normalizePath(schemaFolder.getAbsolutePath() + FILE_SEPARATOR + indexClass + FILE_SEPARATOR
