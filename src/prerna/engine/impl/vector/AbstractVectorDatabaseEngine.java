...
public abstract class AbstractVectorDatabaseEngine extends AbstractEngine
    implements IVectorDatabaseEngine {

  private static final Logger classLogger =
      LogManager.getLogger(AbstractVectorDatabaseEngine.class);

  protected static final String TOKENIZER_INIT_SCRIPT =
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    this.distanceMethod =
        this.smssProp.getProperty(Constants.DISTANCE_METHOD, getDefaultDistanceMethod());
    this.defaultChunkingMethod =
        this.smssProp.getProperty(Constants.DEFAULT_CHUNKING_METHOD, "recursive");
    ...
    this.pyDirectoryBasePath =
        new File(Utility.normalizePath(engineDir + FILE_SEPARATOR + "py" + FILE_SEPARATOR));
  ...
  public List<FileEmbeddingStatus> addDocument(
    ...
    if (parameters.containsKey(VectorDatabaseParamOptionsEnum.EXTRACTION_METHOD.getKey())) {
      extractionMethod =
          (String) parameters.get(VectorDatabaseParamOptionsEnum.EXTRACTION_METHOD.getKey());
    }
    ...
    if (parameters.containsKey(VectorDatabaseParamOptionsEnum.CHUNKING_METHOD.getKey())) {
      chunkingMethod =
          (String) parameters.get(VectorDatabaseParamOptionsEnum.CHUNKING_METHOD.getKey());
    }
    ...
    File indexFilesDir =
        new File(this.schemaFolder + FILE_SEPARATOR + indexClass, INDEXED_FOLDER_NAME);
    ...
    try {
      ...
      for (File document : fileToExtractFrom) {
        ...
        File extractedFile =
            new File(indexFilesDir.getAbsolutePath() + FILE_SEPARATOR + documentName + ".csv");
        ...
        try {
          ...
          if (docLower.endsWith(".csv")) {
            ...
            if (!validCsv) {
              ...
              headerBuilder
                  .append("'")
                  .append(VectorDatabaseCSVTable.SOURCE)
                  .append("', ")
                  .append("'")
                  .append(VectorDatabaseCSVTable.SOURCE)
                  .append("', ")
                  .append("'")
                  .append(VectorDatabaseCSVTable.MODALITY)
                  .append("', ")
                  .append("'")
                  .append(VectorDatabaseCSVTable.DIVIDER)
                  .append("', ")
                  .append("'")
                  .append(VectorDatabaseCSVTable.PART)
                  .append("', ")
                  .append("'")
                  .append(VectorDatabaseCSVTable.TOKENS)
                  .append("', ")
                  .append("'")
                  .append(VectorDatabaseCSVTable.CONTENT)
                  .append("'");
              throw new IllegalArgumentException(
            ...
            if (extractionMethod.equals("fitz")
                && document.getName().toLowerCase().endsWith(".pdf")) {
              StringBuilder extractTextFromDocScript = new StringBuilder();
              extractTextFromDocScript
                  .append("vector_database.extract_text(source_file_name = '")
                  .append(document.getAbsolutePath().replace("\\", FILE_SEPARATOR))
                  .append("', target_folder = '")
                  .append(
                      this.schemaFolder.getAbsolutePath().replace("\\", FILE_SEPARATOR)
                          + FILE_SEPARATOR
                          + indexClass
                          + FILE_SEPARATOR
                          + "extraction_files")
      ...
      if (extractedFiles.size() == 0) {
        ...
        fileNamesAttemptedUpload.append("]");
        throw new IllegalArgumentException(
            "Unable to extract any text from " + fileNamesAttemptedUpload);
      }
  ...
  public List<FileEmbeddingStatus> addEmbeddingFiles(
    ...
    for (File vectorCsvFile : vectorCsvFiles) {
      try {
        ...
        FileEmbeddingStatus failedStatus =
            new FileEmbeddingStatus(vectorCsvFile.getName(), "FAILED", 0, 0, 0);
   ...
   * @return
   */
  protected abstract List<Map<String, Object>> nearestNeighborCall(
  ...
  protected Float[] getEmbeddingsFloat(String content, Insight insight) {
    ...
    List<Double> embeddingsResponse =
        embeddingsEngine
            .embeddings(Arrays.asList(new String[] {content}), getInsight(insight), null)
            .getResponse()
            .get(0);
