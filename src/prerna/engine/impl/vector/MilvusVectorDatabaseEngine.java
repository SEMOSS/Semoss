...
public class MilvusVectorDatabaseEngine extends AbstractVectorDatabaseEngine {
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    if (this.databaseName != null
        && ((this.databaseName = this.databaseName.trim()).isEmpty()
    ...
    if (this.collectionName == null
        || (this.collectionName = this.collectionName.trim()).isEmpty()) {
  ...
  public List<FileEmbeddingStatus> addEmbeddings(
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to add embeddings into collection '{}' within database '{}'",
          this.collectionName,
          this.databaseName);
      for (Map.Entry<String, Integer> entry : fileRecordCountMap.entrySet()) {
        fileStatusList.add(
            new FileEmbeddingStatus(
                entry.getKey(), "FAILED", 0, entry.getValue(), entry.getValue()));
      }
      ...
      classLogger.info(
          "Inserted {} records into Milvus Vector collection: {}",
          insertedCount,
          this.collectionName);
      for (Map.Entry<String, Integer> entry : fileRecordCountMap.entrySet()) {
  ...
  public void removeDocument(List<String> fileNames, Map<String, Object> parameters)
    ...
    String filter =
        fileNames.stream()
            .map(
                fileName ->
                    "Source like '" + fileName.replace("'", "''").replace("\\", "\\\\") + "'")
  ...
  @Override
  protected List<Map<String, Object>> nearestNeighborCall(
    ...
    MilvusVectorQueryFitlerTranslationHelper dbfilter =
        new MilvusVectorQueryFitlerTranslationHelper();
    ...
    String response =
        HttpHelperUtility.postRequestStringBody(
            url, getHeaders(), search.toString(), ContentType.APPLICATION_JSON, null, null, null);
    ...
    for (JsonElement matchEle : data) {
      ...
      retMap.put(
          VectorDatabaseCSVTable.SOURCE, getJsonValue(matchObj.get(VectorDatabaseCSVTable.SOURCE)));
      retMap.put(
          VectorDatabaseCSVTable.MODALITY,
          getJsonValue(matchObj.get(VectorDatabaseCSVTable.MODALITY)));
      retMap.put(
          VectorDatabaseCSVTable.DIVIDER,
          getJsonValue(matchObj.get(VectorDatabaseCSVTable.DIVIDER)));
      retMap.put(
          VectorDatabaseCSVTable.PART, getJsonValue(matchObj.get(VectorDatabaseCSVTable.PART)));
      retMap.put(
          VectorDatabaseCSVTable.TOKENS, getJsonValue(matchObj.get(VectorDatabaseCSVTable.TOKENS)));
      retMap.put(
          VectorDatabaseCSVTable.CONTENT,
          getJsonValue(matchObj.get(VectorDatabaseCSVTable.CONTENT)));
  ...
  private void createEmbeddingsIndex() {
    ...
    String response =
        HttpHelperUtility.postRequestStringBody(
            url, getHeaders(), request.toString(), ContentType.APPLICATION_JSON, null, null, null);
    JsonObject json = JsonParser.parseString(response).getAsJsonObject();
    if (!json.has("code") && json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to create index on field '{}' in collection '{}'", "", this.collectionName);
      throw new RuntimeException(
          "Failed to create index in Milvus collection: "
              + this.collectionName
              + ". Detailed error = "
              + json);
    }
    ...
    classLogger.info(
        "Index '{}' created successfully for collection '{}'", "", this.collectionName);
  }
  ...
  private void createDatabase() {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      ...
      throw new RuntimeException(
          "Failed to create database " + this.databaseName + ". Detailed error = " + json);
    }
  ...
  private boolean doesCollectionExist() {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      ...
      throw new RuntimeException(
          "Failed to execute collection list endpoint. Detailed error = " + json);
    }
    ...
    classLogger.warn(
        "Collection '{}' does not exist in database '{}'.", this.collectionName, this.databaseName);
    return false;
  }
  ...
  private JsonObject dropDatabase(String databaseName) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      ...
      throw new RuntimeException(
          "Failed to drop database " + this.databaseName + ". Detailed error = " + json);
    }
    return json;
  }
  ...
  private JsonObject describeCollection(String collectionName) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to describe collection '{}' in database '{}'",
          this.collectionName,
          this.databaseName);
      throw new RuntimeException(
  ...
  private JsonObject getCollectionStats(String collectionName) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to describe stats for collection '{}' in database '{}'",
          this.collectionName,
          this.databaseName);
      throw new RuntimeException(
          "Failed to describe stats for collection "
              + this.collectionName
              + " in database "
              + this.databaseName
              + ". Detailed error = "
  ...
  private JsonObject getCollectionLoadState(String collectionName, List<String> partitionNames) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to get collection load state for collection '{}' in database '{}'",
          this.collectionName,
          this.databaseName);
      throw new RuntimeException(
          "Failed to get collection load state for collection "
              + this.collectionName
              + " in database "
              + this.databaseName
              + ". Detailed error = "
  ...
  private JsonObject loadCollection(String collectionName) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to load collection '{}' in database '{}'",
          this.collectionName,
          this.databaseName);
      throw new RuntimeException(
  ...
  private JsonObject releaseCollection(String collectionName) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to release collection '{}' in database '{}'",
          this.collectionName,
          this.databaseName);
      throw new RuntimeException(
          "Failed to release collection "
              + this.collectionName
              + " in database "
              + this.databaseName
              + ". Detailed error = "
  ...
  private JsonObject renameCollection(String collectionName, String newCollectionName) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to rename collection '{}' in database '{}'",
          this.collectionName,
          this.databaseName);
      throw new RuntimeException(
  ...
  private JsonObject dropCollection(String collectionName) {
    ...
    if (!json.has("code") || json.get("code").getAsInt() != 0) {
      classLogger.error(
          "Failed to drop collection '{}' in database '{}'",
          this.collectionName,
          this.databaseName);
      throw new RuntimeException(
          "Failed to drop collection "
