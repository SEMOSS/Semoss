...
public final class RestVectorQueryFilterTranslationHelper {
	 ...
	 * @param must_not
	 */
	private static void addAndFilter(AndQueryFilter queryFilter, JsonArray filter, JsonArray should,
			JsonArray must_not) {
	 ...
	 * @return
	 */
	private static JsonObject addSelectorToValuesFilter(NounMetadata leftComp, NounMetadata rightComp,
		...
		if (thisComparator.equals("==") || thisComparator.equals("!=")) {
						...
						if (normalizedValues.isEmpty()) {
							...
							for (int i = 0; i < normalizedValues.size(); i++) {
								JsonObject matchParent = new JsonObject();
								{
									...
									{
										match.addProperty(leftComp.getValue().toString(),
												normalizedValues.get(i).toString());
									}
			...
			return bool;
		} else if (thisComparator.equals("<") || thisComparator.equals(">") || thisComparator.equals(">=")
