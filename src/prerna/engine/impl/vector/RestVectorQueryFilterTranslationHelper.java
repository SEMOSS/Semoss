...
public final class RestVectorQueryFilterTranslationHelper {
   ...
   * @param must_not
   */
  public static void processFilter(
      IQueryFilter queryFilter, JsonArray filter, JsonArray should, JsonArray must_not) {
   ...
   * @param must_not
   */
  private static void addSimpleQueryFilter(
      SimpleQueryFilter filter, JsonArray targetArray, JsonArray must_not) {
   ...
   * @param must_not
   */
  private static void addAndFilter(
      AndQueryFilter queryFilter, JsonArray filter, JsonArray should, JsonArray must_not) {
    ...
    for (int i = 0; i < numAnds; i++) {
      JsonObject nestedBool = new JsonObject();
      {
        ...
        if (filter2.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
          if (isNestedFilter) {
            addSimpleQueryFilter(
                (SimpleQueryFilter) filterList.get(i), nestedFilter, nestedMustNot);
          } else {
          ...
          addAndFilter(
              (AndQueryFilter) filterList.get(i), nestedFilter, nestedShould, nestedMustNot);
   ...
   * @param must_not
   */
  private static void addOrFilter(
      OrQueryFilter queryFilter, JsonArray filter, JsonArray should, JsonArray must_not) {
    List<IQueryFilter> filterList = queryFilter.getFilterList();
    ...
    for (int i = 0; i < numAnds; i++) {
      JsonObject nestedBool = new JsonObject();
      {
        ...
        if (filter2.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
          if (isNestedFilter) {
            addSimpleQueryFilter(
                (SimpleQueryFilter) filterList.get(i), nestedShould, nestedMustNot);
          } else {
   ...
   * @return
   */
  private static JsonObject addSelectorToValuesFilter(
    ...
    if (thisComparator.equals("==") || thisComparator.equals("!=")) {
            ...
            if (normalizedValues.isEmpty()) {
              ...
              for (int i = 0; i < normalizedValues.size(); i++) {
                JsonObject matchParent = new JsonObject();
                {
                  ...
                  {
                    match.addProperty(
                        leftComp.getValue().toString(), normalizedValues.get(i).toString());
                  }
      ...
      JsonObject jsonBuilder = new JsonObject();
      {
        ...
        {
          wildcard.addProperty(
              leftComp.getValue().toString(), "*" + rightComp.getValue().toString() + "*");
        }
