...
public final class PGVectorQueryMetaFitlerTranslationHelper {
	...
	/**
	 * Convert a filter Look at left hand side and right hand side If either is a
	 * column, try to convert
	 *
	 * @param queryFilter
	 * @param meta
	 * @return
	...
	private static IQueryFilter convertSimpleQueryFilter(SimpleQueryFilter queryFilter, String tableName) {
		/*
		 * We need to convert a simple query filter into an AND filter Where we filter
		 * on the attribute name and the associated value
		 */
		...
		if (origL.getNounType() == PixelDataType.COLUMN) {
			...
			SimpleQueryFilter attributeFilter = new SimpleQueryFilter(
					new NounMetadata(
							new QueryColumnSelector(tableName + "__" + VectorDatabaseMetadataCSVTable.ATTRIBUTE),
		...
		// SelectQueryStruct newQs = getPhysicalQs((SelectQueryStruct) origL.getValue(),
		// tableName);
		...
		// SelectQueryStruct newQs = getPhysicalQs((SelectQueryStruct) origR.getValue(),
		// tableName);
	...
	private static SimpleQueryFilter constructValueFilter(NounMetadata existingValue, String tableName,
			String comparator) {
		...
		if (dataType == PixelDataType.CONST_STRING) {
			SimpleQueryFilter valueFilter = new SimpleQueryFilter(new NounMetadata(
					new QueryColumnSelector(tableName + "__" + VectorDatabaseMetadataCSVTable.STR_VALUE),
					PixelDataType.COLUMN), comparator, new NounMetadata(filterVal, PixelDataType.CONST_STRING));
			return valueFilter;
		} else if (dataType == PixelDataType.CONST_INT) {
			SimpleQueryFilter valueFilter = new SimpleQueryFilter(new NounMetadata(
					new QueryColumnSelector(tableName + "__" + VectorDatabaseMetadataCSVTable.INT_VALUE),
					PixelDataType.COLUMN), comparator, new NounMetadata(filterVal, PixelDataType.CONST_INT));
			return valueFilter;
		} else if (dataType == PixelDataType.CONST_DECIMAL) {
			SimpleQueryFilter valueFilter = new SimpleQueryFilter(
					new NounMetadata(
							new QueryColumnSelector(tableName + "__" + VectorDatabaseMetadataCSVTable.NUM_VALUE),
							PixelDataType.COLUMN),
					comparator, new NounMetadata(filterVal, PixelDataType.CONST_DECIMAL));
			return valueFilter;
		} else if (dataType == PixelDataType.BOOLEAN) {
			SimpleQueryFilter valueFilter = new SimpleQueryFilter(new NounMetadata(
					new QueryColumnSelector(tableName + "__" + VectorDatabaseMetadataCSVTable.BOOL_VALUE),
					PixelDataType.COLUMN), comparator, new NounMetadata(filterVal, PixelDataType.BOOLEAN));
			return valueFilter;
		} else if (dataType == PixelDataType.CONST_DATE) {
			SimpleQueryFilter valueFilter = new SimpleQueryFilter(new NounMetadata(
					new QueryColumnSelector(tableName + "__" + VectorDatabaseMetadataCSVTable.DATE_VAL),
					PixelDataType.COLUMN), comparator, new NounMetadata(filterVal, PixelDataType.CONST_DATE));
			return valueFilter;
		} else if (dataType == PixelDataType.CONST_TIMESTAMP) {
			SimpleQueryFilter valueFilter = new SimpleQueryFilter(
					new NounMetadata(
							new QueryColumnSelector(tableName + "__" + VectorDatabaseMetadataCSVTable.TIMESTAMP_VAL),
							PixelDataType.COLUMN),
					comparator, new NounMetadata(filterVal, PixelDataType.CONST_TIMESTAMP));
			return valueFilter;
		}
