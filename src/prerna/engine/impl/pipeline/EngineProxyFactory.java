...
/**
 * Factory for creating a dynamic proxy that wraps an IEngine instance to apply input and output
...
public class EngineProxyFactory {
      ...
      if (pipelineValue != null && !pipelineValue.isBlank()) {
        ...
        if (jsonFile.exists() && jsonFile.isFile()) {
          ...
          return (IDatabaseEngine)
              Proxy.newProxyInstance(IEngine.class.getClassLoader(), classes, handler);
        }
      }
    }
    return engine;
   ...
   * @return
   */
  public static IGuardrailReactorFunctionEngine createGuardedGuardrailEngine(
      IGuardrailReactorFunctionEngine engine) {
    if (engine != null && engine.getSmssProp().containsKey(IEngine.PIPELINE)) {
      String pipelineValue = engine.getSmssProp().getProperty(IEngine.PIPELINE);
      if (pipelineValue != null && !pipelineValue.isBlank()) {
        File jsonFile = getJsonFile(engine, pipelineValue);
        if (jsonFile.exists() && jsonFile.isFile()) {
          PipelineInvocationHandler handler = new PipelineInvocationHandler(engine, jsonFile);
          return (IGuardrailReactorFunctionEngine)
              Proxy.newProxyInstance(
                  IEngine.class.getClassLoader(),
                  new Class<?>[] {IEngine.class, IGuardrailReactorFunctionEngine.class},
                  handler);
        }
      }
    }
