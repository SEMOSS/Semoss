...
/**
 * The invocation handler for the dynamic proxy. This class intercepts all method calls, executes
 * the appropriate pipelines, and then calls the real engine method.
 */
public class PipelineInvocationHandler implements InvocationHandler {
  ...
  public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
    ...
    try {
      for (IInputReactor reactor : specificPipeline.getInputPipeline()) {
        ...
        processedArguments.put(
            PipelineReactorUtils.CONFIG, specificPipeline.getInputParams().get(inputIndex));
        ...
        Map<String, Object> resultMap =
            (Map<String, Object>) processedArguments.get(PipelineReactorUtils.INTERIM_RESULT);
    ...
    for (IOutputReactor reactor : specificPipeline.getOutputPipeline()) {
      ...
      processedArguments.put(PipelineReactorUtils.METHOD_NAME, method);
      processedArguments.put(
          PipelineReactorUtils.CONFIG, specificPipeline.getOutputParams().get(inputIndex));
  ...
  /**
   * Maps the method arguments into a Map, including both named parameters and argN. Also sets
   * Insight on the reactor if present.
   *
      ...
      if (pipelineConfig.has("input")) {
        ...
        for (int i = 0; i < inputArray.length(); i++) {
          ...
          Map<String, Object> inputParam =
              (Map<String, Object>) inputReactor.getNounStore().getNoun("param").get(0);
      ...
      if (pipelineConfig.has("output")) {
        ...
        for (int i = 0; i < outputArray.length(); i++) {
          ...
          Map<String, Object> outputParam =
              (Map<String, Object>) outputReactor.getNounStore().getNoun("param").get(0);
          outputParams.add(outputParam);
          outputReactors.add(outputReactor);
        }
      }

      this.pipelinesMap.put(
          methodName, new Pipeline(inputReactors, outputReactors, inputParams, outputParams));
    }
  }
  ...
  private static class Pipeline {
     ...
     */
    Pipeline(
        List<IInputReactor> inputPipeline,
        List<IOutputReactor> outputPipeline,
