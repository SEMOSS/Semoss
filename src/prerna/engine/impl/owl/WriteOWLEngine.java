...
public class WriteOWLEngine extends AbstractOWLEngine implements Closeable {
  ...
  protected void loadDatabaseValues() {
    ...
    boolean isRdbms =
        (this.dbType == IDatabaseEngine.DATABASE_TYPE.RDBMS
  ...
  /**
   * Add a concept to the OWL If RDF : a concept has a data type (String) If RDBMS : this will
   * represent a table and not have a datatype
   *
   * @param tableName
   * @param dataType
   * @param conceptual
  ...
  /**
   * This method will add a property onto a concept in the OWL file There are some differences based
   * on how the information is used based on if it is a RDF engine or a RDBMS engine
   *
   * @param tableName For RDF: This is the name of the concept For RDBMS: This is the name of the
   *     table where the concept exists. If the concept doesn't exist, it is assumed the column name
   *     of the concept is the same as the table name
   ...
   */
  public String addProp(
      String tableName, String propertyCol, String dataType, String adtlDataType) {
  ...
  /**
   * This method will calculate the unique values in each column/property and add it to the owl
   * file.
   *
   * @param queryEngine
   */
  ...
  /**
   * Have one class a subclass of another class This code is really intended for RDF databases...
  ...
  // //////////////////////////////////
  /**
   * Remove a concept from the OWL If RDF : a concept has a data type (String) If RDBMS : this will
   * represent a table and not have a datatype
   *
  ...
  public NounMetadata removeConcept(String tableName) {
      ...
      for (int i = 0; i < tablesAndPrimaryKeys.length; i += 2) {
        ...
        if (tableName.equalsIgnoreCase(key)) {
          this.baseDataEngine.removeStatement(
              new Object[] {relations[0], relations[2], relations[1], true});
          this.baseDataEngine.removeStatement(
              new Object[] {
                relations[2],
    ...
    noun.addAdditionalReturn(
        new NounMetadata(
            "Successfully removed concept and all its dependencies",
  ...
  /**
   * This method will remove a property from a concept in the OWL file There are some differences
   * based on how the information is used based on if it is a RDF engine or a RDBMS engine
   *
   * @param tableName
   * @param propertyCol
   * @return
  ...
  public NounMetadata removeProp(String tableName, String propertyCol) {
...

    {
      ...
      String downstreamQuery =
          "select ?s ?p ?o where { bind(<" + property + "> as ?s) " + "{?s ?p ?o} }";
      IRawSelectWrapper it = null;
      try {
   ...
   * @return
   */
  public NounMetadata renameConcept(
      String oldConceptName, String newConceptName, String newConceptualName) {
    ...
    for (String[] relations : fkRelationships) {
      ...
      String relationName =
          Utility.getInstanceName(relURI); // this is either a.b.c.d or x.a.b.y.c.d
      ...
      if (editRelation) {
        ...
        newTriplesToAdd.add(
            new Object[] {newRelationURI, RDFS.SUBPROPERTYOF.toString(), BASE_RELATION_URI, true});
      }
    }
    ...
    if (bindings.length() > 0) {
      ...
      {
        String query =
            "select ?s ?p ?o where { {?s ?p ?o} } bindings ?s {" + bindings.toString() + "}";
      ...
      {
        String query =
            "select ?s ?p ?o where { {?s ?p ?o} } bindings ?o {" + bindings.toString() + "}";
    ...
    {
      String query =
          "select ?s ?p ?o where { bind(<" + oldConceptPhysical + "> as ?s) {?s ?p ?o} }";
      ...
      try {
        ...
        while (it.hasNext()) {
          ...
          if (isLiteral) {
            newTriplesToAdd.add(
                new Object[] {newConceptPhysicalUri, p, headerRows.getValues()[2], false});
          } else {
    ...
    {
      String query =
          "select ?s ?p ?o where { bind(<" + oldConceptPhysical + "> as ?o) {?s ?p ?o} }";
  ...
  public NounMetadata renameProp(String tableName, String oldPropName, String newPropName) {
...

    {
      // remove everything downstream of the property
      String downstreamQuery =
          "select ?s ?p ?o where { bind(<" + propPhysicalUri + "> as ?s) " + "{?s ?p ?o} }";
      IRawSelectWrapper it = null;
      try {
        it = WrapperManager.getInstance().getRawWrapper(this.baseDataEngine, downstreamQuery);
        while (it.hasNext()) {
          ...
          if (isLiteral) {
            newTriplesToAdd.add(
                new Object[] {newPropPhysicalUri, p, headerRows.getValues()[2], false});
          } else {
...

    {
      ...
      String upstreamQuery =
          "select ?s ?p ?o where { bind(<" + propPhysicalUri + "> as ?o) {?s ?p ?o} }";
      IRawSelectWrapper it = null;
      try {
        it = WrapperManager.getInstance().getRawWrapper(this.baseDataEngine, upstreamQuery);
        while (it.hasNext()) {
  ...
  private List<String[]> getPhysicalRelationships(IDatabaseEngine engine) {
    String query =
        "SELECT DISTINCT ?start ?end ?rel WHERE { "
  ...
  /**
   * Adding information into the base engine Currently assumes we are only adding URIs (object is
   * never a literal)
   *
  ...
  private void deleteExisitngTimestamp() {
    String getAllTimestampQuery =
        "SELECT DISTINCT ?time ?val WHERE { "
  ...
  public void deleteLogicalNames(String physicalUri, String... logicalNames) {
    if (logicalNames != null) {
      for (String lName : logicalNames) {
        if (lName != null && !lName.isEmpty()) {
          this.removeFromBaseEngine(
              new Object[] {physicalUri, OWL.sameAs.toString(), lName, false});
        }
      }
    }
  }
  ...
  public void deleteDescription(String physicalUri, String description) {
    if (description != null && !description.trim().isEmpty()) {
      description = description.replaceAll("[^\\p{ASCII}]", "");
      this.removeFromBaseEngine(
          new Object[] {physicalUri, RDFS.COMMENT.toString(), description, false});
    }
  }
  ...
  /**
   * Store the custom base URI used to create instance URIs within the OWL E.g. of usage is current
   * RDF MHS databases, which use "http://health.mil/ontologies" as the custom base URI
   *
   * @param customBaseURI The customBaseURI to store
   */
  public void addCustomBaseURI(String customBaseURI) {
    this.addToBaseEngine(
        "SEMOSS:ENGINE_METADATA",
        "CONTAINS:BASE_URI",
