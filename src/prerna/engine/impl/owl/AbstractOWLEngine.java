...
public abstract class AbstractOWLEngine implements IExplorable {
	...
	public Vector<String> getFromNeighbors(String physicalNodeType, int neighborHood) {
		...
		String query = "SELECT DISTINCT ?node WHERE { BIND(<" + physicalNodeType + "> AS ?start) {?rel <"
				+ RDFS.SUBPROPERTYOF + "> <http://semoss.org/ontologies/Relation>} " + "{?node ?rel ?start}}";
	...
	public Vector<String> getToNeighbors(String physicalNodeType, int neighborHood) {
		...
		String query = "SELECT DISTINCT ?node WHERE { BIND(<" + physicalNodeType + "> AS ?start) {?rel <"
				+ RDFS.SUBPROPERTYOF + "> <http://semoss.org/ontologies/Relation>} " + "{?start ?rel ?node}}";
		return Utility.getVectorOfReturn(query, baseDataEngine, true);
	}
	...
	public Map<String, Object[]> getMetamodel() {
		...
		String getSelectorsInformation = "SELECT DISTINCT ?concept ?property WHERE { "
				...
				+ "OPTIONAL {" + "{?property <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <" + CONTAINS_BASE_URI
	...
	public List<String> getPixelSelectors(String conceptPixelName) {
		...
		String query = "SELECT DISTINCT ?pixelName WHERE { " + " BIND(<http://semoss.org/ontologies/Concept/"
				+ conceptPixelName + "> as ?concept) "
				+ " {?concept <http://semoss.org/ontologies/Relation/Pixel> ?pixelName }"
		...
		query = "SELECT DISTINCT ?pixelName WHERE { " + " BIND(<http://semoss.org/ontologies/Concept/"
				+ conceptPixelName + "> as ?concept) "
				+ " {?concept <http://www.w3.org/2000/01/rdf-schema#subClassOf> <http://semoss.org/ontologies/Concept> } "
	...
	public List<String> getPropertyUris4PhysicalUri(String physicalUri) {
		String query = "SELECT DISTINCT ?property WHERE { " + "BIND(<" + physicalUri + "> AS ?concept) "
				+ "{?concept <http://www.w3.org/2000/01/rdf-schema#subClassOf> <http://semoss.org/ontologies/Concept> } "
	...
	public String getPhysicalUriFromPixelSelector(String pixelSelector) {
		...
		if (semossPropertyName == null) {
			...
			query = "SELECT DISTINCT ?concept WHERE { " + " BIND(<http://semoss.org/ontologies/Concept/"
					+ semossConceptName + "> as ?pixelName) "
					+ " {?concept <http://semoss.org/ontologies/Relation/Pixel> ?pixelName } " + " }";
		} else {
			// this is a property
			query = "SELECT DISTINCT ?property WHERE { " + " BIND(<http://semoss.org/ontologies/Relation/Contains/"
					+ semossPropertyName + "/" + semossConceptName + "> as ?pixelName) "
					+ " {?property <http://semoss.org/ontologies/Relation/Pixel> ?pixelName } " + " }";
		}
	...
	public String getPixelSelectorFromPhysicalUri(String physicalUri) {
		String query = "SELECT DISTINCT ?pixel ?type WHERE { " + " {" + " BIND(\"concept\" as ?type) " + " BIND(<"
				+ physicalUri + "> as ?physicalUri) "
				+ " {?physicalUri <http://www.w3.org/2000/01/rdf-schema#subClassOf> <http://semoss.org/ontologies/Concept> }"
				+ " {?physicalUri <http://semoss.org/ontologies/Relation/Pixel> ?pixel } " + " }" + " UNION " + "	{"
				+ " BIND(\"property\" as ?type) " + " BIND(<" + physicalUri + "> as ?physicalUri) "
