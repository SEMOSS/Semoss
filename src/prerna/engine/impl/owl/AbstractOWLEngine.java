...
public abstract class AbstractOWLEngine implements IExplorable {
  ...
  private static final String FROM_SPARQL =
  ...
  private static final String TO_SPARQL =
      "SELECT DISTINCT ?entity WHERE { "
          + "{?rel <http://www.w3.org/2000/01/rdf-schema#subPropertyOf> <http://semoss.org/ontologies/Relation>} "
          + "{?entity <http://www.w3.org/2000/01/rdf-schema#subClassOf> <http://semoss.org/ontologies/Concept>} "
          + "{?x ?rel ?y} "
  ...
  public static final String CONCEPTUAL_RELATION_URI =
      BASE_RELATION_URI + "/" + CONCEPTUAL_RELATION_NAME;
  public static final String PIXEL_RELATION_URI = BASE_RELATION_URI + "/" + PIXEL_RELATION_NAME;
  public static final String ADDITIONAL_DATATYPE_RELATION_URI =
      BASE_RELATION_URI + "/" + ADDITIONAL_DATATYPE_NAME;
  ...
  public Vector<String> getFromNeighbors(String physicalNodeType, int neighborHood) {
    ...
    String query =
        "SELECT DISTINCT ?node WHERE { BIND(<"
            + physicalNodeType
            + "> AS ?start) {?rel <"
            + RDFS.SUBPROPERTYOF
  ...
  public Vector<String[]> getFromNeighborsWithRelation(String physicalNodeType, int neighborHood) {
    ...
    // Utility.getInstanceName(nodeType), false);
    String query =
        "SELECT DISTINCT ?node ?rel WHERE { BIND(<"
            + physicalNodeType
            + "> AS ?start) {?rel <"
            + RDFS.SUBPROPERTYOF
            + "> <http://semoss.org/ontologies/Relation>} "
  ...
  public Vector<String> getConcepts() {
    String query =
        "SELECT ?concept WHERE { {?concept <http://www.w3.org/2000/01/rdf-schema#subClassOf> <http://semoss.org/ontologies/Concept> }"
  ...
  /**
   * This method runs an update query on the base data engine which contains all owl and metamodel
   * information
   */
  ...
  public String getDataTypes(String uri) {
    String query =
        "SELECT DISTINCT ?TYPE WHERE { {<" + uri + "> <" + RDFS.CLASS.toString() + "> ?TYPE} }";
    IRawSelectWrapper wrapper = null;
    try {
  ...
  public Map<String, String> getDataTypes(String... uris) {
    ...
    String query =
        "SELECT DISTINCT ?NODE ?TYPE WHERE { {?NODE <" + RDFS.CLASS.toString() + "> ?TYPE} } ";
  ...
  public String getAdtlDataTypes(String uri) {
    String query =
        "SELECT DISTINCT ?ADTLTYPE WHERE { {<"
            + uri
            + "> <"
            + ADDITIONAL_DATATYPE_RELATION_URI
            + "> ?ADTLTYPE} }";
  ...
  public Map<String, String> getAdtlDataTypes(String... uris) {
    ...
    String query =
        "SELECT DISTINCT ?NODE ?ADTLTYPE WHERE { {?NODE <"
            + ADDITIONAL_DATATYPE_RELATION_URI
            + "> ?ADTLTYPE} } ";
    String bindings = bindBuilder.toString();
    if (!bindings.isEmpty()) {
  ...
  public Map<String, Object[]> getMetamodel() {
    ...
    String getRelationshipsInformation =
        "SELECT DISTINCT ?fromConceptualConcept ?rel ?toConceptualConcept WHERE { "
  ...
  public List<String> getPixelSelectors(String conceptPixelName) {
    ...
    String query =
        "SELECT DISTINCT ?pixelName WHERE { "
  ...
  public String getPhysicalUriFromPixelSelector(String pixelSelector) {
    ...
    if (semossPropertyName == null) {
      ...
      query =
          "SELECT DISTINCT ?property WHERE { "
              + " BIND(<http://semoss.org/ontologies/Relation/Contains/"
              + semossPropertyName
              + "/"
              + semossConceptName
              + "> as ?pixelName) "
  ...
  @Deprecated
  /**
   * We cannot use this cause of the fact that we have not updated the OWL triples for a RDF engine
   * for the properties to contain the Concept in the URL (which would make it unique) Example:
   * Right now we have http://semoss.org/ontologies/Relation/Contains/Description as a property
   * which could point to multiple concepts
   */
  public String getPixelUriFromPhysicalUri(String physicalUri) {
    String query =
        "SELECT DISTINCT ?pixel WHERE { "
            + " BIND(<"
            + physicalUri
            + "> as ?physicalUri) "
  ...
  @Override
  /**
   * This is so annoying... no simple work around for the issue with {@link
   * #getPixelUriFromPhysicalUri(String)}
   */
  public String getPropertyPixelUriFromPhysicalUri(
      String conceptPhysicalUri, String propertyPhysicalUri) {
    String query =
        "SELECT DISTINCT ?pixel ?parentPixel WHERE { "
            + " BIND(<"
            + propertyPhysicalUri
            + "> as ?propertyPhysicalUri) "
            + " BIND(<"
            + conceptPhysicalUri
            + "> as ?conceptPhysicalUri) "
    ...
    try {
      ...
      classLogger.debug(
          "UGH... WHY ARE YOU NOT UNIQUE AS PHYSICAL!!! "
              + conceptPhysicalUri
              + " ::: "
              + propertyPhysicalUri);
      while (wrapper.hasNext()) {
  ...
  public String getPixelSelectorFromPhysicalUri(String physicalUri) {
    String query =
        "SELECT DISTINCT ?pixel ?type WHERE { "
            + " {"
  ...
  public String getConceptualName(String physicalUri) {
    String query =
        "SELECT DISTINCT ?conceptual WHERE { "
            + "BIND(<"
            + physicalUri
            + "> AS ?uri) "
            + "{?uri <"
            + CONCEPTUAL_RELATION_URI
            + "> ?conceptual } "
            + "}";
