...
public class AuditDatabase {
  ...
  @Deprecated private Map<String, String[]> primaryKeyCache = new HashMap<>();
  ...
  public void init(IDatabaseEngine database, String databaseId, String databaseName) {
    ...
    //		RdbmsConnectionBuilder builder = new
    // RdbmsConnectionBuilder(RdbmsConnectionBuilder.CONN_TYPE.DIRECT_CONN_URL);
   ...
   * @param userId
   */
  public synchronized void auditInsertQuery(
      List<IQuerySelector> selectors, List<Object> values, String userId, String query) {
   ...
   * @param userId
   */
  public synchronized void auditUpdateQuery(
      UpdateQueryStruct updateQs, String userId, String query) {
  ...
  public synchronized void auditDeleteQuery(SelectQueryStruct qs, String userId, String query) {
    ...
    try (PreparedStatement statement = conn.prepareStatement(auditDeletes.toString());
  ...
  /**
   * Collect all the simple constraints from the qs This will get all qsName to value
   *
   * @param qs
   */
