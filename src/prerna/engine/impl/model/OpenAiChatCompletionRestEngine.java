...
public class OpenAiChatCompletionRestEngine extends AbstractRESTModelEngine {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(OpenAiChatCompletionRestEngine.class);
  ...
  public void open(Properties smssProp) throws Exception {
    ...
    if (this.modelName == null || (this.modelName = this.modelName.trim()).isEmpty()) {
      throw new IllegalArgumentException(
          "This model requires a valid value for " + Constants.MODEL);
    }

    String maxTokens = this.smssProp.getProperty(Constants.MAX_TOKENS);
    if (maxTokens == null || (maxTokens = maxTokens.trim()).isEmpty()) {
      throw new IllegalArgumentException(
          "Please define the models max tokens with " + Constants.MAX_TOKENS);
    }
    ...
    if (this.smssProp.getProperty(PROVIDER) != null
        && !(this.smssProp.getProperty(PROVIDER).trim().isEmpty())) {
  ...
  protected AskModelEngineResponse askCall(
    ...
    if (fullPrompt != null) {
      ...
      try {
        messages =
            gson.fromJson(jsonString, new TypeToken<List<Map<String, String>>>() {}.getType());
      ...
      IModelEngineResponseHandler modelResponse =
          postRequestStringBody(
              this.endpoint,
              this.headersMap,
              gson.toJson(bodyMap),
      ...
      if (promptTokens >= this.maxTokens) {
        ...
        for (int i = messageChain.size() - 1; i >= 0; i--) {
          ...
          if (tokenCounter - message.getTokenCount() >= 0) {
            tokenCounter -=
                message.getTokenCount(); // Decrement tokenCounter by the message's token count
          } else {
            ...
            if (i + 1 < messageChain.size()) {
              ...
              messages.setMessagesTokenCount(
                  messages.getMessagesTokenCount() - tokensToRemoveFromCount);
      ...
      try {
        tokens =
            getCountTokenScript(
                insight.getPyTranslator(),
                messages.getTokenizerVarName(),
  ...
  private ConversationChain createMessageList(String question, String context, Insight insight) {
    ...
    if (this.keepsConversationHistory()) {
      ...
      if (this.conversationHisotry.containsKey(insightId)) {
        ...
        if (this.keepInputOutput()) {
          List<Map<String, Object>> convoHistoryFromDb =
              ModelInferenceLogsUtils.doRetrieveConversation(insight.getUserId(), insightId, "ASC");
          for (Map<String, Object> record : convoHistoryFromDb) {
            ...
            if (record.get("MESSAGE_TYPE").equals("RESPONSE")) {
              Integer tokens =
                  getCountTokenScript(
                      insight.getPyTranslator(), conversationChain.getTokenizerVarName(), message);
      ...
      //
      //                                          StringBuilder createVarScript = new
      // StringBuilder(tokenizerVarName);
    ...
    if (context != null) {
      Integer tokens =
          getCountTokenScript(
              insight.getPyTranslator(), conversationChain.getTokenizerVarName(), context);
      conversationChain.setContext(context, tokens);
    }

    Integer tokens =
        getCountTokenScript(
            insight.getPyTranslator(), conversationChain.getTokenizerVarName(), question);
  ...
  private Map<String, Object> adjustHyperParameters(Map<String, Object> hyperParameters) {
    ...
    if (hyperParameters.get("frequency_penalty") != null
      ...
      throw new IllegalArgumentException(
          "The hyperparameter frequency_penalty is set but is not a number.");
    }

    if (hyperParameters.get("logit_bias") != null
        && !(hyperParameters.get("logit_bias") instanceof Map)) {
      throw new IllegalArgumentException("The hyperparameter logit_bias is set but is not a map.");
    }

    if (hyperParameters.get("logprobs") != null
        && !(hyperParameters.get("logprobs") instanceof Boolean)) {
      throw new IllegalArgumentException(
          "The hyperparameter logprobs is set but is not a boolean.");
    }

    if (hyperParameters.get("top_logprobs") != null
        && !(hyperParameters.get("top_logprobs") instanceof Integer)) {
      throw new IllegalArgumentException(
          "The hyperparameter top_logprobs is set but is not an integer.");
    }

    if (hyperParameters.get("max_tokens") != null
        || hyperParameters.get("max_new_tokens") != null) {
      ...
      if (hasMaxNewTokens) {
        ...
        if (!(hyperParameters.get("max_tokens") instanceof Integer)) {
          throw new IllegalArgumentException(
              "The hyperparameter max_tokens is set but is not an integer.");
        }
      }
    }
    ...
    if (hyperParameters.get("presence_penalty") != null
      ...
      throw new IllegalArgumentException(
          "The hyperparameter presence_penalty is set but is not a number.");
    }

    if (hyperParameters.get("response_format") != null
        && !(hyperParameters.get("response_format") instanceof Map)) {
      throw new IllegalArgumentException(
          "The hyperparameter response_format is set but is not a map.");
    }
    ...
    if (hyperParameters.get("stop") != null
        && !(hyperParameters.get("stop") instanceof String)
        && !(hyperParameters.get("stop") instanceof List)) {
      throw new IllegalArgumentException(
          "The hyperparameter stop is set but is neither a string nor a list.");
    }

    if (hyperParameters.get("temperature") != null
        && !(hyperParameters.get("temperature") instanceof Number)) {
      throw new IllegalArgumentException(
          "The hyperparameter temperature is set but is not a number.");
    }
    ...
    if (hyperParameters.get("tool_choice") != null
        && !(hyperParameters.get("tool_choice") instanceof String)
  ...
  /**
   * This class is used to process the responses from the Open AI Chat Completions API. This is a
   * static class so that an object can be instantiated without the need to first instantiate {@code
   * OpenAiChatCompletionRestEngine}.
   *
   * <p>https://platform.openai.com/docs/guides/text-generation/chat-completions-api <br>
  ...
  public static class ChatCompletion implements IModelEngineResponseHandler {
    ...
    public class Choice {
      ...
      public LogProbsContent getLogprobs() {
        return logprobs;
      }

      public void setLogprobs(LogProbsContent logprobs) {
    ...
    public class TokenLogProb {
      ...
      public List<TopLogProb> getTop_logprobs() {
        return top_logprobs;
      }

      public void setTop_logprobs(List<TopLogProb> top_logprobs) {
    ...
    public Map<String, Object> getModelEngineResponse() {
      ...
      if (partialResponses != null && !partialResponses.isEmpty()) {
        ...
        modelEngineResponse.put(
            AbstractModelEngineResponse.NUMBER_OF_TOKENS_IN_RESPONSE, partialResponses.size());
      } else {
        modelEngineResponse.put(
            AbstractModelEngineResponse.NUMBER_OF_TOKENS_IN_PROMPT, this.getPromptTokens());
        modelEngineResponse.put(
            AbstractModelEngineResponse.NUMBER_OF_TOKENS_IN_RESPONSE, this.getResponseTokens());
      }
    ...
    public Object getResponse() {
      return this.response != null
          ? this.response
          : this.getChoices().get(0).getMessage().getContent();
    }
  ...
  /**
   * This class is used to process the partial responses for the Open AI Chat Completions API. This
   * is a static class so that an object can be instantiated without the need to first instantiate
   * {@code OpenAiChatCompletionRestEngine}.
   *
   * <p>https://platform.openai.com/docs/api-reference/chat
   */
  ...
  private class ConversationChain {
    ...
    @Expose
    private ChatCompletionMessage context =
        new ChatCompletionMessage("system", "You are a helpful assistant.", 6 + TOKENS_PER_MESSAGE);

    ;

    @Expose private List<ChatCompletionMessage> messages = new ArrayList<>();
    ...
    public void setContext(String context, Integer contentTokenCount) {
      this.context =
          new ChatCompletionMessage("system", context, contentTokenCount + TOKENS_PER_MESSAGE);
    ...
    public void addModelResponse(String content, Integer tokens) {
      this.messages.add(
          new ChatCompletionMessage("assistant", content, tokens + TOKENS_PER_MESSAGE));
      this.messagesTokenCount += tokens + TOKENS_PER_MESSAGE;
    }
  ...
  public class ConversationChainSerializer implements JsonSerializer<ConversationChain> {
    ...
    public JsonElement serialize(
        ConversationChain src, Type typeOfSrc, JsonSerializationContext context) {
