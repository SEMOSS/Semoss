...
public class EditWorkspaceReactor extends AbstractReactor {
	...
	public EditWorkspaceReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.WORKSPACE_ID.getKey(), NAME, DESCRIPTION, SYSTEM_PROMPT,
				SHARING_ENABLED, IS_ACTIVE, ReactorKeysEnum.VECTORDB.getKey(), ReactorKeysEnum.FUNCTION.getKey()};
	...
	public NounMetadata execute() {
		...
		int permissionLevel = Math.min(hasOwnerPermission ? AccessPermissionEnum.OWNER.getId() : Integer.MAX_VALUE,
				currentlyShared
						? ModelInferenceLogsUtils.getWorkspaceSharePermission(workspaceId, user,
								AccessPermissionEnum.OWNER.getId(), AccessPermissionEnum.EDIT.getId())
						: Integer.MAX_VALUE);
		int neededPermissionLevel = ((currentlyShared ^ sharingEnabled) || (currentlyActive ^ isActive))
		...
		try {
			ModelInferenceLogsUtils.updateWorkspaceEntry(workspaceId, workspaceName, workspaceDescription,
					workspaceSystemPrompt, sharingEnabled, isActive, workspaceResources);
			if (!currentlyShared && sharingEnabled) {
				if (AbstractSecurityUtils.containsProjectId(workspaceId)) {
					...
					ModelInferenceLogsUtils.createWorkspaceProject(user, workspaceId,
							ModelInferenceLogsUtils.WORKSPACE_PROJECT_TAG + "_" + workspaceId);
				}
