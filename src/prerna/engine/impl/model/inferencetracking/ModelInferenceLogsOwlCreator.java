...
public class ModelInferenceLogsOwlCreator {
	...
	private void createColumnsAndTypes(AbstractSqlQueryUtil queryUtil) {
		...
		this.agentColumns = Arrays.asList(Pair.with("AGENT_ID", "VARCHAR(50)"), Pair.with("AGENT_NAME", "VARCHAR(255)"),
				Pair.with("DESCRIPTION", "VARCHAR(255)"), Pair.with("AGENT_TYPE", "VARCHAR(50)"),
				Pair.with("AUTHOR", "VARCHAR(255)"), Pair.with("DATE_CREATED", TIMESTAMP_DATATYPE_NAME));
		...
		this.roomColumns = Arrays.asList(Pair.with("INSIGHT_ID", "VARCHAR(50)"), // insight id the room was made with
				Pair.with("ROOM_ID", "VARCHAR(50)"), Pair.with("ROOM_NAME", "VARCHAR(255)"),
				Pair.with("ROOM_CONTEXT", CLOB_DATATYPE_NAME), Pair.with("USER_ID", "VARCHAR(255)"),
				Pair.with("USER_NAME", "VARCHAR(255)"), Pair.with("USER_EMAIL_ID", "VARCHAR(50)"),
				Pair.with("AGENT_TYPE", "VARCHAR(50)"), Pair.with("AGENT_ID", "VARCHAR(50)"),
				Pair.with("IS_ACTIVE", BOOLEAN_DATATYPE_NAME), Pair.with("DATE_CREATED", TIMESTAMP_DATATYPE_NAME),
				Pair.with("UPDATED_AT", TIMESTAMP_DATATYPE_NAME), Pair.with("PROJECT_ID", "VARCHAR(50)"),
				Pair.with("PROJECT_NAME", "VARCHAR(255)"), Pair.with("MODEL_ID", "VARCHAR(255)"), // NEW
		...
		this.messageColumns = Arrays.asList(Pair.with("MESSAGE_ID", "VARCHAR(50)"),
				Pair.with("MESSAGE_TYPE", "VARCHAR(50)"), Pair.with("MESSAGE_DATA", BLOB_DATATYPE_NAME),
				Pair.with("MESSAGE_TOKENS", INTEGER_DATATYPE_NAME), Pair.with("MESSAGE_METHOD", "VARCHAR(50)"),
				// Pair.with("MESSAGE_SEPARATOR", "VARCHAR(50)"),
				Pair.with("RESPONSE_TIME", DOUBLE_DATATYPE_NAME), Pair.with("DATE_CREATED", TIMESTAMP_DATATYPE_NAME),
				Pair.with("AGENT_ID", "VARCHAR(50)"), Pair.with("MODEL_ID", "VARCHAR(50)"),
				Pair.with("INSIGHT_ID", "VARCHAR(50)"), Pair.with("ROOM_ID", "VARCHAR(50)"),
				Pair.with("SESSIONID", "VARCHAR(255)"), Pair.with("USER_ID", "VARCHAR(255)"),
				Pair.with("USER_NAME", "VARCHAR(255)"), Pair.with("USER_EMAIL_ID", "VARCHAR(50)"));

		this.feedbackColumns = Arrays.asList(Pair.with("MESSAGE_ID", "VARCHAR(50)"),
				Pair.with("MESSAGE_TYPE", "VARCHAR(50)"), Pair.with("FEEDBACK_TEXT", CLOB_DATATYPE_NAME),
				Pair.with("FEEDBACK_DATE", TIMESTAMP_DATATYPE_NAME), Pair.with("RATING", BOOLEAN_DATATYPE_NAME));

		this.workspaceColumns = Arrays.asList(Pair.with("WORKSPACE_ID", "VARCHAR(255)"),
				Pair.with("OWNER", "VARCHAR(255)"), Pair.with("NAME", "VARCHAR(255)"),
				Pair.with("DESCRIPTION", CLOB_DATATYPE_NAME), Pair.with("SYSTEM_PROMPT", CLOB_DATATYPE_NAME),
				Pair.with("SHARING_ENABLED", BOOLEAN_DATATYPE_NAME), Pair.with("IS_ACTIVE", BOOLEAN_DATATYPE_NAME),
				Pair.with("DATE_CREATED", TIMESTAMP_DATATYPE_NAME), Pair.with("DATE_UPDATED", TIMESTAMP_DATATYPE_NAME));

		this.workspaceResourceColumns = Arrays.asList(Pair.with("WORKSPACE_RESOURCE_ID", "VARCHAR(255)"),
				Pair.with("WORKSPACE_ID", "VARCHAR(255)"), Pair.with("RESOURCE_ID", "VARCHAR(255)"),
				Pair.with("RESOURCE_TYPE", "VARCHAR(255)"), Pair.with("RESOURCE_SUBTYPE", "VARCHAR(255)"));

		this.allSchemas = Arrays.asList(Pair.with("AGENT", agentColumns), Pair.with("ROOM", roomColumns),
				Pair.with("MESSAGE", messageColumns), Pair.with("FEEDBACK", feedbackColumns),
				Pair.with("WORKSPACE", workspaceColumns), Pair.with("WORKSPACE_RESOURCE", workspaceResourceColumns));
	}
	...
	// Pair.with(Arrays.asList("AGENT"),
	// Arrays.asList("AGENT_ID")))),
