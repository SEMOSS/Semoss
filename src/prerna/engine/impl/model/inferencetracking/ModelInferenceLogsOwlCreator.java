...
public class ModelInferenceLogsOwlCreator {
	...
	private List<Pair<String, String>> memoryColumns = null;
	private List<Pair<String, String>> memoryEmbeddingColumns = null;
	...
	static {
		...
		conceptsRequired.add("MEMORY");
		conceptsRequired.add("MEMORY_EMBEDDING");
	}
...
//			);
		...
		// MEMORY — stores user-scoped persistent facts, preferences, summaries, and episodic notes
		// that the AI recalls across conversations for personalized context
		this.memoryColumns = Arrays.asList(
				Pair.with("MEMORY_ID", "VARCHAR(50)"),           // unique memory identifier (UUID)
				Pair.with("USER_ID", "VARCHAR(255)"),            // owning user (from auth token)
				Pair.with("ROOM_ID", "VARCHAR(50)"),             // source room (nullable — manual memories have no room)
				Pair.with("MEMORY_TYPE", "VARCHAR(50)"),         // category: FACT, PREFERENCE, SUMMARY, EPISODE
		...
		// MEMORY_EMBEDDING — vector embeddings for semantic recall (Phase 2)
		// One embedding per memory, generated by the user's configured embedding model
		this.memoryEmbeddingColumns = Arrays.asList(
				Pair.with("MEMORY_ID", "VARCHAR(50)"),           // FK to MEMORY.MEMORY_ID
				Pair.with("EMBEDDING", BLOB_DATATYPE_NAME),      // serialized float[] vector
				Pair.with("MODEL_ID", "VARCHAR(255)")            // embedding model used (for re-embedding if model changes)
			);
		...
		this.allSchemas = Arrays.asList(
				...
				Pair.with("MEMORY", memoryColumns),
				Pair.with("MEMORY_EMBEDDING", memoryEmbeddingColumns)
			);
	}
