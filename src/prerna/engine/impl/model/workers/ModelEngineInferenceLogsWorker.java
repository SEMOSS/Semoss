...
public class ModelEngineInferenceLogsWorker implements Runnable {
	...
	public ModelEngineInferenceLogsWorker(String messageId, String messageMethod, IEngine engine, String insightId,
			String projectContextId, String projectId, User user, String sessionId, String roomId, String context,
			String prompt, Object fullPrompt, Integer promptTokens, ZonedDateTime inputTime, String response,
			Integer responseTokens, ZonedDateTime responseTime) {
	...
	public void run() {
		...
		if (!ModelInferenceLogsUtils.doCheckRoomExists(roomId)) {
			...
			ModelInferenceLogsUtils.doCreateNewConversation(insightId, roomId, roomName, null, userId, userName,
					userEmail, agentType, engine.getEngineId(), true, projectId, projectName, null);
		}
		...
		if (keepInputOutput) {
			ModelInferenceLogsUtils.doRecordMessage(this.messageId, INPUT, this.prompt, this.messageMethod,
					this.promptTokens, millisecondsDouble, this.inputTime, this.engine.getEngineId(), insightId,
					this.sessionId, this.roomId, userId, userName, userEmail);
			ModelInferenceLogsUtils.doRecordMessage(this.messageId, RESPONSE, this.response, this.messageMethod,
					this.responseTokens, millisecondsDouble, this.responseTime, this.engine.getEngineId(), insightId,
					this.sessionId, this.roomId, userId, userName, userEmail);
		} else {
			ModelInferenceLogsUtils.doRecordMessage(this.messageId, INPUT, null, this.messageMethod, this.promptTokens,
					millisecondsDouble, this.inputTime, this.engine.getEngineId(), insightId, this.sessionId,
					this.roomId, userId, userName, userEmail);
			ModelInferenceLogsUtils.doRecordMessage(this.messageId, RESPONSE, null, this.messageMethod,
					this.responseTokens, millisecondsDouble, this.responseTime, this.engine.getEngineId(), insightId,
					this.sessionId, this.roomId, userId, userName, userEmail);
		}
	}
