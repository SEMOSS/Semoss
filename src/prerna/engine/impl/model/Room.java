...
public class Room {
  ...
  public Room(
    ...
    this.roomFolderPath =
        Utility.getBaseFolder() + File.separator + "room" + File.separator + this.room_id;
    parseMessages();
  }
   ...
   * @return
   */
  public ResponseMessage ask(
      InputMessage msg, String systemMessage, IModelEngine modelEngine, String parentMessageId) {
    // if a specific system message is sent to use, overwrite the existing in the db
    if (systemMessage != null) {
      this.systemMessage = systemMessage;
      ModelInferenceLogsUtils.setRoomContext(
    ...
    if (!abstractModel.keepInputOutput || !useHistory) {
      ...
      AskModelEngineResponse llmResponse =
          modelEngine.askRoom(msg.getInputPrompt(), this.getSystemMessage(), this, kwArgMap);
      ResponseMessage response =
          ResponseMessage.Builder.fromAskModelEngineResponse(llmResponse).build();
    ...
    try {
      ...
      String messageJsonString =
          MessageUtils.getMessageHistoryFromMessageId(this.messages, msg.getMessageId());
    ...
    if ((prevRoomName == null || prevRoomName.trim().isEmpty())
      ...
      ModelInferenceLogsUtils.llm2_updateRoomMessages(
          room_id, insight.getUser().getPrimaryLoginToken().getId(), getMessagesAsString());
    }
  ...
  public AskModelEngineResponse addToolExecutionResult(
    ...
    if (matchingToolCall == null) {
      throw new IllegalArgumentException(
          "No matching tool_call_id in last assistant tool_calls response.");
    }

    // 3. Add tool execution message
    AbstractMessage toolExecution =
        InputMessage.toolExecution(
            this, toolCallId, toolName, toolExecutionResponse, toolParameterValues);
    ...
    for (int i = lastToolRespIdx + 1; i < messages.size(); ++i) {
      ...
      if (m.getMessageType() == MessageType.INPUT_TOOL_EXEC) {
        ...
        if (toolCallId != null) answeredIds.add(toolCallId);
      }
    }
  ...
  public void setMessages(List<AbstractMessage> messagesList) {
    this.messages.clear();
    if (messagesList != null) this.messages.addAll(messagesList);
  }
