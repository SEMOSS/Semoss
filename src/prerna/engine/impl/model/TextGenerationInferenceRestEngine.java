...
public class TextGenerationInferenceRestEngine extends AbstractRESTModelEngine {
	...
	public void open(Properties smssProp) throws Exception {
		...
		if (maxTokensStr != null && !(maxTokensStr = maxTokensStr.trim()).isEmpty()) {
			try {
				...
				classLogger.error("Invalid maxTokens input for engine "
		...
		if (maxInputTokensStr != null && !(maxInputTokensStr = maxInputTokensStr.trim()).isEmpty()) {
			try {
				...
				classLogger.error("Invalid maxInputTokens input for engine "
						+ SmssUtilities.getUniqueName(this.engineName, this.engineId));
				classLogger.error(Constants.STACKTRACE, e);
			}
		}
	...
	public AskModelEngineResponse askCall(String question, Object fullPrompt, String context, Insight insight,
		...
		IModelEngineResponseHandler modelResponse = postRequestStringBody(this.endpoint, this.headersMap,
				new Gson().toJson(bodyMap), ContentType.APPLICATION_JSON, null, null, null, stream,
				TextGenPayload.class, insightId);
	...
	public static class TextGenPayload implements IModelEngineResponseHandler {
		...
		public class GeneratedTextItem {
			...
			public String toString() {
				return "GeneratedTextItem{" + "generatedText='" + generated_text + '\'' + ", details=" + details + '}';
			}
			...
			public class GeneratedTextDetails {
				...
				@Override
				public String toString() {
					return "GeneratedTextDetails{" + "finishReason='" + finish_reason + '\'' + ", generatedTokens="
							+ generated_tokens + ", seed=" + seed + ", prefill=" + prefill + ", tokens=" + tokens + '}';
				}
				...
				public class Token {
					...
					@Override
					public String toString() {
						return "Token{" + "id=" + id + ", text='" + text + '\'' + ", logprob=" + logprob + ", special="
								+ special + '}';
					}
				}
			}
		...
		public Map<String, Object> getModelEngineResponse() {
			...
			if (partialResponses != null && !partialResponses.isEmpty()) {
				...
				modelEngineResponse.put(AbstractModelEngineResponse.NUMBER_OF_TOKENS_IN_RESPONSE,
						this.getResponseTokens());
			}

			// copied from open ai chat completion rest engine
			// need to do the travelsal and add null checks
			// if (this.getChoices() != null && this.getChoices().get(0).getLogprobs() !=
			// null) {
  ...
  ////////////////////////////////////////////////////////////////////////////////
	...
	// TextGenerationInferenceRestEngine engine = new
	// TextGenerationInferenceRestEngine();
	//
	// engine.open("C:\\workspace\\Semoss_Dev\\model\\Platypus-70B__4801422a-5c62-421e-a00c-05c6a9e15de8.smss");
	//
	// Insight insight = new Insight();
	// AskModelEngineResponse response = engine.ask("tell me a joke", null, insight,
	// new
	// HashMap<>());
