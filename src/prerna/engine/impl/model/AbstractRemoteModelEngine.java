...
/**
 * This is a class used to be extended by models running on KServe. It contains methods for
 * deploying the model to the cluster and making HTTP requests to the model. See
 * https://github.com/SEMOSS/kubernetes-model-scaler for Kubernetes model scaling.
 */
public class AbstractRemoteModelEngine extends AbstractModelEngine {
  ...
  private String createServiceUrl(Services service) throws Exception {
    ...
    if (service == Services.KMS_SHUTDOWN) {
      if (devPortForwarding) {
        serviceUrl =
            String.format(
                "http://localhost:8000/api/v2/stop?model_id=%s&model=%s",
                this.engineId, this.model);
      } else if (kmsIngressUrl != null) {
        serviceUrl =
            String.format(
                "%sapi/v2/stop?model_id=%s&model=%s", kmsIngressUrl, this.engineId, this.model);
      } else {
        ...
        serviceUrl =
            String.format(
                "http://%s/api/v2/stop?model_id=%s&model=%s",
  ...
  public boolean initiateAndWaitForDeployment(long timeoutMs) throws Exception {
        ...
        CompletableFuture.runAsync(
            () -> {
              try (CloseableHttpClient httpClient = HttpClients.createDefault()) {
                ...
                StringEntity entity =
                    new StringEntity(payload.toString(), ContentType.APPLICATION_JSON);
    ...
    while (System.currentTimeMillis() - startTime < warmingTimeout) {
      if (zkClient.isModelWarming(this.engineId)) {
        classLogger.info(
            "Model {} has entered warming state, waiting for activation", this.engineId);
        // Canceling the HTTP request if still running
        deploymentFuture.cancel(true);
        return zkClient.waitForModelActive(
            this.engineId, timeoutMs - (System.currentTimeMillis() - startTime));
      }
  ...
  public String shutdownModelRequest() throws Exception {
    ...
    try (CloseableHttpClient httpClient =
      ...
      try (CloseableHttpResponse response = httpClient.execute(httpPost)) {
        ...
        String message =
            String.format("Successfully initiated shutdown for model %s", this.engineId);
      ...
      String error =
          String.format(
              "Error making shutdown request for model %s: %s", this.engineId, e.getMessage());
  ...
  protected JSONObject makeModelRequest(JSONObject requestPayload) throws Exception {
    ...
    // Always wait for active state whether it started as WARMING or just became WARMING after
    // deployment
  ...
  private JSONObject makeInferenceRequest(String clusterIp, JSONObject requestPayload)
      throws Exception {
    ...
    RequestConfig requestConfig =
        RequestConfig.custom().setConnectTimeout(30000).setSocketTimeout(900000).build();
    ...
    try (CloseableHttpClient httpClient =
      ...
      StringEntity entity =
          new StringEntity(kservePayload.toString(), ContentType.APPLICATION_JSON);
      httpPost.setEntity(entity);
  ...
  protected AskModelEngineResponse askCall(
    ...
    try {
      ...
      return implementingEngineClass.askCall(
          question, fullPrompt, context, insight, hyperParameters);
    } catch (Exception e) {
  ...
  @Override
  protected InstructModelEngineResponse instructCall(
      String task,
      String context,
    ...
    return implementingEngineClass.instructCall(
        task, context, projectData, insight, hyperParameters);
  }
}
