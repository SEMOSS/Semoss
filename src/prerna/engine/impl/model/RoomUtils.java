...
/**
 * Utility methods for fetching and managing Room objects. - createRoomIfNotExists: creates (if
 * needed) and returns a Room - getOrLoadRoom: looks up or loads room to memory hash, but never
 * creates a Room
 */
public final class RoomUtils {
  ...
  public static Room createRoomIfNotExists(
      String roomId, Insight insight, IModelEngine modelEngine, String question) {
  ...
  public static Room createRoomIfNotExists(
    ...
    if (!roomExistsInDB) {
      ...
      String roomName =
          (question != null) ? question.substring(0, Math.min(question.length(), 100)) : null;
      ModelInferenceLogsUtils.doCreateNewConversation(
          ...
          workspaceId,
          options);
      // Always get the loaded room object (avoiding any skipping, ensures in-memory cache is
      // filled)
  ...
  public static Room getOrLoadRoom(String roomId, Insight insight) {
    ...
    room =
        ModelInferenceLogsUtils.getRoomById(
            roomId, insight.getUser().getPrimaryLoginToken().getId());
  ...
  private static void updateRoom(Room room, Insight insight) {
    List<Map<String, Object>> output =
        ModelInferenceLogsUtils.doRetrieveConversation(
            insight.getUser().getPrimaryLoginToken().getId(), room.getId(), "ASC", -1, -1);
  ...
  private static AbstractMessage convertLegacyMessage(Room room, Map<String, Object> entry) {
    ...
    if ("INPUT".equals(type)) {
      ...
      ResponseMessage rm =
          ResponseMessage.builder().withText(data).withType(MessageType.RESPONSE_TEXT).build();
  ...
  public static List<AbstractMessage> getPagedMessages(
      List<AbstractMessage> messages, String sortOrder, int offset, int limit) {
