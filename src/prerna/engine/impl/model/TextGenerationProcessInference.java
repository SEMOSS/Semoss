...
public class TextGenerationProcessInference extends TextGenerationEngine {
	...
	public void startServer(int port) {
		...
		// processBuilder.inheritIO(); // Redirect the subprocess's standard error and
		// output to the
		// current process
	...
	public void close() {
		if (process != null) {
			...
			try {
				// Create a separate thread to wait for the process to exit with a timeout of 5
				// seconds
				...
				// Optionally, you can try to forcibly terminate the process if it's still
				// running after
				// graceful shutdown
				if (process.isAlive()) {
	...
	private static ImmutableMap<String, String> getLauncherArgs() {
		return new ImmutableMap.Builder<String, String>().put(Constants.GPU_ID, "CUDA_VISIBLE_DEVICES")
				.put(Constants.MODEL, "--model-id").put("REVISION", "--revision").put("SHARDED", "--sharded")
				.put(Constants.NUM_GPU, "--num-shard").put("QUANTIZE", "--quantize")
				.put("TRUST_REMOTE_CODE", "--trust-remote-code")
				.put("MAX_CONCURRENT_REQUESTS", "--max-concurrent-requests").put("MAX_BEST_OF", "--max-best-of")
				.put("MAX_STOP_SEQUENCES", "--max-stop-sequences").put("MAX_INPUT_LENGTH", "--max-input-length")
				.put("MAX_TOTAL_TOKENS", "--max-total-tokens").put("MAX_BATCH_SIZE", "--max-batch-size")
				.put("WAITING_SERVED_RATIO", "--waiting-served-ratio")
				.put("MAX_BATCH_TOTAL_TOKENS", "--max-batch-total-tokens")
				.put("MAX_WAITING_TOKENS", "--max-waiting-tokens").put("PORT", "--port")
				.put("SHARD_UDS_PATH", "--shard-uds-path").put("MASTER_ADDR", "--master-addr")
				.put("MASTER_PORT", "--master-port").put("HUGGINGFACE_HUB_CACHE", "--huggingface-hub-cache")
				.put("WEIGHTS_CACHE_OVERRIDE", "--weights-cache-override")
				.put("DISABLE_CUSTOM_KERNERLS", "--disable-custom-kernels").put("JSON_OUTPUT", "--json-output")
				.put("OTLP_ENDPOINT", "--otlp-endpoint").put("CORS_ALLOW_ORIGIN", "--cors-allow-origin")
				.put("WATERMARK_GAMMA", "--watermark-gamma").put("WATERMARK_DELTA", "--watermark-delta").build();
	}
}
