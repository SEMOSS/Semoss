...
public class TextGenerationProcessInference extends TextGenerationEngine {
  ...
  public void open(String smssFilePath) {
    try {
      if (smssFilePath != null) {
        classLogger.info(
            "Loading Model - " + Utility.cleanLogString(FilenameUtils.getName(smssFilePath)));
  ...
  public void startServer(int port) {
    ...
    if (this.launchArguments.containsKey(Constants.GPU_ID)) {
      command.add(
          possibleInputs.get(Constants.GPU_ID) + "=" + this.launchArguments.get(Constants.GPU_ID));
    ...
    // processBuilder.inheritIO(); // Redirect the subprocess's standard error and output to the
    // current process
  ...
  public void close() {
    if (process != null) {
      ...
      try {
        ...
        // Optionally, you can try to forcibly terminate the process if it's still running after
        // graceful shutdown
        if (process.isAlive()) {
