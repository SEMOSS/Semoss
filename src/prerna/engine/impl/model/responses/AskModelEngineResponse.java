...
public abstract class AskModelEngineResponse<T> extends AbstractModelEngineResponse<T> {
	...
	public static AskModelEngineResponse<?> fromMap(Object responseObject) {
		...
		Map<TokenTypeEnum, Integer> additionalTokenTypeCounts = new HashMap<>();
        for (TokenTypeEnum tokenType : TokenTypeEnum.getAdditionalTokenTypes()) {
            Integer count = getTokens(modelResponse.get(tokenType.getResponseMapKey()));
            if (count != null) {
                additionalTokenTypeCounts.put(tokenType, count);
            }
        }
		...
		if (TOOL.equals(messageType)) {
			if (response instanceof List) {
				...
				if (!responseList.isEmpty()) {
					askResponse = new AskToolModelEngineResponse((List<Map<String, Object>>) responseList,
							tokensInPrompt, tokensInResponse, additionalTokenTypeCounts, providerUsageMap);
				} else {
			...
			if (response instanceof List) {
				...
				askResponse = AskImageModelEngineResponse.getOpenAIImageResponse(imageList, tokensInPrompt,
						tokensInResponse, additionalTokenTypeCounts, providerUsageMap);
