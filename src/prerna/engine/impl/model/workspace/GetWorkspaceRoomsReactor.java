...
public class GetWorkspaceRoomsReactor extends AbstractReactor {
	...
	public GetWorkspaceRoomsReactor() {
		this.keysToGet = new String[] { WORKSPACE_ID, ReactorKeysEnum.LIMIT.getKey(), ReactorKeysEnum.OFFSET.getKey(), 
	...
	public NounMetadata execute() {
		...
		if(!hasPermission && (Boolean.TRUE != currentlyShared || !ModelInferenceLogsUtils.isWorkspaceSharedWithUser(workspaceId, user))) {
			throw new IllegalArgumentException("User unauthorized to perform this operation");
		}
		
		Map<String, Object> rooms = ModelInferenceLogsUtils.getWorkspaceRoomsForUser(workspaceId, user, limit, offset, filters, sorts);
