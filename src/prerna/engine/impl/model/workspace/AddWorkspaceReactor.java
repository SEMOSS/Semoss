...
public class AddWorkspaceReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		try {
			ModelInferenceLogsUtils.createNewWorkspaceEntry(workspaceId, owner.getPrimaryLoginToken().getId(), workspaceName, workspaceDescription, workspaceSystemPrompt, sharingEnabled);
		} catch(Exception e) {
		...
		if(sharingEnabled) {
			try {
				ModelInferenceLogsUtils.createWorkspaceProject(owner, workspaceId, ModelInferenceLogsUtils.WORKSPACE_PROJECT_TAG + "_" + workspaceId);
			} catch(Exception e) {
