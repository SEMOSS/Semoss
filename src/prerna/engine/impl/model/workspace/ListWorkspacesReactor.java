...
public class ListWorkspacesReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		Map<String, Object> workspaces = ModelInferenceLogsUtils.getWorkspaceEntriesForUser(user, limit, offset, filters, sorts, sharedWorkspaceIds);
		if(workspaces == null) {
	...
	private Set<String> getSharedWorkspaceIds(User user) {
		...
		List<Map<String, Object>> projectInfo = SecurityProjectUtils.getUserProjectList(user, null, 
				false, false, projectMetadataFilter, null, null, null, null);
		Set<String> sharedWorkspaceIds = projectInfo.stream().map(info -> (String) info.get("project_id")).collect(Collectors.toSet());
	...
	private GenRowFilters getFilters() {
		...
		if(grf != null && !grf.isEmpty()) {
			List<IQueryFilter> filters = QSFilterToTypedConverter.convertFilters(grf.getFilters(), "subquery", TYPES_FOR_SUBQUERY_COLUMNS);
