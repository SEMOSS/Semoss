...
public class LocalPythonCustomEmbeddingsFunctionEngine extends LocalPythonFunctionEngine
		implements
			ICustomEmbeddingsFunctionEngine {
	...
	public boolean canProcessDocument(File fileToProcess) {
		if (this.canProcessFunctionName == null
				|| (this.canProcessFunctionName = this.canProcessFunctionName.trim()).isEmpty()) {
			return true;
		}
		...
		callMaker.append("(fileToProcess=").append(PyUtils.determineStringType(fileToProcess)).append("')");
	...
	public int processDocument(String outputCsvFilePath, File fileToProcess, Map<String, Object> parameters) {
		...
		callMaker.append("(outputCsvFilePath=").append(PyUtils.determineStringType(outputCsvFilePath))
				.append(",fileToProcess=").append(PyUtils.determineStringType(fileToProcess)).append(",parameters=")
				.append(PyUtils.determineStringType(parameters)).append(")");
