...
public class WebScrapeEngine extends JsonAPIEngine {
  ...
  public DATABASE_TYPE getDatabaseType() {
    return IDatabaseEngine.DATABASE_TYPE.WEB;
  }
  ...
  public Object execQuery(String query) {
    ...
    try {
      ...
      String method = "get";
      if (smssProp.containsKey(INPUT_METHOD)) method = smssProp.getProperty(INPUT_METHOD);

      InputStream is = null;

      if (method.equalsIgnoreCase("get")) is = doGetI(url);
      else is = doPostI(new Hashtable());
      ...
      List<String[]> values =
          collectValues(thisTable.getElementsByTag("tr"), headers, Arrays.asList(selectedIndex));
  ...
  public String[] getHeaders(String url, Map aliasMap) {
    ...
    try {
      String method = "get";
      if (aliasMap.containsKey(INPUT_METHOD)) method = (String) aliasMap.get(INPUT_METHOD);
  ...
  public List collectValues(
      Elements values, String[] headerList, List<Integer> selectedHeaderIndicies) {
    int rowIndex = 0;
    ...
    for (rowIndex = 1; rowIndex < values.size(); rowIndex++) {
      ...
      for (Integer headerIndex : selectedHeaderIndicies) {
        ...
        if (rowColValue.containsKey(headerIndex)) {
          ...
          } else rowSpanCount.put(headerIndex, numrows);
      ...
      if (dataFilled) valueList.add(oneRow);
    }
  ...
  public String[] getWebTypes(List<String[]> values) {
    ...
    if (values.size() <= 10) rowLimit = values.size();
    ...
    for (int rowIndex = 0; rowIndex < rowLimit; rowIndex++) {
      ...
      for (int colIndex = 0; colIndex < oneRow.length; colIndex++) {
        ...
        if (NumberUtils.isDigits(thisValue)) intCount[colIndex] = intCount[colIndex] + 1;
        else stringCount[colIndex] = stringCount[colIndex] + 1;
      }
    }

    for (int colIndex = 0; colIndex < intCount.length; colIndex++) {
      if (stringCount[colIndex] > intCount[colIndex]) typeList[colIndex] = "String";
      else typeList[colIndex] = "int";
    }
