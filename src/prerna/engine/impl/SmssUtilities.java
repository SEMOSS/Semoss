...
public class SmssUtilities {
  ...
  /**
   * This method is to update the legacy structure where the OWL was not assumed within the assets
   * folder of an engine Marking as deprecated to remove for next major release
   *
   * @param smssFilePath
   * @param engineId
   * @param prop
   */
  @Deprecated
  public static Properties updateOWLSmssLegacyStructure(
      String smssFilePath, String engineId, Properties prop) {
  ...
  public static File getInsightsRdbmsFile(Properties prop) {
    ...
    String rdbmsInsights =
        Utility.normalizePath(baseFolder)
            + DIR_SEPARATOR
  ...
  public static File getSysTapJnl(Properties prop) {
    ...
    return new File(
        Utility.normalizePath(
            jnlLocation.replace(ENGINE_REPLACEMENT, getUniqueName(engineName, engineId))));
  }
  ...
  public static File getRdfFile(Properties prop) {
    ...
    String engineFolder =
        EngineUtility.getSpecificEngineBaseFolder(
            IEngine.CATALOG_TYPE.DATABASE, getUniqueName(prop));
  ...
  public static File getDataFile(Properties prop) {
    ...
    return new File(
        Utility.normalizePath(
            dataFile.replace(ENGINE_REPLACEMENT, getUniqueName(engineName, engineId))));
  }
  ...
  public static File getTinkerFile(Properties prop) {
    ...
    return new File(
        Utility.normalizePath(
            tinkerFile.replace(ENGINE_REPLACEMENT, getUniqueName(engineName, engineId))));
  }
  ...
  public static File getNeo4jFile(Properties prop) {
    ...
    return new File(
        Utility.normalizePath(
            neo4jFile.replace(ENGINE_REPLACEMENT, getUniqueName(engineName, engineId))));
  }
  ...
  public static File getJanusFile(Properties prop) {
    ...
    return new File(
        Utility.normalizePath(
            janusFile.replace(ENGINE_REPLACEMENT, getUniqueName(engineName, engineId))));
  }

  /**
   * Custom file reader/writer to modify the app name and keep the same order of the smss
   * properties. Need to change the engine alias
   *
   * @param smssFile
   * @param newSmssFile
   * @param newAppName
   * @throws IOException
   */
  public static void changeAppName(String smssFile, String newSmssFile, String newAppName)
      throws IOException {
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        //				String owlLocation = "db" + DIR_SEPARATOR + ENGINE_REPLACEMENT + DIR_SEPARATOR +
        // newAppName
        //						+ "_OWL.OWL";
  ...
  private static String getParamedSmssInsightDatabaseLocation(String rdbmsTypeStr) {
    String connectionUrl =
        Constants.PROJECT_FOLDER
            + DIR_SEPARATOR
            + SmssUtilities.PROJECT_REPLACEMENT
            + DIR_SEPARATOR
            + "insights_database";
  ...
  /**
   * Get the default connection url for the insights database NOTE : ONLY ALLOWING FOR H2 OR SQLITE
   * STORAGE OPTIONS AT THE MOMENT NOTE : THIS IS THE ACTUAL FULL CONNECITON URL TODO: expand how we
   * store this information to be able to keep in another database option / shared database
   *
   * @param appName
   * @return
   */
  private static String getParamedNewInsightDatabaseConnectionUrl(
      RdbmsTypeEnum rdbmsType, String projectId, String projectName) {
   ...
   * @throws IOException
   */
  public static File createTemporaryProjectSmss(
      String projectId,
      String projectName,
      ...
      String gitProvider,
      String gitCloneUrl,
      RdbmsTypeEnum forceInsightDatabaseType)
      throws IOException {
    String projectTempSmssLoc =
        Utility.normalizePath(getProjectTempSmssLoc(projectId, projectName));
    ...
    try {
      ...
      bufferedWriter.write(
          Constants.PROJECT_TYPE + tab + prerna.project.impl.Project.class.getName() + newLine);
      ...
      bufferedWriter.write(
          Constants.RDBMS_INSIGHTS
              + tab
              + getParamedSmssInsightDatabaseLocation(rdbmsTypeStr)
              + newLine);
      ...
      String connectionUrl =
          getParamedNewInsightDatabaseConnectionUrl(rdbmsType, projectId, projectName);
   ...
   * @throws IOException
   */
  public static File createTemporaryAssetAndWorkspaceSmss(
      String projectId, String projectName, boolean isAsset, RdbmsTypeEnum forceInsightDatabaseType)
      throws IOException {
    String baseFolder = Utility.getBaseFolder();
    if (!baseFolder.endsWith("\\") && !baseFolder.endsWith("/")) {
      baseFolder += DIR_SEPARATOR;
  ...
  /**
   * Validate the project name Does validation that: 1) The input is not null/empty 2) That the
   * project folder doesn't exist in the file directory
   *
   * @param user
   * @param projectName
   * @param projectId
   ...
   * @throws IOException
   */
  public static File validateProject(User user, String projectName, String projectId)
      throws IOException {
    ...
    String projectLocation =
        EngineUtility.getSpecificEngineBaseFolder(
            IEngine.CATALOG_TYPE.PROJECT, projectId, projectName);
  ...
  public static void runInsightCreateTableQueries(RDBMSNativeEngine insightEngine) {
    // CREATE TABLE QUESTION_ID (ID VARCHAR(50), QUESTION_NAME VARCHAR(255), QUESTION_PERSPECTIVE
    // VARCHAR(225), QUESTION_LAYOUT VARCHAR(225), QUESTION_ORDER INT, QUESTION_DATA_MAKER
    // VARCHAR(225), QUESTION_MAKEUP CLOB, QUESTION_PROPERTIES CLOB, QUESTION_OWL CLOB,
    // QUESTION_IS_DB_QUERY BOOLEAN, DATA_TABLE_ALIGN VARCHAR(500), QUESTION_PKQL ARRAY)
    ...
    try {
      if (!queryUtil.tableExists(
          insightEngine.getConnection(),
          "QUESTION_ID",
          insightEngine.getDatabase(),
...

    {
      ...
      // CREATE TABLE PARAMETER_ID (PARAMETER_ID VARCHAR(255), PARAMETER_LABEL VARCHAR(255),
      // PARAMETER_TYPE VARCHAR(225), PARAMETER_DEPENDENCY VARCHAR(225), PARAMETER_QUERY
      // VARCHAR(2000), PARAMETER_OPTIONS VARCHAR(2000), PARAMETER_IS_DB_QUERY BOOLEAN,
   ...
   * @return
   */
  public static String unconcealSmssSensitiveInfo(
      String newSmssContent, Properties currentSmssProperties) {
    ...
    for (String key : keysToFind) {
      if (allUpperProps.containsKey(key)
          && allUpperProps.get(key).equals(Constants.SENSITIVE_INFO_MASK)) {
    ...
    CaseInsensitiveProperties allUpperCurrentSmss =
        new CaseInsensitiveProperties(currentSmssProperties);
