...
public class DateManipulationReactor extends AbstractReactor {
  ...
  // how to use
  // Frame() | Select( ...... ) | Filter (DATE_COLUMN_NAME >= DateManipulation("subtract", Date(),
  // 7, "days") ) |
  // different flows
  ...
  private NounMetadata performManip(String type) {
    ...
    return NounMetadata.getErrorNounMessage(
        "Unknown type = '" + type + "' for the date manipulation");
  }
  ...
  private SemossDataType determineReturnOutput() {
    ...
    if (type.equalsIgnoreCase("add")
        || type.equalsIgnoreCase("addition")
        || type.equalsIgnoreCase("subtract")
        || type.equalsIgnoreCase("sub")
        || type.equalsIgnoreCase("subtraction")) {
