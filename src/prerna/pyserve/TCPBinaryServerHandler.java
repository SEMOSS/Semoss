...
public class TCPBinaryServerHandler extends ChannelInboundHandlerAdapter {
	...
	AbstractRJavaTranslator rt = null;
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg)
	{
			...
			if(buf.readableBytes() >= totalBytes)
			{
		    	...
		    	try
		    	{
		    		...
		    		{
			    		PayloadStruct ps = getFinalOutput(ps2);
			    		if(ps != null)
			    			ps2 = ps;
		    		}
	...
	{
		{
			try
			{
				...
				if(ps.methodName.equalsIgnoreCase("EMPTYEMPTYEMPTY"))
					return ps;
				...
				if(ps.engine == PayloadStruct.ENGINE.R)
				{
					try
					{
						Method method = findRMethod(ps.methodName, ps.payloadClasses);
    ...
    public Method findRMethod(String methodName, Class [] arguments)
    {
    ...
    public void setRJavaTranslator(AbstractRJavaTranslator rt)
    {
    	LOGGER.info("Rjava is set to " + rt);
    	this.rt = rt;
    }
}