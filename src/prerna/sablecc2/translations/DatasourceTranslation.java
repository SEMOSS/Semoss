...
public class DatasourceTranslation extends AbstractDatasourceModificationTranslation {
  ...
  public void outAOperation(AOperation node) {
    if (this.currentSourceStatement != null) {
      ...
      Map<String, List<Object>> sourceParams =
          (Map<String, List<Object>>) this.currentSourceStatement.get("params");
  ...
  //				  + "Database(Movie_RDBMS) | Select(Title, Title__MovieBudget, Studio) | Join((Title,
  ...
  // Averageofid ] ) | Group ( Drug ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) |
  // TaskOptions ( { \"0\" : { \"layout\" : \"Pie\" , \"alignment\" : { \"label\" : [ \"Drug\" ] ,
  // \"value\" : [ \"Averageofid\" ] , \"facet\" : [ ] } } } ) | Collect ( 500 ) ) , ( Select ( Drug
  // , id ) .as ( [ Drug , id ] ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) |
  // TaskOptions ( { \"0\" : { \"layout\" : \"Pie\" , \"alignment\" : { \"label\" : [ \"Drug\" ] ,
