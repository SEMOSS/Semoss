...
public class ParamStructSaveRecipeTranslation extends LazyTranslation {
    ...
    for (PRoutine e : copy) {
      ...
      if (expression.contains("Import")) {
        ...
        if (this.importQs == null) {
          ...
          expression =
              PixelUtility.recreateOriginalPixelExpression(
                  expression, this.encodingList, this.encodedToOriginal);
          ...
          if (this.importQs instanceof HardSelectQueryStruct) {
            ...
            try {
              finalQuery =
                  GenExpressionWrapper.transformQueryWithParams(
                      query, thisImportParams, detailsLookup);
            } catch (Exception e1) {
            ...
            newExpr +=
                " | Query(\"<encode>" + finalQuery + "</encode>\") | " + this.importStr + ";";
            ...
            for (IQueryFilter f : currentImportFilters) {
              ...
              for (int i = 0; i < thisImportParams.size(); i++) {
                ...
                if (importParam.getParameterFillType() == PARAMETER_FILL_TYPE.FILTER) {
                  ...
                  modification =
                      QsFilterParameterizeConverter2.modifyFilter(
                          modification, importParam, paramStruct);
            ...
            for (int i = 0; i < thisImportParams.size(); i++) {
              ...
              if (importParam.getParameterFillType() == PARAMETER_FILL_TYPE.FILTER) {
                ...
                if (ParamStruct.PARAM_FILL_USE_ARRAY_TYPES.contains(pStruct.getModelDisplay())
                  ...
                  if (importType == PixelDataType.CONST_INT
                      || importType == PixelDataType.CONST_DECIMAL) {
                    replacement = "<" + pStruct.getParamName() + ">";
                  } else {
                    replacement = "\"<" + pStruct.getParamName() + ">\"";
                  }
                }
                SimpleQueryFilter paramF =
                    new SimpleQueryFilter(
                        new NounMetadata(
                            new QueryColumnSelector(
                                importParam.getTableName() + "__" + importParam.getColumnName()),
            ...
            newExpr +=
                " | "
                    + QsToPixelConverter.getPixel(this.importQs, false)
                    + " | "
                    + this.importStr
                    + ";";
            this.pixels.add(newExpr);
  ...
  public void inAOperation(AOperation node) {
    ...
    if (this.curReactor instanceof DatabaseReactor
        || this.curReactor instanceof FileReadReactor
    ...
    } else if (this.curReactor instanceof ImportReactor
        || this.curReactor instanceof MergeReactor) {
  ...
  protected void deInitReactor() {
    if (curReactor != null) {
      ...
      if (prevReactor != null
          && (prevReactor instanceof ImportReactor || prevReactor instanceof MergeReactor)) {
        importQs =
            (SelectQueryStruct)
                prevReactor.getNounStore().getNoun(PixelDataType.QUERY_STRUCT.getKey()).get(0);
      }
    }
  }
  ...
  // , \"query\" : \"<encode>(<Frame> | UnfilterFrame(<SelectedColumn>));</encode>\" , \"options\" :
  // { } , \"refresh\" : false , \"default\" : true , \"disabledVisuals\" : [ \"Grid\" ,
  ...
  // +
  //				" SELECT Member_Engagement_Tier AS \"Member Engagement
  // Tier\",Percent_of_Members_by_Engagement AS \"Percent of Members by Engagement\",Account_ID AS
  // \"Account ID\",Account_Name AS \"Account Name\"  FROM  (  SELECT MBRSHP.MBR_ENGGMNT_TIER AS
  // Member_Engagement_Tier,case when MAX( TOTL_MBRSHP.TOTL_MDCL_MBR_CVRG_CNT1) = 0 then 0 else
  ...
  // Percent_of_Members_by_Engagement,MBRSHP.ACCT_ID AS Account_ID,MBRSHP.ACCT_NM AS
  // Account_Name,MBRSHP.TM_PRD_NM AS Time_Period  FROM  (  SELECT
  // coalesce(TMBRENGGMNT.ENGGMNT_TIER,'Not Engaged' ) AS MBR_ENGGMNT_TIER,CII_ACCT_PRFL.ACCT_ID AS
  // ACCT_ID,CII_ACCT_PRFL.ACCT_NM AS ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM  FROM
  ...
  // SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID in ('W0004156') and SRC_FLTR_ID in
  // ('4e982e3f-31c4-4518-9805-f43531478a66'))SGMNTN on CII_FACT_CP_ENGGMNT.SGMNTN_DIM_KEY =
  ...
  // CII_FACT_CP_ENGGMNT.ACCT_ID IN ('W0004156')   group by  CII_FACT_CP_ENGGMNT.acct_id,
  ...
  // ENGGMNT_TIER   from DIM_ENGGMNT   \r\n" +
  //				"where DIM_ENGGMNT.TRDTNL_IND= 1   or DIM_ENGGMNT.ENHNCD_IND= 1      union all      select
  // cast('Traditional' as char(20)) as ENGGMNT, cast('Care Coordination' as char(20))  AS
  // ENGGMNT_TIER   from DIM_ENGGMNT   where DIM_ENGGMNT.TRDTNL_IND= 1 ) b   on a.ENGGMNT =
  // b.ENGGMNT    )  TMBRENGGMNT    ON CII_FACT_MBRSHP.ACCT_ID = TMBRENGGMNT.ACCT_ID    and
  // CII_FACT_MBRSHP.MCID=TMBRENGGMNT.MCID    and TM_PRD_FNCTN.TM_PRD_NM =  TMBRENGGMNT.TM_PRD_NM
  // INNER JOIN CII_ACCT_PRFL ON CII_FACT_MBRSHP.ACCT_ID = CII_ACCT_PRFL.ACCT_ID INNER JOIN (Select
  // ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID in
  // ('W0004156') and SRC_FLTR_ID in ('4e982e3f-31c4-4518-9805-f43531478a66'))SGMNTN on
  // CII_FACT_MBRSHP.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and
  // CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID in ('W0004156')   GROUP BY
  // coalesce(TMBRENGGMNT.ENGGMNT_TIER,'Not Engaged'
  // ),CII_ACCT_PRFL.ACCT_ID,CII_ACCT_PRFL.ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM ) AS MBRSHP LEFT JOIN  (
  // SELECT TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM,coalesce(TMBRENGGMNT.ENGGMNT_TIER,'Not Engaged' ) AS
  ...
  //				"LEFT JOIN  (  select a.acct_id, a.MCID,a.TM_PRD_NM, a.ENGGMNT, b.ENGGMNT_TIER   from  (
  ...
  //				"WHEN MAX(DIM_ENGGMNT.TRDTNL_IND) = 0   AND MAX(DIM_ENGGMNT.ENHNCD_IND) = 1 THEN 'Care
  // Coordination'        \r\n" +
  //				"WHEN MAX(DIM_ENGGMNT.TRDTNL_IND) = 0    AND MAX(DIM_ENGGMNT.ENHNCD_IND) = 0   AND
  // MAX(DIM_ENGGMNT.EXPNDD_IND)= 1 THEN 'Comprehensive'        \r\n" +
  ...
  // where ACCT_ID in ('W0004156') and SRC_FLTR_ID in
  // ('4e982e3f-31c4-4518-9805-f43531478a66'))SGMNTN on CII_FACT_CP_ENGGMNT.SGMNTN_DIM_KEY =
  // SGMNTN.SGMNTN_DIM_KEY and CII_FACT_CP_ENGGMNT.ACCT_ID=SGMNTN.ACCT_ID inner join DIM_ENGGMNT
  //   on CII_FACT_CP_ENGGMNT.MBR_ENGGMNT_ID = DIM_ENGGMNT.ENGGMNT_ID JOIN (  SELECT YEAR_CD_NM as
  // TM_PRD_NM,\r\n" +
  ...
  //				"or (dtp.CSTM_STRT_PRIOR_2_MNTH_NBR = 201801 and dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201811) )
  // )  TM_PRD_FNCTN     \r\n" +
  //				"ON CII_FACT_CP_ENGGMNT.ENGGMNT_MNTH_NBR BETWEEN TM_PRD_FNCTN.START_YEAR_MNTH    and
  // TM_PRD_FNCTN. END_YEAR_MNTH   \r\n" +
  //				"WHERE CII_FACT_CP_ENGGMNT.ACCT_ID IN ('W0004156')   group by  CII_FACT_CP_ENGGMNT.acct_id,
  // CII_FACT_CP_ENGGMNT.MCID , TM_PRD_FNCTN.TM_PRD_NM ) a  \r\n" +
  //				"inner join \r\n" +
  //				"(  select   CASE   WHEN DIM_ENGGMNT.TRDTNL_IND= 1 THEN cast('Traditional' as char(20))
  // \r\n" +
  //				"WHEN DIM_ENGGMNT.TRDTNL_IND = 0   AND DIM_ENGGMNT.ENHNCD_IND= 1 THEN cast('Care
  // Coordination' as char(20))   \r\n" +
  //				"WHEN DIM_ENGGMNT.TRDTNL_IND= 0    AND DIM_ENGGMNT.ENHNCD_IND = 0   AND
  ...
  //				"WHEN DIM_ENGGMNT.TRDTNL_IND= 0    AND DIM_ENGGMNT.ENHNCD_IND = 0   AND
  // DIM_ENGGMNT.EXPNDD_IND= 1 THEN cast('Comprehensive' as char(20))   ELSE cast('Not Engaged' as
  // char(20))   END AS ENGGMNT_TIER                         \r\n" +
  //				"from DIM_ENGGMNT      union all      \r\n" +
  //				"select   CASE   WHEN DIM_ENGGMNT.TRDTNL_IND= 1 THEN cast('Traditional' as char(20))   else
  // cast('Care Coordination' as char(20))   end as ENGGMNT , \r\n" +
  //				"cast( 'Comprehensive' as char(20))  AS ENGGMNT_TIER   from DIM_ENGGMNT   where
  // DIM_ENGGMNT.TRDTNL_IND= 1   or DIM_ENGGMNT.ENHNCD_IND= 1      \r\n" +
  //				"union all      \r\n" +
  //				"select cast('Traditional' as char(20)) as ENGGMNT, cast('Care Coordination' as char(20))
  // AS ENGGMNT_TIER   \r\n" +
  //				"from DIM_ENGGMNT   where DIM_ENGGMNT.TRDTNL_IND= 1 ) b   on a.ENGGMNT = b.ENGGMNT    )
  // TMBRENGGMNT    \r\n" +
  //				"ON CII_FACT_MBRSHP.ACCT_ID = TMBRENGGMNT.ACCT_ID    and
  // CII_FACT_MBRSHP.MCID=TMBRENGGMNT.MCID    and TM_PRD_FNCTN.TM_PRD_NM =  TMBRENGGMNT.TM_PRD_NM
  // \r\n" +
  //				"INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG
  // where ACCT_ID in ('W0004156') and SRC_FLTR_ID in
  // ('4e982e3f-31c4-4518-9805-f43531478a66'))SGMNTN on CII_FACT_MBRSHP.SGMNTN_DIM_KEY =
  // SGMNTN.SGMNTN_DIM_KEY and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID
  // in ('W0004156')   GROUP BY TM_PRD_FNCTN.TM_PRD_NM,coalesce(TMBRENGGMNT.ENGGMNT_TIER,'Not
  // Engaged' ),CII_FACT_MBRSHP.ACCT_ID ) AS MBRENGGMNT ON
  // MBRSHP.MBR_ENGGMNT_TIER=MBRENGGMNT.MBR_ENGGMNT_TIER AND MBRSHP.ACCT_ID=MBRENGGMNT.ACCT_ID AND
  // MBRSHP.TM_PRD_NM=MBRENGGMNT.TM_PRD_NM INNER JOIN  (  SELECT TM_PRD_FNCTN.TM_PRD_NM AS
  // TM_PRD_NM,CII_FACT_MBRSHP.ACCT_ID AS ACCT_ID,SUM( case when
  // CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD='001' then CII_FACT_MBRSHP.MBR_CVRG_CNT else 0  end) AS
  ...
  // ('4e982e3f-31c4-4518-9805-f43531478a66'))SGMNTN on CII_FACT_MBRSHP.SGMNTN_DIM_KEY =
  // SGMNTN.SGMNTN_DIM_KEY and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID
  // in ('W0004156')   GROUP BY TM_PRD_FNCTN.TM_PRD_NM,CII_FACT_MBRSHP.ACCT_ID ) AS TOTL_MBRSHP ON
  // MBRSHP.TM_PRD_NM=TOTL_MBRSHP.TM_PRD_NM AND MBRSHP.ACCT_ID=TOTL_MBRSHP.ACCT_ID  GROUP BY
  // MBRSHP.MBR_ENGGMNT_TIER,MBRSHP.ACCT_ID,MBRSHP.ACCT_NM,MBRSHP.TM_PRD_NM ) AS FNL   ORDER BY
  // Member_Engagement_Tier,Percent_of_Members_by_Engagement,Account_ID,Account_Name"
  //				+ "</encode>\" ) | Import ( frame = [ CreateFrame ( frameType = [ R ] , override = [ true ]
  // ) .as ( [ \"External_FRAME887854\" ] ) ] ) ;",
  //
  //				"META | PositionInsightRecipeStep ( positionMap = [ { \"auto\" : false , \"top\" : 24 ,
  // \"left\" : 24 } ] ) ;",
  //				"META |
  // SetInsightConfig({\"panels\":{\"0\":{\"config\":{\"type\":\"golden\",\"backgroundColor\":\"\",\"opacity\":100}}},\"sheets\":{\"0\":{\"order\":0,\"golden\":{\"content\":[{\"type\":\"row\",\"content\":[{\"type\":\"stack\",\"activeItemIndex\":0,\"width\":100,\"content\":[{\"type\":\"component\",\"componentName\":\"panel\",\"componentState\":{\"panelId\":\"0\"}}]}]}]}}},\"sheet\":\"0\"});",
  //		};
  ...
  // ByteArrayInputStream(expression.getBytes("UTF-8"))), expression.length())));
