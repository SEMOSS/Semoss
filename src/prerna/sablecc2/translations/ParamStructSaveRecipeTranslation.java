...
public class ParamStructSaveRecipeTranslation extends LazyTranslation {
		...
		ROUTINE_LOOP : for (PRoutine e : copy) {
			...
			if (expression.contains("Import")) {
				...
				if (this.importQs == null) {
					...
					expression = PixelUtility.recreateOriginalPixelExpression(expression, this.encodingList,
							this.encodedToOriginal);
					...
					if (this.importQs instanceof HardSelectQueryStruct) {
						...
						try {
							finalQuery = GenExpressionWrapper.transformQueryWithParams(query, thisImportParams,
									detailsLookup);
						} catch (Exception e1) {
						...
						for (IQueryFilter f : currentImportFilters) {
							...
							for (int i = 0; i < thisImportParams.size(); i++) {
								...
								if (importParam.getParameterFillType() == PARAMETER_FILL_TYPE.FILTER) {
									...
									modification = QsFilterParameterizeConverter2.modifyFilter(modification,
											importParam, paramStruct);
						...
						for (int i = 0; i < thisImportParams.size(); i++) {
							...
							if (importParam.getParameterFillType() == PARAMETER_FILL_TYPE.FILTER) {
								...
								SimpleQueryFilter paramF = new SimpleQueryFilter(
										new NounMetadata(new QueryColumnSelector(
												importParam.getTableName() + "__" + importParam.getColumnName()),
												PixelDataType.COLUMN),
										comparator, new NounMetadata(replacement, PixelDataType.CONST_STRING));
	...
	protected void deInitReactor() {
		if (curReactor != null) {
			...
			if (prevReactor != null && (prevReactor instanceof ImportReactor || prevReactor instanceof MergeReactor)) {
				importQs = (SelectQueryStruct) prevReactor.getNounStore().getNoun(PixelDataType.QUERY_STRUCT.getKey())
						.get(0);
			}
		}
	}
  ...
  /////////////////////////////////////////////////////////////
	...
	// "Panel ( 0 ) | AddPanelEvents ( { \"onSingleClick\" : { \"Unfilter\" : [ {
	// \"panel\" : \"\"
	// , \"query\" : \"<encode>(<Frame> |
	// UnfilterFrame(<SelectedColumn>));</encode>\" , \"options\" :
	// { } , \"refresh\" : false , \"default\" : true , \"disabledVisuals\" : [
	// \"Grid\" ,
	...
	// Tier\",Percent_of_Members_by_Engagement AS \"Percent of Members by
	// Engagement\",Account_ID AS
	// \"Account ID\",Account_Name AS \"Account Name\" FROM ( SELECT
	// MBRSHP.MBR_ENGGMNT_TIER AS
	// Member_Engagement_Tier,case when MAX( TOTL_MBRSHP.TOTL_MDCL_MBR_CVRG_CNT1) =
	// 0 then 0 else
	...
	// Percent_of_Members_by_Engagement,MBRSHP.ACCT_ID AS Account_ID,MBRSHP.ACCT_NM
	// AS
	// Account_Name,MBRSHP.TM_PRD_NM AS Time_Period FROM ( SELECT
	// coalesce(TMBRENGGMNT.ENGGMNT_TIER,'Not Engaged' ) AS
	// MBR_ENGGMNT_TIER,CII_ACCT_PRFL.ACCT_ID AS
	// ACCT_ID,CII_ACCT_PRFL.ACCT_NM AS ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM
	// FROM
	...
	// SGMNTN_NM,
	// SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID in ('W0004156') and
	// SRC_FLTR_ID in
	// ('4e982e3f-31c4-4518-9805-f43531478a66'))SGMNTN on
	...
	// CII_FACT_CP_ENGGMNT.ACCT_ID IN ('W0004156') group by
	...
	// "where DIM_ENGGMNT.TRDTNL_IND= 1 or DIM_ENGGMNT.ENHNCD_IND= 1 union all
	// select
	// cast('Traditional' as char(20)) as ENGGMNT, cast('Care Coordination' as
	// char(20)) AS
	// ENGGMNT_TIER from DIM_ENGGMNT where DIM_ENGGMNT.TRDTNL_IND= 1 ) b on
	// a.ENGGMNT =
	// b.ENGGMNT ) TMBRENGGMNT ON CII_FACT_MBRSHP.ACCT_ID = TMBRENGGMNT.ACCT_ID and
	// CII_FACT_MBRSHP.MCID=TMBRENGGMNT.MCID and TM_PRD_FNCTN.TM_PRD_NM =
	// TMBRENGGMNT.TM_PRD_NM
	// INNER JOIN CII_ACCT_PRFL ON CII_FACT_MBRSHP.ACCT_ID = CII_ACCT_PRFL.ACCT_ID
	// INNER JOIN (Select
	// ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where
	// ACCT_ID in
	// ('W0004156') and SRC_FLTR_ID in
	// ('4e982e3f-31c4-4518-9805-f43531478a66'))SGMNTN on
	...
	// ),CII_ACCT_PRFL.ACCT_ID,CII_ACCT_PRFL.ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM ) AS
	// MBRSHP LEFT JOIN (
	// SELECT TM_PRD_FNCTN.TM_PRD_NM AS
	...
	// "LEFT JOIN ( select a.acct_id, a.MCID,a.TM_PRD_NM, a.ENGGMNT, b.ENGGMNT_TIER
	// from (
	...
	// "WHEN MAX(DIM_ENGGMNT.TRDTNL_IND) = 0 AND MAX(DIM_ENGGMNT.ENHNCD_IND) = 1
	// THEN 'Care
	// Coordination' \r\n" +
	...
	// "WHERE CII_FACT_CP_ENGGMNT.ACCT_ID IN ('W0004156') group by
	// CII_FACT_CP_ENGGMNT.acct_id,
	...
	// char(20))
	// \r\n" +
	// "WHEN DIM_ENGGMNT.TRDTNL_IND = 0 AND DIM_ENGGMNT.ENHNCD_IND= 1 THEN
	// cast('Care
	...
	// as char(20))
	// AS ENGGMNT_TIER \r\n" +
	// "from DIM_ENGGMNT where DIM_ENGGMNT.TRDTNL_IND= 1 ) b on a.ENGGMNT =
	// b.ENGGMNT )
	// TMBRENGGMNT \r\n" +
	...
	// SGMNTN.SGMNTN_DIM_KEY and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE
	// CII_FACT_MBRSHP.ACCT_ID
	// in ('W0004156') GROUP BY TM_PRD_FNCTN.TM_PRD_NM,CII_FACT_MBRSHP.ACCT_ID ) AS
	// TOTL_MBRSHP ON
	// MBRSHP.TM_PRD_NM=TOTL_MBRSHP.TM_PRD_NM AND MBRSHP.ACCT_ID=TOTL_MBRSHP.ACCT_ID
	// GROUP BY
	// MBRSHP.MBR_ENGGMNT_TIER,MBRSHP.ACCT_ID,MBRSHP.ACCT_NM,MBRSHP.TM_PRD_NM ) AS
	// FNL ORDER BY
	// Member_Engagement_Tier,Percent_of_Members_by_Engagement,Account_ID,Account_Name"
	// + "</encode>\" ) | Import ( frame = [ CreateFrame ( frameType = [ R ] ,
	// override = [ true ]
	// ) .as ( [ \"External_FRAME887854\" ] ) ] ) ;",
	//
	// "META | PositionInsightRecipeStep ( positionMap = [ { \"auto\" : false ,
	// \"top\" : 24 ,
	// \"left\" : 24 } ] ) ;",
	// "META |
	...
	// ParamStructSaveRecipeTranslation translation = new
	// ParamStructSaveRecipeTranslation(in);
	...
	// try {
	// expression = PixelPreProcessor.preProcessPixel(expression.trim(),
	// translation.encodingList,
