...
public class OptimizeRecipeTranslation extends DepthFirstAdapter {
  ...
  ////////////////////////////////////////////////////////////////
	...
	// \"panel\" : \"\"
	// , \"query\" : \"<encode>UnfilterFrame(<SelectedColumn>);</encode>\" ,
	// \"options\" : { } ,
	// \"refresh\" : false , \"default\" : true , \"disabledVisuals\" : [ \"Grid\" ,
	// \"Sunburst\" ] ,
	// \"disabled\" : false } ] } , \"onBrush\" : { \"Filter\" : [ { \"panel\" :
	// \"\" , \"query\" :
	...
	// "Database ( database = [ \"93857bba-5aea-447b-94f4-f9d9179da4da\" ] ) |
	// Select ( Director ,
	// Title , Nominated , Studio , Genre ) .as ( [ Director , Title , Nominated ,
	// Studio , Genre ] )
	// | Join ( ( Title , inner.join , Genre ) , ( Title , inner.join , Nominated )
	// , ( Title ,
	// inner.join , Director ) , ( Title , inner.join , Studio ) ) | Import ( ) ;",
	// "Panel ( 0 ) | SetPanelView ( \"visualization\" ) ;",
	// "Frame ( ) | QueryAll ( ) | AutoTaskOptions ( panel = [ \"0\" ] , layout = [
	// \"Grid\" ] ) |
	// Collect ( 500 ) ;",
	// "Select ( Director , Genre , Nominated , Studio ) .as ( [ Director , Genre ,
	// Nominated ,
	// Studio ] ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) |
	// TaskOptions ( { \"0\" : {
	// \"layout\" : \"Grid\" , \"alignment\" : { \"label\" : [ \"Director\" ,
	// \"Genre\" ,
	// \"Nominated\" , \"Studio\" ] } } } ) | Collect ( 500 ) ;",
	// "Select ( Director , Genre , Nominated ) .as ( [ Director , Genre , Nominated
	// ] ) | With (
	// Panel ( 0 ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"0\" : {
	// \"layout\" : \"Grid\"
	// , \"alignment\" : { \"label\" : [ \"Director\" , \"Genre\" , \"Nominated\" ]
	// } } } ) | Collect
	// ( 500 ) ;",
	// "Select ( Director , Genre ) .as ( [ Director , Genre ] ) | With ( Panel ( 0
	// ) ) | Format (
	// type = [ 'table' ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Grid\" ,
	// \"alignment\" : {
	// \"label\" : [ \"Director\" , \"Genre\" ] } } } ) | Collect ( 500 ) ;",
	// "Select ( Director ) .as ( [ Director ] ) | With ( Panel ( 0 ) ) | Format (
	// type = [ 'table'
	// ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Grid\" , \"alignment\" : {
	// \"label\" : [
	// \"Director\" ] } } } ) | Collect ( 500 ) ;",
	// "Panel ( 0 ) | SetPanelView ( \"visualization\" ,
	// \"<encode>{\"type\":\"echarts\"}</encode>\" ) ;",
	// "if ( ( HasDuplicates ( Studio ) ) , ( Select ( Studio , Count ( Title ) )
	// .as ( [ Studio ,
	// CountofTitle ] ) | Group ( Studio ) | With ( Panel ( 0 ) ) | Format ( type =
	// [ 'table' ] ) |
	// TaskOptions ( { \"0\" : { \"layout\" : \"Column\" , \"alignment\" : {
	// \"label\" : [ \"Studio\"
	// ] , \"value\" : [ \"CountofTitle\" ] , \"facet\" : [ ] } } } ) | Collect (
	// 500 ) ) , ( Select (
	// Studio , Count ( Title ) ) .as ( [ Studio , CountofTitle ] ) | Group ( Studio
	// ) | With ( Panel
	// ( 0 ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"0\" : {
	// \"layout\" : \"Column\" ,
	// \"alignment\" : { \"label\" : [ \"Studio\" ] , \"value\" : [ \"CountofTitle\"
	// ] , \"facet\" : [
	// ] } } } ) | Collect ( 500 ) ) ) ;" ,
	// "Panel ( 0 ) | Clone ( 1 ) ;",
	// "Panel ( 0 ) | Clone ( 2 ) ;",
	// "Select ( Director ) .as ( [ Director ] ) | With ( Panel ( 2 ) ) | Format (
	// type = [ 'table'
	// ] ) | TaskOptions ( { \"2\" : { \"layout\" : \"Grid\" , \"alignment\" : {
	// \"label\" : [
	// \"Director\" ] } } } ) | Collect ( 500 ) ;",
	// // "if ( ( HasDuplicates ( Genre ) ) , ( Select ( Genre , Count ( Title ) )
	// .as ( [ Genre
	// , CountofTitle ] ) | Group ( Genre ) | With ( Panel ( 1 ) ) | Format ( type =
	// [ 'table' ] ) |
	// TaskOptions ( { \"1\" : { \"layout\" : \"Column\" , \"alignment\" : {
	// \"label\" : [ \"Genre\" ]
	// , \"value\" : [ \"CountofTitle\" ] , \"facet\" : [ ] } } } ) | Collect ( 500
	// ) ) , ( Select (
	// Genre , Count ( Title ) ) .as ( [ Genre , CountofTitle ] ) | Group ( Genre )
	// | With ( Panel ( 1
	// ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"1\" : { \"layout\" :
	// \"Column\" ,
	// \"alignment\" : { \"label\" : [ \"Genre\" ] , \"value\" : [ \"CountofTitle\"
	// ] , \"facet\" : [
	// ] } } } ) | Collect ( 500 ) ) ) ;",
	...
	// "Panel(0) | AddPanelColorByValue(name=[\"abc\"], qs=[x],
	// options=[{\"a\":\"b\"}]);",
	// "Panel(0) | RetrievePanelColorByValue(name=[\"abc\"]) | Collect(-1);",
	// "Select ( Genre , Average ( MovieBudget ) ) .as ( [ Genre ,
	// Average_of_MovieBudget ] ) |
	// Group ( Genre ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) |
	// TaskOptions ( { \"0\"
	// : { \"layout\" : \"Column\" , \"alignment\" : { \"label\" : [ \"Genre\" ] ,
	// \"value\" : [
	// \"Average_of_MovieBudget\" ] , \"tooltip\" : [ ] , \"facet\" : [ ] } } } ) |
	// Collect ( 2000
	// );",
	// "Select ( Genre , Average ( Revenue_Domestic ) ) .as ( [ Genre ,
	// Average_of_Revenue_Domestic
	// ] ) | Group ( Genre ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] )
	// | TaskOptions ( {
	// \"0\" : { \"layout\" : \"Line\" , \"alignment\" : { \"label\" : [ \"Genre\" ]
	// , \"value\" : [
	// \"Average_of_Revenue_Domestic\" ] , \"tooltip\" : [ ] , \"facet\" : [ ] },
	// \"layer\": { \"id\":
	// \"1\", \"addYAxis\": true, \"addXAxis\": true } } } ) | Collect ( 2000 );",
	// "Select ( Genre , Average ( Revenue_International ) ) .as ( [ Genre ,
	// Average_of_Revenue_International ] ) | Group ( Genre ) | With ( Panel ( 0 ) )
	// | Format ( type =
	// [ 'table' ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Area\" ,
	// \"alignment\" : { \"label\" :
	// [ \"Genre\" ] , \"value\" : [ \"Average_of_Revenue_International\" ] ,
	// \"tooltip\" : [ ] ,
	// \"facet\" : [ ] }, \"layer\": { \"id\": \"2\", \"addYAxis\": true,
	// \"addXAxis\": true } } } ) |
	// Collect ( 2000 );",
	// "Panel ( 0 ) | Clone ( 3 ) ;",
	...
	// // when we apply the translation, we will change encoded expressions back to
	// their original
	// form
	...
	// }
	// }
	// // we want to run the finalizeExpressionsToKeep method only after all
