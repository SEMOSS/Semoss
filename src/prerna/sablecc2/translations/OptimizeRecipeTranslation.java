...
public class OptimizeRecipeTranslation extends DepthFirstAdapter {
  ...
  private static final Logger logger =
      LogManager.getLogger(OptimizeRecipeTranslation.class.getName());
  ...
  private HashMap<String, Map<String, List<Integer>>> layerMap =
  ...
  // we will add all expressions with no TaskOptions and then add the expressions that we do need
  ...
  // create a variable that will be reset for each expression to indicate whether or not we
  // encountered an instance of TaskOptions
  // this is needed because if we get through a whole expression without hitting TaskOptions then we
  // want to go ahead and add it to expressionsToKeep
  ...
  /**
   * This method overrides caseAConfiguration, adds each expression to the expressionMap, adds
   * expression indexes to the expression map, and updates the index
   *
   * @param node
   */
  @Override
  public void caseARoutineConfiguration(ARoutineConfiguration node) {
    ...
    for (PRoutine e : copy) {
      ...
      // when we put the expression in the expression map, this is when we should change to the
      // unencoded version
      expression =
          PixelUtility.recreateOriginalPixelExpression(expression, encodingList, encodedToOriginal);
      expressionMap.put(index, expression);
      logger.info("Processing " + Utility.cleanLogString(expression) + "at index: " + index);
      e.apply(this);
      // if we made it through all reactors of the expression without ever hitting TaskOptions, go
      // ahead and add the expression to expressionsToKeep
  ...
  /**
   * This method overrides inAOperation and determines the actions to take based on the reactor
   * (TaskOptions and ClosePanel reactors)
   *
   * @param node
   */
  @Override
  public void inAOperation(AOperation node) {
    ...
    if (reactorId.equals("TaskOptions")) {
      ...
      // we need to make sure that each of the panel ids is included in panelMap with the associated
      // expression index
      ...
      if (currentLayers != null && !currentLayers.isEmpty()) {
        LAYER_LOOP:
        for (String layerId : currentLayers.keySet()) {
          // grab the last index
  ...
  /**
   * Add the panel and the current index If first time seeing panel, adds panel key If already seen
   * panel, adds to the set of indices for this panel
   *
   * @param panel
   */
  private void addPanelForCurrentIndex(String panel) {
    ...
    if (panelMap.keySet().contains(panel)) {
      // if the panel DOES already exist in the panelMap, we just need to add the expression id to
      // the set of values
  ...
  /**
   * Add the panel and the current index If first time seeing panel, adds panel key + layer key If
   * already seen panel, adds to the set of indices for this panel
   *
   * @param panel
   */
  ...
  private void addPanelView(String panel, String view) {
    ...
    if (this.panelToPanelView.keySet().contains(panel)) {
      // if the panel DOES already exist in the panelToPanelView, we just need to add the new
      // expression
      // of index to view
  ...
  public List<String> finalizeExpressionsToKeep() {
    ...
    // we want to grab the last expression index from each panel in the panelMap and add this to
    // expressions to keep
  ...
  public List<String> getCachedPixelRecipeSteps() {
    ...
    PANEL_LAYER_MAP_LOOP:
    for (String panelId : layerMap.keySet()) {
        ...
        if (origTaskIndex != null) {
          ...
          if (origLayerTaskIndices != null) {
            ...
            for (String origLayerIndex : origLayerTaskIndices.keySet()) {
              ...
              Integer currentLastTaskForPanel =
                  origMapTaskIndices.get(origMapTaskIndices.size() - 1);
    ...
    if (sheetList.isEmpty()) {
      ...
      cacheRecipe.add(
          "CachedSheet(\""
              + Insight.DEFAULT_SHEET_ID
              + "\", \""
              + Insight.DEFAULT_SHEET_LABEL
              + "\");");
    } else {
  ...
  /**
   * This method is used for adding expressions to the list of expressionsToKeep by first checking
   * to see if the index is already contained we don't want to add an expression index twice
   *
   * @param index
   */
  ...
  public void outAMapNegNum(AMapNegNum node) {
    if (captureMap) {
      ...
      if (type == PixelDataType.CONST_INT) {
        negNum =
            new NounMetadata(
                -1 * ((Number) numNoun.getValue()).intValue(), PixelDataType.CONST_INT);
      } else if (type == PixelDataType.CONST_DECIMAL) {
        negNum =
            new NounMetadata(
                -1.0 * ((Number) numNoun.getValue()).doubleValue(), PixelDataType.CONST_DECIMAL);
      } else {
  ...
  // , \"query\" : \"<encode>UnfilterFrame(<SelectedColumn>);</encode>\" , \"options\" : { } ,
  // \"refresh\" : false , \"default\" : true , \"disabledVisuals\" : [ \"Grid\" , \"Sunburst\" ] ,
  // \"disabled\" : false } ] } , \"onBrush\" : { \"Filter\" : [ { \"panel\" : \"\" , \"query\" :
  // \"<encode>if(IsEmpty(<SelectedValues>), UnfilterFrame(<SelectedColumn>),
  ...
  // \"<encode>{\"app_id\":\"93857bba-5aea-447b-94f4-f9d9179da4da\"}</encode>\" ) ;",
  //				"CreateFrame ( frameType = [ GRID ] ) .as ( [ 'FRAME228199' ] ) ;",
  //				"Database ( database = [ \"93857bba-5aea-447b-94f4-f9d9179da4da\" ] ) | Select ( Director ,
  // Title , Nominated , Studio , Genre ) .as ( [ Director , Title , Nominated , Studio , Genre ] )
  // | Join ( ( Title , inner.join , Genre ) , ( Title , inner.join , Nominated ) , ( Title ,
  // inner.join , Director ) , ( Title , inner.join , Studio ) ) | Import ( ) ;",
  //				"Panel ( 0 ) | SetPanelView ( \"visualization\" ) ;",
  //				"Frame ( ) | QueryAll ( ) | AutoTaskOptions ( panel = [ \"0\" ] , layout = [ \"Grid\" ] ) |
  // Collect ( 500 ) ;",
  //				"Select ( Director , Genre , Nominated , Studio ) .as ( [ Director , Genre , Nominated ,
  // Studio ] ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"0\" : {
  // \"layout\" : \"Grid\" , \"alignment\" : { \"label\" : [ \"Director\" , \"Genre\" ,
  // \"Nominated\" , \"Studio\" ] } } } ) | Collect ( 500 ) ;",
  //				"Select ( Director , Genre , Nominated ) .as ( [ Director , Genre , Nominated ] ) | With (
  // Panel ( 0 ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Grid\"
  // , \"alignment\" : { \"label\" : [ \"Director\" , \"Genre\" , \"Nominated\" ] } } } ) | Collect
  // ( 500 ) ;",
  //				"Select ( Director , Genre ) .as ( [ Director , Genre ] ) | With ( Panel ( 0 ) ) | Format (
  // type = [ 'table' ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Grid\" , \"alignment\" : {
  // \"label\" : [ \"Director\" , \"Genre\" ] } } } ) | Collect ( 500 ) ;",
  //				"Select ( Director ) .as ( [ Director ] ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table'
  // ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Grid\" , \"alignment\" : { \"label\" : [
  // \"Director\" ] } } } ) | Collect ( 500 ) ;",
  //				"Panel ( 0 ) | SetPanelView ( \"visualization\" ,
  // \"<encode>{\"type\":\"echarts\"}</encode>\" ) ;",
  //				"if ( ( HasDuplicates ( Studio ) ) , ( Select ( Studio , Count ( Title ) ) .as ( [ Studio ,
  // CountofTitle ] ) | Group ( Studio ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) |
  // TaskOptions ( { \"0\" : { \"layout\" : \"Column\" , \"alignment\" : { \"label\" : [ \"Studio\"
  // ] , \"value\" : [ \"CountofTitle\" ] , \"facet\" : [ ] } } } ) | Collect ( 500 ) ) , ( Select (
  // Studio , Count ( Title ) ) .as ( [ Studio , CountofTitle ] ) | Group ( Studio ) | With ( Panel
  // ( 0 ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Column\" ,
  // \"alignment\" : { \"label\" : [ \"Studio\" ] , \"value\" : [ \"CountofTitle\" ] , \"facet\" : [
  // ] } } } ) | Collect ( 500 ) ) ) ;" ,
  //				"Panel ( 0 ) | Clone ( 1 ) ;",
  //				"Panel ( 0 ) | Clone ( 2 ) ;",
  //				"Select ( Director ) .as ( [ Director ] ) | With ( Panel ( 2 ) ) | Format ( type = [ 'table'
  // ] ) | TaskOptions ( { \"2\" : { \"layout\" : \"Grid\" , \"alignment\" : { \"label\" : [
  // \"Director\" ] } } } ) | Collect ( 500 ) ;",
  //				//				"if ( ( HasDuplicates ( Genre ) ) , ( Select ( Genre , Count ( Title ) ) .as ( [ Genre
  // , CountofTitle ] ) | Group ( Genre ) | With ( Panel ( 1 ) ) | Format ( type = [ 'table' ] ) |
  // TaskOptions ( { \"1\" : { \"layout\" : \"Column\" , \"alignment\" : { \"label\" : [ \"Genre\" ]
  // , \"value\" : [ \"CountofTitle\" ] , \"facet\" : [ ] } } } ) | Collect ( 500 ) ) , ( Select (
  // Genre , Count ( Title ) ) .as ( [ Genre , CountofTitle ] ) | Group ( Genre ) | With ( Panel ( 1
  // ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"1\" : { \"layout\" : \"Column\" ,
  // \"alignment\" : { \"label\" : [ \"Genre\" ] , \"value\" : [ \"CountofTitle\" ] , \"facet\" : [
  // ] } } } ) | Collect ( 500 ) ) ) ;",
  //				"RunNumericalCorrelation ( attributes = [ MovieBudget, Revenue_Domestic,
  ...
  //				"Select ( Genre , Average ( MovieBudget ) ) .as ( [ Genre , Average_of_MovieBudget ] ) |
  // Group ( Genre ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( { \"0\"
  // : { \"layout\" : \"Column\" , \"alignment\" : { \"label\" : [ \"Genre\" ] , \"value\" : [
  // \"Average_of_MovieBudget\" ] , \"tooltip\" : [ ] , \"facet\" : [ ] } } } ) | Collect ( 2000
  // );",
  //				"Select ( Genre , Average ( Revenue_Domestic ) ) .as ( [ Genre , Average_of_Revenue_Domestic
  // ] ) | Group ( Genre ) | With ( Panel ( 0 ) ) | Format ( type = [ 'table' ] ) | TaskOptions ( {
  // \"0\" : { \"layout\" : \"Line\" , \"alignment\" : { \"label\" : [ \"Genre\" ] , \"value\" : [
  // \"Average_of_Revenue_Domestic\" ] , \"tooltip\" : [ ] , \"facet\" : [ ] }, \"layer\": { \"id\":
  // \"1\", \"addYAxis\": true, \"addXAxis\": true } } } ) | Collect ( 2000 );",
  //				"Select ( Genre , Average ( Revenue_International ) ) .as ( [ Genre ,
  // Average_of_Revenue_International ] ) | Group ( Genre ) | With ( Panel ( 0 ) ) | Format ( type =
  // [ 'table' ] ) | TaskOptions ( { \"0\" : { \"layout\" : \"Area\" , \"alignment\" : { \"label\" :
  // [ \"Genre\" ] , \"value\" : [ \"Average_of_Revenue_International\" ] , \"tooltip\" : [ ] ,
  // \"facet\" : [ ] }, \"layer\": { \"id\": \"2\", \"addYAxis\": true, \"addXAxis\": true } } } ) |
  // Collect ( 2000 );",
  //				"Panel ( 0 ) | Clone ( 3 ) ;",
  ...
  //			}
  //		}
  //		// we want to run the finalizeExpressionsToKeep method only after all expressions have been
  // run
