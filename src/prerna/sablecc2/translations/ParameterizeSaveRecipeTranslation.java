...
public class ParameterizeSaveRecipeTranslation extends LazyTranslation {
  ...
  private static final Logger logger =
      LogManager.getLogger(ParameterizeSaveRecipeTranslation.class);
          ...
          for (IQueryFilter f : currentImportFilters) {
            ...
            for (String colToParam : inputsToParameterize) {
              if (modification.containsColumn(colToParam)) {
                ...
                modification =
                    QsFilterParameterizeConverter.modifyFilter(
                        modification, colToParam, colToComparators);

                // also add the param to source
                if (!this.paramToSource.containsKey(colToParam)) {
                  List<String> qsNames = new Vector<>();
                  QsFilterParameterizeConverter.findSelectorsForAlias(
                      modification, colToParam, qsNames);
                  Map<String, String> inner = new HashMap<String, String>();
  ...
  // , \"query\" : \"UnfilterFrame(%3CSelectedColumn%3E)%3B\" , \"options\" : { } , \"refresh\" :
  // false , \"default\" : true , \"disabledVisuals\" : [ \"Grid\" ] , \"disabled\" : false } ] } ,
  // \"onBrush\" : { \"Filter\" : [ { \"panel\" : \"\" , \"query\" :
  // \"if(IsEmpty(%3CSelectedValues%3E)%2C%20UnfilterFrame(%3CSelectedColumn%3E)%2C%20SetFrameFilter(%3CSelectedColumn%3E%3D%3D%3CSelectedValues%3E))%3B\" , \"options\" : { } , \"refresh\" : false , \"default\" : true , \"disabled\" : false } ] } } ) ; ",
  ...
  //						+ "| Filter ( ( ( DIABETES2__AGE > [ 20 ] )  OR  ( ( DIABETES2__BP_2S != [ null ] )  AND
  // ( DIABETES2__BP_2D != [ null ] ) ) ) ) "
  //						+ "| Import ( frame = [ CreateFrame ( frameType = [ PY ] , override = [ true ] ) .as ( [
  // \"Diabetes_FRAME224822\" ] ) ] ) ;",
