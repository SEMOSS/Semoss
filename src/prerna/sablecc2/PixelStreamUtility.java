...
public class PixelStreamUtility {
        ...
        ps =
            new PrintStream(
                new BufferedOutputStream(
                    new CipherOutputStream(new FileOutputStream(fileToWrite), cipher)),
                false,
                "UTF-8");
      } else {
  ...
  private static void processPixelRunner(PrintStream ps, Gson gson, PixelRunner runner) {
    ...
    if (size > 0) {
      ...
      for (int i = 0; i < delayedMessages.size(); i++) {
        ...
        Pixel pixelObj =
            new Pixel("thread_execution_" + Utility.getRandomString(6), "\"delayed message\";");
   ...
   */
  private static void processNounMetadata(
      Insight in, PrintStream ps, Gson gson, NounMetadata noun, Pixel pixelObj) {
    ...
    if (nounT == PixelDataType.FRAME) {
      ...
      // adding logic to auto send back headers if PixelOperationsType.FRAME_HEADERS_CHANGE is
      // passed back
        ...
        if (task instanceof ConstantDataTask) {
          ...
          try {
            ...
            if (noCache) {
              ...
              if (cw.first()) {
                // some more processing can be saved by not having set query every time and doing it
                // up front
      ...
      // we just have data to send
        ...
        if (obj instanceof Map
            && ((Map) obj).containsKey("type")
            && ((Map) obj).get("type").equals("GRAPH")) {
          ps.print("{");
          Map mapObj = (Map) obj;
  ...
  private static void printMapData(PrintStream ps, Map<String, Object> retData, Gson gson) {
    ...
    ps.print(
        "{\""
            + GraphFormatter.GRAPH_META
            + "\":"
            + gson.toJson(retData.get(GraphFormatter.GRAPH_META)));
