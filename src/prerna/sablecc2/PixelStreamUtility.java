...
public class PixelStreamUtility {
	 ...
	 *            Insight
	 * @param ps
	 *            PrintStream to write output json
	 * @param gson
	 *            Gson utility instance
	 * @param noun
	 *            Nounmetadata to process
	 * @param pixelObj
	 *            Pixel object for the step
	 */
	private static void processNounMetadata(Insight in, PrintStream ps, Gson gson, NounMetadata noun, Pixel pixelObj) {
		...
		if (nounT == PixelDataType.FRAME) {
			...
			// PixelOperationsType.FRAME_HEADERS_CHANGE is
			// passed back
	...
	// /**
	// * Made for testing generation purposes, uncomment if you'd like to generate
	// test output
	// */
	// public static File writePixelDataForTest(PixelRunner runner, File
	// fileToWrite) {
	// Gson gson = GsonUtility.getDefaultGson();
	...
	// test output
	// */
	// private static void processPixelRunnerForTest(PrintStream ps, Gson gson,
	// PixelRunner runner) {
	// Insight in = runner.getInsight();
	// List<NounMetadata> resultList = runner.getResults();
	// List<Pixel> pixelList = runner.getReturnPixelList();
