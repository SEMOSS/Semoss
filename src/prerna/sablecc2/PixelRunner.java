...
public class PixelRunner extends Thread {
   ...
   *
   * @param expression The sequence of semicolon delimited pixel expressions. If just one
   *     expression, still must end with a semicolon
  ...
  public void runPixel(String expression, Insight insight) {
    ...
    expression =
        PixelPreProcessor.preProcessPixel(
            expression.trim(), this.encodingList, this.encodedTextToOriginal);
              ...
              ". Error in syntax around "
                  + expression
                      .substring(
                          ...
                          Math.min(findIndex + 10, expression.length()))
                      .trim();
        }
      }
      // treat this as a META so that FE doesn't record it
      addInvalidSyntaxResult(
          expression,
          new NounMetadata(
              eMessage,
              PixelDataType.INVALID_SYNTAX,
              PixelOperationType.ERROR,
              PixelOperationType.INVALID_SYNTAX),
          false);
    } finally {
      // help clean up
      if (translation != null) {
  ...
  public void addResult(String pixelExpression, NounMetadata result, boolean isMeta) {
    String origExpression =
        PixelUtility.recreateOriginalPixelExpression(
            pixelExpression, this.encodingList, this.encodedTextToOriginal);
  ...
  /**
   * @param expression
   * @return Method to take a string and return the parsed value of the pixel
   */
  public static List<String> parsePixel(String expression)
  ...
  /**
   * @param expression
   * @return returns set of reactors that are not implemented throws exception if pixel cannot be
   *     parsed
   */
  public static Set<String> validatePixel(String expression)
      throws ParserException, LexerException, IOException {
    Parser p =
        new Parser(
            new Lexer(
