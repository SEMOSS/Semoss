...
public class BasicIteratorTask extends AbstractTask {
  ...
  public void setHeaderInfo(List<Map<String, Object>> headerInfo) {
    ...
    if (this.headerInfo != null && this.grabTypesFromWrapper) {
      ...
      if (this.iterator instanceof IRawSelectWrapper) {
        ...
        String[] types =
            Arrays.asList(sTypes).stream()
                .map(p -> p == null ? "STRING" : p)
                .map(p -> p.toString())
                .map(p -> (p.equals("DOUBLE") || p.equals("INT") ? "NUMBER" : p))
                .collect(Collectors.toList())
                .toArray(new String[sTypes.length]);
      ...
      if (this.iterator instanceof IRawSelectWrapper) {
        ...
        for (int i = 0; i < headers.length; i++) {
          ...
          if (sTypes[i] != null) headerMap.put("dataType", sTypes[i].toString());
          else headerMap.put("dataType", SemossDataType.STRING.toString());
  ...
  private void generateIterator() throws Exception {
    ...
    if (qsType == SelectQueryStruct.QUERY_STRUCT_TYPE.ENGINE
      ...
      iterator =
          WrapperManager.getInstance().getRawWrapper(this.qs.retrieveQueryStructEngine(), this.qs);
      ...
      iterator =
          ExcelWorkbookFileHelper.buildSheetIterator(
              (ExcelQueryStruct) this.qs); // new ExcelFileIterator((ExcelQueryStruct) qs);
    } else {
  ...
  private void optimizeFrame(ITableDataFrame dataframe, List<IQuerySort> orderBys) {
    if (dataframe instanceof AbstractRdbmsFrame) {
      ...
      for (int i = 0; i < orderBys.size(); i++) {
        ...
        IQuerySort convertedOrderByS =
            QSAliasToPhysicalConverter.convertOrderByOperation(origOrderS, meta);
  ...
  public void optimizeQuery(int collectNum) throws Exception {
    if (this.isOptimize) {
      if (this.qs != null && !(this.qs instanceof HardSelectQueryStruct)) {
        ...
        if (this.qs.getCombinedOrderBy().isEmpty()
            && !this.qs.getBigDataEngine()
            && setImplicitOrderBy) {
