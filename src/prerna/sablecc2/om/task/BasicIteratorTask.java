...
public class BasicIteratorTask extends AbstractTask {
	...
	public void setHeaderInfo(List<Map<String, Object>> headerInfo) {
		...
		if (this.headerInfo != null && this.grabTypesFromWrapper) {
			...
			if (this.iterator instanceof IRawSelectWrapper) {
				SemossDataType[] sTypes = this.iterator.getTypes();
				String[] types = Arrays.asList(sTypes).stream().map(p -> p == null ? "STRING" : p)
						.map(p -> p.toString()).map(p -> (p.equals("DOUBLE") || p.equals("INT") ? "NUMBER" : p))
	...
	public void close() throws IOException {
		if (this.iterator instanceof IEngineWrapper) {
			IEngineWrapper x = (this.iterator);
			x.close();
		}
	...
	public void optimizeQuery(int collectNum) throws Exception {
		if (this.isOptimize) {
			if (this.qs != null && !(this.qs instanceof HardSelectQueryStruct)) {
				classLogger.info(SelectQueryStruct.getExecutingQueryMessage(this.qs));
				...
				generateIterator();
				classLogger.info("Finished executing the query");
				// we got the iterator
