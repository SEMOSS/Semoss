...
public enum ReactorKeysEnum {
	...
	ACTION_ITEM_ID("actionItemId", 							"Id of a memory action item"),
...
//	APP("app", 													"Name of the app on the local SEMOSS instance"),
	...
	DUE_DATE("dueDate", 										"Due date for an action item"),
	EMAIL_SESSION("emailSession",								"The javax.mail email session object"),
	EMBEDDING_ENGINE_ID("embeddingEngineId", 					"Engine id used to create embeddings for memory duplicate detection"),
	ENABLE("enable",											"Boolean whether to enable"),
	EVENTS_KEY("events", 										"Events map input"),
	EVENT_TYPE("eventType", 									"Memory event type classification (e.g. memory, decision, lesson, error, task)"),
	...
	MEMORY_ENABLED("memoryEnabled", 							"Boolean whether to attach the platform Memory MCP tools to this room"),
	MEMORY_ID("memoryId", 										"Id of a stored memory record"),
	MEMORY_IDS("memoryIds", 									"List of memory ids, e.g. the source memories for a compaction"),
	AGENT_ID("agentId", 										"Id of the agent/workspace associated to a record (ROOM.WORKSPACE_ID - the attached agent persona, not the room's project or underlying model)"),
	MEMORY_PROJECT_ID("projectId", 							"Id of the agent/project a memory or query is scoped to (an agent is a WORKSPACE-type project)"),
	INCLUDE_SUPERSEDED("includeSuperseded", 					"Whether to include memories already folded into a CompactMemories summary (default false - hidden as redundant with the summary)"),
	...
	OWNER("owner", 												"Owner or assignee, e.g. for a memory action item"),
	...
	REASONING_ENGINE_ID("reasoningEngineId", 					"Engine id used as the reasoning model for memory classification"),
	...
	STATUS("status", 											"Status value, e.g. for a memory action item (open/in_progress/blocked/completed/cancelled)"),
