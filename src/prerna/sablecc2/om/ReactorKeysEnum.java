...
public enum ReactorKeysEnum {
  ...
  ADDITIONAL_PIXELS(
      "additionalPixels",
      "Additional pixels to be executed in addition to the pixel steps saved within the insight"),
  ...
  ATTRIBUTES(
      "attributes",
      "List of columns used as properties/characteristics to describe an instance/object"),
  ...
  CLEAN_UP_CLOUD_STORAGE(
      "cleanUpCloudStorage",
      "Boolean whether to clean up the cloud storage account used to persist databases in the cluster (defaults to false)"),
  ...
  COMMAND(
      "command",
      "The shell command to execute. Commands allowed cd, dir, ls, copy, cp, mv, move, del <specific file>, rm <specific file>, git"),
  COMMENT_KEY(
      "comment",
      "This key can represent a string comment, a map containing the data for a given comment, or the id for an existing comment"),
  COMMENT_ID_KEY("commentId", "Unique ID for the insight comment"),
  CONCEPTUAL_NAMES(
      "conceptualNames", "Conceptual names that are defined for the app tables and columns"),
  ...
  CONNECTION_DETAILS(
      "conDetails",
      "Map containing the necessary details to establish a JDBC connection to an external database"),
  CONNECTION_STRING_KEY(
      "connectionString", "JDBC connection string to connect to an external database"),
  ...
  CONTENT_LENGTH(
      "contentLength",
      "Length of the content to chunk information into. Assumes a default of 512 characters"),
  CONTENT_OVERLAP(
      "contentOverlap",
      "Length of the overlap for a specific content chunk to be included in the next chunk - in chars"),
  CONTEXT("context", "The workspace to be set command line. This is the base directory."),
  CUSTOM_SUCCESS_MESSAGE(
      "customSuccessMessage",
      "Defines for certain reactors a custom success message after running"),
  ADDITIONAL_CONNECTION_PARAMS_KEY(
      "connParams", "Additional JDBC connection params to perform connection"),
  ...
  DATE_TIME_FIELD(
      "dateTimeField", "Data time field (SECOND, MINUTE, HOUR, DAY, WEEK, MONTH, YEAR)"),
  DB_DRIVER_KEY(
      "dbDriver",
      "Name of the JDBC driver.  Not all JDBC drivers are open source so make sure you include the driver within the classpath of SEMOSS"),
  ...
  DRY_RUN(
      "dryRun",
      "Boolean whether to perform a dry run of the operation without impacting application state (default is true)"),
  ...
  ENCODED(
      "encoded",
      "Boolean if the input is encoded using <encode></encode> and must be decoded. To be used when the reactor allows encoded and unencoded input based on the complexity of the input"),
  ...
  EXPRESSION(
      "expression",
      "R / Python expression that needs to be dynamically calculated for this variable"),
  ...
  FRAME_TYPE(
      "frameType",
      "Type of frame to generate - grid (sql based frame), graph (frame based on tinkerpop), r (data sits within r, must have r installed to use), native (leverages the database to execute queries)"),
  FUNCTION("function", "The id/name of the function engine to use"),
  FUNCTION_DETAILS(
      "functionDetails",
      "Map containing the necessary details to establish a connection to function engine"),
  ...
  GITLAB_JOB_SCOPE(
      "gitlabJobScope",
      "The scope of GitLab jobs to show. Either one of or an array of the following: created, pending, running, failed, success, canceled, skipped, waiting_for_resource, or manual. All jobs are returned if scope is not provided."),
  ...
  HEADERS(
      "headers",
      "All the headers strings we want to push as part of the header on the excel / powerpoint export"),
  ...
  ID(
      "id",
      "This key can represent the unique id of the insight instance or the unique id of the saved insight relative to the app"),
  ID_TYPE(
      "id_type", "Type of the id for setting param. colum / column_table / column_table_operator"),
  IMAGE("image", "The location of the image file or the encoding of the image as a png"),
  IMAGE_WAIT_TIME(
      "imageWaitTime",
      "Time in ms to wait for the image to be generated on the BE before operations like screenshots"),
  INCLUDE_META_KEY("meta", "Boolean indication (true or false) of whether to retrieve metadata"),
  INCLUDE_USERTRACKING_KEY(
      "userT", "Boolean indication (true or false) of whether to retrieve user tracking metrics"),
  ...
  META_FILTERS(
      "metaFilters",
      "Map containing key-value pairs for filters to apply on the data source / project / insight metadata"),
  META_KEYS(
      "metaKeys", "List of the metadata keys to return with each data source / project / insight"),
  METADATA("metadata", "Additional metadata"),
  METAMODEL("metamodel", "The metamodel map for a database."),
  METAMODEL_ADDITIONS(
      "metamodelAdd",
      "Basic metamodel containing {tableName:{columnName:columnType}} for a database"),
  METAMODEL_DELETIONS(
      "metamodelDelete",
      "Basic metamodel containing {tableName:[column1Name, column2Name]} for database columns to be deleted."),
  ...
  MODEL_DETAILS(
      "modelDetails",
      "Map containing the necessary details to establish a connection to model engine"),
  MUSTACHE("mustache", "Boolean true/false if the html passed in is a mustache template"),
  MUSTACHE_VARMAP(
      "mustacheVars",
      "Map containing the replacement values for a mustache tempalte. Most reactors will require mustache=true for this to be utilized"),
  ...
  NO_META(
      "noMeta", "Don't return any additional metadata around the data source / project / insight"),
  ...
  OPERATORU(
      "operatoru",
      "Unique operator to use for identifying this filter such as > , < = != etc. For instance this can be a = 1 AND a = 2 in which case the first = would be and.left.="),
  ...
  OUTPUT_FILE_PATH(
      "outputFilePath",
      "Output file path to be used to destinguish if a reactor already has filePath passed in for an input file"),
  OVERRIDE(
      "override",
      "Indicates if the existing value will be overridden or if a new value will be created"),
  ...
  PANEL_VIEW_KEY(
      "panelView", "Text that specifies what is displayed in the panel (e.g., \"visualization\")"),
  ...
  PAYLOAD_CLASSES(
      "payloadClasses", "Payload Classes as a list. This needs to correspond with the payload"),
  PDF_PAGE_NUMBERS("pdfPageNumbers", "Boolean to add the page numbers to the pdf"),
  PDF_PAGE_NUMBERS_IGNORE_FIRST(
      "pdfPageNumbersIgnoreFirst",
      "Boolean to include the page numbers on teh first page of the pdf (defaults to true)"),
  PDF_START_PAGE_NUM(
      "pdfStartPageNumbers",
      "Integer containing the start value for the page number (default to 1)"),
  PDF_SIGNATURE_BLOCK(
      "pdfSignatureBlock", "Boolean to add digital signature block in exisitng pdf file"),
  PDF_SIGNATURE_LABEL(
      "pdfSignatureLabel",
      "String containing text to add above the signature block to specify signature label"),
  PERMISSION("permission", "Permission level"),
  PERMISSION_FILTERS(
      "permissionFilters",
      "Additional filter to append based on permission level (1=Owner, 2=Editor, 3=ReadOnly)"),
  ...
  PREDEFINED_PARAM_STRUCT(
      "preDefinedParamStruct", "Map containing the details for a predefined paramter"),
  ...
  QUERY_PART(
      "querypart", "Specify the query part that is being replaced. This will help toward REPL"),
  REACTOR("reactor", "Reactor name"),
  RECIPE("recipe", "Recipe that shows the sequence of pixels resulting in the insight"),
  RECIPE_PARAMETERS(
      "recipeParameters", "Parameters for the recipe to execute on the insight recipe"),
  ...
  RULES_MAP(
      "rulesMap",
      "The map of rules for validation, including information such as the name of the rule, the rule definition, the columns, and the description"),
  ...
  SPACE(
      "space",
      "The space to work with assets (user project space, current insight space, project id space)."),
  ...
  STORAGE_DETAILS(
      "storageDetails",
      "Map containing the necessary details to establish a connection to storage engine"),
  ...
  SYNC_PULL(
      "dual",
      "True/False value to determine if the sync should also pull the latest updates from the repository"),
  SYNC_DATABASE(
      "syncDatabase",
      "True/False value to detetermine if the database should be published with the app"),
  SPLOT("splot", "Seaborn plot expression"),
  SOURCE(
      "source",
      "Include source - defaults to False, if you want to include source trigger to true"),
  ...
  TASK(
      "task",
      "Task object (can retrieve the object by using Task(taskId) where taskId is the unique id for the task)"),
  ...
  URL("url", "The url"),
  USE_APPLICATION_CERT(
      "useApplicationCert",
      "Boolean if we should use the default application certificate when making the request"),
  USE_HISTORY(
      "useHistory",
      "Boolean to determine whether to include the conversation history in the model ask call"),
  USE_PANEL(
      "usePanel",
      "Boolean to export each panel as a separate sheet on exports instead of the entire dashboard"),
  USE_LABEL("useLabel", "Boolean to use the label to query a graph database"),
  VALUE(
      "value",
      "The instance value in a column, or the numeric or string value used in a operation"),
  ...
  VECTORDB_DETAILS(
      "vectorDbDetails",
      "Map containing the necessary details to establish a connection to vector db engine"),
  VENV("venv", "The id/name of the virtual environment engine"),
  VENV_DETAILS(
      "venvDetails",
      "Map containing the necessary details to establish a connection to venv engine"),
