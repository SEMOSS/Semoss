...
public class Expression {
	...
	/**
	 * @return Builds the expression by recursively evaluating its left and right
	 *         sub expressions and returns the string representation which will feed
	 *         directly to java
	 */
	...
	/**
	 * @param expr
	 * @return This function evaluates one side of the expression and returns the
	...
	private String getEvaluatedString(Object expr) {
		...
		// if we have a lambda (Or IReactor) execute and get the string representation
		// of the execution
		...
		// if we have an expression then get the evaluated Expression String from the
		// instance
		// Ex: 3 + Sum(MovieBudget) -> 3 + 1324792349
		else if (expr instanceof Expression) {
			return "(" + ((Expression) expr).getExpression() + ")"; // add the parenthesis to preserve order of
																	// operations
		}
	...
	/**
	 * @return This method will return this expression's inputs as well as the
	 *         inputs of all of its subExpressions
	 */
	 ...
	 *            <p>
	 *            If obj is an expression, add the inputs and go down left and ride
	 *            side If obj is not an expression we do not need to go further
	 */
