...
public class Filter implements JavaExecutable {
	...
	/**
	 * Method to evaluate the filter condition Only works if all inputs are scalar
	 * values (or variables that are scalar values)
	 *
	 * @param planner
	 * @return
	 */
	...
	/**
	 * @return Creates new and stores, OR retrieves existing filter evaluator class
	 *         we need
	 */
	private FilterEvaluator getFilterEvaluator() {
		...
		// else create a new one, store it, return
			...
			// NounMetadata newEvaluator = new NounMetadata(evaluator,
			// PixelDataTypes.CACHED_CLASS);
	...
	/**
	 * @param stringMethod
	 * @return This method is responsible for creating a new filter evaluator from a
	 *         stringified method
	 */
	...
	/**
	 * @return This method builds the stringified method we will need for the filter
	 *         evaluator
	 */
	private String buildMethodString() {
		...
		method.append("if(" + lString + this.comparator + rString + ") {" + "return true;" + "} else {"
				+ "return false;" + "}" + "}");
	...
	/**
	 * Method to get the left hand or right hand side of the filter Currently only
	 * handles expressions or constant values
	 *
	 * <p>
	...
	/**
	 * @param key
	 * @return Stringified code bit to get a string value from the abstract
	 */
	...
	/**
	 * @param key
	 * @return Stringified code bit to get a double value from the abstract
	 */
	...
	/**
	 * @param grs
	 * @return This method sets the values to the evaluator (specifically through
	 *         the abstract)
	 */
