...
public class Filter implements JavaExecutable {
  ...
  /**
   * Method to evaluate the filter condition Only works if all inputs are scalar values (or
   * variables that are scalar values)
   *
   * @param planner
   * @return
   */
  ...
  /**
   * @return Creates new and stores, OR retrieves existing filter evaluator class we need
   */
  ...
  /**
   * @param stringMethod
   * @return This method is responsible for creating a new filter evaluator from a stringified
   *     method
   */
  private FilterEvaluator buildFilterEvaluator(String stringMethod) {
    ...
    CtClass cc =
        pool.makeClass("t" + Utility.getRandomString(12) + ".c" + Utility.getRandomString(12));
    try {
  ...
  /**
   * @return This method builds the stringified method we will need for the filter evaluator
   */
  ...
  /**
   * Method to get the left hand or right hand side of the filter Currently only handles expressions
   * or constant values
   *
   * <p>key is 'LEFT' or 'RIGHT'
   *
   * @param grs
   * @return
   */
  ...
  /**
   * @param grs
   * @return This method sets the values to the evaluator (specifically through the abstract)
   */
