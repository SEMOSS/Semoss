...
public class GenRowStruct implements Serializable {
	...
	public List<NounMetadata> vector = Collections.synchronizedList(new ArrayList<NounMetadata>());
	...
	// I need to run through to find what the input columns are in order for me to
	// run through and add to selectors
	// while, I am doing this for sql expression here, we could replace sql
	// expression and the same story kicks in
	...
	// imagine the case of if where this could be a full operational formula that
	...
	public void addRelation(String leftCol, String joinType, String rightCol, String comparator,
	...
	public List<Object> getValuesOfType(PixelDataType type) {
		List<Object> retVector = new ArrayList<Object>();
		for (NounMetadata noun : vector) {
	...
	public List<NounMetadata> getNounsOfType(PixelDataType type) {
		List<NounMetadata> retVector = new ArrayList<NounMetadata>();
		for (NounMetadata noun : vector) {
			if (noun.getNounType() == type) {
				retVector.add(noun);
			}
	...
	public List<Object> getAllNumericColumns() {
		...
		for (NounMetadata noun : vector) {
			if (noun.getNounType() == PixelDataType.CONST_DECIMAL || noun.getNounType() == PixelDataType.CONST_INT) {
