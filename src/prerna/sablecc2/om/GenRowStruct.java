...
public class GenRowStruct implements Serializable {
  ...
  // I need to run through to find what the input columns are in order for me to run through and add
  // to selectors
  // while, I am doing this for sql expression here, we could replace sql expression and the same
  // story kicks in
  ...
  public List<String> getAllStrValues() {
    ...
    for (NounMetadata noun : vector) {
      if (noun.getNounType() == PixelDataType.COLUMN
          || noun.getNounType() == PixelDataType.CONST_STRING) {
        retVector.add((String) noun.getValue());
      }
    }
    return retVector;
