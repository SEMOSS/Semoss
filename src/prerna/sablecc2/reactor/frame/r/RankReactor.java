...
public class RankReactor extends AbstractRFrameReactor {
	...
	public NounMetadata execute() {
		...
		script.append(colVector).append(",").append(order).
		append(", ties.method = \"dense\", na.last=TRUE)");
		
		//partition by ex.(by=\"Age_Range\")
		String partitionby=this.keyValue.get(PARTITION_BY_COL);
		if (partitionby != null) {
			script.append(",by=\"" + partitionby + "\"" + "]");
		} else {
			script.append("]");
		}
		...
		if(partitionby!=null){
			sortByRankScript.append(partitionby+",");
		}
		...
		for (int i = 0; i < columns.size(); i++) {
			...
			if(grs == null || grs.isEmpty() || i>=grs.size()) {
				...
				if(!grs.get(i).toString().isEmpty() && grs.get(i).toString() != null
					...
					if(grs.get(i).toString().equalsIgnoreCase(ASC) || (grs.get(i).toString().isEmpty()
							|| grs.get(i)==null)) {
						order.append("1L");
					} else {
						order.append("-1L");
