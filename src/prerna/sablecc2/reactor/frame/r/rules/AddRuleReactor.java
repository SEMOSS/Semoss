...
public class AddRuleReactor extends AbstractRFrameReactor {
	...
	public NounMetadata execute() {
		...
		// read the json. this is the file that we will be adding the rules to
		...
		// iterate through the rule list to get each item that we want to add
		for (int i = 0; i < ruleList.size(); i++) {
			Map<String, Object> mapOptions = (Map<String, Object>) ruleList.get(i);
			...
			// in order to validate testRule, we will need to 
			// (1) determine if rule is a regex and if so add 1 additional escape character, where applicable, to make the regex R-syntax acceptable
			// (2) replace the <variableNames> with just the variableNames (remove the brackets)
			...
			// isValid will either be true or false and indicate if the rule is
			// valid based on editrules in r
			...
			if (isValid.equalsIgnoreCase("true")) {
				...
				if (validateRulesTemplate.keySet().contains(name)) {
					throw new IllegalArgumentException(
							"The name of the rule already exists. Please use a different name for the rule.");
				}
		...
		// use the GsonBuilder for proper formatting of the json
