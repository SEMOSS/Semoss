...
public class CalcVarReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		if(this.getNounStore().getNoun(keysToGet[0]) != null)
			vars = (List)this.getNounStore().getNoun(this.keysToGet[0]).getAllValues();
		else
			vars = insight.getAllVars();
		...
		{
			String name = (String)vars.get(varIndex);
			Variable var = insight.getVariable(name);
			// get the variable
			// get the frames
			// get the language
			...
			for(int frameIndex = 0;frameIndex < frameNames.size();frameIndex++)
			{
				...
				if(!oldNew.containsKey(thisFrameName)) // if not already processed. Generate one
				{
					...
					try {
						...
						{
							String newName = ((PandasFrame)frame).createVarFrame();
							oldNew.put(thisFrameName, newName);
						}
						else if(frame instanceof RData)
						*/
					} catch (Exception e) 
					{
