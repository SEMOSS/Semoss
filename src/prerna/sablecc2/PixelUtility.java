...
public class PixelUtility {
   ...
   * @return Returns true if the pixel is a valid pixel that can be executed Returns false otherwise
   * @throws IOException
   ...
   * @throws LexerException
   * @throws IOException
   *     <p>Returns a list of the parsed pixels from the expression
   */
  public static List<String> parsePixel(String pixelExpression)
      throws ParserException, LexerException, IOException {
    List<String> parsed = new Vector<>();
    List<String> encodingList = new ArrayList<>();
    Map<String, String> encodedTextToOriginal = new HashMap<>();
    String processedPixel =
        PixelPreProcessor.preProcessPixel(pixelExpression, encodingList, encodedTextToOriginal);
    ...
    for (int i = 0; i < parsedResults.size(); i++) {
      String origExpression =
          PixelUtility.recreateOriginalPixelExpression(
              parsedResults.get(i), encodingList, encodedTextToOriginal);
  ...
  /**
   * @param value
   * @return Returns the noun for a STRING or NUMBER ONLY if value is an instanceof another object
              ...
              new Lexer(
                  new PushbackReader(
                      new InputStreamReader(
                          new ByteArrayInputStream(pixelString.getBytes(StandardCharsets.UTF_8)),
  ...
  /**
   * @param literal
   * @return input: 'literal' OR "literal" output: literal
   *     <p>input: 'literal OR "literal output: literal
   *     <p>input: literal' OR literal" output: literal
   *     <p>input: literal output: literal
   */
  ...
  /**
   * @param literal
   * @return Determines if the string is a literal
   */
  ...
  /**
   * Checks if recipe is not cacheable True if it is a parameter insight, or grid-delta, etc.
   *
   * @param pixels
   * @return
   */
  ...
  public static boolean isNotCacheable(String pixel) {
    pixel =
        PixelPreProcessor.preProcessPixel(
            pixel, new ArrayList<String>(), new HashMap<String, String>());
    try {
      Parser p =
          new Parser(
              new Lexer(
                  new PushbackReader(
                      new InputStreamReader(
                          new ByteArrayInputStream(pixel.getBytes("UTF-8")), "UTF-8"),
                      pixel.length())));
      ...
      if (eMessage.startsWith("[")) {
        ...
        if (matcher.find()) {
          ...
          eMessage +=
              ". Error in syntax around "
                  + pixel
                      .substring(
                          Math.max(findIndex - 10, 0), Math.min(findIndex + 10, pixel.length()))
                      .trim();
        }
      }
   ...
   * @return Returns the string of the NOT encoded expression allows us to get the expression in its
   *     not encoded form from a mapping of the expression to what it looked like originally
   */
  public static String recreateOriginalPixelExpression(
      String expression, List<String> encodingList, Map<String, String> encodedTextToOriginal) {
  ...
  public static List<Map<String, Object>> getDatasourcesMetadata(User user, String expression) {
    ...
    try {
      expression =
          PixelPreProcessor.preProcessPixel(
              expression, new ArrayList<String>(), new HashMap<String, String>());
      Parser p =
          new Parser(
              new Lexer(
                  new PushbackReader(
                      new InputStreamReader(
                          new ByteArrayInputStream(expression.getBytes("UTF-8")), "UTF-8"),
                      expression.length())));
      Start tree = p.parse();
      // apply the translation.
      tree.apply(translation);
   ...
   * @param replacementOptions List of maps containing "index" and "pixel" which represents the
   *     pixel step to change and the new pixel to put in its place If no index is found and the
   *     size of the list is 1, we will replace the first datasource
   * @return
   */
  public static List<String> modifyInsightDatasource(
      Insight in, String fullRecipe, List<Map<String, Object>> replacementOptions) {
    ...
    try {
      fullRecipe =
          PixelPreProcessor.preProcessPixel(
              fullRecipe, translation.encodingList, translation.encodedToOriginal);
      Parser p =
          new Parser(
              new Lexer(
                  new PushbackReader(
                      new InputStreamReader(
                          new ByteArrayInputStream(fullRecipe.getBytes("UTF-8")), "UTF-8"),
                      fullRecipe.length())));
      Start tree = p.parse();
      // apply the translation.
      tree.apply(translation);
  ...
  /**
   * Determine if an operation op type is an error that requires user input and then a re-run of the
   * insight
   *
   * @param opTypes
   * @return
   ...
   * @return
   */
  public static List<String> parameterizeRecipe(
      Insight currentInsight,
      List<String> recipe,
      List<String> recipeIds,
      List<ParamStruct> params,
      String insightName) {
    Insight in = new Insight();
    ...
    List<Map<String, Object>> insightJsonObject =
        ParamStructToJsonGenerator.generateInsightJsonForParameters(
            insightName, fullRecipe.toString(), params);

    Gson gson = new GsonBuilder().disableHtmlEscaping().create();
    List<String> paramedRecipe =
        new Vector<>(currentInsight.getVarStore().getInsightParameterKeys().size() + 2);
    // if i add parameters here
    appendAddInsightParameter(currentInsight, paramedRecipe);
    paramedRecipe.add("META | AddPanel(0);");
    paramedRecipe.add(
        "META | Panel (0) | SetPanelView(\"param\", \"<encode> {\"beFill\":true, \"json\":"
            + gson.toJson(insightJsonObject)
            + "}</encode>\");");
    return paramedRecipe;
  }

  /**
   * Get additional insight meta recipe steps for maintaining additional metadata in the insight
   *
   * @param in
   * @return
   */
   ...
   */
  public static void appendPositionInsightRecipeStep(
      PixelList pixelList, List<String> additionalSteps) {
  ...
  /**
   * This will return pixel steps to set the current value for the insight parameters NOTE ::: These
   * have placeholders for params to be filled. This will NOT compile as proper pixel until they are
   * filled in
   *
   * @param in
   * @return
  ...
  public static List<String> getSetParamValuePixels(Insight in) {
    ...
    for (String paramKey : params) {
      ...
      if (ParamStruct.PARAM_FILL_USE_ARRAY_TYPES.contains(param.getModelDisplay())
        ...
        additionalSteps.add(
            "META | SetInsightParamValue(paramName=\""
                + paramName
                + "\", paramValue=[<"
                + paramName
                + ">]);");
  ...
  public static List<String> getCachedInsightRecipe(Insight in) {
    ...
    for (String panelId : panels.keySet()) {
      ...
      boolean isVisualizaiton =
          panel.getPanelView() != null && panel.getPanelView().equalsIgnoreCase("visualization");
      if (isVisualizaiton) {
        ...
        if (qsMap != null) {
          for (String layer : qsMap.keySet()) {
            ...
            if (viewPixel != null) {
              ...
              StringBuffer taskPixel =
                  new StringBuffer(QsToPixelConverter.getPixel(qs, true))
                      .append(" | Format(type=['")
                      .append(format.getFormatType())
                      .append("']");
              if (format.getOptionsMap() != null && !format.getOptionsMap().isEmpty()) {
                taskPixel
                    .append(", options=[")
                    .append(gson.toJson(format.getOptionsMap()))
                    .append("])");
              } else {
                taskPixel.append(")");
              }
    ...
    for (String panelId : panels.keySet()) {
      ...
      for (ColorByValueRule cbv : cbvs) {
        cacheRecipe.add(
            "Panel(\""
                + panelId
                + "\") | RetrievePanelColorByValue(name=[\""
                + cbv.getId()
                + "\"]) | Collect(2000);");
      }
    }
  ...
  public static PixelList getOptimizedPixelList(Insight in) {
    ...
    for (String sheetId : sheets.keySet()) {
      ...
      String sheetState =
          (String) InsightUtility.getSheetState(sheets.get(sheetId), "string").getValue();
    ...
    for (String panelId : panels.keySet()) {
      ...
      pList.addPixel(
          new Pixel(
              (newPixelId++) + "", "SetPanelState(\"<encode>" + panelState + "</encode>\");"));
    }

    // add the main of the recipe - data loading / transformations / code blocks
    for (int i = 0; i < numSteps; i++) {
      Pixel pixelObject = insightPixelList.get(i);
      if (pixelObject.isFrameTransformation()
          || pixelObject.isCodeExecution()
    ...
    for (String panelId : panels.keySet()) {
      ...
      if (isVisualizaiton) {
        ...
        List<ColorByValueRule> cbvs = panel.getColorByValue();
        for (ColorByValueRule cbv : cbvs) {
          StringBuffer cbvTask =
              new StringBuffer("Panel(\"")
                  .append(panelId)
  ...
  public static Map<String, Object> generatePipeline(Insight in) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      try {
        pixelString =
            PixelPreProcessor.preProcessPixel(pixelString, encodingList, encodedTextToOriginal);
        Parser p =
            new Parser(
                new Lexer(
                    new PushbackReader(
          ...
          if (matcher.find()) {
            ...
            eMessage +=
                ". Error in syntax around "
                    + pixelString
                        .substring(
                            Math.max(findIndex - 10, 0),
                            Math.min(findIndex + 10, pixelString.length()))
                        .trim();
          }
        }
   ...
   */
  @Deprecated
  public static List<String> getParameterizedRecipe(
      User user, List<String> recipe, List<Map<String, Object>> paramsMap, String insightName) {
    ...
    for (int i = 0; i < numParams; i++) {
      ...
      for (String comparator : comparators) {
        ...
        if (isNumeric) {
          paramViewMap.put(
              "label",
              "Enter value for : " + param + "  " + comparatorDisplay); // + " [user input]");
          paramViewMap.put("displayType", "number");
        } else {
          paramViewMap.put(
              "label",
              "Select values for : " + param + "  " + comparatorDisplay); // + " [user input]");
        ...
        String paramQ =
            "("
                ...
                + " ?like \"<"
                + jsonParamName
                + "_Search>\") | Sort(columns=["
                + physicalQs
                + "], sort=[asc]) | Iterate()) | Collect(20);";
    ...
    paramedRecipe.add(
        "META | Panel (0) | SetPanelView(\"param\", \"<encode> {\"json\":"
            + gson.toJson(vec)
            + "}</encode>\");");
    return paramedRecipe;
  }
   ...
   * @param newMap
   */
  private static void recursivelyMergeMaps(
      Map<String, Object> mainMap, Map<String, Object> newMap) {
    if (newMap != null) {
