...
public class PixelUtility {
	 ...
	 * @return Returns true if the pixel is a valid pixel that can be executed
	...
	/**
	 * @param value
	 * @return Returns the noun for a STRING or NUMBER ONLY if value is an
	...
	public static boolean isNotCacheable(String pixel) {
		...
		try {
			Parser p = new Parser(new Lexer(new PushbackReader(
					new InputStreamReader(new ByteArrayInputStream(pixel.getBytes("UTF-8")), "UTF-8"),
					pixel.length())));
	 ...
	 * @return Returns the string of the NOT encoded expression allows us to get the
	 ...
	 * @param in
	 * @param fullRecipe
	 *            String containing the original recipe to change
	 * @param replacementOptions
	 *            List of maps containing "index" and "pixel" which represents the
	 *            pixel step to change and the new pixel to put in its place If no
	 *            index is found and the size of the list is 1, we will replace the
	 *            first datasource
	 * @return
	 */
	public static List<String> modifyInsightDatasource(Insight in, String fullRecipe,
		...
		try {
			...
			Parser p = new Parser(new Lexer(new PushbackReader(
					new InputStreamReader(new ByteArrayInputStream(fullRecipe.getBytes("UTF-8")), "UTF-8"),
					fullRecipe.length())));
			Start tree = p.parse();
			// apply the translation.
			tree.apply(translation);
	...
	public static Map<String, Object> generatePipeline(Insight in) {
		...
		for (int i = 0; i < size; i++) {
			...
			try {
				...
				Parser p = new Parser(new Lexer(new PushbackReader(
						new InputStreamReader(new ByteArrayInputStream(pixelString.getBytes("UTF-8")), "UTF-8"),
						pixelString.length())));
