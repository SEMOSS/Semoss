...
public class GreedyTranslation extends LazyTranslation {
	...
	public void caseARoutineConfiguration(ARoutineConfiguration node) {
		...
		for (int pixelstep = 0; pixelstep < size; pixelstep++) {
			if (!this.runner.isCancelRequested()) {
				...
				try {
					...
					if (!(e instanceof AMetaRoutine)) {
						this.pixelObj
								.setStartingFrameHeaders(InsightUtility.getAllFrameHeaders(this.planner.getVarStore()));
					}
					...
					classLogger.error("Error executing pixel step '{}'", e.toString(), ex);
					...
					trackError(e.toString(), this.pixelObj.isMeta(), ex);
					classLogger.error("Unexpected error executing pixel step '{}'", e.toString(), ex);
					planner.addVariable(this.resultKey,
	...
	/**
	 * This is greedy because we get the variable value (which means if its an
	 * embedded reactor we call execute) instead of just passing the noun which
	 * points to a lambda
	 */
	@Override
	public void inAIdWordOrId(AIdWordOrId node) {
		...
		if (this.planner.hasVariable(idInput)) {
			...
			if (curReactor != null) {
				...
				else if (curReactor instanceof SelectReactor || curReactor instanceof GroupReactor
						|| curReactor instanceof QuerySelectorExpressionAssimilator
						|| curReactor instanceof FilterReactor) {
					if (varType == PixelDataType.CONST_STRING || varType == PixelDataType.CONST_INT
							|| varType == PixelDataType.CONST_DECIMAL || varType == PixelDataType.CONST_DATE
							|| varType == PixelDataType.CONST_TIMESTAMP
							// also account for complex expressions!
							|| varType == PixelDataType.TASK || varType == PixelDataType.FORMATTED_DATA_SET) {
			...
			if (curReactor != null) {
				if (curReactor instanceof SelectReactor || curReactor instanceof GroupReactor
						|| curReactor instanceof QuerySelectorExpressionAssimilator
						|| curReactor instanceof FilterReactor) {
	...
	protected void deInitReactor() {
		...
		// c. If it can't be done as native query <--- This will be the last piece I
		// will come to
		...
		// we need some kind of a java class where I can continue to plug what I need to
		// plug
		...
		// - and the end of the block is either a query or it is some kind of a reduce
		// value.. we should possibly keep this as an object as well
		// - When the expression is typically added, it should add the components i.e.
		// the projectors to the block and add the other portion of the code to the
		// block to indicate
		// this is what will be run at every block
		// Question - What decides the separation of blocks
		// a. Reduce operation - This should include the update and insert into the
		// frame if you would like the to think of it that way..
		// b. FlatMap operation - This means something else being added
		// c. It also needs to follow George's logic in terms of breaking it into
		// components
...
//    			this.pixelObj.addReactorInput(prevReactor.getStoreMap());
			...
			// the beginning of the pixel command the beginning of each pipe is an
			// independent routine and doesn't have a parent
			// these will push their output to the result in the pixel planner
			// if a routine does have children, we will push the to the result directly to
			// the parent
			// so the out of the parent can utilize it for its execution
			// ( ex. Select(Studio, Sum(Movie_Budget) where the Sum is a child of the Select
			// reactor )
			...
			if (output != null) {
				...
				if (curReactor != null && !(curReactor instanceof AssignmentReactor)) {
					...
					// otherwise if we have an assignment reactor or no reactor then add the result
					// to the planner
...
//		    		}
					...
					if (prevReactor instanceof ICodeExecution) {
						...
						if (codeExecuted != null && !codeExecuted.isEmpty()) {
							pixelObj.setCodeDetails(true, codeExecuted, ((ICodeExecution) prevReactor).getLanguage(),
	...
	protected void postProcess(String pixelExpression) {
		...
		if (noun != null) {
			...
			this.runner.addResult(pixelExpression, new NounMetadata("no output", PixelDataType.CONST_STRING),
					this.pixelObj.isMeta());
		}
		this.curReactor = null;
	...
	@Override
	protected void postRuntimeErrorProcess(String pixelExpression, NounMetadata errorNoun,
			List<String> unexecutedPixels) {
		errorNoun.addAdditionalReturn(
				new NounMetadata(unexecutedPixels, PixelDataType.CONST_STRING, PixelOperationType.UNEXECUTED_PIXELS));
