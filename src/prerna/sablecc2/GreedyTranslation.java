...
public class GreedyTranslation extends LazyTranslation {
  ...
  /**
   * This is greedy because we get the variable value (which means if its an embedded reactor we
   * call execute) instead of just passing the noun which points to a lambda
   */
  @Override
        ...
        // so that we only push in basic data types
          ...
          if (curReactor instanceof Assimilator) {
            ...
            if (varType == PixelDataType.FRAME) {
              // store the frame as an input
              if (this.pixelObj != null) {
                this.pixelObj.addFrameInput(
                    ((ITableDataFrame) varValue.getValue()).getOriginalName());
              }
            }
          }
        }
  ...
  protected void deInitReactor() {
    ...
    // - and the end of the block is either a query or it is some kind of a reduce value.. we should
    // possibly keep this as an object as well
    // - When the expression is typically added, it should add the components i.e. the projectors to
    ...
    // a. Reduce operation - This should include the update and insert into the frame if you would
    ...
    if (curReactor != null) {
      ...
      // the beginning of the pixel command the beginning of each pipe is an independent routine and
      ...
      if (output != null) {
        ...
        if (curReactor != null && !(curReactor instanceof AssignmentReactor)) {
          ...
          // otherwise if we have an assignment reactor or no reactor then add the result to the
          // planner
  ...
  protected void postProcess(String pixelExpression) {
    ...
    if (noun != null) {
      ...
      this.runner.addResult(
          pixelExpression,
          new NounMetadata("no output", PixelDataType.CONST_STRING),
  ...
  protected void postRuntimeErrorProcess(
    ...
    errorNoun.addAdditionalReturn(
        new NounMetadata(
            unexecutedPixels, PixelDataType.CONST_STRING, PixelOperationType.UNEXECUTED_PIXELS));
