...
public class LazyTranslation extends DepthFirstAdapter {
  ...
  /**
   * Set the end time of this pixel obj Intended for other translations to also override
   *
  ...
  protected void postRuntimeErrorProcess(
      String pixelExpression, NounMetadata errorNoun, List<String> unexecutedPixels) {
    // do nothing
    ...
    for (int pixelstep = 0; pixelstep < size; pixelstep++) {
      ...
      try {
        ...
        if (!(e instanceof AMetaRoutine)) {
          this.pixelObj.setStartingFrameHeaders(
              InsightUtility.getAllFrameHeaders(this.planner.getVarStore()));
        }
  ...
  public void caseASubRoutine(ASubRoutine node) {
    inASubRoutine(node);
    {
      List<PSubRoutineOptions> copy =
          new ArrayList<PSubRoutineOptions>(node.getSubRoutineOptions());
      int counter = 0;
  ...
  public void inAHelpExpr(AHelpExpr node) {
    ...
    NounMetadata noun =
        new NounMetadata(reactor.getHelp(), PixelDataType.CONST_STRING, PixelOperationType.HELP);
    if (curReactor == null) {
  ...
  public void inASubRoutine(ASubRoutine node) {
    ...
    embeddedScriptReactor.setPixel(
        node.getSubRoutineOptions().toString().trim(), node.toString().trim());
  ...
  public void inAMainCommentRoutine(AMainCommentRoutine node) {
    ...
    NounMetadata noun =
        new NounMetadata(comment, PixelDataType.CONST_STRING, PixelOperationType.RECIPE_COMMENT);
    this.planner.addVariable(this.resultKey, noun);
    postProcess(node.toString().trim());
  }
  ...
  public void inAEmbeddedScriptchainExprComponent(AEmbeddedScriptchainExprComponent node) {
    ...
    embeddedScriptReactor.setPixel(
        node.getMandatoryScriptchain().toString().trim(), node.toString().trim());
    initReactor(embeddedScriptReactor);
  }
  ...
  // in the out functions of each of these, it needs to check if the gen row from the child i.e.
  // could be another frame op or something
    ...
    if (this.planner.hasVariable(idInput)) {
      ...
      if (curReactor != null) {
        ...
        if (curReactor instanceof SelectReactor
          ...
          if (varType == PixelDataType.CONST_STRING
              || varType == PixelDataType.CONST_INT
              || varType == PixelDataType.CONST_DECIMAL
              || varType == PixelDataType.CONST_DATE
  ...
  public void inAFractionDecimal(AFractionDecimal node) {
    ...
    if (curReactor != null && !(curReactor instanceof AssignmentReactor)) {
      ...
      // value of the numerical without any extra spaces
      // plain string will always modify to a integer even if we return double value
      curReactor.modifySignature(
          node.toString().trim(), BigDecimal.valueOf(retNum.doubleValue()).toPlainString());
    } else {
  ...
  private IReactor getOrComparison() {
    ...
    } else if (this.curReactor != null && this.curReactor instanceof AbstractQueryStructReactor
        || this.curReactor instanceof AbstractFilterReactor) {
  ...
  /**
   * Use this to determine if we need to make an assimilator Otherwise, figure out what reactor we
   * need to generate
   *
   * @param nodeExpr
   * @param math
  ...
  private boolean requireAssimilator(String nodeExpr, String math, boolean encapsulated) {
    ...
    // we need to aggregate this side of the column expression
      ...
      return false;
        ...
        && ((curReactor instanceof prerna.reactor.qs.selectors.IfReactor)
      ...
      return false;
        ...
        && ((curReactor instanceof prerna.reactor.qs.filter.BetweenReactor)
            || (curReactor.getParentReactor()
                instanceof prerna.reactor.qs.filter.BetweenReactor))) {
      QuerySelectorExpressionAssimilator qAssm = new QuerySelectorExpressionAssimilator();
      qAssm.setMathExpr(math);
      qAssm.setPixel("EXPR", nodeExpr);
      initReactor(qAssm);
  ...
  private void initExpressionToReactor(
      IReactor reactor, String left, String right, String operation) {
  ...
  protected IReactor getReactor(String reactorId, String nodeString) {
    IReactor newReactor =
        ReactorFactory.getReactor(this.insight, reactorId, this.curReactor, this.currentFrame);
  ...
  public void outAMapNegNum(AMapNegNum node) {
    ...
    if (type == PixelDataType.CONST_INT) {
      negNum =
          new NounMetadata(-1 * ((Number) numNoun.getValue()).intValue(), PixelDataType.CONST_INT);
