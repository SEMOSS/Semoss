...
public class LazyTranslation extends DepthFirstAdapter {
	...
	/**
	 * Set the end time of this pixel obj Intended for other translations to also
	 * override
	 *
  ...
  //////////////////////////////////////////
	...
	// in the out functions of each of these, it needs to check if the gen row from
	// the child i.e.
	// could be another frame op or something
	// and evaluates 2 things - is this a reduce operation and is this fully sqlable
	// if either one of these, it needs to execute that before we can execute this
	// one
	...
	public void inAFractionDecimal(AFractionDecimal node) {
		...
		if (curReactor != null && !(curReactor instanceof AssignmentReactor)) {
			...
			// value of the numerical without any extra spaces
			// plain string will always modify to a integer even if we return double value
			curReactor.modifySignature(node.toString().trim(),
					BigDecimal.valueOf(retNum.doubleValue()).toPlainString());
		} else {
	...
	private void processAliasReference(String colName) {
		...
		if (frame != null) {
			...
			if (curReactor != null) {
				...
				this.planner.addVariable(this.resultKey,
  ...
  /////////////////////////////////////////////////////////////
	...
	/**
	 * Use this to determine if we need to make an assimilator Otherwise, figure out
	 * what reactor we need to generate
	 *
	 * @param nodeExpr
	 * @param math
	 * @return
	...
	private boolean requireAssimilator(String nodeExpr, String math, boolean encapsulated) {
		...
		// we need to aggregate this side of the column expression
			...
			initReactor(qAssm);
			return false;
		} else if (curReactor != null && ((curReactor instanceof prerna.reactor.qs.filter.BetweenReactor)
				|| (curReactor.getParentReactor() instanceof prerna.reactor.qs.filter.BetweenReactor))) {
			QuerySelectorExpressionAssimilator qAssm = new QuerySelectorExpressionAssimilator();
			qAssm.setMathExpr(math);
			qAssm.setPixel("EXPR", nodeExpr);
			initReactor(qAssm);
	...
	/**
	 * Return the reactor based on the reactorId. Will first check project specific
	 * reactors
	 *
	 * @param reactorId
	 *            - Reactor id or operation
	 * @param nodeString
	 *            - Full operation pixel string
  ...
  //////////////////////////////////////////////////////////////////
	...
	/*
	 * This section is for processing maps These are all just things to store No
	 * additional processing occurs within the map inputs
	 */
  ...
  //////////////////////////////////////////////////////////////////
	...
	public void defaultIn(Node node) {
		...
		// LOGGER.info("PARENT REACTOR:
		// "+curReactor.getParentReactor().getClass().getSimpleName()
		// + "\n");
		// }
		//
