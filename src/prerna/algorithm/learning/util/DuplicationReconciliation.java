...
public class DuplicationReconciliation {
	...
	public enum ReconciliationMode {
		MEAN, MODE, MEDIAN, MAX, MIN, COUNT
	}
	...
	public Double getReconciliatedValue() {
		if (recValue == null) {
			switch (mode) {
				case MEAN :
					recValue = getMean(values.toArray(), ignoreEmpty);
					break;
				case MEDIAN :
					recValue = getMedian(values.toArray(), ignoreEmpty);
					break;
				case MODE :
					recValue = getMode(values.toArray(), ignoreEmpty);
					break;
				case MAX :
					recValue = getMax(values.toArray(), ignoreEmpty);
					break;
				case MIN :
					recValue = getMin(values.toArray(), ignoreEmpty);
					break;
				case COUNT :
					break;
				default :
	 ...
	 *
	 * @param o
	 *            add object o to array called values.
	 */
	...
	public static Double getMedian(Object[] row, boolean ignoreEmpty) {
		...
		// If Even length array take the average of the two middle except if one of them
		// is non numeric
