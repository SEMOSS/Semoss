...
public class Cluster {
  ...
  public void addToCluster(
      List<Object[]> valuesList, String[] names, boolean[] isNumeric, double factor) {
  ...
  private void addToCategoricalCluster(
      String attributeName, String attributeInstance, double factor) {
    categoricalCluster.addToCluster(attributeName, attributeInstance, factor);
  }

  private void removeFromCategoricalCluster(
      String attributeName, String attributeInstance, double factor) {
   ...
   * @param attributeNames The names of associated for each value the instanceValues array,
   *     essentially the 'column name'
   * @param isNumeric The boolean representing if value of i in the array is numeric
   * @return
   */
  public double getSimilarityForInstance(
      List<Object[]> instanceValues,
   ...
   */
  public double getSimilarityForInstance(
      Object[] instanceValues, String[] attributeNames, boolean[] isNumeric, int instanceIndex) {
    ...
    if (!categoricalValues.isEmpty()) {
      if (isNumeric[instanceIndex]) {
        categoricalSimVal =
            ((double) categoricalValues.size() / totalAttributes)
                * getSimilarityFromCategoricalValues(categoricalValues, categoricalValueNames, -1);
      } else {
        categoricalSimVal =
            ((double) (categoricalValues.size() - 1) / totalAttributes)
                * getSimilarityFromCategoricalValues(
                    categoricalValues, categoricalValueNames, indexToSkip);
      }
    }
    ...
    if (!numericalValues.isEmpty()) {
      if (isNumeric[instanceIndex]) {
        numericalSimVal =
            ((double) (numericalValues.size() - 1) / totalAttributes)
                * getSimilarityFromNumericalValues(
                    numericalValues, numericalValueNames, indexToSkip);
      } else {
        numericalSimVal =
            ((double) numericalValues.size() / totalAttributes)
                * getSimilarityFromNumericalValues(numericalValues, numericalValueNames, -1);
      }
    }
  ...
  private double getSimilarityFromNumericalValues(
      List<Double> numericalValues, List<String> numericalValueNames, int index) {
    return numericalCluster.getSimilarity(numericalValueNames, numericalValues, index);
  }

  private double getSimilarityFromCategoricalValues(
      List<String> categoricalValues, List<String> categoricalValueNames, int index) {
  ...
  public double getClusterSimilarity(Cluster c2, String instanceType) {
    ...
    double numericalClusterSim =
        ((double) numNumeric / totalAttributes)
            * this.numericalCluster.getClusterSimilarity(c2.numericalCluster, instanceType);
    double categoricalClusterSim =
        ((double) numCategorical / totalAttributes)
            * this.categoricalCluster.getClusterSimilarity(c2.categoricalCluster, instanceType);
  ...
  public void setDistanceMode(
      String[] levelNames, IClusterDistanceMode[] distanceMeasures, boolean[] isNumeric) {
