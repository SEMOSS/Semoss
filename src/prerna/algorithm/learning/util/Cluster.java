...
public class Cluster {
	 ...
	 *            The values for the instance for each column
	 * @param attributeNames
	 *            The names of associated for each value the instanceValues array,
	 *            essentially the 'column name'
	 * @param isNumeric
	 *            The boolean representing if value of i in the array is numeric
	 * @return
	 */
	public double getSimilarityForInstance(List<Object[]> instanceValues, String[] attributeNames, boolean[] isNumeric,
			int indexToSkip) {
	...
	public double getSimilarityForInstance(Object[] instanceValues, String[] attributeNames, boolean[] isNumeric,
		...
		if (!categoricalValues.isEmpty()) {
			if (isNumeric[instanceIndex]) {
				categoricalSimVal = ((double) categoricalValues.size() / totalAttributes)
						* getSimilarityFromCategoricalValues(categoricalValues, categoricalValueNames, -1);
			} else {
				categoricalSimVal = ((double) (categoricalValues.size() - 1) / totalAttributes)
						* getSimilarityFromCategoricalValues(categoricalValues, categoricalValueNames, indexToSkip);
			}
		}
		...
		if (!numericalValues.isEmpty()) {
			if (isNumeric[instanceIndex]) {
				numericalSimVal = ((double) (numericalValues.size() - 1) / totalAttributes)
						* getSimilarityFromNumericalValues(numericalValues, numericalValueNames, indexToSkip);
			} else {
				numericalSimVal = ((double) numericalValues.size() / totalAttributes)
	...
	private double getSimilarityFromCategoricalValues(List<String> categoricalValues,
			List<String> categoricalValueNames, int index) {
	...
	public double getClusterSimilarity(Cluster c2, String instanceType) {
		...
		double numericalClusterSim = ((double) numNumeric / totalAttributes)
				* this.numericalCluster.getClusterSimilarity(c2.numericalCluster, instanceType);
		double categoricalClusterSim = ((double) numCategorical / totalAttributes)
