...
public class NumericalCluster {
  ...
  public void partialRemoveFromCluster(
      List<String> attributeName, List<Double> value, double factor) {
    for (int i = 0; i < attributeName.size(); i++) {
  ...
  public double getClusterSimilarity(NumericalCluster c2, String instanceType) {
    ...
    for (String attributeName : this.distanceMeasureForAttribute.keySet()) {
      ...
      if (thisCenterValue == null
          || thisCenterValue.isNaN()
          || centerValue == null
          || centerValue.isNaN()) {
        ...
        similarity =
            similarity
                + (thisNumNull + numNull)
                    / (thisNumVals + numVals + thisNumNull + numNull)
                    * weight;
      } else {
        // using euclidean distance
