...
public class CategoricalCluster extends Hashtable<String, Hashtable<String, Double>> {
	...
	public void removeFromCluster(String attributeName, String attributeInstance, Double value) {
		...
		if (this.containsKey(attributeName)) {
			...
			if (valCount.containsKey(attributeInstance)) { // reduce count by value
				...
				if (currValue < 0) {
					LOGGER.error("WARNING!!! Attribute " + attributeName + " with value " + attributeInstance
							+ " is now a negative value...");
				}
			}
			// instance value cannot be found
			else {
				throw new NullPointerException("Attribute " + attributeName + " with value " + attributeInstance
	...
	public Double getSimilarity(List<String> attributeNames, List<String> attributeInstances, int indexToSkip) {
		...
		// loop through all the categorical properties (each weight corresponds to one
		// categorical
		// property)
		for (int i = 0; i < attributeNames.size(); i++) {
			if (i == indexToSkip) {
			...
			// numOccuranceInCluster contains the number of instances in the cluster that
			// contain the same
