...
public class CategoricalCluster extends Hashtable<String, Hashtable<String, Double>> {
  ...
  public void addToCluster(
      List<String> attributeNames, List<String> attributeInstances, List<Double> values) {
  ...
  public void removeFromCluster(String attributeName, String attributeInstance, Double value) {
      ...
      // instance value cannot be found
        ...
        throw new NullPointerException(
            "Attribute "
                ...
                + " with value "
                + attributeInstance
                + " cannot be found in cluster to remove...");
      }
    }
    ...
    // property not found
      ...
      throw new NullPointerException(
          "Attribute " + attributeName + " cannot be found in cluster to remove...");
    }
  }
  ...
  public Double getSimilarity(
      List<String> attributeNames, List<String> attributeInstances, int indexToSkip) {
    double similarity = 0.0;
    // loop through all the categorical properties (each weight corresponds to one categorical
    // property)
    for (int i = 0; i < attributeNames.size(); i++) {
      if (i == indexToSkip) {
        continue;
      ...
      Hashtable<String, Double> propertyHash =
          this.get(attributeNames.get(i)); // categoryClusterInfo.get(i);
      Collection<Double> valueCollection = propertyHash.values();
      for (Double val : valueCollection) {
        sumProperties += val;
      }

      // numOccuranceInCluster contains the number of instances in the cluster that contain the same
      // prop value as the instance
