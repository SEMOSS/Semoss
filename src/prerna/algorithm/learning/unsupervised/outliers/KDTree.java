...
public class KDTree<T> {
	 ...
	 * @param num
	 *            - Number of results
	 * @return Array of Item Objects, distances within the items are the square of
	 *         the actual distance between them and the key
	 */
	...
	private class NodeKD {
		...
		public boolean contains(double[] upper, double[] lower, double[] point) {
			...
			for (int i = 0; i < point.length; ++i) {
				if (point[i] > upper[i] || point[i] < lower[i])
					return false;
			}
			return true;
		}
		...
		private void extendBounds(double[] key) {
			...
			for (int i = 0; i < key.length; ++i) {
				if (maxBounds[i] < key[i])
					maxBounds[i] = key[i];
				if (minBounds[i] > key[i])
					minBounds[i] = key[i];
			}
		}
	}
