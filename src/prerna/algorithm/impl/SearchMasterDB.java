...
//		requireInstance = true;
	 ...
	 */
	public void setKeywordAndEdgeList(List<String> keywordList, List<String> edgeVertOutList, List<String> edgeVertInList) {
	...
	public void setMCsForKeywords(List<String> masterConceptsList) {
	...
	public List<Hashtable<String,Object>> findRelatedEnginesWeb() {
		fillAPIHash();
	 ...
	 */
	public List<Hashtable<String,Object>> findRelatedEngines() {
		...
		if(instanceList!=null && !instanceList.isEmpty()) {
			List<String> databaseList = filterDatabaseList();
		...
		String simMCQuery = addBindings(similarMasterConceptsQuery,"MasterConcept",mcBaseURI,masterConceptsList);
		simMCQuery = addDatabaseFilter(simMCQuery,databaseFilter);
		List<Object []> similarMasterConceptsResults = processQuery(simMCQuery);
		
		String simMCC = addBindings(similarMasterConceptConnectionsQuery,"MasterConceptFrom",mcBaseURI,masterConceptsList);
		simMCC = addDatabaseFilter(simMCC,databaseFilter);
		List<Object []> similarEdgesResults = processQuery(simMCC);
		similarEdgesResults = processSimilarEdgesList(edgeVertOutList,edgeVertInList,similarEdgesResults);
		
		String allMCC = addBindings(similarMasterConceptConnectionsOutsideQuery,"BoundMasterConcept",mcBaseURI,masterConceptsList);
		allMCC = addDatabaseFilter(allMCC,databaseFilter);			
		List<Object []> allEdgesList = processQuery(allMCC);
	...
	public List<Hashtable<String,Object>> findRelatedQuestionsWeb() {
		fillAPIHash();
	 ...
	 * Deletes the old master database if necessary.
	 */
	public List<Hashtable<String,Object>> findRelatedQuestions() {

		List<Hashtable<String, Object>> retList = findRelatedEngines();
		if(retList.isEmpty())
			return retList;
		
		String questions;
		if(instanceList!=null && !instanceList.isEmpty()){
			questions = addBindings(relatedQuestionsQuery,"MasterConcept",mcBaseURI,mcForInstanceList);				
		} else {
			questions = addBindings(relatedQuestionsQuery,"MasterConcept",mcBaseURI,masterConceptsList);
		}
	...
	private List<String> filterDatabaseList() {
		...
		Hashtable<String, List<String>> keywordsForEngines = new Hashtable<String, List<String>>();
		//only process each master concept once.
		
		String getPossibleKeywordsQueryFilled = addBindings(getPossibleKeywordsQuery,"MasterConcept",mcBaseURI,mcForInstanceList);
		...
		List<String> filteredEngines = new ArrayList<String>();
	...
	private Boolean engineContainsAllInstances(String engineName, Hashtable<String, List<String>> keywordURIsForEngines) {
		...
		Hashtable<String, List<String>> thisEngineURIs = new Hashtable<String, List<String>>();
		for(int iInd=0;iInd<mcForInstanceList.size();iInd++) {
			...
			List<String> possibleKeywordURIs = keywordURIsForEngines.get(mcEngineKey);

			Hashtable<String, List<String>> thisInstanceURIs = new Hashtable<String, List<String>>();
	...
	private Hashtable<String, List<String>> instanceKeywordURIs(String engineName, String possibleKeywordURI, String instance, String mc) {
		Hashtable<String, List<String>> thisInstanceURIs = new Hashtable<String, List<String>>();
		IEngine engine = (BigDataEngine)DIHelper.getInstance().getLocalProp(engineName);
		if(engine==null)
		...
		if(proc.processQuery()) {
			...
			List<String> instanceURIs = processListQuery(engine,instanceURIQueryFilled,true);
	...
	private String createDatabaseFilter(List<String> databaseList) {
		String databaseFilter ="";
	...
	private List<Object []> processSimilarEdgesList(List<String> edgeVertOutList, List<String> edgeVertInList, List<Object[]> similarEdgesList) {
		List<Object []> processedSimilarEdgesList = new ArrayList<Object []>();
		List<String> databaseMccList = new ArrayList<String>();
	 ...
	 */
	private String addBindings(String query,String variable,String baseURI, List<String> vals) {
		...
		if(query.contains("@BINDINGS@"))
			return query.replace("@BINDINGS@",bindings);
	 ...
	 */
	private List<String> processListQuery(IEngine engine,String query,Boolean getURI) {
		...
		while(wrapper.hasNext())
		{
			...
			if(instanceList==null || instanceList.isEmpty())
				...
				if(urisForMCEng.containsKey(mcEngineKey)) {
					List<String> instanceURIs = urisForMCEng.get(mcEngineKey);
	 ...
	 */
	private List<Object[]> countDatabaseRows(List<Object []> databaseRowList){
		List<Object[]> databaseCountList = new ArrayList<Object[]>();
	 ...
	 */
	private Hashtable<String, Double> aggregateScores(List<Object[]> similarMasterConceptsResults, List<Object[]> similarEdgesResults, List<Object[]> allEdgeResults,int n) {
...
//			scoreList.add(i,newDatabaseRow);
	 ...
	 */
	private Integer getIndex(List<Object[]> list, String valToFind){
