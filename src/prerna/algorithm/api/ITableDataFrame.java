...
public interface ITableDataFrame extends IDataMaker {
   ...
   *
   * @return Boolean true if the column is numerical, false if it is categorical, for the ordered
   *     values in the column headers
   */
  boolean[] isNumeric();
  ...
  /**
   * * Returns the iterator that will iterate through a numeric column the iterator will return the
   * each unique value in the column as a function -> x' = (x -
   * min(columnHeader))/(max(columnHeader) - min(columnHeader)) * @param columnHeader
   *
   * @return
   */
  Iterator<List<Object[]>> scaledUniqueIterator(
  ...
  /**
   * Get the values for a specific column in the data-frame If column is non-numeric, returns null
  ...
  /**
   * Persist a filter on the frame Set will override any existing filter on the frame for a given
   * column
   *
   * @param filter
   */
  ...
  /**
   * Is the column unique within the frame TODO: this is assuming your column is part of a table
   * note - even if native frame with joins, the result set where this column is returned is still
  ...
  //	/**
  //	 * Perform the inputed analytical routine onto the data frame. The routine does not necessarily
  // have to
  //	 * alter/modify the existing data-frame
  //	 * @param routine				The IAnalytics routine to perform onto the data-frame
  //	 */
  //	@Deprecated
  //	void performAnalyticTransformation(IAnalyticTransformationRoutine routine) throws
  // RuntimeException;
  //
  //	/**
  //	 * Perform the inputed analytical routine onto the data frame. The routine does not necessarily
