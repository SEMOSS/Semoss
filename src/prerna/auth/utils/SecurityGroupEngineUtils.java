...
public class SecurityGroupEngineUtils extends AbstractSecurityUtils {
  ...
  public static boolean userGroupCanViewEngine(User user, String engineId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "GROUPENGINEPERMISSION__PERMISSION", "!=", null, PixelDataType.CONST_INT));
    ...
    for (AuthProvider login : logins) {
      ...
      andFilter.addFilter(
          SimpleQueryFilter.makeColToValFilter(
              "GROUPENGINEPERMISSION__TYPE", "==", user.getAccessToken(login).getUserGroupType()));
      andFilter.addFilter(
          SimpleQueryFilter.makeColToValFilter(
    ...
    try {
      ...
      while (wrapper.hasNext()) {
        ...
        if (val != null) {
          // actually do not care what the value is - we have a record so that means we can at least
          // view
          return true;
        }
      }
      ...
      throw new IllegalArgumentException(
          "Failed to retrieve existing group database permissions for user", e);
    } finally {
      if (wrapper != null) {
        try {
          wrapper.close();
  ...
  public static boolean userGroupIsOwner(User user, String databaseId) {
    ...
    qs.addSelector(new QueryColumnSelector("GROUPENGINEPERMISSION__PERMISSION"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPENGINEPERMISSION__ENGINEID", "==", databaseId));
    OrQueryFilter orFilter = new OrQueryFilter();
    List<AuthProvider> logins = user.getLogins();
    boolean anyUserGroups = false;
    for (AuthProvider login : logins) {
  ...
  //		Integer bestUserDatabasePermission = getBestDatabasePermission(user, databaseId);
  //		return bestUserDatabasePermission != null &&
  // AccessPermission.isEditor(bestUserDatabasePermission);
  //	}
  //
  //	/**
  ...
  //		Integer bestUserDatabasePermission = getBestDatabasePermission(user, databaseId);
  //		return bestUserDatabasePermission != null &&
  // AccessPermission.isOwner(bestUserDatabasePermission);
  //	}
  ...
  public static Integer getBestDatabasePermission(User user, String databaseId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINEPERMISSION__ENGINEID", "==", databaseId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "Failed to retrieve existing database permissions for user", e);
    } finally {
      if (wrapper != null) {
        try {
          wrapper.close();
    ...
    if (bestUserDatabasePermission != null
        && AccessPermissionEnum.isOwner(bestUserDatabasePermission)) {
    ...
    if (bestGroupDatabasePermission == null && bestUserDatabasePermission == null) {
      ...
      return null;
    } else if (bestGroupDatabasePermission == null
        || bestGroupDatabasePermission.compareTo(bestUserDatabasePermission) >= 0) {
      return bestUserDatabasePermission;
    } else {
   ...
   */
  public static void addEngineGroupPermission(
      User user,
      String groupId,
      String groupType,
      String engineId,
    ...
    if (getGroupDatabasePermission(groupId, groupType, engineId) != null) {
      throw new IllegalArgumentException(
          "This group already has access to this engine. Please edit the existing permission level.");
    }
   ...
   * @return
   */
  public static Integer getGroupDatabasePermission(
      String groupId, String groupType, String engineId) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("GROUPENGINEPERMISSION__PERMISSION"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPENGINEPERMISSION__ENGINEID", "==", engineId));
   ...
   * @throws IllegalAccessException
   */
  public static void editDatabaseGroupPermission(
      User user,
      String groupId,
      String groupType,
      String engineId,
   ...
   * @throws IllegalAccessException
   */
  public static void removeDatabaseGroupPermission(
      User user, String groupId, String groupType, String engineId) throws IllegalAccessException {
    // make sure user can edit the database
    Integer userPermissionLvl = getBestDatabasePermission(user, engineId);
    if (userPermissionLvl == null || !AccessPermissionEnum.isEditor(userPermissionLvl)) {
      throw new IllegalAccessException(
    ...
    try {
      ps =
          securityDb.getPreparedStatement(
              "DELETE FROM GROUPENGINEPERMISSION WHERE ID=? AND TYPE=? AND ENGINEID=?");
      int parameterIndex = 1;
      ps.setString(parameterIndex++, groupId);
      ps.setString(parameterIndex++, groupType);
      ps.setString(parameterIndex++, engineId);
   ...
   * @throws IllegalAccessException
   */
  public static void removeExpiredEngineGroupPermission(
      String groupId, String groupType, String engineId) throws IllegalAccessException {
