...
public class SecurityGroupInsightsUtils extends AbstractSecurityUtils {
  ...
  public static boolean userGroupCanViewInsight(User user, String projectId, String insightId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "GROUPINSIGHTPERMISSION__PERMISSION", "!=", null, PixelDataType.CONST_INT));
    ...
    for (AuthProvider login : logins) {
      ...
      andFilter.addFilter(
          SimpleQueryFilter.makeColToValFilter(
              "GROUPINSIGHTPERMISSION__TYPE", "==", user.getAccessToken(login).getUserGroupType()));
      andFilter.addFilter(
          SimpleQueryFilter.makeColToValFilter(
              "GROUPINSIGHTPERMISSION__ID", "==", user.getAccessToken(login).getUserGroups()));
  ...
  //		Integer bestUserInsightPermission = getBestInsightPermission(user, projectId, insightId);
  //		return bestUserInsightPermission != null &&
  // AccessPermission.isEditor(bestUserInsightPermission);
  //	}
  //
  //	/**
  ...
  //		Integer bestUserInsightPermission = getBestInsightPermission(user, projectId, insightId);
  //		return bestUserInsightPermission != null &&
  // AccessPermission.isOwner(bestUserInsightPermission);
  //	}
  ...
  public static Integer getBestInsightPermission(User user, String projectId, String insightId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "USERINSIGHTPERMISSION__USERID", "==", getUserFiltersQs(user)));
    ...
    try {
      ...
      logger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "Failed to retrieve existing insight permissions for user", e);
    } finally {
      if (wrapper != null) {
        try {
          wrapper.close();
    ...
    if (bestUserInsightPermission != null
        && AccessPermissionEnum.isOwner(bestUserInsightPermission)) {
   ...
   */
  public static void addInsightGroupPermission(
      User user,
      String groupId,
      String groupType,
      String projectId,
   ...
   * @return
   */
  public static Integer getGroupInsightPermission(
      String groupId, String groupType, String projectId, String insightId) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("GROUPINSIGHTPERMISSION__PERMISSION"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPINSIGHTPERMISSION__PROJECTID", "==", projectId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPINSIGHTPERMISSION__INSIGHTID", "==", insightId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPINSIGHTPERMISSION__ID", "==", groupId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPINSIGHTPERMISSION__TYPE", "==", groupType));
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(securityDb, qs);
      if (wrapper.hasNext()) {
   ...
   * @throws IllegalAccessException
   */
  public static void editInsightGroupPermission(
      User user,
      String groupId,
      String groupType,
      String projectId,
    ...
    Integer existingGroupPermission =
        getGroupInsightPermission(groupId, groupType, projectId, insightId);
   ...
   * @throws IllegalAccessException
   */
  public static void removeInsightGroupPermission(
      User user, String groupId, String groupType, String projectId, String insightId)
      throws IllegalAccessException {
    // make sure user can edit the insight
    Integer userPermissionLvl = getBestInsightPermission(user, projectId, insightId);
    if (userPermissionLvl == null || !AccessPermissionEnum.isEditor(userPermissionLvl)) {
   ...
   * @throws IllegalAccessException
   */
  public static void removeExpiredInsightGroupPermission(
      String groupId, String groupType, String projectId, String insightId)
      throws IllegalAccessException {
