...
public class AdminLoadLdapUsersReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (searchContextScopeStr != null
        && !(searchContextScopeStr = searchContextScopeStr.trim()).isEmpty()) {
      try {
        searchContextScope = Integer.parseInt(searchContextScopeStr);
      } catch (NumberFormatException nfe) {
        throw new IllegalArgumentException(
            this.keysToGet[2]
                + " must be an integer value. Received input = "
                + searchContextScopeStr);
      }
    }
    ...
    if (newProvider != null && !(newProvider = newProvider.trim()).isEmpty()) {
      try {
        ...
        throw new IllegalArgumentException(
            "New provider " + newProvider + " is not a valid auth provider");
      }
    }
