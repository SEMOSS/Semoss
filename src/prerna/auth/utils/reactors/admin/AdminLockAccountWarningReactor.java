...
public class AdminLockAccountWarningReactor extends AbstractReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(AdminLockAccountWarningReactor.class);
    ...
    if (!listToEmail.isEmpty()) {
      ...
      if (templateFile.exists() && templateFile.isFile()) {
        try {
          ...
          template =
              "<html><p>Our records show you have not logged into the SEMOSS application for "
                  + DAYS_SINCE_LAST_LOGIN_REPLACEMENT
                  + " days. "
                  + "Your account will be locked if the number of days exceeds "
                  + DAYS_TO_LOCK_REPLACEMENT
                  + " days. "
