...
public class SecurityUpdateUtils extends AbstractSecurityUtils {
	 ...
	 *
	 * @param userName
	 *            String representing the name of the user to add
	 */
	...
	public static void validateUserLogin(AccessToken newUser) throws Exception {
		...
		if (lastLoginDetails != null) {
			...
			// {
			// logger.info("User " + newUser.getId() + " is now locked due to not resetting
			// password
			// for over " + daysToResetPass + " days");
			// // we should lock the account
			// SecurityUpdateUtils.lockUserAccount(true, newUser.getId(),
			// newUser.getProvider());
			// newUser.setLocked(true);
			// return false;
			// }
