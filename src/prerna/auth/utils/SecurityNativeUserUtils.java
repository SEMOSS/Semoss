...
public class SecurityNativeUserUtils extends AbstractSecurityUtils {
  ...
  public static Boolean addNativeUser(AccessToken newUser, String password) {
    ...
    try {
      ...
      if (wrapper.hasNext()) {
        ...
        String updateQuery =
            "UPDATE SMSS_USER SET ID=?, NAME=?, USERNAME=?, EMAIL=?, TYPE=?, "
        ...
        storeUserPassword(
            newUser.getId(), newUser.getProvider().toString(), hashedPassword, salt, timestamp);
        return true;
      } else {
        // not added by admin
        ...
        String insertQuery =
            "INSERT INTO SMSS_USER (ID, NAME, USERNAME, EMAIL, TYPE, ADMIN, PASSWORD, SALT, DATECREATED, "
  ...
  public static void storeUserPassword(
      String userId, String type, String hashPassword, String salt, java.sql.Timestamp timestamp)
      throws Exception {
    String insertQuery =
        "INSERT INTO PASSWORD_HISTORY (ID, USERID, TYPE, PASSWORD, SALT, DATE_ADDED) "
            + "VALUES (?,?,?,?,?,?)";
    ...
    if (passReuseCount > 0) {
      ...
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("PASSWORD_HISTORY__USERID", "==", userId));
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("PASSWORD_HISTORY__TYPE", "==", type));
  ...
  /**
   * Verifies user information provided in the log in screen to allow or not the entry in the
   * application.
   *
  ...
  public static String getUserId(String username) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(TYPE_COL, "==", AuthProvider.NATIVE.getLabel()));
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(securityDb, qs);
      if (wrapper.hasNext()) {
