...
public class WorkspaceAssetUtils extends AbstractSecurityUtils {
   ...
   * @throws Exception
   */
  public static String createUserWorkspaceProject(User user, AuthProvider provider)
      throws Exception {
  ...
  private static String createEmptyProject(
      User user, AuthProvider provider, String projectName, boolean isAsset) throws Exception {
    AccessToken token = user.getAccessToken(provider);
    // Create a new project id
    String projectId = UUID.randomUUID().toString();

    String userFolderLocation =
        AssetUtility.getUserAssetAndWorkspaceAppRootFolder(projectName, projectId);
    ...
    File tempSmss =
        SmssUtilities.createTemporaryAssetAndWorkspaceSmss(projectId, projectName, isAsset, null);
   ...
   * @throws SQLException
   */
  public static void registerUserWorkspaceProject(AccessToken token, String projectId)
      throws SQLException {
   ...
   * @throws SQLException
   */
  public static void registerUserWorkspaceProject(
      User user, AuthProvider provider, String projectId) throws SQLException {
    registerUserWorkspaceProject(user.getAccessToken(provider), projectId);
  }
   ...
   * @throws SQLException
   */
  public static void registerUserAssetProject(AccessToken token, String projectId)
      throws SQLException {
    PreparedStatement ps = null;
    try {
      ps =
          securityDb.getPreparedStatement(
              "INSERT INTO ASSETENGINE(TYPE, USERID, PROJECTID) VALUES(?,?,?)");
      int parameterIndex = 1;
      ps.setString(parameterIndex++, token.getProvider().name());
      ps.setString(parameterIndex++, token.getId());
      ps.setString(parameterIndex++, projectId);
  ...
  public static String getUserWorkspaceProject(AccessToken token) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "WORKSPACEENGINE__TYPE", "==", token.getProvider().name()));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("WORKSPACEENGINE__USERID", "==", token.getId()));
  ...
  /**
   * Get the user workspace project for the provided user and auth provider; returns null if is
   * there is none
   *
   * @param user
   * @param provider
  ...
  public static String getUserAssetProject(AccessToken token) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ASSETENGINE__TYPE", "==", token.getProvider().name()));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ASSETENGINE__USERID", "==", token.getId()));
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(securityDb, qs);
      if (wrapper.hasNext()) {
  ...
  /**
   * Get the user asset project for the provided user and auth provider; returns null if there is
   * none
   *
   * @param user
   * @param provider
  ...
  public static boolean isAssetOrWorkspaceProject(String projectId) {
    ...
    qs.addSelector(new QueryColumnSelector("WORKSPACEENGINE__PROJECTID"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("WORKSPACEENGINE__PROJECTID", "==", projectId));
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(securityDb, qs);
      if (wrapper.hasNext()) {
  ...
  public static boolean isAssetProject(String projectId) {
    ...
    qs.addSelector(new QueryColumnSelector("ASSETENGINE__PROJECTID"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ASSETENGINE__PROJECTID", "==", projectId));
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(securityDb, qs);
