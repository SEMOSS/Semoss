...
public class AdminSecurityGroupUtils extends AbstractSecurityUtils {
   ...
   * @throws Exception
   */
  public void addGroup(User user, String groupId, String groupType, String description)
      throws Exception {
    ...
    try {
      ...
      if (groupExists(groupId, groupType)) {
        throw new IllegalArgumentException(
            "Group " + groupId + " with type " + groupType + " already exists");
      }
      if (groupType == null) {
      ...
      Gson gson = new Gson();
      String query =
          "INSERT INTO SMSS_GROUP (ID, TYPE, DESCRIPTION, DATEADDED, USERID, USERIDTYPE) "
              + "VALUES (?,?,?,?,?,?)";
      try (PreparedStatement ps = conn.prepareStatement(query)) {
        ...
        securityDb
            .getQueryUtil()
            .handleInsertionOfClob(conn, ps, description, parameterIndex++, gson);
   ...
   * @throws Exception
   */
  public void editGroupAndPropagate(
      User user,
      String curGroupId,
      String curGroupType,
      String newGroupId,
    ...
    groupQuery =
        "UPDATE SMSS_GROUP SET ID=?, TYPE=?, DESCRIPTION=?, DATEADDED=?, USERID=?, USERIDTYPE=? WHERE ID=? AND TYPE=?";
    propagateQueries =
        new String[] {
    ...
    try {
      ...
      try {
        ...
        try (PreparedStatement ps = conn.prepareStatement(groupQuery)) {
          ...
          securityDb
              .getQueryUtil()
              .handleInsertionOfClob(conn, ps, newDescription, parameterIndex++, gson);
          ps.setTimestamp(parameterIndex++, Utility.getCurrentSqlTimestampUTC());
          ps.setString(parameterIndex++, userDetails.getValue0());
          ps.setString(parameterIndex++, userDetails.getValue1());
          // where
  ...
  public void editGroupDetailsAndPropagate(
      User user,
      String curGroupId,
      String curGroupType,
      String newGroupId,
  ...
  public void addUserToGroup(
      User user, String groupId, String userId, String userType, String endDate) throws Exception {
    ...
    if (userInCustomGroup(groupId, userId, userType)) {
      throw new IllegalArgumentException(
          "User " + userId + " already has access to group " + groupId);
    }
    ...
    try {
      conn = securityDb.makeConnection();
      String query =
          "INSERT INTO CUSTOMGROUPASSIGNMENT (GROUPID, USERID, TYPE, "
  ...
  public void removeUserFromGroup(String groupId, String userId, String userType) throws Exception {
    ...
    if (!userInCustomGroup(groupId, userId, userType)) {
      throw new IllegalArgumentException(
          "User " + userId + " does not have access to group " + groupId);
    }
  ...
  public List<Map<String, Object>> getGroups(String searchTerm, long limit, long offset) {
    ...
    if (searchTerm != null && !(searchTerm = searchTerm.trim()).isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("SMSS_GROUP__ID", "?like", searchTerm));
    }
    if (limit > 0) {
      qs.setLimit(limit);
    }
  ...
  public List<Map<String, Object>> getGroupMembers(
      String groupId, String searchTerm, long limit, long offset) {
    if (!groupExists(groupId, "CUSTOM")) {
  ...
  public Long getNumMembersInGroup(String groupId, String searchTerm) {
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.COUNT, "CUSTOMGROUPASSIGNMENT__USERID", "numUsers"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CUSTOMGROUPASSIGNMENT__GROUPID", "==", groupId));
    if (searchTerm != null && !(searchTerm = searchTerm.trim()).isEmpty()) {
      qs.addRelation("CUSTOMGROUPASSIGNMENT__USERID", "SMSS_USER__ID", "inner.join");
   ...
   * @return
   */
  public List<Map<String, Object>> getNonGroupMembers(
      String groupId, String searchTerm, long limit, long offset) {
    if (!groupExists(groupId, "CUSTOM")) {
      throw new IllegalArgumentException("Group " + groupId + " with type custom does not exist");
    }
    ...
    qs.addSelector(new QueryColumnSelector("SMSS_USER__COUNTRYCODE"));
    {
      ...
      exisitngMembersQs.addSelector(
          QueryFunctionSelector.makeConcat2ColumnsFunction(
      ...
      qs.addExplicitFilter(
          SimpleQueryFilter.makeQuerySelectorToSubQuery(
              QueryFunctionSelector.makeConcat2ColumnsFunction(
                  "SMSS_USER__ID", "SMSS_USER__TYPE", "UUID"),
              "!=",
              exisitngMembersQs));
    }
    if (searchTerm != null && !(searchTerm = searchTerm.trim()).isEmpty()) {
  ...
  public Long getNumNonMembersInGroup(String groupId, String searchTerm) {
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.COUNT, "SMSS_USER__ID", "numUsers"));
    {
      SelectQueryStruct exisitngMembersQs = new SelectQueryStruct();
      exisitngMembersQs.addSelector(
          QueryFunctionSelector.makeConcat2ColumnsFunction(
  ...
  public void addGroupProjectPermission(
    ...
    if (!groupExists(groupId, groupType)) {
      throw new IllegalArgumentException(
          "Group " + groupId + " with type " + groupType + " does not exist");
    }
    ...
    if (curPermission != -1) {
      throw new IllegalArgumentException(
          "Group "
              + groupId
              + " already has access to project "
              + projectId
              + " with permission = "
              + AccessPermissionEnum.getPermissionValueById(curPermission));
    }
    Pair<String, String> userDetails = User.getPrimaryUserIdAndTypePair(user);
  ...
  public void editGroupProjectPermission(
    ...
    if (curPermission == -1) {
      throw new IllegalArgumentException(
          "Group "
              + groupId
              + " does not currently have access to project "
              + projectId
              + " to edit");
    }
    ...
    updateQuery =
        "UPDATE GROUPPROJECTPERMISSION SET PERMISSION=?, DATEADDED=?, ENDDATE=?, PERMISSIONGRANTEDBY=?, PERMISSIONGRANTEDBYTYPE=? WHERE ID=? AND PROJECTID=? AND TYPE=?";
    PreparedStatement ps = null;
    try {
  ...
  public void removeGroupProjectPermission(
      User user, String groupId, String groupType, String projectId) {
    groupType = groupType == null ? groupType : groupType.toUpperCase();
    int curPermission = groupProjectPermission(groupId, groupType, projectId);
    if (curPermission == -1) {
      throw new IllegalArgumentException(
   ...
   * @return
   */
  public List<Map<String, Object>> getProjectsForGroup(
      String groupId,
      String groupType,
      String searchTerm,
      long limit,
    ...
    qs.addSelector(
        new QueryColumnSelector(groupProjectPermission + "TYPE")); // this is the group type
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(groupProjectPermission + "ID", "==", groupId));
    qs.addExplicitFilter(
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.LOWER, projectPrefix + "PROJECTNAME", "low_project_name"));
   ...
   * @return
   */
  public Long getNumProjectsForGroup(
      String groupId, String groupType, String searchTerm, boolean onlyApps) {
    groupType = groupType == null ? groupType : groupType.toUpperCase();
    if (!groupExists(groupId, groupType)) {
      throw new IllegalArgumentException(
          "Group " + groupId + " with type " + groupType + " does not exist");
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.COUNT, groupProjectPermission + "PROJECTID", "numProjects"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(groupProjectPermission + "ID", "==", groupId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(groupProjectPermission + "TYPE", "==", groupType));
    ...
    if (searchTerm != null && !(searchTerm = searchTerm.trim()).isEmpty()) {
      qs.addRelation(
          groupProjectPermission + "PROJECTID", projectPrefix + "PROJECTID", "inner.join");
   ...
   * @return
   */
  public List<Map<String, Object>> getAvailableProjectsForGroup(
      String groupId,
      String groupType,
      String searchTerm,
      long limit,
    ...
    // filter out projects that are already added
    {
      ...
      subQs.addSelector(
          new QueryColumnSelector(groupProjectPermission + "PROJECTID")); // this is the group id
      // filter for the group being specified
   ...
   * @return
   */
  public Long getNumAvailableProjectsForGroup(
      String groupId, String groupType, String searchTerm, boolean onlyApps) {
    groupType = groupType == null ? groupType : groupType.toUpperCase();
    if (!groupExists(groupId, groupType)) {
      throw new IllegalArgumentException(
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.COUNT, projectPrefix + "PROJECTID", "numProjects"));
  ...
  public void editGroupEnginePermission(
    ...
    if (curPermission == -1) {
      throw new IllegalArgumentException(
          "Group "
              + groupId
              + " does not currently have access to engine "
              + engineId
              + " to edit");
    }
    if (curPermission == permission) {
    ...
    String updateQuery = null;
    updateQuery =
        "UPDATE GROUPENGINEPERMISSION SET PERMISSION=?, DATEADDED=?, ENDDATE=?, PERMISSIONGRANTEDBY=?, PERMISSIONGRANTEDBYTYPE=? WHERE ID=? AND ENGINEID=? AND TYPE=?";
    PreparedStatement ps = null;
    try {
      ps = securityDb.getPreparedStatement(updateQuery);
      int parameterIndex = 1;
  ...
  public void removeGroupEnginePermission(
      User user, String groupId, String groupType, String engineId) {
    groupType = groupType == null ? groupType : groupType.toUpperCase();
    int curPermission = groupEnginePermission(groupId, groupType, engineId);
    if (curPermission == -1) {
      throw new IllegalArgumentException(
   ...
   * @return
   */
  public List<Map<String, Object>> getEnginesForGroup(
      String groupId, String groupType, String searchTerm, long limit, long offset) {
    groupType = groupType == null ? groupType : groupType.toUpperCase();
    if (!groupExists(groupId, groupType)) {
      throw new IllegalArgumentException(
          "Group " + groupId + " with type " + groupType + " does not exist");
    ...
    qs.addSelector(
        new QueryColumnSelector(groupEnginePermission + "TYPE")); // this is the group type
    ...
    // filter for the group being specified
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(groupEnginePermission + "ID", "==", groupId));
    qs.addExplicitFilter(
    ...
    qs.addSelector(
        new QueryColumnSelector(enginePrefix + "CREATEDBYTYPE", "engine_created_by_type"));
    qs.addSelector(new QueryColumnSelector(enginePrefix + "DATECREATED", "engine_date_created"));
    // back to the others
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.LOWER, enginePrefix + "ENGINENAME", "low_engine_name"));

    if (hasSearchTerm) {
      OrQueryFilter searchFilter = new OrQueryFilter();
      searchFilter.addFilter(
          securityDb.getQueryUtil().getSearchRegexFilter(enginePrefix + "ENGINEID", searchTerm));
      searchFilter.addFilter(
  ...
  public Long getNumEnginesForGroup(String groupId, String groupType, String searchTerm) {
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.COUNT, groupEnginePermission + "ENGINEID", "numEngines"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(groupEnginePermission + "ID", "==", groupId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(groupEnginePermission + "TYPE", "==", groupType));
   ...
   * @return
   */
  public List<Map<String, Object>> getAvailableEnginesForGroup(
      String groupId, String groupType, String searchTerm, long limit, long offset) {
    groupType = groupType == null ? groupType : groupType.toUpperCase();
    if (!groupExists(groupId, groupType)) {
      throw new IllegalArgumentException(
    ...
    // filter out engines that are already added
    {
      SelectQueryStruct subQs = new SelectQueryStruct();
      subQs.addSelector(
          new QueryColumnSelector(groupEnginePermission + "ENGINEID")); // this is the group id
      // filter for the group being specified
      subQs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter(groupEnginePermission + "ID", "==", groupId));
      subQs.addExplicitFilter(
  ...
  public Long getNumAvailableEnginesForGroup(String groupId, String groupType, String searchTerm) {
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.COUNT, enginePrefix + "ENGINEID", "numEngines"));
    if (searchTerm != null && !(searchTerm = searchTerm.trim()).isEmpty()) {
      OrQueryFilter searchFilter = new OrQueryFilter();
      searchFilter.addFilter(
          securityDb.getQueryUtil().getSearchRegexFilter(enginePrefix + "ENGINEID", searchTerm));
  ...
  public boolean userInCustomGroup(String groupId, String userId, String userType) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CUSTOMGROUPASSIGNMENT__TYPE", "==", userType));
  ...
  public boolean groupExists(String groupId, String groupType) {
    ...
    if (groupType != null && !(groupType = groupType.trim()).isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("SMSS_GROUP__TYPE", "==", groupType));
    }
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(securityDb, qs);
  ...
  public int groupProjectPermission(String groupId, String groupType, String projectId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPPROJECTPERMISSION__ID", "==", groupId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPPROJECTPERMISSION__TYPE", "==", groupType));
    qs.addExplicitFilter(
  ...
  public int groupEnginePermission(String groupId, String groupType, String engineId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPENGINEPERMISSION__ID", "==", groupId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPENGINEPERMISSION__TYPE", "==", groupType));
    qs.addExplicitFilter(
