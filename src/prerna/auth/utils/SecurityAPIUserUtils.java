...
public class SecurityAPIUserUtils extends AbstractSecurityUtils {
  ...
  public static Boolean getApplicationRequireDynamicToken() {
    String requireDynamicToken =
        SocialPropertiesUtil.getInstance().getProperty(REQUIRE_DYNAMIC_API_TOKEN);
    if (requireDynamicToken == null
        || (requireDynamicToken = requireDynamicToken.trim()).isEmpty()) {
      // default to true
      return true;
    }
  ...
  public static boolean validCredentials(String clientId, String secretKey) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(TYPE_COL, "==", AuthProvider.API_USER.toString()));
