...
public class SecurityInsightUtils extends AbstractSecurityUtils {
	...
	public static Insight getInsight(String projectId, String insightId) {
		...
		try {
			...
			if (wrapper.hasNext()) {
				...
				Insight in = new Insight(projectId, project.getProjectName(), insightId, cacheable, cacheMinutes,
						cacheCron, cacheEncrypt, pixel.size());
	...
	public static String insightNameExists(String projectId, String insightName) {
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter(fun, "==", insightName.toLowerCase(), PixelDataType.CONST_STRING));
		qs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("INSIGHT__PROJECTID", "==", projectId));
	...
	public static List<String> getUserInsightIdList(User user, boolean includeGlobal, boolean includeExistingAccess) {
		...
		}
		{
			...
			qs2.addExplicitFilter(
					SimpleQueryFilter.makeColToValFilter(userInsightPermissionPrefix + "USERID", "==", userIds));
			orFilter.addFilter(
					SimpleQueryFilter.makeColToSubQuery(insightPrefix + "INSIGHTID", existingAccessComparator, qs2));
		}
		{
			// project access
			SelectQueryStruct qs2 = new SelectQueryStruct();
			...
			orFilter.addFilter(
					SimpleQueryFilter.makeColToSubQuery(insightPrefix + "PROJECTID", existingAccessComparator, qs2));
		}
		{
			...
			for (AuthProvider login : logins) {
				...
				andFilter1.addFilter(SimpleQueryFilter.makeColToValFilter(groupInsightPermissionPrefix + "TYPE", "==",
						user.getAccessToken(login).getUserGroupType()));
				andFilter1.addFilter(SimpleQueryFilter.makeColToValFilter(groupInsightPermissionPrefix + "ID", "==",
				...
				andFilter2.addFilter(SimpleQueryFilter.makeColToValFilter(groupProjectPermissionPrefix + "TYPE", "==",
						user.getAccessToken(login).getUserGroupType()));
				andFilter2.addFilter(SimpleQueryFilter.makeColToValFilter(groupProjectPermissionPrefix + "ID", "==",
						user.getAccessToken(login).getUserGroups()));
			...
			if (!groupInsightOrFilters.isEmpty()) {
				...
				andFilter1.addFilter(
						SimpleQueryFilter.makeColToValFilter(groupInsightPermissionPrefix + "TYPE", "==", null));
				andFilter1.addFilter(
						SimpleQueryFilter.makeColToValFilter(groupInsightPermissionPrefix + "ID", "==", null));
				qs3.addExplicitFilter(andFilter1);
			}
			orFilter.addFilter(
					SimpleQueryFilter.makeColToSubQuery(insightPrefix + "INSIGHTID", existingAccessComparator, qs3));
		}
		{
			...
			if (!groupProjectOrFilters.isEmpty()) {
				...
				AndQueryFilter andFilter1 = new AndQueryFilter();
				andFilter1.addFilter(
						SimpleQueryFilter.makeColToValFilter(groupProjectPermissionPrefix + "TYPE", "==", null));
				andFilter1.addFilter(
						SimpleQueryFilter.makeColToValFilter(groupProjectPermissionPrefix + "ID", "==", null));
				qs4.addExplicitFilter(andFilter1);
			}
			orFilter.addFilter(
					SimpleQueryFilter.makeColToSubQuery(insightPrefix + "INSIGHTID", existingAccessComparator, qs4));
		}
	...
	public static List<Map<String, Object>> getUserEditableInsights(User user, String projectId) {
		...
		if (permission.equals(AccessPermissionEnum.OWNER.getPermission())) {
			...
			qs.addExplicitFilter(
					SimpleQueryFilter.makeColToValFilter("USERINSIGHTPERMISSION__PERMISSION", "==", permissions));
			qs.addOrderBy(new QueryColumnOrderBySelector("INSIGHT__INSIGHTNAME"));
			return QueryExecutionUtility.flushRsToMap(securityDb, qs);
		}
	}
	...
	public static Integer getUserInsightPermission(String singleUserId, String projectId, String insightId) {
		// String query = "SELECT DISTINCT USERINSIGHTPERMISSION.PERMISSION FROM
	 ...
	 * @return
	 */
	public static Map<String, Integer> getUserInsightPermissions(List<String> userIds, String projectId,
			String insightId) {
  ...
  ///////////////////////////////////////////////////////////////////////////////////
	...
	public static List<Map<String, Object>> getInsightUsers(User user, String projectId, String insightId,
		...
		return SecurityUserInsightUtils.getInsightUsers(user, projectId, insightId, searchTerm, permission, limit,
				offset);
	}
  ...
  ///////////////////////////////////////////////////////////////////////////////////
	...
	/**
	 * Update the insight metadata for the insight Will delete existing values and
	 * then perform a bulk insert
	 *
	 * @param projectId
	 * @param insightId
	 * @param metadata
	...
	/**
	 * Update the insight description Will perform an insert if the description
	...
	/**
	 * Update the insight tags for the insight Will delete existing values and then
	...
	public static void updateInsightFrames(String projectId, String insightId, Set<ITableDataFrame> insightFrames) {
		...
		// now we do the new insert with the order of the tags
		query = securityDb.getQueryUtil().createInsertPreparedStatementString("INSIGHTFRAMES", new String[]{"PROJECTID",
				"INSIGHTID", "TABLENAME", "TABLETYPE", "COLUMNNAME", "COLUMNTYPE", "ADDITIONALTYPE"});
		ps = null;
		try {
			ps = securityDb.getPreparedStatement(query);
  ...
  ///////////////////////////////////////////////////////////////////////////////////
	...
	/**
	 * User has access to specific insights within a project User can access if: 1)
	 * Is Owner, Editer, or Reader of insight 2) Insight is global 3) Is Owner of
	 * project
	 *
	 * <p>
	...
	public static List<Map<String, Object>> searchUserInsights(User user, List<String> projectFilter, String searchTerm,
		...
		// add a join to get the user permission level and if favorite
		{
			...
			QueryFunctionSelector castFavorite = QueryFunctionSelector.makeFunctionSelector(QueryFunctionHelper.CAST,
					userInsightPrefix + "FAVORITE", "castFavorite");
			...
			if (favoritesOnly) {
				qs2.addExplicitFilter(SimpleQueryFilter.makeColToValFilter(userInsightPrefix + "FAVORITE", "==", true,
						PixelDataType.BOOLEAN));
				// we can set this to inner join if only favorites
				subQuery = new SubqueryRelationship(qs2, "INSIGHT_USER_PERMISSIONS", "inner.join",
		...
		// filtering by insight meta key-value pairs (i.e. <tag>:value): for each pair,
		// add in-filter
		...
		// 2 - insight that are global within projects i have access to
		{
			...
			if (hasProjectFilters) {
				...
				// project and insight must be global must be global
				OrQueryFilter projectSubset = new OrQueryFilter();
				projectSubset.addFilter(SimpleQueryFilter.makeColToValFilter(projectPrefix + "GLOBAL", "==", true,
						PixelDataType.BOOLEAN));
				projectSubset
						.addFilter(SimpleQueryFilter.makeColToValFilter(userProjectPrefix + "USERID", "==", userIds));
				subQs2.addExplicitFilter(projectSubset);
			}
		}
	...
	// /**
	// * TODO >>> Kunal: change app_name and app_name_id to project references
	// * Search through all insights with an optional filter on engines and an
	...
	// * @return
	// */
	// public static List<Map<String, Object>> searchInsights(List<String>
	// projectFilter, String
	// searchTerm,
	// QueryColumnOrderBySelector sortBy, Map<String, Object> insightMetadataFilter,
	// String limit,
	// String offset) {
	...
	// "app_insight_id"));
	// qs.addSelector(new QueryColumnSelector("INSIGHT__INSIGHTNAME", "name"));
	// qs.addSelector(new QueryColumnSelector("INSIGHT__EXECUTIONCOUNT",
	// "view_count"));
	// qs.addSelector(new QueryColumnSelector("INSIGHT__LAYOUT", "layout"));
	// qs.addSelector(new QueryColumnSelector("INSIGHT__CREATEDON", "created_on"));
	...
	// securityDb.getQueryUtil().appendSearchRegexFilter(qs, "INSIGHT__INSIGHTNAME",
	// searchTerm);
	// }
  ...
  //		// sort
	...
	// }
	// // filtering by insightmeta key-value pairs (i.e. <tag>:value): for each
	// pair, add in-filter
	// against insightids from subquery
	// if (insightMetadataFilter!=null && !insightMetadataFilter.isEmpty()) {
	// for (String k : insightMetadataFilter.keySet()) {
	...
	public static SelectQueryStruct searchUserInsightsUsage(User user, List<String> projectFilter, String searchTerm,
		...
		if (hasEngineFilters) {
			...
			OrQueryFilter orFilter = new OrQueryFilter();
			{
				...
				embedAndFilter.addFilter(SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__PERMISSION", "==", 1,
						PixelDataType.CONST_INT));
				embedAndFilter
						.addFilter(SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__USERID", "==", userIds));
			...
			// remove those engines you have visibility as false
			{
				...
				subQs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__VISIBILITY", "==",
			...
			OrQueryFilter secondOrFilter = new OrQueryFilter();
			{
				...
				embedAndFilter
						.addFilter(SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__USERID", "==", userIds));
				embedAndFilter.addFilter(SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__VISIBILITY", "==",
						true, PixelDataType.BOOLEAN));
	...
	// * @return
	// */
	// public static SelectQueryStruct searchInsightsUsage(List<String>
	// projectFilter, String
	...
	// // always add tags
	////		if(tagFiltering) {
	// qs.addRelation("INSIGHT__INSIGHTID", "INSIGHTMETA__INSIGHTID",
	// "left.outer.join");
	// qs.addRelation("INSIGHT__PROJECTID", "INSIGHTMETA__PROJECTID",
	// "left.outer.join");
	////		}
	//
	// return qs;
	...
	public static IRawSelectWrapper getInsightMetadataWrapper(Map<String, List<String>> projectToInsightMap,
		...
		for (String projectId : projectToInsightMap.keySet()) {
			...
			andFilter.addFilter(SimpleQueryFilter.makeColToValFilter("INSIGHTMETA__INSIGHTID", "==",
					projectToInsightMap.get(projectId)));
	...
	public static List<Object[]> getInsightFrames(String projectId, String insightId, String frameNamePattern) {
		...
		if (frameNamePattern != null && !(frameNamePattern = frameNamePattern.trim()).isEmpty()) {
			qs.addExplicitFilter(
					SimpleQueryFilter.makeColToValFilter("INSIGHTFRAMES__TABLENAME", "?like", frameNamePattern));
		}
	...
	/**
	 * User will see specific insight predictions for their searches User can see
	 * records if: 1) Is Owner, Editer, or Reader of insight 2) Insight is global 3)
	...
	public static List<String> predictUserInsightSearch(User user, String searchTerm, String limit, String offset) {
		...
		// + "LEFT JOIN USERINSIGHTPERMISSION ON
		// USERINSIGHTPERMISSION.ENGINEID=INSIGHT.ENGINEID "
		// + "WHERE "
		// + "(USERINSIGHTPERMISSION.USERID IN " + userFilters + " OR
		// INSIGHT.GLOBAL=TRUE OR "
		// + "(ENGINEPERMISSION.PERMISSION=1 AND ENGINEPERMISSION.USERID IN " +
		// userFilters + ") )
		// "
		...
		OrQueryFilter orFilters = new OrQueryFilter();
		{
			...
			AndQueryFilter andFilter = new AndQueryFilter();
			{
				andFilter.addFilter(SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__PERMISSION", "==", 1,
						PixelDataType.CONST_INT));
  ...
  /////////////////////////////////////////////////////////////////////////////////////
	...
	public static List<Map<String, Object>> getInsightUsersNoCredentials(User user, String projectId, String insightId,
		 ...
		 * SMSS_USER.EMAIL FROM SMSS_USER WHERE SMSS_USER.ID NOT IN (SELECT u.USERID
		 * FROM USERINSIGHTPERMISSION u WHERE u.ENGINEID == '" + appID + "' AND
		 * u.INSIGHTID == '"+insightID +"'AND u.PERMISSION IS NOT NULL);"
		 */
	...
	public static Integer getUserAccessRequestInsightPermission(String userId, String projectId, String insightId) {
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("INSIGHTACCESSREQUEST__APPROVER_DECISION", "==", null));
	...
	public static List<Map<String, Object>> getUserAccessRequestsByInsight(String projectId, String insightId) {
		...
		qs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("INSIGHTACCESSREQUEST__INSIGHTID", "==", insightId));
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("INSIGHTACCESSREQUEST__APPROVER_DECISION", "==", "NEW_REQUEST"));
