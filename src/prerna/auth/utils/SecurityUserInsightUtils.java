...
class SecurityUserInsightUtils extends AbstractSecurityUtils {
  ...
  public static String getActualUserInsightPermission(
    ...
    // "
    //				+ "WHERE ENGINEID='" + engineId + "' AND INSIGHTID='" + insightId + "' AND USERID IN " +
    // userFilters;
  ...
  public static boolean userCanViewInsight(User user, String projectId, String insightId) {
    ...
    try {
      boolean isExpired =
          insightPermissionIsExpired(User.getSingleLogginName(user), projectId, insightId);
      // If permission is expired remove permission
      if (isExpired) {
        SecurityInsightUtils.removeExpiredInsightUser(
            User.getSingleLogginName(user), projectId, insightId);
      }
    } catch (Exception e) {
      classLogger.error(Constants.STACKTRACE, e);
    }
  ...
  static int getMaxUserInsightPermission(User user, String projectId, String insightId) {
    ...
    // "
    //				+ "WHERE ENGINEID='" + engineId + "' AND INSIGHTID='" + insightId + "' AND USERID IN " +
    // userFilters + " ORDER BY PERMISSION";
    //		IRawSelectWrapper wrapper = WrapperManager.getInstance().getRawWrapper(securityDb, query);
  ...
  public static void setInsightFavorite(
    ...
    if (!SecurityProjectUtils.projectIsGlobal(projectId)
      ...
      throw new IllegalAccessException(
          "The user doesn't have the permission to modify this insight");
    }
    ...
    try {
      ...
      if (wrapper.hasNext()) {
        ...
        PreparedStatement ps =
            securityDb.getPreparedStatement(
                "INSERT INTO USERINSIGHTPERMISSION "
                    + "(USERID, PROJECTID, INSIGHTID, FAVORITE, PERMISSION) VALUES (?,?,?,?,?)");
        if (ps == null) {
          throw new IllegalArgumentException(
              "Error generating prepared statement to set app visibility");
        }
        try {
          // we will set the permission to read only
          for (AuthProvider loginType : user.getLogins()) {
    ...
    if (hasSearchId) {
      ...
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter(
              "USERINSIGHTPERMISSION__USERID", "?like", searchTerm));
   ...
   * @param userId
   */
  public static boolean insightPermissionIsExpired(
      String userId, String projectId, String insightId) throws Exception {
    LocalDateTime currentTime = LocalDateTime.now();
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("USERINSIGHTPERMISSION__ENDDATE"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("USERINSIGHTPERMISSION__USERID", "==", userId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("USERINSIGHTPERMISSION__PROJECTID", "==", projectId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("USERINSIGHTPERMISSION__INSIGHTID", "==", insightId));
