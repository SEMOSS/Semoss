...
class SecurityUserEngineUtils extends AbstractSecurityUtils {
  ...
  public static String getHighestEnginePermission(
      String userPermission, List<String> groupUserPermissions) {
    ...
    try {
      boolean isExpired =
          enginePermissionIsExpired(
              Utility.inputSQLSanitizer(User.getSingleLogginName(user)), engineId);
      // If permission is expired remove permission
      if (isExpired) {
        SecurityEngineUtils.removeExpiredEngineUser(
            Utility.inputSQLSanitizer(User.getSingleLogginName(user)), engineId);
      }
    } catch (Exception e) {
      classLogger.error(Constants.STACKTRACE, e);
   ...
   * @throws Exception
   */
  public static boolean checkUserHasAccessToEngine(String engineId, String userId)
      throws Exception {
    try {
   ...
   * @return
   */
  public static Map<String, Integer> getUserEnginePermissions(
      List<String> userIds, String engineId) {
   ...
   * @return
   */
  public static IRawSelectWrapper getUserEnginePermissionsWrapper(
      List<String> userIds, String engineId) throws Exception {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("ENGINEPERMISSION__USERID"));
    qs.addSelector(new QueryColumnSelector("ENGINEPERMISSION__PERMISSION"));
    qs.addExplicitFilter(
    ...
    if (SecurityEngineUtils.engineIsGlobal(engineId)) {
      ...
      qs.addSelector(
          new QueryColumnSelector(
              "ENGINEPERMISSION__PERMISSIONGRANTEDBY", "permission_granted_by"));
      qs.addSelector(
          new QueryColumnSelector(
              "ENGINEPERMISSION__PERMISSIONGRANTEDBYTYPE", "permission_granted_by_type"));
      ...
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter(
              "PERMISSION__ID", "==", permissionValues, PixelDataType.CONST_INT));
  ...
  public static List<Map<String, Object>> getEngineUsers(
    ...
    // also return who did this and when
    qs.addSelector(
        new QueryColumnSelector("ENGINEPERMISSION__PERMISSIONGRANTEDBY", "permission_granted_by"));
    qs.addSelector(
        new QueryColumnSelector(
            "ENGINEPERMISSION__PERMISSIONGRANTEDBYTYPE", "permission_granted_by_type"));
    qs.addSelector(new QueryColumnSelector("ENGINEPERMISSION__DATEADDED", "date_added"));
    // usage restriction
    qs.addSelector(
        new QueryColumnSelector("ENGINEPERMISSION__USAGERESTRICTION", "usage_restriction"));
    qs.addSelector(new QueryColumnSelector("ENGINEPERMISSION__MAXTOKENS", "max_tokens"));
    qs.addSelector(
        new QueryColumnSelector("ENGINEPERMISSION__MAXRESPONSETIME", "max_response_time"));
