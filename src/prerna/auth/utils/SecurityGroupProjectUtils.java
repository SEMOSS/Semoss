...
public class SecurityGroupProjectUtils extends AbstractSecurityUtils {
  ...
  public static boolean userGroupCanViewProject(User user, String projectId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "GROUPPROJECTPERMISSION__PERMISSION", "!=", null, PixelDataType.CONST_INT));
    ...
    for (AuthProvider login : logins) {
      ...
      andFilter.addFilter(
          SimpleQueryFilter.makeColToValFilter(
              "GROUPPROJECTPERMISSION__TYPE", "==", user.getAccessToken(login).getUserGroupType()));
      andFilter.addFilter(
          SimpleQueryFilter.makeColToValFilter(
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Failed to retrieve existing group project permissions for user", e);
    } finally {
      if (wrapper != null) {
        try {
          wrapper.close();
  ...
  //		Integer bestUserProjectPermission = getBestProjectPermission(user, projectId);
  //		return bestUserProjectPermission != null &&
  // AccessPermission.isEditor(bestUserProjectPermission);
  //	}
  //
  //	/**
  ...
  //		Integer bestUserProjectPermission = getBestProjectPermission(user, projectId);
  //		return bestUserProjectPermission != null &&
  // AccessPermission.isOwner(bestUserProjectPermission);
  //	}
  ...
  public static Integer getBestProjectPermission(User user, String projectId) {
    ...
    try {
      ...
      logger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "Failed to retrieve existing project permissions for user", e);
    } finally {
      if (wrapper != null) {
        try {
          wrapper.close();
    ...
    if (bestUserProjectPermission != null
        && AccessPermissionEnum.isOwner(bestUserProjectPermission)) {
    ...
    qs.addSelector(new QueryColumnSelector("GROUPPROJECTPERMISSION__PERMISSION"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPPROJECTPERMISSION__ENGINEID", "==", projectId));
    OrQueryFilter orFilter = new OrQueryFilter();
    List<AuthProvider> logins = user.getLogins();
    for (AuthProvider login : logins) {
      AndQueryFilter andFilter = new AndQueryFilter();
   ...
   */
  public static void addProjectGroupPermission(
      User user,
      String groupId,
      String groupType,
      String projectId,
   ...
   * @return
   */
  public static Integer getGroupProjectPermission(
      String groupId, String groupType, String projectId) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("GROUPPROJECTPERMISSION__PERMISSION"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("GROUPPROJECTPERMISSION__PROJECTID", "==", projectId));
   ...
   * @throws IllegalAccessException
   */
  public static void editProjectGroupPermission(
      User user,
      String groupId,
      String groupType,
      String projectId,
   ...
   * @throws IllegalAccessException
   */
  public static void removeProjectGroupPermission(
      User user, String groupId, String groupType, String projectId) throws IllegalAccessException {
    // make sure user can edit the project
    Integer userPermissionLvl = getBestProjectPermission(user, projectId);
    if (userPermissionLvl == null || !AccessPermissionEnum.isEditor(userPermissionLvl)) {
      throw new IllegalAccessException(
    ...
    try {
      ps =
          securityDb.getPreparedStatement(
              "DELETE FROM GROUPPROJECTPERMISSION WHERE ID=? AND TYPE=? AND PROJECTID=?");
      int parameterIndex = 1;
      ps.setString(parameterIndex++, groupId);
      ps.setString(parameterIndex++, groupType);
      ps.setString(parameterIndex++, projectId);
   ...
   * @throws IllegalAccessException
   */
  public static void removeExpiredProjectGroupPermission(
      String groupId, String groupType, String projectId) throws IllegalAccessException {
