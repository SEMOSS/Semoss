...
public class SecurityAdminUtils extends AbstractSecurityUtils {
  ...
  public static Boolean userIsAdmin(User user) {
    ...
    //		String query = "SELECT * FROM SMSS_USER WHERE ADMIN=TRUE AND ID IN " + userFilters + " LIMIT
    // 1;";
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("SMSS_USER__ID", "==", getUserFiltersQs(user)));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "SMSS_USER__ADMIN", "==", true, PixelDataType.BOOLEAN));
  ...
  public List<Map<String, Object>> getAllUsers(String searchTerm, long limit, long offset)
    ...
    qs.addSelector(
        new QueryColumnSelector(
            SMSS_USER_PREFIX + "MODELUSAGERESTRICTION", "model_usage_restriction"));
    qs.addSelector(
        new QueryColumnSelector(SMSS_USER_PREFIX + "MODELMAXTOKENS", "model_max_tokens"));
    qs.addSelector(
        new QueryColumnSelector(
            SMSS_USER_PREFIX + "MODELMAXRESPONSETIME", "model_max_response_time"));
    qs.addSelector(
        new QueryColumnSelector(SMSS_USER_PREFIX + "MODELUSAGEFREQUENCY", "model_usage_frequency"));
    ...
    if (hasSearchTerm) {
      ...
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter(SMSS_USER_PREFIX + "ID", "?like", searchTerm));
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter(SMSS_USER_PREFIX + "NAME", "?like", searchTerm));
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter(SMSS_USER_PREFIX + "USERNAME", "?like", searchTerm));
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter(SMSS_USER_PREFIX + "EMAIL", "?like", searchTerm));
  ...
  public List<Map<String, Object>> getAllUserEngines(String userId, List<String> engineTypes)
    ...
    if (engineTypes != null && !engineTypes.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINE__ENGINETYPE", "==", engineTypes));
    }
   ...
   * @throws IllegalArgumentException
   */
  public List<Map<String, Object>> getAllUserProjects(String userId)
      throws IllegalArgumentException {
    SelectQueryStruct qs = new SelectQueryStruct();
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__USERID", "==", userId));
  ...
  public List<Map<String, Object>> getAllUserInsights(
      User user, List<String> projectFilter, String searchTerm, long limit, long offset)
      throws IllegalArgumentException {
    boolean hasSearchTerm = searchTerm != null && !(searchTerm = searchTerm.trim()).isEmpty();
    ...
    // add a join to get the user permission level and if favorite
    {
      ...
      qs2.addSelector(
          QueryFunctionSelector.makeFunctionSelector(
              QueryFunctionHelper.MAX, userInsightPrefix + "FAVORITE", "FAVORITE"));
      qs2.addSelector(
          QueryFunctionSelector.makeFunctionSelector(
  ...
  public List<Map<String, Object>> getAllUserInsightAccess(String projectId, String userId)
    ...
    String query =
        "SELECT DISTINCT "
            ...
            + "WHERE USERINSIGHTPERMISSION.PROJECTID = '"
            + projectId
            + "' AND USERINSIGHTPERMISSION.USERID = '"
            + userId
            + "'"
            + ") AS SUB_Q ON SUB_Q.INSIGHTID = INSIGHT.INSIGHTID "
            + "WHERE INSIGHT.PROJECTID = '"
            + projectId
  ...
  public boolean editUser(Map<String, Object> userInfo) {
    ...
    String email =
        userInfo.get("email") != null ? userInfo.get("email").toString().trim().toLowerCase() : "";
    ...
    String phoneExtension =
        userInfo.get("phoneextension") != null ? userInfo.get("phoneextension").toString() : "";
    String countryCode =
        userInfo.get("countrycode") != null ? userInfo.get("countrycode").toString() : "";
    // model restrictions
    String modelUsageRestriction =
        userInfo.get("model_usage_restriction") != null
            ? userInfo.get("model_usage_restriction").toString()
            : null;
    String modelUsageFrequency =
        userInfo.get("model_usage_frequency") != null
    ...
    String editUserQuery =
        securityDb
            .getQueryUtil()
            .createUpdatePreparedStatementString("SMSS_USER", columnsToUpdate, whereCol);
    ...
    try {
      ...
      if (modelUsageRestriction == null
          || (modelUsageRestriction = modelUsageRestriction.trim()).isEmpty()) {
      ...
      if (modelUsageFrequency == null
          || (modelUsageFrequency = modelUsageFrequency.trim()).isEmpty()) {
        editUserPs.setNull(i++, java.sql.Types.VARCHAR);
      } else {
    ...
    // TODO: need to start binding on userId + type
    {
      String[] deleteQueries =
          new String[] {
            ...
            "DELETE FROM USERMETA WHERE USERID=?",
  ...
  public List<Map<String, Object>> getAllEngineSettings(
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.LOWER, "ENGINE__ENGINENAME", "low_engine_name"));
    ...
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.LOWER, "ENGINE__ENGINENAME", "low_database_name"));
    ...
    if (engineFilter != null && !engineFilter.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINE__ENGINEID", "==", engineFilter));
    }
    if (engineTypes != null && !engineTypes.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINE__ENGINETYPE", "==", engineTypes));
    ...
    if (hasSearchTerm) {
      OrQueryFilter searchFilter = new OrQueryFilter();
      searchFilter.addFilter(
          securityDb.getQueryUtil().getSearchRegexFilter("ENGINE__ENGINENAME", searchTerm));
      searchFilter.addFilter(
          securityDb.getQueryUtil().getSearchRegexFilter("ENGINE__ENGINEID", searchTerm));
      qs.addExplicitFilter(searchFilter);
    }
    // filtering by enginemeta key-value pairs (i.e. <tag>:value): for each pair, add in-filter
    // against engineids from subquery
    if (engineMetadataFilter != null && !engineMetadataFilter.isEmpty()) {
      for (String k : engineMetadataFilter.keySet()) {
        ...
        subQs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter("ENGINEMETA__METAKEY", "==", k));
        subQs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter(
                "ENGINEMETA__METAVALUE", "==", engineMetadataFilter.get(k)));
  ...
  public List<Map<String, Object>> getAllProjectSettings(
    ...
    qs.addSelector(
        new QueryColumnSelector(projectPrefix + "CREATEDBYTYPE", "project_created_by_type"));
    qs.addSelector(new QueryColumnSelector(projectPrefix + "DATECREATED", "project_date_created"));
    qs.addSelector(
        new QueryColumnSelector(projectPrefix + "DATELASTEDITED", "project_date_last_edited"));
    ...
    qs.addSelector(
        new QueryColumnSelector(
            projectPrefix + "PORTALPUBLISHED", "project_portal_published_date"));
    qs.addSelector(
        new QueryColumnSelector(projectPrefix + "PORTALPUBLISHEDUSER", "project_published_user"));
    qs.addSelector(
        new QueryColumnSelector(
            projectPrefix + "PORTALPUBLISHEDTYPE", "project_published_user_type"));
    qs.addSelector(
        new QueryColumnSelector(
            projectPrefix + "REACTORSCOMPILED", "project_reactors_compiled_date"));
    qs.addSelector(
        new QueryColumnSelector(
            projectPrefix + "REACTORSCOMPILEDUSER", "project_reactors_compiled_user"));
    qs.addSelector(
        new QueryColumnSelector(
            projectPrefix + "REACTORSCOMPILEDTYPE", "project_reactors_compiled_user_type"));
    // for sort
    qs.addSelector(
        QueryFunctionSelector.makeFunctionSelector(
            QueryFunctionHelper.LOWER, "PROJECT__PROJECTNAME", "low_project_name"));

    if (projectFilter != null && !projectFilter.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("PROJECT__PROJECTID", "==", projectFilter));
    }
    if (hasSearchTerm) {
      OrQueryFilter searchFilter = new OrQueryFilter();
      searchFilter.addFilter(
          securityDb.getQueryUtil().getSearchRegexFilter(projectPrefix + "PROJECTID", searchTerm));
      searchFilter.addFilter(
          securityDb
              .getQueryUtil()
              .getSearchRegexFilter(projectPrefix + "PROJECTNAME", searchTerm));
      qs.addExplicitFilter(searchFilter);
    }
    // filtering by projectmeta key-value pairs (i.e. <tag>:value): for each pair, add in-filter
    // against projectids from subquery
    if (projectMetadataFilter != null && !projectMetadataFilter.isEmpty()) {
      for (String k : projectMetadataFilter.keySet()) {
        ...
        subQs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter("PROJECTMETA__METAKEY", "==", k));
        subQs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter(
                "PROJECTMETA__METAVALUE", "==", projectMetadataFilter.get(k)));
        qs.addExplicitFilter(
    ...
    try {
      ps =
          securityDb.getPreparedStatement(
              "UPDATE PROJECT SET HASPORTAL=?, PORTALNAME=? WHERE PROJECTID=?");
      int parameterIndex = 1;
   ...
   * @return
   */
  public List<Map<String, Object>> getEngineUsers(
      String engineId, String searchParam, String permission, long limit, long offset) {
  ...
  public static long getEngineUsersCount(String engineId, String searchParam, String permission) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINEPERMISSION__ENGINEID", "==", engineId));
    if (hasSearchParam) {
      OrQueryFilter or = new OrQueryFilter();
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINEPERMISSION__USERID", "?like", searchParam));
      or.addFilter(SimpleQueryFilter.makeColToValFilter("SMSS_USER__NAME", "?like", searchParam));
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter("SMSS_USER__USERNAME", "?like", searchParam));
   ...
   * @return
   */
  public List<Map<String, Object>> getProjectUsers(
      String projectId, String searchParam, String permission, long limit, long offset) {
    return SecurityUserProjectUtils.getProjectUsers(
        projectId, searchParam, permission, limit, offset);
  }
  ...
  public static long getProjectUsersCount(String projectId, String searchParam, String permission) {
    ...
    if (hasSearchParam) {
      OrQueryFilter or = new OrQueryFilter();
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__USERID", "?like", searchParam));
      or.addFilter(SimpleQueryFilter.makeColToValFilter("SMSS_USER__NAME", "?like", searchParam));
      or.addFilter(
          SimpleQueryFilter.makeColToValFilter("SMSS_USER__USERNAME", "?like", searchParam));
      or.addFilter(SimpleQueryFilter.makeColToValFilter("SMSS_USER__EMAIL", "?like", searchParam));
  ...
  public void addEngineUser(
    ...
    String insertQ =
        "INSERT INTO ENGINEPERMISSION (USERID, ENGINEID, PERMISSION, VISIBILITY, PERMISSIONGRANTEDBY, PERMISSIONGRANTEDBYTYPE, DATEADDED, ENDDATE, USAGERESTRICTION, USAGEFREQUENCY, MAXTOKENS, MAXRESPONSETIME) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
    PreparedStatement ps = null;
    try {
   ...
   * @param user
   */
  public void addEngineUserPermissions(
      String engineId, List<Map<String, Object>> permission, User user) {
    ...
    List<String> userIds =
        permission.stream().map(map -> (String) map.get("userid")).collect(Collectors.toList());
    // this returns a list of existing permissions
    Map<String, Integer> existingUserPermission =
        SecurityUserEngineUtils.getUserEnginePermissions(userIds, engineId);
    ...
    try {
      ...
      for (int i = 0; i < permission.size(); i++) {
        ...
        if (thisPermissionMap.get("endDate") != null) {
          verifiedEndDate =
              AbstractSecurityUtils.calculateEndDate((String) thisPermissionMap.get("endDate"));
          ps.setTimestamp(parameterIndex++, verifiedEndDate);
        } else {
        ...
        if (thisPermissionMap.get("usageRestriction") != null
          ...
          ps.setString(
              parameterIndex++, ((String) thisPermissionMap.get("usageRestriction")).trim());
        } else {
          ps.setNull(parameterIndex++, java.sql.Types.VARCHAR);
        }
        ...
        if (thisPermissionMap.get("maxResponseTime") != null) {
          ps.setDouble(
              parameterIndex++, ((Number) thisPermissionMap.get("maxResponseTime")).doubleValue());
        } else {
          ps.setNull(parameterIndex++, java.sql.Types.DOUBLE);
        }
   ...
   * @return
   */
  public void addProjectUserPermissions(
      String projectId, List<Map<String, String>> permission, User user) {
    ...
    // this returns a list of existing permissions
    Map<String, Integer> existingUserPermission =
        SecurityProjectUtils.getUserProjectPermissions(userIds, projectId);
    if (!existingUserPermission.isEmpty()) {
      throw new IllegalArgumentException(
    ...
    try {
      ...
      for (int i = 0; i < permission.size(); i++) {
        ...
        if (thisPermissionMap.get("endDate") != null) {
          verifiedEndDate =
              AbstractSecurityUtils.calculateEndDate(thisPermissionMap.get("endDate"));
          ps.setTimestamp(parameterIndex++, verifiedEndDate);
        } else {
          ps.setNull(parameterIndex++, java.sql.Types.TIMESTAMP);
        }
   ...
   * @return
   */
  public void addInsightUserPermissions(
      String projectId,
      String insightId,
  ...
  public void addProjectUser(
      String newUserId, String projectId, String permission, User user, String endDate) {
  ...
  public List<String> getProjectsUserHasExplicitAccess(String singleUserId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("PROJECTPERMISSION__USERID", "==", singleUserId));
   ...
   * @param isAddNew boolean - If false, modifying existing project permissions to the new
   *     permission level If true, adding new projects with the permission level specified
   */
  public void grantAllProjects(String userId, String permission, boolean isAddNew, User user) {
    ...
    if (isAddNew) {
      ...
      String insertQuery =
          "INSERT INTO PROJECTPERMISSION (USERID, PROJECTID, VISIBILITY, PERMISSION, PERMISSIONGRANTEDBY, PERMISSIONGRANTEDBYTYPE, DATEADDED) VALUES(?, ?,?,?,?,?,?)";
      ...
      Map<String, Boolean> currentProjectToVisibilityMap =
          getProjectsAndVisibilityUserHasExplicitAccess(userId);
   ...
   * @return
   */
  public static List<String> getEnginesUserHasExplicitAccess(
      String singleUserId, List<String> engineTypes) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("ENGINEPERMISSION__ENGINEID"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINEPERMISSION__USERID", "==", singleUserId));
    if (engineTypes != null && !engineTypes.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINE__ENGINETYPE", "==", engineTypes));
   ...
   * @return
   */
  public Map<String, Boolean> getEnginesAndVisibilityUserHasExplicitAccess(
      String singleUserId, List<String> engineTypes) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("ENGINEPERMISSION__ENGINEID"));
   ...
   * @param engineTypes
   */
  public void grantAllEngines(
      String userId, String permission, boolean isAddNew, List<String> engineTypes, User user) {
    String logETypes =
    ...
    if (isAddNew) {
      ...
      String insertQuery =
          "INSERT INTO ENGINEPERMISSION (USERID, ENGINEID, VISIBILITY, PERMISSION, PERMISSIONGRANTEDBY, PERMISSIONGRANTEDBYTYPE, DATEADDED) VALUES(?,?,?,?,?,?,?)";
      int permissionLevel = AccessPermissionEnum.getIdByPermission(permission);
      boolean visible = true;
      PreparedStatement ps = null;
      ...
      try {
        ...
        classLogger.error(Constants.STACKTRACE, e);
        throw new IllegalArgumentException(
            "An error occurred granting the user permission for all the engines of type "
                + logETypes);
      } finally {
        ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
      }
      ...
      Map<String, Boolean> currentEngineToVisibilityMap =
          getEnginesAndVisibilityUserHasExplicitAccess(userId, engineTypes);
   ...
   * @param endDate
   */
  public void grantNewUsersEngineAccess(
      String engineId, String permission, User user, String endDate) {
    Timestamp startDate = Utility.getCurrentSqlTimestampUTC();
    Timestamp verifiedEndDate = null;
    if (endDate != null) {
      verifiedEndDate = AbstractSecurityUtils.calculateEndDate(endDate);
    ...
    String query =
        "INSERT INTO ENGINEPERMISSION (USERID, ENGINEID, PERMISSION, VISIBILITY, PERMISSIONGRANTEDBY, PERMISSIONGRANTEDBYTYPE, DATEADDED, ENDDATE) VALUES(?,?,?,?,?,?,?,?)";
    int permissionLevel = AccessPermissionEnum.getIdByPermission(permission);
    PreparedStatement ps = null;
    try {
      ps = securityDb.getPreparedStatement(query);
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred adding user permissions for engine "
              + engineId
              + " with permission "
              + permission);
    } finally {
   ...
   * @param endDate
   */
  public void grantNewUsersProjectAccess(
      String projectId, String permission, User user, String endDate) {
    Timestamp startDate = Utility.getCurrentSqlTimestampUTC();
    Timestamp verifiedEndDate = null;
    if (endDate != null) {
      verifiedEndDate = AbstractSecurityUtils.calculateEndDate(endDate);
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred adding user permissions for this project");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
   ...
   * @param permission
   */
  public void grantAllProjectInsights(
      String projectId, String userId, String permission, User user) {
    int permissionLevel = AccessPermissionEnum.getIdByPermission(permission);
    Pair<String, String> userDetails = User.getPrimaryUserIdAndTypePair(user);
   ...
   * @param usageFrequency
   */
  public void editEngineUserPermission(
    ...
    Integer existingUserPermission =
        SecurityUserEngineUtils.getUserEnginePermission(existingUserId, engineId);
   ...
   * @throws IllegalAccessException
   */
  public static void editEngineUserPermissions(
      String engineId, List<Map<String, Object>> permission, User user)
      throws IllegalAccessException {
    ...
    Map<String, Integer> existingUserPermission =
        SecurityUserEngineUtils.getUserEnginePermissions(existingUserIds, engineId);
   ...
   * @return
   */
  public void editProjectUserPermission(
      String existingUserId, String projectId, String newPermission, User user, String endDate) {
    // make sure we are trying to edit a permission that exists
    Integer existingUserPermission =
        SecurityUserProjectUtils.getUserProjectPermission(existingUserId, projectId);
    if (existingUserPermission == null) {
      throw new IllegalArgumentException(
   ...
   * @throws IllegalAccessException
   */
  public static void editProjectUserPermissions(
      String projectId, List<Map<String, String>> requests, User user, String endDate)
      throws IllegalAccessException {
    Timestamp startDate = Utility.getCurrentSqlTimestampUTC();
    Timestamp verifiedEndDate = null;
    if (endDate != null) {
    ...
    // get user permissions to edit
    Map<String, Integer> existingUserPermission =
        SecurityProjectUtils.getUserProjectPermissions(existingUserIds, projectId);
  ...
  public void removeEngineUser(String existingUserId, String engineId) {
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred removing the users access to this engine");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
  ...
  public void removeEngineUsers(List<String> existingUserIds, String engineId) {
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred removing user permissions from this engine");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
  ...
  public void removeProjectUsers(List<String> existingUserIds, String projectId) {
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred removing user permissions from this project");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
  ...
  public void removeInsightUsers(List<String> existingUserIds, String projectId, String insightId) {
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred removing insight permissions for this project");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
  ...
  public void removeProjectUser(String existingUserId, String projectId) {
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred removing user permissions for this project");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
  ...
  public void deleteProjectInsights(String projectId, List<String> insightIds) throws Exception {
    ...
    String query =
        "DELETE FROM INSIGHT WHERE INSIGHTID "
            ...
            + "';";
    query +=
        "DELETE FROM USERINSIGHTPERMISSION  WHERE INSIGHTID "
            + insightFilters
            + " AND PROJECTID='"
            + projectId
            + "'";
   ...
   * @throws IllegalAccessException
   */
  public List<Map<String, Object>> getInsightUsers(
      String projectId,
      String insightId,
      String userId,
      String permission,
    ...
    if (hasPermission) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter(
              "USERINSIGHTPERMISSION__PERMISSION",
              "==",
              AccessPermissionEnum.getIdByPermission(permission)));
    }
  ...
  public static long getInsightUsersCount(
      String projectId, String insightId, String userId, String permission) {
    boolean hasUserId = userId != null && !(userId = userId.trim()).isEmpty();
    boolean hasPermission = permission != null && !(permission = permission.trim()).isEmpty();
    SelectQueryStruct qs = new SelectQueryStruct();
    QueryFunctionSelector fSelector = new QueryFunctionSelector();
   ...
   * @return
   */
  public void addAllInsightUsers(
      String projectId, String insightId, String permission, User user, String endDate) {
    Timestamp startDate = Utility.getCurrentSqlTimestampUTC();
    Timestamp verifiedEndDate = null;
    if (endDate != null) {
      verifiedEndDate = AbstractSecurityUtils.calculateEndDate(endDate);
    ...
    try {
      ...
      if (projectId != null && permission != null) {
        List<Map<String, Object>> users =
            getInsightUsersNoCredentials(projectId, insightId, null, -1, -1);
        for (Map<String, Object> userMap : users) {
          String userId = (String) userMap.get("id");
          int parameterIndex = 1;
          ps.setString(parameterIndex++, userId);
   ...
   * @return
   */
  public void editInsightUserPermission(
      String existingUserId,
      String projectId,
      String insightId,
      String newPermission,
    ...
    // make sure we are trying to edit a permission that exists
    Integer existingUserPermission =
        SecurityInsightUtils.getUserInsightPermission(existingUserId, projectId, insightId);
    if (existingUserPermission == null) {
      throw new IllegalArgumentException(
  ...
  public void removeInsightUser(String existingUserId, String projectId, String insightId) {
    ...
    try {
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred deleting user permissions for this insight");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
   ...
   * @return
   */
  public List<Map<String, Object>> getEngineUsersNoCredentials(
      String engineId, String searchTerm, long limit, long offset) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("SMSS_USER__ID", "id"));
    ...
    // Filter for sub-query
    {
      ...
      subQs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter(
              "ENGINEPERMISSION__PERMISSION", "!=", null, PixelDataType.NULL_VALUE));
    }
   ...
   * @return
   */
  public List<Map<String, Object>> getProjectUsersNoCredentials(
      String projectId, String searchTerm, long limit, long offset) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("SMSS_USER__ID", "id"));
    qs.addSelector(new QueryColumnSelector("SMSS_USER__TYPE", "type"));
    qs.addSelector(new QueryColumnSelector("SMSS_USER__USERNAME", "username"));
    ...
    // Filter for sub-query
    {
      ...
      subQs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter(
              "PROJECTPERMISSION__PERMISSION", "!=", null, PixelDataType.NULL_VALUE));
    }
    if (searchTerm != null && !(searchTerm = searchTerm.trim()).isEmpty()) {
      OrQueryFilter or = new OrQueryFilter();
      or.addFilter(SimpleQueryFilter.makeColToValFilter("SMSS_USER__ID", "?like", searchTerm));
   ...
   * @return
   */
  public List<Map<String, Object>> getInsightUsersNoCredentials(
      String projectId, String insightId, String searchTerm, long limit, long offset) {
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(new QueryColumnSelector("SMSS_USER__ID", "id"));
    qs.addSelector(new QueryColumnSelector("SMSS_USER__USERNAME", "username"));
    qs.addSelector(new QueryColumnSelector("SMSS_USER__NAME", "name"));
   ...
   * @param endDate
   */
  public void updateEngineUserPermissions(
      String engineId, String newPermission, User user, String endDate) {
    Pair<String, String> userDetails = User.getPrimaryUserIdAndTypePair(user);
    ...
    String query =
        "UPDATE ENGINEPERMISSION SET PERMISSION=?, PERMISSIONGRANTEDBY=?, PERMISSIONGRANTEDBYTYPE=?, DATEADDED=?, ENDDATE=? WHERE ENGINEID=?";
    PreparedStatement ps = null;
    try {
      ps = securityDb.getPreparedStatement(query);
      int parameterIndex = 1;
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred editing user permissions for this engine");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
   ...
   * @param endDate
   */
  public void updateProjectUserPermissions(
      String projectId, String newPermission, User user, String endDate) {
    Timestamp startDate = Utility.getCurrentSqlTimestampUTC();
    Timestamp verifiedEndDate = null;
    if (endDate != null) {
      verifiedEndDate = AbstractSecurityUtils.calculateEndDate(endDate);
    ...
    String query =
        "UPDATE PROJECTPERMISSION SET PERMISSION=?, PERMISSIONGRANTEDBY=?, PERMISSIONGRANTEDBYTYPE=?, DATEADDED=?, ENDDATE=? WHERE PROJECTID=?";
    PreparedStatement ps = null;
    try {
      ps = securityDb.getPreparedStatement(query);
      int parameterIndex = 1;
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred editing user permissions for this project");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
   ...
   * @param endDate
   */
  public void updateInsightUserPermissions(
      String projectId, String insightId, String newPermission, User user, String endDate) {
    Timestamp startDate = Utility.getCurrentSqlTimestampUTC();
    Timestamp verifiedEndDate = null;
    if (endDate != null) {
      verifiedEndDate = AbstractSecurityUtils.calculateEndDate(endDate);
    ...
    String updateQ =
        "UPDATE USERINSIGHTPERMISSION SET PERMISSION=?, PERMISSIONGRANTEDBY=?, PERMISSIONGRANTEDBYTYPE=?, DATEADDED=?, ENDDATE=? WHERE PROJECTID=? AND INSIGHTID=?";
    PreparedStatement ps = null;
    try {
      ps = securityDb.getPreparedStatement(updateQ);
      int parameterIndex = 1;
      ...
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "An error occurred updating the permissions for this insight");
    } finally {
      ConnectionUtils.closeAllConnectionsIfPooling(securityDb, ps);
    }
  }
   ...
   * @param endDate
   */
  public void grantNewUsersInsightAccess(
      String projectId, String insightId, String permission, User user, String endDate) {
    Timestamp startDate = Utility.getCurrentSqlTimestampUTC();
    Timestamp verifiedEndDate = null;
    if (endDate != null) {
  ...
  public List<Object[]> getUserEmailsGettingLocked() {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "SMSS_USER__LOCKED", "==", values, PixelDataType.BOOLEAN));
    ...
    //		String dateDiff = queryUtil.getDateDiffFunctionSyntax("day", "SMSS_USER.LASTLOGIN",
    // queryUtil.getCurrentSqlTimestampUTC());
    //
    //		String query = "SELECT DISTINCT SMSS_USER.EMAIL, (" + dateDiff + ") as DAYS_SINCE_LASTLOGIN
    // FROM SMSS_USER WHERE "
    //				+ "(LOCKED IS NULL OR LOCKED='false') AND (" + dateDiff + ") > " + (daysToLock -
    // daysToLockEmail);
    //
    //		IRawSelectWrapper wrapper = null;
    //		try {
   ...
   * @param endDate
   */
  public void approveEngineUserAccessRequests(
      String userId,
      String userType,
      String engineId,
    ...
    String updateQ =
        "UPDATE ENGINEACCESSREQUEST SET PERMISSION = ?, APPROVER_USERID = ?, APPROVER_TYPE = ?, APPROVER_DECISION = ?, APPROVER_TIMESTAMP = ? WHERE ID = ? AND ENGINEID = ?";
   ...
   * @param requests
   */
  public void denyEngineUserAccessRequests(
      String userId, String userType, String engineId, List<String> requestIds) {
    // bulk update to databaseaccessrequest table
    String updateQ =
        "UPDATE ENGINEACCESSREQUEST SET APPROVER_USERID = ?, APPROVER_TYPE = ?, APPROVER_DECISION = ?, APPROVER_TIMESTAMP = ? WHERE ID = ? AND ENGINEID = ?";
    PreparedStatement ps = null;
    try {
      java.sql.Timestamp timestamp = Utility.getCurrentSqlTimestampUTC();
      ps = securityDb.getPreparedStatement(updateQ);
   ...
   * @param endDate
   */
  public void approveProjectUserAccessRequests(
      String userId,
      String userType,
      String projectId,
      List<Map<String, Object>> requests,
    ...
    String updateQ =
        "UPDATE PROJECTACCESSREQUEST SET PERMISSION = ?, APPROVER_USERID = ?, APPROVER_TYPE = ?, APPROVER_DECISION = ?, APPROVER_TIMESTAMP = ? WHERE ID = ?";
    PreparedStatement updatePs = null;
    try {
      java.sql.Timestamp timestamp = Utility.getCurrentSqlTimestampUTC();
      updatePs = securityDb.getPreparedStatement(updateQ);
   ...
   * @param requests
   */
  public void denyProjectUserAccessRequests(
      String userId, String userType, String projectId, List<String> RequestIdList) {
    // bulk update to projectaccessrequest table
    String updateQ =
        "UPDATE PROJECTACCESSREQUEST SET APPROVER_USERID = ?, APPROVER_TYPE = ?, APPROVER_DECISION = ?, APPROVER_TIMESTAMP = ? WHERE ID = ?";
    PreparedStatement ps = null;
    try {
      java.sql.Timestamp timestamp = Utility.getCurrentSqlTimestampUTC();
      ps = securityDb.getPreparedStatement(updateQ);
   ...
   * @param endDate
   */
  public void approveInsightUserAccessRequests(
      String userId,
      String userType,
      String projectId,
      String insightId,
   ...
   * @param requests
   */
  public void denyInsightUserAccessRequests(
      String userId,
      String userType,
      String projectId,
      String insightId,
  ...
  public String getProjectMarkdown(String projectId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("PROJECTMETA__PROJECTID", "==", projectId));
  ...
  public String getEngineMarkdown(String engineId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINEMETA__METAKEY", "==", Constants.MARKDOWN));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINEMETA__ENGINEID", "==", engineId));
    return QueryExecutionUtility.flushToString(securityDb, qs);
  }
