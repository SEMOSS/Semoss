...
public class ReadOnlyAccessToken extends AccessToken implements Serializable {
	...
	public static AccessToken unmodifiableToken(AccessToken token) {
		...
		if (token.meta != null) {
			Map<String, Collection<String>> deepUnmodifiableMeta = token.meta.entrySet().stream().collect(
					Collectors.toMap(Map.Entry::getKey, e -> Collections.unmodifiableCollection(e.getValue())));
