...
public class ExternalDatabaseMetadataHelper {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(ExternalDatabaseMetadataHelper.class);
  ...
  private static String getClientApiJsonResponse(String databaseId) throws Exception {
    ...
    String requestKey =
        Utility.getDIHelperProperty(Constants.EXTERNAL_DATABASE_MANAGEMENT_REQUEST_KEY);

    JSONObject requestBody = new JSONObject();
    requestBody.put(requestKey, databaseId);

    String username =
        Utility.getDIHelperProperty(Constants.EXTERNAL_DATABASE_MANAGEMENT_AUTH_USERNAME);
    String password =
        Utility.getDIHelperProperty(Constants.EXTERNAL_DATABASE_MANAGEMENT_AUTH_PASSWORD);
  ...
  private static Map<String, Map<String, String>> extractTableData(String apiResponse) {
    final String JMES_PATH_EXPRESSION =
        Utility.getDIHelperProperty(Constants.EXTERNAL_DATABASE_MANAGEMENT_RESPONSE_JMES_PATH);
    if (JMES_PATH_EXPRESSION == null || JMES_PATH_EXPRESSION.isEmpty()) {
      throw new IllegalArgumentException(
          "Must have a valid value for "
              + Constants.EXTERNAL_DATABASE_MANAGEMENT_RESPONSE_JMES_PATH);
    }
    final String TABLE_KEY =
        Utility.getDIHelperProperty(Constants.EXTERNAL_DATABASE_MANAGEMENT_TABLENAME);
    final String COLUMN_KEY =
        Utility.getDIHelperProperty(Constants.EXTERNAL_DATABASE_MANAGEMENT_COLUMNNAME);
    final String DATATYPE_KEY =
        Utility.getDIHelperProperty(Constants.EXTERNAL_DATABASE_MANAGEMENT_DATATYPE);
   ...
   * @throws Exception
   */
  private static void writeToOwl(
      Map<String, Map<String, String>> tableData, AbstractDatabaseEngine database)
      throws Exception {
    try (WriteOWLEngine owlWriter = database.getOWLEngineFactory().getWriteOWL()) {
      ...
      for (Map.Entry<String, Map<String, String>> tableEntry : tableData.entrySet()) {
        ...
        for (Map.Entry<String, String> columnEntry : tableEntry.getValue().entrySet()) {
          ...
          classLogger.info(
              "Adding table: {} with column: {} (type: {})", tableName, columnName, columnDataType);
