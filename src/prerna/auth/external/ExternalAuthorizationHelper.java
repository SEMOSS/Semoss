...
public class ExternalAuthorizationHelper {
  ...
  public static void updateEnginePermissionsBasedOnApiCall(User user) throws Exception {
    try {
      ...
      List<Map<String, Object>> newEngines =
      ...
      for (Map<String, Object> newE : newEngines) {
        ...
        if (engineType == IEngine.CATALOG_TYPE.DATABASE) {
          ...
          properties.put(
              Constants.OWL, Constants.DATABASE_FOLDER + "/@ENGINE@/" + engineName + "_OWL.OWL");
        }

        File tempSmss =
            UploadUtilities.createTemporaryEngineSmss(
                engineType, engineId, engineName, engineClass, properties);
        DIHelper.getInstance()
            .setEngineProperty(engineId + "_" + Constants.STORE, tempSmss.getAbsolutePath());
        File smssFile = new File(tempSmss.getAbsolutePath().replace(".temp", ".smss"));
        FileUtils.copyFile(tempSmss, smssFile);
        DIHelper.getInstance()
            .setEngineProperty(engineId + "_" + Constants.STORE, smssFile.getAbsolutePath());
        tempSmss.delete();
  ...
  private static String getClientApiJsonResponse(String emailId) throws Exception {
    ...
    String requestKey =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_REQUEST_KEY);

    JSONObject requestBody = new JSONObject();
    requestBody.put(requestKey, emailId);

    String username =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_AUTH_USERNAME);
    String password =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_AUTH_PASSWORD);
    ...
    return HttpHelperUtility.postRequestStringBody(
        url, headersMap, requestBody.toString(), ContentType.APPLICATION_JSON, null, null, null);
  }
  ...
  private static List<Map<String, Object>> transformApiResponse(User user, String apiResponse) {
    String defaultPermission =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_DEFAULT_PERMISSION);
    ...
    String defaultETypeStr =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_DEFAULT_ENGINE_TYPE);
    if (defaultETypeStr != null && !(defaultETypeStr = defaultETypeStr.trim()).isEmpty()) {
      try {
        ...
        classLogger.warn(
            "Invalid "
                + Constants.EXTERNAL_PERMISSION_MANAGEMENT_DEFAULT_ENGINE_TYPE
                + " value = "
                + defaultEType);
      }
    }

    final String ENGINEID_KEY =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_ENGINEID);
    ...
    final String ENGINENAME_KEY =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_ENGINENAME);
    ...
    final String JMES_PATH_EXPRESSION =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_RESPONSE_JMES_PATH);
    if (JMES_PATH_EXPRESSION == null || JMES_PATH_EXPRESSION.isEmpty()) {
      throw new IllegalArgumentException(
          "Must have a valid value for "
              + Constants.EXTERNAL_PERMISSION_MANAGEMENT_RESPONSE_JMES_PATH);
    }
    final String ENGINETYPE_KEY =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_ENGINETYPE);
    final String ENGINESUBTYPE_KEY =
        Utility.getDIHelperProperty(Constants.EXTERNAL_PERMISSION_MANAGEMENT_ENGINESUBTYPE);
      ...
      for (JsonNode detail : parsedJsonNode) {
        ...
        if (ENGINETYPE_KEY != null && !ENGINETYPE_KEY.isEmpty() && detail.has(ENGINETYPE_KEY)) {
          ...
          try {
            ...
            classLogger.warn(
                "Engine type not found for value : "
                    + engineTypeStr
                    + " which was returned for user "
        ...
        if (ENGINESUBTYPE_KEY != null
            && !ENGINESUBTYPE_KEY.isEmpty()
            && detail.has(ENGINESUBTYPE_KEY)) {
          ...
          if (engineType == IEngine.CATALOG_TYPE.DATABASE) {
            ...
            if (rdbmsType != null) {
              ...
              classLogger.warn(
                  "Engine sub type not found for value : "
                      + engineSubTypeStr
                      + " which was returned for user "
                      + User.getSingleLogginName(user));
            }
