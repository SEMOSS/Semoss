...
public final class StatisticsUtilityMethods {
  ...
  public static Double quartileIgnoringNull(
      Double[] values, final double lowerPercent, final boolean isOrdered) {
    if (values == null || values.length == 0) {
      throw new IllegalArgumentException(ILLEGAL_ARGS_ERR);
    }
  ...
  public static double[] calculateZScoresIgnoringInfinity(
      final double[] values, final boolean isOrdered) {
    if (values == null || values.length == 0) {
      throw new IllegalArgumentException(ILLEGAL_ARGS_ERR);
    }
  ...
  public static double[] calculateZScoreRange(final double[] values, final boolean isOrdered) {
    ...
    double[] zScore =
        new double[end - start + 3]; // +3 due to minZScore, maxZScore, and including the end value
    zScore[0] = minZScore;
  ...
  public static String[] getZScoreRangeAsStringIgnoringNull(
      Double[] values, final boolean isOrdered) {
    NumberFormat formatter = new DecimalFormat("#.##");
    if (values == null || values.length == 0) {
      throw new IllegalArgumentException(ILLEGAL_ARGS_ERR);
    }
  ...
  public static boolean areValuesUniformlyDistributed(
      final int[] valueArr, final int p, final int N, final int m, final double alpha) {
    ...
    for (int i = 0; i < p; i++) {
      ...
      System.out.println(
          "Starting at index " + startIndex + " the average KSStat for all subsets is " + ksStat);
    }
