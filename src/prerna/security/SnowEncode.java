...
class SnowEncode extends BitFilter {
	...
	SnowEncode(boolean encode, boolean quiet, BitFilter output, BufferedReader in, PrintWriter out, int linelen) {
		encode_flag = encode;
		quiet_flag = quiet;
		next_filter = output;
		stream_in = in;
	...
	public boolean flush() {
		...
		if (!quiet_flag) {
			...
			if (lines_extra > 0) {
				/*
				 * System.err.println ( "Message exceeded available space by approximately " +
				 * (usage - 100.0) + "%."); System.err.println ("An extra  " + lines_extra +
				 * " lines were added.");
				 */
			} else {
				/*
				 * System.err.println ("Message used approximately " + usage +
				 * "% of available space.");
				 */
			}
		}
