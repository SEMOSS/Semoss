...
class SnowEncode extends BitFilter {
  ...
  private int wsgets(BufferedReader ins, char buf[]) {
    ...
    try {
      if ((s = ins.readLine()) == null) return (-1);
    } catch (IOException e) {
      return (-1);
    }

    for (i = s.length(); i > 0; i--) if (s.charAt(i - 1) > ' ') break;
  ...
  private void buffer_load(BufferedReader ins) {
    ...
    for (int i = 0; i < buffer_length; i++)
      if (buffer[i] == '\t') buffer_column = tabpos(buffer_column);
      else buffer_column++;
  ...
  private boolean append_whitespace(int nsp) {
    ...
    if (needs_tab) col = tabpos(col);

    if (nsp == 0) col = tabpos(col);
    else col += nsp;

    if (col >= line_length) return (false);
  ...
  private boolean write_value(int val) {
    if (!buffer_loaded) buffer_load(stream_in);

    if (!first_tab) { // Tab shows start of data
      while (tabpos(buffer_column) >= line_length) {
        if (!wsputs(stream_out, buffer, buffer_length)) return (false);
    ...
    if (lines_extra == 0) bits_available += 3;
  ...
  public boolean flush() {
    if (!encode_flag) return (next_filter.flush());
    ...
    while ((n = wsgets(stream_in, buffer)) >= 0) {
      ...
      if (!wsputs(stream_out, buffer, n)) return (false);
    }
  ...
  private boolean decode_bits(int spc) {
    ...
    if (!next_filter.receive_bit((spc & 1) != 0)) return (false);
    if (!next_filter.receive_bit((spc & 2) != 0)) return (false);
    if (!next_filter.receive_bit((spc & 4) != 0)) return (false);
  ...
  private boolean decode_whitespace(int idx_lo, int idx_hi) {
    ...
    for (int i = idx_lo; i < idx_hi; i++) {
      if (buffer[i] == '\t') {
        if (!decode_bits(spc)) return (false);
        spc = 0;
      } else if (buffer[i] == ' ') spc++;
    }

    if (spc > 0 && !decode_bits(spc)) return (false);
  ...
  private int read_whitespace(BufferedReader ins, char buf[]) {
    ...
    for (i = len; i > 0; i--) if (s.charAt(i - 1) > ' ') break;
  ...
  public boolean decode() {
    ...
    while ((len = read_whitespace(stream_in, buffer)) >= 0) {
      ...
      if (len == 0) continue;
      if (!start_tab_found && buffer[0] == ' ') continue;
      ...
      if (!decode_whitespace(start_idx, len)) return (false);
    }
  ...
  public void space_calculate() {
    ...
    storage[0] = storage[1] = 0;
    while ((n = wsgets(stream_in, buffer)) >= 0) whitespace_storage(n, storage);
