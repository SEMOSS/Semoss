...
class IceKey {
  ...
  private void scheduleBuild(int kb[], int n, int krot_idx) {
    ...
    for (i = 0; i < 8; i++) {
      ...
      for (j = 0; j < 3; j++) keySchedule[n + i][j] = 0;
  ...
  public void set(byte key[]) {
    ...
    if (rounds == 8) {
      for (i = 0; i < 4; i++) kb[3 - i] = ((key[i * 2] & 0xff) << 8) | (key[i * 2 + 1] & 0xff);
  ...
  public void clear() {
    ...
    for (i = 0; i < rounds; i++) for (j = 0; j < 3; j++) keySchedule[i][j] = 0;
  }
