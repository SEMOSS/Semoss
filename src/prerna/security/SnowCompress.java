...
class SnowCompress extends BitFilter {
  ...
  SnowCompress(boolean encode, boolean quiet, BitFilter output) {
    ...
    if (!encode_flag) bit_array = new boolean[256];
  }
    ...
    if (encode_flag) {
      ...
      if (++bit_count == 8) {
        ...
        for (int i = 0; i < s.length; i++) {
          if (!next_filter.receive_bit(s[i])) return (false);
          bits_out++;
        }
      ...
      if ((code = huffcode_find(bit_array, bit_count)) >= 0) {
        for (int i = 0; i < 8; i++) {
          ...
          if (!next_filter.receive_bit(b)) return (true);
        }
  ...
  public boolean flush() {
    if (encode_flag) {
      ...
      if (bits_out > 0 && !quiet_flag) {
        ...
        if (cpc < 0.0)
          System.err.println(
              "Compression enlarged data by " + -cpc + "% recommend not using compression.");
        else System.err.println("Compressed by " + cpc + "%.");
      }
    } else {
  ...
  private int huffcode_find(boolean bits[], int len) {
    for (int i = 0; i < 256; i++) {
      ...
      if (huffcodes[i].length != len) continue;
  ...
  private boolean huffcodes[][] = {
    {
      false, true, false, false, true, true, true, false, true, true, true, false, false, true,
