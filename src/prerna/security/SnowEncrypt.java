...
class SnowEncrypt extends BitFilter {
  ...
  SnowEncrypt(boolean encode, boolean quiet, BitFilter output, String passwd) {
    encode_flag = encode;
    quiet_flag = quiet;
    set_key(passwd);
    next_filter = output;
  ...
  private void set_key(String passwd) {
    ...
    if (level == 0) {
      if (!quiet_flag) System.err.println("Warning: an empty password is being used.");
      level = 1;
    } else if (level > 128) {
      if (!quiet_flag) System.err.println("Warning: password truncated to 1170 chars.");
      level = 128;
    }
  ...
  public boolean receive_bit(boolean bit) {
    if (key == null) return (next_filter.receive_bit(bit));

    if (encode_flag) {
      byte buf[] = new byte[8];

      key.encrypt(iv_block, buf);
      if ((buf[0] & 128) != 0) bit = !bit;

      // Shift the IV block one bit left
      for (int i = 0; i < 8; i++) {
        iv_block[i] <<= 1;
        if (i < 7 && (iv_block[i + 1] & 128) != 0) iv_block[i] |= 1;
      }
      iv_block[7] |= bit ? 1 : 0;
