...
public final class HttpHelperUtility {
  ...
  public static CloseableHttpClient getCustomClient(
      CookieStore cookieStore, String keyStore, String keyStorePass, String keyPass) {
    ...
    try {
      ...
      if (keyStore != null
        ...
        if (!keyStoreF.exists() && !keyStoreF.isFile()) {
          classLogger.warn(
              "Defined a keystore to use in the request but the file "
                  + keyStoreF.getAbsolutePath()
                  + " does not exist");
        } else {
          if (keyPass == null || keyPass.isEmpty()) {
            sslContextBuilder.loadKeyMaterial(
                keyStoreF, keyStorePass.toCharArray(), keyStorePass.toCharArray());
          } else {
            sslContextBuilder.loadKeyMaterial(
                keyStoreF, keyStorePass.toCharArray(), keyPass.toCharArray());
          }
        }
      }
      ...
      tlsStrategy =
          new DefaultClientTlsStrategy(
              ...
              new String[] {
                TLS.V_1_2.getId(), TLS.V_1_3.getId()
    ...
    connectionManager.setDefaultConnectionConfig(
        ConnectionConfig.custom().setConnectTimeout(Timeout.DISABLED).build());
  ...
  public static String getRequest(
    ...
    try (CloseableHttpClient httpClient =
        HttpHelperUtility.getCustomClient(null, keyStore, keyStorePass, keyPass)) {
   ...
   * @return
   */
  public static File getRequestFileDownload(
      String url,
      Map<String, String> headerMap,
      String keyStore,
      String keyStorePass,
    ...
    try (CloseableHttpClient httpClient =
      ...
      if (!fileDir.exists()) {
        ...
        if (!success) {
          classLogger.warn(
              "Unable to make the directory to save the file at location: "
   ...
   * @return
   */
  public static String postRequestUrlEncodedBody(
      String url,
   ...
   * @return
   */
  public static String postRequestStringBody(
      String url,
      Map<String, String> headersMap,
      String body,
   ...
   * @return
   */
  public static String putRequestStringBody(
      String url,
      Map<String, String> headersMap,
      String body,
      ContentType contentType,
   ...
   * @return
   */
  public static String putRequestUrlEncodedBody(
      String url,
      Map<String, String> headersMap,
      Map<String, String> bodyMap,
      String keyStore,
   ...
   * @return
   */
  public static int headRequestStatus(
      String url,
      Map<String, String> headersMap,
      String keyStore,
      String keyStorePass,
   ...
   * @return
   */
  public static String deleteRequestStringBody(
      String url,
      Map<String, String> headersMap,
      String keyStore,
      String keyStorePass,
  ...
  public static AccessToken getAccessToken(
      String url, Map<String, String> params, boolean json, boolean extract) {
    ...
    try {
      ...
      params.keySet().stream()
          .forEach(param -> paramList.add(new BasicNameValuePair(param, params.get(param))));
      // set within post
    ...
    if (tok != null && tok.getAccess_token() == null) {
      classLogger.warn(
          "Error occurred grabbing the access token: " + Utility.cleanLogString(result));
    }
   ...
   * @return
   */
  public static AccessToken getIdToken(
      String url, Map<String, String> params, boolean json, boolean extract) {
  ...
  public static String makeGetCall(
      String urlStr, String accessToken, Map<String, Object> params, boolean auth) {
    if (urlStr == null) {
    ...
    classLogger.info(
        "Return from " + urlStr + " with response " + responseCode + " = " + retString);
  ...
  // makes the call to every resource going forward with the specified keys as get
  public static BufferedReader getHttpStream(
      String urlStr, String accessToken, Map<String, Object> params, boolean auth) {
    // fill the params on the get since it is not null
    // fill the params on the get since it is not null
    if (params != null) {
  ...
  public static String makeBinaryFilePutCall(
      String url, String accessToken, String fileName, String localPath) {
    CloseableHttpClient httpclient = null;
    try {
      httpclient = HttpClients.createDefault();
      HttpPut httpput = new HttpPut(url);
  ...
  public static String makeBinaryFilePostCall(
      String url, String accessToken, String filename, String filepath) {
    CloseableHttpClient httpclient = null;
    try {
      httpclient = HttpClients.createDefault();
      HttpPost httppost = new HttpPost(url);
