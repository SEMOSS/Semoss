...
public class MSGraphAPICall {
  ...
  public String getUserDetails(
      AccessToken accessToken, String groupId, String searchTerm, String nextLink)
      throws Exception {
    classLogger.info("getUserDetails based on Graph Api");

    if (accessToken == null) {
      classLogger.info(
          "Graph call for Access Token is null, will attempt to use system credentials");
    ...
    if (nextLink == null) {
      ...
      if (groupId == null || groupId.isEmpty()) {
        uri =
            MicrosoftTokenFiller.MS_GRAPH_BASE_API
                + "/v1.0/users?"
                + searchParam
                + "$orderby=displayName&"
                + "$top=999&"
  ...
  private synchronized AccessToken refreshSystemAccessToken() throws IOException {
    ...
    if (newToken != null) {
      // Set the token expiration time based on the current time and the expires_in value,
      // subtracting 5 minutes (300 seconds)
