...
public class UpdateSqlInterpreter extends SqlInterpreter {
    ...
    for (int i = 0; i < numSelectors; i++) {
      ...
      if (v == null) {
        ...
        if (v.equals("<UUID>")) {
          ...
          sets.append(
              column + "=" + "'" + AbstractSqlQueryUtil.escapeForSQLStatement(userId) + "'");
        } else {
          sets.append(
              column + "=" + "'" + AbstractSqlQueryUtil.escapeForSQLStatement(v + "") + "'");
        }
  ...
  //		 * UPDATE Genre SET Genre.Genre='Comedy' WHERE Genre.Title_FK IN (SELECT Title.Title FROM
  // Title WHERE Title = 'Avatar')
  //		 */
  ...
  ////
  //////		// Add to qs
  ////		NounMetadata col = new NounMetadata(new QueryColumnSelector("Genre__Title_FK"),
  ...
  ////
  ////		// viewing results
  //////		IRawSelectWrapper it = WrapperManager.getInstance().getRawWrapper(coreEngine, "select *
  // from Nominated");
  ////		IRawSelectWrapper it = WrapperManager.getInstance().getRawWrapper(coreEngine, "select * from
  // Genre");
  ////		while(it.hasNext()) {
