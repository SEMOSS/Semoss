...
public class SetParamsReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (keyValue.containsKey(keysToGet[1])) type = keyValue.get(keysToGet[1]);

    if (keyValue.containsKey(keysToGet[4])) table = keyValue.get(keysToGet[4]);

    if (keyValue.containsKey(keysToGet[5])) operator = keyValue.get(keysToGet[5]);

    if (keyValue.containsKey(keysToGet[6])) operatorU = keyValue.get(keysToGet[6]);

    Object obj = insight.getVar(ImportParamOptionsReactor.PARAM_OPTIONS);

    if (obj == null) return getError("There is no params available to modify ");

    Map pixelMap = (Map) obj;

    if (!pixelMap.containsKey(pixelId)) return getError("No such pixel ");

    Map paramMap = (Map) pixelMap.get(pixelId);

    if (type.equalsIgnoreCase("column")) {
      if (column == null) return getError("No Column key specified ");
      updateColumn(paramMap, column, value);
    } else if (type.equalsIgnoreCase("table")) {
      if (column == null || table == null) return getError("No column or table key specified ");
  ...
  public void updateColumn(Map paramMap, String columnName, Object value) {
    if (columnName == null || !paramMap.containsKey(columnName)) return;
  ...
  public void updateAllColumnTable(Map tableMap, String table, Object value) {
    if (table != null && !tableMap.containsKey(table)) return;
  ...
  public void updateAllColumnTableOperator(
      Map columnTableOperatorMap, String columnTableOperator, Object value) {
  ...
  public void updateAllColumnTableOperatorU(
      Map columnTableOperatorUMap, String columnTableOperatorU, Object value) {
  ...
  public void updateParamStruct(
      Map columnTableOperatorUMap, String columnTableOperatorU, Object value) {
    List<ParamStruct> curList =
        (List<ParamStruct>) columnTableOperatorUMap.get(columnTableOperatorU);
