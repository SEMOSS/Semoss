...
public class SQLSetParamsReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    String type = "column";
    if (keyValue.containsKey(keysToGet[2])) type = keyValue.get(keysToGet[2]);
    ...
    if (frame != null && frame instanceof NativeFrame)
      sqs =
          ((NativeFrame) frame)
              .getQueryStruct(); // this.insight.getLastQS(insight.getLastPanelId());
    ...
    if (obj == null && sqs != null) {
      ...
      try {
        ...
        insight
            .getVarStore()
            .put(
                SQLGetParamsReactor.QS_WRAPPER,
    ...
    if (obj != null) {
      ...
      if (type.equalsIgnoreCase("column")) wrapper.replaceColumn(id, value);
      else if (type.equalsIgnoreCase("column_table")) wrapper.replaceTableColumn(id, value);
