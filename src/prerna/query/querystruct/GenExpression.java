...
public class GenExpression extends SelectQueryStruct implements IQuerySelector, Serializable {
	...
	public boolean telescope = false; // join - <- within it join join (select a, b,c from d) a -
										// GenExpression1(telescope =
	// true, alias=a).body(GenExpression2(select a, b,c from d))
	...
	public static StringBuffer printQS(GenExpression qs, StringBuffer buf) {
		...
		// System.err.println("Processing " + qs.aQuery + " <>" + qs.expression + "<>" +
		// qs.operation);
		String newLine = "\n";
		if (buf == null) {
		...
		if (qs != null && qs.operation != null && !qs.neutralize) {
			if (qs.operation.equalsIgnoreCase("select") || qs.operation.equalsIgnoreCase("querystruct")) {
				...
				for (int selIndex = 0; selIndex < qs.nselectors.size(); selIndex++) {
					...
					/*
					 * // if it is a column I need to put alias too else
					 * if(sqs.operation.equalsIgnoreCase("column") ||
					 * sqs.operation.equalsIgnoreCase("double") ||
					 * sqs.operation.equalsIgnoreCase("date") ||
					 * sqs.operation.equalsIgnoreCase("time") ||
					 * sqs.operation.equalsIgnoreCase("string") ) { buf.append(sqs.leftExpr);
					 * if(sqs.leftAlias != null) buf.append(" as ").append(sqs.leftAlias); } // need
					 * to handle the union, case, between
					 * 
					 * // add the alias if(sqs.alias != null) buf.append("as " + sqs.alias);
					 */
				}
				processed = true;
				...
				// table prefix on selectors necessary to disambiguate when join columns have
				// same name
				...
				processed = true;
			} else if (qs.operation.equalsIgnoreCase("double") || qs.operation.equalsIgnoreCase("date")
					|| qs.operation.equalsIgnoreCase("time") || qs.operation.equalsIgnoreCase("string")
					|| qs.operation.equalsIgnoreCase("long")) {
		...
		if (!processed && !qs.neutralize) {
			...
			} else if (qs.leftExpr != null) // accomodating for the paranthesis from
			{
				...
				// System.err.println("Dont know to handle " + qs.aQuery + "<> " +
				// qs.operation);
			}
		}
		// like the filter
		...
		if (qs.from != null) {
			...
			// if(qs.from.operation != null &&
			// qs.from.operation.equalsIgnoreCase("querystruct"))
			{
				// System.err.println("From operation is set to " + qs.from.operation + "
				// composite ?" +
				// qs.from.composite );
				buf.append(newLine);
				buf.append("  FROM ");
			...
			/*
			 * else { buf.append("  FROM " ); if(qs.from.leftExpr != null)
			 * buf.append(qs.from.leftExpr).append("  "); if(qs.from.leftAlias != null)
	...
	public static StringBuffer printQSRecursive(GenExpression qs, StringBuffer buf) {
		...
		// Account for when it is an or and both left item and right item are
		// neutralized
		// if(leftItem != null && leftItem instanceof GenExpression &&
		// ((GenExpression)leftItem).neutralize && rightItem != null && rightItem
		// instanceof
		// GenExpression && ((GenExpression)rightItem).neutralize)
		...
		if (leftItem instanceof GenExpression) {
			if (!((GenExpression) leftItem).neutralize) {
				...
				StringBuffer leftBuf = printQS((GenExpression) leftItem, new StringBuffer()); // need to account for
																								// neutralize here

				if (leftBuf.length() > 0) {
					if ((childLeftItem != null && childLeftItem instanceof GenExpression
							&& ((GenExpression) childLeftItem).composite)
							&& (childRightItem != null && childRightItem instanceof GenExpression
									&& ((GenExpression) childRightItem).composite)) {
						buf.append("(");
						buf.append(leftBuf);
						buf.append(")");
					} else
		...
		if (rightItem instanceof GenExpression) {
			if (!((GenExpression) rightItem).neutralize) {
				...
				if (rightBuf.length() > 0) {
					if ((childLeftItem != null && childLeftItem instanceof GenExpression
							&& ((GenExpression) childLeftItem).composite)
							&& (childRightItem != null && childLeftItem instanceof GenExpression
									&& ((GenExpression) childRightItem).composite)) {
						buf.append("(");
						buf.append(rightBuf);
						buf.append(")");
	...
	// public void replaceTableAlias(GenExpression gep, String oldName, String
	// newName)
	// {
	...
	public void addSelect(GenExpression selector) {
		...
		for (int selectorIndex = 0; selectorIndex < nselectors.size(); selectorIndex++) {
			...
			if (thisSelector.leftAlias != null && selector.leftAlias != null)
				if (thisSelector.leftAlias.contentEquals(selector.leftAlias))
					return;
				else if (thisSelector.leftAlias != null && selector.getLeftExpr() != null)
					if (thisSelector.leftAlias.contentEquals(selector.getLeftExpr()))
						return;
					else if (thisSelector.getLeftExpr() != null && selector.leftAlias != null)
						if (thisSelector.getLeftExpr().contentEquals(selector.leftAlias))
							return;
						else if (thisSelector.getLeftExpr() != null && selector.getLeftExpr() != null)
							if (thisSelector.getLeftExpr().contentEquals(selector.getLeftExpr()))
								return;
		}
	...
	public boolean compareSelectors(GenExpression inputExpression) {
		...
		for (int selectorIndex = 0; match && selectorIndex < inputExpression.nselectors.size(); selectorIndex++) {
			...
			for (int thisSelectorIndex = 0; !foundMatch
					&& thisSelectorIndex < this.nselectors.size(); thisSelectorIndex++) {
	...
	public static StringBuffer printLevel2(GenExpression qs, List<String> realTables, // this is where all the real
																						// tables are kept
			int level, GenExpression derivedKey, Map<GenExpression, List<GenExpression>> derivedColumns,
			...
			Map<String, String> tableAliases, // aliases for this column - also need something that will keep // also
			// keeps aliases for table
			String tableName, boolean derived, boolean projection) {
		// if the type is join.. you need to do other things
		...
		if (qs != null && qs.operation != null && !qs.neutralize) {
			if (qs.operation.equalsIgnoreCase("select") || qs.operation.equalsIgnoreCase("querystruct")) {
				...
				for (int selIndex = 0; selIndex < qs.nselectors.size(); selIndex++) {
					...
					GenExpression sqs = qs.nselectors.get(selIndex);
					// need to handle telescope
					printLevel2(sqs, realTables, level, null, derivedColumns, levelProjections, tableColumns,
							tableAliases, tableName, false, true);
				}
				processed = true;
			} else if (qs.operation.equalsIgnoreCase("column")) {
				String columnName = qs.leftExpr;
				// System.err.println(qs.currentTable + "<<>>" + qs.tableName + "<<>>" +
				// qs.tableAlias +
				// "<<>>" + tableName);

				processColumn(qs, derived, projection, tableColumns, tableAliases, projectionList, allColumnList,
						derivedColumns, derivedKey, qs.tableName);
				...
				String columnName = qs.leftExpr;
				processColumn(qs, derived, projection, tableColumns, tableAliases, projectionList, allColumnList,
						derivedColumns, derivedKey, tableName);
				processed = true;
			} else if (qs.operation.equalsIgnoreCase("opaque")) {
		...
		if (qs.operation != null && qs.operation.contains("union") && !qs.neutralize) {
			...
			for (int opIndex = 0; opIndex < opNames.size(); opIndex++) {
				printLevel2((GenExpression) operands.get(opIndex), realTables, level, null, derivedColumns,
						levelProjections, tableColumns, tableAliases, tableName, derived, projection);
			}
		...
		if (qs.operation != null && qs.operation.equalsIgnoreCase("between") && !qs.neutralize) {
			...
			printLevel2((GenExpression) qs.body, realTables, level, derivedKey, derivedColumns, levelProjections,
					tableColumns, tableAliases, tableName, true, projection);
			printLevel2((GenExpression) qs.leftItem, realTables, level, derivedKey, derivedColumns, levelProjections,
					tableColumns, tableAliases, tableName, true, projection);
			printLevel2((GenExpression) qs.rightItem, realTables, level, derivedKey, derivedColumns, levelProjections,
					tableColumns, tableAliases, tableName, true, projection);
			processed = true;
		...
		if (qs.operation != null && qs.operation.equalsIgnoreCase("cast") && !qs.neutralize) {
			...
			if (qs.leftItem != null && qs.leftItem instanceof GenExpression) {
				printLevel2((GenExpression) qs.leftItem, realTables, level, derivedKey, derivedColumns,
						levelProjections, tableColumns, tableAliases, tableName, true, projection);
			} else {
		...
		if (qs.operation != null && qs.operation.equalsIgnoreCase("function") && !qs.neutralize) {
			...
			List<GenExpression> parameters = thisExpr.expressions;
			for (int paramIndex = 0; paramIndex < parameters.size(); paramIndex++) {
				printLevel2((GenExpression) parameters.get(paramIndex), realTables, level, derivedKey, derivedColumns,
						levelProjections, tableColumns, tableAliases, tableName, true, projection);
			}
			if (qs.aQuery != null && qs.aQuery.equalsIgnoreCase(derivedKey.aQuery))
				derivedKey = null;
		...
		if (!processed && !qs.neutralize) {
			if (qs.recursive) {
				...
				printLevel2Recursive(qs, realTables, level, derivedKey, derivedColumns, levelProjections, tableColumns,
						tableAliases, tableName, derived, projection);
				if (qs.aQuery != null && qs.aQuery.equalsIgnoreCase(derivedKey.aQuery))
					derivedKey = null;
			} else if (qs instanceof WhenExpression) {
				// buf.append(((WhenExpression)qs).printOutput());
				...
				((WhenExpression) qs).printLevel(qs, realTables, level, derivedKey, derivedColumns, levelProjections,
						tableColumns, tableAliases, tableName, derived, projection);
				if (qs.aQuery != null && qs.aQuery.equalsIgnoreCase(derivedKey.aQuery))
					derivedKey = null;
			} else if (qs instanceof InGenExpression) {
				// add this to the
				...
				if (ig.inList.size() > 0 && ig.rightItem == null) {
					...
					// I have made this opaque
					for (int itemIndex = 0; itemIndex < ig.inList.size(); itemIndex++) {
						printLevel2(ig.inList.get(itemIndex), realTables, level, derivedKey, derivedColumns,
								levelProjections, tableColumns, tableAliases, tableName, derived, projection);
					}
					// buf.append(")");
				} else if (qs.rightItem != null && qs.rightItem instanceof GenExpression) {
					...
					// the subquery is stored it is stored in the body
					if (((GenExpression) qs.rightItem).telescope) {
						printLevel2(((GenExpression) qs.rightItem).body, realTables, level, derivedKey, derivedColumns,
								levelProjections, tableColumns, tableAliases, tableName, derived, projection);
					} else {
						printLevel2((GenExpression) qs.rightItem, realTables, level, derivedKey, derivedColumns,
								levelProjections, tableColumns, tableAliases, tableName, derived, projection);
					}
				}
				processed = true;
		...
		if (qs.from != null) {
			...
			// qs.from.operation.equalsIgnoreCase("querystruct"))
			{
				...
				if (qs.from.composite) {
					printLevel2((GenExpression) qs.from, realTables, level, derivedKey, derivedColumns,
							levelProjections, tableColumns, tableAliases, tableName, derived, false);
					if (qs.from.leftAlias != null)
						tableAliases.put(qs.from.leftAlias, qs.from.aQuery);
				} else {
					if (!realTables.contains(qs.from.leftExpr) && qs.from.leftExpr != null
							&& qs.from.leftExpr.length() > 0)
		...
		for (int joinIndex = 0; joinIndex < qs.joins.size(); joinIndex++) {
			...
			if (sqs.from != null && !sqs.neutralize) {
				if (sqs.from != null && sqs.from.composite) {
					printLevel2(sqs.from, realTables, level, derivedKey, derivedColumns, levelProjections, tableColumns,
							tableAliases, tableName, derived, false);
				} else {
					printLevel2((GenExpression) sqs.from, realTables, level, derivedKey, derivedColumns,
							levelProjections, tableColumns, tableAliases, tableName, derived, false);
					if (!realTables.contains(sqs.currentTable) && sqs.currentTable != null
							&& sqs.currentTable.length() > 0)
			...
			// process this as a query struct
			if (sqs.body != null && !sqs.neutralize) {
				printLevel2((GenExpression) sqs.body, realTables, level, derivedKey, derivedColumns, levelProjections,
						tableColumns, tableAliases, tableName, derived, false);
			}
		}

		// add the where
		if (qs.filter != null) {
			printLevel2((GenExpression) qs.filter, realTables, level, derivedKey, derivedColumns, levelProjections,
					tableColumns, tableAliases, tableName, derived, false);
		}
		...
		if (qs.ngroupBy.size() > 0) {
			for (int groupIndex = 0; groupIndex < qs.ngroupBy.size(); groupIndex++) {
				GenExpression gep = qs.ngroupBy.get(groupIndex);
				printLevel2((GenExpression) gep, realTables, level, derivedKey, derivedColumns, levelProjections,
						tableColumns, tableAliases, tableName, derived, false);
			}
		}
	...
	public static void printLevel2Recursive(GenExpression qs, List<String> realTables, // this is where all the real
																						// tables are kept
			int level, GenExpression derivedKey, Map<GenExpression, List<GenExpression>> derivedColumns,
			Map<Integer, List<GenExpression>> levelProjections, // all the columns in a given level
		...
		if (leftItem instanceof GenExpression) {
			if (!((GenExpression) leftItem).neutralize) {
				...
				printLevel2((GenExpression) leftItem, realTables, level, derivedKey, derivedColumns, levelProjections,
						levelColumns, aliases, tableName, derived, projection); // need to account for neutralize here
			}
		}
		...
		if (rightItem instanceof GenExpression) {
			if (!((GenExpression) rightItem).neutralize) {
				...
				printLevel2((GenExpression) rightItem, realTables, level, derivedKey, derivedColumns, levelProjections,
						levelColumns, aliases, tableName, derived, projection); // need to account for neutralize here
			}
		}
	}
	...
	// since there could be the same alias being used across levels for different
	// tables
	public static void processColumn(GenExpression qs, boolean derived, boolean projection,
			Map<String, List<GenExpression>> tableColumns, Map<String, String> aliases, List projectionList,
			List allColumnList, Map<GenExpression, List<GenExpression>> derivedColumns, GenExpression derivedKey,
			String tableName) {
