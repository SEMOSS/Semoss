...
public class GenExpression extends SelectQueryStruct implements IQuerySelector, Serializable {
  ...
  public boolean telescope =
      false; // join - <- within it join join (select a, b,c from d) a - GenExpression1(telescope =
  ...
  public void setRightExpresion(Object rightItem) {
    ...
    if (rightItem instanceof GenExpression) ((GenExpression) rightItem).parent = this;
  }

  public void setLeftExpresion(Object leftItem) {
    this.leftItem = leftItem;
    if (leftItem instanceof GenExpression) ((GenExpression) leftItem).parent = this;
  }
  ...
  public static StringBuffer printQS(GenExpression qs, StringBuffer buf) {
    ...
    if (qs != null && qs.operation != null && !qs.neutralize) {
      if (qs.operation.equalsIgnoreCase("select") || qs.operation.equalsIgnoreCase("querystruct")) {
        ...
        processed = true;
      } else if (qs.operation.equalsIgnoreCase("double")
          || qs.operation.equalsIgnoreCase("date")
          || qs.operation.equalsIgnoreCase("time")
          || qs.operation.equalsIgnoreCase("string")
    ...
    if (qs.operation != null && qs.operation.contains("union") && !qs.neutralize) {
      ...
      for (int opIndex = 0; opIndex < opNames.size(); opIndex++) {
        if (opIndex == 0) printQS((GenExpression) operands.get(opIndex), buf);
    ...
    if (qs.operation != null && qs.operation.equalsIgnoreCase("function") && !qs.neutralize) {
      ...
      if (!thisExpr.neutralizeFunction) {
        ...
        if (qs.distinct) buf.append(" DISTINCT ");
      ...
      buf.append(")");
      if (qs.leftAlias != null) buf.append(" AS ").append(qs.leftAlias);
      processed = true;
    }
    ...
    if (!processed && !qs.neutralize) {
      if (qs.recursive) {
        ...
        if (qs.leftAlias != null) buf.append(qs.leftAlias);
        ...
        if (ig.inList.size() > 0 && ig.rightItem == null) {
          ...
          for (int itemIndex = 0; itemIndex < ig.inList.size(); itemIndex++) {
            if (itemIndex != 0) buf.append(", ");
            StringBuffer newBuf = printQS(ig.inList.get(itemIndex), null);
            if (!newBuf.toString().startsWith("(")) buf.append("(");
            buf.append(newBuf);
            if (!newBuf.toString().endsWith(")")) buf.append(")");
          }
          // buf.append(")");
    ...
    if (qs.from != null) {
      ...
      {
        // System.err.println("From operation is set to " + qs.from.operation + " composite ?" +
        // qs.from.composite );
        buf.append(newLine);
        buf.append("  FROM ");
        if (qs.from.composite) buf.append("( ");
    ...
    for (int joinIndex = 0; joinIndex < qs.joins.size(); joinIndex++) {
      ...
      if (sqs.from != null && !sqs.neutralize) {
        ...
        buf.append(sqs.on);
        buf.append("  ");
        if (sqs.from != null && sqs.from.composite) buf.append("(");
        printQS(sqs.from, buf);
        if (sqs.from != null && sqs.from.composite) buf.append(")");
        if (sqs.from.leftAlias != null) buf.append("  AS ").append(sqs.from.leftAlias);
      }
      if (sqs.body != null
          && sqs.body.operation.equalsIgnoreCase("querystruct")
          && !sqs.neutralize) {
        open = "(";
        close = ")  ";
      }
    ...
    if (qs.norderBy.size() > 0) {
      ...
      for (int orderIndex = 0; orderIndex < qs.norderBy.size(); orderIndex++) {
        if (orderIndex != 0) buf = buf.append(" , ");
        ...
        if (!gep.neutralize) // accomodating if it has been removed
        {
          ...
          if (direction.length() > 0) buf.append(direction);
        }
      }
    }

    // limit and offset
    if (qs.limit != -1) buf.append(" LIMIT ").append(qs.limit);

    if (qs.offset != -1) buf.append(" OFFSET ").append(qs.offset);
  ...
  public static StringBuffer printQSRecursive(GenExpression qs, StringBuffer buf) {
    ...
    // ((GenExpression)leftItem).neutralize && rightItem != null && rightItem instanceof
    ...
    if (leftItem instanceof GenExpression) {
      if (!((GenExpression) leftItem).neutralize) {
        ...
        StringBuffer leftBuf =
            printQS(
                (GenExpression) leftItem,
                new StringBuffer()); // need to account for neutralize here

        if (leftBuf.length() > 0) {
          if ((childLeftItem != null
                  && childLeftItem instanceof GenExpression
                  && ((GenExpression) childLeftItem).composite)
              && (childRightItem != null
                  && childRightItem instanceof GenExpression
                  && ((GenExpression) childRightItem).composite)) {
            buf.append("(");
            buf.append(leftBuf);
            buf.append(")");
          } else buf.append(leftBuf).append(" ");
    ...
    } else buf.append(qs.leftItem).append(" ");

    if (operation != null) buf.append(" ").append(operation).append(" ");
      ...
      if (!((GenExpression) rightItem).neutralize) {
        ...
        if (rightBuf.length() > 0) {
          if ((childLeftItem != null
            ...
            buf.append(rightBuf);
            buf.append(")");
          } else buf.append(rightBuf).append(" ");
        } else if (operation.equalsIgnoreCase("and")) {
          // get the "and" gen expression to be added
          // True and True = True
          // False and True = false;
    ...
    } else buf.append(qs.rightItem).append(" ");
    return buf;
  }
  ...
  public void replaceTableAlias2(GenExpression qs, String oldName, String newName) {
    ...
    if (qs != null && qs.operation != null) {
      ...
      if (qs.operation.equalsIgnoreCase("column")) {
        ...
        else if (oldName == null) qs.tableName = newName;
      }
    }
    ...
    if (!processed) {
      if (qs.recursive) {
        ...
        InGenExpression ig = (InGenExpression) qs;
        if (qs.leftItem != null) replaceTableAlias2((GenExpression) qs.leftItem, oldName, newName);
    ...
    // add the from
    if (qs.from != null) replaceTableAlias2((GenExpression) qs.from, oldName, newName);
    ...
    for (int joinIndex = 0; joinIndex < qs.joins.size(); joinIndex++) {
      ...
      // this is is the inner join on <from>
      // how to tell if a join is a subjoin ?
      if (sqs.from != null) replaceTableAlias2((GenExpression) sqs.from, oldName, newName);
      if (sqs.body != null) replaceTableAlias2((GenExpression) sqs.body, oldName, newName);
    }

    // add the where
    if (qs.filter != null) replaceTableAlias2((GenExpression) qs.filter, oldName, newName);
  ...
  public void addQuoteToColumn(GenExpression qs, String quote) {
    ...
    if (qs.operation != null && qs.operation.contains("union")) {
      ...
      for (int opIndex = 0; opIndex < opNames.size(); opIndex++) {
        if (opIndex == 0) addQuoteToColumn((GenExpression) operands.get(opIndex), quote);
    ...
    // add the from
    if (qs.from != null) addQuoteToColumn((GenExpression) qs.from, quote);
    ...
    for (int joinIndex = 0; joinIndex < qs.joins.size(); joinIndex++) {
      ...
      // this is is the inner join on <from>
      // how to tell if a join is a subjoin ?
      if (sqs.from != null) addQuoteToColumn((GenExpression) sqs.from, quote);
      if (sqs.body != null) addQuoteToColumn((GenExpression) sqs.body, quote);
    }

    // add the where
    if (qs.filter != null) addQuoteToColumn((GenExpression) qs.filter, quote);
  ...
  public boolean compareSelectors(GenExpression inputExpression) {
    ...
    for (int selectorIndex = 0;
        match && selectorIndex < inputExpression.nselectors.size();
        selectorIndex++) {
      GenExpression thisSelector = nselectors.get(selectorIndex);
      boolean foundMatch = false;
      for (int thisSelectorIndex = 0;
          !foundMatch && thisSelectorIndex < this.nselectors.size();
          thisSelectorIndex++) {
  ...
  public static StringBuffer printLevel2(
      GenExpression qs,
      ...
      Map<String, List<GenExpression>>
          tableColumns, // all the columns in a given level do we need this ?
      Map<String, String>
          tableAliases, // aliases for this column - also need something that will keep // also
    ...
    if (levelProjections.containsKey(level)) projectionList = levelProjections.get(level);

    if (derivedColumns.containsKey(derivedKey)) derivedColumnList = derivedColumns.get(derivedKey);
    ...
    if (qs != null && qs.operation != null && !qs.neutralize) {
      if (qs.operation.equalsIgnoreCase("select") || qs.operation.equalsIgnoreCase("querystruct")) {
        ...
        String columnName = qs.leftExpr;
        // System.err.println(qs.currentTable + "<<>>" + qs.tableName + "<<>>" + qs.tableAlias +
        // "<<>>" + tableName);
        ...
        if (qs.tableName != null) tableName = qs.tableName;
        processColumn(
            qs,
            derived,
            projection,
    ...
    if (qs.operation != null && qs.operation.contains("union") && !qs.neutralize) {
      ...
      for (int opIndex = 0; opIndex < opNames.size(); opIndex++) {
        printLevel2(
            (GenExpression) operands.get(opIndex),
            realTables,
            level,
            null,
            derivedColumns,
    ...
    if (qs.operation != null && qs.operation.equalsIgnoreCase("between") && !qs.neutralize) {
      ...
      if (qs.aQuery != null && qs.aQuery.equalsIgnoreCase(derivedKey.aQuery)) derivedKey = null;
    }
    if (qs.operation != null && qs.operation.equalsIgnoreCase("cast") && !qs.neutralize) {
      // name of the function is in the left alias
      if (derivedKey == null) {
    ...
    if (qs.operation != null && qs.operation.equalsIgnoreCase("function") && !qs.neutralize) {
      ...
      for (int paramIndex = 0; paramIndex < parameters.size(); paramIndex++) {
        printLevel2(
            (GenExpression) parameters.get(paramIndex),
            realTables,
            level,
            derivedKey,
            derivedColumns,
    ...
    if (!processed && !qs.neutralize) {
      if (qs.recursive) {
        ...
        if (ig.inList.size() > 0 && ig.rightItem == null) {
          ...
          if (((GenExpression) qs.rightItem).telescope) {
            printLevel2(
                ((GenExpression) qs.rightItem).body,
                realTables,
                level,
                derivedKey,
                derivedColumns,
      ...
      // if(qs.from.operation != null && qs.from.operation.equalsIgnoreCase("querystruct"))
      {
        ...
        if (qs.from.composite) {
          printLevel2(
              ...
              derived,
              false);
          if (qs.from.leftAlias != null) tableAliases.put(qs.from.leftAlias, qs.from.aQuery);
        } else {
          if (!realTables.contains(qs.from.leftExpr)
              && qs.from.leftExpr != null
              && qs.from.leftExpr.length() > 0) realTables.add(qs.from.leftExpr);
          if (qs.from.leftAlias != null) tableAliases.put(qs.from.leftAlias, qs.from.leftExpr);
          // add the alias
        }
      ...
      if (sqs.from != null && !sqs.neutralize) {
        if (sqs.from != null && sqs.from.composite) {
          ...
          printLevel2(
              ...
              derived,
              false);
          if (!realTables.contains(sqs.currentTable)
              && sqs.currentTable != null
    ...
    if (derived) derivedColumns.put(derivedKey, derivedColumnList);
    ...
    if (leftItem instanceof GenExpression) {
      if (!((GenExpression) leftItem).neutralize) {
        ...
        printLevel2(
            ...
            tableName,
            derived,
            projection); // need to account for neutralize here
      }
    }
  ...
  public static void processColumn(
    ...
    if (tableColumns.containsKey(tableName)) tableColumnList = tableColumns.get(tableName);
    else tableColumnList = new Vector<GenExpression>();
