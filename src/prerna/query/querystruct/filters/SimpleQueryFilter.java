...
public class SimpleQueryFilter implements IQueryFilter {
	...
	public static enum FILTER_TYPE {
		COL_TO_QUERY, QUERY_TO_COL, COL_TO_COL, COL_TO_VALUES, VALUES_TO_COL, VALUE_TO_VALUE, COL_TO_LAMBDA,
		LAMBDA_TO_COL
	};
	...
	/**
	 * Looks at the type of the query filter To figure out how to merge values such
	 * that the filters are consolidated and can be appropriately constructed
	 * 
	 * @param otherFilter
	 */
	...
	private NounMetadata determineMerge(String comparator, Object filter1, Object filter2, PixelDataType type) {
		if (isAdditive(comparator)) {
			List<Object> newFilters = new ArrayList<Object>();
	...
	/**
	 * See if the values overlap due to some regex expression This assumes one of
	 * these is a regex comparator (?like or ?nlike"
	 * 
	 * @param otherQueryFilter
	 * @return
	 */
	...
	public boolean subtractInstanceFilters(SimpleQueryFilter otherQueryFilter) {
		...
		if (this.thisFilterType == FILTER_TYPE.COL_TO_VALUES && otherFilterType == FILTER_TYPE.COL_TO_VALUES) {
			...
			List<Object> thisRHS = new ArrayList<Object>();
			...
			List<Object> otherRHS = new ArrayList<Object>();
			...
			// lets make sure everything is a list
			// even if it is a single value
			List<Object> thisLHS = new ArrayList<Object>();
			...
			List<Object> otherLHS = new ArrayList<Object>();
	...
	/**
	 * See if the two query filters are equal in order to be equal, the left side,
	 * right side, and comparator must match
	 * 
	 * @param queryFilter
	 * @return
	 */
	...
	public String getStringRepresentation() {
		if (this.thisFilterType == FILTER_TYPE.COL_TO_VALUES) {
			...
			if (rObj instanceof List) {
				...
				if (size == 1) {
					return ((IQuerySelector) this.lComparison.getValue()).getQueryStructName() + " " + this.comparator
							+ " " + ((List) rObj).get(0);
				} else {
			...
			if (lObj instanceof List) {
				...
				return ((IQuerySelector) this.rComparison.getValue()).getQueryStructName() + " "
						+ IQueryFilter.getReverseNumericalComparator(this.comparator) + " " + lObj;
			}
	...
	private static boolean isValues(PixelDataType type) {
		return type == PixelDataType.CONST_DECIMAL || type == PixelDataType.CONST_INT
				|| type == PixelDataType.CONST_STRING || type == PixelDataType.CONST_DATE
				|| type == PixelDataType.CONST_TIMESTAMP || type == PixelDataType.BOOLEAN
				|| type == PixelDataType.NULL_VALUE || type == PixelDataType.FORMATTED_DATA_SET
	...
	/**
	 * Helper method to generate a column to values filter where the column is a set
	 * of strings
	 * 
	 * @param colQs      The string representing the pixel for a unique column
	 * @param comparator The comparator for the filter
	 * @param values     The value for the filter - can be List or scalar
	 *                   (interpreter handles both)
	 * @return
	 */
	 ...
	 * @param values        The value for the filter - can be List or scalar
	 *                      (interpreter handles both)
	 * @param rightDataType The data type for the object (if list, the values inside
	 *                      the list)
	 * @return
	 */
	public static SimpleQueryFilter makeColToValFilter(String colQs, String comparator, Object values,
			PixelDataType rightDataType) {
	 ...
	 * @return
	 */
	public static SimpleQueryFilter makeColToValFilter(IQuerySelector colSelector, String comparator, Object values,
			PixelDataType rightDataType) {
	 ...
	 * @return
	 */
	public static SimpleQueryFilter makeColToColFilter(IQuerySelector firstColSelector, String comparator,
			IQuerySelector secondColSelector) {
	 ...
	 * @return
	 */
	public static SimpleQueryFilter makeQuerySelectorToSubQuery(IQuerySelector selector, String comparator,
			SelectQueryStruct subQs) {
