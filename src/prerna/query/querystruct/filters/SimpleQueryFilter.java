...
public class SimpleQueryFilter implements IQueryFilter {
  ...
  /**
   * Looks at the type of the query filter To figure out how to merge values such that the filters
   * are consolidated and can be appropriately constructed
   *
   * @param otherFilter
   */
  public void merge(SimpleQueryFilter otherFilter) {
    ...
    if (this.thisFilterType == FILTER_TYPE.COL_TO_VALUES
        && otherType == FILTER_TYPE.COL_TO_VALUES) {
      // both match
      ...
      this.rComparison =
          determineMerge(this.comparator, myRFilter, otherRFilter, this.rComparison.getNounType());

    } else if (this.thisFilterType == FILTER_TYPE.COL_TO_VALUES
        && otherType == FILTER_TYPE.VALUES_TO_COL) {
      ...
      // merge them based on the comparator type
      this.rComparison =
          determineMerge(this.comparator, myRFilter, otherLFilter, this.rComparison.getNounType());
      ...
      // merge them based on the comparator type
      this.lComparison =
          determineMerge(this.comparator, myLFilter, otherRFilter, this.lComparison.getNounType());
      ...
      // merge them based on the comparator type
      this.lComparison =
          determineMerge(this.comparator, myLFilter, otherLFilter, this.lComparison.getNounType());
    }
  ...
  private NounMetadata determineMerge(
      String comparator, Object filter1, Object filter2, PixelDataType type) {
  ...
  private boolean isAdditive(String comparator) {
    if (this.comparator.equals("==")
        ...
        || this.comparator.equals("<>")
        || this.comparator.equals("?like")
        || this.comparator.equals("?nlike")
        || this.comparator.equals("?begins")
        || this.comparator.equals("?nbegins")
        || this.comparator.equals("?ends")
        || this.comparator.equals("?nends")) {
      return true;
    }
    return false;
  }
   ...
   * @return
   */
  public boolean equivalentColumnModifcation(
      SimpleQueryFilter otherQueryFilter, boolean compareComparators) {
    ...
    if (thisFilterType == FILTER_TYPE.COL_TO_VALUES
        && otherFilterType == FILTER_TYPE.COL_TO_VALUES) {
      // compare left hand side
      if (!this.lComparison
          .getValue()
          .toString()
          .equals(otherQueryFilter.lComparison.getValue().toString())) {
        return false;
      }
    } else if (thisFilterType == FILTER_TYPE.VALUES_TO_COL
        && otherFilterType == FILTER_TYPE.VALUES_TO_COL) {
      // compare right hand side
      if (!this.rComparison
          .getValue()
          .toString()
          .equals(otherQueryFilter.rComparison.getValue().toString())) {
        return false;
      }
    } else if (thisFilterType == FILTER_TYPE.COL_TO_VALUES
        && otherFilterType == FILTER_TYPE.VALUES_TO_COL) {
  ...
  /**
   * See if the values overlap due to some regex expression This assumes one of these is a regex
   * comparator (?like or ?nlike"
   *
   * @param otherQueryFilter
   * @return
   */
  public boolean isOverlappingRegexValues(
      SimpleQueryFilter otherQueryFilter, boolean biDirectional) {
    FILTER_TYPE thisFilterType = determineFilterType(this);
    FILTER_TYPE otherFilterType = determineFilterType(otherQueryFilter);
  ...
  public boolean subtractInstanceFilters(SimpleQueryFilter otherQueryFilter) {
    ...
    if (this.thisFilterType == FILTER_TYPE.COL_TO_VALUES
      ...
      // recreate this
      this.rComparison =
          new NounMetadata(thisRHS, this.rComparison.getNounType(), this.rComparison.getOpType());
      ...
      // recreate this
      this.lComparison =
          new NounMetadata(thisLHS, this.lComparison.getNounType(), this.lComparison.getOpType());
  ...
  /**
   * See if the two query filters are equal in order to be equal, the left side, right side, and
   * comparator must match
   *
   * @param queryFilter
   * @return
   */
  ...
  public IQueryFilter copy() {
    SimpleQueryFilter copy =
        new SimpleQueryFilter(lComparison.copy(), comparator, rComparison.copy());
    return copy;
  }
  ...
  public Set<String> getAllUsedTables() {
    ...
    // is the left hand side a column?
    if (isCol(lComparison)) {
      List<QueryColumnSelector> colValues =
          ((IQuerySelector) lComparison.getValue()).getAllQueryColumns();
    ...
    if (isCol(rComparison)) {
      List<QueryColumnSelector> colValues =
          ((IQuerySelector) rComparison.getValue()).getAllQueryColumns();
      for (QueryColumnSelector c : colValues) {
        usedCols.add(c.getTable());
      }
    }
  ...
  public String getStringRepresentation() {
    if (this.thisFilterType == FILTER_TYPE.COL_TO_VALUES) {
      ...
      if (rObj instanceof List) {
        ...
        if (size == 1) {
          return ((IQuerySelector) this.lComparison.getValue()).getQueryStructName()
              + " "
              + this.comparator
              + " "
              + ((List) rObj).get(0);
      ...
      if (lObj instanceof List) {
        ...
        if (size == 1) {
          return this.rComparison.getValue()
              + " "
              + IQueryFilter.getReverseNumericalComparator(this.comparator)
              + " "
              + ((List) lObj).get(0);
        } else {
          StringBuilder builder = new StringBuilder("[");
      ...
      return ((IQuerySelector) this.lComparison.getValue()).getQueryStructName()
          ...
          + this.comparator
          + " "
          + ((IQuerySelector) this.rComparison.getValue()).getQueryStructName();
    } else {
      return this.lComparison.getValue()
          + " "
          + this.comparator
          + " "
          + this.rComparison.getValue();
    }
  }
   ...
   * @return
   */
  public static boolean requireOrBetweenFilters(
      SimpleQueryFilter leftFilterObj, SimpleQueryFilter rightFilterObj) {
    ...
    if (!IQueryFilter.comparatorIsNumeric(lComparator)
        || !IQueryFilter.comparatorIsNumeric(rComparator)) {
      return false;
    }
   ...
   * @return
   */
  public static SimpleQueryFilter makeColToValFilter(
      String colQs, String comparator, Object values) {
   ...
   */
  public static SimpleQueryFilter makeColToValFilter(
      String colQs, String comparator, Object values, PixelDataType rightDataType) {
   ...
   */
  public static SimpleQueryFilter makeColToValFilter(
      IQuerySelector colSelector, String comparator, Object values, PixelDataType rightDataType) {
   ...
   * @return
   */
  public static SimpleQueryFilter makeColToColFilter(
      String firstColQs, String comparator, String secondColQs) {
    NounMetadata lComparison =
        new NounMetadata(new QueryColumnSelector(firstColQs), PixelDataType.COLUMN);
    NounMetadata rComparison =
        new NounMetadata(new QueryColumnSelector(secondColQs), PixelDataType.COLUMN);
    SimpleQueryFilter filter = new SimpleQueryFilter(lComparison, comparator, rComparison);
    return filter;
  }
   ...
   */
  public static SimpleQueryFilter makeColToColFilter(
      IQuerySelector firstColSelector, String comparator, IQuerySelector secondColSelector) {
   ...
   * @return
   */
  public static SimpleQueryFilter makeColToSubQuery(
      String colQs, String comparator, SelectQueryStruct subQs) {
    NounMetadata lComparison =
        new NounMetadata(new QueryColumnSelector(colQs), PixelDataType.COLUMN);
   ...
   * @return
   */
  public static SimpleQueryFilter makeQuerySelectorToSubQuery(
      IQuerySelector selector, String comparator, SelectQueryStruct subQs) {
