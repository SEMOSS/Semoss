...
public class WhenExpression extends GenExpression {
  ...
  public StringBuffer printOutput2() {
    ...
    if (whens.size() > 0) {
      ...
      for (int whenIndex = 0; whenIndex < whens.size(); whenIndex++) {
        output
            .append("when ")
            .append(whens.get(whenIndex))
            .append(" then ")
            .append(thens.get(whenIndex))
            .append(" ");
      }
      ...
      if (this.leftAlias != null) output.append(" AS " + this.leftAlias);
    }
  ...
  public static StringBuffer printLevel(
      GenExpression qs,
      ...
      Map<String, String>
          aliases, // aliases for this column - also need something that will keep // also keeps
      // aliases for table
      String tableName,
      boolean derived,
      boolean projection) {
