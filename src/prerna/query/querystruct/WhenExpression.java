...
public class WhenExpression extends GenExpression {
	...
	public StringBuffer printOutput2() {
		...
		if (whens.size() > 0) {
			...
			for (int whenIndex = 0; whenIndex < whens.size(); whenIndex++) {
				output.append("when ").append(whens.get(whenIndex)).append(" then ").append(thens.get(whenIndex))
						.append(" ");
			}
	...
	public static StringBuffer printLevel(GenExpression qs, List<String> realTables, // this is where all the real
																						// tables are kept
			...
			Map<String, String> aliases, // aliases for this column - also need something that will keep // also keeps
			// aliases for table
		...
		if (we.whenE.size() > 0) {
			...
			for (int whenIndex = 0; whenIndex < we.whenE.size(); whenIndex++) {
				...
				thisWhen.printLevel2(thisWhen, realTables, level, derivedKey, derivedColumns, levelProjections,
						tableColumns, aliases, tableName, true, false);

				GenExpression thisThen = we.thenE.get(whenIndex);
				thisThen.printLevel2(thisThen, realTables, level, derivedKey, derivedColumns, levelProjections,
						tableColumns, aliases, tableName, true, false);
			}

			if (we.elseE != null) {
				printLevel2(we.elseE, realTables, level, derivedKey, derivedColumns, levelProjections, tableColumns,
						aliases, tableName, true, false);
			}
			output.append(" end");
		}
