...
public class SelectQueryStruct extends AbstractQueryStruct {
  ...
  /**
   * Returns order bys defined in the qs NOTE - USE COMBINED ORDER BY TO COMBINE PANEL ORDER BY ON
   * VISUALIZATIONS
   *
   * @return
   */
  ...
  public boolean isEmpty() {
    ...
    return (!this.selectors.isEmpty()
        || !this.relationsSet.isEmpty()
        || !this.explicitFilters.isEmpty());
  }

  /**
   * @param incomingQS
   *     <p>This method is responsible for merging "incomingQS's" data with THIS querystruct
   */
    ...
    for (IQuerySelector selector : this.selectors) {
      ...
      if (selectorType == IQuerySelector.SELECTOR_TYPE.FUNCTION) {
        ...
        if (innerSelector.get(0).getSelectorType() == IQuerySelector.SELECTOR_TYPE.COLUMN) {
          addDataType(
              (QueryColumnSelector) innerSelector.get(0), frameQuery, engineQuery, selectorMap);
        }
  ...
  private void addDataType(
    ...
    if (frameQuery && this.frame != null) {
      ...
      additionalDataType =
          MasterDatabaseUtility.getAdditionalDataType(additionalDataType, name, parent);
    }
  ...
  //	 */
  //	private void processRelationship(String startNode, Map<String, List> relMap, Map<String,
  // Set<String>> edgeHash) {
  ...
  //	/**
  //	 * Recursive method to find the shortest path to all the nearest concepts that are being
  // returned as selectors
  //	 * @param endNode					The endNode that is node a selector which we are trying to find the
  // shortest path to
  //	 * @param edgeHash					The edgeHash to find the current selectors
  //	 * @param newEndNodeList			The list of endNodes that have been found using the logic to find
  // the shortest
  ...
  public Map<String, Set<String>> getReturnConnectionsHash() {
    ...
    for (IQuerySelector anySelector : this.selectors) {
      ...
      if (!isPrimKey) {
        edgeHash.put(
            table + "__" + column,
            new HashSet<String>()); // add the property as a key pointing to nothing
    ...
    //			// if the props doesn't contain a prim_key_placeholder... then it is actually just a
    // property and not a concept
