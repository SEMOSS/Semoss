...
public class QsToPixelConverter {
  ...
  /**
   * Convert the joins into its pixel string Does not include the Join syntax but only provides the
   * contents inside
   *
   * @param joins
   * @return
  ...
  private static String convertArithmeticSelector(QueryArithmeticSelector selector) {
    StringBuilder b = new StringBuilder();
    b.append("(")
        .append(convertSelector(selector.getLeftSelector()))
        .append(selector.getMathExpr())
        .append(convertSelector(selector.getRightSelector()))
        .append(")");
    return b.toString();
  }
  ...
  /**
   * Convert a GenRowFilter into its pixel string Since GenRowFilter is used in many situations, the
   * method calling this is responsible for adding a Filter or Having around the contents it returns
   *
   * @param grf
   * @return
   */
  ...
  /**
   * Convert a simple filter into its pixel string Takes into account when we are doing this and
   * have a parameter Which must be a single item and starts with "<" and ends with ">"
   *
   * @param queryFilter
   * @return
   */
  ...
  //	 * @return
  //	 */
  //	public static QueryColumnOrderBySelector convertOrderBySelector(QueryColumnOrderBySelector
  // selector) {
