...
public class QsToPixelConverter {
	...
	public static String getPixel(SelectQueryStruct qs, boolean includeSource) {
		...
		// List<QueryColumnOrderBySelector> convertedOrderBys = new
		// Vector<QueryColumnOrderBySelector>();
		// for(int i = 0; i < origOrders.size(); i++) {
		// QueryColumnOrderBySelector origOrderS = origOrders.get(i);
		// QueryColumnOrderBySelector convertedOrderByS =
		// convertOrderBySelector(origOrderS);
	...
	/**
	 * Convert the joins into its pixel string Does not include the Join syntax but
	 * only provides the contents inside
	 *
	 * @param joins
	 * @return
	 */
	...
	/**
	 * Convert a GenRowFilter into its pixel string Since GenRowFilter is used in
	 * many situations, the method calling this is responsible for adding a Filter
	 * or Having around the contents it returns
	 *
	 * @param grf
	 * @return
	 */
	...
	/**
	 * Convert a simple filter into its pixel string Takes into account when we are
	 * doing this and have a parameter Which must be a single item and starts with
	 * "<" and ends with ">"
	 *
	 * @param queryFilter
	 * @return
	...
	// * @return
	// */
	// public static QueryColumnOrderBySelector
	// convertOrderBySelector(QueryColumnOrderBySelector
	// selector) {
	// String newQsName =
	// meta.getUniqueNameFromAlias(selector.getQueryStructName());
	// if(newQsName == null) {
	// // nothing to do
	// // return the original
	// return selector;
