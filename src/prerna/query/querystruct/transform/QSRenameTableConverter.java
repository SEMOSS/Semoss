...
public class QSRenameTableConverter {
	...
	private static final Logger classLogger = LogManager.getLogger(QSRenameTableConverter.class.getName());

	private QSRenameTableConverter() {

	}

	public static SelectQueryStruct convertQs(SelectQueryStruct qs, Map<String, String> transformationMap,
	...
	private static Set<IRelation> convertJoins(Set<IRelation> relationsSet, Map<String, String> transformationMap) {
		...
		for (IRelation relationship : relationsSet) {
			if (relationship.getRelationType() == IRelation.RELATION_TYPE.BASIC) {
				...
				convertedJoins.add(new BasicRelationship(
						new String[] { newStart, joinType, newEnd, rel.getComparator(), rel.getRelationName() }));
			} else {
	...
	private static IQuerySelector convertColumnSelector(QueryColumnSelector selector,
			Map<String, String> transformationMap, boolean keepOrigAlias) {
	...
	private static IQuerySelector convertArithmeticSelector(QueryArithmeticSelector selector,
			Map<String, String> transformationMap, boolean keepOrigAlias) {
	...
	private static IQuerySelector convertFunctionSelector(QueryFunctionSelector selector,
			Map<String, String> transformationMap, boolean keepOrigAlias) {
	...
	/**
	 * Convert an order by selector Same as conversion of a column selector, but
	 * adding the sort direction
	 * 
	 * @param selector
	 * @param meta
	 * @return
	...
	public static IQuerySort convertOrderByOperation(IQuerySort orderBy, Map<String, String> transformationMap) {
		if (orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN) {
			...
			QueryColumnSelector newS = new QueryColumnSelector(
					newTable + "__" + customSort.getColumnToSort().getColumn());
	...
	private static SimpleQueryFilter convertSimpleQueryFilter(SimpleQueryFilter queryFilter,
			Map<String, String> transformationMap, boolean keepOrigAlias) {
