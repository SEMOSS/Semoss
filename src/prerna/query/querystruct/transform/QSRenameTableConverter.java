...
public class QSRenameTableConverter {
  ...
  public static SelectQueryStruct convertQs(
      SelectQueryStruct qs, Map<String, String> transformationMap, boolean keepOrigAlias) {
    ...
    convertedQs.setImplicitFilters(
        convertGenRowFilters(qs.getImplicitFilters(), transformationMap, keepOrigAlias));
    convertedQs.setExplicitFilters(
        convertGenRowFilters(qs.getExplicitFilters(), transformationMap, keepOrigAlias));
    convertedQs.setHavingFilters(
        convertGenRowFilters(qs.getHavingFilters(), transformationMap, keepOrigAlias));

    // also do the frame and panel filters
    convertedQs.setFrameImplicitFilters(
        convertGenRowFilters(qs.getFrameImplicitFilters(), transformationMap, keepOrigAlias));
    convertedQs.setPanelImplicitFilters(
        convertGenRowFilters(qs.getPanelImplicitFilters(), transformationMap, keepOrigAlias));
    ...
    if (origGroups != null && !origGroups.isEmpty()) {
      ...
      for (int i = 0; i < origGroups.size(); i++) {
        ...
        IQuerySelector convertedGroupS =
            convertSelector(origGroupS, transformationMap, keepOrigAlias);
  ...
  private static Set<IRelation> convertJoins(
      Set<IRelation> relationsSet, Map<String, String> transformationMap) {
  ...
  public static IQuerySelector convertSelector(
      IQuerySelector selector, Map<String, String> transformationMap, boolean keepOrigAlias) {
    ...
    if (selectorType == IQuerySelector.SELECTOR_TYPE.CONSTANT) {
      ...
      return convertColumnSelector(
          (QueryColumnSelector) selector, transformationMap, keepOrigAlias);
    } else if (selectorType == IQuerySelector.SELECTOR_TYPE.FUNCTION) {
      return convertFunctionSelector(
          (QueryFunctionSelector) selector, transformationMap, keepOrigAlias);
    } else if (selectorType == IQuerySelector.SELECTOR_TYPE.ARITHMETIC) {
      return convertArithmeticSelector(
          (QueryArithmeticSelector) selector, transformationMap, keepOrigAlias);
    }
    return null;
  }

  private static IQuerySelector convertColumnSelector(
      QueryColumnSelector selector, Map<String, String> transformationMap, boolean keepOrigAlias) {
    ...
    if (transformationMap.containsKey(tableName)) {
      ...
      QueryColumnSelector newS =
          new QueryColumnSelector(newTableName + "__" + selector.getColumn());
  ...
  private static IQuerySelector convertArithmeticSelector(
      ...
      boolean keepOrigAlias) {
    QueryArithmeticSelector newS = new QueryArithmeticSelector();
    newS.setLeftSelector(
        convertSelector(selector.getLeftSelector(), transformationMap, keepOrigAlias));
    newS.setRightSelector(
        convertSelector(selector.getRightSelector(), transformationMap, keepOrigAlias));
   ...
   * @return
   */
  public static IQuerySort convertOrderByOperation(
      IQuerySort orderBy, Map<String, String> transformationMap) {
    if (orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN) {
      ...
      QueryColumnOrderBySelector newS =
          new QueryColumnOrderBySelector(newTable + selector.getColumn());
      ...
      QueryColumnSelector newS =
          new QueryColumnSelector(newTable + "__" + customSort.getColumnToSort().getColumn());
  ...
  public static GenRowFilters convertGenRowFilters(
      GenRowFilters grs, Map<String, String> transformationMap, boolean keepOrigAlias) {
  ...
  public static IQueryFilter convertFilter(
      IQueryFilter queryFilter, Map<String, String> transformationMap, boolean keepOrigAlias) {
    if (queryFilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
      return convertSimpleQueryFilter(
          (SimpleQueryFilter) queryFilter, transformationMap, keepOrigAlias);
  ...
  private static IQueryFilter convertOrQueryFilter(
      OrQueryFilter queryFilter, Map<String, String> transformationMap, boolean keepOrigAlias) {
  ...
  private static IQueryFilter convertAndQueryFilter(
      AndQueryFilter queryFilter, Map<String, String> transformationMap, boolean keepOrigAlias) {
  ...
  private static SimpleQueryFilter convertSimpleQueryFilter(
      SimpleQueryFilter queryFilter, Map<String, String> transformationMap, boolean keepOrigAlias) {
    ...
    if (origL.getNounType() == PixelDataType.COLUMN) {
      ...
      newL =
          new NounMetadata(
              convertSelector((IQuerySelector) origL.getValue(), transformationMap, keepOrigAlias),
    ...
    if (origR.getNounType() == PixelDataType.COLUMN) {
      ...
      newR =
          new NounMetadata(
              convertSelector((IQuerySelector) origR.getValue(), transformationMap, keepOrigAlias),
              PixelDataType.COLUMN);
    } else {
      newR = origR;
    }
