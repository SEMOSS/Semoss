...
public class QSAliasToPhysicalConverter {
	...
	private static IQuerySelector convertIfElseSelector(QueryIfSelector selector, OwlTemporalEngineMeta meta,
			String customTableName) {
	...
	private static IQuerySelector convertJsonifiedSelector(String uniqueName, OwlTemporalEngineMeta meta) {
		...
		if (selectorData != null) {
			...
			IQuerySelector selector = (IQuerySelector) GsonUtility.getDefaultGson().fromJson(selectorData[2].toString(),
	...
	private static IQuerySelector convertArithmeticSelector(QueryArithmeticSelector selector,
			OwlTemporalEngineMeta meta, String customTableName) {
	...
	private static SimpleQueryFilter convertSimpleQueryFilter(SimpleQueryFilter queryFilter, OwlTemporalEngineMeta meta,
		...
		if (origL.getNounType() == PixelDataType.COLUMN) {
			// need to convert
			newL = new NounMetadata(convertSelector((IQuerySelector) origL.getValue(), meta, customTableName),
		...
		if (origR.getNounType() == PixelDataType.COLUMN) {
			// need to convert
			newR = new NounMetadata(convertSelector((IQuerySelector) origR.getValue(), meta, customTableName),
					PixelDataType.COLUMN);
	...
	private static BetweenQueryFilter convertBetweenQueryFilter(BetweenQueryFilter queryFilter,
			OwlTemporalEngineMeta meta, String customTableName) {
	...
	private static GenRowFilters convertHavingGenRowFilters(GenRowFilters grs, OwlTemporalEngineMeta meta,
			Map<String, IQuerySelector> aliases, String customTableName) {
		List<IQueryFilter> origGrf = grs.getFilters();
		if (origGrf != null && !origGrf.isEmpty()) {
			GenRowFilters convertedGrf = new GenRowFilters();
			for (int i = 0; i < origGrf.size(); i++) {
	...
	private static IQuerySelector getNewSelectorForAlias(IQuerySelector selector, Map<String, IQuerySelector> aliases) {
		if (selector.getSelectorType() == IQuerySelector.SELECTOR_TYPE.COLUMN
			...
			// if(inners.size() == 1 && inners.get(0).getSelectorType() ==
