...
public class QSAliasToPhysicalConverter {
  ...
  private static final Logger logger =
      LogManager.getLogger(QSAliasToPhysicalConverter.class.getName());

  private QSAliasToPhysicalConverter() {}

  public static AbstractQueryStruct getPhysicalQs(
      AbstractQueryStruct qs, OwlTemporalEngineMeta meta) {
  ...
  public static UpdateQueryStruct getPhysicalQs(UpdateQueryStruct qs, OwlTemporalEngineMeta meta) {
    ...
    convertedQs.setHavingFilters(
        convertHavingGenRowFilters(qs.getHavingFilters(), meta, aliases, null));
  ...
  public static SelectQueryStruct getPhysicalQs(SelectQueryStruct qs, OwlTemporalEngineMeta meta) {
    ...
    // now go through the filters
    convertedQs.setImplicitFilters(
        convertGenRowFilters(qs.getImplicitFilters(), meta, customTableName));
    convertedQs.setExplicitFilters(
        convertGenRowFilters(qs.getExplicitFilters(), meta, customTableName));
    convertedQs.setHavingFilters(
        convertHavingGenRowFilters(qs.getHavingFilters(), meta, aliases, customTableName));

    // also do the frame and panel filters
    convertedQs.setFrameImplicitFilters(
        convertGenRowFilters(qs.getFrameImplicitFilters(), meta, customTableName));
    convertedQs.setPanelImplicitFilters(
        convertGenRowFilters(qs.getPanelImplicitFilters(), meta, customTableName));
  ...
  private static Set<IRelation> convertJoins(
      Set<IRelation> relationsSet, OwlTemporalEngineMeta meta) {
      ...
      if (relationship.getRelationType() == IRelation.RELATION_TYPE.BASIC) {
        ...
        convertedJoins.add(
            new BasicRelationship(
                new String[] {
                  newStart, joinType, newEnd, rel.getComparator(), rel.getRelationName()
                }));
      } else {
  ...
  public static IQuerySelector convertSelector(
      IQuerySelector selector, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  private static IQuerySelector convertIfElseSelector(
      QueryIfSelector selector, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  private static IQuerySelector convertColumnSelector(
      QueryColumnSelector selector, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  private static IQuerySelector convertJsonifiedSelector(
      String uniqueName, OwlTemporalEngineMeta meta) {
    Object[] selectorData = meta.getComplexSelector(uniqueName);
    if (selectorData != null) {
      // position 1 is the query type
      // position 2 is the json
      IQuerySelector.SELECTOR_TYPE type =
          IQuerySelector.convertStringToSelectorType(selectorData[1].toString());
      IQuerySelector selector =
          (IQuerySelector)
              GsonUtility.getDefaultGson()
                  .fromJson(
                      selectorData[2].toString(),
                      IQuerySelector.getQuerySelectorClassFromType(type));
      return selector;
    }
  ...
  private static IQuerySelector convertArithmeticSelector(
      QueryArithmeticSelector selector, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  private static IQuerySelector convertFunctionSelector(
      QueryFunctionSelector selector, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  /**
   * Convert an order by selector Same as conversion of a column selector, but adding the sort
   * direction
   *
   * @param selector
   * @param meta
  ...
  public static IQuerySort convertOrderByOperation(IQuerySort orderBy, OwlTemporalEngineMeta meta) {
    if (orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN) {
      ...
      String newQsName =
          meta.getUniqueNameFromAlias(customSort.getColumnToSort().getQueryStructName());
      if (newQsName == null) {
        // nothing to do
        // return the original
        return customSort;
  ...
  public static GenRowFilters convertGenRowFilters(
      GenRowFilters grs, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  public static IQueryFilter convertFilter(
      IQueryFilter queryFilter, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  private static IQueryFilter convertOrQueryFilter(
      OrQueryFilter queryFilter, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  private static IQueryFilter convertAndQueryFilter(
      AndQueryFilter queryFilter, OwlTemporalEngineMeta meta, String customTableName) {
  ...
  private static SimpleQueryFilter convertSimpleQueryFilter(
      SimpleQueryFilter queryFilter, OwlTemporalEngineMeta meta, String customTableName) {
    ...
    if (origL.getNounType() == PixelDataType.COLUMN) {
      ...
      newL =
          new NounMetadata(
              convertSelector((IQuerySelector) origL.getValue(), meta, customTableName),
    ...
    if (origR.getNounType() == PixelDataType.COLUMN) {
      ...
      newR =
          new NounMetadata(
              convertSelector((IQuerySelector) origR.getValue(), meta, customTableName),
              PixelDataType.COLUMN);
  ...
  private static BetweenQueryFilter convertBetweenQueryFilter(
      BetweenQueryFilter queryFilter, OwlTemporalEngineMeta meta, String customTableName) {
    // need to convert column to the full name
    queryFilter.setColumn(convertSelector(queryFilter.getColumn(), meta, customTableName));
    return queryFilter;
  }

  private static GenRowFilters convertHavingGenRowFilters(
      GenRowFilters grs,
      OwlTemporalEngineMeta meta,
      Map<String, IQuerySelector> aliases,
  ...
  public static IQueryFilter convertFilter(
      ...
      String customTableName) {
    if (queryFilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
      return convertSimpleQueryFilter(
          (SimpleQueryFilter) queryFilter, meta, aliases, customTableName);
    } else if (queryFilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.AND) {
  ...
  private static SimpleQueryFilter convertSimpleQueryFilter(
    ...
    if (origL.getNounType() == PixelDataType.COLUMN) {
      ...
      try {
        newL =
            new NounMetadata(
                convertSelector(selector, meta, customTableName), PixelDataType.COLUMN);
  ...
  private static IQuerySelector getNewSelectorForAlias(
      IQuerySelector selector, Map<String, IQuerySelector> aliases) {
    if (selector.getSelectorType() == IQuerySelector.SELECTOR_TYPE.COLUMN
        && aliases.containsKey(selector.getAlias())) {
      ...
      //				List<IQuerySelector> inners = ((QueryFunctionSelector)
      // actualSelector).getInnerSelector();
