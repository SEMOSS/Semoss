...
public class QSParseParamStruct {
	...
	private static final Logger classLogger = LogManager.getLogger(QSParseParamStruct.class);
	...
	public void convertSimpleQueryFilter(SimpleQueryFilter filter, List<ParamStruct> paramList) {
		...
		if ((parameterizeLeft && parameterizeRight) || (!parameterizeLeft && !parameterizeRight)) {
			// cannot have both be columns
			// and need at least one to be a column
			// that is already in a sense a dynamically defined filter
			classLogger.debug("Must have at one part of the filter be a column");
			return;
		}

		if (!(selector instanceof QueryColumnSelector)) {
			classLogger.debug("Cannot parameterize on derived columns");
			return;
		}
		...
		if (IQueryFilter.comparatorIsNumeric(comparator) || IQueryFilter.isRegexComparator(comparator)) {
			// single valued
