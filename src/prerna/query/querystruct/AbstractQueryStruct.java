...
public abstract class AbstractQueryStruct {
	...
	public enum QUERY_STRUCT_TYPE {
		ENGINE, FRAME, CSV_FILE, EXCEL_FILE, PARQUET_FILE, RAW_ENGINE_QUERY, RAW_JDBC_ENGINE_QUERY,
	 ...
	 * 
	 * selectors: table name (equivalent to the name of a vertex for graphs) column
	 * name (equivalent to the property name on a vertex for graphs) alias single
	 * math operation
	 * 
	 * filters: column to column filtering column to values filtering values to
	 * column filtering
	 * 
	 * relationships: start column -> joinType -> end column
	 * 
	 */

	// For selectors
	protected List<IQuerySelector> selectors = new ArrayList<IQuerySelector>();
	...
	protected transient List<InsightPanel> panelList = new ArrayList<>();
	protected List<String> panelIdList = new ArrayList<>();
	...
	public List<GenExpression> joins = new ArrayList<GenExpression>();

	public List<GenExpression> nselectors = new ArrayList<GenExpression>();
	...
	public List<GenExpression> ngroupBy = new ArrayList<GenExpression>();

	public List<GenExpression> norderBy = new ArrayList<GenExpression>();
	...
	public static AbstractSemossTypeAdapter getAdapterForQueryStruct(QUERY_STRUCT_TYPE type) {
		...
		} else if (type == QUERY_STRUCT_TYPE.RAW_JDBC_ENGINE_QUERY
				|| type == QUERY_STRUCT_TYPE.RAW_RDF_FILE_ENGINE_QUERY) {
	...
	private List<GenExpression> removeExpression(String column, List<GenExpression> input) {
		...
		List<GenExpression> output = new ArrayList<GenExpression>();
		for (int selectIndex = 0; selectIndex < input.size(); selectIndex++) {
			...
			if (!(thisSelector.getLeftExpr() != null && thisSelector.getLeftExpr().equalsIgnoreCase(column))
					&& !(thisSelector.getLeftAlias() != null && thisSelector.getLeftAlias().equalsIgnoreCase(column))) {
