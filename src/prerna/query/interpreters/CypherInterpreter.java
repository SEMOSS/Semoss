...
public class CypherInterpreter extends AbstractQueryInterpreter {
    ...
    for (int i = 0; i < selectors.size(); i++) {
      ...
      if (selectorType == IQuerySelector.SELECTOR_TYPE.COLUMN) {
        ...
        if (selectorInfo != null) {
          ...
          selectorList.add(
              listCriteria
                  .append(intermediateNode)
                  .append(".")
                  .append(originalPropertyName)
                  .append(" ")
                  .append(AS)
                  .append(" ")
                  .append(aliasPropertyName)
                  .toString());
        }
  ...
  public void addFilters(
      List<IQueryFilter> filters, StringBuilder builder, boolean useAlias, IDatabaseEngine engine) {
  ...
  private StringBuilder processFilter(
      IQueryFilter filter, boolean useAlias, IDatabaseEngine engine) {
  ...
  private StringBuilder addSelectorToSelectorFilter(
      NounMetadata leftComp, NounMetadata rightComp, String thisComparator, boolean useAlias) {
    // get the left side
          ...
          if (thisComparator.equals("==")) {
            ...
            for (int i = 0; i < size; i++) {
              filterBuilder
                  ...
                  .append(" = ")
                  .append(
                      RSyntaxHelper.formatFilterValue(objects.get(i), SemossDataType.DATE, null));
              if ((i + 1) < size) {
                filterBuilder.append(" OR ");
              }
            }
            ...
            myFilterFormatted.append(
                RSyntaxHelper.formatFilterValue(objects.get(0), SemossDataType.DATE, null));
            filterBuilder
                .append(tempNode + ".")
                .append(leftSelectorExpression)
                .append(" ")
