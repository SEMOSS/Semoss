...
public class JsonInterpreter implements IQueryInterpreter {
	...
	private StringBuilder processSimpleQueryFilter(SimpleQueryFilter filter) {
		...
		if (fType == FILTER_TYPE.COL_TO_VALUES) {
			return processColToValFilter((IQuerySelector) filter.getLComparison().getValue(),
					filter.getRComparison().getValue());
		} else if (fType == FILTER_TYPE.VALUES_TO_COL) {
			return processColToValFilter((IQuerySelector) filter.getRComparison().getValue(),
					filter.getLComparison().getValue());
