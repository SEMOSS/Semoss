	...
	private String getRSubSyntax(String jFormat, String rFormat) {
		...
		if (matchedRRegex.size() > 0) {
			...
			for (int i = 0; i < rFormat.length(); i++) {
				...
				if (c.equals("%")) {
					String substr = (rFormat.substring(i, i + 2).equals("%O")) ? rFormat.substring(i, i + 3)
	...
	private String processConstantSelector(QueryConstantSelector selector) {
		...
		if (constant instanceof SubQueryExpression) {
			...
			try {
				...
				classLogger.error("Error generating or iterating the subquery task for the constant selector", e);
			} finally {
				if (innerTask != null) {
					try {
						innerTask.close();
					} catch (IOException e) {
						classLogger.error("Error closing the subquery task for the constant selector", e);
					}
				}
			}
