...
public class RInterpreter extends AbstractQueryInterpreter {
	...
	public String composeQuery() {
		...
		query.append(cachedFrame).append("[ ").append(this.filterCriteria.toString()).append(", ")
				.append(this.selectorCriteria.toString()).append(this.groupBys).append("]");

		mainQuery.append(cachedFrame)
				// mainQuery.append(this.dataTableName)
				.append("[ ").append(this.filterCriteria.toString()).append(", ")
				.append(this.selectorCriteria.toString()).append(this.groupBys).append("]");
		...
		if (!having.isEmpty()) {
			query.append("; ").append(this.tempVarName).append("<-").append(this.tempVarName).append("[ ")
		...
		for (String column : this.colDataTypes.keySet()) {
			...
			if (dataType == SemossDataType.DATE || dataType == SemossDataType.TIMESTAMP) {
				...
				if (validHeaders.contains(column)) {
					...
					// String javaFormat = this.additionalTypes.get(this.dataTableName + "__" +
					// column);
	...
	private void addDateConversionFunction(StringBuilder query, String tempVarName, String column, SemossDataType type,
		...
		query.append(
				"if (is.Date(" + tempVarName + "$" + column + ") || is.POSIXct(" + tempVarName + "$" + column + ")) {")
				.append("options(digits.secs =" + rFormat[1] + ");").append(tempVarName + "$" + column + " <- format("
						+ tempVarName + "$" + column + ", format='" + rFormat[0] + "')");
	...
	private String getRSubSyntax(String jFormat, String rFormat) {
		...
		if (matchedRRegex.size() > 0) {
			...
			for (int i = 0; i < rFormat.length(); i++) {
				...
				if (c.equals("%")) {
					...
					firstParam = firstParam.substring(0, firstParamCurIndex) + "\\\\"
			...
			// if minutes is one where we have to handle leading zeros, then need to ensure
			// "00" is left
			// as such
		...
		// check if delimiter between seconds and milliseconds needs to be addressed
		// (replaced if not a
		// period)
	...
	private void addSelector(List<IQuerySelector> selectors) {
		...
		for (int i = 0; i < numSelectors; i++) {
			...
			String tempName = "V" + i;
			selectorBuilder.append(tempName).append("=")
		...
		// append selectors + outputs to perform correct calculations + add correct
		// alias
	 ...
	 * Note, this returns everything without the alias (unless we have indicated to
	 * use alias) since this is called again from the base methods it calls to allow
	 * for complex math expressions
	 *
	 * @param selector
	 * @param tableName
	 ...
	 * @return
	 */
	private String processSelector(IQuerySelector selector, String tableName, boolean includeTableName,
			boolean useAlias) {
		if (useAlias) {
			if (includeTableName) {
	...
	private String processIfElseSelector(QueryIfSelector selector, String tableName, boolean includeTableName,
			boolean useAlias) {
	...
	private String processColumnSelector(QueryColumnSelector selector, String tableName, boolean includeTableName,
			boolean useAlias) {
		if (includeTableName) {
			if (useAlias) {
				return tableName + "$" + selector.getAlias();
	...
	private String processFunctionSelector(QueryFunctionSelector selector, String tableName, boolean includeTableName,
			boolean useAlias) {
	...
	private String processArithmeticSelector(QueryArithmeticSelector selector, String tableName,
			boolean includeTableName, boolean useAlias) {
		...
		return "(" + processSelector(leftSelector, tableName, includeTableName, useAlias) + " " + mathExpr + " "
	...
	private StringBuilder processSimpleQueryFilter(SimpleQueryFilter filter, String tableName, boolean useAlias,
		...
		if (fType == FILTER_TYPE.COL_TO_COL) {
			...
			return addSelectorToValuesFilter(rightComp, leftComp,
					IQueryFilter.getReverseNumericalComparator(thisComparator), tableName, useAlias, captureColumns);
			...
			return addSelectorToLambda(rightComp, leftComp, IQueryFilter.getReverseNumericalComparator(thisComparator),
					tableName, useAlias, captureColumns);
	...
	private StringBuilder addSelectorToSelectorFilter(NounMetadata leftComp, NounMetadata rightComp,
			String thisComparator, String tableName, boolean useAlias, boolean captureColumns) {
		// get the left side
		...
		if (thisComparator.equals("!=") || thisComparator.equals("<>")) {
			...
			// some operation
			filterBuilder.append(" !(as.character(").append(lSelector).append(") %like% as.character(")
					.append(rSelector).append(")) ");
		} else {
			...
			if (multi) {
				...
				if (SemossDataType.DATE == leftDataType || SemossDataType.TIMESTAMP == leftDataType) {
					if (thisComparator.equals("==")) {
						filterBuilder.append("(");
						for (int i = 0; i < size; i++) {
							filterBuilder.append(leftSelectorExpression).append(" == ").append(RSyntaxHelper
									.formatFilterValue(objects.get(i), leftDataType, additionalTimestampParameter));
							if ((i + 1) < size) {
								filterBuilder.append(" | ");
							}
						}
						...
						myFilterFormatted = RSyntaxHelper.formatFilterValue(objects.get(0), leftDataType,
								additionalTimestampParameter);
						filterBuilder.append(leftSelectorExpression).append(" ").append(thisComparator)
				...
				// now all the other types
					...
					// use in
					if (thisComparator.equals("==")) {
						filterBuilder.append(leftSelectorExpression).append(" ").append(" %in% ")
								.append(myFilterFormatted);
						// use %in%
					} else if (thisComparator.equals("!=") | thisComparator.equals("<>")) {
						filterBuilder.append("!(").append(leftSelectorExpression).append(" ").append(" %in% ")
					...
					// loop through results
						...
						if (SemossDataType.STRING == leftDataType) {
							...
							for (int i = 0; i < size; i++) {
								filterBuilder.append(startFilter).append("tolower(as.character(")
										.append(leftSelectorExpression).append(")) %like% tolower(")
										.append(RSyntaxHelper.formatFilterValue(objects.get(i), leftDataType, null))
										.append(")").append(endFilter);
								if ((i + 1) < size) {
									filterBuilder.append(" | ");
				...
				if (thisComparator.equals(BEGINS_COMPARATOR) || thisComparator.equals(ENDS_COMPARATOR)) {
					...
					if (SemossDataType.STRING == leftDataType) {
						expression = rFunction + "( tolower(" + leftSelectorExpression + "),"
								+ myFilterFormatted.toLowerCase() + ")";
					} else {
						expression = rFunction + "( tolower(as.character(" + leftSelectorExpression + ")),"
								+ myFilterFormatted.toLowerCase() + ")";
					}
					filterBuilder.append(expression);
					...
					if (SemossDataType.STRING == leftDataType) {
						filterBuilder.append("tolower(").append(leftSelectorExpression).append(") %like% tolower(")
								.append(myFilterFormatted).append(")");
					} else {
						filterBuilder.append("tolower(as.character(").append(leftSelectorExpression)
					...
					filterBuilder.append(leftSelectorExpression).append(" ").append(thisComparator).append(" ")
							.append(myFilterFormatted);
				}
			}
		}
	...
	private void addOrderBy(String tempTableName) {
		...
		for (IQuerySort orderBy : orderByList) {
			if (orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN) {
				...
				customSortArray.append(randomSortMatchVar).append(" <- ")
						.append(RSyntaxHelper.createRColVec(customSort.getCustomOrder(), SemossDataType.STRING, null))
						.append(";");

				builderOrdering.append("match(").append(tempTableName).append("$").append(orderByName).append(" , ")
						.append(randomSortMatchVar).append(")");
			}
		}
		...
		if (builderOrdering != null) {
			...
			orderBys.append(tempTableName).append(" <- ").append(tempTableName).append("[order(")
					.append(builderOrdering.toString()).append("),]");
		}
	}
  ...
  ////		System.out.println(query);
	...
	//
	// StringBuilder sb = new StringBuilder();
	// //regex that correspond to values that may potentially not need a leading
	// zero
	// String regex = "MdHm" ;
	// String firstParam = rFormat;
	// String secondParam = "";
	...
	// }
	//
	// //if minutes is one where we have to handle leading zeros, then need to
	// ensure "00"
	// is left as such
	// if (matchedRRegex.contains("%M")) {
	...
	// }
	//
	// //get delimiter between seconds and milliseconds, if present, from the java
	// format
	// int indexSeconds = jFormat.lastIndexOf("s");
	// int indexMilliSeconds = jFormat.indexOf("S");
	// if (indexSeconds < indexMilliSeconds) {
