...
public class RInterpreter extends AbstractQueryInterpreter {
  ...
  public String composeQuery() {
    if (this.dataTableName == null) {
      throw new IllegalArgumentException(
          "Please define the table name to use for the r data table query syntax to use");
    }
    ...
    if (this.qs instanceof HardSelectQueryStruct) {
      ...
      if (customQuery.length() > "select".length()
        ...
        // wrap as R
        query
            .append("as.data.table(sqldf(\"")
            .append(customQuery.replace("\"", "\\\""))
            .append("\"));");
    ...
    addFilters(
        qs.getCombinedFilters().getFilters(),
        this.dataTableName,
        this.filterCriteria,
        false,
        false);
    // add having filters
    addFilters(
        qs.getHavingFilters().getFilters(),
        this.tempVarName,
        this.havingFilterCriteria,
        true,
        true);
    ...
    for (String column : aggregatedDateVals.keySet()) {
      if (validHeaders.contains(column)) {
        addedColToDateChange = true;
        addDateConversionFunction(
            query, this.tempVarName, column, aggregatedDateVals.get(column), null);
      }
    }
  ...
  private void addDateConversionFunction(
      StringBuilder query,
      String tempVarName,
      String column,
      SemossDataType type,
      String javaFormatAdditionalType) {
    ...
    query
        .append(
            "if (is.Date("
                ...
                + column
                + ")) {")
        .append("options(digits.secs =" + rFormat[1] + ");")
        .append(
            tempVarName
                + "$"
                + column
  ...
  /**
   * Remove leading zeroes if instructed by the java date/time format and/or update delimiter
   * separating seconds and milliseconds, if applicable
   *
   * @param jFormat
   * @param rFormat
   * @return
  ...
  private String getRSubSyntax(String jFormat, String rFormat) {
    ...
    for (char ch : regex.toCharArray()) {
      ...
      if (count == 1) {
        ...
        String rRegex =
            (String.valueOf(ch).equals("M"))
                ? RSyntaxHelper.getValueJavaRDatTimeTranslationMap(ch + "1")
    ...
    if (matchedRRegex.size() > 0) {
      ...
      for (int i = 0; i < rFormat.length(); i++) {
        ...
        if (c.equals("%")) {
          String substr =
              (rFormat.substring(i, i + 2).equals("%O"))
                  ? rFormat.substring(i, i + 3)
                  : rFormat.substring(i, i + 2);
          ...
          firstParam =
              firstParam.substring(0, firstParamCurIndex)
                  + "\\\\"
                  + firstParam.substring(firstParamCurIndex);
          firstParamCurIndex += 3;
          secondParam += c;
        }
      }

      // if minutes is one where we have to handle leading zeros, then need to ensure "00" is left
      // as such
    ...
    // check if delimiter between seconds and milliseconds needs to be addressed (replaced if not a
    // period)
  ...
  private void addSelector(List<IQuerySelector> selectors) {
    ...
    for (int i = 0; i < numSelectors; i++) {
      ...
      selectorBuilder
          .append(tempName)
          .append("=")
          .append(processSelector(selector, this.dataTableName, false, false));
  ...
  /**
   * Method is used to generate the appropriate syntax for each type of selector Note, this returns
   * everything without the alias (unless we have indicated to use alias) since this is called again
  ...
  private String processSelector(
      IQuerySelector selector, String tableName, boolean includeTableName, boolean useAlias) {
    if (useAlias) {
      if (includeTableName) {
    ...
    if (selectorType == IQuerySelector.SELECTOR_TYPE.CONSTANT) {
      ...
      return processColumnSelector(
          (QueryColumnSelector) selector, tableName, includeTableName, useAlias);
    } else if (selectorType == IQuerySelector.SELECTOR_TYPE.FUNCTION) {
      return processFunctionSelector(
          (QueryFunctionSelector) selector, tableName, includeTableName, useAlias);
    } else if (selectorType == IQuerySelector.SELECTOR_TYPE.ARITHMETIC) {
      return processArithmeticSelector(
          (QueryArithmeticSelector) selector, tableName, includeTableName, useAlias);
    } else if (selectorType == IQuerySelector.SELECTOR_TYPE.IF_ELSE) {
      return processIfElseSelector(
          (QueryIfSelector) selector, tableName, includeTableName, useAlias);
    }
    return null;
  }

  private String processIfElseSelector(
      QueryIfSelector selector, String tableName, boolean includeTableName, boolean useAlias) {
  ...
  private String processColumnSelector(
      QueryColumnSelector selector, String tableName, boolean includeTableName, boolean useAlias) {
    if (includeTableName) {
      if (useAlias) {
        return tableName + "$" + selector.getAlias();
      }
  ...
  private String processFunctionSelector(
    ...
    if (function.equals(QueryFunctionHelper.GROUP_CONCAT)) {
      ...
      endExpr.append("))), collapse = \", \")");
    } else if (function.equals(QueryFunctionHelper.COUNT)
        || function.equals(QueryFunctionHelper.UNIQUE_COUNT)) {
      expression.append(QueryFunctionHelper.convertFunctionToRSyntax(function));
      expression.append("(na.omit(");
      endExpr.append("))");
    ...
    for (int i = 0; i < size; i++) {
      if (i == 0) {
        expression.append(
            processSelector(innerSelectors.get(i), tableName, includeTableName, false));
      } else {
        expression
            .append(",")
            .append(processSelector(innerSelectors.get(i), tableName, includeTableName, useAlias));
      }

      if (mathFunction
          && innerSelectors.get(i).getSelectorType() == IQuerySelector.SELECTOR_TYPE.COLUMN) {
  ...
  private String processArithmeticSelector(
    ...
    return "("
        + processSelector(leftSelector, tableName, includeTableName, useAlias)
        + " "
        + mathExpr
        + " "
        + processSelector(rightSelector, tableName, includeTableName, useAlias)
        + ")";
  }
  ...
  private StringBuilder processFilter(
      IQueryFilter filter, String tableName, boolean useAlias, boolean captureColumns) {
    IQueryFilter.QUERY_FILTER_TYPE filterType = filter.getQueryFilterType();
    if (filterType == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
      return processSimpleQueryFilter(
          (SimpleQueryFilter) filter, tableName, useAlias, captureColumns);
  ...
  private StringBuilder processOrQueryFilter(
      OrQueryFilter filter, String tableName, boolean useAlias, boolean captureColumns) {
    StringBuilder filterBuilder = new StringBuilder();
  ...
  private StringBuilder processAndQueryFilter(
      AndQueryFilter filter, String tableName, boolean useAlias, boolean captureColumns) {
    StringBuilder filterBuilder = new StringBuilder();
    List<IQueryFilter> filterList = filter.getFilterList();
    int numAnds = filterList.size();
    for (int i = 0; i < numAnds; i++) {
  ...
  private StringBuilder processBetweenQueryFilter(
      BetweenQueryFilter filter, String tableName, boolean useAlias) {
  ...
  private StringBuilder processSimpleQueryFilter(
      SimpleQueryFilter filter, String tableName, boolean useAlias, boolean captureColumns) {
    ...
    if (fType == FILTER_TYPE.COL_TO_COL) {
      return addSelectorToSelectorFilter(
          leftComp, rightComp, thisComparator, tableName, useAlias, captureColumns);
  ...
  private StringBuilder addSelectorToLambda(
    ...
    if (mergeMetadata
      ...
      return addSelectorToValuesFilter(
          leftComp, nounEvaluated, thisComparator, tableName, useAlias, captureColumns);
    }
  ...
  private StringBuilder addSelectorToValuesFilter(
    ...
    if (!objects.isEmpty()) {
      ...
      if (objects.size() > 1) {
        multi = true;
        myFilterFormatted =
            RSyntaxHelper.createRColVec(objects, formatDataType, additionalTimestampParameter);
      } else {
        // dont bother doing this if we have a date
        // since we cannot use "in" with dates
        myFilterFormatted =
            RSyntaxHelper.formatFilterValue(
                objects.get(0), formatDataType, additionalTimestampParameter);
      }
      ...
      if (multi) {
          ...
          if (thisComparator.equals("==")) {
            ...
            for (int i = 0; i < size; i++) {
              filterBuilder
                  ...
                  .append(
                      RSyntaxHelper.formatFilterValue(
                          objects.get(i), leftDataType, additionalTimestampParameter));
              if ((i + 1) < size) {
                filterBuilder.append(" | ");
              }
            }
            ...
            myFilterFormatted =
                RSyntaxHelper.formatFilterValue(
                    objects.get(0), leftDataType, additionalTimestampParameter);
            filterBuilder
                .append(leftSelectorExpression)
                .append(" ")
                .append(thisComparator)
        ...
        // now all the other types
          ...
          if (thisComparator.equals("==")) {
            ...
            String rFunction =
                thisComparator.equals(NOT_BEGINS_COMPARATOR) ? "startsWith" : "endsWith";
        ...
        if (thisComparator.equals(BEGINS_COMPARATOR) || thisComparator.equals(ENDS_COMPARATOR)) {
          ...
          if (myFilterFormatted.endsWith("\\\"")) {
            myFilterFormatted =
                myFilterFormatted.substring(0, myFilterFormatted.length() - 1) + "\\\"";
          }
          if (SemossDataType.STRING == leftDataType) {
            filterBuilder
                .append("tolower(")
  ...
  private StringBuilder addSelectorToQueryFilter(
    ...
    return addSelectorToValuesFilter(
        leftComp, newRightComp, thisComparator, tableName, useAlias, captureColumns);
  }
  ...
  private void addOrderBy(String tempTableName) {
    ...
    for (IQuerySort orderBy : orderByList) {
      if (orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN) {
        ...
        customSortArray
            .append(randomSortMatchVar)
            .append(" <- ")
            .append(
                RSyntaxHelper.createRColVec(
                    customSort.getCustomOrder(), SemossDataType.STRING, null))
            .append(";");
    ...
    if (builderOrdering != null) {
      ...
      orderBys
          ...
          .append(tempTableName)
          .append("[order(")
          .append(builderOrdering.toString())
          .append("),]");
    }
  }
  ...
  //            }
  //            if (count == 1) {
  //            	//if the character is matched once in the jFormat, then fetch corresponding r
  // syntax
  //            	String rRegex = (String.valueOf(ch).equals("M")) ?
  // RSyntaxHelper.getValueJavaRDatTimeTranslationMap(ch + "1") :
  //                	RSyntaxHelper.getValueJavaRDatTimeTranslationMap(String.valueOf(ch));
  //            	matchedRRegex.add(rRegex);
  //            }
  ...
  //                String c = Character.toString(rFormat.charAt(i));
  //                if (c.equals("%")) {
  //                    String substr = (rFormat.substring(i, i+2).equals("%O")) ?
  // rFormat.substring(i, i+3) : rFormat.substring(i, i+2);
  ...
  //            }
  //
  //            //if minutes is one where we have to handle leading zeros, then need to ensure "00"
  // is left as such
  //            if (matchedRRegex.contains("%M")) {
  //            	sb.append("('" + firstParam + "','" + secondParam + "', COLUMNNAME) %>% gsub(':0:',
  // ':00:', .)");
  //            } else {
