...
public class SparqlInterpreter extends AbstractQueryInterpreter {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (!colCast.isEmpty()) {
        expression
            .append(colCast)
            .append("(")
            .append(processSelector(innerSelectors.get(i)))
            .append(")");
      } else {
  ...
  private String processArithmeticSelector(QueryArithmeticSelector selector) {
    ...
    return "("
        + processSelector(leftSelector)
        + " "
        + mathExpr
        + " "
        + processSelector(rightSelector)
        + ")";
  }
   ...
   * @param nodeVarName
   */
  private String addNodePropertySelectorTriple(
      String propVarName, String property, String concept, String conceptVarName) {
    if (!this.addedSelectors.containsKey(propVarName)) {
      ...
      if (this.qsFilteredColumns.contains(propVarName)
          && !this.qsFilteredColumnsToNull.contains(propVarName)) {
        this.selectorWhereClause
            .append("{?")
            .append(conceptVarName)
            .append(" <")
            .append(propUri)
            .append("> ?")
            .append(propVarName)
  ...
  private void addJoin(String fromNode, String joinType, String toNode) {
    ...
    // TODO: how do i do these types of joins? i need to use this information to update the
    // selectors to have coalesce with them
    // TODO: how do i do these types of joins? i need to use this information to update the
    // selectors to have coalesce with them
    // TODO: how do i do these types of joins? i need to use this information to update the
    // selectors to have coalesce with them
    ...
    if (joinType.equals("inner.join")) {
      ...
      //			this.relationshipWhereClause.append("{?").append(randomRelVarName).append("
      // <").append(RDFS.SUBPROPERTYOF).append("> <").append(predURI).append(">}");
      this.relationshipWhereClause
          .append("{?")
          .append(fromNodeVarName)
          .append(" <")
          .append(predURI)
          .append("> ?")
          .append(toNodeVarName)
          .append("}");
      ...
      this.relationshipWhereClause
          .append("OPTIONAL {")
          //				.append("{?").append(randomRelVarName).append("
          // <").append(RDFS.SUBPROPERTYOF).append("> <").append(predURI).append(">}")
          .append("{?")
          .append(toNodeVarName)
          .append(" <")
  ...
  private void addHavings(GenRowFilters grf, String baseUri) {
    ...
    boolean first = true;
    for (IQueryFilter filter : filters) {
      // do not want this to go through the simple process which would try to add bind() and
      // bindings()
  ...
  private StringBuilder processSimpleQueryFilter(
      SimpleQueryFilter filter, String baseUri, boolean startSimple) {
  ...
  private StringBuilder addSelectorToLambda(
    ...
    if (mergeMetadata
      ...
      return addSelectorToValuesFilter(
          leftComp, nounEvaluated, thisComparator, baseUri, startSimple);
    }
  ...
  private StringBuilder addSelectorToSelectorFilter(
    ...
    StringBuilder filterBuilder =
        new StringBuilder(processSelector(leftSelector))
            .append(" ")
            .append(thisComparator)
            .append(" ")
            .append(processSelector(rightSelector));
  ...
  private StringBuilder addSelectorToValuesFilter(
    ...
    if (leftSelector instanceof QueryColumnSelector) {
      ...
      return addComplexExpressionToValuesFilter(
          columnExpression, thisComparator, rightObjects, rightComp.getNounType());
    }
  }

  private StringBuilder addComplexExpressionToValuesFilter(
      String leftExpression,
      String thisComparator,
      List<Object> rightObjects,
      PixelDataType nounType) {
  ...
  /**
   * Add a filter of variable name to set of values Will figure out the most optimal way to add the
   * filter
   *
   ...
   * @param isProp
   */
  private StringBuilder addColToValuesFilter(
      String leftCleanVarName,
      String thisComparator,
      List<Object> rightObjects,
      PixelDataType nounType,
      boolean isProp,
    ...
    if (!addNullCheck) {
      ...
      addNullCheck =
          IQueryInterpreter.getAllSearchComparators().contains(thisComparator)
              && (rightObjects.contains("n")
                  || rightObjects.contains("nu")
                  || rightObjects.contains("nul")
    ...
    if (!addNullCheck && numObjects == 1 && isSimple && thisComparator.equals("==")) {
      ...
      if (isProp) {
        if (nounType == PixelDataType.CONST_DECIMAL || nounType == PixelDataType.CONST_INT) {
          this.bindWhereClause
              .append("BIND(\"")
              .append(rightObjects.get(0))
              .append("\"^^<")
              .append(XSD.xdouble)
              .append("> AS ?")
              .append(leftCleanVarName)
              .append(")");
        } else {
          this.bindWhereClause
              .append("BIND(\"")
      ...
      if (hasMoreValuesThanNull) {
          ...
          if (isProp) {
            ...
            for (int i = 0; i < numObjects; i++) {
              ...
              filterBuilder
                  ...
                  .append(concpetType)
                  .append("/.*")
                  .append(rightObjects.get(i).toString().replace("%", "\\\\%"))
                  .append("\", 'i')");
            }
          }
        } else if (thisComparator.equals(BEGINS_COMPARATOR)
            || thisComparator.equals(NOT_BEGINS_COMPARATOR)) {
        ...
        } else if (thisComparator.equals(ENDS_COMPARATOR)
  ...
  /**
   * Need to determine if a filter is set to equal null Need to make sure this property definition
   * is an optional
   *
