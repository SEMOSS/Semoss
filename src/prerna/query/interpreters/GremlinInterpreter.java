...
public class GremlinInterpreter extends AbstractQueryInterpreter {
	...
	/**
	 * Get the selectors from the QS Store both the list of selectors (which
	 * includes names of vertices and properties) Also maintain a list of vertex to
	 * list of properties
	 */
	...
	protected void traverseRelations() {
		...
		if (edgeMap.isEmpty()) {
			...
			if (props != null) {
				// this.gt.has(getNodeType(selector),
				// getPhysicalNodeType(selector)).as(selector);

				// it is for a property on a vertex
				GraphTraversal twoStepT = __.as(selector);

				// logic to filter
				List<SimpleQueryFilter> startNodeFilters = this.allFilters
						.getAllSimpleQueryFiltersContainingColumn(selector);
	 ...
	 * Assumes all the joins are inner.joins... at the moment, not sure how to/what
	 * it would mean to do something like a left join or right join since there is
	 * only one graph backing a insight If there are no relations (like in the case
	 * where you want one column to be returned), it adds the selectors in the
	 * edgeMap
	 *
	 * @return
	 */
	...
	/**
	 * This is the bulk of the class Uses the edgeMap to figure out what things are
	 * connected
	 */
	 ...
	 * @param filterVec
	 */
	protected void addFiltersToPath(GraphTraversal traversalSegment, List<SimpleQueryFilter> filterVec,
		...
		for (SimpleQueryFilter filter : filterVec) {
			...
			if (filterType == SimpleQueryFilter.FILTER_TYPE.COL_TO_VALUES) {
				...
				processFilterColToValues(traversalSegment, rComp, lComp,
						IQueryFilter.getReverseNumericalComparator(comp), filterPropertyName);
			}
		}
	}
	 ...
	 * @param comparison
	 */
	protected void processFilterColToValues(GraphTraversal traversalSegment, NounMetadata colComp,
			NounMetadata valuesComp, String comparison, String filterPropertyName) {
		...
		if (comparison.equals("==")) {
			...
			int size = filterValues.size();
			if (size == 1) {
				traversalSegment = traversalSegment.has(filterPropertyName,
						GremlinRegexMatch.regex(filterValues.get(0)));
			} else {
				GraphTraversal[] ors = new GraphTraversal[size];
				for (int i = 0; i < size; i++) {
			...
			if (size == 1) {
				traversalSegment = traversalSegment
						.not(__.has(filterPropertyName, GremlinRegexMatch.regex(filterValues.get(0))));
			} else {
				GraphTraversal[] ors = new GraphTraversal[size];
				for (int i = 0; i < size; i++) {
					ors[i] = __.has(filterPropertyName, GremlinRegexMatch.regex(filterValues.get(i)));
	 ...
	 * @return
	 */
	protected List<GraphTraversal<Object, Object>> visitNode(String startName, Map<String, Set<String>> edgeMap,
			List<String> travelledEdges, List<String> travelledNodeProps,
			List<GraphTraversal<Object, Object>> traversals) {
		// TODO: should automatically add any properties that are required for the
		// passed in node
		...
		if (edgeMap.containsKey(startName)) {
			...
			while (downstreamIt.hasNext()) {
				...
				if (!travelledEdges.contains(edgeKey)) {
					if (logger.isDebugEnabled()) {
						logger.debug(
								"travelling from node = '" + startName + "' to node = '" + downstreamNodeType + "'");
					}
					...
					// add filters
					List<SimpleQueryFilter> nodeFilters = this.allFilters
							.getAllSimpleQueryFiltersContainingColumn(downstreamNodeType);
		...
		if (upstreamNodes != null && !upstreamNodes.isEmpty()) {
			...
			while (upstreamIt.hasNext()) {
				...
				if (!travelledEdges.contains(edgeKey)) {
					...
					// add filtering
					List<SimpleQueryFilter> nodeFilters = this.allFilters
							.getAllSimpleQueryFiltersContainingColumn(upstreamNodeType);
	...
	protected List<GraphTraversal<Object, Object>> getProperties(GraphTraversal twoStepT, String startName) {
		...
		if (propList != null) {
			for (String property : propList) { // iterate through properties
				...
				List<SimpleQueryFilter> propFilters = this.allFilters
						.getAllSimpleQueryFiltersContainingColumn(property);
	...
	/*
	 * Getters for the type and name of the node Default will be what we create on
	 * upload of tinker / frames of tinker
	 *
	...
	/**
	 * Method to get the actual type of the node This is important for loops when we
	 * want the node to not be doubled But need a way to distinguish on the FE
	 *
	 * @param node
	 * @return
	 */
