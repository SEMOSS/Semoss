...
public class GremlinInterpreter extends AbstractQueryInterpreter {
  ...
  /**
   * Get the selectors from the QS Store both the list of selectors (which includes names of
   * vertices and properties) Also maintain a list of vertex to list of properties
   */
  ...
  protected void traverseRelations() {
    ...
    if (edgeMap.isEmpty()) {
      ...
      if (props != null) {
        ...
        // logic to filter
        List<SimpleQueryFilter> startNodeFilters =
            this.allFilters.getAllSimpleQueryFiltersContainingColumn(selector);
  ...
  /**
   * Generates the edgeMap to determine what to traverse based on the relations Assumes all the
   * joins are inner.joins... at the moment, not sure how to/what it would mean to do something like
   * a left join or right join since there is only one graph backing a insight If there are no
   * relations (like in the case where you want one column to be returned), it adds the selectors in
   * the edgeMap
   *
   * @return
   */
  ...
  /** This is the bulk of the class Uses the edgeMap to figure out what things are connected */
  public void addNodeEdge(Map<String, Set<String>> edgeMap) {
    ...
    List<SimpleQueryFilter> startNodeFilters =
        this.allFilters.getAllSimpleQueryFiltersContainingColumn(startNode);
   ...
   * @param filterVec
   */
  protected void addFiltersToPath(
      GraphTraversal traversalSegment,
      List<SimpleQueryFilter> filterVec,
    ...
    for (SimpleQueryFilter filter : filterVec) {
      ...
      if (filterType == SimpleQueryFilter.FILTER_TYPE.COL_TO_VALUES) {
        ...
        processFilterColToValues(
            ...
            rComp,
            lComp,
            IQueryFilter.getReverseNumericalComparator(comp),
            filterPropertyName);
      }
    }
  }
   ...
   * @param comparison
   */
  protected void processFilterColToValues(
      GraphTraversal traversalSegment,
      NounMetadata colComp,
      NounMetadata valuesComp,
      String comparison,
      String filterPropertyName) {
    ...
    if (comparison.equals("==")) {
      ...
      if (size == 1) {
        traversalSegment =
            traversalSegment.has(filterPropertyName, GremlinRegexMatch.regex(filterValues.get(0)));
      } else {
        GraphTraversal[] ors = new GraphTraversal[size];
        for (int i = 0; i < size; i++) {
      ...
      if (size == 1) {
        traversalSegment =
            traversalSegment.not(
                __.has(filterPropertyName, GremlinRegexMatch.regex(filterValues.get(0))));
      } else {
        GraphTraversal[] ors = new GraphTraversal[size];
        for (int i = 0; i < size; i++) {
          ors[i] = __.has(filterPropertyName, GremlinRegexMatch.regex(filterValues.get(i)));
      ...
      while (downstreamIt.hasNext()) {
        ...
        if (!travelledEdges.contains(edgeKey)) {
          ...
          if (!travelledNodeProps.contains(startName)) {
            List<GraphTraversal<Object, Object>> propTraversals =
                getProperties(twoStepT, startName);
          ...
          // add filters
          List<SimpleQueryFilter> nodeFilters =
              this.allFilters.getAllSimpleQueryFiltersContainingColumn(downstreamNodeType);
          ...
          if (!travelledNodeProps.contains(downstreamNodeType)) {
            ...
            List<GraphTraversal<Object, Object>> propTraversals =
                getProperties(downStepT, downstreamNodeType);

            if (propTraversals.size() > 0) {
              GraphTraversal[] propArray = new GraphTraversal[propTraversals.size()];
              twoStepT =
                  twoStepT.match(propTraversals.toArray(propArray)).select(downstreamNodeType);
            }
          ...
          traversals =
              visitNode(
                  downstreamNodeType, edgeMap, travelledEdges, travelledNodeProps, traversals);
        }
      }
    }
    ...
    if (upstreamNodes != null && !upstreamNodes.isEmpty()) {
      ...
      while (upstreamIt.hasNext()) {
        ...
        if (!travelledEdges.contains(edgeKey)) {
          if (logger.isDebugEnabled()) {
            logger.debug(
                "travelling from node = '" + upstreamNodeType + "' to node = '" + startName + "'");
          }
          ...
          // add filtering
          List<SimpleQueryFilter> nodeFilters =
              this.allFilters.getAllSimpleQueryFiltersContainingColumn(upstreamNodeType);
          ...
          if (!travelledNodeProps.contains(upstreamNodeType)) {
            ...
            List<GraphTraversal<Object, Object>> propTraversals =
                getProperties(upStepT, upstreamNodeType);
          ...
          traversals =
              visitNode(upstreamNodeType, edgeMap, travelledEdges, travelledNodeProps, traversals);
        }
      }
    }
  ...
  protected List<GraphTraversal<Object, Object>> getProperties(
      GraphTraversal twoStepT, String startName) {
    List<GraphTraversal<Object, Object>> propTraversals =
        new Vector<GraphTraversal<Object, Object>>();
    ...
    if (propList != null) {
      for (String property : propList) { // iterate through properties
        ...
        List<SimpleQueryFilter> propFilters =
            this.allFilters.getAllSimpleQueryFiltersContainingColumn(property);
   ...
   * @return
   */
  protected Set<String> getUpstreamNodes(
      String downstreamNodeToFind, Map<String, Set<String>> edgeMap) {
  ...
  /**
   * This is the method that will query the node based on how the engine has been defined to query
   * the node
   *
   * @param gt
   * @param nodeType
  ...
  /**
   * Method to get the actual type of the node This is important for loops when we want the node to
   * not be doubled But need a way to distinguish on the FE
   *
   * @param node
   * @return
   */
