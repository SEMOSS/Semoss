		...
		for (IQuerySort orderBy : orderByList) {
			if (orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN) {
				...
				// account for sort being on table/column being returned
						...
						&& orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN && this.qsSelectorToAlias
								.containsKey(((QueryColumnOrderBySelector) orderBy).getQueryStructName())) {
					// we need to find the alias for this column and order by the alias
					String alias = this.qsSelectorToAlias
							.get(((QueryColumnOrderBySelector) orderBy).getQueryStructName());
