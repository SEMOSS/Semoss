...
public class SqlInterpreter extends AbstractQueryInterpreter {
	...
	public String composeQuery() {
		...
		if (customFrom != null && !customFrom.isEmpty()) {
			...
			query.append(distinct).append(selectors).append(" FROM (").append(customFrom)
	...
	private String processIfElseSelector(QueryIfSelector selector, boolean addProcessedColumn,
		...
		if (precedent.getSelectorType() == IQuerySelector.SELECTOR_TYPE.IF_ELSE) {
			// note - this is a full case when that is embedded and has its own CASE WHEN +
			// END
		...
		if (antecedent != null) {
			// if the antecedent is another if reactor.. we need to pull this and then
			// continue
	...
	protected String processArithmeticSelector(QueryArithmeticSelector selector) {
		...
		if (mathExpr.equals("/")) {
			return "( CAST(" + processSelector(leftSelector, false) + " AS DECIMAL) " + mathExpr + " CAST(NULLIF("
					+ processSelector(rightSelector, false) + ",0) AS DECIMAL) )";
		} else {
			return "( CAST(" + processSelector(leftSelector, false) + "  AS DECIMAL) " + mathExpr + " CAST("
					+ processSelector(rightSelector, false) + " AS DECIMAL) )";
		}
	}
  ...
  // ////////////////////////////////////////////
	...
	protected StringBuilder addSelectorToQueryFilter(NounMetadata leftComp, NounMetadata rightComp,
	...
	protected StringBuilder addSelectorToValuesFilter(NounMetadata leftComp, NounMetadata rightComp,
		...
		if (!objects.isEmpty() || !addNullCheck) {
			...
			if (isSearch || isNotSearch) {
				...
				filterBuilder.append(") " + thisFilterSearch + " (").append(myFilterFormatted.toLowerCase())
						.append(")");
				i++;
				for (; i < size; i++) {
	...
	public String getFormatedObject(String dataType, List<Object> objects, String comparator) {
		...
		if (dataType != null) {
			...
			if (SemossDataType.INT == type || SemossDataType.DOUBLE == type || SemossDataType.BOOLEAN == type) {
				...
				boolean isSearch = comparator.equalsIgnoreCase(SEARCH_COMPARATOR)
	...
	private StringBuilder createCustomOrderBy(String identifier, List<Object> values) {
		...
		for (Object val : values) {
			...
			if (val == null) {
				...
				builder.append("=").append("'").append(AbstractSqlQueryUtil.escapeForSQLStatement(val + ""))
						.append("'");
			}
		...
		for (int i = 0; i < numGroups; i++) {
			...
			if (this.subQsAliasNames.contains(tableConceptualName)) {
				...
				if (columnConceptualName.equals(SelectQueryStruct.PRIM_KEY_PLACEHOLDER)) {
					...
					groupByColumn = getPhysicalPropertyNameFromConceptualName(tableConceptualName,
							columnConceptualName);
				}
			}
  ...
  // /////////////////////////////////////////
	 ...
	 *
	 * @param tablePixelName
	 *            The pixel name of the table
	 * @return
	 */
	 ...
	 * @param tablePixelName
	 *            The pixel name of the table
	 * @param columnPixelName
	 *            The pixel name of the property
	 ...
	 *
	 * @param fromString
	 *            The start string defining the table/column
	 * @param toString
	 *            The end string defining the table/column
