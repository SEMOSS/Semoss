...
public class SqlInterpreter extends AbstractQueryInterpreter {
  ...
  // we will keep track of the conceptual names to physical names so we don't re-query the owl
  // multiple times
  ...
  protected transient Map<String, List<String[]>> relationshipConceptPropertiesMap =
      new HashMap<>();
  ...
  public String composeQuery() {
    ...
    if (customFrom != null && !customFrom.isEmpty()) {
      ...
      query
          ...
          .append(" FROM (")
          .append(customFrom)
          .append(" ) AS " + this.customFromAliasName);
    } else {
  ...
  /**
   * Loops through the selectors defined in the QS to add them to the selector string and considers
   * if the table should be added to the from string
   */
   ...
   * everything without the alias since this is called again from the base methods it calls to allow
  ...
  private String processIfElseSelector(
      QueryIfSelector selector, boolean addProcessedColumn, boolean anotherCondition) {
    ...
    if (precedent.getSelectorType() == IQuerySelector.SELECTOR_TYPE.IF_ELSE) {
      ...
      buf.append(" ( ")
          .append(processIfElseSelector((QueryIfSelector) precedent, addProcessedColumn, false))
          .append(" ) ");
    } else {
  ...
  protected String processArithmeticSelector(QueryArithmeticSelector selector) {
    ...
    if (mathExpr.equals("/")) {
      return "( CAST("
          + processSelector(leftSelector, false)
          + " AS DECIMAL) "
          + mathExpr
          + " CAST(NULLIF("
   ...
   *
   * @param fromCol The starting column, this can be just a table or table__column
      ...
      String toColumn = null;
      {
        ...
        if (!fromConcept.contains("__")) {
          throw new IllegalArgumentException(
              "Subquery Joins require join details in format TABLE__COLUMN");
        }
  ...
  protected StringBuilder processSimpleQueryFilter(SimpleQueryFilter filter) {
    ...
    if (fType == FILTER_TYPE.COL_TO_COL) {
      ...
      return addSelectorToValuesFilter(
          rightComp, leftComp, IQueryFilter.getReverseNumericalComparator(thisComparator));
      ...
      return addSelectorToQueryFilter(
          leftComp, rightComp, IQueryFilter.getReverseNumericalComparator(thisComparator));
  ...
  protected StringBuilder addSelectorToQueryFilter(
      NounMetadata leftComp, NounMetadata rightComp, String thisComparator) {
    // get the left side
   ...
   * @param thisComparator
   */
  protected StringBuilder addSelectorToValuesFilter(
      NounMetadata leftComp, NounMetadata rightComp, String thisComparator) {
    ...
    if (leftDataType == null) {
      ...
      if (engine != null && !engine.isBasic()) {
        leftDataType =
            this.engine.getDataTypes(
                "http://semoss.org/ontologies/Concept/" + leftProperty + "/" + leftConcept);
    ...
    if (!objects.isEmpty() || !addNullCheck) {
      ...
      if (isSearch || isNotSearch) {
        ...
        boolean cast =
            SemossDataType.convertStringToDataType(leftDataType) != SemossDataType.STRING;
        if (cast) {
        ...
        filterBuilder
            .append(") " + thisFilterSearch + " (")
            .append(myFilterFormatted.toLowerCase())
            .append(")");
        i++;
        for (; i < size; i++) {
   ...
   * @param thisComparator
   */
  protected StringBuilder addSelectorToSelectorFilter(
      NounMetadata leftComp, NounMetadata rightComp, String thisComparator) {
    // get the left side
    IQuerySelector leftSelector = (IQuerySelector) leftComp.getValue();
    ...
    filterBuilder
        ...
        .append(" ")
        .append(processSelector(rightSelector, false));

    return filterBuilder;
  }

  protected StringBuilder addValueToValueFilter(
      NounMetadata leftComp, NounMetadata rightComp, String comparator) {
  ...
  /**
   * This is an optimized version when we know we can get all the objects into the proper sql query
   * string in one go
   *
   * @param dataType
   * @param objects
  ...
  public String getFormatedObject(String dataType, List<Object> objects, String comparator) {
    ...
    if (dataType != null) {
      ...
      if (SemossDataType.INT == type
          || SemossDataType.DOUBLE == type
          || SemossDataType.BOOLEAN == type) {
        // get the first value
        ...
        boolean isSearch =
            comparator.equalsIgnoreCase(SEARCH_COMPARATOR)
                || comparator.equals(NOT_SEARCH_COMPARATOR);
        if (isSearch) {
          leftWrapper = "'%";
          rightWrapper = "%'";
        } else {
        ...
        if (comparator.equalsIgnoreCase(SEARCH_COMPARATOR)
          ...
          leftWrapper = "'%";
          rightWrapper = "%'";
        } else if (comparator.equalsIgnoreCase(BEGINS_COMPARATOR)
            || comparator.equals(NOT_BEGINS_COMPARATOR)) {
          leftWrapper = "'";
          rightWrapper = "%'";
        } else if (comparator.equalsIgnoreCase(ENDS_COMPARATOR)
            || comparator.equals(NOT_ENDS_COMPARATOR)) {
          leftWrapper = "'%";
          rightWrapper = "'";
        } else {
          leftWrapper = "'";
    ...
    for (IQuerySort orderBy : orderByList) {
      if (orderBy.getQuerySortType() == IQuerySort.QUERY_SORT_TYPE.COLUMN) {
        ...
        // account for sort being on table/column being returned
          ...
          String orderByTable =
              getAlias(getPhysicalTableNameFromConceptualName(tableConceptualName));
          String orderByColumn = columnConceptualName;

          //							if(columnConceptualName.equals(SelectQueryStruct.PRIM_KEY_PLACEHOLDER)){
          //								orderByColumn = getPrimKey4Table(tableConceptualName);
          //							} else {
          //								orderByColumn = getPhysicalPropertyNameFromConceptualName(tableConceptualName,
          // columnConceptualName);
          //							}
    ...
    for (Object val : values) {
      ...
      if (val == null) {
        ...
        builder
            .append("=")
            .append("'")
            .append(AbstractSqlQueryUtil.escapeForSQLStatement(val + ""))
            .append("'");
      }
  ...
  public StringBuilder appendGroupBy(StringBuilder query) {
    ...
    for (int i = 0; i < numGroups; i++) {
      ...
      if (groupBySelector.getSelectorType() == IQuerySelector.SELECTOR_TYPE.COLUMN) {
        ...
        String errorMessage =
            "Cannot group by non QueryColumnSelector and QueryFunctionSelector and QueryIfSelector types yet...";
   ...
   */
  protected String getPhysicalPropertyNameFromConceptualName(
      String tablePixelName, String columnPixelName) {
  ...
  /**
   * Gets the 4 parts needed to define a relationship 1) the start table 2) the start tables column
   * 3) the end table 4) the end tables column
   *
   * <p>We have 3 situations 1) If all 4 parts are defined within the fromString and toString
   * parameters by utilizing a "__", then it just converts to the physical names and is done 2) If
   * startTable and start column is defined but endTable/endColumn is not, it assumes the input for
   * endString is a concept and should bind on its primary key. This is analogous for when endTable
   * and end column are defined but the startString is not. 3) Neither are defined, so we must use
   * the OWL to define the relationship between the 2 tables
   *
   * @param fromString The start string defining the table/column
   * @param toString The end string defining the table/column
   * @return String[] of length 4 where the indices are [startTable, startCol, endTable, endCol]
   */
  protected List<String[]> getRelationshipConceptProperties(String fromString, String toString) {
    ...
    if (fromString.contains("__")) {
      ...
      if (engine != null && !engine.isBasic()) {
        ...
        fromCol =
            getPhysicalPropertyNameFromConceptualName(fromConceptualTable, fromConceptualColumn);
      } else {
    ...
    // if neither has a property specified, use owl to look up foreign key relationship
        ...
        && (fromCol == null
            && toCol
                == null)) // in this case neither has a property specified. time to go to owl to get
    // fk relationship
    {
      String fromURI = null;
      String toURI = null;
      ...
      try {
        ...
        while (res.hasNext()) {
          ...
          {
            // EHUB_CLM_SDS . EHUB_CLM_EVNT . CLM_EVNT_KEY . EHUB_CLM_SDS . EHUB_CLM_PROV_DMGRPHC .
            // CLM_EVNT_KEY
            // [0]               [1]            [2]             [3]             [4]
            //   [5]
  ...
  /**
   * Returns the physical concept name and property name for a given input If the input contains a
   * "__" it returns the physical from both the the concept and the property If the input doesn't
   * contain a "__", get the concept and the primary key
   *
   * @param concept_property The input string
   * @return String[] containing the concept physical at index 0 and property physical at index 1
   */
