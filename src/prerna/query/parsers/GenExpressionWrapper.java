...
public class GenExpressionWrapper {
  ...
  // this is the highest level - acctid - when I replace this it will replace this across all of the
  ...
  // this is the next highest level - clms.acctid, mbrshp.acctid - this will replace it only for the
  // specified operators not all of it
  ...
  public Map<String, List<GenExpression>> functionExpressionMapper =
      new HashMap<String, List<GenExpression>>();
  ...
  public void appendParameter(
      List<String> columnsToRemove, Map<String, GenExpression> paramValues) {
    // I still need to account for if the user already comes with this
    ...
    for (int colIndex = 0; colIndex < columnsToRemove.size(); colIndex++) {
      ...
      while (allSelects.hasNext()) {
        ...
        // there is a possibility where there are 2 different aliases refering to the same column
        // name ?
    ...
    while (paramIterator.hasNext()) {
      ...
      if (exprs != null) {
        for (int exprIndex = 0; exprIndex < exprs.size(); exprIndex++) {
          ...
          String quote = "";
          if (daStruct.getType() == PixelDataType.CONST_STRING) quote = "";
          ...
          else thisExpression.setLeftExpr(finalValue.toString());
        }
      }
    }
  }
   ...
   */
  public void fillParameters(
      List<ParamStructDetails> incomingStructs,
    ...
    for (int paramIndex = 0; paramIndex < incomingStructs.size(); paramIndex++) {
      ...
      if (thisStructLevel == LEVEL.COLUMN) {
        ...
        if (operatorTableColumnParamIndex.containsKey(paramStructDetailsKey)) {
          ParamStructDetails targetStruct =
              operatorTableColumnParamIndex.get(paramStructDetailsKey);
          // replace the target struct with the user defined param name
          List<GenExpression> exprs = paramToExpressionMap.get(targetStruct);
          for (int exprIndex = 0; exprIndex < exprs.size(); exprIndex++) {
            // we will replace the existing parameter
  ...
  public void addFunctionExpression(String functionName, GenExpression expr) {
    ...
    else exprList = new ArrayList<GenExpression>();

    if (!exprList.contains(expr)) exprList.add(expr);

    functionExpressionMapper.put(functionName, exprList);
  }

  public static Object[] getLineage(
      GenExpression starter,
      String name,
      Map<Integer, List<GenExpression>> selectLineage,
      Map<Integer, GenExpression> columnLineage,
      List<GenExpression> allInstances,
      int level) {
    ...
    if (selectLineage == null) selectLineage = new HashMap<Integer, List<GenExpression>>();
    if (columnLineage == null) columnLineage = new HashMap<Integer, GenExpression>();

    if (allInstances == null) allInstances = new Vector<GenExpression>();

    GenExpression identifiedSelector = null;

    List<GenExpression> levelLineage = null;
    if (selectLineage.containsKey(level)) levelLineage = selectLineage.get(level);
    else levelLineage = new Vector<GenExpression>();
      ...
      for (int selectorIndex = 0; selectorIndex < starter.nselectors.size(); selectorIndex++) {
        ...
        if (selectorAlias != null) {
          ...
          if (name.contentEquals(selectorAlias)) {
            ...
            else if (selectorColumn != null) newName = selectorColumn;
            else newName = selectorAlias;
    ...
    for (int joinIndex = 0; joinIndex < starter.joins.size(); joinIndex++) {
      ...
      if (curJoin.from.composite) levelLineage.add(curJoin.from); // search for next level
  ...
  public static String getColumnFromFunctionExpression(FunctionExpression expr, boolean table) {
    ...
    if (expr.expressions.size() == 1) {
      ...
      if (gep instanceof FunctionExpression)
        ...
        if (table) retString = tableName + "." + retString;
      }
    }
  ...
  public static List<GenExpression> getSelectorInComposite(
      String name, GenExpression qs, List<GenExpression> selectorList) {
    if (selectorList == null) selectorList = new Vector<GenExpression>();
    ...
    if (qs.leftItem != null) {
      if (qs.leftItem instanceof GenExpression
          && !((GenExpression) qs.leftItem).composite
        ...
        if (selectorName == null) selectorName = leftItem.getLeftExpr();
    ...
    if (qs.rightItem != null) {
      if (qs.rightItem instanceof GenExpression
          && !((GenExpression) qs.rightItem).composite
        ...
        if (selectorName == null) selectorName = rightItem.getLeftExpr();
  ...
  public static String getPhysicalColumnName(GenExpression starter, String projectionName) {
    ...
    if (selector != null) {
      ...
      else return selector.tableName + "." + selector.getLeftExpr();
    }
    return null;
  }

  public static void neutralizeSelector(
      GenExpression starter, String projectionName, boolean neutralize) {
    ...
    if (functionExpressionMapper.containsKey(functionName)) {
      ...
      for (int exprIndex = 0; exprIndex < allExprs.size(); exprIndex++) {
        ...
        // if(expr instanceof FunctionExpression)
        {
          ...
          else if (curSelector.leftAlias != null) curName = curSelector.leftAlias;
          else curName = curSelector.getLeftExpr();
        }
        ...
        if (physicalName != null) retList.add(physicalName);
        else retList.add(curName);
      }
    }
  ...
  public void addFunctionToSelector(
      GenExpression select, String selectorName, String functionName) {
    ...
    if (selector != null) {
      ...
      if (alias == null) alias = selector.getLeftExpr();
  ...
  // get the final query
  public static String transformQueryWithParams(
      String originalQuery, List<ParamStructDetails> incomingStructs) {
    String retQuery = null;
    try {
      ...
      for (int paramIndex = 0; paramIndex < incomingStructs.size(); paramIndex++) {
        ...
        if (wrapper.operatorTableColumnParamIndex.containsKey(ParamStructDetailsKey)) {
          ParamStructDetails targetStruct =
              wrapper.operatorTableColumnParamIndex.get(ParamStructDetailsKey);
          // remove this struct from the overall so it wont fill
  ...
  /**
   * Transform the query and replace the param struct with the user defined param names via the
   * lookup
   *
   * @param originalQuery
   * @param incomingStructs
  ...
  public String getQueryForParam(String paramName) {
    ...
    // the front end should be able to directly substitute this value
    ParamStructDetails daStruct = paramStringToParamMap.get(paramName);
    if (daStruct != null) return daStruct.getDefQuery();
    return null;
  }

  public String generateQuery(boolean validate) throws Exception {
    String finalQuery =
        ((GenExpression) root).printQS(((GenExpression) root), new StringBuffer()).toString();
