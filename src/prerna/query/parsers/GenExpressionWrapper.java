...
public class GenExpressionWrapper {
	...
	// this is the highest level - acctid - when I replace this it will replace this
	// across all of the
	...
	// this is the next highest level - clms.acctid, mbrshp.acctid - this will
	// replace it only for the
	// specified operators not all of it
	// key is column table and value is column table operator i.e. key to the next
	// level
	public Map<String, List<String>> columnTableOperatorIndex = new HashMap<>();
	// this is the next level - clms.acctid=, clms.acctid < - this now also includes
	// the unique count
	// key is column table operator and valus is the actual parameter i.e. gen
	// expression
  ...
  // ////////////////////////////////////////////
	...
	// the key is the table name and the value is the GenExpression that should be
	// used
	...
	public void appendParameter(List<String> columnsToRemove, Map<String, GenExpression> paramValues) {
		...
		for (int colIndex = 0; colIndex < columnsToRemove.size(); colIndex++) {
			...
			while (allSelects.hasNext()) {
				...
				// there is a possibility where there are 2 different aliases refering to the
				// same column
				// name ?
	...
	public String makeParameters(String columnName, Object constantValue, String operationName,
		...
		// System.err.println("Parameterizing " + columnName + " with <><> " +
		// defQuery);
  ...
  // /////////////////////////////////////////////////////
	...
	public static Object[] getLineage(GenExpression starter, String name,
		...
		// from this point on, tries to find and print the lineage of this column i.e.
		// which particular
		...
		// if(identifiedSelector != null)
		{
			if (starter.from != null && starter.from.composite) {
				...
				// if(fromAlias.contentEquals(starter.tableName) &&
		...
		if (levelLineage.size() > 0) {
			for (int nextIndex = 0; nextIndex < levelLineage.size(); nextIndex++)
				return getLineage(levelLineage.get(nextIndex), newName, selectLineage, columnLineage, allInstances,
						(level + 1));
		}
	...
	public static String transformQueryWithParams(String originalQuery, List<ParamStructDetails> incomingStructs) {
		...
		try {
			...
			// wrapper.root.printQS(wrapper.root, null) +
			// "");
			for (int paramIndex = 0; paramIndex < incomingStructs.size(); paramIndex++) {
				ParamStructDetails thisStruct = incomingStructs.get(paramIndex);
				String ParamStructDetailsKey = thisStruct.getParamKey();
	...
	public Map<String, Object> getAllParamNames() {
		...
		/*
		 * SELECT actor_name, title, gender FROM actor WHERE gender >
		 * <actor_genderand0_left>> AND title IN ( SELECT title FROM mv WHERE director =
		 * <mv_directorand1_left=> AND revenue_domestic > budget ) AND actor_name IN
		 * (<actor_actor_namein2>)
		 * 
		 */

		// here the param names would be <actor_genderand0_left>> or
		// <mv_directorand1_left=> etc.
	...
	public String getQueryForParam(String paramName) {
		// somehow need to construct the query struct that can be used to query the
		// possible values
		// sets the current value fo the set param name
		// like <actor_genderand0_left>>
		// the front end should be able to directly substitute this value
