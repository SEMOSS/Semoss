...
public class ParamStructDetails {
  ...
  public enum BASE_QS_TYPE {
    SQS,
    HQS
  };

  public enum PARAMETER_FILL_TYPE {
    DATASOURCE,
    FILTER
  }
  ...
  @Deprecated private String appId = null;
  ...
  public String getFormattedValue() {
    ...
    if (currentValue instanceof List) {
      ...
      for (int itemIndex = 0; itemIndex < itemList.size(); itemIndex++) {
        if (itemIndex != 0) formattedOutput.append(", ");
        formattedOutput.append(itemList.get(itemIndex));
      }
      formattedOutput.append("]");
    } else formattedOutput.append(currentValue);
  ...
  /**
   * Append a single value to the string builder for generating pixel replacement for parameter
   * input
   *
   * @param builder
   * @param value
