...
public class SqlTranslator {
	...
	public SelectQueryStruct processQuery(String query) throws Exception {
		...
		this.fromTableName = fromTable.getName();
	...
	private IQuerySelector processBasicSelector(Expression expr) {
		...
		if(expr instanceof LongValue) {
			...
			constSelector = new QueryColumnSelector();
				...
				// translate table
				if (translationMap.containsKey(tableValue)) {
					tableValue = this.translationMap.get(tableValue).get(0);
				}
			
			// translate column
			if(translationMap.containsKey(colValue)) {
				colValue = this.translationMap.get(colValue).get(0);
			}
	...
	public void fillJoins(SelectQueryStruct qs, String tableName, List <Join> joins) {
		...
		for(int joinIndex = 0; joinIndex < joins.size(); joinIndex++) {
			...
			if(!simple) {
				...
				if(translationMap.containsKey(fromTable)) {
					fromTable = this.translationMap.get(fromTable).get(0);
				} 
				// translate from column
				if(translationMap.containsKey(fromColumn)) {
					fromColumn = this.translationMap.get(fromColumn).get(0);
				}
				
				
				// translate to table
				if(translationMap.containsKey(rightTableName)) {
					rightTableName = this.translationMap.get(rightTableName).get(0);
				} 
				// translate to column
				if(translationMap.containsKey(toColumn)) {
					toColumn = this.translationMap.get(toColumn).get(0);
				}
				// keep track of column and table in schema
				// from 
				Set<String> columns = new HashSet<String>();
		...
		for(int orderIndex = 0; orderIndex < orders.size(); orderIndex++) {
			...
			if(expr instanceof Column) {
				...
				if(columnAlias.containsKey(colName)) {
					...
					// translate to table
					if(translationMap.containsKey(concept)) {
						concept = this.translationMap.get(concept).get(0);
					} 
					// translate to column
					if(translationMap.containsKey(property)) {
						property = this.translationMap.get(property).get(0);
					}
