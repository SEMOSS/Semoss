...
public class SqlParser2 {
  ...
  public String generateQuery(SelectQueryStruct qs) throws Exception {
    String finalQuery =
        ((GenExpression) qs).printQS(((GenExpression) qs), new StringBuffer()).toString();
  ...
  public GenExpression processSelectFromItem(FromItem fi, GenExpression thisQs) {
    if (fi instanceof SubSelect) {
      ...
      if (this.wrapper.tableSelect.containsKey(fromTableName))
        selectList = this.wrapper.tableSelect.get(fromTableName);
      else selectList = new Vector<GenExpression>();
      if (!selectList.contains(qs)) selectList.add(qs);
  ...
  public void fillJoins(GenExpression qs, List<Join> joins) {
    ...
    for (int joinIndex = 0; joinIndex < joins.size(); joinIndex++) {
      ...
      // if somebody -- need to see if sql grammar can accomodate for stupidity where alias and
  ...
  public GenExpression processJoinExpression(GenExpression qs, GenExpression expr, Join thisJoin) {
    ...
    // the base join has
    // body - This is the on part of the overall. See the comments below. This is a AST so if I have
    // a = b and c = d then I will have 3 gen expressions. 1. GenExpression (a = b), 2.
    ...
    GenExpression gep =
        new GenExpression(); // this is the one I am processing for the actual join itself.
    GenExpression retExpr =
        processExpression(
            qs,
            thisJoin.getOnExpression(),
            expr); // this processes the on stuff i.e. on a = b. This has the a = b
    gep.telescope = true;
    gep.body = retExpr;
    ...
    // process the from
    FromItem fi = thisJoin.getRightItem();
    if (fi instanceof ParenthesisFromItem) System.err.println("Found the culprint");
    ...
    if (joinType == null) joinType = "JOIN";
  ...
  private GenExpression processJoinFromItem(FromItem fi, Join thisJoin, GenExpression gep) {
    ...
    if (fi instanceof Table) {
      ...
      if (rightTable.getAlias() != null) rightTableAlias = rightTable.getAlias().getName();
  ...
  public GenExpression processExpression(
      SelectQueryStruct qs, Expression joinExpr, GenExpression expr) {
    ...
    // only the binary expression has 2 sides
        ...
        && (processAllBinary
            || IQueryFilter.comparatorIsValidSQL(
                ((BinaryExpression) joinExpr).getStringExpression()))) {
      ...
      if (paramBinary) // dont do it for case
      {
        if (((GenExpression) sqs).getOperation().equalsIgnoreCase("column")) {
          ...
          if (sqs.userTableAlias != null) aliasName = sqs.userTableAlias;
        ...
        // If the operation is function, get the data from the expression
          ...
          while (fnsqs.expressions.size() > 0
              && fnsqs.expressions.get(0).getOperation().equals("function")) {
          ...
          if (fnsqs.expressions.size() > 0) {
            ...
            } else if ((((GenExpression) fnsqs.expressions.get(0))
                    .getOperation()
                    .equalsIgnoreCase("string"))
                || (((GenExpression) fnsqs.expressions.get(0))
                    .getOperation()
        ...
        if (((GenExpression) sqs2).getOperation().equalsIgnoreCase("column")) {
          ...
          aliasName = columnName;
          if (sqs2.userTableAlias != null) aliasName = sqs2.userTableAlias;
          ...
          if (columnName != null && parameterize) {
            ...
            // the value is now replaced with table_column_left or right of the and expression
        ...
        // If the operation is function, get the data from the expression
          ...
          if (fnsqs2.expressions.size() > 0) {
            // Going with the normal flow
            if (((GenExpression) fnsqs2.expressions.get(0))
                .getOperation()
            ...
            } else if ((((GenExpression) fnsqs2.expressions.get(0))
                    .getOperation()
                    .equalsIgnoreCase("string"))
                || (((GenExpression) fnsqs2.expressions.get(0))
                    .getOperation()
                    .equalsIgnoreCase("double"))
                || (((GenExpression) fnsqs2.expressions.get(0))
                    .getOperation()
        ...
        if (binary && column && tableName != null && constantValue != null) {
          ...
          this.wrapper.makeParameters(
              columnName,
              constantValue,
              modifier + eqExpr.getOperation().trim(),
              eqExpr.getOperation().trim(),
              constantType,
              exprToTrack,
              tableName,
    ...
    } else if (joinExpr instanceof Between) // between // this needs to be handled better
    {
      ...
      if (retExpr.body.getOperation().equalsIgnoreCase("column")) {
        ...
        aliasName = columnName;
        if (retExpr.body.userTableAlias != null) aliasName = retExpr.body.userTableAlias;
      }
      ...
      if (itemList instanceof SubSelect) {
        ...
        if (columnName != null && paramIn) {
          ...
          this.wrapper.makeParameters(
              columnName, constantValue, modifier, "in", constantType, ge, tableName, defQuery);
          if (parameterize) {
  ...
  // if the expression is complex then it will create another filter and add it into it - FE cant
  // handle it right now
  ...
  public void fillOrder(SelectQueryStruct qs, List<OrderByElement> orders) {
    ...
    for (int orderIndex = 0; orderIndex < orders.size(); orderIndex++) {
      ...
      if (!sortDir.equalsIgnoreCase("ASC")) obe.direction = sortDir;
      qs.norderBy.add(obe);
    }
  }
  ...
  public Map<String, List<GenExpression>> getTableColumns(String query) {
    ...
    try {
      ...
      Map<Integer, List<GenExpression>> levelSelectors =
          new HashMap<Integer, List<GenExpression>>();
      List<String> realTables = new ArrayList();
      Map<GenExpression, List<GenExpression>> derivedColumns =
          new HashMap<GenExpression, List<GenExpression>>();
  ...
  public Map<String, List<GenExpression>> remasterColumns(
      List<String> realTables, Map<String, List<GenExpression>> tableColumns) {
  ...
  //				+ "concept c, engineconcept ec, engine e inner join sometable s on c.logicalname=s.logical
  // where (ec.localconceptid=c.localconceptid and "
  //				+ "c.conceptualname in ('val1', 'val2')) or (ec.localconceptid + 5) =1 group by ln order by
  // ln limit 200 offset 50 ";// order by c.logicalname";
  //
  //		query = "select distinct f.studio, (f.movie_budget - 3) / 2 from f where f.movie_budget * 4 >
  // 10";
  //
  //		// thank you so much anthem
  //		String query2 = " SELECT	Member_Engagement_Tier AS `Member Engagement Tier`,Site_Service_Name
  ...
  //		// selectbody - it can either be a simple one or a set operation - This is where the union
  // sits. This can be a plain select or something else - I only accomodate for plain select at this
  // point
  ...
  //
  //		String query4 = " SELECT Health_Condition_Category_Description AS \"Health Condition Category
  // Description\",\r\n" +
  ...
  //
  //		String query5 = "SELECT Time_Period AS \"Time Period\",Health_Condition_Category_Description
  // AS \"Health Condition Category Description\",\r\n" +
  //				"		Total_Utilization_Count AS \"Total Utilization Count\",Paid_Amount AS \"Paid
  // Amount\",\r\n" +
  //				"		Account_Name AS \"Account Name\",CBSA_Name AS \"CBSA
  // Name\",ICD_Diagnosis_Source_Short_Code_Definition_Text AS \"ICD Diagnosis Source Short Code
  // Definition Text\",\r\n" +
  ...
  //				"		SUM(CLMS.TOTL_UTIL_CNT) AS Total_Utilization_Count,SUM(CLMS.ACCT_PAID_AMT) AS
  // Paid_Amount,\r\n" +
  ...
  //				"	ELSE SUM(CAST (CLMS.ACCT_PAID_AMT as DECIMAL(18,6))) / SUM(CAST (MBRSHP.TOTL_CVRG_CNT as
  // DECIMAL(18,\r\n" +
  //				"		6))) \r\n" +
  ...
  //				"		sum(cii_fact_clm_line.ACCT_ADMT_CNT +cii_fact_clm_line.ACCT_VST_CNT) AS
  // TOTL_UTIL_CNT,\r\n" +
  //				"		SUM(CII_FACT_CLM_LINE.ACCT_PAID_AMT) AS ACCT_PAID_AMT,CII_ACCT_PRFL.ACCT_NM AS
  // CII_ACCT_PRFL_ACCT_NM,\r\n" +
  //				"		DIM_CBSA.CBSA_NM AS DIM_CBSA_CBSA_NM,DIM_DIAG.SRC_SHRT_CD_DEFN_TXT AS
  // DIM_DIAG_SRC_SHRT_CD_DEFN_TXT,\r\n" +
  //				"		COALESCE(HCC_CHK.PAID_HCC_IND,'N') AS PAID_HCC_IND,CII_FACT_CLM_LINE.CBSA_ID AS
  // CBSA_ID,\r\n" +
  //				"		CII_FACT_CLM_LINE.ACCT_ID AS ACCT_ID  \r\n" +
  //				"FROM	CII_FACT_CLM_LINE INNER JOIN (\r\n" +
  //				"SELECT	B.*, \r\n" +
  ...
  //				"select acct_id as FRC_JN,ELGBLTY_CY_MNTH_END_NBR, MCID,  MIN(CVRG_PRTY_NBR) as
  // MIN_CVRG_PRTY_NBR,\r\n" +
  //				"		TM_PRD_FNCTN.TM_PRD_NM\r\n" +
  //				"from cii_fact_mbrshp \r\n" +
  //				"JOIN (\r\n" +
  ...
  //
  //		String query6 = "\r\n" +
  //				" SELECT Time_Period AS \"Time_Period\",Health_Condition_Category_Description AS
  // \"Health_Condition_Category_Description\",\r\n" +
  ...
  //				"		DIM_AGE_GRP.AGE_GRP_DESC AS DIM_AGE_GRP_AGE_GRP_DESC,CII_FACT_CLM_LINE.CBSA_ID AS
  // CBSA_ID,\r\n" +
  ...
  //				"	WHEN TMBRSHP.MIN_CVRG_PRTY_NBR IS NULL THEN 0 \r\n" +
  //				"	ELSE CII_FACT_MBRSHP.MBR_CVRG_CNT \r\n" +
  //				"END) AS TOTL_CVRG_CNT,CII_FACT_MBRSHP.CBSA_ID AS CBSA_ID,CII_FACT_MBRSHP.ACCT_ID AS
  // ACCT_ID,\r\n" +
  //				"		CII_FACT_MBRSHP.ACCT_AGE_GRP_KEY AS ACCT_AGE_GRP_KEY  \r\n" +
  //				"FROM	CII_FACT_MBRSHP INNER JOIN (\r\n" +
  //				"SELECT	B.*, \r\n" +
  ...
  //				"	";
  //		String query7 = "SELECT Time_Period AS \"Time Period\",Health_Condition_Category_Description
  // AS \"Health Condition Category Description\",\r\n" +
  //				"		Total_Utilization_Count AS \"Total Utilization Count\",Paid_Amount AS \"Paid
  // Amount\",\r\n" +
  //				"		Account_Name AS \"Account Name\",CBSA_Name AS \"CBSA
  ...
  //				"		Paid_PMPM";
  //
  //		String query8 = "Select a.abc, b.b1,  coalesce(a.a1, 'abcd'), a.a1 + 2 from a as a, (b) as b,
  // (c) as c on a.id = b.id";
  //
  //		String query9 = "SELECT DISTINCT Title.Title AS \"Genre\" , CASE WHEN
  // TMBRSHP.MIN_CVRG_PRTY_NBR IS NULL THEN 0 ELSE CII_FACT_MBRSHP.MBR_CVRG_CNT END AS mango,
  // sum(MovieBudget), ( CAST(( CAST(Title.MovieBudget  AS DECIMAL) * CAST(2 AS DECIMAL) )  AS
  // DECIMAL) + CAST(Title.RevenueDomestic AS DECIMAL) ) AS \"Der_value\" FROM Title Title ";
  //
  //		String query10 = "SELECT DISTINCT Title.Title AS \"Genre\" , CASE WHEN
  // TMBRSHP.MIN_CVRG_PRTY_NBR in ('a', 'b') THEN 0 WHEN TMBRSHP.MIN_CVRG_PRTY_NBR=30 THEN 21 ELSE
  // CII_FACT_MBRSHP.MBR_CVRG_CNT END AS mango, sum(MovieBudget), ( CAST(( CAST(Title.MovieBudget
  // AS DECIMAL) * CAST(2 AS DECIMAL) )  AS DECIMAL) + CAST(Title.RevenueDomestic AS DECIMAL) ) AS
  // \"Der_value\" FROM Title Title WHERE Genre in ('abc', 'y') or Genre=20";
  //
  //		String query11 = "SELECT DISTINCT Title.MovieBudget AS \"MovieBudget\" , Title.RevenueDomestic
  // AS \"RevenueDomestic\" , Title.RevenueInternational AS \"RevenueInternational\" ,
  // Title.RottenTomatoesAudience AS \"RottenTomatoesAudience\" , Title.RottenTomatoesCritics AS
  // \"RottenTomatoesCritics\" , Title.Title AS \"Title\" FROM Title Title  WHERE
  // (Title.MovieBudget) IN ( 1000000 , 1700000 ) \r\n";
  //
  //		String query12 = "SELECT DISTINCT customquery.\"MovieBudget\" AS \"MovieBudget\" ,
  // customquery.\"RevenueDomestic\" AS \"RevenueDomestic\" , customquery.\"RevenueInternational\"
  // AS \"RevenueInternational\" , customquery.\"RottenTomatoesAudience\" AS
  // \"RottenTomatoesAudience\" , customquery.\"RottenTomatoesCritics\" AS \"RottenTomatoesCritics\"
  // , customquery.\"Title\" AS \"Title\" FROM (SELECT  avqLBU.\"MovieBudget\",
  // avqLBU.\"RevenueDomestic\", avqLBU.\"RevenueInternational\", avqLBU.\"RottenTomatoesAudience\",
  // avqLBU.\"RottenTomatoesCritics\", avqLBU.\"Title\"  FROM ( SELECT  Title.MovieBudget as
  // \"MovieBudget\", Title.RevenueDomestic as \"RevenueDomestic\", Title.RevenueInternational as
  // \"RevenueInternational\", Title.RottenTomatoesAudience as \"RottenTomatoesAudience\",
  // Title.RottenTomatoesCritics as \"RottenTomatoesCritics\", Title.Title as \"Title\"  FROM Title
  // as Title  WHERE (Title.MovieBudget)  IN  (1000000, 1700000)  UNION ALL  SELECT
  // Title.MovieBudget as \"MovieBudget\", Title.RevenueDomestic as \"RevenueDomestic\",
  // Title.RevenueInternational as \"RevenueInternational\", Title.RottenTomatoesAudience as
  // \"RottenTomatoesAudience\", Title.RottenTomatoesCritics as \"RottenTomatoesCritics\",
  // Title.Title as \"Title\"  FROM Title as Title  WHERE (Title.MovieBudget)  IN  (500000, 800000))
  ...
  //
  //
  //		String query19 = "Select Nominated, Genre, Studio, Title, MovieBudget, RevenueDomestic,
  // RevenueInternational from Movie2 where Studio = 'WB' and (MovieBudget > 10000000 or MovieBudget
  // > 20000000)";
  ...
  //		// I have 2 joins
  //		String query20 = "Select Nominated, Genre, Studio, Title, MovieBudget, RevenueDomestic,
  // RevenueInternational from Movie2 where MovieBudget = 1000 and Studio ='WB' and RevenueDomestic
  // in (1,2,3,4)";
  //
  ...
  // Code\",Reporting_Member_Relationship_Description AS \"Reporting Member Relationship
  // Description\",Age_Group_Description AS \"Age Group Description\",Age_In_Years AS \"Age In
  // Years\",Contract_Type_Code AS \"Contract Type Code\",Eligibility_Year_Month_Ending_Number AS
  // \"Eligibility Year Month Ending Number\",State_Code AS \"State Code\",CBSA_Name AS \"CBSA
  // Name\",Member_PCP_Indicator AS \"Member PCP Indicator\",Subscriber_ID AS \"Subscriber
  ...
  // Name\",Time_Period_Start AS \"Time Period Start\",Time_Period_End AS \"Time Period
  // End\",Non_Utilizer_Indicator AS \"Non Utilizer Indicator\",Member_Coverage_Count AS \"Member
  ...
  // Age_Group_Description,MBRSHP.AGE_IN_YRS_NBR AS Age_In_Years,MBRSHP.CNTRCT_TYPE_CD AS
  // Contract_Type_Code,MBRSHP.ELGBLTY_CY_MNTH_END_NBR AS
  // Eligibility_Year_Month_Ending_Number,MBRSHP.ST_CD AS State_Code,MBRSHP.CBSA_NM AS
  ...
  // Continuous_Enrollment_for_1_Period,MBRSHP.MBR_BRTH_DT AS Member_Birth_Date,MBRSHP.ACCT_NM AS
  // Account_Name,MBRSHP.TIME_PRD_STRT_NBR AS Time_Period_Start,MBRSHP.TIME_PRD_END_NBR AS
  // Time_Period_End,MBRSHP.Non_Utilizer_Ind AS Non_Utilizer_Indicator,SUM(MBRSHP.SUM_MBR_CVRG_CNT)
  // AS Member_Coverage_Count,MBRSHP.TM_PRD_NM AS Time_Period  FROM  (  SELECT CII_ACCT_PRFL.ACCT_ID
  // AS ACCT_ID,CII_FACT_MBRSHP.MCID AS MCID,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC AS
  // MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.MBR_GNDR_CD AS
  // MBR_GNDR_CD,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC AS RPTG_MBR_RLTNSHP_DESC,case when
  // DIM_AGE_GRP.AGE_GRP_DESC='1-17' then '1 through 17' else  DIM_AGE_GRP.AGE_GRP_DESC end AS
  // AGE_GRP_DESC,CII_FACT_MBRSHP.AGE_IN_YRS_NBR AS AGE_IN_YRS_NBR,CII_FACT_MBRSHP.CNTRCT_TYPE_CD AS
  ...
  // FMBRSHP_SBSCRBR_ID,CE.CNTNUS_ENRLMNT_1_PRD_CD AS CNTNUS_ENRLMNT_1_PRD_CD,DIM_MCID.MBR_BRTH_DT
  // AS MBR_BRTH_DT,CII_ACCT_PRFL.ACCT_NM AS ACCT_NM,CASE WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Current
  // Period'  THEN 201910 WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period' THEN 201810 WHEN
  // TM_PRD_FNCTN.TM_PRD_NM=  'Prior Period 2'  THEN 201801 end AS TIME_PRD_STRT_NBR,CASE WHEN
  // TM_PRD_FNCTN.TM_PRD_NM= 'Current Period'  THEN 202009 WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Prior
  // Period' THEN 201909 WHEN TM_PRD_FNCTN.TM_PRD_NM=  'Prior Period 2'  THEN 201809 end AS
  // TIME_PRD_END_NBR,UT.Non_Utilizer_Ind AS Non_Utilizer_Ind,SUM(MBR_CVRG_CNT) AS
  // SUM_MBR_CVRG_CNT,TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM  FROM CII_FACT_MBRSHP INNER JOIN (SELECT
  ...
  // CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN TM_PRD_FNCTN.START_YEAR_MNTH and
  ...
  // '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and FLTR_SRC_NM= 'User Session')SGMNTN on
  ...
  // ACIISST_USER_ID= 16200068 and ACCT_ID in ('W0004156') and SRC_FLTR_ID=
  // '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and FLTR_SRC_NM= 'User Session')SGMNTN on
  // clm.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and clm.ACCT_ID=SGMNTN.ACCT_ID\r\n" +
  ...
  // CII_ACCT_PRFL ON CII_FACT_MBRSHP.ACCT_ID = CII_ACCT_PRFL.ACCT_ID INNER JOIN DIM_MBR_CVRG_TYPE
  // ON CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD = DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_CD INNER JOIN
  ...
  // CII_FACT_MBRSHP.ACCT_AGE_GRP_KEY = DIM_AGE_GRP.AGE_GRP_KEY INNER JOIN DIM_CBSA ON
  ...
  // ACCT_ID,SGMNTN_DIM_KEY from ACIISST_SGMNTN_BRDG where ACIISST_USER_ID= 16200068 and ACCT_ID in
  // ('W0004156') and SRC_FLTR_ID= '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and FLTR_SRC_NM= 'User
  // Session')SGMNTN on fact.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and fact.ACCT_ID=SGMNTN.ACCT_ID
  //  \r\n" +
  //				"                        WHERE            fact .ACCT_ID = 'W0004156'
  // \r\n" +
  ...
  //				"            and CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD = CE.MBR_CVRG_TYPE_CD INNER JOIN DIM_MCID
  // ON CII_FACT_MBRSHP.MCID = DIM_MCID.MCID AND CII_FACT_MBRSHP.ACCT_ID = DIM_MCID.ACCT_ID INNER
  // JOIN (Select ACCT_ID,SGMNTN_DIM_KEY from ACIISST_SGMNTN_BRDG where ACIISST_USER_ID= 16200068
  // and ACCT_ID in ('W0004156') and SRC_FLTR_ID= '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and
  // FLTR_SRC_NM= 'User Session')SGMNTN on CII_FACT_MBRSHP.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY
  // and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID in ('W0004156')
  // GROUP BY
  ...
  // ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on fact.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY
  ...
  //				"                INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from
  // ACIISST_SGMNTN_BRDG where ACCT_ID in ('W0016437') and SRC_FLTR_ID in
  // ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on clm.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY
  ...
  // (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID
  // in ('W0016437') and SRC_FLTR_ID in ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on
  // fact.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and fact.ACCT_ID=SGMNTN.ACCT_ID   \r\n" +
  //				"                                WHERE fact .ACCT_ID = 'W0016437'
  // \r\n" +
  //				"                                GROUP BY  fact .acct_id,\r\n" +
  ...
  // DIM_MCID ON CII_FACT_MBRSHP.MCID = DIM_MCID.MCID AND CII_FACT_MBRSHP.ACCT_ID = DIM_MCID.ACCT_ID
  // INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG
  // where ACCT_ID in ('W0016437') and SRC_FLTR_ID in
  // ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on CII_FACT_MBRSHP.SGMNTN_DIM_KEY =
  // SGMNTN.SGMNTN_DIM_KEY and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID
  // in ('W0016437')   GROUP BY
  // CII_ACCT_PRFL.ACCT_ID,CII_FACT_MBRSHP.MCID,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.MBR_GNDR_CD,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC,case when DIM_AGE_GRP.AGE_GRP_DESC='1-17' then '1 through 17' else  DIM_AGE_GRP.AGE_GRP_DESC end,CII_FACT_MBRSHP.AGE_IN_YRS_NBR,CII_FACT_MBRSHP.CNTRCT_TYPE_CD,CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR,CII_FACT_MBRSHP.ST_CD,DIM_CBSA.CBSA_NM,CII_FACT_MBRSHP.PCP_IND,CII_FACT_MBRSHP.SBSCRBR_ID,CE.CNTNUS_ENRLMNT_1_PRD_CD,DIM_MCID.MBR_BRTH_DT,CII_ACCT_PRFL.ACCT_NM,CASE WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Current Period'  THEN 201910 WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period' THEN 201810 WHEN TM_PRD_FNCTN.TM_PRD_NM=  'Prior Period 2'  THEN 201801 end,CASE WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Current Period'  THEN 202009 WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period' THEN 201909 WHEN TM_PRD_FNCTN.TM_PRD_NM=  'Prior Period 2'  THEN 201809 end,UT.Non_Utilizer_Ind,TM_PRD_FNCTN.TM_PRD_NM ) AS MBRSHP  GROUP BY MBRSHP.ACCT_ID,MBRSHP.MCID,MBRSHP.MBR_CVRG_TYPE_DESC,MBRSHP.MBR_GNDR_CD,MBRSHP.RPTG_MBR_RLTNSHP_DESC,MBRSHP.AGE_GRP_DESC,MBRSHP.AGE_IN_YRS_NBR,MBRSHP.CNTRCT_TYPE_CD,MBRSHP.ELGBLTY_CY_MNTH_END_NBR,MBRSHP.ST_CD,MBRSHP.CBSA_NM,MBRSHP.PCP_IND,MBRSHP.FMBRSHP_SBSCRBR_ID,MBRSHP.CNTNUS_ENRLMNT_1_PRD_CD,MBRSHP.MBR_BRTH_DT,MBRSHP.ACCT_NM,MBRSHP.TIME_PRD_STRT_NBR,MBRSHP.TIME_PRD_END_NBR,MBRSHP.Non_Utilizer_Ind,MBRSHP.TM_PRD_NM ) AS FNL   ORDER BY Account_ID,Master_Consumer_ID,Member_Coverage_Type_Description,Member_Gender_Code,Reporting_Member_Relationship_Description,Age_Group_Description,Age_In_Years,Contract_Type_Code,Eligibility_Year_Month_Ending_Number,State_Code,CBSA_Name,Member_PCP_Indicator,Subscriber_ID,Continuous_Enrollment_for_1_Period,Member_Birth_Date,Account_Name,Time_Period_Start,Time_Period_End,Non_Utilizer_Indicator,Member_Coverage_Count";
  //
  //		String query30 = "SELECT Account_Name AS \"Account Name\",Gaps_Closed AS \"Gaps
  // Closed\",Member_Engaged_Indicator AS \"Member Engaged Indicator\",Member_Engagement_Tier AS
  // \"Member Engagement Tier\",Time_Period AS \"Time Period\",Earned_Incentive_Indicator AS
  ...
  // Member_Engaged_Indicator,GAPSVNGS.MBR_ENGGMNT_TIER AS Member_Engagement_Tier,GAPSVNGS.TM_PRD_NM
  // AS Time_Period,GAPSVNGS.EARND_INCNTV_CD AS Earned_Incentive_Indicator  FROM  (  SELECT
  // CII_ACCT_PRFL.ACCT_NM AS ACCT_NM,SUM( CASE WHEN CII_FACT_GAP_SVNGS.CMPLNC_YEAR_MNTH_YEAR_NBR <=
  // CASE WHEN substr(cast(TM_PRD_FNCTN.END_YEAR_MNTH  as varchar(6)) ,5,2) = '01' THEN
  // TM_PRD_FNCTN.END_YEAR_MNTH   - 89 ELSE TM_PRD_FNCTN.END_YEAR_MNTH   - 1 END  THEN
  // CII_FACT_GAP_SVNGS.CLSUR_CNT ELSE 0 END) AS GPS_CLSD,coalesce( TMBRENGGMNT2.MBR_ENGGD_IND,'N')
  // AS MBR_ENGGD_IND,coalesce(TMBRENGGMNT.ENGGMNT_TIER, 'Not Engaged') AS
  // MBR_ENGGMNT_TIER,TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM,INCNTV_MBRSHP.EARND_INCNTV_CD AS
  ...
  //				"or (dtp.CSTM_STRT_PRIOR_2_MNTH_NBR = 201801 and dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201811) ))
  ...
  // TM_PRD_FNCTN.START_YEAR_MNTH  - 94 ELSE TM_PRD_FNCTN.START_YEAR_MNTH  - 6 END and CASE WHEN
  // substr(cast(TM_PRD_FNCTN.END_YEAR_MNTH as varchar(6)) ,5,2) <= '06' THEN
  // TM_PRD_FNCTN.END_YEAR_MNTH  - 94 ELSE TM_PRD_FNCTN.END_YEAR_MNTH  - 6 END INNER JOIN (
  // SELECT MBRSHP.TM_PRD_NM AS Time_Period,MBRSHP.DIM_MBR_CVRG_TYPE_MBR_CVRG_TYPE_DESC AS
  // Member_Coverage_Type_Description,   MBRSHP.CII_FACT_MBRSHP_MCID AS Master_Consumer_ID  FROM (
  //     SELECT TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM,       DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC AS
  // DIM_MBR_CVRG_TYPE_MBR_CVRG_TYPE_DESC,       CII_FACT_MBRSHP.MCID AS CII_FACT_MBRSHP_MCID
  // FROM CII_FACT_MBRSHP     INNER JOIN (       \r\n" +
  //				"Select YEAR_CD_NM as TM_PRD_NM,\r\n" +
  //				"STRT_MNTH_NBR as START_YEAR_MNTH,\r\n" +
  //				"END_MNTH_NBR as END_YEAR_MNTH,\r\n" +
  ...
  // DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_CD  INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM,
  // SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID in ('W0016437') and SRC_FLTR_ID in
  // ('d97ef60a-2503-41d2-acac-2c91c904e9e0'))SGMNTN on CII_FACT_MBRSHP.SGMNTN_DIM_KEY =
  // SGMNTN.SGMNTN_DIM_KEY and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID    WHERE
  // CII_FACT_MBRSHP.ACCT_ID IN ('W0016437')       AND DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC =
  // 'Medical'      GROUP BY TM_PRD_FNCTN.TM_PRD_NM,       DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC,
  //   CII_FACT_MBRSHP.MCID ) AS MBRSHP    GROUP BY MBRSHP.TM_PRD_NM,
  // MBRSHP.DIM_MBR_CVRG_TYPE_MBR_CVRG_TYPE_DESC,    CII_FACT_MBRSHP_MCID ) AS MDCL_MBRSHP  on
  // (TM_PRD_FNCTN.TM_PRD_NM = MDCL_MBRSHP.Time_Period  and CII_FACT_GAP_SVNGS.MCID =
  // MDCL_MBRSHP.Master_Consumer_ID) INNER JOIN  (SELECT MAX(CII_FACT_MBRSHP.EARND_INCNTV_CD)
  // EARND_INCNTV_CD, CII_FACT_MBRSHP.MCID, CII_FACT_MBRSHP.ACCT_ID, TM_PRD_NM FROM CII_FACT_MBRSHP
  // \r\n" +
  //				"JOIN\r\n" +
  //				"(SELECT YEAR_CD_NM as TM_PRD_NM,\r\n" +
  //				"STRT_MNTH_NBR as START_YEAR_MNTH,\r\n" +
  ...
  //				"or (dtp.CSTM_STRT_PRIOR_2_MNTH_NBR = 201801 and dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201811) )
  // )  TM_PRD_FNCTN   \r\n" +
  //				" ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN TM_PRD_FNCTN.START_YEAR_MNTH  and
  ...
  //				" or DIM_ENGGMNT.ENHNCD_IND= 1                          \r\n" +
  //				"union    all                          \r\n" +
  //				"select    cast('Traditional' as char(20)) as ENGGMNT, cast('Care Coordination' as char(20))
  //  AS ENGGMNT_TIER    \r\n" +
  //				"from    DIM_ENGGMNT    \r\n" +
  //				"where    DIM_ENGGMNT.TRDTNL_IND= 1 ) b            \r\n" +
  //				" on a.ENGGMNT = b.ENGGMNT                   )  TMBRENGGMNT             \r\n" +
  ...
  // ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID in
  // ('W0016437') and SRC_FLTR_ID in ('d97ef60a-2503-41d2-acac-2c91c904e9e0'))SGMNTN on
  // CII_FACT_GAP_SVNGS.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and
  // CII_FACT_GAP_SVNGS.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_GAP_SVNGS.ACCT_ID in ('W0016437')
  // GROUP BY CII_ACCT_PRFL.ACCT_NM,coalesce(
  // TMBRENGGMNT2.MBR_ENGGD_IND,'N'),coalesce(TMBRENGGMNT.ENGGMNT_TIER, 'Not
  // Engaged'),TM_PRD_FNCTN.TM_PRD_NM,INCNTV_MBRSHP.EARND_INCNTV_CD ) AS GAPSVNGS  GROUP BY
  // GAPSVNGS.ACCT_NM,GAPSVNGS.MBR_ENGGD_IND,GAPSVNGS.MBR_ENGGMNT_TIER,GAPSVNGS.TM_PRD_NM,GAPSVNGS.EARND_INCNTV_CD ) AS FNL   ORDER BY Account_Name,Gaps_Closed,Member_Engaged_Indicator,Member_Engagement_Tier,Time_Period,Earned_Incentive_Indicator\r\n" +
  //				"";
  //
  //		String query31 = "SELECT Account_Name AS \"Account Name\",Gaps_Closed AS \"Gaps
  // Closed\",Member_Engaged_Indicator AS \"Member Engaged Indicator\",Member_Engagement_Tier AS
  // \"Member Engagement Tier\",Time_Period AS \"Time Period\",Earned_Incentive_Indicator AS
  // \"Earned Incentive Indicator\"  FROM  (  SELECT GAPSVNGS.ACCT_NM AS
  // Account_Name,SUM(GAPSVNGS.GPS_CLSD) AS Gaps_Closed,GAPSVNGS.MBR_ENGGD_IND AS
  ...
  //				"";
  //
  //		String query24 = "Select a,b,c from d where (age > 20 and age < 40) or (age > 40 and age <
  // 70)";
  //
  ...
  //
  //		String query29 = "select substr(cast(TM_PRD_FNCTN.START_YEAR_MNTH as varchar(6)) ,5,2) from
  // abc";
  //
  //		String query32 = "select stupid(count(*)) from mango";
  //
  //		String query33 = "select count(*) from ( SELECT AS \"Account ID\",AS \"Account Name\",AS
  // \"Time Period\",AS \"Member Coverage Type Description\",AS \"Eligibility Year Month Ending
  // Number\",AS \"Member Coverage Count\",CASE WHEN sum(CAST(AS decimal (22,6))) over(partition by
  // )=0 then 0.00 else SUM(CAST(AS decimal (22,6))) over(partition by )/ sum(CAST(AS decimal
  // (22,6))) over(partition by ) *100 end AS \"Member Coverage Count - Total %: Member Coverage
  // Type Description,Reporting Member Relationship Description,Eligibility Year Month Ending
  // Number,,Eligibility Calendar Month Year  Name\",AS \"Reporting Member Relationship
  // Description\",AS \"Eligibility Calendar Month Year  Name\", AS \"Contract Type Description\"
  // FROM  (  SELECT MBRSHP.ACCT_ID AS MBRSHP.ACCT_NM AS MBRSHP.TM_PRD_NM AS
  // MBRSHP.MBR_CVRG_TYPE_DESC AS MBRSHP.ELGBLTY_CY_MNTH_END_NBR AS SUM(MBRSHP.SUM_MBR_CVRG_CNT) AS
  // MBRSHP.RPTG_MBR_RLTNSHP_DESC AS MBRSHP.CLNDR_MNTH_YEAR_NM AS MBRSHP.CNTRCT_TYPE_DESC AS   FROM
  // (  SELECT CII_ACCT_PRFL.ACCT_ID AS ACCT_ID,CII_ACCT_PRFL.ACCT_NM AS
  // ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC AS
  ...
  // CLNDR_MNTH_YEAR_NM,DIM_CNTRCT_TYPE.CNTRCT_TYPE_DESC AS CNTRCT_TYPE_DESC  FROM CII_FACT_MBRSHP
  ...
  //				"or (dtp.CSTM_STRT_PRIOR_2_MNTH_NBR = 201803 and dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201902) ) )
  // TM_PRD_FNCTN ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN TM_PRD_FNCTN.START_YEAR_MNTH
  // and TM_PRD_FNCTN.END_YEAR_MNTH INNER JOIN DIM_MBR_CVRG_TYPE ON CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD
  // = DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_CD INNER JOIN DIM_RPTG_MBR_RLTNSHP ON
  // CII_FACT_MBRSHP.RPTG_MBR_RLTNSHP_CD = DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_CD INNER JOIN
  // DIM_MNTH ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR = DIM_MNTH.YEAR_MNTH_NBR INNER JOIN
  // DIM_CNTRCT_TYPE ON CII_FACT_MBRSHP.CNTRCT_TYPE_CD = DIM_CNTRCT_TYPE.CNTRCT_TYPE_CD INNER JOIN
  // CII_ACCT_PRFL ON CII_FACT_MBRSHP.ACCT_ID = CII_ACCT_PRFL.ACCT_ID INNER JOIN (Select
  // ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID in
  // ('W0003618') and SRC_FLTR_ID in ('195d1b33-4e3c-4ebc-b51f-acfb59d288a1'))SGMNTN on
  // CII_FACT_MBRSHP.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and
  // CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID in ('W0003618')   GROUP BY
  // CII_ACCT_PRFL.ACCT_ID,CII_ACCT_PRFL.ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC,DIM_MNTH.CLNDR_MNTH_YEAR_NM,DIM_CNTRCT_TYPE.CNTRCT_TYPE_DESC ) AS MBRSHP  GROUP BY MBRSHP.ACCT_ID,MBRSHP.ACCT_NM,MBRSHP.TM_PRD_NM,MBRSHP.MBR_CVRG_TYPE_DESC,MBRSHP.ELGBLTY_CY_MNTH_END_NBR,MBRSHP.RPTG_MBR_RLTNSHP_DESC,MBRSHP.CLNDR_MNTH_YEAR_NM,MBRSHP.CNTRCT_TYPE_DESC ) AS FNL    ) t";
  //
  ...
  //		// you can set any of this and then invoke fill parameters which will fill the query and send
