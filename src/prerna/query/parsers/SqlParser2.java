...
public class SqlParser2 {
	...
	public GenExpression processSelect(GenExpression qs, PlainSelect sb) {
		...
		if (dis != null) {
			...
			/*
			 * // not sure when this is even used.. but setting at qs level List
			 * <SelectItem> allSelects = dis.getOnSelectItems();
		...
		{
			/*
			 * Table fromTable = (Table) sb.getFromItem(); String fromTableName =
			 * fromTable.getName();
			 * 
			 * // if there is no alias // we will determine to set the table as the alias
			 * Alias fromTableAliasObj = fromTable.getAlias(); if(fromTableAliasObj != null)
			 * { tableAlias.put(fromTable.getAlias().getName(), fromTable.getName()); } else
	...
	public GenExpression processJoinExpression(GenExpression qs, GenExpression expr, Join thisJoin) {
		...
		// the base join has
		// body - This is the on part of the overall. See the comments below. This is a
		// AST so if I have
		// a = b and c = d then I will have 3 gen expressions. 1. GenExpression (a = b),
		// 2.
		// GenExpression(c = d) 3. 1. AND 2.
		...
		GenExpression retExpr = processExpression(qs, thisJoin.getOnExpression(), expr); // this processes the on stuff
																							// i.e. on a = b. This has
																							// the a = b
		gep.telescope = true;
		gep.body = retExpr;
	...
	public GenExpression processExpression(SelectQueryStruct qs, Expression joinExpr, GenExpression expr) {
		...
		if (joinExpr instanceof AndExpression) // || joinExpr instanceof OrExpression)
		{
			...
			// System.out.println("Arbitrary gen expression print" +
			// expr2.printQS(expr2,null));
		...
		// only the binary expression has 2 sides
			...
			/*
			 * // remove left expression operator =
			 * operator.replace(joinExpr2.getLeftExpression()+"", ""); operator =
			 * operator.replace(joinExpr2.getRightExpression()+"", ""); operator =
			 * operator.trim();
			 * 
			...
			if (paramBinary) // dont do it for case
			{
				if (((GenExpression) sqs).getOperation().equalsIgnoreCase("column")) {
					...
					if (columnName != null) {
						((GenExpression) sqs).setLeftExpresion(
					...
					if (fnsqs.expressions.size() > 0) {
						...
						if (fnsqs.expressions.get(0).getOperation().equalsIgnoreCase("column")) {
							...
							if (fnsqs.expressions.get(0).userTableAlias != null) {
								...
								|| (((GenExpression) fnsqs.expressions.get(0)).getOperation().equalsIgnoreCase("date"))
								|| (((GenExpression) fnsqs.expressions.get(0)).getOperation().equalsIgnoreCase("time"))
								|| (((GenExpression) fnsqs.expressions.get(0)).getOperation()
				...
				if (((GenExpression) sqs2).getOperation().equalsIgnoreCase("column")) {
					...
					if (columnName != null && parameterize) {
						...
						((GenExpression) sqs2).setLeftExpresion(
								"<" + tableName + "_" + columnName + modifier + eqExpr.getOperation().trim() + ">");
					}
					exprToTrack = sqs2;
						...
						if (((GenExpression) fnsqs2.expressions.get(0)).getOperation().equalsIgnoreCase("column")) {
							...
							if (fnsqs2.expressions.get(0).userTableAlias != null) {
								...
								|| (((GenExpression) fnsqs2.expressions.get(0)).getOperation()
										.equalsIgnoreCase("double"))
								|| (((GenExpression) fnsqs2.expressions.get(0)).getOperation().equalsIgnoreCase("date"))
								|| (((GenExpression) fnsqs2.expressions.get(0)).getOperation().equalsIgnoreCase("time"))
								|| (((GenExpression) fnsqs2.expressions.get(0)).getOperation()
										.equalsIgnoreCase("long"))) {
		...
		} else if (joinExpr instanceof Between) // between // this needs to be handled better
		{
			...
			if (!processCase) // so while this idea is good.. this needs to be eventually a stack since
				// you are running recursion.. for now it might work
			...
			/*
			 * if(itemList instanceof ExpressionList) { List <Expression> el =
			 * ((ExpressionList)itemList).getExpressions();
			 * 
			 * for(int itemIndex = 0;itemIndex < el.size();itemIndex++) { Expression
			 * thisExpression = el.get(itemIndex); GenExpression ge = processExpression(qs,
			 * thisExpression, expr); gep.inList.add(ge);
			 * 
			 * // need to put the parameter here } // process the start value if it is a
			 * constant }
			 */
			...
			// gep.setLeftExpresion(processExpression(qs, nullExpr.getLeftExpression(),
			// expr));
			return gep;
		} else {
	...
	public Map<String, List<GenExpression>> getTableColumns(String query) {
		...
		try {
			...
			qs.printLevel2(qs, realTables, 0, null, derivedColumns, levelSelectors, tableColumns, aliases, null, false,
					true);
  ...
  ///////////////////////////////////////////////////////////
	...
	// + "concept c, engineconcept ec, engine e inner join sometable s on
	// c.logicalname=s.logical
	// where (ec.localconceptid=c.localconceptid and "
	// + "c.conceptualname in ('val1', 'val2')) or (ec.localconceptid + 5) =1 group
	// by ln order by
	// ln limit 200 offset 50 ";// order by c.logicalname";
	//
	// query = "select distinct f.studio, (f.movie_budget - 3) / 2 from f where
	// f.movie_budget * 4 >
	// 10";
	//
	// // thank you so much anthem
	// String query2 = " SELECT Member_Engagement_Tier AS `Member Engagement
	// Tier`,Site_Service_Name
	// AS `Site Service Name`," +
	...
	// " SUM(" +
	// " case " +
	// " when CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD='001' then
	// CII_FACT_MBRSHP.MBR_CVRG_CNT " +
	// " else 0 " +
	...
	// " ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN
	// TM_PRD_FNCTN.START_YEAR_MNTH " +
	// " and TM_PRD_FNCTN." +
	// " END_YEAR_MNTH " +
	...
	// " WHERE A.LKUP_ID=1 " +
	// " AND A.YEAR_ID <= 1 ) TM_PRD_FNCTN " +
	// " ON CII_FACT_CP_ENGGMNT.ENGGMNT_MNTH_NBR BETWEEN
	// TM_PRD_FNCTN.START_YEAR_MNTH " +
	// " and TM_PRD_FNCTN." +
	// " END_YEAR_MNTH " +
	...
	// " SUM(" +
	// " case " +
	// " when CII_FACT_CLM_LINE.MBR_CVRG_TYPE_CD='001' then
	// CII_FACT_CLM_LINE.ACCT_PAID_AMT " +
	// " else 0 " +
	...
	// " WHERE A.LKUP_ID=1 " +
	// " AND A.YEAR_ID <= 1) TM_PRD_FNCTN " +
	// " ON CII_FACT_CLM_LINE.CLM_SRVC_YEAR_MNTH_NBR BETWEEN
	// TM_PRD_FNCTN.STRT_SRVC_YEAR_MNTH " +
	// " and TM_PRD_FNCTN." +
	// " END_SRVC_YEAR_MNTH " +
	// " AND CII_FACT_CLM_LINE.RPTG_PAID_YEAR_MNTH_NBR BETWEEN
	// TM_PRD_FNCTN.START_YEAR_MNTH " +
	// " and TM_PRD_FNCTN." +
	// " END_RPTG_PAID_YEAR_MNTH " +
	...
	// // selectbody - it can either be a simple one or a set operation - This is
	// where the union
	// sits. This can be a plain select or something else - I only accomodate for
	// plain select at this
	// point
	...
	//
	// String query4 = " SELECT Health_Condition_Category_Description AS \"Health
	// Condition Category
	// Description\",\r\n" +
	// " Total_Utilization_Count AS \"Total Utilization Count\" \r\n" +
	// "FROM ( \r\n" +
	// "SELECT CLMS.DIM_DIAG_HLTH_CNDTN_CTGRY_DESC AS
	// Health_Condition_Category_Description,\r\n" +
	// " SUM(CLMS.TOTL_UTIL_CNT) AS Total_Utilization_Count,CLMS.TM_PRD_NM AS
	// Time_Period \r\n" +
	// "FROM ( \r\n" +
	// "SELECT DIM_DIAG.HLTH_CNDTN_CTGRY_DESC AS
	// DIM_DIAG_HLTH_CNDTN_CTGRY_DESC,\r\n" +
	// " sum(CII_FACT_CLM_LINE.ACCT_ADMT_CNT + CII_FACT_CLM_LINE.ACCT_VST_CNT) AS
	// TOTL_UTIL_CNT,\r\n" +
	// " TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM \r\n" +
	...
	//
	// String query5 = "SELECT Time_Period AS \"Time
	// Period\",Health_Condition_Category_Description
	// AS \"Health Condition Category Description\",\r\n" +
	// " Total_Utilization_Count AS \"Total Utilization Count\",Paid_Amount AS
	// \"Paid
	// Amount\",\r\n" +
	// " Account_Name AS \"Account Name\",CBSA_Name AS \"CBSA
	// Name\",ICD_Diagnosis_Source_Short_Code_Definition_Text AS \"ICD Diagnosis
	// Source Short Code
	// Definition Text\",\r\n" +
	// " Paid_PMPM AS \"Paid PMPM\",Paid_HCC_Indicator AS \"Paid HCC Indicator\"
	// \r\n" +
	// "FROM ( \r\n" +
	// "SELECT CLMS.TM_PRD_NM AS Time_Period,CLMS.DIM_DIAG_HLTH_CNDTN_CTGRY_DESC AS
	// Health_Condition_Category_Description,\r\n" +
	// " SUM(CLMS.TOTL_UTIL_CNT) AS Total_Utilization_Count,SUM(CLMS.ACCT_PAID_AMT)
	// AS
	// Paid_Amount,\r\n" +
	// " CLMS.CII_ACCT_PRFL_ACCT_NM AS Account_Name,CLMS.DIM_CBSA_CBSA_NM AS
	// CBSA_Name,\r\n" +
	// " CLMS.DIM_DIAG_SRC_SHRT_CD_DEFN_TXT AS
	// ICD_Diagnosis_Source_Short_Code_Definition_Text,\r\n" +
	// " CASE \r\n" +
	// " WHEN SUM(MBRSHP.TOTL_CVRG_CNT) = 0 THEN 0 \r\n" +
	// " ELSE SUM(CAST (CLMS.ACCT_PAID_AMT as DECIMAL(18,6))) / SUM(CAST
	// (MBRSHP.TOTL_CVRG_CNT as
	// DECIMAL(18,\r\n" +
	// " 6))) \r\n" +
	// "END AS Paid_PMPM,CLMS.PAID_HCC_IND AS Paid_HCC_Indicator \r\n" +
	// "FROM ( \r\n" +
	// "SELECT TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM,DIM_DIAG.HLTH_CNDTN_CTGRY_DESC AS
	// DIM_DIAG_HLTH_CNDTN_CTGRY_DESC,\r\n" +
	// " sum(cii_fact_clm_line.ACCT_ADMT_CNT +cii_fact_clm_line.ACCT_VST_CNT) AS
	// TOTL_UTIL_CNT,\r\n" +
	// " SUM(CII_FACT_CLM_LINE.ACCT_PAID_AMT) AS ACCT_PAID_AMT,CII_ACCT_PRFL.ACCT_NM
	// AS
	// CII_ACCT_PRFL_ACCT_NM,\r\n" +
	// " DIM_CBSA.CBSA_NM AS DIM_CBSA_CBSA_NM,DIM_DIAG.SRC_SHRT_CD_DEFN_TXT AS
	// DIM_DIAG_SRC_SHRT_CD_DEFN_TXT,\r\n" +
	// " COALESCE(HCC_CHK.PAID_HCC_IND,'N') AS
	// PAID_HCC_IND,CII_FACT_CLM_LINE.CBSA_ID AS
	// CBSA_ID,\r\n" +
	// " CII_FACT_CLM_LINE.ACCT_ID AS ACCT_ID \r\n" +
	// "FROM CII_FACT_CLM_LINE INNER JOIN (\r\n" +
	// "SELECT B.*, \r\n" +
	...
	// TM_PRD_FNCTN.START_YEAR_MNTH \r\n" +
	// " and TM_PRD_FNCTN.END_RPTG_PAID_YEAR_MNTH INNER JOIN DIM_DIAG \r\n" +
	// " ON CII_FACT_CLM_LINE.RPTG_PRNCPAL_DIAG_KEY = DIM_DIAG.DIAG_KEY INNER JOIN
	// CII_ACCT_PRFL
	// \r\n" +
	...
	// "END) AS TOTL_CVRG_CNT,CII_FACT_MBRSHP.CBSA_ID AS
	// CBSA_ID,CII_FACT_MBRSHP.ACCT_ID AS ACCT_ID
	// \r\n" +
	...
	// "select acct_id as FRC_JN,ELGBLTY_CY_MNTH_END_NBR, MCID, MIN(CVRG_PRTY_NBR)
	// as
	// MIN_CVRG_PRTY_NBR,\r\n" +
	// " TM_PRD_FNCTN.TM_PRD_NM\r\n" +
	// "from cii_fact_mbrshp \r\n" +
	...
	// "WHERE CII_FACT_MBRSHP.ACCT_ID = 'W0016437' \r\n" +
	// "GROUP BY
	// TM_PRD_FNCTN.TM_PRD_NM,CII_FACT_MBRSHP.CBSA_ID,CII_FACT_MBRSHP.ACCT_ID ) AS
	// MBRSHP
	// \r\n" +
	...
	// "GROUP BY
	// CLMS.TM_PRD_NM,CLMS.DIM_DIAG_HLTH_CNDTN_CTGRY_DESC,CLMS.CII_ACCT_PRFL_ACCT_NM,\r\n"
	// +
	// " CLMS.DIM_CBSA_CBSA_NM,CLMS.DIM_DIAG_SRC_SHRT_CD_DEFN_TXT,CLMS.PAID_HCC_IND
	// ) AS FNL
	// \r\n" +
	// "ORDER BY
	// Time_Period,Health_Condition_Category_Description,Total_Utilization_Count,\r\n"
	// +
	// "
	...
	//
	// String query6 = "\r\n" +
	// " SELECT Time_Period AS \"Time_Period\",Health_Condition_Category_Description
	// AS
	// \"Health_Condition_Category_Description\",\r\n" +
	// " Total_Utilization_Count AS \"Total_Utilization_Count\",Paid_Amount AS
	// \"Paid_Amount\",\r\n" +
	// " Account_Name AS \"Account_Name\",CBSA_Name AS
	// \"CBSA_Name\",ICD_Diagnosis_Source_Short_Code_Definition_Text AS
	// \"ICD_Diagnosis_Source_Short_Code_Definition_Text \" ,\r\n" +
	// " Paid_PMPM AS \"Paid_PMPM\", Paid_HCC_Indicator AS
	// \"Paid_HCC_Indicator\",\r\n" +
	...
	// " DIM_AGE_GRP.AGE_GRP_DESC AS
	// DIM_AGE_GRP_AGE_GRP_DESC,CII_FACT_CLM_LINE.CBSA_ID AS
	// CBSA_ID,\r\n" +
	// " CII_FACT_CLM_LINE.ACCT_ID AS ACCT_ID,CII_FACT_CLM_LINE.ACCT_AGE_GRP_KEY AS
	// ACCT_AGE_GRP_KEY \r\n" +
	// "FROM CII_FACT_CLM_LINE INNER JOIN (\r\n" +
	// "SELECT B.*, \r\n" +
	// "case \r\n" +
	...
	// " ELSE CII_FACT_MBRSHP.MBR_CVRG_CNT \r\n" +
	// "END) AS TOTL_CVRG_CNT,CII_FACT_MBRSHP.CBSA_ID AS
	// CBSA_ID,CII_FACT_MBRSHP.ACCT_ID AS
	// ACCT_ID,\r\n" +
	// " CII_FACT_MBRSHP.ACCT_AGE_GRP_KEY AS ACCT_AGE_GRP_KEY \r\n" +
	// "FROM CII_FACT_MBRSHP INNER JOIN (\r\n" +
	// "SELECT B.*, \r\n" +
	...
	// "WHERE CII_FACT_MBRSHP.ACCT_ID = 'W0016437' \r\n" +
	// "GROUP BY
	// TM_PRD_FNCTN.TM_PRD_NM,CII_FACT_MBRSHP.CBSA_ID,CII_FACT_MBRSHP.ACCT_ID,\r\n"
	// +
	// " CII_FACT_MBRSHP.ACCT_AGE_GRP_KEY ) AS MBRSHP \r\n" +
	// " ON CLMS.CBSA_ID=MBRSHP.CBSA_ID \r\n" +
	// " AND CLMS.ACCT_ID=MBRSHP.ACCT_ID \r\n" +
	...
	// " ";
	// String query7 = "SELECT Time_Period AS \"Time
	// Period\",Health_Condition_Category_Description
	// AS \"Health Condition Category Description\",\r\n" +
	// " Total_Utilization_Count AS \"Total Utilization Count\",Paid_Amount AS
	// \"Paid
	...
	// " CII_ACCT_PRFL.ACCT_NM,DIM_CBSA.CBSA_NM,DIM_DIAG.SRC_SHRT_CD_DEFN_TXT,\r\n"
	// +
	// " CII_FACT_CLM_LINE.CBSA_ID,CII_FACT_CLM_LINE.ACCT_ID ) AS CLMS LEFT OUTER
	// JOIN ( \r\n" +
	// "SELECT TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM,SUM(\r\n" +
	// "CASE \r\n" +
	// " WHEN TMBRSHP.MIN_CVRG_PRTY_NBR IS NULL THEN 0 \r\n" +
	...
	// " Paid_PMPM";
	//
	// String query8 = "Select a.abc, b.b1, coalesce(a.a1, 'abcd'), a.a1 + 2 from a
	// as a, (b) as b,
	// (c) as c on a.id = b.id";
	//
	// String query9 = "SELECT DISTINCT Title.Title AS \"Genre\" , CASE WHEN
	// TMBRSHP.MIN_CVRG_PRTY_NBR IS NULL THEN 0 ELSE CII_FACT_MBRSHP.MBR_CVRG_CNT
	// END AS mango,
	// sum(MovieBudget), ( CAST(( CAST(Title.MovieBudget AS DECIMAL) * CAST(2 AS
	// DECIMAL) ) AS
	// DECIMAL) + CAST(Title.RevenueDomestic AS DECIMAL) ) AS \"Der_value\" FROM
	// Title Title ";
	//
	// String query10 = "SELECT DISTINCT Title.Title AS \"Genre\" , CASE WHEN
	// TMBRSHP.MIN_CVRG_PRTY_NBR in ('a', 'b') THEN 0 WHEN
	// TMBRSHP.MIN_CVRG_PRTY_NBR=30 THEN 21 ELSE
	// CII_FACT_MBRSHP.MBR_CVRG_CNT END AS mango, sum(MovieBudget), ( CAST((
	// CAST(Title.MovieBudget
	// AS DECIMAL) * CAST(2 AS DECIMAL) ) AS DECIMAL) + CAST(Title.RevenueDomestic
	// AS DECIMAL) ) AS
	// \"Der_value\" FROM Title Title WHERE Genre in ('abc', 'y') or Genre=20";
	//
	// String query11 = "SELECT DISTINCT Title.MovieBudget AS \"MovieBudget\" ,
	// Title.RevenueDomestic
	// AS \"RevenueDomestic\" , Title.RevenueInternational AS
	// \"RevenueInternational\" ,
	// Title.RottenTomatoesAudience AS \"RottenTomatoesAudience\" ,
	// Title.RottenTomatoesCritics AS
	// \"RottenTomatoesCritics\" , Title.Title AS \"Title\" FROM Title Title WHERE
	// (Title.MovieBudget) IN ( 1000000 , 1700000 ) \r\n";
	//
	// String query12 = "SELECT DISTINCT customquery.\"MovieBudget\" AS
	// \"MovieBudget\" ,
	// customquery.\"RevenueDomestic\" AS \"RevenueDomestic\" ,
	// customquery.\"RevenueInternational\"
	// AS \"RevenueInternational\" , customquery.\"RottenTomatoesAudience\" AS
	// \"RottenTomatoesAudience\" , customquery.\"RottenTomatoesCritics\" AS
	// \"RottenTomatoesCritics\"
	// , customquery.\"Title\" AS \"Title\" FROM (SELECT avqLBU.\"MovieBudget\",
	// avqLBU.\"RevenueDomestic\", avqLBU.\"RevenueInternational\",
	// avqLBU.\"RottenTomatoesAudience\",
	// avqLBU.\"RottenTomatoesCritics\", avqLBU.\"Title\" FROM ( SELECT
	// Title.MovieBudget as
	// \"MovieBudget\", Title.RevenueDomestic as \"RevenueDomestic\",
	// Title.RevenueInternational as
	// \"RevenueInternational\", Title.RottenTomatoesAudience as
	// \"RottenTomatoesAudience\",
	// Title.RottenTomatoesCritics as \"RottenTomatoesCritics\", Title.Title as
	// \"Title\" FROM Title
	// as Title WHERE (Title.MovieBudget) IN (1000000, 1700000) UNION ALL SELECT
	// Title.MovieBudget as \"MovieBudget\", Title.RevenueDomestic as
	// \"RevenueDomestic\",
	// Title.RevenueInternational as \"RevenueInternational\",
	// Title.RottenTomatoesAudience as
	// \"RottenTomatoesAudience\", Title.RottenTomatoesCritics as
	// \"RottenTomatoesCritics\",
	// Title.Title as \"Title\" FROM Title as Title WHERE (Title.MovieBudget) IN
	// (500000, 800000))
	// AS avqLBU ) AS customquery\r\n";
	//
	...
	//
	//
	// String query17 = "Select a,b, c from tabla where 1 = 0 and (a = 'a' or c =
	// 'q')";
	//
	...
	//
	//
	// String query19 = "Select Nominated, Genre, Studio, Title, MovieBudget,
	// RevenueDomestic,
	// RevenueInternational from Movie2 where Studio = 'WB' and (MovieBudget >
	// 10000000 or MovieBudget
	// > 20000000)";
	// // there are a couple of scenarios to model for join
	// // I have only one join easy for me to drop this - what happens to remaining
	// stuff ?
	// // I have 2 joins
	// String query20 = "Select Nominated, Genre, Studio, Title, MovieBudget,
	// RevenueDomestic,
	// RevenueInternational from Movie2 where MovieBudget = 1000 and Studio ='WB'
	// and RevenueDomestic
	// in (1,2,3,4)";
	//
	// String query21 = "Select d123.a, b, c from d as d123 where a = NULL";
	//
	// String query22 = "SELECT Account_ID AS \"Account ID\",Master_Consumer_ID AS
	// \"Master Consumer
	// ID\",Member_Coverage_Type_Description AS \"Member Coverage Type
	// Description\",Member_Gender_Code AS \"Member Gender
	// Code\",Reporting_Member_Relationship_Description AS \"Reporting Member
	// Relationship
	// Description\",Age_Group_Description AS \"Age Group Description\",Age_In_Years
	// AS \"Age In
	// Years\",Contract_Type_Code AS \"Contract Type
	// Code\",Eligibility_Year_Month_Ending_Number AS
	// \"Eligibility Year Month Ending Number\",State_Code AS \"State
	// Code\",CBSA_Name AS \"CBSA
	// Name\",Member_PCP_Indicator AS \"Member PCP Indicator\",Subscriber_ID AS
	// \"Subscriber
	// ID\",Continuous_Enrollment_for_1_Period AS \"Continuous Enrollment for 1
	// Period\",Member_Birth_Date AS \"Member Birth Date\",Account_Name AS \"Account
	// Name\",Time_Period_Start AS \"Time Period Start\",Time_Period_End AS \"Time
	// Period
	// End\",Non_Utilizer_Indicator AS \"Non Utilizer
	// Indicator\",Member_Coverage_Count AS \"Member
	// Coverage Count\" FROM ( SELECT MBRSHP.ACCT_ID AS Account_ID,MBRSHP.MCID AS
	// Master_Consumer_ID,MBRSHP.MBR_CVRG_TYPE_DESC AS
	// Member_Coverage_Type_Description,MBRSHP.MBR_GNDR_CD AS
	// Member_Gender_Code,MBRSHP.RPTG_MBR_RLTNSHP_DESC AS
	// Reporting_Member_Relationship_Description,MBRSHP.AGE_GRP_DESC AS
	// Age_Group_Description,MBRSHP.AGE_IN_YRS_NBR AS
	// Age_In_Years,MBRSHP.CNTRCT_TYPE_CD AS
	// Contract_Type_Code,MBRSHP.ELGBLTY_CY_MNTH_END_NBR AS
	// Eligibility_Year_Month_Ending_Number,MBRSHP.ST_CD AS
	// State_Code,MBRSHP.CBSA_NM AS
	// CBSA_Name,MBRSHP.PCP_IND AS Member_PCP_Indicator,MBRSHP.FMBRSHP_SBSCRBR_ID AS
	// Subscriber_ID,MBRSHP.CNTNUS_ENRLMNT_1_PRD_CD AS
	// Continuous_Enrollment_for_1_Period,MBRSHP.MBR_BRTH_DT AS
	// Member_Birth_Date,MBRSHP.ACCT_NM AS
	// Account_Name,MBRSHP.TIME_PRD_STRT_NBR AS
	// Time_Period_Start,MBRSHP.TIME_PRD_END_NBR AS
	// Time_Period_End,MBRSHP.Non_Utilizer_Ind AS
	// Non_Utilizer_Indicator,SUM(MBRSHP.SUM_MBR_CVRG_CNT)
	// AS Member_Coverage_Count,MBRSHP.TM_PRD_NM AS Time_Period FROM ( SELECT
	// CII_ACCT_PRFL.ACCT_ID
	// AS ACCT_ID,CII_FACT_MBRSHP.MCID AS MCID,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC
	// AS
	// MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.MBR_GNDR_CD AS
	// MBR_GNDR_CD,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC AS
	// RPTG_MBR_RLTNSHP_DESC,case when
	// DIM_AGE_GRP.AGE_GRP_DESC='1-17' then '1 through 17' else
	// DIM_AGE_GRP.AGE_GRP_DESC end AS
	// AGE_GRP_DESC,CII_FACT_MBRSHP.AGE_IN_YRS_NBR AS
	// AGE_IN_YRS_NBR,CII_FACT_MBRSHP.CNTRCT_TYPE_CD AS
	// CNTRCT_TYPE_CD,CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR AS
	// ELGBLTY_CY_MNTH_END_NBR,CII_FACT_MBRSHP.ST_CD AS ST_CD,DIM_CBSA.CBSA_NM AS
	// CBSA_NM,CII_FACT_MBRSHP.PCP_IND AS PCP_IND,CII_FACT_MBRSHP.SBSCRBR_ID AS
	// FMBRSHP_SBSCRBR_ID,CE.CNTNUS_ENRLMNT_1_PRD_CD AS
	// CNTNUS_ENRLMNT_1_PRD_CD,DIM_MCID.MBR_BRTH_DT
	// AS MBR_BRTH_DT,CII_ACCT_PRFL.ACCT_NM AS ACCT_NM,CASE WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Current
	// Period' THEN 201910 WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period' THEN 201810
	// WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period 2' THEN 201801 end AS
	// TIME_PRD_STRT_NBR,CASE WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Current Period' THEN 202009 WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Prior
	// Period' THEN 201909 WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period 2' THEN 201809
	// end AS
	// TIME_PRD_END_NBR,UT.Non_Utilizer_Ind AS Non_Utilizer_Ind,SUM(MBR_CVRG_CNT) AS
	// SUM_MBR_CVRG_CNT,TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM FROM CII_FACT_MBRSHP
	// INNER JOIN (SELECT
	...
	// " and dm.YEAR_MNTH_NBR = 202009) ) TM_PRD_FNCTN ON
	// CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN TM_PRD_FNCTN.START_YEAR_MNTH
	// and
	// TM_PRD_FNCTN.END_YEAR_MNTH INNER JOIN ( Select\r\n" +
	// "mbrshp.ACCT_ID,\r\n" +
	// "mbrshp.MCID,\r\n" +
	...
	// \r\n" +
	// " and TM_PRD_FNCTN.END_YEAR_MNTH \r\n" +
	// " INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY from ACIISST_SGMNTN_BRDG where
	// ACIISST_USER_ID= 16200068 and ACCT_ID in ('W0004156') and SRC_FLTR_ID=
	// '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and FLTR_SRC_NM= 'User Session')SGMNTN
	// on
	// fact.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and fact.ACCT_ID=SGMNTN.ACCT_ID
	// \r\n" +
	// "WHERE fact.ACCT_ID = 'W0004156' \r\n" +
	// "GROUP BY fact.acct_id, fact.MCID, TM_PRD_FNCTN.TM_PRD_NM,
	...
	// '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and FLTR_SRC_NM= 'User Session')SGMNTN
	// on
	// clm.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and
	// clm.ACCT_ID=SGMNTN.ACCT_ID\r\n" +
	// "WHERE clm.ACCT_ID = 'W0004156' \r\n" +
	// "GROUP BY clm.acct_id, clm.MCID, TM_PRD_FNCTN.TM_PRD_NM,
	// clm.MBR_CVRG_TYPE_CD\r\n" +
	// ") clms\r\n" +
	// "\r\n" +
	// " on\r\n" +
	...
	// " and CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD = UT.MBR_CVRG_TYPE_CD INNER JOIN
	// CII_ACCT_PRFL ON CII_FACT_MBRSHP.ACCT_ID = CII_ACCT_PRFL.ACCT_ID INNER JOIN
	// DIM_MBR_CVRG_TYPE
	// ON CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD = DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_CD
	// INNER JOIN
	// DIM_RPTG_MBR_RLTNSHP ON CII_FACT_MBRSHP.RPTG_MBR_RLTNSHP_CD =
	// DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_CD INNER JOIN DIM_AGE_GRP ON
	// CII_FACT_MBRSHP.ACCT_AGE_GRP_KEY = DIM_AGE_GRP.AGE_GRP_KEY INNER JOIN
	// DIM_CBSA ON
	// CII_FACT_MBRSHP.CBSA_ID = DIM_CBSA.CBSA_ID INNER JOIN ( \r\n" +
	...
	// " fact.MBR_CVRG_TYPE_CD ,\r\n" +
	// " count (distinct fact
	// .ELGBLTY_CY_MNTH_END_NBR) mnths ,\r\n" +
	// " min(fact .ELGBLTY_CY_MNTH_END_NBR)
	// strt_mnth ,\r\n" +
	...
	// " and dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201809\r\n" +
	// " and dm.YEAR_MNTH_NBR = 202009) ) TM_PRD_FNCTN \r\n" +
	// " ON fact .ELGBLTY_CY_MNTH_END_NBR BETWEEN
	// TM_PRD_FNCTN.START_YEAR_MNTH \r\n" +
	// " and TM_PRD_FNCTN. \r\n" +
	// " END_YEAR_MNTH INNER JOIN (Select
	// ACCT_ID,SGMNTN_DIM_KEY from ACIISST_SGMNTN_BRDG where ACIISST_USER_ID=
	// 16200068 and ACCT_ID in
	// ('W0004156') and SRC_FLTR_ID= '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and
	// FLTR_SRC_NM= 'User
	// Session')SGMNTN on fact.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and
	// fact.ACCT_ID=SGMNTN.ACCT_ID
	// \r\n" +
	// " WHERE fact .ACCT_ID = 'W0004156'
	// \r\n" +
	...
	// " select \r\n" +
	// " case \r\n" +
	// " when YEAR_ID= 1 then 'Current
	// Period' \r\n" +
	// " when YEAR_ID= 2 then 'Prior
	// Period' \r\n" +
	// " when YEAR_ID= 3 then 'Prior
	// Period 2' \r\n" +
	// " ELSE 'FOO' \r\n" +
	// " END as TM_PRD_NM,\r\n" +
	...
	// " and CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD = CE.MBR_CVRG_TYPE_CD INNER JOIN
	// DIM_MCID
	// ON CII_FACT_MBRSHP.MCID = DIM_MCID.MCID AND CII_FACT_MBRSHP.ACCT_ID =
	// DIM_MCID.ACCT_ID INNER
	// JOIN (Select ACCT_ID,SGMNTN_DIM_KEY from ACIISST_SGMNTN_BRDG where
	// ACIISST_USER_ID= 16200068
	// and ACCT_ID in ('W0004156') and SRC_FLTR_ID=
	// '2c64ebe7-d64b-48ee-99cc-d66eb263819d' and
	// FLTR_SRC_NM= 'User Session')SGMNTN on CII_FACT_MBRSHP.SGMNTN_DIM_KEY =
	// SGMNTN.SGMNTN_DIM_KEY
	// and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID in
	// ('W0004156')
	// GROUP BY
	// CII_ACCT_PRFL.ACCT_ID,CII_FACT_MBRSHP.MCID,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.MBR_GNDR_CD,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC,case
	// when DIM_AGE_GRP.AGE_GRP_DESC='1-17' then '1 through 17' else
	// DIM_AGE_GRP.AGE_GRP_DESC
	// end,CII_FACT_MBRSHP.AGE_IN_YRS_NBR,CII_FACT_MBRSHP.CNTRCT_TYPE_CD,CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR,CII_FACT_MBRSHP.ST_CD,DIM_CBSA.CBSA_NM,CII_FACT_MBRSHP.PCP_IND,CII_FACT_MBRSHP.SBSCRBR_ID,CE.CNTNUS_ENRLMNT_1_PRD_CD,DIM_MCID.MBR_BRTH_DT,CII_ACCT_PRFL.ACCT_NM,CASE
	// WHEN TM_PRD_FNCTN.TM_PRD_NM= 'Current Period' THEN 201910 WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period' THEN 201810 WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period 2' THEN 201801 end,CASE WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Current Period' THEN 202009 WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period' THEN 201909 WHEN
	// TM_PRD_FNCTN.TM_PRD_NM= 'Prior Period 2' THEN 201809
	// end,UT.Non_Utilizer_Ind,TM_PRD_FNCTN.TM_PRD_NM ) AS MBRSHP GROUP BY
	// MBRSHP.ACCT_ID,MBRSHP.MCID,MBRSHP.MBR_CVRG_TYPE_DESC,MBRSHP.MBR_GNDR_CD,MBRSHP.RPTG_MBR_RLTNSHP_DESC,MBRSHP.AGE_GRP_DESC,MBRSHP.AGE_IN_YRS_NBR,MBRSHP.CNTRCT_TYPE_CD,MBRSHP.ELGBLTY_CY_MNTH_END_NBR,MBRSHP.ST_CD,MBRSHP.CBSA_NM,MBRSHP.PCP_IND,MBRSHP.FMBRSHP_SBSCRBR_ID,MBRSHP.CNTNUS_ENRLMNT_1_PRD_CD,MBRSHP.MBR_BRTH_DT,MBRSHP.ACCT_NM,MBRSHP.TIME_PRD_STRT_NBR,MBRSHP.TIME_PRD_END_NBR,MBRSHP.Non_Utilizer_Ind,MBRSHP.TM_PRD_NM
	// ) AS FNL ORDER BY
	// Account_ID,Master_Consumer_ID,Member_Coverage_Type_Description,Member_Gender_Code,Reporting_Member_Relationship_Description,Age_Group_Description,Age_In_Years,Contract_Type_Code,Eligibility_Year_Month_Ending_Number,State_Code,CBSA_Name,Member_PCP_Indicator,Subscriber_ID,Continuous_Enrollment_for_1_Period,Member_Birth_Date,Account_Name,Time_Period_Start,Time_Period_End,Non_Utilizer_Indicator,Member_Coverage_Count\r\n";
	//
	// String query23 = "SELECT Account_ID AS \"Account ID\",Master_Consumer_ID AS
	// \"Master Consumer
	// ID\",Member_Coverage_Type_Description AS \"Member Coverage Type
	// Description\",Member_Gender_Code AS \"Member Gender
	// Code\",Reporting_Member_Relationship_Description AS \"Reporting Member
	...
	// \r\n" +
	// " and TM_PRD_FNCTN.END_YEAR_MNTH \r\n" +
	// " INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from
	// ACIISST_SGMNTN_BRDG where ACCT_ID in ('W0016437') and SRC_FLTR_ID in
	// ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on fact.SGMNTN_DIM_KEY =
	// SGMNTN.SGMNTN_DIM_KEY
	// and fact.ACCT_ID=SGMNTN.ACCT_ID \r\n" +
	...
	// " INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from
	// ACIISST_SGMNTN_BRDG where ACCT_ID in ('W0016437') and SRC_FLTR_ID in
	// ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on clm.SGMNTN_DIM_KEY =
	// SGMNTN.SGMNTN_DIM_KEY
	// and clm.ACCT_ID=SGMNTN.ACCT_ID\r\n" +
	...
	// .ELGBLTY_CY_MNTH_END_NBR) mnths ,\r\n" +
	// " min(fact
	// .ELGBLTY_CY_MNTH_END_NBR) strt_mnth ,\r\n" +
	// " max(fact
	...
	// TM_PRD_FNCTN.START_YEAR_MNTH \r\n" +
	// " and TM_PRD_FNCTN. \r\n" +
	// " END_YEAR_MNTH INNER JOIN
	// (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from
	// ACIISST_SGMNTN_BRDG where ACCT_ID
	// in ('W0016437') and SRC_FLTR_ID in
	// ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on
	// fact.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and fact.ACCT_ID=SGMNTN.ACCT_ID
	// \r\n" +
	// " WHERE fact .ACCT_ID = 'W0016437'
	// \r\n" +
	...
	// " case \r\n" +
	// " when
	// YEAR_ID= 1 then 'Current Period' \r\n" +
	// " when
	// YEAR_ID= 2 then 'Prior Period' \r\n" +
	// " when
	// YEAR_ID= 3 then 'Prior Period 2' \r\n" +
	// " ELSE
	// 'FOO' \r\n" +
	// " END as TM_PRD_NM,\r\n" +
	// " max(e_abs) - max(s_abs)+ 1
	// mnths,\r\n" +
	// " max(s_prd) as
	// start_prd,\r\n" +
	// " max(e_prd) as \r\n" +
	...
	// " or YEAR_MNTH_NBR = 201810
	// \r\n" +
	// " or YEAR_MNTH_NBR = 201801
	// then ABS_YEAR_MNTH_NBR \r\n" +
	// "
	// else null \r\n" +
	// " end as
	// s_abs ,\r\n" +
	...
	// " AND CII_FACT_MBRSHP.MCID =CE.MCID \r\n" +
	// " and CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD = CE.MBR_CVRG_TYPE_CD INNER JOIN
	// DIM_MCID ON CII_FACT_MBRSHP.MCID = DIM_MCID.MCID AND CII_FACT_MBRSHP.ACCT_ID
	// = DIM_MCID.ACCT_ID
	// INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from
	// ACIISST_SGMNTN_BRDG
	// where ACCT_ID in ('W0016437') and SRC_FLTR_ID in
	// ('4733bea4-a055-4641-b834-2c6818e13f45'))SGMNTN on
	// CII_FACT_MBRSHP.SGMNTN_DIM_KEY =
	// SGMNTN.SGMNTN_DIM_KEY and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE
	// CII_FACT_MBRSHP.ACCT_ID
	// in ('W0016437') GROUP BY
	// CII_ACCT_PRFL.ACCT_ID,CII_FACT_MBRSHP.MCID,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.MBR_GNDR_CD,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC,case
	// when DIM_AGE_GRP.AGE_GRP_DESC='1-17' then '1 through 17' else
	// DIM_AGE_GRP.AGE_GRP_DESC
	// end,CII_FACT_MBRSHP.AGE_IN_YRS_NBR,CII_FACT_MBRSHP.CNTRCT_TYPE_CD,CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR,CII_FACT_MBRSHP.ST_CD,DIM_CBSA.CBSA_NM,CII_FACT_MBRSHP.PCP_IND,CII_FACT_MBRSHP.SBSCRBR_ID,CE.CNTNUS_ENRLMNT_1_PRD_CD,DIM_MCID.MBR_BRTH_DT,CII_ACCT_PRFL.ACCT_NM,CASE
	...
	// Account_ID,Master_Consumer_ID,Member_Coverage_Type_Description,Member_Gender_Code,Reporting_Member_Relationship_Description,Age_Group_Description,Age_In_Years,Contract_Type_Code,Eligibility_Year_Month_Ending_Number,State_Code,CBSA_Name,Member_PCP_Indicator,Subscriber_ID,Continuous_Enrollment_for_1_Period,Member_Birth_Date,Account_Name,Time_Period_Start,Time_Period_End,Non_Utilizer_Indicator,Member_Coverage_Count";
	//
	// String query30 = "SELECT Account_Name AS \"Account Name\",Gaps_Closed AS
	// \"Gaps
	// Closed\",Member_Engaged_Indicator AS \"Member Engaged
	// Indicator\",Member_Engagement_Tier AS
	// \"Member Engagement Tier\",Time_Period AS \"Time
	// Period\",Earned_Incentive_Indicator AS
	// \"Earned Incentive Indicator\" FROM ( SELECT GAPSVNGS.ACCT_NM AS
	// Account_Name,SUM(GAPSVNGS.GPS_CLSD) AS Gaps_Closed,GAPSVNGS.MBR_ENGGD_IND AS
	// Member_Engaged_Indicator,GAPSVNGS.MBR_ENGGMNT_TIER AS
	// Member_Engagement_Tier,GAPSVNGS.TM_PRD_NM
	// AS Time_Period,GAPSVNGS.EARND_INCNTV_CD AS Earned_Incentive_Indicator FROM (
	// SELECT
	// CII_ACCT_PRFL.ACCT_NM AS ACCT_NM,SUM( CASE WHEN
	// CII_FACT_GAP_SVNGS.CMPLNC_YEAR_MNTH_YEAR_NBR <=
	// CASE WHEN substr(cast(TM_PRD_FNCTN.END_YEAR_MNTH as varchar(6)) ,5,2) = '01'
	// THEN
	// TM_PRD_FNCTN.END_YEAR_MNTH - 89 ELSE TM_PRD_FNCTN.END_YEAR_MNTH - 1 END THEN
	// CII_FACT_GAP_SVNGS.CLSUR_CNT ELSE 0 END) AS GPS_CLSD,coalesce(
	// TMBRENGGMNT2.MBR_ENGGD_IND,'N')
	// AS MBR_ENGGD_IND,coalesce(TMBRENGGMNT.ENGGMNT_TIER, 'Not Engaged') AS
	// MBR_ENGGMNT_TIER,TM_PRD_FNCTN.TM_PRD_NM AS
	// TM_PRD_NM,INCNTV_MBRSHP.EARND_INCNTV_CD AS
	// EARND_INCNTV_CD FROM CII_FACT_GAP_SVNGS INNER JOIN (\r\n" +
	// "Select YEAR_CD_NM as TM_PRD_NM,\r\n" +
	// "STRT_MNTH_NBR as START_YEAR_MNTH,\r\n" +
	// "END_MNTH_NBR as END_YEAR_MNTH,\r\n" +
	// "case\r\n" +
	...
	// "and (\r\n" +
	// "(dtp.CSTM_STRT_CURNT_MNTH_NBR = 201912 and dtp.CSTM_END_CURNT_MNTH_NBR =
	// 202011)\r\n" +
	// "or (dtp.CSTM_STRT_PRIOR_MNTH_NBR = 201812 and dtp.CSTM_END_PRIOR_MNTH_NBR =
	// 201911)\r\n" +
	// "or (dtp.CSTM_STRT_PRIOR_2_MNTH_NBR = 201801 and
	// dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201811) ))
	// TM_PRD_FNCTN ON CII_FACT_GAP_SVNGS.RLSET_RUN_YEAR_MNTH_NBR BETWEEN CASE WHEN
	// substr(cast(TM_PRD_FNCTN.START_YEAR_MNTH as varchar(6)) ,5,2) <= '06' THEN
	// TM_PRD_FNCTN.START_YEAR_MNTH - 94 ELSE TM_PRD_FNCTN.START_YEAR_MNTH - 6 END
	// and CASE WHEN
	// substr(cast(TM_PRD_FNCTN.END_YEAR_MNTH as varchar(6)) ,5,2) <= '06' THEN
	// TM_PRD_FNCTN.END_YEAR_MNTH - 94 ELSE TM_PRD_FNCTN.END_YEAR_MNTH - 6 END INNER
	// JOIN (
	// SELECT MBRSHP.TM_PRD_NM AS
	// Time_Period,MBRSHP.DIM_MBR_CVRG_TYPE_MBR_CVRG_TYPE_DESC AS
	// Member_Coverage_Type_Description, MBRSHP.CII_FACT_MBRSHP_MCID AS
	// Master_Consumer_ID FROM (
	// SELECT TM_PRD_FNCTN.TM_PRD_NM AS TM_PRD_NM,
	// DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC AS
	// DIM_MBR_CVRG_TYPE_MBR_CVRG_TYPE_DESC, CII_FACT_MBRSHP.MCID AS
	// CII_FACT_MBRSHP_MCID
	// FROM CII_FACT_MBRSHP INNER JOIN ( \r\n" +
	// "Select YEAR_CD_NM as TM_PRD_NM,\r\n" +
	// "STRT_MNTH_NBR as START_YEAR_MNTH,\r\n" +
	// "END_MNTH_NBR as END_YEAR_MNTH,\r\n" +
	...
	// "or (dtp.CSTM_STRT_PRIOR_2_MNTH_NBR = 201801 and
	// dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201811) ))
	// TM_PRD_FNCTN ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN
	// TM_PRD_FNCTN.START_YEAR_MNTH and TM_PRD_FNCTN. END_YEAR_MNTH INNER JOIN
	// DIM_MBR_CVRG_TYPE ON CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD =
	// DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_CD INNER JOIN (Select ACCT_ID,SGMNTN_DIM_KEY,
	// SGMNTN_NM,
	// SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where ACCT_ID in ('W0016437') and
	// SRC_FLTR_ID in
	// ('d97ef60a-2503-41d2-acac-2c91c904e9e0'))SGMNTN on
	// CII_FACT_MBRSHP.SGMNTN_DIM_KEY =
	// SGMNTN.SGMNTN_DIM_KEY and CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE
	// CII_FACT_MBRSHP.ACCT_ID IN ('W0016437') AND
	// DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC =
	// 'Medical' GROUP BY TM_PRD_FNCTN.TM_PRD_NM,
	// DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC,
	// CII_FACT_MBRSHP.MCID ) AS MBRSHP GROUP BY MBRSHP.TM_PRD_NM,
	// MBRSHP.DIM_MBR_CVRG_TYPE_MBR_CVRG_TYPE_DESC, CII_FACT_MBRSHP_MCID ) AS
	// MDCL_MBRSHP on
	// (TM_PRD_FNCTN.TM_PRD_NM = MDCL_MBRSHP.Time_Period and CII_FACT_GAP_SVNGS.MCID
	// =
	// MDCL_MBRSHP.Master_Consumer_ID) INNER JOIN (SELECT
	// MAX(CII_FACT_MBRSHP.EARND_INCNTV_CD)
	// EARND_INCNTV_CD, CII_FACT_MBRSHP.MCID, CII_FACT_MBRSHP.ACCT_ID, TM_PRD_NM
	// FROM CII_FACT_MBRSHP
	// \r\n" +
	// "JOIN\r\n" +
	// "(SELECT YEAR_CD_NM as TM_PRD_NM,\r\n" +
	...
	// ) TM_PRD_FNCTN \r\n" +
	// " ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN
	// TM_PRD_FNCTN.START_YEAR_MNTH and
	// TM_PRD_FNCTN.END_YEAR_MNTH WHERE CII_FACT_MBRSHP.ACCT_ID IN ('W0016437') and
	// ELGBL_INCNTV_IND='Y'\r\n" +
	// "group by CII_FACT_MBRSHP.MCID, CII_FACT_MBRSHP.ACCT_ID, TM_PRD_NM)\r\n" +
	// "INCNTV_MBRSHP \r\n" +
	// "ON CII_FACT_GAP_SVNGS.MCID=INCNTV_MBRSHP.MCID AND
	// TM_PRD_FNCTN.TM_PRD_NM=INCNTV_MBRSHP.TM_PRD_NM AND
	// CII_FACT_GAP_SVNGS.ACCT_ID=INCNTV_MBRSHP.ACCT_ID INNER JOIN CII_ACCT_PRFL ON
	// CII_FACT_GAP_SVNGS.ACCT_ID = CII_ACCT_PRFL.ACCT_ID LEFT JOIN ( \r\n" +
	...
	// " MAX(DIM_ENGGMNT.EXPNDD_IND) as isExpanded , MAX(DIM_ENGGMNT.TRDTNL_IND) as
	// isTraditional
	// \r\n" +
	...
	// " or DIM_ENGGMNT.ENHNCD_IND= 1 \r\n" +
	// "union all \r\n" +
	// "select cast('Traditional' as char(20)) as ENGGMNT, cast('Care Coordination'
	// as char(20))
	// AS ENGGMNT_TIER \r\n" +
	// "from DIM_ENGGMNT \r\n" +
	// "where DIM_ENGGMNT.TRDTNL_IND= 1 ) b \r\n" +
	...
	// ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where
	// ACCT_ID in
	// ('W0016437') and SRC_FLTR_ID in
	// ('d97ef60a-2503-41d2-acac-2c91c904e9e0'))SGMNTN on
	// CII_FACT_GAP_SVNGS.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and
	// CII_FACT_GAP_SVNGS.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_GAP_SVNGS.ACCT_ID in
	// ('W0016437')
	// GROUP BY CII_ACCT_PRFL.ACCT_NM,coalesce(
	// TMBRENGGMNT2.MBR_ENGGD_IND,'N'),coalesce(TMBRENGGMNT.ENGGMNT_TIER, 'Not
	// Engaged'),TM_PRD_FNCTN.TM_PRD_NM,INCNTV_MBRSHP.EARND_INCNTV_CD ) AS GAPSVNGS
	// GROUP BY
	// GAPSVNGS.ACCT_NM,GAPSVNGS.MBR_ENGGD_IND,GAPSVNGS.MBR_ENGGMNT_TIER,GAPSVNGS.TM_PRD_NM,GAPSVNGS.EARND_INCNTV_CD
	// ) AS FNL ORDER BY
	// Account_Name,Gaps_Closed,Member_Engaged_Indicator,Member_Engagement_Tier,Time_Period,Earned_Incentive_Indicator\r\n"
	// +
	// "";
	//
	// String query31 = "SELECT Account_Name AS \"Account Name\",Gaps_Closed AS
	// \"Gaps
	// Closed\",Member_Engaged_Indicator AS \"Member Engaged
	// Indicator\",Member_Engagement_Tier AS
	// \"Member Engagement Tier\",Time_Period AS \"Time
	...
	// "";
	//
	// String query24 = "Select a,b,c from d where (age > 20 and age < 40) or (age >
	// 40 and age <
	// 70)";
	//
	...
	//
	// String query27 = "select cast('Traditional' as char(20)) as abc from
	// table123";
	// String query28 = "select '123' as abc from table123";
	//
	// String query29 = "select substr(cast(TM_PRD_FNCTN.START_YEAR_MNTH as
	// varchar(6)) ,5,2) from
	// abc";
	//
	// String query32 = "select stupid(count(*)) from mango";
	//
	// String query33 = "select count(*) from ( SELECT AS \"Account ID\",AS
	// \"Account Name\",AS
	// \"Time Period\",AS \"Member Coverage Type Description\",AS \"Eligibility Year
	// Month Ending
	// Number\",AS \"Member Coverage Count\",CASE WHEN sum(CAST(AS decimal (22,6)))
	// over(partition by
	// )=0 then 0.00 else SUM(CAST(AS decimal (22,6))) over(partition by )/
	// sum(CAST(AS decimal
	// (22,6))) over(partition by ) *100 end AS \"Member Coverage Count - Total %:
	// Member Coverage
	// Type Description,Reporting Member Relationship Description,Eligibility Year
	// Month Ending
	// Number,,Eligibility Calendar Month Year Name\",AS \"Reporting Member
	// Relationship
	// Description\",AS \"Eligibility Calendar Month Year Name\", AS \"Contract Type
	// Description\"
	// FROM ( SELECT MBRSHP.ACCT_ID AS MBRSHP.ACCT_NM AS MBRSHP.TM_PRD_NM AS
	// MBRSHP.MBR_CVRG_TYPE_DESC AS MBRSHP.ELGBLTY_CY_MNTH_END_NBR AS
	// SUM(MBRSHP.SUM_MBR_CVRG_CNT) AS
	// MBRSHP.RPTG_MBR_RLTNSHP_DESC AS MBRSHP.CLNDR_MNTH_YEAR_NM AS
	// MBRSHP.CNTRCT_TYPE_DESC AS FROM
	// ( SELECT CII_ACCT_PRFL.ACCT_ID AS ACCT_ID,CII_ACCT_PRFL.ACCT_NM AS
	// ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM AS
	// TM_PRD_NM,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC AS
	// MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR AS
	// ELGBLTY_CY_MNTH_END_NBR,SUM(MBR_CVRG_CNT) AS
	// SUM_MBR_CVRG_CNT,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC AS
	// RPTG_MBR_RLTNSHP_DESC,DIM_MNTH.CLNDR_MNTH_YEAR_NM AS
	// CLNDR_MNTH_YEAR_NM,DIM_CNTRCT_TYPE.CNTRCT_TYPE_DESC AS CNTRCT_TYPE_DESC FROM
	// CII_FACT_MBRSHP
	// INNER JOIN (SELECT YEAR_CD_NM as TM_PRD_NM,\r\n" +
	// " STRT_MNTH_NBR as START_YEAR_MNTH,\r\n" +
	// " END_MNTH_NBR as END_YEAR_MNTH,\r\n" +
	// " case\r\n" +
	// " when TM_PRD_TYPE_CD = 'Custom'\r\n" +
	...
	// "and (\r\n" +
	// "(dtp.CSTM_STRT_CURNT_MNTH_NBR = 202003 and dtp.CSTM_END_CURNT_MNTH_NBR =
	// 202102)\r\n" +
	// "or (dtp.CSTM_STRT_PRIOR_MNTH_NBR = 201903 and dtp.CSTM_END_PRIOR_MNTH_NBR =
	// 202002)\r\n" +
	// "or (dtp.CSTM_STRT_PRIOR_2_MNTH_NBR = 201803 and
	// dtp.CSTM_END_PRIOR_2_MNTH_NBR = 201902) ) )
	// TM_PRD_FNCTN ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR BETWEEN
	// TM_PRD_FNCTN.START_YEAR_MNTH
	// and TM_PRD_FNCTN.END_YEAR_MNTH INNER JOIN DIM_MBR_CVRG_TYPE ON
	// CII_FACT_MBRSHP.MBR_CVRG_TYPE_CD
	// = DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_CD INNER JOIN DIM_RPTG_MBR_RLTNSHP ON
	// CII_FACT_MBRSHP.RPTG_MBR_RLTNSHP_CD =
	// DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_CD INNER JOIN
	// DIM_MNTH ON CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR = DIM_MNTH.YEAR_MNTH_NBR
	// INNER JOIN
	// DIM_CNTRCT_TYPE ON CII_FACT_MBRSHP.CNTRCT_TYPE_CD =
	// DIM_CNTRCT_TYPE.CNTRCT_TYPE_CD INNER JOIN
	// CII_ACCT_PRFL ON CII_FACT_MBRSHP.ACCT_ID = CII_ACCT_PRFL.ACCT_ID INNER JOIN
	// (Select
	// ACCT_ID,SGMNTN_DIM_KEY, SGMNTN_NM, SRC_FLTR_ID from ACIISST_SGMNTN_BRDG where
	// ACCT_ID in
	// ('W0003618') and SRC_FLTR_ID in
	// ('195d1b33-4e3c-4ebc-b51f-acfb59d288a1'))SGMNTN on
	// CII_FACT_MBRSHP.SGMNTN_DIM_KEY = SGMNTN.SGMNTN_DIM_KEY and
	// CII_FACT_MBRSHP.ACCT_ID=SGMNTN.ACCT_ID WHERE CII_FACT_MBRSHP.ACCT_ID in
	// ('W0003618') GROUP BY
	// CII_ACCT_PRFL.ACCT_ID,CII_ACCT_PRFL.ACCT_NM,TM_PRD_FNCTN.TM_PRD_NM,DIM_MBR_CVRG_TYPE.MBR_CVRG_TYPE_DESC,CII_FACT_MBRSHP.ELGBLTY_CY_MNTH_END_NBR,DIM_RPTG_MBR_RLTNSHP.RPTG_MBR_RLTNSHP_DESC,DIM_MNTH.CLNDR_MNTH_YEAR_NM,DIM_CNTRCT_TYPE.CNTRCT_TYPE_DESC
	// ) AS MBRSHP GROUP BY
	// MBRSHP.ACCT_ID,MBRSHP.ACCT_NM,MBRSHP.TM_PRD_NM,MBRSHP.MBR_CVRG_TYPE_DESC,MBRSHP.ELGBLTY_CY_MNTH_END_NBR,MBRSHP.RPTG_MBR_RLTNSHP_DESC,MBRSHP.CLNDR_MNTH_YEAR_NM,MBRSHP.CNTRCT_TYPE_DESC
	// ) AS FNL ) t";
	//
	...
	// // every <> block can then be used to understand what are the values inside
	// of it
	// // the main index is primarily paramStringToParamStruct
	// // which then tags into gen expression
	// // you can set any of this and then invoke fill parameters which will fill
	// the query and send
	// result back as a query
	...
	// plist.add(pStruct);
	// String finalQuery = GenExpressionWrapper.transformQueryWithParams(query20,
	// plist);
	...
	//
	// // works with Query 2
	// //Object [] output = root.printLineage(root, "Member Engagement Tier", null
	// ,null, null, 0);
	//
	// // with alias
	...
	// //Object [] output = root.printLineage(root, "B1", null ,null, null, 0);
	//
	// System.out.println("Physical Name for B3 >> " +
	// wrapper.getPhysicalColumnName(root, "B3"));
	// System.out.println("Physical Name for A3 >> " +
	// wrapper.getPhysicalColumnName(root, "A3"));
	//
	// System.out.println("Detok List >> " +
	// wrapper.getColumnsForFunction("detok"));
	//
	...
	// // =CII_FACT_MBRSHP.ACCT_ID
	// /*
	// ParamStruct replaceStruct =
	// test.operatorTableColumnParamIndex.get("=CII_FACT_MBRSHP.ACCT_ID");
