...
public class ParserTest {
	...
	// keeps the table to alias name
	// it also keeps  alias to table
	Hashtable <String, String> tableAlias = new Hashtable <String, String>();
	Hashtable <String, String> columnAlias = new Hashtable <String, String>();
	...
	public static void main2(String [] args) throws SqlParseException
	{
		...
		query =  "select distinct c.logicalname, ec.physicalname from "
				...
				+ "c.conceptualname in ('val1', 'val2') and ec.localconceptid=1";// order by c.logicalname";
		...
		if(node instanceof SqlSelect)
		{
			...
			if(whereNode != null)
			{
				System.out.println("Where node " +whereNode);
				// try to add a node
				
				if(whereNode instanceof SqlBasicCall)
				{
					SqlBasicCall bcNode = (SqlBasicCall)whereNode;
					System.out.println("Where node " +bcNode);
					
					//SqlIdentifier id1 = new SqlIdentifier("random", 0);
					//SqlIdentifier id2 = new SqlIdentifier("random2", 0);
					//SqlBinaryOperator op = new SqlBinaryOperator("AND", SqlKind.AND, prec, leftAssoc, returnTypeInference, operandTypeInference, operandTypeChecker)
					//SqlBasicCall newNode = new SqlBasicCall(SqlOperator., operands, pos)
				}
			}
		}
		//node.
		SqlPrettyWriter spw = new SqlPrettyWriter(SqlDialect.DUMMY);
