...
public class SqlParser {
   ...
   * @return
   */
  private IQuerySelector determineSelector(
      Expression expr, IQuerySelector parentQuerySelector, EXPR_TYPE expressionType, String alias) {
   ...
   * @return
   */
  private IQuerySelector processExpressionSelector(
      Expression expr, IQuerySelector parentSelector, EXPR_TYPE expressionType, String alias) {
    ...
    if (expr instanceof Addition) {
      ...
      if (aExpr.isAllColumns()) {
        ...
        for (int paramIndex = 0; paramIndex < numParamExprs; paramIndex++) {
          ...
          ((QueryFunctionSelector) thisSelector)
              .addInnerSelector(
                  determineSelector(paramExprs.get(paramIndex), null, EXPR_TYPE.INNER, alias));
        }
      }
      ...
      thisSelector =
          processExpressionSelector(
              ((Parenthesis) expr).getExpression(), parentSelector, expressionType, alias);
    }
   ...
   * @param expressionType
   */
  private void setChildSelectorInParentSelector(
      IQuerySelector parentSelector, IQuerySelector childSelector, EXPR_TYPE expressionType) {
