  ...
  protected void setVariables() {
    try {
      ...
      for (int colIndex = 1; colIndex <= numColumns; colIndex++) {
        ...
        // IMPALA EDITS
        // Remove the front appended math function and re-add it to address case issue due to impala
        // returning lowercase only
        if (qs != null && !(qs instanceof HardSelectQueryStruct)) {
          if ((qs.getSelectors().get(colIndex - 1).getSelectorType()
              == IQuerySelector.SELECTOR_TYPE.FUNCTION)) {
            QueryFunctionSelector currentSelect =
                (QueryFunctionSelector) qs.getSelectors().get(colIndex - 1);
            String aggregate = currentSelect.getFunction();
            rawHeaders[colIndex - 1] =
                rawHeaders[colIndex - 1].replaceFirst((aggregate.toLowerCase()), aggregate);
            headers[colIndex - 1] =
                headers[colIndex - 1].replaceFirst((aggregate.toLowerCase()), aggregate);
          }
        }
