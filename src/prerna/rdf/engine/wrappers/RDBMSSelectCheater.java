...
public class RDBMSSelectCheater extends AbstractWrapper implements IConstructWrapper {
  ...
  public boolean hasNext() {
    ...
    if (subjectParent == null && objectParent == null && predParent == null) {
      ...
      if (curStmt != null) hasMore = true;
      else ConnectionUtils.closeAllConnections(conn, stmt, rs);
    } else hasMore = true;
    return hasMore;
  }

  private IConstructStatement populateQueryResults() {
    IConstructStatement stmt =
        null; // I know I need to run the magic of doing multiple indexes, but this is how we run it
    // for now i.e. assumes 3 only
    try {
      if (rs.next()) {
        ...
        // var[2] might be empty String for SQL Server because of rs metadata error - if so, get the
        // header by index instead
        if (!rawHeaders[2].equals("")) {
          object = rs.getObject(rawHeaders[2]) + "";
        } else {
          object = rs.getObject(3).toString();
        }
        if (rs.getObject(rawHeaders[0]) != null
            && columnTables.contains(rawHeaders[0].toUpperCase())) {
          String displayName = headers[0];
          //					subjectParent = engine.getTransformedNodeName(Constants.DISPLAY_URI + displayName,
          // false);
          subject = Constants.CONCEPT_URI + displayName + "/" + subject + "";
        }
        if (rs.getObject(rawHeaders[2]) != null
            && columnTables.contains(rawHeaders[2].toUpperCase())) {
          String displayName = headers[2];
          //					objectParent = engine.getTransformedNodeName(Constants.DISPLAY_URI + displayName,
          // false);
  ...
  /**
   * setting variables to be used when querying for traverse freely/explore an instance of a
   * concept. FYI not using displaynames var because we actuallly do the translation later when we
   * pass through GraphDataModel.
   */
  private void setVariables() {
    try {
      ...
      for (int colIndex = 1; colIndex <= numColumns; colIndex++) {
        ...
        if (logName.contains(".")
            && !logName.contains("http://semoss.org/ontologies")
            && !logName.contains(RDF.TYPE + "")
            && !logName.contains(RDFS.SUBCLASSOF + "")
        ...
        if (logName != null
            && logName.length() != 0) // will use this to find what is the type to strap it together
        {
