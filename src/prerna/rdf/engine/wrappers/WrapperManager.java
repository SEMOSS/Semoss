...
public class WrapperManager {
  ...
  public IRawSelectWrapper getRawWrapper(IDatabaseEngine engine, SelectQueryStruct qs)
      throws Exception {
    return getRawWrapper(engine, qs, false);
  }

  public IRawSelectWrapper getRawWrapper(
      IDatabaseEngine engine, SelectQueryStruct qs, boolean delayExecIfPossible) throws Exception {
    String engineId = engine.getEngineId();
    boolean ignoreQueryLogging =
        ignoreDatabases.contains(engineId)
        ...
        try {
          ...
          if (error && returnWrapper != null) {
            if (classLogger.isDebugEnabled()) {
              classLogger.error(
                  "Error occurred executing query on engine "
                      + engine.getEngineId()
                      + " with query = "
                      + query);
            }
