...
public class RawRDBMSSelectWrapper extends AbstractWrapper implements IRawSelectWrapper {
  ...
  protected void setVariables() {
    try {
      ...
      for (int colIndex = 1; colIndex <= numColumns; colIndex++) {
        ...
        types[colIndex - 1] =
            SemossDataType.convertStringToDataType(rsmd.getColumnTypeName(colIndex));
      }
    } catch (SQLException e) {
      logger.error(Constants.STACKTRACE, e);
    }
  ...
  public long getNumRows() {
    if (this.numRows == 0) {
      ...
      try {
        if (this.engine != null
            && ((IRDBMSEngine) this.engine).getDbType() == RdbmsTypeEnum.SQL_SERVER) {
          query = this.query;
        } else {
  ...
  /**
   * This method allows me to perform the execution of a query on a given connection without having
   * to go through a formal RDBMSNativeEngine construct i.e. the naked engine ;)
   *
   * @param conn
   * @param query
   * @throws Exception
   */
  public static RawRDBMSSelectWrapper directExecutionViaConnection(
      Connection conn, String query, boolean closeIfFail) throws Exception {
   ...
   */
  public static RawRDBMSSelectWrapper directExecutionViaConnection(
      IRDBMSEngine database, Connection conn, SelectQueryStruct qs, boolean closeIfFail)
      throws Exception {
   ...
   * @throws Exception
   */
  public static RawRDBMSSelectWrapper directExecutionPreparedStatement(
