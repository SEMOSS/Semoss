...
public class QueryBuilderData {
  ...
  List<Map<String, String>>
      nodeProps; // List of selected property objects. Each object contains SubjectVar-->Logical
  // Name of node, uriKey-->Physical uri for property, varKey-->logical name of
  // property
  Map<String, List<Object>>
      filterData; // For each var to filter, has Logical name-->List of values to include
  List<String> returnVars; // Specifies what variables to return and in what order
  Boolean limitReturnToVarsList =
      false; // If you want to only return the vars specified in the returnVars list, set this to
  ...
  public QueryBuilderData(Map json) {
    ...
    if (json.containsKey(QUERY_DATA_KEY)) {
      ...
      if (queryData.get(FILTERS_KEY) != null) setFilterData((Map) queryData.get(FILTERS_KEY));
        ...
        if (!limitReturnToVarsList || this.returnVars.contains(propVarName)) {
          ...
          logger.info(
              "yes, adding now. "
                  + propVarName
                  + " is downstream of "
                  + nodeVarName); // TODO: What if nodeVarName is not part of query return... ? what
    ...
    if (this.relTriples != null) {
      for (List<String> relTriple : this.relTriples) {
        if (relTriple.size() > 1) { // if this is a full triple... rather than just a single node
          ...
          logger.info(
              "checking if need need to add node to edgeHash:::: "
                  + Utility.cleanLogString(logicalObj));
          if (!limitReturnToVarsList || this.returnVars.contains(logicalObj)) {
            ...
            logger.info(
                Utility.cleanLogString(
                    "yes, adding now. "
                        + logicalObj
                        + " is downstream of "
                        + logicalSub)); // TODO: What if subject is not part of query return... ?
    ...
    if (this.returnVars != null) {
      ...
      if (otherBuilderData.getReturnVars() != null) {
        ...
        for (String var : otherRetVars) {
          if (!thisRetVars.contains(var)) thisRetVars.add(var);
        }
      }
      //			else {
      //				// do nothing
  ...
  public boolean determineEligibleForCombining(
      QueryBuilderData nextDmcBuilderData, Map<String, Object> joinProps) {
    ...
    // the triples will be the same but first its bound on one title node and the second on
    // nominated
  ...
  public QueryStruct getQueryStruct(boolean includeFilters) {
    ...
    if (this.nodeProps != null) {
      for (Map<String, String> nodeProp : this.nodeProps) {
        ...
        logger.info(
            "checking if we need to add property to edgeHash::: "
                + Utility.cleanLogString(propVarName));
        if (!limitReturnToVarsList || this.returnVars.contains(propVarName)) {
          ...
          qs.addSelector(
              Utility.getInstanceName(nodeVarName), Utility.getInstanceName(propVarName));
        }
      }
    }
      ...
      for (List<String> relTriple : this.relTriples) {
        if (relTriple.size() > 1) { // if this is a full triple... rather than just a single node
          ...
          logger.info(
              "checking if need need to add node to edgeHash:::: "
                  + Utility.cleanLogString(logicalObj));
          if (!limitReturnToVarsList
              || this.returnVars.contains(logicalObj)
            ...
            logger.info(
                "yes, adding now. "
                    ...
                    + " is downstream of "
                    + Utility.cleanLogString(
                        logicalSub)); // TODO: What if subject is not part of query return... ? what
            // do we connect this property to?
