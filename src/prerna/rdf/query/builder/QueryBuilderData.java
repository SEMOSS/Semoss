...
public class QueryBuilderData {
	...
	// Name of node, uriKey-->Physical uri for property, varKey-->logical name of
	// property
	...
	Boolean limitReturnToVarsList = false; // If you want to only return the vars specified in the returnVars list, set
											// this to
	// true. Otherwise all node and prop vars will return. THESE ARE LOGICAL NAMES

	private final transient String QUERY_DATA_KEY = "QueryData";
	private final transient String REL_TRIPLES_KEY = "relTriples";
	private final transient String FILTERS_KEY = "filter";
		...
		if (this.nodeProps != null) {
			for (Map<String, String> nodeProp : this.nodeProps) {
				...
				if (!limitReturnToVarsList || this.returnVars.contains(propVarName)) {
					...
					logger.info("yes, adding now. " + propVarName + " is downstream of " + nodeVarName); // TODO: What
																											// if
																											// nodeVarName
																											// is not
																											// part of
																											...
																											// return...
																											// ? what
					// do we connect this property to?
					...
					if (!limitReturnToVarsList || this.returnVars.contains(logicalObj)) {
						...
						logger.info(Utility
								.cleanLogString("yes, adding now. " + logicalObj + " is downstream of " + logicalSub)); // TODO:
																														...
																														// return...
																														// ?
						// what do we connect this property to?
		...
		if (this.nodeProps != null) {
			for (Map<String, String> nodeProp : this.nodeProps) {
				...
				if (!limitReturnToVarsList || this.returnVars.contains(propVarName)) {
					...
					logger.info("yes, adding now. " + Utility.cleanLogString(propVarName) + " is downstream of "
			...
			for (List<String> relTriple : this.relTriples) {
				if (relTriple.size() > 1) { // if this is a full triple... rather than just a single node
					...
					if (!limitReturnToVarsList || this.returnVars.contains(logicalObj)
						...
						// yes it seems valid. need to get subject logical name
						logger.info("yes, adding now. " + Utility.cleanLogString(logicalObj) + " is downstream of "
								+ Utility.cleanLogString(logicalSub)); // TODO: What if subject is not part of query
																		// return... ? what
						// do we connect this property to?
