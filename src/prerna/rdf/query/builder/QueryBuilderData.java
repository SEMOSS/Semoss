...
public class QueryBuilderData {
	...
	static final Logger classLogger = LogManager.getLogger(QueryBuilderData.class);

	Map<String, String> returnMapping; // Logical name --> Physical Name
	List<List<String>> relTriples; // List of physical name triples
	List<Map<String, String>> nodeProps; // List of selected property objects. Each object contains SubjectVar-->Logical
											// Name of node, uriKey-->Physical uri for property, varKey-->logical name
											// of property
	...
	Boolean limitReturnToVarsList = false; // If you want to only return the vars specified in the returnVars list, set
											// this to true. Otherwise all node and prop vars will return. THESE ARE
											// LOGICAL NAMES
	...
	public QueryBuilderData() {
		classLogger.info("opening empty query builder data");
	}
	...
	public void setLimitReturnToVarsList(Boolean limitReturnToVarsList) {
		classLogger.info("Setting to limit return var list :: " + limitReturnToVarsList);
	...
	public void setVarReturnOrder(String var, int location) {
		classLogger.info("setting var " + var + " return order to " + location);
		if (returnVars == null) {
			returnVars = new Vector<String>();
		}
		int varIdx = returnVars.indexOf(var);
		classLogger.info(var + " is currently in position " + varIdx);
		if (varIdx != -1) {
		...
		classLogger.info(var + " is now in position " + returnVars.indexOf(var));
	}
	...
	/*
	 * This method returns how the returned results are related to one another This
	 * is needed for correctly building the results graph in TinkerFrame The
	 * structure of the return hash is returnVar -> {downstreamReturnVar,
	 * downstreamReturnVar...}
	 */
	public Map<String, Set<String>> getReturnConnectionsHash() {
		...
		if (this.nodeProps != null) {
			for (Map<String, String> nodeProp : this.nodeProps) {
				...
				classLogger.info("checking if we need to add property to edgeHash::: " + propVarName);
				if (!limitReturnToVarsList || this.returnVars.contains(propVarName)) {
					String nodeVarName = nodeProp.get("SubjectVar");
					classLogger.info("yes, adding now. " + propVarName + " is downstream of " + nodeVarName); // TODO:
																												// What
																												// if
																												// nodeVarName
																												// is
		...
		if (this.relTriples != null) {
			for (List<String> relTriple : this.relTriples) {
				if (relTriple.size() > 1) { // if this is a full triple... rather than just a single node
					...
					classLogger.info(
							"checking if need need to add node to edgeHash:::: " + Utility.cleanLogString(logicalObj));
					if (!limitReturnToVarsList || this.returnVars.contains(logicalObj)) {
						...
						classLogger.info(Utility
								.cleanLogString("yes, adding now. " + logicalObj + " is downstream of " + logicalSub)); // TODO:
																														// What
																														// if
																														// subject
																														// is
...
//			}
	...
	public boolean determineEligibleForCombining(QueryBuilderData nextDmcBuilderData, Map<String, Object> joinProps) {
		...
		// the triples will be the same but first its bound on one title node and the
		// second on nominated
		...
		for (String uri : currValues) {
			if (newValues.contains(uri)) {
				// need to make sure it is not the join column since that is allowed to be the
				// same
	...
	public QueryStruct getQueryStruct(boolean includeFilters) {
		...
		if (this.nodeProps != null) {
			for (Map<String, String> nodeProp : this.nodeProps) {
				...
				classLogger.info(
						"checking if we need to add property to edgeHash::: " + Utility.cleanLogString(propVarName));
				if (!limitReturnToVarsList || this.returnVars.contains(propVarName)) {
					String nodeVarName = nodeProp.get("equivalentURI");
					classLogger.info("yes, adding now. " + Utility.cleanLogString(propVarName) + " is downstream of "
							+ Utility.cleanLogString(nodeVarName));
			...
			for (List<String> relTriple : this.relTriples) {
				if (relTriple.size() > 1) { // if this is a full triple... rather than just a single node
					...
					classLogger.info(
							"checking if need need to add node to edgeHash:::: " + Utility.cleanLogString(logicalObj));
					if (!limitReturnToVarsList || this.returnVars.contains(logicalObj)
							|| this.returnVars.contains(logicalSub)) {
						// yes it seems valid. need to get subject logical name

						// TODO: What if subject is not part of query return... ? what do we connect
						// this property to?
						classLogger.info("yes, adding now. " + Utility.cleanLogString(logicalObj) + " is downstream of "
								+ Utility.cleanLogString(logicalSub));
