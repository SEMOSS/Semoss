...
public class SPARQLQueryParser extends AbstractQueryParser {
  ...
  public void parseQuery() {
    ...
    if (query.toLowerCase().contains("select")) {
      ...
      matcher =
          pattern.matcher(query.substring(query.indexOf("CONSTRUCT"), query.indexOf("WHERE")));
    }
    while (matcher.find()) {
  ...
  private void getURIList() {
    ...
    // run through the types first, you need to do this first so that you can get all of the types
    // their variables,
    // then when you get the properties you can use the mapping you create between variables and
    // types to properly map the variables to their respective type
    for (int patIndex = 0; patIndex < patterns.size(); patIndex++) {
      ...
      if (predicateVar.isConstant()
        ...
        types.put(
            Utility.getInstanceName(objectVar.getValue().toString()) + "",
            objectVar.getValue()
                + ""); // types.put(subjectVar.getName() + "", objectVar.getValue() + "");
        aliasTableMap.put(
            subjectVar.getName(), Utility.getInstanceName(objectVar.getValue().toString()));
      }
    }

    // now that you have their types and their aliases, run through the properties and generate the
    // triples list
    for (int patIndex = 0; patIndex < patterns.size(); patIndex++) {
      StatementPattern thisPattern = patterns.get(patIndex);
      ...
      if (predicateVar.isConstant()
          && (predicateVar.getValue() + "").contains("ontologies/Relation/Contains/")) {
        ...
        addToVariablesMap(
            typePropVariables, nodeType, objectVar.getName(), predicateVar.getValue().toString());
      ...
      } else if (predicateVar.isConstant()
          && (predicateVar.getValue() + "").contains("ontologies/Relation")) {
        // must a triple!
  ...
  private static void basicParseTest() {
    ...
    // &lt;http://semoss.org/ontologies/Relation/Contains/MovieBudget&gt; ?Title__MovieBudget}  }";
