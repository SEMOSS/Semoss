...
public class SPARQLQueryParser extends AbstractQueryParser {
	...
	private Map<String, Double> finalHash;
	private Map<String, Integer> countHash;
	...
	public void parseQuery() {
		...
		try {
			...
			classLogger.error("Failed to parse SPARQL query and build URI list", e);
		}

		// calculate weights for all return variables
		finalHash = new HashMap<>();
		int max = 0;
	...
	private List<String> getKeyFromVal(String val, Map<String, String> map) {
	...
	private void getURIList() {
		...
		Map<String, Integer> dataHash = new HashMap<String, Integer>();
		types = new HashMap<String, String>();
		props = new HashMap<String, String>();

		// run through the types first, you need to do this first so that you can get
		// all of the types their variables,
		// then when you get the properties you can use the mapping you create between
		// variables and types to properly map the variables to their respective type
		for (int patIndex = 0; patIndex < patterns.size(); patIndex++) {
			...
			if (predicateVar.isConstant() && (predicateVar.getValue() + "")
					.equalsIgnoreCase("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")) {
				types.put(Utility.getInstanceName(objectVar.getValue().toString()) + "", objectVar.getValue() + "");
				aliasTableMap.put(subjectVar.getName(), Utility.getInstanceName(objectVar.getValue().toString()));
			}
		}

		// now that you have their types and their aliases, run through the properties
		// and generate the triples list
		for (int patIndex = 0; patIndex < patterns.size(); patIndex++) {
			StatementPattern thisPattern = patterns.get(patIndex);
			...
			if (predicateVar.isConstant() && (predicateVar.getValue() + "").contains("ontologies/Relation/Contains/")) {
				...
				props.put(propPlainText + "", predicateVar.getValue() + "");// props.put(objectVar.getName() + "",
																			// predicateVar.getValue() + "");
				...
				if (returnVariables.contains(objectVar.getName())) {
					addToVariablesMap(typeReturnVariables, nodeType, objectVar.getName(),
							predicateVar.getValue().toString());
				}
	...
	private Map<String, Integer> recordVar(Var var, Map<String, Integer> inputHash) {
		if (var.hasValue()) {
...
//	}
	...
	private static void basicParseTest() {
		...
		// query = "SELECT DISTINCT ?Title ?Nominated ?Genre
		// ?Title__RevenueInternational ?Title__MovieBudget WHERE { {?Title
		// &lt;http://www.w3.org/1999/02/22-rdf-syntax-ns#type&gt;
		// &lt;http://semoss.org/ontologies/Concept/Title&gt;} {?Nominated
		// &lt;http://www.w3.org/1999/02/22-rdf-syntax-ns#type&gt;
		// &lt;http://semoss.org/ontologies/Concept/Nominated&gt;} {?Genre
		// &lt;http://www.w3.org/1999/02/22-rdf-syntax-ns#type&gt;
		// &lt;http://semoss.org/ontologies/Concept/Genre&gt;} {?Title
		// &lt;http://semoss.org/ontologies/Relation/Was&gt; ?Nominated} {?Title
		// &lt;http://semoss.org/ontologies/Relation/BelongsTo&gt; ?Genre} {?Title
		// &lt;http://semoss.org/ontologies/Relation/Contains/Revenue-International&gt;
		// ?Title__RevenueInternational} {?Title
		// &lt;http://semoss.org/ontologies/Relation/Contains/MovieBudget&gt;
		// ?Title__MovieBudget} }";
		// query = "SELECT DISTINCT ?Title ?Title__RevenueDomestic WHERE { {?Title
		// &lt;http://www.w3.org/1999/02/22-rdf-syntax-ns#type&gt;
		// &lt;http://semoss.org/ontologies/Concept/Title&gt;} {?Studio
		// &lt;http://www.w3.org/1999/02/22-rdf-syntax-ns#type&gt;
		// &lt;http://semoss.org/ontologies/Concept/Studio&gt;} {?Title
		// &lt;http://semoss.org/ontologies/Relation/Title_Studio&gt; ?Studio} {?Title
		// &lt;http://semoss.org/ontologies/Relation/Contains/Revenue-Domestic&gt;
		// ?Title__RevenueDomestic} {?Title
		// &lt;http://semoss.org/ontologies/Relation/Contains/Revenue-International&gt;
		// ?Title__RevenueInternational} }";
		query = query.replace("&lt;", "<");
		query = query.replace("&gt;", ">");
		...
		Map<String, Map<String, String>> returnVariables1 = parse.getReturnVariables();
		Map<String, String> types1 = parse.getNodesFromQuery();
		Map<String, Map<String, String>> props1 = parse.getPropertiesFromQuery();
		List<String[]> mytrips = parse.getTriplesData();
		boolean hasAggregate = parse.hasAggregateFunction();
		classLogger.info("Aggregate function {}", hasAggregate);
	}
}