...
public class SQLQueryParser extends AbstractQueryParser {
  ...
  public void parseQuery() {
    ...
    try {
      ...
      // only support parsing select at this time, we don't need update and insert parsing in the
      // forseeable future
      if (statement instanceof Select) {
        ...
        parseAllPropertiesAndVarsFromQuery(
            statement); // generate the return variables list and properties list as well other part
        // of the joins

      } else {
        System.err.println(
            "An error occurred, the sql statement you are trying to parse is not parseable "
                + query);
      }
  ...
  /**
   * Get the tables and their aliases from the select statement generate a partial list of joins
   * (these would be the explicit inner/outer joins)
   *
   * @param statement
   ...
   * that comes in and determine the table name linked to that alias, then store the name of the
   * table and the join column as a map within the columnMappings. This way we can figure out the
   * triples
   *
   * <p>T.Title = N.Title_FK Figure out the table name for T - Title Figure out the table name for N
   * - Nominated
   *
   * <p>leftColumnMap will be a map of Title Title
   *
   * @param joinColumnsMap
   */
  private void processAllTableJoins(HashMap<Column, Column> joinColumnsMap) {
    String joinColumnTable =
        ""; // may be the table name or alias, will use this as if its an alias to look up the table
    // name
    String joinColumnTableName = "";
    String joinColumnName = "";

    // now I have my tables and their aliases, I can be sure I am grabbing the right table name from
    // the join, and build the join/triple properly
  ...
  private void setTableAndAlias(Table addTable) {
    ...
    String tableAliasText =
        tableName; // so if there is no table alias, use the table name by default
    if (tableAlias != null) {
      tableAliasText = tableAlias.getName();
      aliasTableMap.put(
          tableAliasText,
          tableName); // the alias is the key because you can join the same table several times in a
      // query
    }
    ...
    if (plainSelectList != null && plainSelectList.size() > 0) {
      for (PlainSelect ps : plainSelectList) {
        ...
        for (int i = 0; i < selectList.size(); i++) {
          ...
          if (alias != null) {
            expressionAlias =
                alias.getName(); // heres the alias for the select clause expression you are working
            // with, unused at the moment
          } else {
          ...
          // expression returned MAY contain the table name/table alias prior to the expression
          // value.  Keeping this for now

          // to do, actually save it as expression, table alias, so you can look up the table if you
          // need to
  ...
  /**
   * generate a partial list of joins (these would be the implicit inner/outer joins) also maintain
   * the where clause variables (unused at this time)
   *
   * @param props
   * @param exp
   */
  private void getIndividualWhereClauseValues(Hashtable<String, String> props, Expression exp) {
    ...
    while (individualExpressions.getLeftExpression() != null) {
      ...
      if ((leftExpression != null && rightExpression != null)
        ...
        // if both left and right expressions are columns, special logic! You will add these to the
        // joins map,
        ...
        if (rightExpression != null) {
          ...
          // use individualExpressions.getRightExpression().getRightExpression, then check cast as
          // necessary to get values.
  ...
  private static void basicParseTest() {
    ...
    String sql =
        "SELECT 1 as r,2,a,b FROM MY_TABLE1 MT, yourtable YT, andanothertable AAT where YT.x=1 and YT.r = MT.F and MT.T is not null";

    String subquery =
        "select x.col1, y.col2 from (select col1, col3 from newtable) x, anothertable y";

    String anotherAdvSql =
    ...
    String anotherJoin =
        "select n.nominated, t.title from title t, nominated n where n.title_fk = t.title";
    ...
    for (String key : propsVariables.keySet()) {
      ...
      Hashtable<String, String> variablesInTable =
          (Hashtable<String, String>) propsVariables.get(key);
    ...
    for (String key : returnVariables.keySet()) {
      ...
      Hashtable<String, String> variablesInTable =
          (Hashtable<String, String>) returnVariables.get(key);
      for (String singleVariable : variablesInTable.keySet()) {
        System.out.println(
            "returns table : "
                + key
