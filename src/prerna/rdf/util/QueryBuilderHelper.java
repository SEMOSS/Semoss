...
public class QueryBuilderHelper {
	...
	// this the primary class that will be turned into a REST service for query
	// builder
	...
	// this is impending on selecting a particular instance to get to those
	// properties
	// Get all instances for a given relation
	// this is again impending on selecting a instance of the class

	// The type is always shown as tree node with the instance of this type right
	// below it
	...
	// When a class type node is selected it creates a triple <outputclass> <typeof>
	// <selectednode>
	// When a property on the class type node it adds <outputclass> <propertyname>
	// <outputproperty>
	// When an instance is selected it adds <bind> <outputclass> <instancename>
	// When a relationship is selected <outputclass> <selectedrelation>
	// <outputclass2>,
	...
	public String[][] getClasses() {
		...
		// the array is 2 dimensional because it will have the concatenated class name
		// and the value
	...
	public String[][] getRelationInstance(String fromType, String fromInstanceName, String relationName,
			String toType) {
	...
	public void testQueryGen() {
		...
		try {
			...
			Var typeRelation = new Var("y", vf.createURI("http://www.w3.org/1999/02/22-rdf-syntax-ns#type")); // ,
																												// factory.createURI(...));
			// URI y1 = vf
			...
			// gp.addConstraint(new Compare(p, new
			// ValueConstant(vf.createURI("http://semoss.org/ontologies/Concept/System"))));
			// gp.addConstraint(new Compare(w, new
			// ValueConstant(vf.createURI("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"))));
			...
			/*
			 * gp.addRequiredTE(new Projection(gp.buildTupleExpr(), new
			 * ProjectionElemList(new ProjectionElem("x") ,new ProjectionElem("t"))));
			 */
			// System.out.println(gp.buildTupleExpr());
			//

			TupleExpr query2 = new Projection(gp.buildTupleExpr(),
					new ProjectionElemList(new ProjectionElem("x"), new ProjectionElem("t")
			...
			SailTupleQuery q = new MyTupleQuery(query3, (SailRepositoryConnection) rc);
