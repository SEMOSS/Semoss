...
public class SEMOSSVertex {
  ...
  public transient Hashtable<String, SEMOSSVertex> edgeHash = new Hashtable<String, SEMOSSVertex>();
  ...
  public SEMOSSVertex(String uri) {
    ...
    if (instanceName == null) instanceName = uri;
    if (className == null) className = instanceName;
  ...
  public void resetColor() {
    this.setColor(
        TypeColorShapeTable.getInstance()
            .getColor(
                this.getProperty(Constants.VERTEX_TYPE) + "",
                this.getProperty(Constants.VERTEX_NAME) + ""));
  }
  ...
  public Color getColor() {
    if (!this.propHash.containsKey(Constants.VERTEX_COLOR)) resetColor();
  ...
  public void addVertexCounter(SEMOSSVertex outVert) {
    ...
    try {
      if (propHash.containsKey(vertType)) vertTypeCount = (Integer) propHash.get(vertType);
      vertTypeCount++;
  ...
  public void setProperty(String propNameURI, Object propValue) {
    ...
    for (int tokIndex = 0; tokIndex <= totalTok && tokens.hasMoreElements(); tokIndex++) {
      if (tokIndex + 2 == totalTok) className = tokens.nextToken();
      else if (tokIndex + 1 == totalTok) instanceName = tokens.nextToken();
  ...
  public void createTree() {
    ...
    if (navHash == null) navHash = new Hashtable<String, Hashtable>();
  ...
  public void loadEdge(SEMOSSEdge edge) {
    ...
    if (navHash.containsKey(edgeType)) vertHash = navHash.get(edgeType);
    ...
    // System.err.println("From " + getProperty(Constants.VERTEX_NAME) + " Adding Vertex " +
    // vertexName + "To Hash " + edgeType);
