...
public class OldInsight extends Insight {
  ...
  @Deprecated private String engineId;
  @Deprecated private String engineName;
  ...
  private transient IDatabaseEngine
      makeupEngine; // the in-memory engine created to store the data maker components and
  // transformations for the insight
  private transient IPlaySheet playSheet; // the playsheet for the insight
  private transient Map<String, String>
      dataTableAlign; // the data table align for the insight corresponding to the playsheet
  private transient Gson gson = new Gson();

  private transient Boolean append =
      false; // currently used to distinguish when performing overlay in gdm data maker

  private transient String dataMakerName; // the name of the data maker
  private transient List<DataMakerComponent>
      dmComponents; // the list of data maker components in order for creation of insight
  private transient Map<String, List<Object>>
      paramHash; // the parameters selected by user for filtering on insights
  ...
  /**
   * Constructor for the insight REQUIRES PROJECT AND APP TO HAVE SAME ID
   *
  ...
  /**
   * Append the parameter filters to the correct component Each parameter stores the component and
   * filter number that it is affecting
   */
  public void appendParamsToDataMakerComponents() {
    if (insightParameters != null) {
      for (int i = 0; i < insightParameters.size(); i++) {
        ...
        // logic inside setParamHash in each component to append this information to a filter
        // pre-transformation
  ...
  /**
   * Process the make-up engine storing the -Triples information to get the data maker components
   * and transformations
   *
   * @param makeupEng
   * @return
  ...
  public List<DataMakerComponent> digestNTriples(IDatabaseEngine makeupEng) {
    ...
    String countQuery =
        "SELECT (COUNT(DISTINCT(?Component)) AS ?Count) WHERE {?Component a <http://semoss.org/ontologies/Concept/Component>. BIND('x' AS ?x) } GROUP BY ?x";
    ...
    String theQuery =
        "SELECT ?Component ?Engine ?Query ?Metamodel ?DataMakerType ?PreTrans ?PostTrans ?Actions WHERE { {?Component a <http://semoss.org/ontologies/Concept/Component>} {?EngineURI a <http://semoss.org/ontologies/Concept/Engine>} {?EngineURI <http://semoss.org/ontologies/Relation/Contains/Name> ?Engine } {?Component <Comp:Eng> ?EngineURI} OPTIONAL {?Component <http://semoss.org/ontologies/Relation/Contains/Query> ?Query} OPTIONAL {?Component <http://semoss.org/ontologies/Relation/Contains/Metamodel> ?Metamodel} {?Component <http://semoss.org/ontologies/Relation/Contains/Order> ?Order} OPTIONAL {?Component <Comp:PreTrans> ?PreTrans} OPTIONAL {?Component <Comp:PostTrans> ?PostTrans} OPTIONAL {?Component <Comp:Action> ?Actions} } ORDER BY ?Order";

    int idx = -1;
    ISelectWrapper ss = WrapperManager.getInstance().getSWrapper(makeupEng, theQuery);

    String curComponent = null;
    // Keeping a set of pre/post/action's that exist on each component such that they are not added
    // twice when number of pre/post/actions is not the same
    ...
    while (ss.hasNext()) {
      ...
      if (!newComponent.equals(curComponent)) {
        ...
        // old insights store information in a query string while new insights store the metamodel
        // information to construct the query
        if (!query.isEmpty()) {
          ...
          if (metamodelData.getRelTriples() == null) {
            ...
            if (qsData.getSelectors() == null) {
              logger.info(
                  "failed to get QueryBuilderData.... this must be a legacy insight with metamodel data. Setting metamodel data into QueryBuilderData");
              Hashtable<String, Object> dataHash =
                  gson.fromJson(
                      metamodelString, new TypeToken<Hashtable<String, Object>>() {}.getType());
   ...
   */
  private void addPreTrans(
      DataMakerComponent dmc, IDatabaseEngine makeupEng, Object preTrans, String compId) {
   ...
   */
  private void addPostTrans(
      DataMakerComponent dmc, IDatabaseEngine makeupEng, Object postTrans, String compId) {
  ...
  private void addAction(
      DataMakerComponent dmc, IDatabaseEngine makeupEng, Object action, String compId) {
  ...
  private Map<String, Object> getProperties(String uri, IDatabaseEngine makeupEng) {
    ...
    if (wrap
        .hasNext()) { // there should only be one prop map associated with each transformation or
      // action
    ...
    if (wrap.hasNext()) {
      logger.error(
          "More than one prop map has shown up for uri ::::: " + Utility.cleanLogString(uri));
  ...
  /**
   * Get the data maker object from the dataMakerString If cannot find specified dataMaker, get the
   * playSheet and see if is supposed to be the data maker
   *
   * @return
   */
  @Override
  ...
  /**
   * Getter for the DataMakerComponents To prevent unnecessary creation of the dmComponents list, we
   * call digestNTriples if it is null
   *
   * @return
   */
  ...
  /**
   * Setter for the data table align used for the view Primary used when no data table align exists
   * and need to get it based on the playsheet
   *
  ...
  /**
   * Setter for the data table align used for the view Primarily used when data table align is
   * coming from rdbms insight
   *
  ...
  public IPlaySheet getPlaySheet() {
    if (this.playSheet == null) {
      ...
      if (playSheet != null) {
        ...
        logger.error(
            "Broken insight... cannot get playsheet :: " + Utility.cleanLogString(this.layout));
      }
    }
   ...
   */
  public void processPostTransformation(
      List<ISEMOSSTransformation> postTrans, IDataMaker... dataMaker) throws RuntimeException {
  ...
  //	 */
  //	public List<Object> processActions(List<ISEMOSSAction> actions, IDataMaker... dataMaker) throws
  // RuntimeException {
  ...
  /**
   * Undo a set of actions Actions create a new data structure to hold information and do not affect
   * the current data maker Thus, only need to remove them from the list of actions stored on the
   * component
   *
   ...
   * @return true if a JoinTransformation was removed, false otherwise. Used to signal whether the
   *     component still should be kept or not
   */
  ...
  public Map<String, Object> getWebData() {
    ...
    if (dataTableAlign
        != null) { // some playsheets don't require data table align, like grid play sheet. Should
      // probably change this so they all have data table align (like if i want to
    ...
    // TODO: how do i get this outside of here? we need to return incremental stores during
    // traversing but not when recreating insight
    ...
    // if has a do-method, dataMaker is the playsheet and can use those methods but can also use a
    // datamaker
    ...
    //		} else
    if (this.layout.equals("Graph")
        || this.layout.equals("VivaGraph")) { // TODO: Remove hardcoded layout values
  ...
  public int getNumComponents() {
    ...
    if (this.dmComponents != null) {
      ...
      String countQuery =
          "SELECT (COUNT(DISTINCT(?Component)) AS ?Count) WHERE {?Component a <http://semoss.org/ontologies/Concept/Component>. BIND('x' AS ?x) } GROUP BY ?x";
      ISelectWrapper countss =
          WrapperManager.getInstance().getSWrapper(this.makeupEngine, countQuery);
      while (countss.hasNext()) {
        ISelectStatement countst = countss.next();
  ...
  public String getNTriples() {
    ...
    ISelectWrapper wrapper =
        WrapperManager.getInstance()
            .getSWrapper(this.makeupEngine, "SELECT ?S ?P ?O WHERE {?S ?P ?O}");
    ...
    while (wrapper.hasNext()) {
      ...
      if (ss.getRawVar(names[2]) instanceof Literal) {
        ...
        returnStrBuilder.append(
            "<" + ss.getRawVar(names[0]) + "> <" + ss.getRawVar(names[1]) + "> " + val + " .\n");
      } else {
        returnStrBuilder.append(
            "<"
                + ss.getRawVar(names[0])
                + "> <"
                + ss.getRawVar(names[1])
                + "> <"
                + ss.getRawVar(names[2])
                + "> .\n");
      }
    }
  ...
  public void recalcDerivedColumns() {
    ...
    for (; firstComp < dmcList.size() && firstTransId == null; firstComp++) {
      ...
      for (ISEMOSSTransformation trans : dmc.getPostTrans()) {
        if (!(trans instanceof FilterTransformation)
            && !(trans instanceof JoinTransformation)
            && !(trans instanceof PKQLTransformation)) {
    ...
    // now that i've found where my first derived column is, i need to undo everything that comes
    // after that in correct order
    ...
    for (int dmcIdx = dmcList.size() - 1; dmcIdx >= firstComp && !hitFirst; dmcIdx--) {
      ...
      for (int transIdx = postList.size() - 1; transIdx >= 0 && !hitFirst; transIdx--) {
        ...
        if (!(trans instanceof JoinTransformation)
            && !(trans instanceof FilterTransformation)
            && !(trans instanceof PKQLTransformation)) {
