...
public class OldInsight extends Insight {
	...
	/*
	 * THIS IS FOR LEGACY INSIGHTS IF YOU NEED TO USE THIS CLASS... YOU ARE PROBABLY
	 ...
	 *
	 * @param mainEngine
	 *            The main engine which holds the insight
	 * @param dataMakerName
	 *            The name of the data maker
	 * @param layout
	 *            The layout to view the insight
	 */
	 ...
	 *
	 * @param mainProject
	 *            The main project which holds the insight
	 * @param dataMakerName
	 *            The name of the data maker
	 * @param layout
	 *            The layout to view the insight
	...
	/**
	 * Exposed another constructor for creating insights to be stored in some MHS
	 * custom playsheets
	 *
	 * @param playsheet
	 */
	...
	/**
	 * Takes the input stream for the N-Triples string to create the insight makeup
	 * database
	 *
	 * @param insightMakeup
	 */
	...
	/**
	 * Append the parameter filters to the correct component Each parameter stores
	 * the component and filter number that it is affecting
	 */
	public void appendParamsToDataMakerComponents() {
		if (insightParameters != null) {
			for (int i = 0; i < insightParameters.size(); i++) {
				...
				// logic inside setParamHash in each component to append this information to a
				// filter
				// pre-transformation
	...
	/**
	 * Generates an in-memory database based on the N-Triples makeup input stream
	 * for the insight
	 *
	 * @param nTriples
	 *            The inputstream holding the N-Triples string
	 * @return
	 */
	...
	/**
	 * Process the make-up engine storing the -Triples information to get the data
	 * maker components and transformations
	 *
	 * @param makeupEng
	 * @return
	 */
	public List<DataMakerComponent> digestNTriples(IDatabaseEngine makeupEng) {
		...
		// Keeping a set of pre/post/action's that exist on each component such that
		// they are not added
		...
		while (ss.hasNext()) {
				...
				if (!query.isEmpty()) {
					...
					if (metamodelData.getRelTriples() == null) {
						...
						if (qsData.getSelectors() == null) {
							...
							Hashtable<String, Object> dataHash = gson.fromJson(metamodelString,
									new TypeToken<Hashtable<String, Object>>() {
									}.getType());
			...
			// run queries to get information on each transformation/action and append to
			// dmc list
	 ...
	 *
	 * @param dmc
	 *            The DataMakerComponent to add the preTransformation
	 * @param makeupEng
	 *            The makeupEngine containing the N-Triples information regarding
	 *            the insight
	 * @param preTrans
	 *            The preTransformation URI
	 * @param compId
	 *            The name of the component to make sure the preTransforamtion has
	 *            the correct id
	 */
	 ...
	 *            the insight
	 * @param postTrans
	 *            The postTransformation URI
	 * @param compId
	 *            The name of the component to make sure the postTransformation has
	 *            the correct id
	 */
	 ...
	 *            The Action URI
	 * @param compId
	 *            The name of the component to make sure the Action has the correct
	 *            id
	 */
	 ...
	 *
	 * @param uri
	 *            The URI for the transformation or action
	 * @param makeupEng
	 *            The makeupEngine containing the N-Triples information regarding
	 *            the insight
	 * @return
	...
	/**
	 * Get the data maker object from the dataMakerString If cannot find specified
	 * dataMaker, get the playSheet and see if is supposed to be the data maker
	 *
	 * @return
	 */
	@Override
	...
	/**
	 * Getter for the DataMakerComponents To prevent unnecessary creation of the
	 * dmComponents list, we call digestNTriples if it is null
	 *
	 * @return
	 */
	...
	/**
	 * Setter for the data table align used for the view Primary used when no data
	 * table align exists and need to get it based on the playsheet
	 *
	...
	/**
	 * Setter for the data table align used for the view Primarily used when data
	 * table align is coming from rdbms insight
	 *
	...
	/**
	 * Process a list of post transformation on the last data maker component stored
	 * in cmComponents
	 *
	 * @param postTrans
	 *            The list of post transformation to run
	 * @param dataMaker
	 *            Additional dataMakers if required by the transformation
	 */
	...
	// /**
	// * Process a list of actions on the last data maker component stored in
	// cmComponents
	// * @param postTrans The list of actions to run
	// * @param dataMaker Additional dataMakers if required by the actions
	// */
	// public List<Object> processActions(List<ISEMOSSAction> actions, IDataMaker...
	// dataMaker) throws
	// RuntimeException {
	// DataMakerComponent dmc =
	// getDataMakerComponents().get(this.dmComponents.size() - 1);
	//
	...
	// }
	//
	// List<Object> actionResults = getDataMaker().processActions(dmc, actionsCopy,
	// dataMaker);
	...
	// getDataMakerComponents().get(this.dmComponents.size() -
	// 1).addAction(actions.get(i));
	// }
	//
	...
	public void undoProcesses(List<String> processes) {
		...
		for (int i = dmComponents.size() - 1; i >= 0; i--) {
			...
			if (joinUndone) {
				// assumption that when no transformations exist, to remove the component from
				// the list
	...
	/**
	 * Undo a set of actions Actions create a new data structure to hold information
	 * and do not affect the current data maker Thus, only need to remove them from
	 * the list of actions stored on the component
	 *
	 * @param actions
	 *            The list of actions on a specific data maker component
	 * @param processes
	 *            The master list of processes to udno
	 */
	 ...
	 *
	 * @param actions
	 *            The list of transformations on a specific data maker component
	 * @param processes
	 *            The master list of processes to undo
	 * @return true if a JoinTransformation was removed, false otherwise. Used to
	...
	private boolean undoTransformations(List<ISEMOSSTransformation> trans, List<String> processes) {
		...
		// remove from largest to smallest as it is most efficient way to remove from
		// BTreeDataFrame
	...
	public Map<String, Object> getWebData() {
		...
		if (dataTableAlign != null) { // some playsheets don't require data table align, like grid play sheet. Should
		...
		// TODO: how do i get this outside of here? we need to return incremental stores
		// during
		...
		// if has a do-method, dataMaker is the playsheet and can use those methods but
		// can also use a
		// datamaker
		...
		if (this.layout.equals("Graph") || this.layout.equals("VivaGraph")) { // TODO: Remove hardcoded layout values
			...
			// TinkerFrame tframe =
			// TableDataFrameFactory.convertToTinkerFrameForGraph((H2Frame)dm);
	...
	public String getNTriples() {
		...
		ISelectWrapper wrapper = WrapperManager.getInstance().getSWrapper(this.makeupEngine,
		...
		while (wrapper.hasNext()) {
			...
			if (ss.getRawVar(names[2]) instanceof Literal) {
				...
				returnStrBuilder
						.append("<" + ss.getRawVar(names[0]) + "> <" + ss.getRawVar(names[1]) + "> " + val + " .\n");
			} else {
				returnStrBuilder.append("<" + ss.getRawVar(names[0]) + "> <" + ss.getRawVar(names[1]) + "> <"
	...
	public void recalcDerivedColumns() {
		...
		// now that i've found where my first derived column is, i need to undo
