...
public class OldInsight extends Insight {
	...
	private static final Logger classLogger = LogManager.getLogger(OldInsight.class);
	...
	public void setMakeup(InputStream insightMakeup) {
		if (insightMakeup != null) {
			...
			classLogger.error("Invalid insight. No insight makeup available");
		}
	}
	...
	public List<DataMakerComponent> digestNTriples(IDatabaseEngine makeupEng) {
		classLogger.debug("Creating data component array from makeup engine");
		...
		// they are not added twice when number of pre/post/actions is not the same
		...
		while (ss.hasNext()) {
			...
			if (!newComponent.equals(curComponent)) {
				...
				// metamodel information to construct the query
				if (!query.isEmpty()) {
					dmc = new DataMakerComponent(engine, query);
					dmCompVec.add(dmc);
				} else if (!metamodelString.isEmpty()) {
					classLogger.info("trying to get QueryBuilderData object");
	 ...
	 * @param makeupEng The makeupEngine containing the N-Triples information
	 *                  regarding the insight
	 * @param preTrans  The preTransformation URI
	 * @param compId    The name of the component to make sure the preTransforamtion
	 *                  has the correct id
	 */
	 ...
	 * @param compId    The name of the component to make sure the
	 ...
	 * @param compId    The name of the component to make sure the Action has the
	 *                  correct id
	 */
	...
	private Map<String, Object> getProperties(String uri, IDatabaseEngine makeupEng) {
		...
		if (wrap.hasNext()) { // there should only be one prop map associated with each transformation or
			...
			classLogger.info(Utility.cleanLogString(jsonPropMap));
...
//	}
	 ...
	 * 
	 * @param actions   The list of transformations on a specific data maker
	 *                  component
...
//			}
		...
		// during traversing but not when recreating insight
...
//			}
	...
	public void recalcDerivedColumns() {
		...
		// everything that comes after that in correct order
