...
public class GraphDataModel implements IDataMaker {
  ...
  boolean subclassCreate =
      false; // this is used for our metamodel graphs. Need to be modify the base graph base and
  ...
  // these are used for keeping track of only what was added or subtracted and will only be
  ...
  public void fillStoresFromModel(IDatabaseEngine engine) {
    if (rc != null) {
      if (containsRelation == null) containsRelation = findContainsRelation();
      ...
      if (isRDF) {
        baseConceptSelectQuery =
            "SELECT DISTINCT ?Subject ?Predicate ?Object WHERE {"
                ...
                "BIND(\"\" AS ?Predicate)"
                + // these are only used so that I can use select cheater...
                "BIND(\"\" AS ?Object)"
                + "}";
  ...
  public void processData(Iterator<IConstructStatement> sjw, IDatabaseEngine engine) {
    ...
    try {
        ...
        if (subjects.indexOf("(<" + st.getSubject() + ">)") < 0) {
          if (engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.SESAME
              ...
              || engine.getDatabaseType()
                  == IDatabaseEngine.DATABASE_TYPE
                      .RDBMS) // RDBMS because the the in memory is RDBMS
          subjects.append("(<").append(st.getSubject()).append(">)");
          else if (engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.JENA)
            subjects.append("<").append(st.getSubject()).append(">");
          // do the block for RDBMS - For RDBMS - What I really need are the instances - actually I
          // dont need anything
        }
      ...
      if (rc != null) { // if rc is null here, that means our original query didn't return anything.
        ...
        loadBaseData(
            engine); // this method will work fine too.. although I have no use for it for later
        ...
        boolean isRDF =
            (engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.SESAME
                || engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.JENA
                || engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.SEMOSS_SESAME_REMOTE);
        boolean loadHierarchy =
            !(subjects.length() == 0 && predicates.length() == 0 && objects.length() == 0)
                && isRDF; // Load Hierarchy if and only if this is a RDF Engine - else dont worry
        // about it
        if (loadHierarchy) {
          try {
            logger.info("starting with processing hierarchy");
            RDFEngineHelper.loadConceptHierarchy(
                engine, subjects.toString(), objects.toString(), this);
  ...
  public void processData(String query, IDatabaseEngine engine) {
    this.coreEngine =
        engine; // right now we need this for logical names... really shouldn't be needed in this
    // class at all
    ...
    else sjw = WrapperManager.getInstance().getChWrapper(engine, query);
  ...
  public void addToSesame(IConstructStatement st, boolean overrideURI, boolean add2Base) {
    try {
      ...
      if ((overrideURI || obj instanceof URI || obj.toString().startsWith("http://"))
        ...
        if (obj instanceof org.openrdf.model.Resource) object = (org.openrdf.model.Resource) obj;
        else object = new URIImpl(st.getObject() + "");
        ...
        if (add2Base) {
          baseRelEngine.addStatement(
              new Object[] {st.getSubject(), st.getPredicate(), st.getObject(), true});
        }
      ...
      else if (obj instanceof Literal) // all the sesame routine goes here
      {
        ...
        if (add2Base) {
          baseRelEngine.addStatement(
              new Object[] {st.getSubject(), st.getPredicate(), st.getObject(), false});
        }
        rc.add(subject, predicate, (Literal) obj);
      } else if (obj instanceof org.apache.jena.rdf.model.Literal
          || !obj.toString().startsWith("http://")) {
        ...
        System.err.println(
            "Adding to sesame "
                + subject
                + predicate
                + rc.getValueFactory().createLiteral(obj + ""));
  ...
  public void addNodeProperty(String subject, Object object, String predicate) {
    ...
    String propName =
        this.getDisplayName(
            predicate); // Utility.getInstanceName(this.getDisplayName(subjectInstance +"%"+
    // predicate));
    if (vert1.getProperty(propName) == null) {
      if (this.subclassCreate
          && object instanceof LiteralImpl
          && ((LiteralImpl) object).getLabel().isEmpty()) {
   ...
   * @param predicate String
   */
  public void addEdgeProperty(
      String edgeName, Object value, String propName, String outNode, String inNode) {
  ...
  public String findContainsRelation() {
    String query2 =
        "SELECT DISTINCT ?Subject ?subProp ?contains WHERE { BIND( <http://www.w3.org/2000/01/rdf-schema#subPropertyOf> AS ?subProp) BIND( <http://semoss.org/ontologies/Relation/Contains> AS ?contains) {?Subject ?subProp  ?contains}}";
    ...
    else sjsc = WrapperManager.getInstance().getChWrapper(jenaEngine, query2);
  ...
  public void genBaseConcepts(String conceptSelectQuery) throws Exception {
    ...
    try {
      ...
      while (sjsc.hasNext()) {
        ...
        if (!baseFilterHash.containsKey(
            subject)) // && !baseFilterHash.containsKey(sct.getPredicate()) &&
  ...
  public void genBaseGraph(String predicateSelectQuery) throws Exception {
    ...
    try {
      ...
      while (sjsc.hasNext()) {
        ...
        if (!baseFilterHash.containsKey(subjectName)
            && !baseFilterHash.containsKey(predicateName)
          ...
          {
            if (sct.getObject() instanceof URI) vert2 = new SEMOSSVertex(objectName);
          ...
          if (!predicateName.contains(
              vert1.getProperty(Constants.VERTEX_NAME)
                  + ":"
          ...
          if (edge == null) edge = edgeStore.get(predicateName);
          if (edge == null) {
  ...
  public void setOverlay(boolean overlay) {
    if (overlay) this.method = CREATION_METHOD.OVERLAY;
    else this.method = CREATION_METHOD.CREATE_NEW;
  }

  public void setUndo(boolean undo) {
    if (undo) this.method = CREATION_METHOD.UNDO;
    else this.method = CREATION_METHOD.CREATE_NEW;
  }
  ...
  public void processTraverseCourse() {
    // if you're at a spot where you have forward models, extensions will reset the future, thus we
  ...
  public void loadBaseData(IDatabaseEngine engine) {
    ...
    if (!loadedOWLS.containsKey(engine.getEngineId()) && engine instanceof AbstractDatabaseEngine) {
      if (this.baseRelEngine == null) {
        ...
        RDFEngineHelper.addAllData(
            ((AbstractDatabaseEngine) engine).getBaseDataEngine(), this.baseRelEngine.getRc());
      }
  ...
  public void processDataMakerComponent(DataMakerComponent component) {
    ...
    // empty incremental stores indicate no new data is available, which means the traversal is
    // invalid. this will set the traversal back one step and remove the invalid traversal from rc
    // and model stores.
    if (getIncrementalVertStore() != null
        && getIncrementalEdgeStore() != null
        && getIncrementalVertStore().isEmpty()
        && getIncrementalEdgeStore().isEmpty()
        && getIncrementalVertPropStore() != null
        && getIncrementalVertPropStore().isEmpty()) {
      setUndo(true);
      undoData();
  ...
  @Override
  public void processPostTransformations(
  ...
  @Override
  public void processPreTransformations(
      DataMakerComponent dmc, List<ISEMOSSTransformation> transforms) {
    for (ISEMOSSTransformation transform : transforms) {
      transform.setDataMakers(this);
      transform.setDataMakerComponent(dmc);
      transform.runMethod();
  ...
  public Map<String, String> getScriptReactors() {
    ...
    //		reactorNames.put(PKQLEnum.COL_CSV, "prerna.sablecc.ColCsvReactor"); // it almost feels like
