...
public class GraphDataModel implements IDataMaker {
	...
	boolean subclassCreate = false; // this is used for our metamodel graphs. Need to be modify the base graph base
									// and
	// base concepts queries to use subclass of concept rather than type
	...
	// will only be
	// populated when overlay is true
		...
		if (rc != null) {
			...
			if (isRDF) {
				...
				// <http://semoss.org/ontologies/Relation>;}" +
						...
						"BIND(\"\" AS ?Object)" + "}";
				if (subclassCreate) // this is used for our metamodel graphs. Need to be subclass of concept
					// rather than type
	...
	public void processData(Iterator<IConstructStatement> sjw, IDatabaseEngine engine) {
		/*
		 * logger.debug("Query is " + query); sjw.setEngine(engine);
		 * sjw.setQuery(query);
		 * 
		...
		try {
			...
			while (sjw.hasNext()) {
				...
				if (subjects.indexOf("(<" + st.getSubject() + ">)") < 0) {
					if (engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.SESAME
							|| engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.SEMOSS_SESAME_REMOTE
							|| engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.RDBMS) // RDBMS because the the
																								// in memory is RDBMS
			...
			if (rc != null) { // if rc is null here, that means our original query didn't return anything.
				...
				boolean loadHierarchy = !(subjects.length() == 0 && predicates.length() == 0 && objects.length() == 0)
						&& isRDF; // Load Hierarchy if and only if this is a RDF Engine - else dont worry
				// about it
				if (loadHierarchy) {
					try {
				...
				if (prop && isRDF) { // hmm this is something i need wor worry about but I will work it with an
					// RDF plug right now
	...
	public void addToSesame(IConstructStatement st, boolean overrideURI, boolean add2Base) {
		try {
			...
			else if (obj instanceof Literal) // all the sesame routine goes here
			{
				 ...
				 * overlay) { //logger.info("Adding to the new model");
		...
		try {
			...
			while (sjsc.hasNext()) {
				...
				if (!baseFilterHash.containsKey(subjectName) && !baseFilterHash.containsKey(predicateName)
					...
					// logger.warn("Found Edge " + edge.getURI() + "<<>>" + vert1.getURI() + "<<>>"
					// +
					// vert2.getURI());
					...
					// logger.warn("Missing Edge " + edge.getURI() + "<<>>" + vert1.getURI() +
					// "<<>>" +
					// vert2.getURI());
	...
	public void processTraverseCourse() {
		...
		// future, thus we
		// need to remove all future models
	...
	public void processDataMakerComponent(DataMakerComponent component) {
		...
		// traversal is
		// invalid. this will set the traversal back one step and remove the invalid
		// traversal from rc
		// and model stores.
	...
	public Map getDataMakerOutput(String... selectors) {
		...
		// if(this.getOverlay()){ // removing this for now to keep aligned with tinker
		// frame
	...
	public Map<String, String> getScriptReactors() {
		...
		// almost feels like
		// I need a way to tell when to do this and when not but let me see
		...
		// "prerna.sablecc.GDMImportDataReactor");
		...
		// reactorNames.put(PKQLReactor.INPUT.toString(),
		// "prerna.sablecc.InputReactor");
		//
