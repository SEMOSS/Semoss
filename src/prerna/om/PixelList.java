...
public class PixelList implements Iterable<Pixel> {
   ...
   */
  public void updateAllIdsAndPositions(
      List<String> pixelIds, List<Map<String, Object>> pixelPositions) {
    if (pixelIds.size() != this.pixelList.size()
        && pixelPositions.size() != this.pixelList.size()) {
      throw new IllegalArgumentException("Array size must match current recipe size");
    }
    for (int i = 0; i < pixelPositions.size(); i++) {
      pixelList.get(i).setId(pixelIds.get(i));
  ...
  /**
   * Find the last pixel used to paint this panel/layer combination That was not a refresh panel
   * task
   *
   * @param panelId
   * @param layerId
