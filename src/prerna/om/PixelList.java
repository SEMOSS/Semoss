...
//	private AtomicInteger counter = new AtomicInteger(0);
	...
	public PixelList(int capacity) {
		...
		this.pixelList = Collections.synchronizedList(new ArrayList<>(capacity));
...
//		counter.getAndIncrement();
		...
		if (p.isCodeExecution() && p.isUserScript() && pixelList.size() > 1) {
			...
			LAST_DATA_LOOP: while (pixelList.size() - counter >= 0) {
				...
				if (prevPixel.isCodeExecution() && prevPixel.isUserScript()
						...
						&& (p.getLanguage() == Variable.LANGUAGE.R || p.getLanguage() == Variable.LANGUAGE.PYTHON)) {
	...
	public List<Pixel> addPixel(List<String> pixelRecipe) {
		synchronized (this) {
			List<Pixel> subset = Collections.synchronizedList(new ArrayList<>());
	...
	public List<String> getPixelRecipe() {
		List<String> pixelRecipe = Collections.synchronizedList(new ArrayList<>(pixelList.size()));
	...
	public List<String> getPixelIds() {
		List<String> pixelIds = Collections.synchronizedList(new ArrayList<>(pixelList.size()));
		for (Pixel p : pixelList) {
	...
	public List<Map<String, Object>> getPixelPositions() {
		List<Map<String, Object>> pixelPositions = Collections.synchronizedList(new ArrayList<>(pixelList.size()));
		for (Pixel p : pixelList) {
	...
	public List<Integer> removeIds(List<String> pixelIds, boolean propogate) {
		synchronized (this.pixelList) {
			...
			List<Integer> indices = Collections.synchronizedList(new ArrayList<>(pixelIds.size()));
			...
			// to smallest index
			List<Integer> finalList = Collections.synchronizedList(new ArrayList<>());
	...
	/**
	 * Find the last pixel used to paint this panel/layer combination That was not a
	 * refresh panel task
	 * 
	 * @param panelId
	 * @param layerId
