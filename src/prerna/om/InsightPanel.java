...
public class InsightPanel {
  ...
  /**
   * Merge new panel configuration options into the existing panel config map This will merge child
   * maps together if possible
   *
   * @param config
   */
  ...
  /**
   * Merge new ornaments into the existing ornament map This will merge child maps together if
   * possible
   *
   * @param ornaments
   */
  ...
  /**
   * Merge new event info into the existing event map This will merge child maps together if
   * possible
   *
   * @param ornaments
   */
      ...
      for (int i = 0; i < arr.size(); i++) {
        if (arr.get(i) instanceof Map) {
          ...
          if (innerMap.get("dimension") != null) {
            ...
            if (innerMap.get("prepend") != null
                && !(innerMap.get("prepend").toString().isEmpty())) {
            ...
            if (innerMap.get("delimiter") != null
                && !(innerMap.get("delimiter").toString().isEmpty())) {
  ...
  public void setFinalViewOptions(
      SelectQueryStruct lastQs, TaskOptions lastTaskOptions, IFormatter lastFormatter) {
    this.lastQs = lastQs;
    this.lastTaskOptions = lastTaskOptions;
    this.lastFormatter = lastFormatter;
    // grab the panel map
  ...
  /**
   * Take all the properties of another insight panel and set them for this panel
   *
   * @param existingPanel
   */
  public void clone(InsightPanel existingPanel) {
    ...
    if (existingPanel.lastTaskOptions != null) {
      TaskOptions copyTaskOptions =
          gson.fromJson(gson.toJson(existingPanel.lastTaskOptions), TaskOptions.class);
    ...
    if (existingPanel.layerTaskOption != null) {
      ...
      for (String layerId : existingPanel.layerTaskOption.keySet()) {
        ...
        if (thisTaskOptions != null) {
          TaskOptions copyTaskOptions =
              gson.fromJson(gson.toJson(thisTaskOptions), TaskOptions.class);
          copyTaskOptions.swapPanelIds(this.panelId, existingPanel.getPanelId());
