...
public class AddToMasterDB {
	 ...
	 *
	 *
	 * a. Need multiple primary keys b. Need a way to specify property with same
	 * name across the multiple concepts c. Need for multiple foreign keys d. Being
	 * able to handle loop elegantly e. Being able to link based on multiple keys
	 * i.e. I should be able to query the database through 2 keys instead of one f.
	 * Same as e. but also being able to do it across tables on create g. Need a way
	 * to tag instance level data so it can be compared and new recommendations can
	 * be made i.e. being able to send the data. Need some way of doing a PKI so the
	 * actual data is never sent
	 *
	 *
	 */
	...
	public boolean registerEngineLocal(String smssFile, Properties prop, String engineId) {
		...
		if (!new File(owlFile).exists()) {
			classLogger.warn(
					"Attempting to load engine " + engineId + " into the local master but the OWL File does not exist");
			return false;
		}
		...
		try {
			...
			if (propEngType.contains("RDBMS") || propEngType.contains("H2EmbeddedServerEngine")
			...
			classLogger.info("For engine " + Utility.cleanLogString(engineName) + " : Total Concepts Found = "
					+ concepts.size());
			classLogger.info("For engine " + Utility.cleanLogString(engineName) + " : Total Relationships Found = "
					+ relationships.size());
			...
			for (int conceptIndex = 0; conceptIndex < concepts.size(); conceptIndex++) {
				...
				masterConcept(conceptPs, engineConceptPs, conceptMetaDataPs, engineName, conceptPhysicalUri, owlReader,
						dbType);
			}
			...
			throw new IllegalArgumentException(
					"An error occurred adding the database to local master. Detailed message = " + e.getMessage(), e);
		} finally {
	 ...
	 * @throws SQLException
	 */
	private void masterConcept(PreparedStatement conceptPs, PreparedStatement engineConceptPs,
			PreparedStatement conceptMetaDataPs, String engineName, String conceptPhysicalUri,
			ReadOnlyOWLEngine owlReader, IDatabaseEngine.DATABASE_TYPE dbType) throws SQLException {
		int parameterIndex = 1;
		...
		for (int propIndex = 0; propIndex < properties.size(); propIndex++) {
			...
			masterProperty(conceptPs, engineConceptPs, conceptMetaDataPs, engineName, conceptPhysicalUri,
					propertyPhysicalUri, engineConceptGuid, conceptPhysicalInstance, conceptGuid, semossName, owlReader,
					dbType);
		}
	}
	 ...
	 * @throws SQLException
	 */
	private void masterProperty(PreparedStatement conceptPs, PreparedStatement engineConceptPs,
			PreparedStatement conceptMetaDataPs, String engineName, String conceptPhysicalUri,
			String propertyPhysicalUri, String parentEngineConceptGuid, String parentPhysicalName,
			String parentConceptGuid, String parentSemossName, ReadOnlyOWLEngine owlReader,
			IDatabaseEngine.DATABASE_TYPE dbType) throws SQLException {
		int parameterIndex = 1;
	 ...
	 * @throws SQLException
	 */
	private void masterRelationship(PreparedStatement relationPs, PreparedStatement engineRelationPs, String engineName,
			String[] relationship, ReadOnlyOWLEngine owlReader) throws SQLException {
		int parameterIndex = 1;
		...
		try {
			...
			// update
				...
				String updateSql = "UPDATE " + Constants.CONCEPT_METADATA_TABLE + " SET " + Constants.LM_META_VALUE
						+ " = ? " + " WHERE " + Constants.LM_PHYSICAL_NAME_ID + " = ? " + " AND "
  ...
  //////////////////////////////////////////////////////////////////////////////////////
	...
	// Properties localMasterProp = loadEngineProp(DB_DIRECTORY,
	// Constants.LOCAL_MASTER_DB);
	// IDatabaseEngine localMaster =
	// Utility.loadDatabase(determineSmssPath(DB_DIRECTORY,
	...
	// Utility.loadDatabase(determineSmssPath(DB_DIRECTORY, engineName),
	// engineProp);
	//
