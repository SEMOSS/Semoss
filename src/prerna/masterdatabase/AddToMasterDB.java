  ...
  */
  ...
  public boolean registerEngineLocal(String smssFile, Properties prop, String engineId) {
    ...
    if (!new File(owlFile).exists()) {
      classLogger.warn(
          "Attempting to load engine "
              + engineId
              + " into the local master but the OWL File does not exist");
      return false;
    }
    ...
    try {
      ...
      ReadOnlyOWLEngine owlReader =
          new OWLEngineFactory(rfse, null, engineId, engineName).getReadOWL();
      ...
      if (propEngType.contains("RDBMS")
          || propEngType.contains("H2EmbeddedServerEngine")
          || propEngType.contains("Impala")) {
      ...
      String enginePsQuery =
          "INSERT INTO ENGINE (ID, ENGINENAME, MODIFIEDDATE, TYPE) VALUES (?,?,?,?)";
      ...
      classLogger.info(
          "For engine "
              + Utility.cleanLogString(engineName)
      ...
      String conceptPsQuery =
          "INSERT INTO CONCEPT (LOCALCONCEPTID, CONCEPTUALNAME, LOGICALNAME, DOMAINNAME, GLOBALID) VALUES (?,?,?,?,?)";
      String engineConceptPsQuery =
          "INSERT INTO ENGINECONCEPT (ENGINE, PARENTSEMOSSNAME, SEMOSSNAME, PARENTPHYSICALNAME, "
      ...
      String conceptMetadataPsQuery =
          "INSERT INTO CONCEPTMETADATA ("
      ...
      String relationPsQuery =
          "INSERT INTO RELATION (ID, SOURCEID, TARGETID, GLOBALID) VALUES (?,?,?,?)";
      String engineRelationPsQuery =
          "INSERT INTO ENGINERELATION (Engine, RelationID, InstanceRelationID, SourceConceptID, "
      ...
      for (int relIndex = 0; relIndex < relationships.size(); relIndex++) {
        ...
        masterRelationship(
            relationPs, engineRelationPs, engineName, relationshipToInsert, owlReader);
      }
      ...
      throw new IllegalArgumentException(
          "An error occurred adding the database to local master. Detailed message = "
              + e.getMessage(),
          e);
    } finally {
  ...
  /**
   * Will add a concept and all of its properties into the local master This will add the following
   * information: The concept into the CONCEPT TABLE if it does not already exist The properties of
   * the concept into the CONCEPT TABLE if it does not already exist The concept into the
   * ENGINECONCEPT TABLE The properties of the concept into the ENGINECONCEPT TABLE Since we are
   * adding a new engine, we do not need to check if the concept/properties exist in the
   * ENGINECONEPT TABLE
   *
   * @param conceptPs
   ...
   * @throws SQLException
   */
  private void masterConcept(
      PreparedStatement conceptPs,
      PreparedStatement engineConceptPs,
      PreparedStatement conceptMetaDataPs,
      String engineName,
    ...
    if (this.conceptIdHash.containsKey(conceptualName + "_CONCEPTUAL")) {
      ...
      Collection<String> curLogicals =
          MasterDatabaseUtility.getAllLogicalNamesFromConceptualRDBMS(conceptualName);
      // add new logicals
      if (!logicals.isEmpty()) {
    ...
    this.conceptIdHash.put(
        engineName + "_" + conceptPhysicalInstance + "_PHYSICAL", engineConceptGuid);
    ...
    for (int propIndex = 0; propIndex < properties.size(); propIndex++) {
      ...
      classLogger.debug(
          "For concept = " + conceptPhysicalUri + ", adding property ::: " + propertyPhysicalUri);
      masterProperty(
          conceptPs,
          engineConceptPs,
          conceptMetaDataPs,
  ...
  /**
   * Will add a property for a given concept into the local master This will add the following
   * information: The property into the CONCEPT TABLE if it does not already exist The property into
   * the ENGINECONCEPT TABLE Since we are adding a new engine, we do not need to check if the
   * properties exist in the ENGINECONEPT TABLE
   *
   * @param conceptPs
   * @param engineConceptPs
   * @param conceptMetaDataPs
  ...
  private void masterProperty(
    ...
    String propertyPixelUri =
        owlReader.getPropertyPixelUriFromPhysicalUri(conceptPhysicalUri, propertyPhysicalUri);
    ...
    if (this.conceptIdHash.containsKey(propertyConceptualName)) {
      ...
      Collection<String> curLogicals =
          MasterDatabaseUtility.getAllLogicalNamesFromConceptualRDBMS(propertyConceptualName);
      // add new logicals
      if (!logicals.isEmpty()) {
        for (String logical : logicals) {
          if (!curLogicals.contains(logical)) {
    ...
    if (dbType == IDatabaseEngine.DATABASE_TYPE.RDBMS
        || dbType == IDatabaseEngine.DATABASE_TYPE.R) {
  ...
  /**
   * Master a relationship into the local master Relationship array is [startNodePhysicalUri,
   * endNodePhysicalUri, relationshipUri]
   *
   * @param relationPs
   ...
   * @throws SQLException
   */
  private void masterRelationship(
      PreparedStatement relationPs,
      PreparedStatement engineRelationPs,
      String engineName,
      String[] relationship,
      ReadOnlyOWLEngine owlReader)
      throws SQLException {
    ...
    if (this.conceptIdHash.containsKey(pixelStartNodeName + "_" + pixelEndNodeName + "_RELATION")) {
      relationGuid =
          this.conceptIdHash.get(pixelStartNodeName + "_" + pixelEndNodeName + "_RELATION");
    } else {
      ...
      // store it in the hash
      this.conceptIdHash.put(
          pixelStartNodeName + "_" + pixelEndNodeName + "_RELATION", relationGuid);
    ...
    // just add the engine
    String startConceptGuid =
        this.conceptIdHash.get(engineName + "_" + startNodePhysicalInstance + "_PHYSICAL");
    String endConceptGuid =
        this.conceptIdHash.get(engineName + "_" + endNodePhysicalInstance + "_PHYSICAL");
    String engineId = this.conceptIdHash.get(engineName + "_ENGINE");
      ...
      if (duplicateCheck == null) {
        stmt =
            conn.prepareStatement(
                queryUtil.createInsertPreparedStatementString(tableName, colNames));
  ...
  //        IDatabaseEngine localMaster = Utility.loadDatabase(determineSmssPath(DB_DIRECTORY,
  // Constants.LOCAL_MASTER_DB), localMasterProp);
  //
   ...
   * @throws IOException
   */
  private static Properties loadEngineProp(final String DB_DIRECTORY, String engineName)
      throws IOException {
    try (FileInputStream fis =
        new FileInputStream(new File(determineSmssPath(DB_DIRECTORY, engineName)))) {
