...
public class MasterDatabaseUtility {
	...
	public static List<String> getConceptualIdsWithSimilarLogicalNames(List<String> conceptualIds) {
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("CONCEPT__LOGICALNAME", "==", subQs, PixelDataType.QUERY_STRUCT));
	...
	public static List<Object[]> getAllTablesAndColumns(String databaseId) {
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__IGNORE_DATA", "==", false, PixelDataType.BOOLEAN));
	 ...
	 * @return
	 */
	public static List<Map<String, Object>> getDatabaseConnections(List<String> localConceptIds,
			List<String> databaseFilter) {
		RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);

		List<Map<String, Object>> returnData = new ArrayList<>();

		/*
		 * Grab all the matching tables and columns based on the logical names Once we
		 * have those, we will grab all the relationships for the tables and all the
		 * other columns that we can traverse to
		 */
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__LOCALCONCEPTID", "==", localConceptIds));
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PK", "==", true, PixelDataType.BOOLEAN));
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__IGNORE_DATA", "==", false, PixelDataType.BOOLEAN));
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PARENTPHYSICALNAMEID", "==", idsForProperties));
		qs.addRelation("ENGINE__ID", "ENGINECONCEPT__ENGINE", "inner.join");
		qs.addOrderBy("ENGINE__ENGINENAME");
		qs.addOrderBy("ENGINECONCEPT__PARENTSEMOSSNAME");
		qs.addOrderBy("ENGINECONCEPT__IGNORE_DATA");
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINERELATION__SOURCECONCEPTID", "==", idsForRelationships));
		...
		// qs.addSelector(new QueryColumnSelector("ENGINECONCEPT__PROPERTY_TYPE"));
		// qs.addSelector(new
		// QueryColumnSelector("ENGINECONCEPT__PARENTPHYSICALNAMEID"));
		...
		// }
		//
		// qs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PARENTPHYSICALNAMEID",
		// "==", new Vector<String>(relationshipEquivMap.keySet())));
		// qs.addRelation("ENGINE__ID", "ENGINECONCEPT__ENGINE", "inner.join");
		// qs.addOrderBy("ENGINECONCEPT__ENGINE");
		// try {
		// wrapper = WrapperManager.getInstance().getRawWrapper(engine, qs);
		...
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINERELATION__TARGETCONCEPTID", "==", idsForRelationships));
		qs.addRelation("ENGINE__ID", "ENGINERELATION__ENGINE", "inner.join");
	...
	/**
	 * Get the properties for a given concept for a specific database THIS IS THE
	 * SAME QUERY AS {@link #getConceptPropertiesRDBMS} BUT DIFFERENT RETURN
	 *
	 * @param conceptName
	 * @param engineId
	 * @return
	...
	/**
	 * Get the properties for a given concept across all the databases THIS IS THE
	 * SAME QUERY AS {@link #getConceptProperties} BUT DIFFERENT RETURN
	 *
	 * @param conceptName
	 * @param engineId
	 *            optional filter for the properties
	 * @return
	 */
	public static Map<String, Object[]> getConceptProperties(List<String> logicalNames, List<String> databaseFilter) {
		...
		RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
		{
			...
			}
			{
				SelectQueryStruct subQs = new SelectQueryStruct();
				// store first and fill in sub query after
				qs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__IGNORE_DATA", "==", true,
		...
		}
		{
			...
			}
			{
				SelectQueryStruct subQs = new SelectQueryStruct();
				// store first and fill in sub query after
				qs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__IGNORE_DATA", "==", false,
						PixelDataType.BOOLEAN));
				qs.addExplicitFilter(
						SimpleQueryFilter.makeColToSubQuery("ENGINECONCEPT__PARENTPHYSICALNAMEID", "==", subQs));
	 ...
	 *
	 * <p>
	 * Direction upstream/downstream is always in reference to the node being
	 * searched For example, if the relationship in the direction Title -> Genre The
	 * result would be { upstream -> [Genre] } because Title is upstream of Genre
	 *
	 * @param conceptType
	 * @return
	 */
	...
	public static Map getConnectedConceptsRDBMS(List<String> conceptLogicalNames, List<String> databaseFilters) {
		...
		// select e.enginename, ec.engine, c.logicalname, ec.physicalnameid from concept
		// c,
		// engineconcept ec, engine e where c.logicalname in ('Title') and
		// c.localconceptid=ec.localconceptid and e.id = ec.engine
		// String conceptMasterQuery = "select ec.engine, c.conceptualname,
		// ec.physicalnameid,
		// ec.physicalname "
		...
		// select distinct e.enginename, 'Title', 'downstream' as downstream,
		// er.relationname,
		// c.logicalname , er.engine, er.targetconceptid, ec.physicalname from
		// enginerelation er,
		// engineconcept ec, concept c, engine e where er.sourceconceptid in (select
		// physicalnameid from
		// engineconcept where localconceptid in (select localconceptid from concept
		// where logicalname
		// in ('Title')))
		// and ec.physicalnameid=er.targetconceptid and
		// c.localconceptid=ec.localconceptid and
		// e.id=er.engine;

		// now time to run the upstream and downstream queries
		// String downstreamQuery = "select distinct ec.engine, er.sourceconceptid,
		// 'upstream' as
		// upstream, "
		// + "er.relationname, c.conceptualname , er.engine, er.targetconceptid,
		// ec.physicalname "
		// + "from enginerelation er, engineconcept ec, concept c "
		// + "where "
		// + (engineFilters != null ? (" ec.engine in " + engineString + " and ") : "")
		// + "er.sourceconceptid in (select physicalnameid from engineconcept where
		// localconceptid in
		// "
		// + "(select localconceptid from concept where logicalname in " + conceptString
		// + ")) "
		// + "and ec.physicalnameid=er.targetconceptid and
		// c.localconceptid=ec.localconceptid;";
		...
		// now time to run the upstream and downstream queries
		// String upstreamQuery = "select distinct ec.engine, er.targetconceptid,
		// 'downstream' as
		// downstream, "
		// + "er.relationname, c.conceptualname , er.engine, er.sourceconceptid,
		// ec.physicalname "
		// + "from enginerelation er, engineconcept ec, concept c "
		// + "where "
		// + (engineFilters != null ? (" ec.engine in " + engineString + " and ") : "")
		// + "er.targetconceptid in (select physicalnameid from engineconcept where
		// localconceptid in
		// "
		// + "(select localconceptid from concept where logicalname in " + conceptString
		// + ")) "
		// + "and ec.physicalnameid=er.sourceconceptid and
		// c.localconceptid=ec.localconceptid";
	...
	public static String getBasicDataType(String databaseId, String pixelName, String parentPixelName) {
		...
		if (parentPixelName != null && !parentPixelName.isEmpty()) {
			// additional filters
			qs.addExplicitFilter(
					SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PARENTSEMOSSNAME", "==", parentPixelName));
		}
	...
	public static String getAdditionalDataType(String databaseId, String conceptualName, String parentConceptualName) {
		...
		if (parentConceptualName != null && !parentConceptualName.isEmpty()) {
			// additional filters
			qs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PARENTSEMOSSNAME", "==",
					parentConceptualName));
		}
	 ...
	 * @param conceptName
	 * @param databaseId
	 *            the database to get the properties for
	 * @return
	 */
	...
	/**
	 * Get the queryable pixel selectors for the table If RDBMS this will have all
	 * the properties If RDF/Graph this will have the concept and all its properties
	 * Return will be in TABLE__COLUMN format
	 *
	 * @param parentName
	 *            String with the name of the concept
	 * @param databaseId
	 *            String with the engine to get the selectors for
	 * @return
	 */
	public static List<String> getConceptPixelSelectors(String parentName, String databaseId) {
		...
		// or
		// 2) the parent itself has data (i.e column name is the parent + pk = true +
		// ignoreData=false)
	...
	/**
	 * Get the physical concept id for a concept given the engine id + conceptual
	 * name
	 *
	 * @param databaseId
	 * @param conceptualName
	...
	public static String getPhysicalConceptId(String databaseId, String conceptualName) {
		...
		// concept.localconceptid=engineconcept.localconceptid WHERE
		// engineconcept.engine='' AND
	...
	public static String getMetadataValue(String databaseId, String concept, String key) {
		// String query = "select " + Constants.VALUE + " from " +
		...
		qs.addExplicitFilter(SimpleQueryFilter
				.makeColToValFilter(Constants.CONCEPT_METADATA_TABLE + "__" + Constants.KEY, "==", key));
		{
			SelectQueryStruct subQs = new SelectQueryStruct();
			// store first and fill in sub query after
			qs.addExplicitFilter(SimpleQueryFilter
					.makeColToSubQuery(Constants.CONCEPT_METADATA_TABLE + "__LOCALCONCEPTID", "==", subQs));
	...
	public static String testDatabaseIdIfAlias(String databaseId) {
		...
		if (databaseIds.size() == 1) {
			...
			throw new IllegalArgumentException("There are 2 databases with the name " + databaseId
	 ...
	 *            optional filter based on engines
	 * @return
	 */
	...
	public static List<String> getConceptualNamesFromPhysicalIds(List<String> physicalNameIds) {
		...
		qs.addSelector(new QueryColumnSelector("CONCEPT__CONCEPTUALNAME"));
		qs.addExplicitFilter(
				SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PHYSICALNAMEID", "==", physicalNameIds));
		qs.addRelation("CONCEPT__LOCALCONCEPTID", "ENGINECONCEPT__LOCALCONCEPTID", "inner.join");
	 ...
	 * @return
	 */
	public static List<String[]> getConceptualToLogicalToPhysicalModel(List<String> conceptualNames,
			Collection<String> databaseFilters) {
		RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
  ...
  ///////////////////////////////////////////////////////////////////////////////////
	...
	// concept);
	// RDBMSNativeEngine engine = (RDBMSNativeEngine)
  ...
  ///////////////////////////////////////////////////////////////////////////////////
	...
	public static HashMap<String, Object> getXrayConfigList() {
		...
		try {
			...
			// Don't print stack trace... xrayConfigList table is missing if no config file
			// exists
		} finally {
			ConnectionUtils.closeAllConnectionsIfPooling(engine, conn, stmt, rs);
		}
	...
	public static Map<String, Object> getXrayExisitingMetamodelRDBMS(String engineName) {
		...
		try (PreparedStatement statement = conn.prepareStatement(nodeQuery)) {
			// String nodeQuery = "select c.conceptualname, ec.physicalname,
			// ec.localconceptid,
			// ec.physicalnameid, ec.parentphysicalid, ec.property from "
			// + "engineconcept ec, concept c, engine e " + "where ec.engine=e.id " + "and
			// e.enginename='"
			// + engineName + "' " + "and c.localconceptid=ec.localconceptid order by
			// ec.property";
		...
		try (PreparedStatement statement = conn.prepareStatement(edgeQuery)) {
			...
			// String edgeQuery = "SELECT er.sourceconceptid, er.targetconceptid FROM
			// ENGINERELATION er,
			...
			while (rs.next()) {
				...
				if (foundNode) {
					edgeHash.put(engineName + delim + sourceName + delim + sourceName + delim + engineName + delim
							+ targetName + delim + targetName, newEdge);
				}
			}
	...
	/**
	 * It is your responsibility to close the connection object if connection
	 * pooling and using this method
	 *
	 * @param databaseId
	 * @param positions
	 * @param conn
	 * @throws Exception
	 */
	public static void saveMetamodelPositions(String databaseId, Map<String, Object> positions, Connection conn)
			throws Exception {
