...
public class MasterDatabaseUtility {
  ...
  private static void executeInitLocalMaster(IRDBMSEngine engine, Connection conn)
    ...
    colNames =
        new String[] {
          ...
          "SEMOSSNAME",
          "PARENTPHYSICALNAME",
          "PARENTPHYSICALNAMEID",
          "PHYSICALNAME",
          "PHYSICALNAMEID",
          "PARENTLOCALCONCEPTID",
          "LOCALCONCEPTID",
          "IGNORE_DATA",
          "PK",
          "ORIGINAL_TYPE",
    ...
    // add index
    {
      ...
      if (allowIfExistsIndexs) {
        String sql =
            queryUtil.dropIndexIfExists("ENGINE_CONCEPT_ENGINE_LOCAL_CONCEPT_ID", "ENGINECONCEPT");
        classLogger.info("Running sql " + sql);
        executeSql(conn, sql);
      } else {
        if (queryUtil.indexExists(
            engine, "ENGINE_CONCEPT_ENGINE_LOCAL_CONCEPT_ID", "ENGINECONCEPT", database, schema)) {
          String sql =
              queryUtil.dropIndex("ENGINE_CONCEPT_ENGINE_LOCAL_CONCEPT_ID", "ENGINECONCEPT");
          classLogger.info("Running sql " + sql);
          executeSql(conn, sql);
        }
      }
    ...
    if (allowIfExistsIndexs) {
      ...
      String sql =
          queryUtil.createIndexIfNotExists(
              "ENGINECONCEPT_ENGINE_LOCALCONCEPTID_INDEX", "ENGINECONCEPT", iCols);
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);
      sql =
          queryUtil.createIndexIfNotExists(
              "ENGINECONCEPT_PHYSICALNAMEID_INDEX", "ENGINECONCEPT", "PHYSICALNAMEID");
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);
    } else {
      // see if index exists
      if (!queryUtil.indexExists(
          engine, "ENGINECONCEPT_ENGINE_LOCALCONCEPTID_INDEX", "ENGINECONCEPT", database, schema)) {
        List<String> iCols = new ArrayList<>();
        iCols.add("ENGINE");
        iCols.add("LOCALCONCEPTID");
      ...
      if (!queryUtil.indexExists(
          engine, "ENGINECONCEPT_PHYSICALNAMEID_INDEX", "ENGINECONCEPT", database, schema)) {
        String sql =
            queryUtil.createIndex(
                "ENGINECONCEPT_PHYSICALNAMEID_INDEX", "ENGINECONCEPT", "PHYSICALNAMEID");
        classLogger.info("Running sql " + sql);
    ...
    // concept table
    colNames =
        new String[] {"LOCALCONCEPTID", "CONCEPTUALNAME", "LOGICALNAME", "DOMAINNAME", "GLOBALID"};
    types =
        new String[] {
    ...
    if (allowIfExistsIndexs) {
      String sql =
          queryUtil.createIndexIfNotExists("CONCEPT_ID_INDEX", "CONCEPT", "LOCALCONCEPTID");
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);
    } else {
      // see if index exists
    ...
    if (allowIfExistsIndexs) {
      String sql =
          queryUtil.createIndexIfNotExists("RELATION_TARGETID_INDEX", "RELATION", "TARGETID");
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);
    ...
    if (allowIfExistsIndexs) {
      String sql =
          queryUtil.createIndexIfNotExists(
              "ENGINERELATION_ENGINE_INDEX", "ENGINERELATION", "ENGINE");
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);

      sql =
          queryUtil.createIndexIfNotExists(
              "ENGINERELATION_TARGETCONCEPTID_INDEX", "ENGINERELATION", "TARGETCONCEPTID");
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);

      sql =
          queryUtil.createIndexIfNotExists(
              "ENGINERELATION_SOURCECONCEPTID_INDEX", "ENGINERELATION", "SOURCECONCEPTID");
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);
    } else {
      // see if index exists
      if (!queryUtil.indexExists(
          engine, "ENGINERELATION_ENGINE_INDEX", "ENGINERELATION", database, schema)) {
        String sql =
            queryUtil.createIndex("ENGINERELATION_ENGINE_INDEX", "ENGINERELATION", "ENGINE");
        classLogger.info("Running sql " + sql);
        executeSql(conn, sql);
      }
      if (!queryUtil.indexExists(
          engine, "ENGINERELATION_TARGETCONCEPTID_INDEX", "ENGINERELATION", database, schema)) {
        String sql =
            queryUtil.createIndex(
                "ENGINERELATION_TARGETCONCEPTID_INDEX", "ENGINERELATION", "TARGETCONCEPTID");
        classLogger.info("Running sql " + sql);
      ...
      if (!queryUtil.indexExists(
          engine, "ENGINERELATION_SOURCECONCEPTID_INDEX", "ENGINERELATION", database, schema)) {
        String sql =
            queryUtil.createIndex(
                "ENGINERELATION_SOURCECONCEPTID_INDEX", "ENGINERELATION", "SOURCECONCEPTID");
        classLogger.info("Running sql " + sql);
    ...
    // concept metadata
    updateMetadataTable(
        engine, conn, queryUtil, Constants.CONCEPT_METADATA_TABLE, database, schema);
    colNames =
        new String[] {
          Constants.LM_PHYSICAL_NAME_ID, Constants.LM_META_KEY, Constants.LM_META_VALUE
        };
    types = new String[] {"varchar(255)", "varchar(800)", CLOB_DATATYPE};
    if (allowIfExistsTable) {
      String sql =
          queryUtil.createTableIfNotExists(Constants.CONCEPT_METADATA_TABLE, colNames, types);
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);
    } else {
      // see if table exists
    ...
    if (allowIfExistsIndexs) {
      String sql =
          queryUtil.createIndexIfNotExists(
              "CONCEPTMETADATA_KEY_INDEX", Constants.CONCEPT_METADATA_TABLE, Constants.LM_META_KEY);
      classLogger.info("Running sql " + sql);
      executeSql(conn, sql);

      sql =
          queryUtil.createIndexIfNotExists(
              "CONCEPTMETADATA_PHYSICALNAMEID_INDEX",
              Constants.CONCEPT_METADATA_TABLE,
  ...
  @Deprecated
  private static void requireRemakeAndAlter(
      IRDBMSEngine engine,
      Connection conn,
  ...
  @Deprecated
  private static void updateMetadataTable(
      IRDBMSEngine engine,
      Connection conn,
      AbstractSqlQueryUtil queryUtil,
      String tableName,
    ...
    if (queryUtil.tableExists(engine, tableName, database, schema)) {
      ...
      if (allCols.contains(Constants.VALUE) || allCols.contains(Constants.VALUE.toLowerCase())) {
        ...
        sql =
            queryUtil.modColumnType(
                tableName, Constants.LM_META_VALUE, queryUtil.getClobDataTypeName());
        classLogger.info("Running sql " + sql);
        executeSql(conn, sql);
      }
      ...
      if (queryUtil.allowDropColumn()) {
        if (allowIfExists) {
          ...
          if (allCols.contains("LOCALCONCEPTID")
              || allCols.contains("LOCALCONCEPTID".toLowerCase())) {
            String sql = queryUtil.alterTableDropColumnIfExists(tableName, "LOCALCONCEPTID");
            classLogger.info("Running sql " + sql);
            executeSql(conn, sql);
          }
      ...
      if (queryUtil.allowAddColumn()) {
        if (allowIfExists) {
          executeSql(
              conn,
              queryUtil.alterTableAddColumnIfNotExists(
                  tableName, "PHYSICALNAMEID", "varchar(255)"));
        } else {
          // check column exists in table
          if (!allCols.contains("PHYSICALNAMEID")
              && !allCols.contains("PHYSICALNAMEID".toLowerCase())) {
  ...
  public static List<String> getAllLogicalNamesFromConceptualRDBMS(String conceptualName) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CONCEPT__CONCEPTUALNAME", "==", conceptualName));
  ...
  public static List<String> getAllLogicalNamesFromPixelName(List<String> pixelNames) {
    ...
    qs.addSelector(new QueryColumnSelector("CONCEPT__LOGICALNAME"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__SEMOSSNAME", "==", pixelNames));
  ...
  public static List<String> getLocalConceptIdsFromLogicalName(List<String> logicalNames) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CONCEPT__LOGICALNAME", "==", logicalNames));
  ...
  public static List<String> getConceptualIdsWithSimilarLogicalNames(List<String> conceptualIds) {
    ...
    subQs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CONCEPT__LOCALCONCEPTID", "==", conceptualIds));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "CONCEPT__LOGICALNAME", "==", subQs, PixelDataType.QUERY_STRUCT));
  ...
  public static List<Object[]> getAllTablesAndColumns(String databaseId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__IGNORE_DATA", "==", false, PixelDataType.BOOLEAN));
  ...
  public static List<Object[]> getAllTablesAndColumns(Collection<String> databaseIds) {
    ...
    qs.addSelector(new QueryColumnSelector("ENGINECONCEPT__PK"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseIds));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__IGNORE_DATA", "==", false, PixelDataType.BOOLEAN));
  ...
  public static List<String[]> getRelationships(Collection<String> databaseIds) {
    ...
    if (databaseIds != null && !databaseIds.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINERELATION__ENGINE", "==", databaseIds));
    }
   ...
   * @return
   */
  public static List<Map<String, Object>> getDatabaseConnections(
      List<String> localConceptIds, List<String> databaseFilter) {
    RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__LOCALCONCEPTID", "==", localConceptIds));
    if (databaseFilter != null && !databaseFilter.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseFilter));
    }
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__PK", "==", true, PixelDataType.BOOLEAN));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__IGNORE_DATA", "==", false, PixelDataType.BOOLEAN));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PHYSICALNAMEID", "==", parentIds));
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__PARENTPHYSICALNAMEID", "==", idsForProperties));
    qs.addRelation("ENGINE__ID", "ENGINECONCEPT__ENGINE", "inner.join");
    qs.addOrderBy("ENGINE__ENGINENAME");
    qs.addOrderBy("ENGINECONCEPT__PARENTSEMOSSNAME");
    qs.addOrderBy("ENGINECONCEPT__IGNORE_DATA");
    ...
    if (databaseFilter != null && !databaseFilter.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINERELATION__ENGINE", "==", databaseFilter));
    }
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINERELATION__SOURCECONCEPTID", "==", idsForRelationships));
    qs.addRelation("ENGINE__ID", "ENGINERELATION__ENGINE", "inner.join");
    qs.addRelation(
        "ENGINERELATION__TARGETCONCEPTID", "ENGINECONCEPT__PHYSICALNAMEID", "inner.join");
    ...
    //				qs.addExplicitFilter(SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==",
    // engineFilter));
    //			}
    //
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINERELATION__TARGETCONCEPTID", "==", idsForRelationships));
    qs.addRelation("ENGINE__ID", "ENGINERELATION__ENGINE", "inner.join");
    qs.addRelation(
        "ENGINERELATION__SOURCECONCEPTID", "ENGINECONCEPT__PHYSICALNAMEID", "inner.join");
    qs.addOrderBy("ENGINERELATION__ENGINE");
  ...
  public static Map<String, Object> getMetamodelRDBMS(String databaseId, boolean includeDataTypes) {
    ...
    qs.addSelector(new QueryColumnSelector("ENGINERELATION__RELATIONNAME"));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINERELATION__ENGINE", "==", databaseId));
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(engine, qs);
      while (wrapper.hasNext()) {
        Object[] row = wrapper.next().getValues();
  ...
  /**
   * Get the properties for a given concept for a specific database THIS IS THE SAME QUERY AS {@link
   * #getConceptPropertiesRDBMS} BUT DIFFERENT RETURN
   *
   * @param conceptName
   * @param engineId
   * @return
   */
  public static Map<String, List<String>> getConceptProperties(
      List<String> logicalNames, String databaseFilter) {
    RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
    ...
    }
    {
      ...
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToSubQuery("ENGINECONCEPT__PARENTPHYSICALNAMEID", "==", subQs));
      ...
      subQs.addExplicitFilter(
          SimpleQueryFilter.makeColToSubQuery("ENGINECONCEPT__LOCALCONCEPTID", "==", subQs2));
      ...
      subQs2.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("CONCEPT__CONCEPTUALNAME", "==", logicalNames));
    }
  ...
  /**
   * Get the properties for a given concept across all the databases THIS IS THE SAME QUERY AS
   * {@link #getConceptProperties} BUT DIFFERENT RETURN
   *
   * @param conceptName
   * @param engineId optional filter for the properties
   * @return
   */
  public static Map<String, Object[]> getConceptProperties(
      List<String> logicalNames, List<String> databaseFilter) {
    ...
    RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
    {
      ...
      }
      {
        ...
        qs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter(
                "ENGINECONCEPT__IGNORE_DATA", "==", true, PixelDataType.BOOLEAN));
        qs.addExplicitFilter(
            SimpleQueryFilter.makeColToSubQuery(
                "ENGINECONCEPT__PARENTPHYSICALNAMEID", "==", subQs));
   ...
   *
   * <p>Direction upstream/downstream is always in reference to the node being searched For example,
   * if the relationship in the direction Title -> Genre The result would be { upstream -> [Genre] }
   * because Title is upstream of Genre
   *
   * @param conceptType
   * @return
   */
  @Deprecated
  public static Map getConnectedConceptsRDBMS(
      List<String> conceptLogicalNames, List<String> databaseFilters) {
    ...
    // select e.enginename, ec.engine, c.logicalname, ec.physicalnameid from concept c,
    // engineconcept ec, engine e where c.logicalname in ('Title') and
    // c.localconceptid=ec.localconceptid and e.id = ec.engine
    //		String conceptMasterQuery = "select ec.engine, c.conceptualname, ec.physicalnameid,
    // ec.physicalname "
...

    {
      ...
      if (databaseFilters != null && !databaseFilters.isEmpty()) {
        qs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseFilters));
      }
      qs.addExplicitFilter(
    ...
    // select distinct  e.enginename, 'Title', 'downstream' as downstream,  er.relationname,
    // c.logicalname , er.engine, er.targetconceptid, ec.physicalname from enginerelation er,
    // engineconcept ec, concept c, engine e where er.sourceconceptid in (select physicalnameid from
    // engineconcept where localconceptid in (select localconceptid from concept where logicalname
    // in ('Title')))
    // and ec.physicalnameid=er.targetconceptid and c.localconceptid=ec.localconceptid and
    // e.id=er.engine;

    // now time to run the upstream and downstream queries
    //		String downstreamQuery = "select distinct ec.engine, er.sourceconceptid, 'upstream' as
    // upstream, "
...

    {
      ...
      if (databaseFilters != null && !databaseFilters.isEmpty()) {
        downQs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseFilters));
      }
      {
        SelectQueryStruct subQs = new SelectQueryStruct();
        downQs.addExplicitFilter(
            SimpleQueryFilter.makeColToSubQuery("ENGINERELATION__SOURCECONCEPTID", "==", subQs));
        ...
        subQs2.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter(
                "CONCEPT__LOGICALNAME", "==", conceptLogicalNames));
      }
      downQs.addRelation(
          "ENGINECONCEPT__PHYSICALNAMEID", "ENGINERELATION__TARGETCONCEPTID", "inner.join");
    ...
    // now time to run the upstream and downstream queries
    //		String upstreamQuery = "select distinct ec.engine, er.targetconceptid, 'downstream' as
    // downstream,  "
...

    {
      ...
      if (databaseFilters != null && !databaseFilters.isEmpty()) {
        upQs.addExplicitFilter(
            SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseFilters));
      }
      {
        SelectQueryStruct subQs = new SelectQueryStruct();
        upQs.addExplicitFilter(
            SimpleQueryFilter.makeColToSubQuery("ENGINERELATION__TARGETCONCEPTID", "==", subQs));
      ...
      upQs.addRelation(
          "ENGINECONCEPT__PHYSICALNAMEID", "ENGINERELATION__SOURCECONCEPTID", "inner.join");
   ...
   * @return
   */
  public static String getBasicDataType(
      String databaseId, String pixelName, String parentPixelName) {
    RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__SEMOSSNAME", "==", pixelName));
    if (parentPixelName != null && !parentPixelName.isEmpty()) {
      // additional filters
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter(
              "ENGINECONCEPT__PARENTSEMOSSNAME", "==", parentPixelName));
    }
   ...
   * @return
   */
  public static String getAdditionalDataType(
      String databaseId, String conceptualName, String parentConceptualName) {
    RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__SEMOSSNAME", "==", conceptualName));
    if (parentConceptualName != null && !parentConceptualName.isEmpty()) {
      // additional filters
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter(
              "ENGINECONCEPT__PARENTSEMOSSNAME", "==", parentConceptualName));
    }
  ...
  public static Map<String, List<String>> getDatabaseLogicalNames(String databaseId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CONCEPTMETADATA__METAKEY", "==", "logical"));
    qs.addRelation(
        "CONCEPTMETADATA__PHYSICALNAMEID", "ENGINECONCEPT__PHYSICALNAMEID", "inner.join");
  ...
  public static Map<String, String> getDatabaseDescriptions(String databaseId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CONCEPTMETADATA__METAKEY", "==", "description"));
    qs.addRelation(
        "CONCEPTMETADATA__PHYSICALNAMEID", "ENGINECONCEPT__PHYSICALNAMEID", "inner.join");
  ...
  public static List<String> getSpecificConceptProperties(String parentName, String databaseId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", databaseId));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PARENTSEMOSSNAME", "==", parentName));
    qs.addOrderBy(new QueryColumnOrderBySelector("ENGINECONCEPT__SEMOSSNAME"));
  ...
  /**
   * Get the queryable pixel selectors for the table If RDBMS this will have all the properties If
   * RDF/Graph this will have the concept and all its properties Return will be in TABLE__COLUMN
   * format
   *
  ...
  public static List<String> getConceptPixelSelectors(String parentName, String databaseId) {
    ...
    andFilter.addFilter(
        SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__SEMOSSNAME", "==", parentName));
    andFilter.addFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__PK", "==", true, PixelDataType.BOOLEAN));
    andFilter.addFilter(
  ...
  public static String getPhysicalConceptId(String databaseId, String conceptualName) {
    // SELECT engineconcept.physicalnameid FROM engineconcept INNER JOIN concept ON
    // concept.localconceptid=engineconcept.localconceptid WHERE engineconcept.engine='' AND
    // concept.conceptualname='Title'
  ...
  public static String getPhysicalConceptIdFromPixelName(String databaseId, String pixelName) {
    ...
    if (pixelName.contains("__")) {
      ...
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__PARENTSEMOSSNAME", "==", split[0]));
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__SEMOSSNAME", "==", split[1]));
    } else {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__SEMOSSNAME", "==", pixelName));
    }
  ...
  public static String getMetadataValue(String databaseId, String concept, String key) {
    ...
    SelectQueryStruct qs = new SelectQueryStruct();
    qs.addSelector(
        new QueryColumnSelector(Constants.CONCEPT_METADATA_TABLE + "__" + Constants.VALUE));
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            Constants.CONCEPT_METADATA_TABLE + "__" + Constants.KEY, "==", key));
    {
      SelectQueryStruct subQs = new SelectQueryStruct();
      // store first and fill in sub query after
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToSubQuery(
              Constants.CONCEPT_METADATA_TABLE + "__LOCALCONCEPTID", "==", subQs));

      // fill in the sub query with the necessary column output + filters
      subQs.addSelector(new QueryColumnSelector("CONCEPT__LOCALCONCEPTID"));
      subQs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("CONCEPT__CONCEPTUALNAME", "==", concept));
      // we have a sub query again
      SelectQueryStruct subQs2 = new SelectQueryStruct();
      subQs.addExplicitFilter(
          SimpleQueryFilter.makeColToSubQuery("CONCEPT__LOCALCONCEPTID", "==", subQs2));
  ...
  public static String testDatabaseIdIfAlias(String databaseId) {
    ...
    if (databaseIds.size() == 1) {
      ...
      throw new IllegalArgumentException(
          "There are 2 databases with the name "
              + databaseId
  ...
  public static List<String> getConceptualNamesFromPhysicalIds(List<String> physicalNameIds) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            "ENGINECONCEPT__PHYSICALNAMEID", "==", physicalNameIds));
    qs.addRelation("CONCEPT__LOCALCONCEPTID", "ENGINECONCEPT__LOCALCONCEPTID", "inner.join");
   ...
   * @return
   */
  public static List<String[]> getConceptualConnections(
      List<String> conceptualNames, Collection<String> dbFilters) {
    RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
    ...
    if (dbFilters != null && !dbFilters.isEmpty()) {
      qs.addExplicitFilter(
          SimpleQueryFilter.makeColToValFilter("ENGINECONCEPT__ENGINE", "==", dbFilters));
    }
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("CONCEPT__CONCEPTUALNAME", "==", conceptualNames));
    qs.addRelation("CONCEPT__LOCALCONCEPTID", "ENGINECONCEPT__LOCALCONCEPTID", "inner.join");
   ...
   * @return
   */
  public static List<String[]> getConceptualToLogicalToPhysicalModel(
      List<String> conceptualNames, Collection<String> databaseFilters) {
    RDBMSNativeEngine engine = (RDBMSNativeEngine) Utility.getDatabase(Constants.LOCAL_MASTER_DB);
  ...
  public static Map<String, List<String>> databaseTranslator(String sourceDB, String targetDB) {
    ...
    try {
      String query =
          "SELECT e.engineName as sourceEngine, c.conceptualName as sourceConceptual, ec.physicalName as sourcePhysical, c.logicalName, "
  ...
  //					+ Constants.PHYSICAL_NAME_ID + " = \'" + localConceptID + "\' and " + Constants.KEY + " =
  // \'" + key
  //					+ "\';";
  //			stmt = conn.createStatement();
  ...
  //	}
  //
  //	public static boolean deleteMetaValue(String engineName, String concept, String key, String
  // value) {
  //		boolean deleted = false;
  ...
  public static Map<String, Object> getXrayExisitingMetamodelRDBMS(String engineName) {
    ...
    String nodeQuery =
        "SELECT c.conceptualname, ec.physicalname, ec.localconceptid, ec.physicalnameid, ec.parentphysicalid, ec.property FROM "
    ...
    try (PreparedStatement statement = conn.prepareStatement(nodeQuery)) {
      //			String nodeQuery = "select c.conceptualname, ec.physicalname, ec.localconceptid,
      // ec.physicalnameid, ec.parentphysicalid, ec.property from "
      //					+ "engineconcept ec, concept c, engine e " + "where ec.engine=e.id " + "and
      // e.enginename='"
    ...
    String edgeQuery =
        "SELECT er.sourceconceptid, er.targetconceptid FROM ENGINERELATION er, engine e WHERE e.id=er.engine AND "
            + "e.enginename = ?";
    try (PreparedStatement statement = conn.prepareStatement(edgeQuery)) {
      ...
      //			String edgeQuery = "SELECT er.sourceconceptid, er.targetconceptid FROM ENGINERELATION er,
      // engine e where e.id = er.engine and "
  ...
  /**
   * It is your responsibility to close the connection object if connection pooling and using this
   * method
   *
   * @param databaseId
   * @param positions
   ...
   * @throws Exception
   */
  public static void saveMetamodelPositions(
      String databaseId, Map<String, Object> positions, Connection conn) throws Exception {
  ...
  public static Map<String, Object> getMetamodelPositions(String databaseId) {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter("METAMODELPOSITION__ENGINEID", "==", databaseId));
