...
public class PushAssetToS3Reactor extends AbstractReactor {
	...
	public PushAssetToS3Reactor() {
		this.keysToGet = S3Utils.addCommonS3Keys(
				new String[]{ReactorKeysEnum.FILE_PATH.getKey(), ReactorKeysEnum.SPACE.getKey(), BUCKET});
	}
		...
		if (relativeAssetPath != null && relativeAssetPath.length() > 0) {
			...
			try {
				if (Files.isDirectory(assetToPush)) {
					MultipleFileUpload xfer = xfer_mgr.uploadDirectory(bucketName, relativeAssetPath,
							assetToPush.toFile(), true);
