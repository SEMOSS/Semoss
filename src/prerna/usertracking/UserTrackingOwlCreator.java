...
public class UserTrackingOwlCreator {
	...
	private void createColumnsAndTypes(AbstractSqlQueryUtil queryUtil) {
		...
		this.userTrackingColumns = Arrays.asList(Pair.with("SESSIONID", "VARCHAR(255)"),
				Pair.with("USERID", "VARCHAR(255)"), Pair.with("TYPE", "VARCHAR(255)"),
				Pair.with("CREATED_ON", TIMESTAMP_DATATYPE_NAME), Pair.with("ENDED_ON", TIMESTAMP_DATATYPE_NAME),
				Pair.with("IP_ADDR", "VARCHAR(255)"), Pair.with("IP_LAT", "VARCHAR(255)"),
				Pair.with("IP_LONG", "VARCHAR(255)"), Pair.with("IP_COUNTRY", "VARCHAR(255)"),
				Pair.with("IP_STATE", "VARCHAR(255)"), Pair.with("IP_CITY", "VARCHAR(255)"));

		this.engineViewsColumns = Arrays.asList(Pair.with("ENGINEID", "VARCHAR(255)"), Pair.with("DATE", "DATE"),
				Pair.with("VIEWS", "INT"));

		this.engineUsesColumns = Arrays.asList(Pair.with("ENGINEID", "VARCHAR(255)"),
				Pair.with("INSIGHTID", "VARCHAR(255)"), Pair.with("PROJECTID", "VARCHAR(255)"),
				Pair.with("DATE", "DATE"));

		this.userCatalogVotes = Arrays.asList(Pair.with("USERID", "VARCHAR(255)"), Pair.with("TYPE", "VARCHAR(255)"),
				Pair.with("ENGINEID", "VARCHAR(255)"), Pair.with("VOTE", "INT"),
				Pair.with("LAST_MODIFIED", TIMESTAMP_DATATYPE_NAME));

		this.emailTracking = Arrays.asList(Pair.with("ID", "VARCHAR(255)"),
				Pair.with("SENT_TIME", TIMESTAMP_DATATYPE_NAME), Pair.with("SUCCESSFUL", BOOLEAN_DATATYPE_NAME),
				Pair.with("E_FROM", "VARCHAR(255)"), Pair.with("E_TO", CLOB_DATATYPE_NAME),
				Pair.with("E_CC", CLOB_DATATYPE_NAME), Pair.with("E_BCC", CLOB_DATATYPE_NAME),
				Pair.with("E_SUBJECT", "VARCHAR(1000)"), Pair.with("BODY", CLOB_DATATYPE_NAME),
				Pair.with("ATTACHMENTS", CLOB_DATATYPE_NAME), Pair.with("IS_HTML", BOOLEAN_DATATYPE_NAME));

		this.insightOpens = Arrays.asList(Pair.with("INSIGHTID", "VARCHAR(255)"), Pair.with("USERID", "VARCHAR(255)"),
				Pair.with("OPENED_ON", TIMESTAMP_DATATYPE_NAME), Pair.with("ORIGIN", "VARCHAR(2000)"));

		this.queryTrackingColumns = Arrays.asList(Pair.with("ID", "VARCHAR(255)"), Pair.with("USERID", "VARCHAR(255)"),
				Pair.with("USERTYPE", "VARCHAR(255)"), Pair.with("DATABASEID", "VARCHAR(255)"),
				Pair.with("QUERY_EXECUTED", CLOB_DATATYPE_NAME), Pair.with("START_TIME", TIMESTAMP_DATATYPE_NAME),
				Pair.with("END_TIME", TIMESTAMP_DATATYPE_NAME), Pair.with("TOTAL_EXECUTION_TIME", "BIGINT"),
				Pair.with("FAILED_EXECUTION", BOOLEAN_DATATYPE_NAME));

		this.allSchemas = Arrays.asList(Pair.with("USER_TRACKING", userTrackingColumns),
				Pair.with("ENGINE_VIEWS", engineViewsColumns), Pair.with("ENGINE_USES", engineUsesColumns),
				Pair.with("USER_CATALOG_VOTES", userCatalogVotes), Pair.with("EMAIL_TRACKING", emailTracking),
				Pair.with("INSIGHT_OPENS", insightOpens), Pair.with("QUERY_TRACKING", queryTrackingColumns));
	}
