...
public class EngineActivityReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    //		return new NounMetadata(engineActivity, PixelDataType.MAP,
    // PixelOperationType.ENGINE_ACTIVITY);
    NounMetadata noun =
        new NounMetadata(engineActivity, PixelDataType.MAP, PixelOperationType.ENGINE_ACTIVITY);
    if (addwarning) {
      noun.addAdditionalReturn(
          getWarning("Update reactor syntax to use engine= instead of database="));
    }
    return noun;
  }
  ...
  private void addViewsByDate(Map<String, Object> engineActivity, String databaseId) {
    ...
    Map<String, Integer> vbd =
        viewsByDate.stream().collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
  ...
  private List<Pair<String, String>> usersCanView(
      List<Pair<String, String>> usesInInsights, User user) {
    List<Pair<String, String>> insightsUserCanView = new ArrayList<>();
    insightsUserCanView =
        usesInInsights.stream()
  ...
  private List<Map<String, String>> convertToMap(List<Pair<String, String>> insightsUserCanView) {
    return insightsUserCanView.stream()
        .map(
            s -> {
  ...
  private void addUsesByDate(Map<String, Object> engineActivity, String databaseId) {
    ...
    Map<String, Integer> ubd =
        usesByDate.stream().collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
