...
public class UserActivitySQLiteReactor extends AbstractQueryStructReactor {
  ...
  // UserActivity ( ) | Import ( frame = [ CreateFrame ( frameType = [ GRID ] , override = [ true ]
  // ) .as ( [ "FRAME961184" ] ) ] ) ;
  // Frame ( frame = [ FRAME961184 ] ) | UserActivity ( ) | AutoTaskOptions ( panel = [ "0" ] ,
  // layout = [ "Grid" ] ) | Collect ( 2000 ) ;

  // date format function example
  // Frame ( frame = [ FRAME961184 ] ) | Select ( DateFormat ( "%Y-%m-%d" , DATE_CREATED ) ) |
  // CollectAll ( ) ;
