...
public class UserTrackingUtils {
   ...
   * @param projectId
   */
  public static void addEngineUsage(
      Set<String> queriedDatabaseIds, String insightId, String projectId) {
   ...
   * @param projectId
   */
  public static void updateEngineUsage(
      Set<String> queriedDatabaseIds, String insightId, String projectId) {
    if (Utility.isUserTrackingEnabled()) {
  ...
  private static void doTrackInsightOpen(String insightId, String userId, String origin) {
    String query =
        "INSERT INTO INSIGHT_OPENS (INSIGHTID, USERID, OPENED_ON, ORIGIN) " + "VALUES (?, ?, ?, ?)";
   ...
   * @param successful
   */
  private static void doTrackEmail(
      String[] toRecipients,
      String[] ccRecipients,
      String[] bccRecipients,
      String from,
    ...
    String query =
        "INSERT INTO EMAIL_TRACKING (ID, SENT_TIME, SUCCESSFUL, E_FROM, E_TO, E_CC, E_BCC, E_SUBJECT, BODY, ATTACHMENTS, IS_HTML) "
            + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    PreparedStatement ps = null;
    try {
      ps = userTrackingDb.getPreparedStatement(query);
   ...
   * @return
   */
  public static void trackQueryExecution(
      User user,
      String databaseId,
      ...
      boolean fail) {
    if (Utility.isUserTrackingEnabled()) {
      doTrackQueryExecution(
          user, databaseId, queryExecuted, startTime, endTime, executionTime, fail);
    }
  }
   ...
   * @return
   */
  private static void doTrackQueryExecution(
      User user,
      String databaseId,
      String queryExecuted,
      Timestamp startTime,
    ...
    try {
      ...
      userTrackingDb
          .getQueryUtil()
          .handleInsertionOfClob(ps.getConnection(), ps, queryExecuted, index++, new Gson());
