...
public class VirusScannerFactory {
	...
	private static final Logger classLogger = LogManager.getLogger(VirusScannerFactory.class);
	...
	public static IVirusScanner getVirusScannerConnector() {
		...
		if (scanType.equals(IVirusScanner.VIRUS_SCANNER_TYPE.CLAM_AV.toString())) {
			...
			classLogger.warn("Using deprecated value - please update parameter value for {} to CLAM_AV",
					Constants.VIRUS_SCANNING_METHOD);
			return ClamAVScannerUtils.getInstance();
		} else {
			classLogger.warn("Virus Scanning is enabled but could not find type for input = '{}'", scanType);
			return null;
		}
	}
}
