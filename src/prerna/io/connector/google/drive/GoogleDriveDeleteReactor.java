...
public class GoogleDriveDeleteReactor extends AbstractGoogleReactor {
	...
	protected NounMetadata executeAuthenticated() {
		...
		if (fileId == null || fileId.trim().isEmpty()) {
			throw new SemossPixelException("File ID is required to delete a Google Drive file.");
		}
		try {
			...
			classLogger.error("Error while deleting a Google Drive file", e);
			throw e;
		} catch (Exception e) {
			classLogger.error("Failed to delete a Google Drive file", e);
	...
	public String getReactorDescription() {
		return "Delete a file from the user's Google Drive.";
	}

	@Override
	protected String getDescriptionForKey(String key) {
		if (key.equals(ReactorKeysEnum.ID.getKey())) {
			return "Google Drive file ID for the file to delete.";
		}
