...
public class UsaJobsUtil {
	...
	// //test.runStateSearch(new String[]{"CA", "CO", "CT", "GA", "IN", "KY", "MN",
	// "MI", "NV", "NH",
	// "NY", "OH", "VA", "WI"});
	...
	// test.runStateSearch(cred, states); //", "CO", "CT", "GA", "IN", "KY", "MN",
	// "MI", "NV", "NH",
	// "NY", "OH", "VA", "WI"});
	// }
	...
	public void runStateSearch(List credentials, List states) {
		...
		try {
			...
			// Position ID -
			// SearchResult.SearchResultItems[*].MatchedObjectDescriptor.PositionID
			...
			for (int stateIndex = 0; stateIndex < states.size(); stateIndex++) {
				...
				total = -1;
				page = 1;
				url = "https://data.usajobs.gov/api/search?LocationName="
	...
	public String[] processJob(String inputData, boolean text, String state) {
		try {
			...
			path = "SearchResult.SearchResultItems[*].MatchedObjectDescriptor.[PositionID, PositionEndDate, JobGrade[*].Code, PositionLocation[].CountrySubDivisionCode, OrganizationName, PositionRemuneration[*].MinimumRange, PositionRemuneration[*].MaximummumRange, "
					+ "PositionTitle, "
					+ "UserArea.Details.LowGrade, UserArea.Details.HighGrade, UserArea.Details.HiringPath, UserArea.Details.Relocation, UserArea.Details.OrganizationCodes]"; // hiring
																																												// path
																																												// (array)
																																												// -public
																																												// or
			...
			if (result instanceof ArrayNode) {
				...
				// if(parNode.size() > 0)
				{
					...
					// for(int recIndex = 0;recIndex < arrNode.size();recIndex++)
					{
						...
						writeRecord(record.toString());
						// System.out.println(recCount + " >> Position " + positionID + " Title " +
						// title + "
						// end Date " + positionEndDate + " Organization " + org + " Min Amount "+
						// minAmount +
	...
	public void getBudgetByTime() {
		...
		// you can see how much money remains in every agency - there is no time to it -
		// but this can
		// tell you how much money remains - You can see what the total is and then
		// prorate it based on
		// state fluctuations
		 ...
		 * 
		 * { "filters": { "keywords": ["Filter is required"], "time_period": [ {
		 * "start_date": "2017-10-01", "end_date": "2018-09-30" } ] }, "scope":
		 * "place_of_performance", "geo_layer": "state", "geo_layer_filters": ["VA",
		 * "CA"]
		 * 
		 */

		// agency wide spending -
