...
public class UsaJobsUtil {
  ...
  //		//test.runStateSearch(new String[]{"CA", "CO", "CT", "GA", "IN", "KY", "MN", "MI", "NV", "NH",
  // "NY", "OH", "VA", "WI"});
  ...
  //		test.runStateSearch(cred, states); //", "CO", "CT", "GA", "IN", "KY", "MN", "MI", "NV", "NH",
  // "NY", "OH", "VA", "WI"});
  //	}
  ...
  public void runStateSearch(List credentials, List states) {
    ...
    try {
      ...
      // LocationName":"Atlanta, Georgia","CountryCode":"United
      // States","CountrySubDivisionCode":"Georgia","CityName":"Atlanta, Georgia"
      ...
      // jmes path
      // SearchResult.SearchResultItems[*].MatchedObjectDescriptor.[PositionID, PositionEndDate,
      // JobGrade[0].Code, length(PositionLocation)]
      ...
      for (int stateIndex = 0; stateIndex < states.size(); stateIndex++) {
        ...
        url =
            "https://data.usajobs.gov/api/search?LocationName="
                + URLEncoder.encode(state, java.nio.charset.StandardCharsets.UTF_8.toString());
        /*
        ...
        do {
          ...
          if (output != null) processJob(output, true, state);
        } while (output != null);
      }
      closeFile();
  ...
  private ObjectMapper getMapper() {
    if (this.mapper == null) mapper = new ObjectMapper();
  ...
  public String[] processJob(String inputData, boolean text, String state) {
    try {
      ...
      String path =
          "SearchResult.SearchResultItems[*].MatchedObjectDescriptor.[PositionID, PositionEndDate, JobGrade[0].Code, PositionLocation[*]]";
      // hiring path says if it is open to publix or not
      path =
          "SearchResult.SearchResultItems[*].MatchedObjectDescriptor.[PositionID, PositionEndDate, JobGrade[*].Code, PositionLocation[].CountrySubDivisionCode, OrganizationName, PositionRemuneration[*].MinimumRange, PositionRemuneration[*].MaximummumRange, "
              + "PositionTitle, "
      ...
      if (result instanceof ArrayNode) {
        ...
        // if(parNode.size() > 0)
        {
          ...
          // for(int recIndex = 0;recIndex < arrNode.size();recIndex++)
          {
            ...
            writeRecord(record.toString());
            // System.out.println(recCount + " >>  Position " + positionID + " Title  " + title + "
            // end Date " + positionEndDate +  "  Organization " + org + " Min Amount "+ minAmount +
  ...
  private String formatArray(String arrayString, boolean doQuote) {
    String quote = "\"";
    if (!doQuote) quote = "";
    if (doQuote) arrayString = arrayString.replace("\"", "'");
    else arrayString = arrayString.replace("\"", "");
  ...
  public void getBudgetByTime() {
    ...
    // you can see how much money remains in every agency - there is no time to it - but this can
    // tell you how much money remains - You can see what the total is and then prorate it based on
    // state fluctuations
