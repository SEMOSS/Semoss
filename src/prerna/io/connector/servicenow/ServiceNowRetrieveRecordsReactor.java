...
public class ServiceNowRetrieveRecordsReactor extends AbstractReactor {
	...
	public String getReactorDescription() {
		return "Retrieve records present in a ServiceNow table via OAuth authentication.";
	}
	
	@Override
	protected String getDescriptionForKey(String key) {
		if (key.equals(ReactorKeysEnum.TABLE.getKey())) {
			return "Required ServiceNow table name from where the records will be retrieved.";
		}else if (key.equals(ReactorKeysEnum.LIMIT.getKey())) {
			return "Required number of records user wants to retrieve from a ServiceNow table.";
