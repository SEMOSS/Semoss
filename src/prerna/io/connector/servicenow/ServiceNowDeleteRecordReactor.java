...
public class ServiceNowDeleteRecordReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		try {
			...
			Map<String, Object> deleteRecord = ServiceNowHelper.deleteRecord(instanceURL, accessToken, table, sysId);
			return new NounMetadata(deleteRecord, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.OPERATION);
		} catch (Exception e) {
			classLogger.error("Error deleting ServiceNow record.", e);
	...
	public String getReactorDescription() {
		return "Deletes a record from a ServiceNow table via OAuth authentication.";
	}

	@Override
	protected String getDescriptionForKey(String key) {
		if (key.equals(ReactorKeysEnum.TABLE.getKey())) {
			return "Required ServiceNow table name from which the record will be deleted.";
		} else if (key.equals(SYS_ID)) {
			return "Required sys_id of the record to delete in the ServiceNow table.";
