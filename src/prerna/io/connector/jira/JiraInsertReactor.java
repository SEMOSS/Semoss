...
public class JiraInsertReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		try {
			...
			profileId = readData(jiraDB, tableName, userId, apiToken, url, date, lused, keyName);
	...
	private String readData(IRDBMSEngine jiraDB, String tableName, String userId, String apiKey, String url,
			Timestamp date, Timestamp lused, String keyName) {
		...
		String query = "SELECT " + JIRA_UNIQUE_ID + " FROM " + tableName
				+ " WHERE USER_ID=? AND API_KEY=? AND URL=? AND DATE_CREATED=? AND DATE_LAST_USED=? AND KEY_NAME=?";
