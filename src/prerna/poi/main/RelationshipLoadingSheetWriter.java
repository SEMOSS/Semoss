...
/**
 * Create an excel file listing all the instance relationships and relationship properties for type
 * triple specified on the DB Modification tab, Export Data section Can export a maximum of 9
 * different relationships, stored in different tabs, at one time
 */
public class RelationshipLoadingSheetWriter {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(RelationshipLoadingSheetWriter.class);

  public void writeRelationshipLoadingSheets(
      IDatabaseEngine engine, ArrayList<String[]> relationships) {
    ...
    for (int row = 0; row < data.size(); row++) {
      ...
      if (valFirstColumn != null
          && !valFirstColumn.equals("")
          && !valFirstColumn.startsWith("-")
          && !valFirstColumn.endsWith("-")
          && !valFirstColumn.endsWith("None")) {
        ...
        for (int col = 0; col < data.get(row).length; col++) {
          ...
          if (val != null) cell.setCellValue(data.get(row)[col].replace("\"", ""));
        }
      }
    }
    ...
    if (showSuccessMessage) Utility.showMessage("Exported relationships successfully: " + fileLoc);
  }

  private Hashtable<String, Vector<String[]>> queryData(
      IDatabaseEngine engine, ArrayList<String[]> relationships) {
    ...
    for (String[] spo : relationships) {
      ...
      if (!(subjectNodeType.equals(this.NONE_SELECTED)
          || objectNodeType.equals(this.NONE_SELECTED)
        ...
        query +=
            "{?out <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/";
        ...
        query +=
            "OPTIONAL { {?contains <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Relation/Contains> ;} {?relationship ?contains ?prop ;} } }";
        ...
        hash.put(
            output, formatData(properties, list, subjectNodeType, objectNodeType, relationship));
      }
    }
    return hash;
  }
   ...
   */
  private Vector<String[]> formatData(
  ...
  /**
   * Reorganize the data from querying the engine into a format similar to a Microsoft Excel Loading
   * Sheet
   *
   * @param oldHash Hashtable containing the data from ExportRelationshipsLoadSheetsListener, which
   *     gets the data from querying the engine
   * @return newHash Hashtable<String,Vector<String[]>> containing the information in a format
   *     similar to the Microsoft Excel Loading Sheet
   */
  public Hashtable<String, Vector<String[]>> prepareLoadingSheetExport(Hashtable oldHash) {
    ...
    while (keyIt.hasNext()) {
      ...
      if (!key.startsWith("-") && !key.endsWith("-") && !key.endsWith("None")) {
        ...
        String[] oldSecondRow =
            new String[oldHeaderRow.length]; // this is in case the sheet is null (other than
        // the headers)
        if (sheetV.size() > 2)
  ...
  /**
   * Writes the sheet containing the information for all the relationship instances and properties
   * for the type triple specified by the user
   *
   * @param key String containing the name of the sheet to write
   * @param sheetVector Vector<String[]> containing the data, all the relationship instance and
