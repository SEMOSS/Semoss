...
/**
 * Create an excel file listing all the instance nodes and their properties for a type node
 * specified on the DB Modification tab, Export Data section Can export a maximum of 9 different
 * node types, stored in different tabs, at one time
 */
public class NodeLoadingSheetWriter {
   ...
   * ExportNodeLoadSheetsListener into a format that is similar to a load sheet and saves it in a
   * workbook The data is reorganized in a Hashtable<String, Vector<String[]>> where the keys become
   * the sheet name and the instance data in the format Vector<String[]>
   *
   * @param fileLoc String containing the path location to save the workbook
   * @param hash Hashtable containing the information gotten from ExportNodeLoadSheetsListener,
   *     which gets the data from querying the engine
   * @param engine
   * @param nodeTypes
   */
  ...
  private Hashtable<String, Vector<String[]>> queryData(
      IDatabaseEngine engine, ArrayList<String> nodeTypes) {
    Hashtable<String, Vector<String[]>> hash = new Hashtable<String, Vector<String[]>>();
    for (String nodeType : nodeTypes) {
      ArrayList<Object[]> list = new ArrayList<Object[]>();
      String query =
          "SELECT ?s ?p ?prop WHERE { {?s <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/";
      query += nodeType;
      query +=
          "> ;}OPTIONAL{ {?p <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Relation/Contains> ;} {?s ?p ?prop ;} } }";
        ...
        while (wrapper.hasNext()) {
          ...
          for (int colIndex = 0; colIndex < names.length; colIndex++) {
            if (sjss.getVar(names[colIndex]) != null
                || sjss.getVar(names[colIndex]).toString().equals("")) {
  ...
  private Vector<String[]> formatData(
      String nodeType, HashSet<String> properties, ArrayList<Object[]> list) {
    Collections.sort(
        list,
   ...
   * Sheet
   *
   * @param oldHash Hashtable containing the data from ExportNodeLoadSheetsListener, which gets the
   *     data from querying the engine
  ...
  private Hashtable<String, Vector<String[]>> prepareLoadingSheetExport(Hashtable oldHash) {
    ...
    while (keyIt.hasNext()) {
      ...
      String[] oldSecondRow =
          new String[oldHeaderRow.length]; // this is in case the sheet is null (other than the
      // headers)
  ...
  /**
   * Writes the sheet containing the information for all the node instances and properties for the
   * node type specified by the user
   *
   * @param key String containing the name of the sheet to write
   * @param sheetVector Vector<String[]> containing the data, all the node instance and properties,
   *     to export in the sheet
