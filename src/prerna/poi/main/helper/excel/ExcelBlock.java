...
public class ExcelBlock {
  ...
  private Map<Integer, SummaryStatistics> columnToRowIndexStats =
      new Hashtable<Integer, SummaryStatistics>();

  // for data types
  private Map<Integer, Map<SemossDataType, SummaryStatistics>> columnToTypeStats =
      new Hashtable<Integer, Map<SemossDataType, SummaryStatistics>>();
  private Map<Integer, Map<String, SummaryStatistics>> additionalFormatTracker =
      new HashMap<Integer, Map<String, SummaryStatistics>>();

  /**
   * Get the ranges of the block This will split up the entire segment if there are empty columns
   *
   * @return
   */
  ...
  public Object[][] getRangeTypes(ExcelRange range) {
    ...
    int colIndex = 0;
    COLUMN_LOOP:
    for (int cellIndex = rangeIndex[0] - 1; cellIndex < rangeIndex[2]; cellIndex++, colIndex++) {
      // get column index
      ...
      if (typesMap.containsKey(SemossDataType.STRING)) {
        containsStr =
            testTypeContainedWtihinRange(startRow, endRow, typesMap.get(SemossDataType.STRING));
      ...
      if (typesMap.containsKey(SemossDataType.INT)) {
        containsInt =
            testTypeContainedWtihinRange(startRow, endRow, typesMap.get(SemossDataType.INT));
      }
      if (typesMap.containsKey(SemossDataType.DOUBLE)) {
        containsDouble =
            testTypeContainedWtihinRange(startRow, endRow, typesMap.get(SemossDataType.DOUBLE));
      }
      if (typesMap.containsKey(SemossDataType.DATE)) {
        containsDate =
            testTypeContainedWtihinRange(startRow, endRow, typesMap.get(SemossDataType.DATE));
      }
      if (typesMap.containsKey(SemossDataType.TIMESTAMP)) {
        containsTimestamp =
            testTypeContainedWtihinRange(startRow, endRow, typesMap.get(SemossDataType.TIMESTAMP));
      }
      ...
      if (!containsInt && !containsDouble && (containsDate || containsTimestamp)) {
        ...
        String mostOccuringFormat =
            Collections.max(
                    mostPopularFormat.entrySet(), Comparator.comparingInt(Map.Entry::getValue))
                .getKey();
  ...
  public boolean sameAs(ExcelBlock newBlock) {
    ...
    double startDiff =
        Math.abs(this.startColumnIndexStats.getMean() - newBlock.startColumnIndexStats.getMean());
    double endDiff =
        Math.abs(this.totalColumnsInRowStats.getMean() - newBlock.totalColumnsInRowStats.getMean());
  ...
  public void addColumnToRowIndexWithData(
      int columnIndex, int rowIndex, SemossDataType type, String additionalType) {
