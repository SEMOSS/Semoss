...
public class ExcelSheetFileIterator extends AbstractFileIterator {
  ...
  /** Since we have types in excel We will use a better version for getting the clean types */
  @Override
      ...
      if (type == SemossDataType.STRING) {
        ...
        if (val instanceof SemossDate) {
          if (additionalFormatting != null) {
            cleanRow[i] =
                new SemossDate(((SemossDate) val).getZonedDateTime(), additionalFormatting);
          } else {
            cleanRow[i] = val;
          }
        } else {
  ...
  private void setSelectors(List<IQuerySelector> qsSelectors) {
    ...
    // order headers
    List<Integer> headerIndiciesList =
        Arrays.stream(headerIndices).boxed().collect(Collectors.toList());
  ...
  private void setUnknownTypes() {
    ...
    Map[] predictionMaps =
        FileHelperUtil.generateDataTypeMapsFromPrediction(this.headers, prediction);
  ...
  private int[] findHeaderIndicies(String[] sheetHeaders, String[] headers) {
    ...
    for (int colIdx = 0; colIdx < numHeadersToGet; colIdx++) {
      ...
      indicesToGet[colIdx] =
          ArrayUtilityMethods.arrayContainsValueAtIndex(sheetHeaders, headerToGet) + startCol;
    }
