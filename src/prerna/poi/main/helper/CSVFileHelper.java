...
public class CSVFileHelper {
  ...
  /**
   * Get all the headers used in the csv file This is the clean version of the csv headers
   *
   * @return
   */
  ...
  public Object[][] predictTypes() {
    ...
    for (int colIndex = 0; colIndex < numCols; colIndex++) {
      ...
      String[] row = null;
      WHILE_LOOP:
      while (rowCounter < NUM_ROWS_TO_PREDICT_TYPES && (row = parser.parseNext()) != null) {
        String val = row[0];
        // ignore empty values
        ...
        if (newTypePrediction == SemossDataType.STRING
            || newTypePrediction == SemossDataType.BOOLEAN) {
  ...
  /**
   * Get each csv column name as it exists in the csv file to the valid csv header that we create on
   * the BE If the name is already valid, it just points to itself
   *
   * @return
   */
  ...
  /**
   * This will return the original headers of the csv file Before we performed any cleaning of data
   *
   * @return
   */
  ...
  /**
   * Gets the properly formatted object from the string[] values object Also handles if the column
   * is a concatenation
   *
  ...
  public static Object createObject(
      String object, String[] values, SemossDataType[] dataTypes, List<String> headers) {
