...
public class AskRoomPromptReactor extends AbstractReactor {
  ...
  private static final String CHAIN_OF_THOUGHT_PROMPT =
      "You are an AI assistant that always reasons step by step before answering."
  ...
  public NounMetadata execute() {
    ...
    List<Map<String, Object>> roomActiveOutput =
        ModelInferenceLogsUtils.getUserActiveRooms(roomId, user.getPrimaryLoginToken().getId());
    ...
    if (modelResponse.getMessageType().equalsIgnoreCase(AskModelEngineResponse.TOOL)
        && executeTool) {
      modelResponse =
          executeToolAndSendToModel((AskToolModelEngineResponse) modelResponse, modelEngine);
    }
    //
    //		// record the output
    //		String messageId =
    // PlaygroundUtils.getInstance().recordRoomMessage(UUID.randomUUID().toString(), roomId,
    //				modelId, paramMap, context, question, modelResponse.toMap(),
  ...
  private Map<String, Object> processModelResponse(
      AskModelEngineResponse modelResponse, IModelEngine modelEngine) {
    Map<String, Object> output = new HashMap<String, Object>();
    output.put("response", new ArrayList<Map<String, Object>>());
    if (modelResponse.getMessageType().equalsIgnoreCase(AskModelEngineResponse.TOOL)) {
      ...
      for (ToolResponse tool : tools) {
        ...
        for (Entry<String, Object> functionParam :
            ((Map<String, Object>) toolArguments.get("map")).entrySet()) {
      ...
      //                toolType = "PROJECT";
      //            } else {
      //                IFunctionEngine function = Utility.getFunctionEngine((String)
      // functionParams.get("id"));
      //                toolName = function.getEngineName();
      //                toolType = "FUNCTION";
      //            }
      //
      //            // object to store params needed to call the tool
      //            List<HashMap<String, Object>> toolCallInfoData = new ArrayList<HashMap<String,
      // Object>>();
      //            for(Entry<String, Object> functionParam : ((Map<String,
      // Object>)functionParams.get("map")).entrySet()){
      //                HashMap<String, Object> paramInfo = new HashMap<String, Object>();
      //                paramInfo.put("name", functionParam.getKey());
      //                paramInfo.put("type", functionParam.getValue().getClass().getSimpleName());
      //                paramInfo.put("value", functionParam.getValue());
      ...
      ProcessedResponse processedResponse =
          processMarkdownCodeBlocks(modelResponse.getStringResponse(), modelEngine);
      ...
      if (!processedResponse.getCodeBlocks().isEmpty()) {
        ...
        while (matcher.find()) {
          ...
          String textChunk =
              processedResponse.getModifiedResponse().substring(lastIndex, start).trim();
          ...
          if (matcher.find()) {
            nextChunk =
                processedResponse
                    .getModifiedResponse()
                    .substring(matcher.start(), matcher.start())
                    .trim();
  ...
  private AskModelEngineResponse executeToolAndSendToModel(
      AskToolModelEngineResponse toolResponse, IModelEngine modelEngine) {
