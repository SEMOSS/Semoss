...
public class SymlinkHelper {
  ...
  public SymlinkHelper(String targetDirName) {
    ...
    if (!targetDir.exists()) {
      classLogger.info(
          "User chroot folder doesn't exist. Making folder now at: " + userChrootFolder);
  ...
  private void setupBashForChroot() {
    try {
      ...
      String[] coreutilsCommands = {
        "/bin/bash", "/bin/sh", "/bin/ls", "/bin/echo", "/usr/bin/coreutils"
      };
      ...
      classLogger.info(
          "Bash and essential commands setup completed for chroot: " + userChrootFolder);
    } catch (Exception e) {
  ...
  private void copySystemBinary(String binaryPath) {
    try {
      ...
      Path targetBinary =
          Paths.get(userChrootFolder, binaryPath.substring(1)); // Remove leading slash
  ...
  private void copyLibraryIfExists(String libraryPath) {
    try {
      ...
      Path targetLib =
          Paths.get(userChrootFolder, libraryPath.substring(1)); // Remove leading slash
  ...
  /**
   * Setup Git inside the chroot by copying the binary, its dependencies, and required helper
   * programs for both RHEL/UBI and Ubuntu systems.
   */
    ...
    try {
      ...
      if (Files.exists(chrootDir)) {
        ...
        if (exitCode == 0) {
          ...
          classLogger.warn(
              "Could not set ownership for: " + relativePath + " (exit code: " + exitCode + ")");
        }
      }
    } catch (Exception e) {
