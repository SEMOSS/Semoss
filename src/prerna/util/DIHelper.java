...
/**
 * DIHelper is used throughout SEMOSS to obtain property names from core propfiles and engine core
 * propfiles.
 */
public final class DIHelper {
  ...
  /**
   * Set up shapes, colors, and layouts. Put properties for each in a hashtable of local properties.
   *
      ...
      if (list != null) {
        ...
        if (repos != null && repos.length > 0) {
          IDatabaseEngine engine =
              (IDatabaseEngine) DIHelper.getInstance().getLocalProp(repos[0] + "");
  ...
  public Shape createStarL() {
    // double points[][] = {{7.5,0} ,{9,5} ,{14.5, 5}, {11, 9}, {12.5, 14}, {7.2, 10.5}, {2.2, 14},
    // {3.5, 9}, {0,5}, {5, 5}, {7.5, 0}};
    double points[][] = {
      {10, 0},
      {13, 6.66},
  ...
  public Shape createHexL() {
    ...
    for (int k = 1; k < points.length; k++) pent.lineTo(points[k][0], points[k][1]);
  ...
  public Shape createPent(final double s) {
    ...
    for (int i = 0; i < 5; i++)
      hexagon.lineTo(
          (float) Math.cos(i * 2 * Math.PI / 5 + Math.PI / 10) * s,
  ...
  public Shape createRhombus(final double s) {
    ...
    for (int k = 1; k < points.length; k++) r.lineTo(points[k][0], points[k][1]);
  ...
  public Shape createRhombusL() {
    ...
    for (int k = 1; k < points2.length; k++) r.lineTo(points2[k][0], points2[k][1]);
