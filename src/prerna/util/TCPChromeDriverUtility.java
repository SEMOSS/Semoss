...
public class TCPChromeDriverUtility {
  ...
  public static Object makeChromeDriver(String feUrl, String url, int height, int width) {
    if (TCPChromeDriverUtility.driver == null) {
      ...
      String sysProp =
          baseFolder + DIR_SEPARATOR + "config" + DIR_SEPARATOR + "Chromedriver" + DIR_SEPARATOR;
  ...
  public static String captureImagePersistent(
      Object driverObj,
      String feUrl,
      String url,
      String imagePath,
    ...
    if (TCPChromeDriverUtility.contextPath != null) {
      ...
      String baseUrl =
          startingUrl.substring(0, startingUrl.lastIndexOf("/") + 1)
  ...
  protected static void updateCookie(ChromeDriver driver, String cookieName, String cookieValue) {
    ...
    if (cookieFound) {
      logger.info(
          "##CHROME DRIVER: found cookie - Name "
              + cook.getName()
              + " domain: "
              + cook.getDomain()
              + " path: "
      ...
      Cookie name =
          new Cookie(
              ...
              cook.isHttpOnly());
      logger.info(
          "##CHROME DRIVER: Adding cookie  - name: "
              + name.getName()
              + " domain: "
              + name.getDomain()
              + " path: "
      ...
      //					true);
      logger.info(
          "##CHROME DRIVER: BASE ADD Adding cookie  - name: "
              + name.getName()
              + " domain: "
              + name.getDomain()
              + " path: "
  ...
  public static String captureDataPersistent(
      Object driverObj, String feUrl, String url, String sessionId, int waitTime) {
    // need to go to the base url first
    // so that the cookie is applied at root level
    ...
    WebDriverWait wait =
        new WebDriverWait(
            TCPChromeDriverUtility.driver,
            600); // giving it 10 min.. we need a better way for this but..
    WebElement we1 =
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//html/body//table")));
  ...
  //	 */
  //	public static ChromeDriver captureImage(String feUrl, String url, String imagePath, String
  // sessionId, int height,
  //			int width, boolean close) {
  //		String baseFolder = DIHelper.getInstance().getProperty(Constants.BASE_FOLDER);
  //		// load driver options
  //		String os = System.getProperty("os.name").toUpperCase();
  //		String sysProp = baseFolder + DIR_SEPARATOR + "config" + DIR_SEPARATOR + "Chromedriver" +
  // DIR_SEPARATOR;
  //		boolean linux = false;
  //		if (os.contains("WIN")) {
  //			sysProp += "chromedriver-win.exe";
  ...
  //			//			Cookie name = new Cookie(ChromeDriverUtility.sessionCookie, sessionId, null, "/", null,
  // secure, true);
  //			Cookie name = new Cookie(ChromeDriverUtility.sessionCookie, sessionId, "/");
  //
  //			driver.manage().addCookie(name);
  ...
  public static String getHTML(Object driverObj, String path) {
    ...
    String html2 =
        TCPChromeDriverUtility.driver.executeScript("return arguments[0].outerHTML;", we) + "";
    return html2;
  }
  ...
  //		ChromeDriver driver =
  // (ChromeDriver)TCPChromeDriverUtility.makeChromeDriver("https://www.buzzfeed.com/hbraga/best-gifts-2020",
