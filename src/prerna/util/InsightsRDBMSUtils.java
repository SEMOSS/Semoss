...
public final class InsightsRDBMSUtils {
	...
	public static void runInsightCreateTableQueries(RDBMSNativeEngine insightEngine) {
		...
		// VARCHAR(225), QUESTION_LAYOUT VARCHAR(225), QUESTION_ORDER INT,
		// QUESTION_DATA_MAKER
		// VARCHAR(225), QUESTION_MAKEUP CLOB, QUESTION_PROPERTIES CLOB, QUESTION_OWL
		// CLOB,
		// QUESTION_IS_DB_QUERY BOOLEAN, DATA_TABLE_ALIGN VARCHAR(500), QUESTION_PKQL
		// ARRAY)
		...
		try {
			if (!queryUtil.tableExists(insightEngine.getConnection(), InsightAdministrator.TABLE_NAME,
					insightEngine.getDatabase(), insightEngine.getSchema())) {
				columns = new String[]{InsightAdministrator.QUESTION_ID_COL, InsightAdministrator.QUESTION_NAME_COL,
						"QUESTION_PERSPECTIVE", "QUESTION_LAYOUT", "QUESTION_ORDER", "QUESTION_DATA_MAKER",
						"QUESTION_MAKEUP", "DATA_TABLE_ALIGN", InsightAdministrator.HIDDEN_INSIGHT_COL,
						InsightAdministrator.CACHEABLE_COL, InsightAdministrator.CACHE_MINUTES_COL,
						InsightAdministrator.CACHE_CRON_COL, InsightAdministrator.CACHED_ON_COL,
						InsightAdministrator.CACHE_ENCRYPT_COL, InsightAdministrator.QUESTION_PKQL_COL,
				...
				types = new String[]{"VARCHAR(50)", "VARCHAR(255)", "VARCHAR(255)", "VARCHAR(255)", "INT",
						...
						"VARCHAR(25)", TIMESTAMP_DATATYPE, BOOLEAN_DATATYPE, "ARRAY", "VARCHAR(255)"};
				// this is annoying
...

		{
			...
			// VARCHAR(255),
			// PARAMETER_TYPE VARCHAR(225), PARAMETER_DEPENDENCY VARCHAR(225),
			// PARAMETER_QUERY
			// VARCHAR(2000), PARAMETER_OPTIONS VARCHAR(2000), PARAMETER_IS_DB_QUERY
			// BOOLEAN,
			...
			try {
				if (!queryUtil.tableExists(insightEngine.getConnection(), "PARAMETER_ID", insightEngine.getDatabase(),
						insightEngine.getSchema())) {
					columns = new String[]{"PARAMETER_ID", "PARAMETER_LABEL", "PARAMETER_TYPE", "PARAMETER_DEPENDENCY",
							"PARAMETER_QUERY", "PARAMETER_OPTIONS", "PARAMETER_IS_DB_QUERY", "PARAMETER_MULTI_SELECT",
							"PARAMETER_COMPONENT_FILTER_ID", "PARAMETER_VIEW_TYPE", "QUESTION_ID_FK"};
					types = new String[]{"VARCHAR(255)", "VARCHAR(255)", "VARCHAR(255)", "VARCHAR(255)",
							"VARCHAR(2000)", "VARCHAR(2000)", BOOLEAN_DATATYPE, BOOLEAN_DATATYPE, "VARCHAR(255)",
							"VARCHAR(255)", "INT"};
	 ...
	 * @return
	 */
	public static final Map<String, Object> getNewInsightDatabaseConnectionPropValues(RdbmsTypeEnum rdbmsType,
		...
		if (rdbmsType == RdbmsTypeEnum.SQLITE) {
			...
			hostname = baseFolder + SmssUtilities.getUniqueName(projectName, projectId) + DIR_SEPARATOR
