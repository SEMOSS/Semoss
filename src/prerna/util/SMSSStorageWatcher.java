 ...
 */
public class SMSSStorageWatcher extends AbstractFileWatcher {

	private static final Logger logger = LogManager.getLogger(SMSSStorageWatcher.class);
	...
	public static String loadNewEngine(String newFile, String folderToWatch) {
		...
		try{
			...
			if(engines.startsWith(engineId) || engines.contains(";"+engineId+";") || engines.endsWith(";"+engineId)) {
				logger.debug("Storage " + folderToWatch + "<>" + newFile + " is already loaded...");
			} else {
	...
	public void loadFirst() {
		...
		for (int fileIdx = 0; fileIdx < fileNames.length; fileIdx++) {
			try {
				...
				logger.fatal("Storage engine Failed " + folderToWatch + "/" + fileNames[fileIdx]);
			}
		}
		
		// remove unused databases
		if (!ClusterUtil.IS_CLUSTER) {
			List<String> engines = SecurityEngineUtils.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.STORAGE.toString()));
	...
	@Override
	public void run() {
		logger.info("Starting SMSSStorageWatcher thread");
