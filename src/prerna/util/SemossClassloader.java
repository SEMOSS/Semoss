...
public class SemossClassloader extends ClassLoader {
  ...
  /**
   * Every request for a class passes through this method. If the requested class is in
   * "javablogging" package, it will load it using the {@link CustomClassLoader#getClass()} method.
   * If not, it will use the super.loadClass() method which in turn will pass the request to the
   * parent.
   *
   * @param name Full class name
   */
  ...
  /**
   * Loads a given file (presumably .class) into a byte array. The file should be accessible as a
   * resource, for example it could be located on the classpath.
   *
   * <p>TODO: Need to incorporate loading jars. Not right now
   *
