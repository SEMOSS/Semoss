...
public class LdapSearchUserStructureConnection extends AbstractLdapAuthenticator {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(LdapSearchUserStructureConnection.class);
  ...
  public void load() throws IOException {
    ...
    this.applicationMasterCredentials =
        socialData.getProperty(LDAP_APPLICATION_SECURITY_CREDENTIALS);
  ...
  public void validate() throws IOException {
    ...
    if (this.applicationMasterPrincipal == null
      ...
      throw new IllegalArgumentException(
          "Must provide the attribute for the application ldap principal");
    }
    if (this.applicationMasterCredentials == null
        || (this.applicationMasterCredentials = this.applicationMasterCredentials.trim())
            .isEmpty()) {
      throw new IllegalArgumentException(
          "Must provide the attribute for the application ldap credentials");
    }
    try {
      this.applicationContext =
          LDAPConnectionHelper.createLdapContext(
              this.providerUrl, this.applicationMasterPrincipal, this.applicationMasterCredentials);
    } catch (Exception e) {
  ...
  public AccessToken authenticate(String username, String password) throws Exception {
    ...
    NamingEnumeration<SearchResult> findUser =
        applicationContext.search(searchContextName, searchFilter, controls);
    ...
    if (foundUser) {
      // see if we can determine that they cannot signin but actually need to reset their password
      // details
      if (result != null) {
        // this method will throw LDAPPasswordChangeRequiredException if the password needs to be
        // reset to login
        try {
          ...
          classLogger.error(
              "Error occurred seeing if login issue is that user must change password");
          classLogger.error(Constants.STACKTRACE, e);
        }
      }
  ...
  public void updateUserPassword(String username, String curPassword, String newPassword)
    ...
    NamingEnumeration<SearchResult> findUser =
        this.applicationContext.search(searchContextName, searchFilter, controls);
    ...
    try {
      ...
      mods[1] =
          new ModificationItem(DirContext.ADD_ATTRIBUTE, new BasicAttribute("unicodePwd", newPwd));
