...
/**
 * Assumes that the authentication for each user has the same DN structure but only the CN will
 * change based on the user input
 */
public class LdapTemplateStructureConnection extends AbstractLdapAuthenticator {

  private static final Logger classLogger =
      LogManager.getLogger(LdapTemplateStructureConnection.class);
  ...
  public void load() throws IOException {
    ...
    if (securityPrincipalTemplateStr != null && securityPrincipalTemplateStr.contains("***")) {
    ...
    if (this.searchContextName == null
        || (this.searchContextName = this.searchContextName.trim()).isEmpty()) {
  ...
  public void validate() throws IOException {
    ...
    if (this.attributeIdKey == null
        || (this.attributeIdKey = this.attributeIdKey.trim()).isEmpty()) {
    ...
    for (String template : this.securityPrincipalTemplate) {
      if (!template.contains(SECURITY_PRINCIPAL_TEMPLATE_USERNAME)) {
        throw new IllegalArgumentException(
            "Must provide the a location to fill the username passed from the user using "
                + SECURITY_PRINCIPAL_TEMPLATE_USERNAME);
      }
    }

    // do we have anything for custom context to change pwds if users are unable to change
    SocialPropertiesUtil socialData = SocialPropertiesUtil.getInstance();
    this.useCustomContextForPwdChange =
        Boolean.parseBoolean(
            socialData.getProperty(LDAP_USE_CUSTOM_CONTEXT_FOR_PWD_CHANGE_KEY) + "");
    if (this.useCustomContextForPwdChange) {
      ...
      try {
        this.customPwdChangeLdapContext =
            LDAPConnectionHelper.createLdapContext(this.providerUrl, customUsername, customPwd);
      } catch (Exception e) {
  ...
  public AccessToken authenticate(String username, String password) throws Exception {
    ...
    try {
      LDAPConnectionHelper loginObj =
          tryLogins(this.providerUrl, this.securityPrincipalTemplate, username, password);
      ...
      NamingEnumeration<SearchResult> users =
          ldapContext.search(principalDN, searchFilter, controls);
      ...
      while (users.hasMoreElements()) {
        ...
        if (result.getNameInNamespace().equals(principalDN)) {
          ...
          return this.generateAccessToken(
              ...
              principalDN,
              this.attributeIdKey,
              this.attributeNameKey,
              this.attributeEmailKey,
              this.attributeUserNameKey,
              this.attributeLastPwdChangeKey,
      ...
      if (ldapContext != null && this.useCustomContextForPwdChange) {
        if (this.customPwdChangeLdapContext == null) {
          classLogger.warn(
              "Invalid configuration - using custom context for pwd change set but context wasn't created");
        } else {
          ...
          String searchContextName =
              this.searchContextName.replace(SECURITY_PRINCIPAL_TEMPLATE_USERNAME, username);
          String searchFilter =
              this.searchMatchingAttributes.replace(SECURITY_PRINCIPAL_TEMPLATE_USERNAME, username);

          SearchControls controls = new SearchControls();
          controls.setSearchScope(this.searchContextScope);
          controls.setReturningAttributes(this.requestAttributes);

          NamingEnumeration<SearchResult> findUser =
              this.customPwdChangeLdapContext.search(searchContextName, searchFilter, controls);
          ...
          while (findUser.hasMoreElements()) {
            ...
            try {
              ...
              classLogger.error(
                  "An error occurred tryign to search for user last pwd change attribute");
  ...
  private LDAPConnectionHelper tryLogins(
      String providerUrl, List<String> securityPrincipalTemplate, String username, String password)
      throws Exception {
    int i = 0;
    ...
    do {
      try {
        ...
        String principalDN =
            principalTemplate.replace(
                ILdapAuthenticator.SECURITY_PRINCIPAL_TEMPLATE_USERNAME, username);
        ...
        String message =
            "Failed connection with template: " + securityPrincipalTemplate.get(i) + ". ";
        if (e instanceof NamingException) {
          String possibleExplanation = ((NamingException) e).getExplanation();
          if (possibleExplanation != null
              && !(possibleExplanation = possibleExplanation.trim()).isEmpty()) {
  ...
  @Override
  public void updateUserPassword(String username, String curPassword, String newPassword)
    ...
    if (this.useCustomContextForPwdChange) {
      ...
      try {
        ...
        mods[0] =
            new ModificationItem(
                DirContext.REMOVE_ATTRIBUTE, new BasicAttribute("unicodePwd", oldPwd));
        mods[1] =
            new ModificationItem(
                DirContext.ADD_ATTRIBUTE, new BasicAttribute("unicodePwd", newPwd));
      ...
      try {
        classLogger.info(
            "Attempting login for user " + username + " to confirm has proper current password");
        LDAPConnectionHelper loginObj =
            tryLogins(this.providerUrl, this.securityPrincipalTemplate, username, curPassword);
        String principalTemplate = loginObj.getPrincipalTemplate();
        ...
        mods[0] =
            new ModificationItem(
                DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("unicodePwd", newPwd));
        ...
        if (principalDN == null) {
          message =
              "User was unable to authenticate with current password, username entered: "
                  + username
                  + ". ";
        } else {
          message = "Failed to change password. ";
  ...
  public void updateForgottenPassword(String username, String newPassword) throws Exception {
    ...
    try {
      ...
      ModificationItem[] mods = new ModificationItem[1];
      mods[0] =
          new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("unicodePwd", pwd));
  ...
  @Override
  public List<AccessToken> findUsers(
      String searchContextName, String searchFilter, int searchContextScope) throws Exception {
    if (this.customPwdChangeLdapContext == null) {
      throw new IllegalArgumentException(
          "Invalid configuration for finding users - please contact your administrator");
    }
