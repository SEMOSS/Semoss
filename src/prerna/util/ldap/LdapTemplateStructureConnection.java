...
public class LdapTemplateStructureConnection extends AbstractLdapAuthenticator {
	...
	public AccessToken authenticate(String username, String password) throws Exception {
		...
		try {
			LDAPConnectionHelper loginObj = tryLogins(this.providerUrl, this.securityPrincipalTemplate, username,
					password);
			...
			searchFilter = searchFilter.replace(SECURITY_PRINCIPAL_TEMPLATE_USERNAME, escapeLdapFilter(username));
			...
			while (users.hasMoreElements()) {
				...
				if (result.getNameInNamespace().equals(principalDN)) {
					...
					return this.generateAccessToken(attr, principalDN, this.attributeIdKey, this.attributeNameKey,
			...
			if (ldapContext != null && this.useCustomContextForPwdChange) {
				if (this.customPwdChangeLdapContext == null) {
					...
					NamingEnumeration<SearchResult> findUser = this.customPwdChangeLdapContext.search(searchContextName,
					...
					while (findUser.hasMoreElements()) {
						...
						try {
							// now grab the last pwd change key and throw any
							// LDAPPasswordChangeRequiredException
	...
	private LDAPConnectionHelper tryLogins(String providerUrl, List<String> securityPrincipalTemplate, String username,
		...
		do {
			try {
				...
				String principalDN = principalTemplate.replace(ILdapAuthenticator.SECURITY_PRINCIPAL_TEMPLATE_USERNAME,
						escapeLdapDn(username));
		...
		// if we dont have any successful login, just throw the last error and also
		// print it to log
	...
	public void updateUserPassword(String username, String curPassword, String newPassword) throws NamingException {
		...
		if (this.useCustomContextForPwdChange) {
			...
			// since we already know they must have the correct password to create the
			// context
			DirContext ldapContext = null;
			String principalDN = null;
			try {
