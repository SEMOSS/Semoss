...
public class LDAPConnectionHelper {
  ...
  public static DirContext createLdapContext(
      String providerUrl, String principalDN, String password) throws Exception {
    try {
  ...
  public static ZonedDateTime convertWinFileTimeToJava(long windowsFileTime) {
    ...
    return ZonedDateTime.ofInstant(
        Instant.ofEpochMilli(fixedTime), TimeZone.getTimeZone("UTC").toZoneId());
  }
  ...
  public static AccessToken generateAccessToken(
    ...
    if (!ignoreLastPwdChange) {
      lastPwdChange =
          getLastPwdChange(attributes, attributeLastPwdChangeKey, requirePwdChangeAfterDays);
    }
  ...
  public static ZonedDateTime getLastPwdChange(
    ...
    if (lastPwdChange instanceof Integer || lastPwdChange instanceof Long) {
      ...
      classLogger.warn(
          "Unhandled data type for password change: " + lastPwdChange.getClass().getName());
    }
    ...
    if (requirePwdChangeAfterDays > 0) {
      ...
      if (requirePasswordChange(pwdChange, requirePwdChangeAfterDays)) {
        throw new LDAPPasswordChangeRequiredException(
            "User must change their password before login");
      }
    }
  ...
  private static boolean requirePasswordChange(
      ZonedDateTime lastPwdChange, int requirePwdChangeAfterDays) throws NamingException {
   ...
   * @throws NamingException
   */
  public static Object getAttributeValue(Attributes attributes, String name)
      throws NamingException {
    if (name == null) {
      return null;
    }
