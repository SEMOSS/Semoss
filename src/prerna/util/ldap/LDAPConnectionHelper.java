...
public class LDAPConnectionHelper {
	...
	public static DirContext createLdapContext(String providerUrl, String principalDN, String password)
			throws Exception {
		try {
	...
	public static ZonedDateTime convertWinFileTimeToJava(long windowsFileTime) {
		...
		// There's 116444736000000000 100ns between 1601 and 1970 which is how Java time
		// is stored
		long fixedTime = (windowsFileTime - 116444736000000000L) / 10000L;
		return ZonedDateTime.ofInstant(Instant.ofEpochMilli(fixedTime), TimeZone.getTimeZone("UTC").toZoneId());
	}

	public static AccessToken generateAccessToken(Attributes attributes, String userDN, String attributeIdKey,
		...
		if (userId == null || userId.toString().isEmpty()) {
			throw new IllegalArgumentException(
					"Cannot login user due to not having a proper attribute for the user id");
		}
	...
	public static ZonedDateTime getLastPwdChange(Attributes attributes, String attributeLastPwdChangeKey,
		...
		if (lastPwdChange.toString().equals("0")) {
			throw new LDAPPasswordChangeRequiredException(
					"User's last password change is 0, must change their password on first login");
		}
		...
		if (requirePwdChangeAfterDays > 0) {
			if (pwdChange == null) {
				throw new IllegalArgumentException(
						"There is a password change requirement but could not parse last password change attribute, please reach out to an administrator");
			}
	...
	private static boolean requirePasswordChange(ZonedDateTime lastPwdChange, int requirePwdChangeAfterDays)
