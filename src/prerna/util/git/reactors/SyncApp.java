  ...
  public NounMetadata execute() {
    ...
    if (database) {
      try {
        ...
        if (keyValue.size() == 6) {
          GitSynchronizer.syncDatabases(
              databaseId, databaseName, repository, username, password, logger);
        } else {
    ...
    if (keyValue.size() == 6) {
      filesChanged =
          GitSynchronizer.synchronize(
              databaseId, databaseName, repository, username, password, dual);
    } else {
      String token = getToken();
