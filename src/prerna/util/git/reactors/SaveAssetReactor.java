...
public class SaveAssetReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    boolean strictScriptSource =
        Boolean.parseBoolean(Utility.getDIHelperProperty(Constants.STRICT_SCRIPT_SOURCE));
    ...
    for (int i = 0; i < fileNames.size(); i++) {
      ...
      if (strictScriptSource) {
        ...
        if ("py".equalsIgnoreCase(extension) || "R".equalsIgnoreCase(extension)) {
          throw new IllegalArgumentException(
              "User is not allowed to create or save R or Py scripts");
        }
      }
      ...
      if (space != null
        ...
        return NounMetadata.getErrorNounMessage(
            "You cannot create directory / files at this level for space = " + space);
      }
    }
