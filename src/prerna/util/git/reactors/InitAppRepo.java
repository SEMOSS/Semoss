  ...
  public NounMetadata execute() {
    ...
    try {
      ...
      if (this.keyValue.size() == 5) {
        GitCreator.makeRemoteFromDatabase(
            databaseId, databaseName, repository, username, password, syncDatabase, "");
      } else {
        ...
        AccessToken accessToken2 =
            (AccessToken)
                BeanFiller.fillFromJson(output, jsonPattern, beanProps, new AccessToken());
        username = accessToken2.getProfile();

        GitCreator.makeRemoteFromDatabase(
            databaseId, databaseName, repository, username, "", syncDatabase, token);
