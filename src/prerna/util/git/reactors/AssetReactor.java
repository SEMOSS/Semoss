...
public class AssetReactor extends AbstractReactor {
	...
	// config
	// dependencies - array with specific engine and the dependent library and the
	// version
	// semoss: version
	// dependencies :[
	// {
	// r version - version[['version.string']]
	// System.out.println("installed... " + re.eval("'moron' %in%
	// rownames(installed.packages()) ==
	// TRUE").asBool().isTRUE()); - checks to see if it is installed
	// need something on the rdf map which says go ahead and install packages if not
	// there
	// "R":"3.3.2"
	// "libraries": {
