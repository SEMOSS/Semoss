...
public class DeleteAssetReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (insight.isSavedInsight()
        && (space == null || AssetUtility.INSIGHT_SPACE_KEY.equalsIgnoreCase(space))) {
      IProject project = Utility.getProject(this.insight.getProjectId());
      gitVersionFolder =
          AssetUtility.getProjectVersionFolder(project.getProjectName(), insight.getProjectId())
              .replace("\\", "/");
      ...
      if (userProjectId != null && !userProjectId.isEmpty()) {
        ...
        gitVersionFolder =
            AssetUtility.getUserAssetAndWorkspaceAssetFolder(
      ...
      gitVersionFolder =
          AssetUtility.getProjectVersionFolder(project.getProjectName(), space).replace("\\", "/");
    }
          ...
          String relativeGit =
              relativePath
                  ...
                  + Utility.normalizePath(
                      f.getAbsolutePath().substring(baseFolderPath.length() + 1));
