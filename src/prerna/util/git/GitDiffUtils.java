...
public class GitDiffUtils {
	...
	public static String printDiff(String gitFolder, String comm1, String comm2, String fileName) throws Exception {
		...
		try (Git thisGit = Git.open(new File(gitFolder))) {
			...
			DiffFormatter df = new DiffFormatter(outputStream); // use NullOutputStream.INSTANCE if you don't need the
																// diff output
			...
			/*
			 * CanonicalTreeParser ctp1 = null; try( ObjectReader reader =
			 * thisGit.getRepository().newObjectReader() ) { ctp1 = new CanonicalTreeParser(
			 * null, reader, comm1.getId() ); }
			 * 
			 * CanonicalTreeParser ctp2 = null; try( ObjectReader reader =
			 * thisGit.getRepository().newObjectReader() ) { ctp2 = new CanonicalTreeParser(
			 * null, reader, comm2.getId() ); }
			 */
			...
			/*
			 * ObjectId treeId = thisGit.getRepository().resolve(comm1.getId()); try(
			 * ObjectReader reader = repository.newObjectReader() ) { treeParser.reset(
			 * reader, treeId ); }
			 */
			...
			/*
			 * String output = outputStream.toString(); BufferedReader sr = new
			 * BufferedReader(new StringReader(output)); String data = null; while((data =
			 * sr.readLine())!= null) { System.out.println(">>>" + data); }
			 */
		}
