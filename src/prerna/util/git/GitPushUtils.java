...
public class GitPushUtils {
  ...
  public static void commitAddedFiles(
    ...
    try {
      if (message == null) message = GitUtils.getDateMessage("Commited on.. ");
      if (author == null) author = "SEMOSS";
      if (email == null) email = "semoss@semoss.org";
  ...
  public static void push(
      String repository, String remoteToPush, String branch, String userName, String password) {
    int attempt = 1;
      ...
      try {
        ...
        if (pr.isSuccessful()) {
          return new NounMetadata(
              "Git Pulled: " + pr.isSuccessful(),
              PixelDataType.CONST_STRING,
              PixelOperationType.HELP);
        } else {
          return new NounMetadata(
              "Git pull error", PixelDataType.CONST_STRING, PixelOperationType.HELP);
        }
      } catch (GitAPIException e) {
        logger.error(Constants.STACKTRACE, e);
        return new NounMetadata(
            "Git Pull Error: " + e, PixelDataType.ERROR, PixelOperationType.HELP);
      } finally {
        thisGit.close();
      }
    }
    return new NounMetadata(
        "Git Pull Error - Git is empty ", PixelDataType.ERROR, PixelOperationType.HELP);
  }

  public static NounMetadata checkout(
      String repository, String branch, String token, AuthProvider prov) {
    ...
    if (thisGit != null) {
      ...
      try {
        checkout.call();
        return new NounMetadata(
            "Git checkout: " + branch, PixelDataType.CONST_STRING, PixelOperationType.HELP);
      } catch (GitAPIException e) {
        logger.error(Constants.STACKTRACE, e);
        return new NounMetadata(
            "Git Checkout Error: " + e, PixelDataType.ERROR, PixelOperationType.HELP);
      } finally {
        thisGit.close();
      }
    }
    return new NounMetadata(
        "Git Checkout Error - Git is empty ", PixelDataType.ERROR, PixelOperationType.HELP);
  }
  ...
  public static NounMetadata clone(
      String workingDir, String repo, String token, AuthProvider prov) {
    return clone(workingDir, repo, token, prov, true);
  }

  public static NounMetadata clone(
      String workingDir, String repo, String token, AuthProvider prov, boolean appendFolderName) {
    Boolean isGitlab = (prov == AuthProvider.GITLAB);
    ...
    if (trustedRepo != null && !trustedRepo.isEmpty()) {
      if (!repo.startsWith(trustedRepo)) {
        return new NounMetadata(
            "Git clone Error: Cloning from unapproved git registry",
            PixelDataType.ERROR,
            PixelOperationType.HELP);
      }
    }
    ...
    try {
      clone.call();
      return new NounMetadata(
          "Git clone success: " + repo, PixelDataType.CONST_STRING, PixelOperationType.HELP);
    } catch (GitAPIException e) {
      logger.error(Constants.STACKTRACE, e);
      return new NounMetadata(
          "Git clone error: " + e, PixelDataType.ERROR, PixelOperationType.HELP);
    }
  }
}
