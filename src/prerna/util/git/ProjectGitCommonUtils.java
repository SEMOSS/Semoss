...
/**
 * Shared, plain-Java-testable primitives used by the project git working-tree
 * utilities ({@link ProjectGitStatusUtils}, {@link ProjectGitStageUtils},
 * {@link ProjectGitDiffUtils}, {@link ProjectGitConflictUtils},
 * {@link ProjectGitBranchUtils}): HEAD/detached-state resolution, repo-relative
 * path validation, binary content detection, and the diff line-count
 * truncation convention. These methods take only {@link Repository}/plain Java
 * types and throw plain exceptions so they can be unit tested against a
 * temporary JGit repository without any pixel/reactor/security dependencies.
 */
public final class ProjectGitCommonUtils {

	/** Matches {@code ProjectCommitDiffReactor}'s existing truncation convention. */
	public static final int MAX_DIFF_LINES = 10000;

	private ProjectGitCommonUtils() {
	}
	 ...
	 * repository (no commits yet, {@code headCommitId} and {@code branch} are
	 * both {@code null}) and for a detached HEAD ({@code branch} is
	 * {@code null}, {@code detached} is {@code true}).
	 */
	 ...
	 * repo-relative path safe to use as a JGit file pattern or in an error
	 * message (never an absolute filesystem path).
	 *
	 * @throws IllegalArgumentException if the path is blank, absolute, escapes
	 *                                  the working tree (via {@code ..}
	 *                                  traversal or a symlinked ancestor), or
	 *                                  references the {@code .git} directory
	 */
	public static String validateRepoRelativePath(Repository repo, String suppliedPath) {
		...
		String firstSegment = normalized.contains("/") ? normalized.substring(0, normalized.indexOf('/'))
				: normalized;
	...
	/** Backed by JGit's own binary-content heuristic, {@link RawText#isBinary(byte[])}. */
