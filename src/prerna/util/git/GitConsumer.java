...
public class GitConsumer {
  ...
  public static String makeDatabaseFromRemote(
      String yourName4Database, String fullRemoteDatabaseName, Logger logger) {
    ...
    if (!db.exists()) {
      // make the folder
      db.mkdirs();
      //			throw new IllegalArgumentException("You already have a folder named " + yourName4App + "
      // in your directory."
    ...
    String versionFolder =
        AssetUtility.getProjectVersionFolder(yourName4Database, temporaryDatabaseId);
    ...
    REMOTE_LOOP:
    for (Map<String, String> remoteMap : remotes) {
    ...
    for (int i = 0; i < files.length; i++) {
      ...
      currAppFolder.renameTo(
          new File(
              Utility.normalizePath(
                  currAppFolder.getParent()
                      + "/"
                      + SmssUtilities.getUniqueName(yourName4Database, actualAppId))));
    }

    if (actualAppId == null) {
      throw new IllegalArgumentException(
          "There is no app id defined within the smss of the new app you are downloading");
    }

    if (SecurityEngineUtils.getAllEngineIds(null).contains(actualAppId)) {
      throw new IllegalArgumentException(
          "The app you are attempting to copy already exists as "
              + SecurityEngineUtils.getEngineAliasForId(actualAppId));
    }
  ...
  public static void moveDataFilesToDatabase(
      String baseFolder, String appId, String yourName4App, Logger logger) {
    ...
    if (gitDataDir.exists()) {
      ...
      for (int i = 0; i < dataFiles.length; i++) {
        try {
          ...
          logger.info(
              "Moving database file : " + FilenameUtils.getName(files[i].getAbsolutePath()));
  ...
  private static File changeSmssEngineName(File file, String yourName4App) {
    ...
    String newFileName =
        mainDirectory + "/" + SmssUtilities.getUniqueName(yourName4App, appId) + ".smss";
  ...
  /**
   * Load any mosfet files since they might not have been added to the insights rdbms that was
   * pushed
   *
   * @param versionFolder
   */
          ...
          try {
            ...
            IRawSelectWrapper wrapper =
                WrapperManager.getInstance()
                    .getRawWrapper(insightsDb, "select id from question_id where id='" + id + "'");
            if (wrapper.hasNext()) {
