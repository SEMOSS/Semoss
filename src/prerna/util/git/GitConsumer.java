...
public class GitConsumer {
	...
	public static String makeDatabaseFromRemote(String yourName4Database, String fullRemoteDatabaseName,
			Logger logger) {
		...
		if (!db.exists()) {
			// make the folder
			db.mkdirs();
			// throw new IllegalArgumentException("You already have a folder named " +
			// yourName4App + "
			// in your directory."
		...
		for (int i = 0; i < files.length; i++) {
			...
			currAppFolder.renameTo(new File(Utility.normalizePath(
					currAppFolder.getParent() + "/" + SmssUtilities.getUniqueName(yourName4Database, actualAppId))));
		}
		...
		if (SecurityEngineUtils.getAllEngineIds(null).contains(actualAppId)) {
			throw new IllegalArgumentException("The app you are attempting to copy already exists as "
		...
		/*
		 * while(!((GitProgressMonitor)mon).complete) { try{ Thread.sleep(200);
	...
	// private static void updateInsightRdbms(File oldSmssFile, String appFolder,
	// String yourName4App)
	// {
	...
	//
	// AppNameRecipeModifier.renameDatabaseForInsights(location, yourName4App,
	// origEngineName);
	// } catch (IOException e) {
	// classLogger.error(Constants.STACKTRACE, e);
	// } finally {
	...
	/**
	 * Load any mosfet files since they might not have been added to the insights
	 * rdbms that was pushed
	 *
	 * @param versionFolder
	 */
			...
			if (in.isDirectory()) {
				...
				for (File mosfetF : mosfetFiles) {
					try {
						...
						IRawSelectWrapper wrapper = WrapperManager.getInstance().getRawWrapper(insightsDb,
								"select id from question_id where id='" + id + "'");
						if (wrapper.hasNext()) {
