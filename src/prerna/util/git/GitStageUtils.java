...
/**
 * Stages and unstages repo-relative paths for the {@code *GitStageReactor}
 * project and engine reactors. Callers are responsible for validating paths
 * first, e.g. via
 * {@link GitCommonUtils#validateRepoRelativePaths(Repository, List)}.
 */
public final class GitStageUtils {

	private GitStageUtils() {

	}

	/**
	 * Stages each of {@code repoRelativePaths}. A path that currently exists on
	 * disk is added (covers new/modified files, and the new-path half of a rename);
	 * a path that is missing on disk but tracked in the index is removed from the
	 * index only (covers deletions, and the old-path half of a rename). Never
