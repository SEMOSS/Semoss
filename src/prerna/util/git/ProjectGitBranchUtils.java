...
/**
 * Lists local/remote branches with tracking status and performs
 * create+checkout for {@code ProjectGitBranchesReactor} and
 * {@code ProjectGitCreateBranchReactor}. {@code ProjectGitCheckoutReactor}
 * performs its own checkout call (see {@link #resolveCheckoutTarget}) since
 * checkout of an existing branch is not a create+checkout operation.
 */
public final class ProjectGitBranchUtils {

	private ProjectGitBranchUtils() {
	}
	...
	/**
	 * Resolves whether {@code branchName} exists as a local branch, or as
	 * exactly one remote-tracking branch across all configured remotes.
	 */
	 ...
	 * itself allows this, but real {@code git branch} refuses it at the
	 * porcelain level to avoid ambiguity with command-line flags, and this API
	 * accepts free-form user-typed names the same way {@code git branch} does.
	 */
