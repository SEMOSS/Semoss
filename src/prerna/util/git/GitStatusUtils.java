...
/**
 * Computes the {@code git status} equivalent used by the
 * {@code *GitStatusReactor} project and engine reactors, and reused by every
 * mutating git reactor to build their embedded "refreshed status" response
 * field.
 */
public final class GitStatusUtils {

	private static final Comparator<GitFileStatus> BY_PATH = Comparator.comparing(f -> f.path);

	private GitStatusUtils() {
	...
	/**
	 * Returns the empty/unborn-repository status shape: no branch, no last commit,
	 * every file list empty, clean.
	 */
