...
/**
 * Interface that provides a common protocol for objects that wish to execute code while they are
 * active. Used to filter filenames. Opens up a thread and watches the file.
 */
public abstract class AbstractFileWatcher implements Runnable, FilenameFilter {
  ...
  /**
   * Optional method to be overriden So that a watcher can perform any required operations Prior to
   * starting up its own thread
   */
  public void init() {}
  ...
  public void run() {
    classLogger.info(
        "Starting Watcher Thread for type "
            + this.engineType
            + " with class "
        ...
        if (kind == StandardWatchEventKinds.ENTRY_CREATE) {
          ...
          if (newFile.endsWith(extension)) {
            ...
            if (file.exists()) {
              ...
              classLogger.info(
                  "Ignoring Folder/File " + newFile + " that has already been removed");
            }
          }
        }
      }
