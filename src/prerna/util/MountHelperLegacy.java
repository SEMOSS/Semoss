...
public class MountHelperLegacy {
  ...
  ////		maker.deleteTarget(true);
	...
	// if(!targetDir.exists()) {
	// logger.info("Target folder doesn't exist. Making folder now at: " +
	// targetDirName);
	...
	public MountHelperLegacy(String targetDirName) {
		...
		// also create the semoss home folder
		String newSemossHomeFolderPath = this.targetDirName + FILE_SEPARATOR
		...
		// TODO make a unique mount for the debootstrap - not mount target. change mount
		// target to
	...
	// mountTarget is for recursive mounting of multiple folders but ignoring files
	// in the top level
	// (ex. M
	// example app_root folder is a target we want to mount because that one folder
	// has all the
	// subfiles/folders we need
	// for Insight Cache, we want to do that as mount folder because we just want
	// that one folder +
	...
	public void mountTarget(String sourceDirName, String subPath, boolean readOnly) {
		...
		for (int sourceFileIndex = 0; sourceFileIndex < allSourceFiles.length; sourceFileIndex++) {
			...
			// Thos is the check where it needs to be a subfolder to get mounted. files at
			// the top
	...
	public void mountFolder(String sourceDirName, String subPath, boolean readOnly) {
		...
		classLogger.info("Mounting folder at srcPat:  " + Utility.cleanLogString(sourceDirName) + " and targetPath: "
	...
	public void createCustomRDFMap() {
		...
		// boolean success = thisTargetFile.mkdirs();
		// System.out.println("KUNAL ::::: making dir at " + targetPath + " was a " +
		// success);
		// System.out.println("KUNAL ::::: mounting dir at srcPat: " + srcPath + " and
		// targetPath: " +
		// targetPath);
		...
		// DIHelper.getInstance().getProperty(Constants.INSIGHT_CACHE_DIR) + " and
		// targetPath: " +
		// targetPath);
		//
		//
		// mountDir(DIHelper.getInstance().getProperty(Constants.INSIGHT_CACHE_DIR) ,
		// targetDirName +
		// DIHelper.getInstance().getProperty(Constants.INSIGHT_CACHE_DIR), enableSudo,
		// true);
		//
	}
