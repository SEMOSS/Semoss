...
public class MountHelperLegacy {
  ...
  public MountHelperLegacy(String targetDirName) {
    ...
    // also create the semoss home folder
    String newSemossHomeFolderPath =
        this.targetDirName
            + FILE_SEPARATOR
    ...
    // TODO make a unique mount for the debootstrap - not mount target. change mount target to
  ...
  //	mountTarget is for recursive mounting of multiple folders but ignoring files in the top level
  // (ex. M
  //	example app_root folder is a target we want to mount because that one folder has all the
  // subfiles/folders we need
  //	for Insight Cache, we want to do that as mount folder because we just want that one folder +
  // the files at that level
  public void mountTarget(String sourceDirName, String subPath, boolean readOnly) {
    ...
    for (int sourceFileIndex = 0; sourceFileIndex < allSourceFiles.length; sourceFileIndex++) {
      ...
      // Thos is the check where it needs to be a subfolder to get mounted. files at the top
      // directory dont get mounted
  ...
  public void mountFolder(String sourceDirName, String subPath, boolean readOnly) {
    ...
    boolean success = thisTargetFile.mkdirs();
    classLogger.info(
        "Making folder at " + Utility.cleanLogString(targetPath) + " was a " + success);
    classLogger.info(
        "Mounting folder at srcPat:  "
            + Utility.cleanLogString(sourceDirName)
            + " and targetPath: "
            + Utility.cleanLogString(targetPath));
  ...
  public void unmountTargetProc() {
    ...
    try {
      ProcessBuilder pb = null;
      String command =
          "grep " + targetDirName + " /proc/mounts | cut -f2 -d' ' | xargs -r -n 1 umount";
  ...
  public void createCustomRDFMap() {
    ...
    String[] propsNeeded =
        new String[] {
          ...
          Constants.NETTY_R,
          Constants.NETTY_PYTHON,
          Constants.USE_R,
          Constants.USE_PYTHON,
          Constants.R_MEM_LIMIT,
          Constants.INSIGHT_CACHE_DIR,
          Settings.MVN_HOME,
          Settings.REPO_HOME,
          Constants.PY_BASE_FOLDER,
          Constants.R_CONNECTION_JRI
        };
    ...
    mountFolder(
        baseFolder + FILE_SEPARATOR + Constants.R_BASE_FOLDER,
        baseFolder + FILE_SEPARATOR + Constants.R_BASE_FOLDER,
        true);
    mountFolder(
        baseFolder + FILE_SEPARATOR + Constants.PY_BASE_FOLDER,
        baseFolder + FILE_SEPARATOR + Constants.PY_BASE_FOLDER,
        true);

    boolean nativePyServer =
        DIHelper.getInstance().getProperty(Settings.NATIVE_PY_SERVER) != null
            && DIHelper.getInstance()
                .getProperty(Settings.NATIVE_PY_SERVER)
    ...
    //		System.out.println("KUNAL ::::: mounting dir at srcPat:  " + srcPath + " and targetPath: " +
    // targetPath);

    //		String targetPath = targetDirName + FILE_SEPARATOR +
    // DIHelper.getInstance().getProperty(Constants.INSIGHT_CACHE_DIR);
    //		File thisTargetFile = new File(targetPath);
    //		boolean success = thisTargetFile.mkdirs();
    //		System.out.println("KUNAL ::::: making dir at " + targetPath + " was a " + success);
    //		System.out.println("KUNAL ::::: mounting dir at srcPat:  " +
    // DIHelper.getInstance().getProperty(Constants.INSIGHT_CACHE_DIR) + " and targetPath: " +
    // targetPath);
    //
    //
    //		mountDir(DIHelper.getInstance().getProperty(Constants.INSIGHT_CACHE_DIR) , targetDirName +
    // DIHelper.getInstance().getProperty(Constants.INSIGHT_CACHE_DIR), enableSudo, true);
    //
  }
