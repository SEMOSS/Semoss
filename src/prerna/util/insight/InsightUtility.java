...
public class InsightUtility {
	...
	/**
	 * Used to transfer important properties when creating an insight within an
	 * insight
	 * 
	 * @param origInsight
	 * @param newInsight
	...
	/**
	 * User to transfer the insight id / saved components for identification of the
	 * insight
	 * 
	 * @param origInsight
	 * @param newInsight
	...
	/**
	 * Useful method to determine if the frame is the same using the
	 * FrameCacheHelper class
	 * 
	 * @param frames
	 * @param frame
	 * @return
	...
	/**
	 * Get a subset of the session Id to use in file locations to do file structure
	 * length restrictions
	 * 
	 * @param sessionId
	 * @return
	...
	public static NounMetadata removeVaraible(InMemStore<String, NounMetadata> varStore, String key) {
		...
		if (nType == PixelDataType.FRAME) {
			...
			try {
				...
				classLogger.error("Failed to close the task while removing variable {}", key, e);
			}
		}
	 ...
	 * @param newNoun
	 */
	public static void replaceNounValue(InMemStore<String, NounMetadata> varStore, Object oldValue,
			NounMetadata newNoun) {
	...
	public static NounMetadata removeFrameVaraible(InMemStore<String, NounMetadata> varStore, String key) {
		...
		if (nType == PixelDataType.FRAME) {
			...
			return new NounMetadata(key, PixelDataType.CONST_STRING, PixelOperationType.REMOVE_FRAME,
					PixelOperationType.REMOVE_VARIABLE);
		}
		...
		synchronized (insight) {
			...
			for (String key : filterCaches.keySet()) {
				try {
					...
					classLogger.error("Failed to close the cached filter model frame {}", key, e);
				}
			}
			...
			for (String key : keys) {
				...
				try {
					...
					classLogger.error("Failed to close the frame for variable {} while clearing the insight", key, e);
				}
			}
			for (ITableDataFrame dm : allCreatedFrames) {
				if (!dm.isClosed()) {
					try {
						classLogger.info("There are untracked frames in this insight. Frame with name = {}",
								Utility.cleanLogString(dm.getOriginalName()));
						dm.close();
					} catch (Exception e) {
						classLogger.error("Failed to close an untracked frame while clearing the insight", e);
					}
				}
			}
					...
					for (String fileKey : fileExports.keySet()) {
						...
						if (insightFile.isDeleteOnInsightClose()) {
							try {
								...
								classLogger.error("Failed to delete the export file used in the insight", e);
							}
						}
					}
				}
			...
			if (insight.rInstantiated() && insight.isDeleteREnvOnDropInsight()) {
				try {
					...
					classLogger.error("Failed to clear the R environment variables while clearing the insight", e);
				}
			}
		...
		synchronized (insight) {
			...
			if (insight.rInstantiated() && insight.isDeleteREnvOnDropInsight()) {
				try {
					...
					classLogger.error("Failed to remove the R environment while dropping the insight", e);
				}
			}
			// if Python is instantiated
	...
	/**
	 * Give the FE all the data needed to get to the same UI for the existing
	 * insight
	 * 
	 * @param in
	 */
	public static PixelRunner recreateInsightState(Insight in) {
		...
		try {
			...
			classLogger.error("Failed to clone the insight sheets and panels while recreating the insight state", e);
		}
		...
		try {
			...
			classLogger.error("Failed to add the cached frame headers while recreating the insight state", e);
		}
	...
	public static NounMetadata getSheetState(InsightSheet sheet, String outputType) {
		...
		try {
			...
			classLogger.error("Failed to serialize the insight sheet state", e);
			throw new IllegalArgumentException(
					"Exeption occurred generate the sheet state with error: " + e.getMessage());
		}
	...
	public static NounMetadata getPanelState(InsightPanel panel, String outputType) {
		...
		try {
			...
			classLogger.error("Failed to serialize the insight panel state", e);
			throw new IllegalArgumentException(
					"Exeption occurred generate the panel state with error: " + e.getMessage());
		}
	...
	public static Map<String, Map<String, Object>> getAllFrameHeaders(VarStore varStore) {
		...
		while (frameKeys.hasNext()) {
			...
			if (noun != null && noun.getValue() instanceof ITableDataFrame) {
				...
				classLogger.info(
						"You are grabbing frame headers but the noun doesn't refer to a frame... very vey weird....");
			}
		}
	...
	/**
	 * Fill and move over the QS state stored in the insight via frame/panels to the
	 * QS
	 * 
	 * @param qs
	 * @param insight
	...
	/**
	 * All insight panels that have a temp filter model referencing a frame should
	 * be reset
	 * 
	 * @param insight
	 * @param frame
	...
	public static Map<String, Object> calculateDynamicVars(Insight insight, List<Object> dynamicVarNames) {
		...
		for (int varIndex = 0; varIndex < dynamicVarNames.size(); varIndex++) {
			 ...
			 * setExpression("'Sum of ages is now set to.. {}'.format(frame_d['age'].astype(int).sum())"
			 * ); var.setName("age_sum"); var.addFrame("frame_d");
			...
			for (int frameIndex = 0; frameIndex < frameNames.size(); frameIndex++) {
				...
				if (!oldNew.containsKey(thisFrameName)) // if not already processed. Generate one
				{
					...
					try {
						...
						classLogger.error("Failed to create the temporary var frame for frame {}", thisFrameName, e);
					}
				}
			}
		}
		...
		for (int varIndex = 0; varIndex < dynamicVarNames.size(); varIndex++) {
			...
			if (var.getLanguage() == Variable.LANGUAGE.R) {
				Object value = insight.getRJavaTranslator(CalcVarReactor.class.getName())
						.runRAndReturnOutput(expression);
				varValue.put(var.getName(), value);
			}
		}
		...
		if (rDeleter.indexOf(",") > 0) { // atleast one is filled up
			insight.getRJavaTranslator(CalcVarReactor.class.getName())
					.executeEmptyR(rDeleter.substring(0, pyDeleter.length() - 2) + ")");
		}
		...
		if (!dynamicVarNames.isEmpty()) {
			...
			for (String dynamicVarName : dynamicVarValues.keySet()) {
				...
				if (variable.getFormat() != null) {
					...
					Object formattedData = FormattingUtility.formatDataValues(dynamicValue, "DOUBLE",
							variable.getFormat().toString(), null);
	 ...
	 * @param logger
	 */
	public static void addInsightPanelRefreshFromPanelFilter(Insight insight, InsightPanel panel,
			NounMetadata filterNoun, Logger logger) {
	 ...
	 * @param logger
	 */
	public static void addInsightPanelRefreshFromFrameFilter(Insight insight, ITableDataFrame frame,
			NounMetadata filterNoun, Logger logger) {
		List<NounMetadata> taskOutput = new ArrayList<>();
		List<NounMetadata> additionalMessages = new ArrayList<>();
	 ...
	 * @param logger
	 */
	public static void refreshInsightPanelTasksFromFrameFilter(Insight insight, ITableDataFrame frame,
			List<NounMetadata> taskOutput, List<NounMetadata> additionalMessages, Logger logger) {
		List<InsightPanel> affectedPanels = getInsightPanelsUsingFrame(insight, frame);
		for (InsightPanel panel : affectedPanels) {
			InsightUtility.refreshPanelTasks(insight, panel, panel.getNumCollect(), taskOutput, additionalMessages,
					logger);
		}
	}
	 ...
	 * @param logger
	 */
	public static void refreshPanelTasks(Insight insight, InsightPanel panel, int panelCollect,
			List<NounMetadata> taskOutput, List<NounMetadata> additionalMessages, Logger logger) {
			...
			LAYER_LOOP: for (String layerId : layers) {
				...
				if (qs != null && taskOptions != null) {
					...
					try {
						...
						logger.info("Previous query on panel {} does not work", panelId);
						InsightUtility.classLogger.error("Failed to optimize the query for panel {}", panelId, e);
						// see if the frame at least exists
						ITableDataFrame queryFrame = qs.getFrame();
						if (queryFrame == null || queryFrame.isClosed()) {
							additionalMessages.add(NounMetadata.getErrorNounMessage("Attempting to refresh panel id "
									+ panelId + " but the frame creating the visualization no longer exists"));
							continue LAYER_LOOP;
						}

						NounMetadata warning = NounMetadata.getWarningNounMessage("Attempting to refresh panel id "
						...
						allQs.setQsType(AbstractQueryStruct.QUERY_STRUCT_TYPE.FRAME);
					...
					if (layout.equals("PivotTable")) {
						...
						taskOutput.add(
								new NounMetadata(task, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA));
					}
				}
			}
		}
	 ...
	 * @param logger
	 */
	public static void refreshViewFromPanelFilter(Insight insight, InsightPanel panel, NounMetadata filterNoun,
			Logger logger) {
		if (!panel.getPanelView().equalsIgnoreCase("text-editor") || panel.getDynamicVars().isEmpty()) {
			return;
		}
	 ...
	 * @param logger
	 */
	public static void refreshViewFromFrameFilter(Insight insight, ITableDataFrame frame, NounMetadata filterNoun,
			Logger logger) {
	...
	/**
	 * Get the QS for the frame with frame filters applied. If panel is not null
	 * then add in the panel filters as well.
	 * 
	 * @param frame
	 * @param panel
	 * @return
