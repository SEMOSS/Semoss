...
public class InsightUtility {
   ...
   * @return
   */
  public static FrameCacheHelper findSameFrame(
      List<FrameCacheHelper> frames, ITableDataFrame frame) {
  ...
  /**
   * Get a subset of the session Id to use in file locations to do file structure length
   * restrictions
   *
   * @param sessionId
   * @return
   ...
   * @param newNoun
   */
  public static void replaceNounValue(
      InMemStore<String, NounMetadata> varStore, Object oldValue, NounMetadata newNoun) {
   ...
   * @return
   */
  public static NounMetadata removeFrameVaraible(
      InMemStore<String, NounMetadata> varStore, String key) {
    ...
    synchronized (insight) {
      ...
      for (ITableDataFrame dm : allCreatedFrames) {
        if (!dm.isClosed()) {
          try {
            classLogger.info(
                "There are untracked frames in this insight. Frame with name = "
          ...
          for (String fileKey : fileExports.keySet()) {
            ...
            if (insightFile.isDeleteOnInsightClose()) {
              try {
                ...
                f.delete();
                classLogger.debug(
                    "Successfully deleted export file used in insight " + f.getName());
              } catch (Exception e) {
                classLogger.error(Constants.STACKTRACE, e);
              }
            }
      ...
      classLogger.info(
          "Successfully cleared insight " + Utility.cleanLogString(insight.getInsightId()));
   ...
   * @param newInsight
   */
  public static void registerNestedInsight(
      Insight parentInsight, Insight childInsight, String sessionId) {
  ...
  public static NounMetadata dropInsight(final Insight insight) {
    synchronized (insight) {
      ...
      if (insight.isSavedInsight() && insight.getUser() != null) {
        insight
            .getUser()
            .removeOpenInsight(insight.getProjectId(), insight.getRdbmsId(), insightId);
      }
      ...
      for (String k : keys) {
        ...
        if (type == PixelDataType.FRAME) {
          try {
            ...
            pixelRunner.addResult(
                "CACHED_FRAME_HEADERS",
                new NounMetadata(
  ...
  public static void fillQsReferencesAndMergeOptions(Insight insight, SelectQueryStruct qs) {
    ...
    if (qsType != QUERY_STRUCT_TYPE.RAW_ENGINE_QUERY
      ...
      if (qs.getSelectors().isEmpty()) {
        throw new IllegalArgumentException(
            "There are no selectors in the query to return.  "
   ...
   * @return
   */
  public static Map<String, Object> calculateDynamicVars(
      Insight insight, List<Object> dynamicVarNames) {
    ...
    for (int varIndex = 0; varIndex < dynamicVarNames.size(); varIndex++) {
      ...
      if (var.getLanguage() == Variable.LANGUAGE.R) {
        Object value =
            insight
                .getRJavaTranslator(CalcVarReactor.class.getName())
  ...
  public static String recalculateHtmlViews(Insight insight, InsightPanel panel) {
    ...
    if (!dynamicVarNames.isEmpty()) {
      ...
      Map<String, Object> dynamicVarValues =
          InsightUtility.calculateDynamicVars(insight, dynamicVarNames);
        ...
        if (variable.getFormat() != null) {
          ...
          Object formattedData =
              FormattingUtility.formatDataValues(
                  dynamicValue, "DOUBLE", variable.getFormat().toString(), null);
   ...
   * @param logger
   */
  public static void addInsightPanelRefreshFromPanelFilter(
      Insight insight, InsightPanel panel, NounMetadata filterNoun, Logger logger) {
    ...
    InsightUtility.refreshPanelTasks(
        insight, panel, panel.getNumCollect(), taskOutput, additionalMessages, logger);
   ...
   * @param logger
   */
  public static void addInsightPanelRefreshFromFrameFilter(
      Insight insight, ITableDataFrame frame, NounMetadata filterNoun, Logger logger) {
    List<NounMetadata> taskOutput = new ArrayList<>();
    List<NounMetadata> additionalMessages = new ArrayList<>();
    InsightUtility.refreshInsightPanelTasksFromFrameFilter(
        insight, frame, taskOutput, additionalMessages, logger);
    filterNoun.addAllAdditionalReturn(taskOutput);
    filterNoun.addAllAdditionalReturn(additionalMessages);
   ...
   * @param logger
   */
  public static void refreshInsightPanelTasksFromFrameFilter(
      Insight insight,
      ITableDataFrame frame,
      List<NounMetadata> taskOutput,
      List<NounMetadata> additionalMessages,
      Logger logger) {
   ...
   * @return
   */
  public static List<InsightPanel> getInsightPanelsUsingFrame(
      Insight insight, ITableDataFrame frame) {
    ...
    for (String panelId : insightPanelsMap.keySet()) {
      ...
      LAYER_LOOP:
      for (String layerId : allQsOnPanel.keySet()) {
   ...
   * @param logger
   */
  public static void refreshPanelTasks(
      Insight insight,
      InsightPanel panel,
      int panelCollect,
      List<NounMetadata> taskOutput,
      ...
      for (String layerId : layers) {
        ...
        if (qs != null && taskOptions != null) {
          ...
          try {
            ...
            taskOptions =
                new TaskOptions(AutoTaskOptionsHelper.generateGridTaskOptions(allQs, panelId));
            try {
              task.setLogger(logger);
              task.toOptimize(true);
              task.setTaskOptions(taskOptions);
              ...
              additionalMessages.add(
                  NounMetadata.getErrorNounMessage(
                      "Attemptingt to refresh panel id "
                          + panelId
                          + " but the underlying data creating the visualization no longer exists "
                          + " or is now incompatible with the view. Displaying a grid of the data "
          ...
          if (layout.equals("PivotTable")) {
            ...
            taskOutput.add(
                new NounMetadata(
                    task, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA));
          }
        }
      }
    }
   ...
   * @param logger
   */
  public static void refreshViewFromPanelFilter(
      Insight insight, InsightPanel panel, NounMetadata filterNoun, Logger logger) {
   ...
   * @param logger
   */
  public static void refreshViewFromFrameFilter(
      Insight insight, ITableDataFrame frame, NounMetadata filterNoun, Logger logger) {
  ...
  /**
   * Get the QS for the frame with frame filters applied. If panel is not null then add in the panel
   * filters as well.
   *
   * @param frame
   * @param panel
