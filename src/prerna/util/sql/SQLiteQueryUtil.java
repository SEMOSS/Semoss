...
public class SQLiteQueryUtil extends AnsiSqlQueryUtil {
  ...
  /**
   * Modification from default as SQLite doesn't do CREATE TABLE AS ( ) And doesn't require the
   * additional ( )
   */
  ...
  public void handleInsertionOfBlob(
      Connection conn, PreparedStatement statement, String object, int index) throws SQLException {
    if (object == null) {
  ...
  public String getIndexDetails(
    ...
    // TODO: MAHER COME BACK TO GETTING THIS COLUMN
    // TODO: MAHER COME BACK TO GETTING THIS COLUMN
    return "SELECT DISTINCT TBL_NAME, null AS COLUMN FROM SQLITE_MASTER WHERE TYPE='index' AND NAME='"
        + indexName
        + "' AND TBL_NAME='"
        + tableName
        + "';";
  ...
  public String allIndexForTableQuery(String tableName, String database, String schema) {
    ...
    return "SELECT NAME, null AS COLUMN FROM SQLITE_MASTER WHERE TYPE='index' AND TBL_NAME='"
        + tableName
        + "';";
  }
