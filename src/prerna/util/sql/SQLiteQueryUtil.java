...
public class SQLiteQueryUtil extends AnsiSqlQueryUtil {
	...
	/**
	 * Modification from default as SQLite doesn't do CREATE TABLE AS ( ) And
		...
		for (int jIdx = 0; jIdx < numJoins; jIdx++) {
			...
			if (leftColType == rightColType) {
				...
				if (leftColType == SemossDataType.DOUBLE && rightColType == SemossDataType.INT) {
					...
					joinString.append(" ").append(LEFT_TABLE_ALIAS).append(".").append(leftTableJoinCol)
							.append(" = CAST(").append(RIGHT_TABLE_ALIAS).append(".").append(rightTableJoinCol)
							.append(" AS DOUBLE)");
					...
					joinString.append(" CAST(").append(LEFT_TABLE_ALIAS).append(".").append(leftTableJoinCol)
							.append(" AS VARCHAR(800)) = CAST(").append(RIGHT_TABLE_ALIAS).append(".")
							.append(rightTableJoinCol).append(" AS VARCHAR(800))");
				}
			}
		}
		joinString.append(")");
	...
	public String getIndexDetails(String indexName, String tableName, String database, String schema) {
		// TODO: COME BACK TO GETTING THIS COLUMN
		return "SELECT DISTINCT TBL_NAME, null AS COLUMN FROM SQLITE_MASTER WHERE TYPE='index' AND NAME='" + indexName
