...
public class ExtractDatabaseMetaReactor extends AbstractRFrameReactor {
	...
	public NounMetadata execute() {
		...
		/*
		 * if (UserTrackerFactory.isTracking()) { // store descriptions if requested //
		 * if (descriptions) { storeColumnDescriptions(engine); // } }
		 */
	...
	// String[] packages = new String[] { "data.table", "WikidataR", "curl",
	// "doParallel", "XML" };
	// Logger logger = this.getLogger(CLASS_NAME);
	// this.rJavaTranslator.checkPackages(packages);
	// int stepCounter = 1;
	// logger.info(stepCounter + ". Loading R scripts to store column
	// descriptions");
	...
	// "\\R\\Recommendations\\SemanticSimilarity\\lsi_dataitem.r\");\n");
	// rsb.append("source(\"" + baseFolder +
	// "\\R\\Recommendations\\db_recom.r\");\n");
	// rsb.append("source(\"" + baseFolder +
	// "\\R\\Recommendations\\datasemantic.r\");\n");
	// rsb.append("source(\"" + baseFolder +
	// "\\R\\Recommendations\\topic_modelling.r\");\n");
	// this.rJavaTranslator.runR(rsb.toString().replace("\\", "/"));
	// logger.info(stepCounter + ". Done");
	// stepCounter++;
	//
	// // GENERATING DESCRIPTIONS
	// logger.info(stepCounter + ". Getting Database schema to generate
	// descriptions");
	...
	// SimpleQueryFilter nulls = new SimpleQueryFilter(new NounMetadata(colSelector,
	...
	// String header = engine.getEngineId() + seperator + engine.getEngineName() +
	// seperator +
	// table
	// + seperator + col;
	// File newFile = MetaSemanticSimilarityReactor.writeResultToFile(newFileLoc,
	// iterator,
	// header);
	...
	// "),15),c(");
	// sb.append("\"" + engineID + seperator + engineName + seperator + table +
	// seperator + col +
	// "\"");
	// sb.append(")];}\n");
	// //logger.info("Searching description for: " + engine.getEngineName() + ":::"
	// + table +
	// ":::" + col);
	//
	...
	// sb.append("result <- result[(result$ENGINE_ID== \"" + engineID + "\" &
	// result$ENGINE_NAME==
	// \""
	// + engineName + "\" & result$TABLE== \"" + table + "\" & result$COLUMN== \"" +
	// col
	// + "\"),]");
	// this.rJavaTranslator.runR(sb.toString());
	// newFile.delete();
	// descriptions =
	// this.rJavaTranslator.getString("as.character(result[1,\"DESCRIPTION\"])");
	...
	// // sending to table
	// if (primFlag) {
	// logger.info("Found description for: " + engine.getEngineName() + ":::" +
	// table);
	//
	// } else {
	// logger.info("Found description for: " + engine.getEngineName() + ":::" +
	...
	//
	//
	// String gc = "rm(\"a5_97b6491748854929b50f55f5818b1634\",
	// \"a9_8ca904d356784e2d88427675e946b591\","
