...
/**
 * Generates column descriptions and stores in the tracking database Adds unique count to owl file
 * for each column
 */
public class ExtractDatabaseMetaReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    if (engineType == DATABASE_TYPE.RDBMS
        || engineType == DATABASE_TYPE.SESAME
  ...
  //					String header = engine.getEngineId() + seperator + engine.getEngineName() + seperator +
  // table
  //							+ seperator + col;
  //					File newFile = MetaSemanticSimilarityReactor.writeResultToFile(newFileLoc, iterator,
  // header);
  ...
  // "\"");
  //					sb.append(")];}\n");
  //					//logger.info("Searching description for: " + engine.getEngineName() + ":::" + table +
  // ":::" + col);
  //
