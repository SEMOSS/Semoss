...
public class GetDatabasesByDescriptionReactor extends AbstractRFrameReactor {
	...
	public GetDatabasesByDescriptionReactor() {
		...
		this.keysToGet = new String[]{ReactorKeysEnum.DESCRIPTION.getKey(), ReactorKeysEnum.ACCESS.getKey(), LOW_LIMIT,
				MARGIN};
	}
	...
	public NounMetadata execute() {
		...
		File lsa = new File(baseFolder + DIR_SEPARATOR + "R" + DIR_SEPARATOR + "Recommendations" + DIR_SEPARATOR
				+ "dataitem-dbid-desc-lsa.rds");
		File vocab = new File(baseFolder + DIR_SEPARATOR + "R" + DIR_SEPARATOR + "Recommendations" + DIR_SEPARATOR
				+ "dataitem-dbid-desc-lsa-vocab.rds");
		File desc = new File(baseFolder + DIR_SEPARATOR + "R" + DIR_SEPARATOR + "Recommendations" + DIR_SEPARATOR
		...
		if (UserTrackerFactory.isTracking()) {
			...
			for (Object[] row : list) {
				...
				if (enginesWithAccess.contains(id)) {
					...
					// insights =
					// SecurityQueryUtils.getUserInsightsForEngine(this.insight.getUser(), id);
				}
