...
public class GetDatabasesByDescriptionReactor extends AbstractRFrameReactor {
  ...
  public GetDatabasesByDescriptionReactor() {
    ...
    this.keysToGet =
        new String[] {
          ReactorKeysEnum.DESCRIPTION.getKey(), ReactorKeysEnum.ACCESS.getKey(), LOW_LIMIT, MARGIN
        };
  }
    ...
    File vocab =
        new File(
            baseFolder
                ...
                + "Recommendations"
                + DIR_SEPARATOR
                + "dataitem-dbid-desc-lsa-vocab.rds");
    File desc =
        new File(
            baseFolder
                + DIR_SEPARATOR
                ...
                + "Recommendations"
                + DIR_SEPARATOR
                + "dataitem-dbid-desc.rds");
    if (!lsa.exists() || !vocab.exists() || !desc.exists()) {
      String message =
          "Necessary files missing to generate search results. Please run UpdateQueryData() and UpdateSemanticData().";
      NounMetadata noun =
    ...
    if (UserTrackerFactory.isTracking()) {
      ...
      gc.append(
          "rm("
              ...
              + "build_tdm,"
              + "cosine_jaccard_sim, data_domain_mgr,"
              + "dataitem_history, dataitem_recom_mgr,"
              + "drilldown_communities, exec_tfidf,"
              + "find_db, get_dataitem_rating,"
              + "get_item_recom, get_items_users,"
              + "get_similar_doc, get_user_recom,"
              + "get_userdata, hop_away_mgr,"
              + "hop_away_recom_mgr, jaccard_sim,"
              + "locate_data_communities, locate_user_communities,"
              + "lsi_mgr, match_desc,"
              + "outputa8VNk3h38, populate_ratings,"
              + "read_datamatrix, refresh_base,"
              + "remove_files);");
      this.rJavaTranslator.runR(gc.toString());
    }
    return new NounMetadata(
        recommendations, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.RECOMMENDATION);
  }
