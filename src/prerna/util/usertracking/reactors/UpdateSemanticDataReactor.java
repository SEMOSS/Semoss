...
/**
 * Pulls column descriptions from semantic table Generates the .rds files to create recommendations
 * and search
 */
public class UpdateSemanticDataReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    if (UserTrackerFactory.isTracking()) {
      ...
      String[] relevantEngines =
          new String
              [10000]; // arbitrary size.. but should not have more than 10,000 relevant databases
      ...
      if (f.exists()) {
        ...
        String message =
            "Unable to access required dataitem-datadistrict.rds file to update semantic data.";
      ...
      List<String> enginesWithAccess =
          SecurityEngineUtils.getFullUserEngineIds(this.insight.getUser());
      for (String row : enginesWithAccess) {
        extension += row + ";";
      }

      String FILE_URL =
          Utility.getDIHelperProperty("T_ENDPOINT") + "exportTable/semantic" + extension;
      ...
      if (!semanticData.exists()) {
        String message =
            "Unable to connect to the server database for data query and visualization information.";
        NounMetadata retNoun = new NounMetadata(false, PixelDataType.BOOLEAN);
        retNoun.addAdditionalReturn(
            new NounMetadata(message, PixelDataType.CONST_STRING, PixelOperationType.ERROR));
        return retNoun;
        ...
        String gc =
            "rm(\"apply_tfidf\",                        \"assign_unique_concepts\","
                ...
                + "\"refresh_semantic_mgr\",               \"remove_files\","
                + "\"semantic_tracking_mgr\", \""
                + rwd
                + "\");";
