...
public class UpdateSemanticDataReactor extends AbstractRFrameReactor {
	...
	public NounMetadata execute() {
		if (UserTrackerFactory.isTracking()) {
			...
			String[] relevantEngines = new String[10000]; // arbitrary size.. but should not have more than 10,000
															// relevant databases
			String baseFolder = Utility.getBaseFolder().replace("\\", "/");
			File f = new File(baseFolder + DIR_SEPARATOR + "R" + DIR_SEPARATOR + "Recommendations" + DIR_SEPARATOR
			...
			String path = Utility.getBaseFolder().replace("\\", "/") + DIR_SEPARATOR + "R" + DIR_SEPARATOR
					+ "Recommendations" + DIR_SEPARATOR;
