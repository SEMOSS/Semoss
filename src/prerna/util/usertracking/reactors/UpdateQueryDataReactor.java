...
/**
 * Pulls user query information from tracking end point from the widget table Updates the
 * dataitemquery.tsv containing user query information Creates the .rds files used to generate data
 * recommendations
 */
public class UpdateQueryDataReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    if (UserTrackerFactory.isTracking()) {
      ...
      String path =
          baseFolder + DIR_SEPARATOR + "R" + DIR_SEPARATOR + "Recommendations" + DIR_SEPARATOR;
      ...
      if (!queryData.exists()) {
        ...
        if (user != null) {
          ...
          String message =
              "Unable to generage datadistrict file. Please login and run UpdateQueryData() again for enhanced data.";
          logger.info(message);
