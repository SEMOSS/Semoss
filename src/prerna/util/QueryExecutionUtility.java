...
public class QueryExecutionUtility {
  ...
  /**
   * Utility method to flush result set into an integer Assumes single return at index 0
   *
   * @param wrapper
   * @return
   */
   ...
   * @return
   */
  public static Set<String> flushToSetString(
      IDatabaseEngine engine, SelectQueryStruct qs, boolean order) {
  ...
  public static List<String[]> flushRsToListOfStrArray(
      IDatabaseEngine engine, SelectQueryStruct qs) {
  ...
  public static List<Object[]> flushRsToListOfObjArray(
      IDatabaseEngine engine, SelectQueryStruct qs) {
   ...
   * @return
   */
  public static List<Map<String, Object>> flushRsToMap(
      IDatabaseEngine engine, SelectQueryStruct qs) {
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(engine, qs);
   ...
   */
  public static List<Map<String, Object>> flushRsToMap(
      IDatabaseEngine engine, SelectQueryStruct qs, Set<String> mapKeys) {
    IRawSelectWrapper wrapper = null;
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(engine, qs);
   ...
   * @return
   */
  public static List<Map<String, Object>> flushWrapperToMap(
      IRawSelectWrapper wrapper, Set<String> mapKeys) {
      ...
      while (wrapper.hasNext()) {
        ...
        for (int i = 0; i < headers.length; i++) {
          ...
          if (mapKeys != null && mapKeys.contains(headers[i])) {
            Map<String, Object> processedValue =
                convertJsonString(value == null ? values[i] : value);
