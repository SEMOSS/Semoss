...
public class CmdExecUtil {
  ...
  public CmdExecUtil(User user, String insightId, String contextDir) {
    ...
    classLogger.info(
        "Insight id / Project " + insightId + " set at working directory " + workingDir);
  }
  ...
  public String executeCommand(String command) {
    ...
    try {
      if (command.equalsIgnoreCase("reset")) {
        ...
        classLogger.debug(
            "CD command processing: original='"
                + originalCommand
                + "', after cd removal='"
                + command
                + "'");
  ...
  private String[] runCommand(String command) {
    ...
    //	        command = command.replaceFirst("git", "git -c safe.directory='" +
    // this.workingDir+"'");
    //	    }

    org.apache.commons.exec.ExecuteWatchdog.Builder executeWatchdogBuilder =
  ...
  private String adjustWorkingDir(String command) {
    ...
    classLogger.debug(
        "Adjusting working dir - current: '"
            + currentWorkingDir
            + "', command: '"
            + command
            + "', chroot: '"
            + this.chrootFolderPath
            + "'");
