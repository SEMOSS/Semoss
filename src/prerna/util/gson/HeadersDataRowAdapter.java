...
public class HeadersDataRowAdapter extends AbstractSemossTypeAdapter<HeadersDataRow> {
	...
	public HeadersDataRow read(JsonReader in) throws IOException {
		...
		Object[] deserializedValues = deserializeValues(values.toArray(new String[0]),
				valueTypes.toArray(new String[0]));
		Object[] deserializedRawValues = deserializeValues(rawValues.toArray(new String[0]),
				rawValueTypes.toArray(new String[0]));
		HeadersDataRow value = new HeadersDataRow(headers.toArray(new String[0]), rawHeaders.toArray(new String[0]),
				deserializedValues, deserializedRawValues);
		return value;
	}
