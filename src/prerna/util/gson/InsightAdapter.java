...
public class InsightAdapter extends TypeAdapter<Insight> {
	...
	public void write(JsonWriter out, Insight value) throws IOException {
		...
		for (FrameCacheHelper fObj : frames) {
			...
			out.beginObject();
			out.name("file")
					.value(parameterizePath(saveFrame.getFrameCacheLocation(), baseFolder, projectName, projectId));
			out.name("meta")
					.value(parameterizePath(saveFrame.getFrameMetaCacheLocation(), baseFolder, projectName, projectId));
			out.name("state").value(
					parameterizePath(saveFrame.getFrameStateCacheLocation(), baseFolder, projectName, projectId));
