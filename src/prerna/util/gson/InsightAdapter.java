...
public class InsightAdapter extends TypeAdapter<Insight> {
  ...
  public void write(JsonWriter out, Insight value) throws IOException {
    ...
    for (FrameCacheHelper fObj : frames) {
      ...
      out.name("file")
          .value(
              parameterizePath(
                  saveFrame.getFrameCacheLocation(), baseFolder, projectName, projectId));
      out.name("meta")
          .value(
              parameterizePath(
                  saveFrame.getFrameMetaCacheLocation(), baseFolder, projectName, projectId));
      out.name("state")
          .value(
              parameterizePath(
                  saveFrame.getFrameStateCacheLocation(), baseFolder, projectName, projectId));
    ...
    File vizOutputFile =
        new File(
            Utility.normalizePath(this.folderDir) + DIR_SEPARATOR + InsightCacheUtility.VIEW_JSON);
    // lets write it
      ...
      while (in.hasNext()) {
        ...
        if (k.equals("file")) {
          ...
          if (!(new File(normalizedPath).exists())) {
            InsightCacheUtility.unzipFile(
                zip, FilenameUtils.getName(normalizedPath), normalizedPath);
          }
  ...
  private static String parameterizePath(
      String path, String baseFolder, String engineName, String engineId) {
    if (path == null) {
      return null;
    }
    ...
    path =
        path.replace(
            SmssUtilities.getUniqueName(engineName, engineId), "@" + Constants.ENGINE + "@");
    return path;
  }

  private static String deparameterizePath(
      String path, String baseFolder, String engineName, String engineId) {
    if (path == null) {
      return null;
    }
    ...
    if (path.contains("@" + Constants.BASE_FOLDER + "@\\")
        || path.contains("@" + Constants.BASE_FOLDER + "@/")) {
      if (baseFolder.endsWith("/") || baseFolder.endsWith("\\")) {
        baseFolder = baseFolder.substring(0, baseFolder.length() - 1);
      }
    }
    ...
    path =
        path.replace(
            "@" + Constants.ENGINE + "@", SmssUtilities.getUniqueName(engineName, engineId));
    return path;
  }
