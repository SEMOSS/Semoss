...
public class SocialPropertiesProcessor {
  ...
  public void setLoginsAllowed() {
    ...
    Set<String> loginProps =
        socialData.stringPropertyNames().stream()
            .filter(str -> str.endsWith("_login"))
  ...
  public boolean accessKeyAllowed(AuthProvider provider) {
    ...
    boolean accessKeysAllowed =
        Boolean.parseBoolean(
            SocialPropertiesUtil.getInstance().getProperty(prefix + "_access_keys_allowed") + "");
    // LEGACY
    if (!accessKeysAllowed && provider == AuthProvider.MICROSOFT) {
      accessKeysAllowed =
          Boolean.parseBoolean(
              SocialPropertiesUtil.getInstance().getProperty("ms_access_keys_allowed") + "");
    }
  ...
  public void updateAllProperties(String newFileContents) throws IOException {
    ...
    try {
      ...
      if (currentContent != null) {
        ...
        throw new IOException(
            "An error occurred writing the new social properties. Detailed message = "
                + e.getMessage());
      }
    }
  }
   ...
   *
   * <p>#### SAML Attributes goes here ##### Rule for specifying the key-values # applicationKey -
   * Used by the application. This is specified in the code(See SamlAttributeMapperObject class
   * enum). saml_<key_in_saml_assertion> - This is the name of the value specified in AM. Needs to
   * be set here. isMandatory - specifies true or false whether the field is required. Need to be
   * set here, either true or false. defaultValue - specifies a default value in case a field is not
   * required. This is optional to set. Add new key value pairs in the below format.
   * saml_<key_in_saml_assertion> <applicationKey->isMandatory->defaultValue> More than 1 mandatory
   * fields can be specified here.
   *
   * <p>Example:
   *
   * <p>saml_id DOD_EDI_PN_ID saml_name FIRST_NAME+MIDDLE_NAME+LAST_NAME saml_email NULL
  ...
  public Map<String, String[]> getSamlAttributeNames() {
    ...
    Set<String> samlProps =
        this.socialData.stringPropertyNames().stream()
