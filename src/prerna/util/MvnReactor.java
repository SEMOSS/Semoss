...
public class MvnReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (javaHome == null) javaHome = DIHelper.getInstance().getProperty(Settings.JAVA_HOME);
    if (javaHome == null)
      return NounMetadata.getErrorNounMessage(
          "JAVA_HOME is not set, set as environment / RDF_MAP", null);
    System.setProperty("JAVA_HOME", javaHome);

    String mvnHome = System.getProperty(Settings.MVN_HOME);
    if (mvnHome == null) mvnHome = DIHelper.getInstance().getProperty(Settings.MVN_HOME);
    if (mvnHome == null)
      return NounMetadata.getErrorNounMessage(
          "MVN_HOME is not set, set as environment / RDF_MAP", null);

    String workingDir = this.insight.getCmdUtil().getWorkingDir();

    // classes dir
    String classesDir =
        workingDir.substring(0, workingDir.indexOf("app_root") + "app_root".length())
            + File.separator
            + "target";
    ...
    if (!pomFile.exists())
      return NounMetadata.getErrorNounMessage(
          "Not a maven project, please navigate to the dir where you have pom.xml", null);
    ...
    try {
      ...
      if (result.getExitCode() != 0) {
        String errorMessage =
            composeErrorMessage(new File(workingDir + File.separator + "temp.mvn.output"));
  ...
  public String composeErrorMessage(File file) {
    ...
    try (BufferedReader br = new BufferedReader(new FileReader(file))) {
      ...
      while ((data = br.readLine()) != null) output.append(data).append("\n");
