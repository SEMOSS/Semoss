...
public class ModelZKServer implements Watcher, CuratorCacheListener {
		...
		try {
			...
			// if(modelInstances.size() < numInstances)
			{
				...
				if (needObject instanceof Map) {
					...
					// accomodate = false;
					if (accomodate) {
						// check the status to see this is not something we got after someone else
						// released it..
	...
	public NativePySocketClient connect2Py(String modelPath) {
		...
		Object[] outputs = ClientProcessWrapper.startTCPServerNativePy(this.workingDirectoryBasePath, port, null,
				timeout, loggerLevel, true);
	...
	public void processEvent(String path, WatchedEvent event) {
		...
		// if the event is listed.. check the predicate ? <-- not sure I need this given
		// I am making all of it specific
		if (listeners.containsKey(path)) {
