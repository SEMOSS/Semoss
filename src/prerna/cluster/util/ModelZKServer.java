...
public class ModelZKServer implements Watcher, CuratorCacheListener {
  ...
  */
  ...
  public void init() {
    ...
    try {
      env = System.getenv();
      if (env.containsKey(ZK_SERVER)) zkServer = env.get(ZK_SERVER);

      if (env.containsKey(ZK_SERVER.toUpperCase())) zkServer = env.get(ZK_SERVER.toUpperCase());

      if (env.containsKey(HOST)) host = env.get(HOST);

      if (env.containsKey(HOST.toUpperCase())) host = env.get(HOST.toUpperCase());

      int timeout = (30 * 60 * 1000);

      if (env.containsKey(TIMEOUT)) host = env.get(TIMEOUT);

      if (env.containsKey(TIMEOUT.toUpperCase())) host = env.get(TIMEOUT.toUpperCase());

      if (env.containsKey(BOOTUSER)) user = env.get(BOOTUSER);

      if (env.containsKey(BOOTUSER.toUpperCase())) user = env.get(BOOTUSER.toUpperCase());

      if (env.containsKey(HOME)) home = env.get(HOME);

      if (env.containsKey(HOME.toUpperCase())) home = env.get(HOME.toUpperCase());

      if (env.containsKey(APP_HOME)) app = env.get(APP_HOME);

      if (env.containsKey(APP_HOME.toUpperCase())) app = env.get(APP_HOME.toUpperCase());

      if (env.containsKey(SEMOSS_HOME)) semossHome = env.get(SEMOSS_HOME);
  ...
  public NativePySocketClient connect2Py(String modelPath) {
    ...
    Object[] outputs =
        Utility.startTCPServerNativePy(
            this.workingDirectoryBasePath, port, null, timeout, loggerLevel);
    ...
    if (modelPath != null) modelClient.put(modelPath, client);
  ...
  public void event(Type type, ChildData oldData, ChildData data) {
    ...
    // TODO Auto-generated method stub
    if (oldData != null) System.err.println("OLD >> " + oldData.getPath());
    if (data != null) System.err.println("NEW >> " + data.getPath());
    ...
    if (data != null && data.getPath().endsWith("/status")) {
      ...
      String modelPath = data.getPath().replace("/status", "");
      {
        try {
          ...
          String status = getNodeData(statusNode);
          if (status != null && status.equalsIgnoreCase("INIT")) spinModel(propFileAsString);
  ...
  public void addZKListener(IModelZKListener listener) {
    ...
    if (listeners.containsKey(path)) listenerList = listeners.get(path);
  ...
  public void processEvent(String path, WatchedEvent event) {
    ...
    // if the event is listed.. check the predicate ? <-- not sure I need this given I am making all
    // of it specific
