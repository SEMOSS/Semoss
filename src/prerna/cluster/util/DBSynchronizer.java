...
public class DBSynchronizer extends ZKClient {
  ...
  Map<String, Object> pathThreadLock =
      new HashMap<String, Object>(); // this is the main place to claim the monitor
  String namespace = "";
  static DBSynchronizer singleton = null;

  String fileExtensions =
      "py;python;r;js;css;mosfet;json;java;"; // thse are the only file extensions to track //
    ...
    if (zk != null) {
      ...
      try {
        ...
        createIfNotExist(
            namespace,
            db + "_lock",
            version); // , version, null); // create it with the same version as db

        // create the local file watcher
        LocalFileWatcher dbFolderWatcher = null;
        if (dbWatcher.containsKey(dbFolderName)) dbFolderWatcher = dbWatcher.get(dbFolderName);
        else dbFolderWatcher = new LocalFileWatcher(this, db);
  ...
  public void registerRecurse(List<String> files, String db) {
      ...
      if (curDir.isDirectory()) {
        ...
        for (int curDirFileIndex = 0; curDirFileIndex < curDirFiles.length; curDirFileIndex++) {
          ...
          if (curDirFile.isDirectory()) {
            ...
            if (isDirectoryAllowed(curDirFile)) nextLevel.add(path);
          } else {
    ...
    if (nextLevel.size() > 0) registerRecurse(nextLevel, db);
  }
