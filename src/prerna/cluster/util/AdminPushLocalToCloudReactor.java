...
public class AdminPushLocalToCloudReactor extends AbstractReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(AdminPushLocalToCloudReactor.class);
  ...
  public NounMetadata execute() {
    ...
    // get all engines
    List<String> dbIds =
        SecurityEngineUtils.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.DATABASE.name()));
    List<String> storageIds =
        SecurityEngineUtils.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.STORAGE.name()));
    List<String> modelIds =
        SecurityEngineUtils.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.MODEL.name()));
    List<String> vectorIds =
        SecurityEngineUtils.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.VECTOR.name()));
    List<String> functionIds =
        SecurityEngineUtils.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.FUNCTION.name()));
    ...
    try {
      ...
      removeExisitngIds(
          CentralCloudStorage.DATABASE_BLOB,
          currentContainers.get(CentralCloudStorage.DATABASE_BLOB),
          dbIds);
      removeExisitngIds(
          CentralCloudStorage.STORAGE_BLOB,
          currentContainers.get(CentralCloudStorage.STORAGE_BLOB),
          storageIds);
      removeExisitngIds(
          CentralCloudStorage.MODEL_BLOB,
          currentContainers.get(CentralCloudStorage.MODEL_BLOB),
          modelIds);
      removeExisitngIds(
          CentralCloudStorage.VECTOR_BLOB,
          currentContainers.get(CentralCloudStorage.VECTOR_BLOB),
          vectorIds);
      removeExisitngIds(
          CentralCloudStorage.FUNCTION_BLOB,
          currentContainers.get(CentralCloudStorage.FUNCTION_BLOB),
          functionIds);
      removeExisitngIds(
          CentralCloudStorage.PROJECT_BLOB,
          currentContainers.get(CentralCloudStorage.PROJECT_BLOB),
          projectIds);
   ...
   * @param startingList
   */
  private void removeExisitngIds(
      String bucket, List<String> cloudFiles, List<String> startingList) {
