...
public class AdminPushLocalToCloudReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		List<String> storageIds = SecurityEngineUtils
				.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.STORAGE.name()));
		...
		List<String> functionIds = SecurityEngineUtils
				.getAllEngineIds(Arrays.asList(IEngine.CATALOG_TYPE.FUNCTION.name()));
		...
		try {
			...
			removeExisitngIds(CentralCloudStorage.DATABASE_BLOB,
					currentContainers.get(CentralCloudStorage.DATABASE_BLOB), dbIds);
			removeExisitngIds(CentralCloudStorage.STORAGE_BLOB, currentContainers.get(CentralCloudStorage.STORAGE_BLOB),
					storageIds);
			removeExisitngIds(CentralCloudStorage.MODEL_BLOB, currentContainers.get(CentralCloudStorage.MODEL_BLOB),
					modelIds);
			removeExisitngIds(CentralCloudStorage.VECTOR_BLOB, currentContainers.get(CentralCloudStorage.VECTOR_BLOB),
					vectorIds);
			removeExisitngIds(CentralCloudStorage.FUNCTION_BLOB,
					currentContainers.get(CentralCloudStorage.FUNCTION_BLOB), functionIds);
			removeExisitngIds(CentralCloudStorage.PROJECT_BLOB, currentContainers.get(CentralCloudStorage.PROJECT_BLOB),
					projectIds);
