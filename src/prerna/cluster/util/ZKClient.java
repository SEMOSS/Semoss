...
public class ZKClient implements Watcher {
  ...
  */
  ...
  public void watchEvent(
      String path, EventType eventType, IZKListener listener, boolean watchAgain) {
    String key = path + "_" + eventType;
    List<IZKListener> llist = new Vector<IZKListener>();
    if (listeners.containsKey(key)) llist = listeners.get(key);

    llist.add(listener);
    listeners.put(path + "_" + eventType, llist);
    repeat.put(path + "_" + eventType, watchAgain);

    if (eventType == EventType.NodeChildrenChanged) watchPath(path);
  ...
  public void process(WatchedEvent event) {
    ...
    if (path != null) {
      ...
      if (repeat.containsKey(key) && repeat.get(key)) {
        if (event.getType() == EventType.NodeChildrenChanged) watchPath(path);
        else if (event.getType() == EventType.NodeDataChanged
            || event.getType() == EventType.NodeDeleted) watchPathD(path);
      }
      /*			if(repeat.containsKey(key) && repeat.get(key))
      			else
      			{
  ...
  public String createIfNotExist(
      String namespace, String newNode, String version, Watcher watcher) {
    try {
  ...
  public void createIfNotExist(String namespace, String newNode, String data) {
    ...
    try {
      ...
      if (stat == null) {
        // create the node
        zk.create(
            namespace + "/" + newNode, data.getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
