...
public class ZKClient implements Watcher {
	...
	/*
	 * // Environment Variables this depnds on zk - semicolon separated list of zk
	 * to use for registration home - what is the main registration root - default
	 * is assumed as /semoss_root app - The root for various apps - default is
	 * assumed as /app host - ip and port - this is useful if you are running
	 * multiple containers on the same box bu - the boot user who is being loaded on
	 * this box - defaults to generic DNS container address
	 * 
	 * // registers to semoss_root/semoss- as a ephemeral_sequential // the
	 * semoss_root is an environment variable - this will allow me to spin as many
	 * cluster as I want Major version minor version semoss_sequential node host ip
	 * and port number of cpus memory User who is booted on it The registration is
	 * typically register_root / #cpu / #memory / semoss-node register_root /
	 * semoss-node nginx will only watch the resigter_root/semoss_node initially I
	 * could even do this for specific databases so the same thing is not loaded
	 * multiple times // zk e
	 * 
	 */
	...
	public void publishNode() {
		...
		try {
			zk.create(home + "/semoss", getPayload().getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE,
	...
	public void publishContainer(String ipPort) {
		try {
			zk.create(home + container + "/" + ipPort, host.getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE,
	...
	// TODO >>>timb:ought to do a true zk watch on this (event comes in, then
	// process in some way)
	...
	public void process(WatchedEvent event) {
		...
		if (path != null) {
			...
			/*
			 * if(repeat.containsKey(key) && repeat.get(key)) else { listeners.remove(key);
			 * }
			 */ }
	}
	...
	public String createSchedulerNode(String node, boolean watch, boolean ephimeral) {
		...
		try {
			...
			if (nodeStat == null) {
				createdNodePath = zk.create(node, new byte[0], Ids.OPEN_ACL_UNSAFE,
	...
	public String createIfNotExist(String namespace, String newNode, String version, Watcher watcher) {
		try {
			...
			if (stat == null) {
				...
				zk.create(namespace + "/" + newNode + "/lock", version.getBytes(), Ids.OPEN_ACL_UNSAFE,
