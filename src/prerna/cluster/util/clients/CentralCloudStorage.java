...
public class CentralCloudStorage implements ICloudClient {
	...
	public void pullEngine(String engineId, IEngine.CATALOG_TYPE engineType, boolean engineAlreadyLoaded)
		...
		try {
			...
			classLogger.info("Pulling engine from remote=" + Utility.cleanLogString(aliasAndEngineId) + " to target="
			...
			classLogger.debug("Done pulling from remote=" + Utility.cleanLogString(aliasAndEngineId) + " to target="
					+ Utility.cleanLogString(localEngineFolder));

			// Now pull the smss
			classLogger.info("Pulling smss from remote=" + Utility.cleanLogString(cloudEngineSmssFolder) + " to target="
			...
			classLogger.debug("Done pulling from remote=" + Utility.cleanLogString(cloudEngineSmssFolder)
					+ " to target=" + localEngineBaseFolder);
	...
	public void pushEngineFolder(String engineId, String localAbsoluteFilePath, String storageRelativePath)
		...
		classLogger.info("Applying lock for engine " + aliasAndEngineId + " to push engine relative folder "
				+ storageRelativePath);
		ReentrantLock lock = EngineSyncUtility.getEngineLock(engineId);
		lock.lock();
		classLogger.info("Engine " + aliasAndEngineId + " is locked");
		try {
			classLogger
					.info("Pushing folder from local=" + localAbsoluteFilePath + " to remote=" + storageRelativePath);
	...
	public void pullEngineFolder(String engineId, String localAbsoluteFilePath, String storageRelativePath)
		...
		classLogger.info("Applying lock for engine " + aliasAndEngineId + " to pull engine relative folder "
				+ storageRelativePath);
		ReentrantLock lock = EngineSyncUtility.getEngineLock(engineId);
		lock.lock();
		classLogger.info("Engine " + aliasAndEngineId + " is locked");
	...
	public void copyLocalFileToEngineCloudFolder(String engineId, CATALOG_TYPE engineType, String localFilePath)
		...
		classLogger.info("Applying lock for engine " + aliasAndEngineId + " to push local file " + localFilePath
				+ " to cloud path " + cloudStorageFolderPath);
		ReentrantLock lock = EngineSyncUtility.getEngineLock(engineId);
		lock.lock();
		classLogger.info("Engine " + aliasAndEngineId + " is locked");
		try {
	...
	public void copyEngineCloudFileToLocalFile(String engineId, CATALOG_TYPE engineType, String localFilePath)
		...
		classLogger.info("Applying lock for engine " + aliasAndEngineId + " to pull cloud path "
				+ cloudStorageFolderPath + " to local file " + localFilePath);
		ReentrantLock lock = EngineSyncUtility.getEngineLock(engineId);
		lock.lock();
		classLogger.info("Engine " + aliasAndEngineId + " is locked");
		try {
	...
	public void deleteEngineCloudFile(String engineId, CATALOG_TYPE engineType, String localFilePath)
		...
		classLogger.info(
				"Applying lock for engine " + aliasAndEngineId + " to delete cloud path " + cloudStorageFolderPath);
		ReentrantLock lock = EngineSyncUtility.getEngineLock(engineId);
		lock.lock();
		classLogger.info("Engine " + aliasAndEngineId + " is locked");
		try {
	...
	public void pushLocalDatabaseFile(String databaseId, RdbmsTypeEnum dbType)
		...
		try {
			...
			classLogger.info(
					"Pushing local database file from " + localDatabaseFolder + " to remote " + storageDatabaseFolder);
	...
	public void pushProjectFolder(String projectId, String localAbsoluteFilePath, String storageRelativePath)
		...
		classLogger.info("Applying lock for project " + aliasAndProjectId + " to push project relative folder "
				+ storageRelativePath);
		ReentrantLock lock = ProjectSyncUtility.getProjectLock(projectId);
		lock.lock();
		classLogger.info("Project " + aliasAndProjectId + " is locked");
		try {
			classLogger.info(
					"Pushing folder from local=" + localAbsoluteFilePath + " to remote=" + storageProjectFolderPath);
	...
	public void pullProjectFolder(String projectId, String localAbsoluteFilePath, String storageRelativePath)
		...
		classLogger.info("Applying lock for project " + aliasAndProjectId + " to pull project relative folder "
				+ storageRelativePath);
		ReentrantLock lock = ProjectSyncUtility.getProjectLock(projectId);
		lock.lock();
		classLogger.info("Project " + aliasAndProjectId + " is locked");
		try {
			classLogger.info(
					"Pulling folder from remote=" + storageProjectFolderPath + " to local=" + localAbsoluteFilePath);
	...
	public void pushInsight(String projectId, String insightId) throws IOException, InterruptedException {
		...
		classLogger.info("Pushing insight from local=" + Utility.cleanLogString(localInsightFolderPath) + " to remote="
		...
		classLogger.debug("Done pushing insight from local=" + Utility.cleanLogString(localInsightFolderPath)
				+ " to remote=" + Utility.cleanLogString(storageInsightFolder));
	}
	...
	public void pullInsight(String projectId, String insightId) throws IOException, InterruptedException {
		...
		classLogger.info("Pulling insight from remote=" + Utility.cleanLogString(storageInsightFolder) + " to target="
		...
		classLogger.debug("Done pulling insight from remote=" + Utility.cleanLogString(storageInsightFolder)
				+ " to target=" + Utility.cleanLogString(localInsightFolderPath));
	}
	...
	public void pushInsightImage(String projectId, String insightId, String oldImageFileName, String newImageFileName)
		...
		try {
			...
			if (oldImageFileName != null) {
				...
				classLogger.info(
						"Deleting old insight image from remote=" + Utility.cleanLogString(storageOldFileToDelete));
			...
			if (newImageFileName != null) {
				...
				classLogger.info("Pushing insight image from local=" + Utility.cleanLogString(localInsightImageFilePath)
						+ " to remote=" + Utility.cleanLogString(storageInsightFolder));
				centralStorageEngine.copyToStorage(localInsightImageFilePath, storageInsightFolder, sharedRCloneConfig,
						null);
				classLogger.debug(
						"Done pushing insight image from local=" + Utility.cleanLogString(localInsightImageFilePath)
								+ " to remote=" + Utility.cleanLogString(storageInsightFolder));
			} else {
	...
	public void pullUserAssetOrWorkspace(String projectId, boolean isAsset, boolean projectAlreadyLoaded)
		...
		try {
			...
			try {
				...
				classLogger.info("Pulling user asset/workspace from remote=" + storageUserAssetWorkspaceFolder
						+ " to target=" + localUserAndAssetFolder);
				centralStorageEngine.syncStorageToLocal(storageUserAssetWorkspaceFolder, localUserAndAssetFolder,
						sharedRCloneConfig);
				classLogger.debug("Done pulling from remote=" + storageUserAssetWorkspaceFolder + " to target="
						+ localUserAndAssetFolder);

				// Now pull the smss
				classLogger.info(
						"Pulling smss from remote=" + storageSmssFolder + " to target=" + EngineUtility.USER_FOLDER);
				...
				classLogger.debug(
						"Done pulling from remote=" + storageSmssFolder + " to target=" + EngineUtility.USER_FOLDER);
			} finally {
				// Re-open the project
				if (projectAlreadyLoaded) {
  ...
  ////////////////////////////////////////////////////////////////////////
	...
	// DIHelper.getInstance().setEngineProperty("56af9395-64fd-40a2-b68c-bbd6961336a5",
	// sampleDb);
	// }
	//
	...
	// centralStorage.pullEngine("56af9395-64fd-40a2-b68c-bbd6961336a5", null,
	// true);
