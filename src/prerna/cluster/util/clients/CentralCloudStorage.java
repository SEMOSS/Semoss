...
public class CentralCloudStorage implements ICloudClient {
  ...
  private static synchronized void buildStorageEngine() throws Exception {
    ...
    if (ClusterUtil.STORAGE_PROVIDER == null
        || ClusterUtil.STORAGE_PROVIDER.equalsIgnoreCase("AZURE")) {

      centralStorageEngine = new AzureBlobStorageEngine();
      propertiesMigratePut(
          props,
          AzureBlobStorageEngine.AZ_ACCOUNT_NAME,
          clientProps,
          AbstractClientBuilder.AZ_NAME);
      propertiesMigratePut(
          props, AzureBlobStorageEngine.AZ_PRIMARY_KEY, clientProps, AbstractClientBuilder.AZ_KEY);
      propertiesMigratePut(
          props,
          AzureBlobStorageEngine.AZ_CONN_STRING,
          clientProps,
          AbstractClientBuilder.AZ_CONN_STRING);
      propertiesMigratePut(
          props,
          AzureBlobStorageEngine.AZ_GENERATE_DYNAMIC_SAS,
          clientProps,
          AbstractClientBuilder.AZ_GENERATE_DYNAMIC_SAS);
      propertiesMigratePut(
          props, AzureBlobStorageEngine.AZ_USE_MSI, clientProps, AzureBlobStorageEngine.AZ_USE_MSI);

      // we have a different structure for AZ storage since it doesn't represent the blobs as
      // folders
      ...
      propertiesMigratePut(
          props, S3StorageEngine.S3_REGION_KEY, clientProps, AbstractClientBuilder.S3_REGION_KEY);
      propertiesMigratePut(
          props, S3StorageEngine.S3_BUCKET_KEY, clientProps, AbstractClientBuilder.S3_BUCKET_KEY);
      propertiesMigratePut(
          props, S3StorageEngine.S3_ACCESS_KEY, clientProps, AbstractClientBuilder.S3_ACCESS_KEY);
      propertiesMigratePut(
          props, S3StorageEngine.S3_SECRET_KEY, clientProps, AbstractClientBuilder.S3_SECRET_KEY);
      ...
      propertiesMigratePut(
          props,
          GoogleCloudStorageEngine.GCS_REGION,
          clientProps,
      ...
      propertiesMigratePut(
          props,
          GoogleCloudStorageEngine.GCS_BUCKET_KEY,
          clientProps,
   ...
   * @param oldKey
   */
  private static void propertiesMigratePut(
      Properties prop, String propKey, AppCloudClientProperties clientProps, String oldKey) {
   ...
   * @param type
   */
  public void catalogPulledEngine(
      String aliasAndEngineId, String localSmssFileName, IEngine.CATALOG_TYPE type) {
    if (IEngine.CATALOG_TYPE.DATABASE == type) {
  ...
  public void pushEngine(String engineId) throws IOException, InterruptedException {
    ...
    String localEngineFolder =
        Utility.normalizePath(localEngineBaseFolder + FILE_SEPARATOR + aliasAndEngineId);
    {
    ...
    classLogger.info(
        "Applying lock for " + aliasAndEngineId + " to push engine " + aliasAndEngineId);
    ...
    try {
      ...
      centralStorageEngine.syncLocalToStorage(
          localEngineFolder, cloudEngineFolder, sharedRCloneConfig, null);
      centralStorageEngine.copyToStorage(
          localSmssFilePath, cloudSmssFolder, sharedRCloneConfig, null);
    } finally {
      try {
        // Re-open the engine
        if (engine.holdsFileLocks()) {
  ...
  @Override
  public void pullEngine(String engineId, IEngine.CATALOG_TYPE engineType)
  ...
  public void pullEngine(
      String engineId, IEngine.CATALOG_TYPE engineType, boolean engineAlreadyLoaded)
      throws IOException, InterruptedException {
    IEngine engine = null;
    ...
    String localSmssFileName = aliasAndEngineId + ".smss";
    String localSmssFilePath =
        Utility.normalizePath(localEngineBaseFolder + FILE_SEPARATOR + localSmssFileName);
    ...
    try {
      if (engine != null) {
        try {
          ...
          classLogger.warn(
              "Error occurred trying to close engine "
                  + aliasAndEngineId
                  + " - will proceed with pulling details in attempt to fix engine when re-opened");
          classLogger.error(Constants.STACKTRACE, e);
        }
      }
      if (centralStorageEngine.canReuseRcloneConfig()) {
      ...
      classLogger.info(
          "Pulling engine from remote="
              + Utility.cleanLogString(aliasAndEngineId)
              + " to target="
              + Utility.cleanLogString(localEngineFolder));
      centralStorageEngine.syncStorageToLocal(
          cloudEngineFolder, localEngineFolder, sharedRCloneConfig);
      classLogger.debug(
          "Done pulling from remote="
              + Utility.cleanLogString(aliasAndEngineId)
              + " to target="
              + Utility.cleanLogString(localEngineFolder));
      ...
      // Now pull the smss
      classLogger.info(
          "Pulling smss from remote="
              + Utility.cleanLogString(cloudEngineSmssFolder)
              + " to target="
              + localEngineBaseFolder);
      // THIS MUST BE COPY AND NOT SYNC TO AVOID DELETING EVERYTHING IN THE DB FOLDER
      centralStorageEngine.copyToLocal(
          cloudEngineSmssFolder, localEngineBaseFolder, sharedRCloneConfig);
      classLogger.debug(
          "Done pulling from remote="
              + Utility.cleanLogString(cloudEngineSmssFolder)
              + " to target="
  ...
  @Override
  public void pushEngineFolder(
      String engineId, String localAbsoluteFilePath, String storageRelativePath)
      throws IOException, InterruptedException {
    IEngine engine = Utility.getEngine(engineId, false);
    if (engine == null) {
      throw new IllegalArgumentException("Engine not found...");
    ...
    classLogger.info(
        "Applying lock for engine "
            + aliasAndEngineId
            + " to push engine relative folder "
            + storageRelativePath);
    ReentrantLock lock = EngineSyncUtility.getEngineLock(engineId);
    lock.lock();
    classLogger.info("Engine " + aliasAndEngineId + " is locked");
  ...
  public void pullEngineFolder(
    ...
    classLogger.info(
        "Applying lock for engine "
            + aliasAndEngineId
            + " to pull engine relative folder "
            + storageRelativePath);
    ReentrantLock lock = EngineSyncUtility.getEngineLock(engineId);
    lock.lock();
    classLogger.info("Engine " + aliasAndEngineId + " is locked");
    try {
      classLogger.info(
          "Pulling folder from remote=" + cloudEngineFolder + " to local=" + localAbsoluteFilePath);
  ...
  @Override
  public void copyLocalFileToEngineCloudFolder(
      String engineId, CATALOG_TYPE engineType, String localFilePath)
      throws IOException, InterruptedException {
    IEngine engine = Utility.getEngine(engineId, false);
    if (engine == null) {
      throw new IllegalArgumentException("Engine not found...");
    ...
    String localEngineFolder =
        EngineUtility.getLocalEngineBaseDirectory(engineType) + FILE_SEPARATOR + aliasAndEngineId;
    ...
    String cloudStorageFolderPath =
        getCloudPrefixForEngine(engineType) + engineId + "/" + storagePath;
    ...
    try {
      classLogger.info(
          "Pushing folder local=" + localFilePath + " to from remote=" + cloudStorageFolderPath);
  ...
  @Override
  public void copyEngineCloudFileToLocalFile(
      String engineId, CATALOG_TYPE engineType, String localFilePath)
      throws IOException, InterruptedException {
    IEngine engine = Utility.getEngine(engineId, false);
    if (engine == null) {
    ...
    try {
      classLogger.info(
          "Pulling remote=" + cloudStorageFolderPath + " to folder local=" + localFilePath);
  ...
  public void pushLocalDatabaseFile(String databaseId, RdbmsTypeEnum dbType)
    ...
    String localDatabaseFolder =
        EngineUtility.DATABASE_FOLDER + FILE_SEPARATOR + aliasAndDatabaseId;
    ...
    try {
      ...
      classLogger.info(
          "Pushing local database file from "
              + localDatabaseFolder
              + " to remote "
      ...
      for (String dbFileName : dbFiles) {
        centralStorageEngine.copyToStorage(
            localDatabaseFolder + "/" + dbFileName, storageDatabaseFolder, null);
      }
    } finally {
      try {
  ...
  public void pullLocalDatabaseFile(String databaseId, RdbmsTypeEnum rdbmsType)
    ...
    try {
      ...
      classLogger.info(
          "Pulling database files for " + aliasAndDatabaseId + " from remote=" + databaseId);
      List<String> filesToPull = new ArrayList<>();
      List<String> cloudFiles =
          centralStorageEngine.list(storageDatabaseFolder, sharedRCloneConfig);
      ...
      for (String fileToPull : filesToPull) {
        centralStorageEngine.copyToLocal(
            storageDatabaseFolder + "/" + fileToPull, localDatabaseFolder, sharedRCloneConfig);
      }
    } finally {
      try {
        // Re-open the database
  ...
  @Override
  public void pushOwl(String databaseId, WriteOWLEngine owlEngine)
      throws IOException, InterruptedException {
    IDatabaseEngine database = Utility.getDatabase(databaseId, false);
    if (database == null) {
      throw new IllegalArgumentException("Database not found...");
    ...
    String localOwlPositionFile =
        localOwlF.getParent() + "/" + AbstractDatabaseEngine.OWL_POSITION_FILENAME;
    ...
    // synchronize on the engine id
    classLogger.info(
        "Applying lock for " + aliasAndDatabaseId + " to push database owl and postions.json");
    ReentrantLock lock = EngineSyncUtility.getEngineLock(databaseId);
    lock.lock();
    classLogger.info("Database " + aliasAndDatabaseId + " is locked");
    try {
      ...
      centralStorageEngine.copyToStorage(
          localOwlFile, storageDatabaseFolder, sharedRCloneConfig, null);
      if (hasPositionFile) {
        centralStorageEngine.copyToStorage(
            localOwlPositionFile, storageDatabaseFolder, sharedRCloneConfig, null);
      }
    } finally {
      if (sharedRCloneConfig != null) {
        try {
  ...
  @Override
  public void pullOwl(String databaseId, WriteOWLEngine owlEngine)
      throws IOException, InterruptedException {
    IDatabaseEngine database = Utility.getDatabase(databaseId, false);
    if (database == null) {
      throw new IllegalArgumentException("Database not found...");
    ...
    String storageDatabaseOwlPosition =
        storageDatabaseFolder + "/" + AbstractDatabaseEngine.OWL_POSITION_FILENAME;
  ...
  public void pushProject(String projectId) throws IOException, InterruptedException {
    ...
    try {
      ...
      centralStorageEngine.syncLocalToStorage(
          localProjectFolder, storageProjectFolder, sharedRCloneConfig, null);
      centralStorageEngine.copyToStorage(
          localSmssFilePath, storageSmssFolder, sharedRCloneConfig, null);
    } finally {
      try {
        // Re-open the project
  ...
  @Override
  public void pullProject(String projectId, boolean projectAlreadyLoaded)
      throws IOException, InterruptedException {
    ...
    try {
      ...
      centralStorageEngine.syncStorageToLocal(
          storageProjectFolder, localProjectFolder, sharedRCloneConfig);
      centralStorageEngine.copyToLocal(
          storageSmssFolder, EngineUtility.PROJECT_FOLDER, sharedRCloneConfig);
  ...
  public void pushProjectSmss(String projectId) throws IOException, InterruptedException {
    ...
    String localSmssFilePath =
        Utility.normalizePath(EngineUtility.PROJECT_FOLDER + FILE_SEPARATOR + localSmssFileName);
  ...
  public void pullInsightsDB(String projectId) throws IOException, InterruptedException {
    ...
    String storageProjectInsightFilePath =
        PROJECT_CONTAINER_PREFIX + projectId + "/" + insightDbFileName;
    // synchronize on the project id
    ...
    try {
      ...
      try {
        // open the insight db
        String insightDbLoc =
            SmssUtilities.getInsightsRdbmsFile(project.getSmssProp()).getAbsolutePath();
        if (insightDbLoc != null) {
          try {
            project.setInsightDatabase(
                ProjectHelper.loadInsightsEngine(
                    project.getSmssProp(), LogManager.getLogger(AbstractDatabaseEngine.class)));
          } catch (Exception e) {
            classLogger.error(Constants.STACKTRACE, e);
  ...
  @Override
  public void pushProjectFolder(
      String projectId, String localAbsoluteFilePath, String storageRelativePath)
      throws IOException, InterruptedException {
    IProject project = Utility.getProject(projectId, false);
    if (project == null) {
      throw new IllegalArgumentException("Project not found...");
    ...
    classLogger.info(
        "Applying lock for project "
            + aliasAndProjectId
            + " to push project relative folder "
            + storageRelativePath);
    ReentrantLock lock = ProjectSyncUtility.getProjectLock(projectId);
    lock.lock();
    classLogger.info("Project " + aliasAndProjectId + " is locked");
    try {
      ...
      centralStorageEngine.syncLocalToStorage(
          localAbsoluteFilePath, storageProjectFolderPath, null);
    } finally {
      // always unlock regardless of errors
      lock.unlock();
      classLogger.info("Project " + aliasAndProjectId + " is unlocked");
  ...
  public void pullProjectFolder(
    ...
    classLogger.info(
        "Applying lock for project "
            + aliasAndProjectId
            + " to pull project relative folder "
            + storageRelativePath);
    ReentrantLock lock = ProjectSyncUtility.getProjectLock(projectId);
    lock.lock();
    classLogger.info("Project " + aliasAndProjectId + " is locked");
  ...
  @Override
  public void pushInsight(String projectId, String insightId)
      throws IOException, InterruptedException {
    ...
    // only need to pull the insight folder - 99% the project is always already loaded to get to
    // this point
    String localInsightFolderPath =
    ...
    classLogger.debug(
        "Done pushing insight from local="
            + Utility.cleanLogString(localInsightFolderPath)
            + " to remote="
            + Utility.cleanLogString(storageInsightFolder));
  }
  ...
  @Override
  public void pullInsight(String projectId, String insightId)
      throws IOException, InterruptedException {
    IProject project = Utility.getProject(projectId);
    if (project == null) {
      throw new IllegalArgumentException("Project not found...");
  ...
  @Override
  public void pushInsightImage(
      String projectId, String insightId, String oldImageFileName, String newImageFileName)
      throws IOException, InterruptedException {
    IProject project = Utility.getProject(projectId, false);
    if (project == null) {
      throw new IllegalArgumentException("Project not found...");
    ...
    try {
      ...
      if (oldImageFileName != null) {
        ...
        classLogger.info(
            "Deleting old insight image from remote="
        ...
        classLogger.debug(
            "Done deleting old insight image from remote="
                + Utility.cleanLogString(storageOldFileToDelete));
      } else {
      ...
      if (newImageFileName != null) {
        String localInsightImageFilePath =
            Utility.normalizePath(
                AssetUtility.getProjectVersionFolder(project.getProjectName(), projectId)
                    + "/"
                    + insightId
                    ...
                    + newImageFileName);
        classLogger.info(
            "Pushing insight image from local="
                + Utility.cleanLogString(localInsightImageFilePath)
                + " to remote="
                + Utility.cleanLogString(storageInsightFolder));
        centralStorageEngine.copyToStorage(
            localInsightImageFilePath, storageInsightFolder, sharedRCloneConfig, null);
        classLogger.debug(
            "Done pushing insight image from local="
                + Utility.cleanLogString(localInsightImageFilePath)
                + " to remote="
                + Utility.cleanLogString(storageInsightFolder));
      } else {
  ...
  @Override
  public void pullUserAssetOrWorkspace(
      String projectId, boolean isAsset, boolean projectAlreadyLoaded)
      throws IOException, InterruptedException {
    IProject project = null;
    String alias = null;
    if (projectAlreadyLoaded) {
    ...
    String localUserAndAssetFolder =
        EngineUtility.USER_FOLDER + FILE_SEPARATOR + aliasAndUserAssetWorkspaceId;
    ...
    try {
      ...
      try {
        ...
        classLogger.info(
            "Pulling user asset/workspace from remote="
                + storageUserAssetWorkspaceFolder
                + " to target="
                + localUserAndAssetFolder);
        centralStorageEngine.syncStorageToLocal(
            storageUserAssetWorkspaceFolder, localUserAndAssetFolder, sharedRCloneConfig);
        classLogger.debug(
            "Done pulling from remote="
                + storageUserAssetWorkspaceFolder
                + " to target="
        ...
        centralStorageEngine.copyToLocal(
            storageSmssFolder, EngineUtility.USER_FOLDER, sharedRCloneConfig);
        classLogger.debug(
            "Done pulling from remote="
                + storageSmssFolder
                + " to target="
  ...
  @Override
  public void pushUserAssetOrWorkspace(String projectId, boolean isAsset)
      throws IOException, InterruptedException {
    ...
    String localUserAssetWorkspaceFolder =
        EngineUtility.USER_FOLDER + FILE_SEPARATOR + aliasAndUserAssetWorkspaceId;
    ...
    try {
      ...
      centralStorageEngine.syncLocalToStorage(
          localUserAssetWorkspaceFolder, storageUserAssetWorkspaceFolder, sharedRCloneConfig, null);
      centralStorageEngine.copyToStorage(
          localSmssFilePath, storageSmssFolder, sharedRCloneConfig, null);
    } finally {
      try {
  ...
  public void pushRoomFolderToCloud(String roomId) throws IOException, InterruptedException {
    ...
    if (Utility.folderHasAnyFiles(localFolderPath)) {
      centralStorageEngine.syncLocalToStorage(
          localFolderPath, ROOM_CONTAINER_PREFIX + roomId, null);
    }
  }
  ...
  protected String getInsightDB(IProject project, String specificProjectFolder) {
    ...
    throw new IllegalArgumentException(
        "There is no insight database for project: " + project.getProjectName());
  }

  ///////////////////////////////////////////////////////////////////////////////////

  @Override
  public Map<String, List<String>> listAllContainersByBucket()
      throws IOException, InterruptedException {
  ...
  //			String engineProp = baseFolder + "\\db\\diabetes sanjay and
  // sarji__56af9395-64fd-40a2-b68c-bbd6961336a5.smss";
  ...
  //		centralStorage.pullLocalDatabaseFile("56af9395-64fd-40a2-b68c-bbd6961336a5",
  // RdbmsTypeEnum.H2_DB);
  //	}
