...
public class LocalFileWatcher implements Runnable {
  ...
  String fileExtensions =
      "py;python;r;js;css;mosfet;json;"; // thse are the only file extensions to track
  ...
  public void watchFilePath(String path) {
    try {
      ...
      Path thisPath = Paths.get(path);
      thisPath.register(
          ws, StandardWatchEventKinds.ENTRY_MODIFY, StandardWatchEventKinds.ENTRY_DELETE);
    } catch (IOException e) {
      // TODO Auto-generated catch block
      classLogger.error(Constants.STACKTRACE, e);
    }
      ...
      while ((key = ws.take()) != null) {
        for (WatchEvent<?> event : key.pollEvents()) {
          ...
          if (event.kind() == StandardWatchEventKinds.ENTRY_MODIFY
              && fileExtensions.indexOf(extn) >= 0) // do it only if it is a valid file extension
          dbs.modifyNode(db, parent + "/" + event.context());
            ...
            if (file.isDirectory()) watchFilePath(parent + "/" + event.context());
