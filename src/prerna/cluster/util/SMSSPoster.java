...
public class SMSSPoster extends ModelZKServer implements Watcher, CuratorCacheListener {
	...
	public void process(WatchedEvent event) {
		...
		/*
		 * if(type == EventType.NodeDataChanged) { try { String path = event.getPath();
		 * // got the status if(path != null && path.equalsIgnoreCase(MODEL_PATH + "/" +
		 * modelId + "/status")) { // ok we got our bogey //Stat ver = new Stat(1, 1, 0,
		 * 0, 1, 0, 0, 1, smssFileAsString.length(), 3, 1); byte [] bstatus =
		 * zk.getData(MODEL_PATH + "/" + modelId + "/status", true, new Stat()); String
		 * status = new String(bstatus, "UTF-8");
		 * if(status.equalsIgnoreCase("Available")) {
		 * System.err.println("We are done here.. ");
		 * 
		 * String endpointNode = MODEL_PATH + "/" + modelId + "/endpoint";
		 * 
		 * // listen to status changes //zk.addWatch(endpointNode,
		 * AddWatchMode.PERSISTENT);
		 * 
		 * String endpoint = getNodeData(endpointNode);
		 * //System.out.println("Endpoint .. " + endpoint); //Stat ver = new Stat(1, 1,
		 * 0, 0, 1, 0, 0, 1, endpoint.length(), 3, 1); //zk.getData(endpointNode, true,
		 * ver); } } if(path != null && path.endsWith("fail")) // process failure {
		 * String strServers = getNodeData("/server"); long servers =
		 * Long.parseLong(strServers);
		 * 
		 * String strFails = getNodeData(path); long fails = Long.parseLong(strFails);
		 * 
		 * if(servers == fails) {
		 * System.out.println("No Servers available to process this request"); } }
		 * 
		 * } catch (KeeperException e) { // TODO Auto-generated catch block
		 * classLogger.error(Constants.STACKTRACE, e); } catch (InterruptedException e)
		 * { // TODO Auto-generated catch block classLogger.error(Constants.STACKTRACE,
		 * e); } catch (UnsupportedEncodingException e) { // TODO Auto-generated catch
		 * block classLogger.error(Constants.STACKTRACE, e); } } if(type ==
	...
	// String smssFileName =
	// "c:/users/pkapaleeswaran/workspacej3/SemossDev/model/Orca_Embedded__EMB_30991037-1e73-49f5-99d3-f28210e6b95c11.smss";
