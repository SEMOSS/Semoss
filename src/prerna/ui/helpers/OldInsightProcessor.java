...
public class OldInsightProcessor implements Runnable {
	...
	public Map<String, Object> runWeb() {
		...
		// if it is not present, we get the table data align by setting the data maker
		// in the playsheet and grabbing it
	...
	private IDataMaker createData() {
		...
		// logic to append the parameter information that was selected on view into the
		// data maker components
		// this either fills the query if the parameter was saved as a string using
		// @INPUT_NAME@ taxonomy or
		// it fills in the values in a Filtering PreTransformation which appends the
		// metamodel
	...
	// * @param runner
	// */
	// private void setRunnerInPKQLTrans(DataMakerComponent copyDmc, PKQLRunner
	// runner) {
	...
	private void preparePlaySheet(IPlaySheet playSheet, OldInsight insight) {
		...
		playSheet.setJDesktopPane((JDesktopPane) Utility.getDIHelperLocalProperty(Constants.DESKTOP_PANE));
