...
public final class TypeColorShapeTable {
  ...
  /**
   * Method getAllShapes. Gets all the shapes from the instance of the shapes array. Sets to default
   * shape list if there is no instance.
   *
  ...
  /**
   * Method getAllColors. Gets all the colors from the instance of the shapes array. Sets to default
   * color list if there is no instance.
   *
  ...
  public Shape getShape(String type, String vertName) {
    ...
    if (shapeHash.containsKey(vertName)) retShape = shapeHash.get(vertName);
    else if (shapeHash.containsKey(type)) retShape = shapeHash.get(type);
      ...
      retShape =
          (Shape)
              DIHelper.getInstance()
                  .getLocalProp(DIHelper.getInstance().getProperty(type + "_SHAPE"));
    if (retShape == null) {
  ...
  public Shape getShapeL(String type, String vertName) {
    ...
    Shape retShape = null;
    // first check the vertex name
    if (shapeHashL.containsKey(vertName)) retShape = shapeHashL.get(vertName);
    else if (shapeHashL.containsKey(type)) retShape = shapeHashL.get(type);
    else if (DIHelper.getInstance().getProperty(type + "_SHAPE") != null)
      // try to search the properties file for the first time
      retShape =
          (Shape)
              DIHelper.getInstance()
                  .getLocalProp(
                      DIHelper.getInstance().getProperty(type + "_SHAPE") + Constants.LEGEND);
    if (retShape == null) {
      // instead of returning default color, going to return random color that hasn't been used yet
      for (String shape : shapes) {
        if (!shapeStringHash.containsValue(shape)) {
  ...
  public Color getColor(String type, String vertName) {
    ...
    // this has to go first so that if the user wants same node to be different color in a separate
    // insights, it will catch it
    ...
    if (colorHash.containsKey(vertName)) {
      ...
      retColor =
          (Color)
              DIHelper.getInstance()
                  .getLocalProp(DIHelper.getInstance().getProperty(type + "_COLOR"));
    }
   ...
   *
   * @param rgbString String - rgb string - the appropriate rgb string the method will try to
   *     convert
   */
  ...
  public String getShapeAsString(String vertName) {
    if (shapeStringHash.containsKey(vertName)) return shapeStringHash.get(vertName);
    else return "TBD";
  }
  ...
  public String getColorAsString(String vertName) {
    if (colorStringHash.containsKey(vertName)) return colorStringHash.get(vertName);
    else return "TBD";
  }
}
