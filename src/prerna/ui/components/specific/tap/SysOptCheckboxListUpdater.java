...
public class SysOptCheckboxListUpdater {
	...
	public Vector<String> getSelectedSystemList(Boolean intDHMSM, Boolean notIntDHMSM, Boolean theater, Boolean garrison, Boolean low, Boolean high, Boolean mhsSpecific, Boolean ehrCore) {
		...
		if(notIntDHMSM)
			interfaced = ArrayListUtilityMethods.createOrUnion(interfaced,notIntDHMSMSysList);
	
		Vector<String> theatGarr = new Vector<String>();
		if(theater)
			theatGarr = theaterSysList;
		if(garrison)
			theatGarr = ArrayListUtilityMethods.createOrUnion(theatGarr,garrisonSysList);
		
		Vector<String> lowHigh = new Vector<String>();
		if(low)
			lowHigh = lowProbSysList;
		if(high)
			lowHigh = ArrayListUtilityMethods.createOrUnion(lowHigh,highProbSysList);
		
		Vector<String> systemsToSelect = new Vector<String>();
		systemsToSelect=ArrayListUtilityMethods.createAndUnionIfBothFilled(interfaced,systemsToSelect);
		systemsToSelect=ArrayListUtilityMethods.createAndUnionIfBothFilled(theatGarr,systemsToSelect);
		systemsToSelect=ArrayListUtilityMethods.createAndUnionIfBothFilled(lowHigh,systemsToSelect);

		if(mhsSpecific)
			systemsToSelect=ArrayListUtilityMethods.createOrUnion(mhsSpecificSysList,systemsToSelect);
		if(ehrCore)
			systemsToSelect=ArrayListUtilityMethods.createOrUnion(ehrCoreSysList,systemsToSelect);
