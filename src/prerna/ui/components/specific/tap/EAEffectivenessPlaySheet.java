...
public class EAEffectivenessPlaySheet extends GridPlaySheet {
	private IDatabaseEngine tapCoreDB = (IDatabaseEngine) DIHelper.getInstance().getLocalProp("TAP_Core_Data");
	...
	// "SELECT DISTINCT ?BusinessLogicUnit WHERE{FILTER(?Review_Status = 'FAC_Approved')FILTER(?RIBEE = 'Low')FILTER(?Device_InterfaceYN = 'N'){?System <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/System>;}{?Provide <http://www.w3.org/2000/01/rdf-schema#subPropertyOf> <http://semoss.org/ontologies/Relation/Provide>;}{?BusinessLogicUnit <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/BusinessLogicUnit>;}{?System ?Provide ?BusinessLogicUnit.}{?System <http://semoss.org/ontologies/Relation/Contains/Review_Status> ?Review_Status;} {?System <http://semoss.org/ontologies/Relation/Contains/Probability_of_Included_BoS_Enterprise_EHRS> ?RIBEE;} {?System <http://semoss.org/ontologies/Relation/Contains/Device> ?Device_InterfaceYN;} {?System <http://semoss.org/ontologies/Relation/Contains/Interface_Needed_w_DHMSM> ?Interface_Needed_w_DHMSM;} }";
	...
	public void createData() {
		dhmsmData = helper.getStringList(getDHMSMDataQuery, tapCoreDB);
		dhmsmBLU = helper.getStringList(getDHMSMBLUQuery, tapCoreDB);
		activityFGMap = helper.getStringListMap(getActivityFGQuery, tapCoreDB);
		fgDataMap = helper.getStringListArrayMap(getFGDataQuery, tapCoreDB);
		fgBLUMap = helper.getStringListArrayMap(getFGBLUQuery, tapCoreDB);
		activityDataWeight = helper.getStringMap(getActivityDataWeightQuery, tapCoreDB);
		activityBLUWeight = helper.getStringMap(getActivityBLUWeightQuery, tapCoreDB);
		activityDataMap = helper.getStringListArrayMap(getActivityDataQuery, tapCoreDB);
		activityBLUMap = helper.getStringListArrayMap(getActivityBLUQuery, tapCoreDB);
		fgProps = helper.getFGPropArray(getFGPropsQuery, tapCoreDB);
		fgActivityCount = helper.getStringMap(getFGActivityCountQuery, tapCoreDB);
		totalActivityCount = helper.getTotalCount(getTotalActivityCountQuery, tapCoreDB);
		bpActivityMap = helper.getStringListArrayMap(getBusinessProcessQuery, tapCoreDB);
		bpFCCs = helper.getStringListArrayMap(getFCCQuery, tapCoreDB);
		fccCosts = helper.fillFCCCostHash();
		yearFCCCost = helper.getCostPerYear();
		activityBP = helper.getStringListArrayMap(getActivityBPQuery, tapCoreDB);
	}
