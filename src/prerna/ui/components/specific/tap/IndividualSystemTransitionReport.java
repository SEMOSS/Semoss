...
public class IndividualSystemTransitionReport extends AbstractRDFPlaySheet{
	...
	private String softwareLifeCycleQuery = "SELECT DISTINCT ?System ?SoftwareVersion (IF( bound(?SoftMEOL) && datatype(?SoftMEOL) = xsd:dateTime, ?SoftMEOL, IF( bound(?SoftVEOL) && datatype(?SoftVEOL) = xsd:dateTime, ?SoftVEOL, 'TBD')) AS ?SupportDate) (IF( bound(?SoftMEOL) && datatype(?SoftMEOL) = xsd:dateTime, ?SoftMEOL, IF( bound(?SoftVEOL) && datatype(?SoftVEOL) = xsd:dateTime, ?SoftVEOL, 'TBD')) AS ?LifeCycle) (COALESCE(?unitcost,0) AS ?UnitCost) (COALESCE(?quantity,0) AS ?Quantity) (COALESCE(?UnitCost*?Quantity,0) AS ?TotalCost) (COALESCE(?budget,0) AS ?SystemSWBudget) WHERE { BIND(<http://health.mil/ontologies/Concept/System/MSAT> AS ?System) {?System <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/ActiveSystem>} {?SoftwareModule <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/SoftwareModule>} {?System <http://semoss.org/ontologies/Relation/Consists> ?SoftwareModule} {?SoftwareModule <http://semoss.org/ontologies/Relation/Contains/Quantity> ?quantity} {?SoftwareVersion <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/SoftwareVersion>} {?SoftwareModule <http://semoss.org/ontologies/Relation/TypeOf> ?SoftwareVersion} OPTIONAL {?SoftwareVersion <http://semoss.org/ontologies/Relation/Contains/EOL> ?SoftVEOL} OPTIONAL {?SoftwareModule <http://semoss.org/ontologies/Relation/Contains/EOL> ?SoftMEOL} OPTIONAL {?SoftwareVersion <http://semoss.org/ontologies/Relation/Contains/Price> ?unitcost} } ORDER BY ?System ?SoftwareVersion";
