...
//						String rowName = sjss.getVar(names[0])+"";
	...
	public void setQueries(String[] system)
	{
		...
		icdSerCreateQuery = "SELECT DISTINCT ?type (COUNT(DISTINCT(?data)) AS ?dataCount) (COUNT(DISTINCT(?ser)) AS ?serCount) WHERE { BIND(\"Provider Count\" AS ?type) {?data <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/DataObject> ;} {?ser <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/Service> ;} {?icd <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/SystemInterface> ;}  {?sys ?OwnedBy <http://health.mil/ontologies/Concept/SystemOwner/Central> } {?sys2 ?OwnedBy2 <http://health.mil/ontologies/Concept/SystemOwner/Central> }  {?ser <http://semoss.org/ontologies/Relation/Exposes> ?data;} {?icd <http://semoss.org/ontologies/Relation/Payload> ?data} {?sys <http://semoss.org/ontologies/Relation/Provide> ?icd} {?icd <http://semoss.org/ontologies/Relation/Consume> ?sys2} } GROUP BY ?type";
		if (system==null)
		{
