...
/**
 * This is the core class used to show which classes are data properties as opposed to object
 * properties.
 */
public class PropertySpecData {
  ...
  /**
   * Adds concepts to the concept hashtable. If the existing hashtable contains the parent string,
   * get the vector from the hashtable. If not, create a new child vector and put it in the concept
   * hashtable.
   *
  ...
  /**
   * This method will read through the predicate hash and generate the data lists. It shows the
   * boolean, concept hierarchy, predicate, concept, and concept boolean.
   */
  ...
  public void setValueAt(String uriVal, Object value, int row, int column) {
    ...
    if (dataList[row][0] != null) conceptParent = dataList[row][0] + "";
  ...
  public void addToString(
    ...
    if (add) {
      if (concept) {
        this.subject2bRemoved =
            this.subject2bRemoved.replace(";" + parentEntity + "@@" + entity, "");
