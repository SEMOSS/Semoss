...
public class PropertySpecData {
	...
	/**
	 * Adds concepts to the concept hashtable. If the existing hashtable contains
	 * the parent string, get the vector from the hashtable. If not, create a new
	 * child vector and put it in the concept hashtable.
	 *
	 * @param parent
	 *            Parent string.
	 * @param child
	 ...
	 *
	 * @param concept
	 *            Concept to be added, in string form.
	 */
	 ...
	 *
	 * @param predicate
	 *            Predicate to be added, in string form.
	 */
	...
	/**
	 * Add available property to the hashtable if it does not already contain the
	 * predicate as a key.
	 *
	 * @param predicate
	 *            Predicate to be added, in string form.
	 ...
	 *
	 * @param parent
	 *            Parent string, serves as the hashtable key.
	 * @param predicateName
	 *            Predicate name.
	 */
	public void addPredicate2(String parent, String predicateName) {
		...
		/*
		 * if(stringofEverything.indexOf(predicateName) < 0 ) { String childVector = "";
		 * if (predicateHash.containsKey(parent)) { childVector =
		 * predicateHash.get(parent); } if (!childVector.contains(predicateName)) {
		 * childVector = childVector + "@@" + predicateName; predicateHash.put(parent,
		 * childVector); predSize++; } stringofEverything = stringofEverything + " " +
	...
	public void addProperty(String parent, String predicateName) {
		...
		/*
		 * if(stringofEverything.indexOf(predicateName) < 0 ) { String childVector = "";
		 * if (propertyHash.containsKey(parent)) { childVector =
		 * propertyHash.get(parent); } if (!childVector.contains(predicateName)) {
		 * childVector = childVector + "@@" + predicateName; propertyHash.put(parent,
		 * childVector); propertySize++; } stringofEverything = stringofEverything + " "
		 * + parent + " " + predicateName; }
		 */
	 ...
	 * It shows the boolean, concept hierarchy, predicate, concept, and concept
	 * boolean.
	 */
	public void genPredList() {
		...
		dataList = new Object[conceptHash.size() + conceptSize + predicateHash.size() + predSize + propertyHash.size()
				+ propertySize][6];
		...
		for (int keyIdx = 0; keyIdx < keyArray.size(); keyIdx++) {
			...
			while (tokens.hasMoreTokens()) {
				...
				dataList[conceptIndex][2] = new Boolean(conceptAvailableHash.containsKey(child));
				dataList[conceptIndex][1] = child;
				dataList[conceptIndex][5] = key;
				dataList[conceptIndex][3] = new Boolean(predicateAvailableHash.containsKey(child)); // not a
				// predicate
		...
		/*
		 * keys = predicateHash.keys(); for (; keys.hasMoreElements(); conceptIndex++) {
		 * String key = keys.nextElement(); dataList[conceptIndex][2] = new
		 * Boolean(false); dataList[conceptIndex][3] = new Boolean(false); // not a
		 * predicate dataList[conceptIndex][4] = new Boolean(false); // not a property
		 * if (predicateAvailableHash.containsKey(key)) { logger.debug("Found the key "
		 * + key); dataList[conceptIndex][3] = new Boolean(true); // predicate // block
		 * } dataList[conceptIndex][0] = key; dataList[conceptIndex][1] = "Select All";
		 * dataList[conceptIndex][5] = key; String childString = predicateHash.get(key);
		 * StringTokenizer tokens = new StringTokenizer(childString, "@@"); while
		 * (tokens.hasMoreTokens()) { String child = tokens.nextToken(); conceptIndex++;
		 * dataList[conceptIndex][3] = new Boolean(
		 * predicateAvailableHash.containsKey(child)); dataList[conceptIndex][1] =
		 * child; dataList[conceptIndex][5] = key; dataList[conceptIndex][2] = new
		 * Boolean(false); // not a // predicate dataList[conceptIndex][4] = new
		 * Boolean(false); // not a // property } }
		 * 
		 * // and the properties // generate the predicate list next keys =
		 * propertyHash.keys(); for (; keys.hasMoreElements(); conceptIndex++) { String
		 * key = keys.nextElement(); dataList[conceptIndex][2] = new Boolean(false);
		 * dataList[conceptIndex][3] = new Boolean(false); // not a predicate
		 * dataList[conceptIndex][4] = new Boolean(false); // not a property if
		 * (propertyAvailableHash.containsKey(key)) { logger.debug("Found the key " +
		 * key); dataList[conceptIndex][4] = new Boolean(true); // predicate // block }
		 * dataList[conceptIndex][0] = key; dataList[conceptIndex][1] = "Select All";
		 * dataList[conceptIndex][5] = key; String childString = propertyHash.get(key);
		 * StringTokenizer tokens = new StringTokenizer(childString, "@@"); while
		 * (tokens.hasMoreTokens()) { String child = tokens.nextToken(); conceptIndex++;
		 * dataList[conceptIndex][4] = new Boolean(
		 * propertyAvailableHash.containsKey(child)); dataList[conceptIndex][1] = child;
		 * dataList[conceptIndex][5] = key; dataList[conceptIndex][2] = new
		 * Boolean(false); // not a // predicate dataList[conceptIndex][3] = new
		 * Boolean(false); // not a // property } }
		 */
	}
	...
	public void setValueAt(String uriVal, Object value, int row, int column) {
		...
		if (conceptParent != null && conceptParent.length() >= 0) {
			...
			// addToString(conceptParent, (Boolean)value, concept, predicate, property,
			// column);
		} else {
			selectRow(uriVal, value, row, column);
			addToString(dataList[row][5] + "", dataList[row][1] + "", (Boolean) value, concept, predicate, property,
					column);
		}
	}
	 ...
	 *
	 * @param parentEntity
	 *            String containing the parent entity.
	 * @param entity
	 *            Entity.
	 * @param add
	 *            True if an entity needs to be added.
	 * @param concept
	 *            True if an entity is a concept.
	 * @param predicate
	 *            True if an entity is a predicate.
	 * @param property
	 *            True if an entity is a property.
	 */
	public void addToString(String parentEntity, String entity, boolean add, boolean concept, boolean predicate,
			boolean property, int column) {
		if (add) {
			if (concept) {
