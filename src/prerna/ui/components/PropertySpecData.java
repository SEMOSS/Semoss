...
public class PropertySpecData {
	...
	static final Logger classLogger = LogManager.getLogger(PropertySpecData.class.getName());
...
//		keyArray.addAll(propertyHash.keySet());
		 ...
		 * classLogger.debug("Found the key " + key); dataList[conceptIndex][3] = new
		 * Boolean(true); // predicate // block } dataList[conceptIndex][0] = key;
		 * dataList[conceptIndex][1] = "Select All"; dataList[conceptIndex][5] = key;
		 * String childString = predicateHash.get(key); StringTokenizer tokens = new
		 * StringTokenizer(childString, "@@"); while (tokens.hasMoreTokens()) { String
		 * child = tokens.nextToken(); conceptIndex++; dataList[conceptIndex][3] = new
		 * Boolean( predicateAvailableHash.containsKey(child));
		 * dataList[conceptIndex][1] = child; dataList[conceptIndex][5] = key;
		 * dataList[conceptIndex][2] = new Boolean(false); // not a // predicate
		 * dataList[conceptIndex][4] = new Boolean(false); // not a // property } }
		 * 
		 ...
		 * (propertyAvailableHash.containsKey(key)) { classLogger.debug("Found the key "
