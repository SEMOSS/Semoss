...
public class AgglomerativeClusteringVizPlaySheet extends BrowserPlaySheet{

	private static final Logger logger = LogManager.getLogger(AgglomerativeClusteringVizPlaySheet.class.getName());
	...
	public AgglomerativeClusteringVizPlaySheet() {
		super();
	...
	public void createData() {
		processQuery();
		AgglomerativeClusteringAlgorithm clusterAlg = new AgglomerativeClusteringAlgorithm(list,names);
	...
	public void setQuery(String query) {
		if(query.startsWith("SELECT") || query.startsWith("CONSTRUCT"))
			...
			String newQuery = query.substring(semi+1, query.length());
			semi = newQuery.indexOf(";");
			n = Double.parseDouble(newQuery.substring(0,semi));
			this.query = newQuery.substring(semi+1);
		}
	}
