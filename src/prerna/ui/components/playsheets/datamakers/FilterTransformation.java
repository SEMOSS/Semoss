...
public class FilterTransformation extends AbstractTransformation {
  ...
  public static final String METHOD_NAME =
      "filter"; // name of the method in all data makers to perform a filter
  public static final String UNDO_METHOD_NAME =
      "unfilter"; // name of the method in all data makers to perform undo filtering
  public static final String COLUMN_HEADER_KEY =
      "colHeader"; // key in properties map for the type to apply the filter for
  public static final String VALUES_KEY =
      "values"; // key in properties map for the list of values to filter
  public static final String VISIBLE_VALUES_KEY =
      "valueSet"; // key in properties map for the list of values to display in drop down
    ...
    if (values == null) {
      ...
      if (dm instanceof ITableDataFrame) {
        ...
        if (dm instanceof H2Frame) {
          qs2.addSelector(
              ((ITableDataFrame) dm).getName(), props.get(COLUMN_HEADER_KEY).toString());
          try {
          ...
          // tinker
          qs2.addSelector(
              props.get(COLUMN_HEADER_KEY).toString(), QueryStruct.PRIM_KEY_PLACEHOLDER);
          try {
            uniqIterator = ((ITableDataFrame) dm).query(qs2);
          } catch (Exception e) {
            logger.error(STACKTRACE, e);
  ...
  /**
   * Appends the filtering into the metamodel data within the component Used when the filtering
   * transformation is a preTransformation
   *
   ...
   */
  private void addFilterToComponentData(
      String colHeader, List<Object> values, QueryStruct builderData) {
  ...
  public void undoTransformation() {
    String colHeader = this.props.get(COLUMN_HEADER_KEY) + "";
    //		Set<Object> values = ((Stack<Set<Object>>)
    // this.props.get(VISIBLE_VALUES_KEY)).firstElement();
