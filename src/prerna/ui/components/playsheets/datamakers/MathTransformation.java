...
public class MathTransformation extends AbstractTransformation {
	...
	public void runMethod() {
		...
		// 1. function to perform (math, min, etc.) but should eventually contain the
		// groovy script?
		// 2. the columnHeader(s) to operate on
		// 3. the name of the new column (after creating, should be sent from the front
		// end)
		...
		// if groupByCols.length is 1 or length is 2 and those two columns are equal do
		// single column
		// boolean singleColumn = groupByCols.size() == 1 || (groupByCols.size() == 2 &&
		// groupByCols.get(0).equals(groupByCols.get(1)));
		...
		// create a routine which will do the group by and add the values to the tinker
		// graph
	...
	/**
	 * This method creates a new column name by combining the function name with the
	 * column header its operated on.
	 *
	 * @param functionMap
	 *            a hashtable that describes what this function will do
	 * @param columnHeader
	 *            a string which describes the relevant column to look at
	 * @return
	 */
	public static Map<String, Object> createColumnNamesForColumnGrouping(List<String> columnHeaders,
			Map<String, Object> functionMap, String[] tableHeaders) {
		String columnHeader = "";
		...
		for (String key : functionMap.keySet()) {
			...
			// Check if name exists already within the table, if so append a counter for
			// uniqueness
	...
	public MathTransformation copy() {
		...
		// Map<String, Object> newProps = gson.fromJson(propCopy, new
