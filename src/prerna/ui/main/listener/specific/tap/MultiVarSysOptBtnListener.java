...
/**
 */
public class MultiVarSysOptBtnListener implements IChakraListener {
	
	private static final Logger LOGGER = LogManager.getLogger(MultiVarSysOptBtnListener.class.getName());
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		{	
			...
			else if(playSheet.rdbtnIRR.isSelected()) this.optimizer = new MultiVarSysIRROptimizer();
			...
			((MultivariateOptimizer)optimizer).setSelectDropDowns(((SysOptPlaySheet)playSheet).systemSelectPanel.getSelectedSystems(), ((SysOptPlaySheet)playSheet).systemSelectPanel.isTheaterCheckBoxSelected(), ((SysOptPlaySheet)playSheet).systemSelectPanel.isGarrisonCheckBoxSelected(), ((SysOptPlaySheet)playSheet).capabilitySelectPanel.getSelectedCapabilities(),((SysOptPlaySheet)playSheet).dataBLUSelectPanel.getSelectedData(),((SysOptPlaySheet)playSheet).dataBLUSelectPanel.getSelectedBLU(),((SysOptPlaySheet)playSheet).systemModernizePanel.getSelectedSystems(),((SysOptPlaySheet)playSheet).systemDecomissionPanel.getSelectedSystems(),((SysOptPlaySheet)playSheet).includeRegionalizationCheckbox.isSelected(),((SysOptPlaySheet)playSheet).garrTheaterCheckbox.isSelected());
