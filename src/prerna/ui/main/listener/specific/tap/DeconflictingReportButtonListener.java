...
//		out = "TAB-D";
//		outputArray.add(out);
//		queries.put(out,"SELECT DISTINCT ?System1 (COALESCE(?Data1, \"\") AS ?Data) (COALESCE(?ICD1, \"\") AS ?icd) WHERE { {?System1 <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/System>;} MINUS { {?Data1 <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/DataObject>;} BIND(<http://semoss.org/ontologies/Relation/Provide> AS ?provide) {?System1 ?provide ?Data1 .} } { SELECT DISTINCT (SAMPLE(?Data1) AS ?Data1) (SAMPLE(?System1) AS ?System1) (SAMPLE(?ICD) AS ?ICD1) WHERE { {?ICD <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/SystemInterface> ;} BIND(<http://semoss.org/ontologies/Relation/Payload> AS ?carries) {?ICD ?carries ?Data1;} { BIND(<http://semoss.org/ontologies/Relation/Provide> AS ?Upstream) {?System1 ?Upstream ?ICD ;}} UNION {BIND(<http://semoss.org/ontologies/Relation/Consume> AS ?Downstream){?ICD ?Downstream ?System1 ;} } BIND(URI(CONCAT(STR(?System1),STR(?Data1))) AS ?sysData) } GROUP BY ?sysData}}");
//		
//		out = "TAB-E";
//		outputArray.add(out);
//		queries.put(out,"SELECT DISTINCT (SAMPLE(?System1) AS ?System1) (SAMPLE(?Data1) AS ?Data1) ?crm (COALESCE(SAMPLE(?ICD), \"\") AS ?ICD1) WHERE { {?System1 <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/System>;} {?Data1 <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/DataObject>;} {?provide <http://www.w3.org/2000/01/rdf-schema#subPropertyOf> <http://semoss.org/ontologies/Relation/Provide>} {?System1 ?provide ?Data1 .} {?provide <http://semoss.org/ontologies/Relation/Contains/CRM> ?crm} OPTIONAL { {?ICD <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/SystemInterface> ;} BIND(<http://semoss.org/ontologies/Relation/Payload> AS ?carries) {?ICD ?carries ?Data1;} BIND(<http://semoss.org/ontologies/Relation/Provide> AS ?Upstream) {?System1 ?Upstream ?ICD ;} } OPTIONAL { {?ICD <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/SystemInterface> ;} BIND(<http://semoss.org/ontologies/Relation/Payload> AS ?carries) {?ICD ?carries ?Data1;} BIND(<http://semoss.org/ontologies/Relation/Consume> AS ?Downstream) {?System1 ?Downstream ?ICD ;} } BIND(URI(CONCAT(STR(?System1),STR(?Data1))) AS ?sysData) FILTER(!BOUND(?ICD))} GROUP BY ?sysData ?crm");
