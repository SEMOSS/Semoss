...
public class NativePyEngineWorker implements Runnable {
  ...
  /*
   * NOTE
   * This class has been repurposed and is only used for database execution
   * All other operations should be send via a reactor call instead of directly invoking the database
   */
    ...
    try {
      ...
      if (canAccess) {
        if (ps.engineType.equalsIgnoreCase("DATABASE")
            && ps.methodName.equalsIgnoreCase("execquery")) {
          IEngine engine = Utility.getDatabase(engineId);
          wrapper =
              WrapperManager.getInstance()
                  .getRawWrapper((IDatabaseEngine) engine, ps.payload[0] + "");
          ...
          if (this.insight == null || (ps.payload.length > 1 && ps.payload[1].equals("json"))) {
            JSONArray jsonPayload =
                Utility.writeResultToJsonObject(
                    wrapper,
                    null,
            ...
            fileLocation =
                insight.getInsightFolder() + "/a_" + Utility.getRandomString(5) + ".json";
      ...
      String errorMessage =
          (e.getCause() != null) ? e.getCause().getMessage() : e.getLocalizedMessage();
