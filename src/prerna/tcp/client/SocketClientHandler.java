    ...
    while (!done) {
      try {
        ...
        if (lenBytes != null) {
          ...
          readBytes =
              in.read(curBytes, bytesReadSoFar, (curBytes.length - bytesReadSoFar)); // block
              ...
              if (retObject != null) {
                if (ps.response) {
                  ...
                  if (ps.operation == PayloadStruct.OPERATION.ENGINE) {
                    ...
                    ew.start();
                    */
                    NativePyEngineWorker ew =
                        new NativePyEngineWorker(((SocketClient) socketClient).getUser(), ps);
                    ew.run();
                ...
                classLogger.info(
                    "Failed to deserialize " + curBytes.length + " <> bytes read " + readBytes);
                lenBytes = null;
                bytesReadSoFar = 0;
                lenBytesReadSoFar = 0;
                curBytes = null;
          ...
          int bytesRead =
              in.read(lenBytes, lenBytesReadSoFar, (lenBytes.length - lenBytesReadSoFar)); // block
