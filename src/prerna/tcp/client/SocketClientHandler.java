...
public class SocketClientHandler implements Runnable {
	...
	// I think we should move this also into stream reader or move stream reader
	// here
				...
				if (lenBytes != null) {
					...
					if (bytesReadSoFar == curBytes.length && readBytes != -1) {
						try {
							...
							if (retObject != null) {
								if (ps.response) {
									...
									if (ps.operation == PayloadStruct.OPERATION.ENGINE) {
										 ...
										 * ew.start();
										 */
										NativePyEngineWorker ew = new NativePyEngineWorker(
												((SocketClient) socketClient).getUser(), ps);
										ew.run();
							...
							classLogger
									.info("Failed to deserialize " + curBytes.length + " <> bytes read " + readBytes);
