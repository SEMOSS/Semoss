...
public class NativePySocketClient extends SocketClient implements Runnable, Closeable {
    ...
    if (connected) {
      ...
      while (!killAll) {
        ...
        try {
          ...
          classLogger.debug(
              "Socket read thread [{}:{}] attempting to read next message", threadName, threadId);
          ...
          classLogger.debug(
              "Socket read thread [{}:{}] completed read of size header: {} bytes",
              threadName,
              threadId,
              size);
          ...
          if (size > 0) {
            ...
            while (size_read < size) {
              ...
              classLogger.debug(
                  "Read chunk of {} bytes, total so far: {}/{}",
                  cur_size,
                  size_read + cur_size,
                  size);
            ...
            classLogger.debug(
                "Parsed message - epoc: {}, operation: {}, response: {}, interim: {}",
                ps.epoc,
                ps.operation,
                ps.response,
                ps.interim);
            ...
            if (ps.operation == PayloadStruct.OPERATION.STDOUT
                && ps.payload != null
                && !ps.response) {
            ...
            // need some way to say this is the output from the actual python vs. something that is
            // a classLogger
                        ...
                        try {
                          ...
                          if (executionInsightId != null) {
                            Insight executionInsight =
                                InsightStore.getInstance().get(executionInsightId);
                          ...
                          StreamingOutput streamedOutput =
                              PixelStreamUtility.collectPixelData(pixelRunner, null);
                          streamedOutput.write(output);
                          JsonElement json =
                              JsonParser.parseString(new String(output.toByteArray(), "UTF-8"));
  ...
  public Object executeCommand(PayloadStruct ps) {
    ...
    long threadId = Thread.currentThread().threadId();
    classLogger.debug(
        "Entering executeCommand for epoc: {} on thread: {} (ID: {})",
        ps.epoc,
        threadName,
        threadId);
    if (killAll) {
    ...
    try {
      synchronized (ps) {
        classLogger.debug(
            "Inside synchronized block for epoc: {} on thread: {} (ID: {})",
            ps.epoc,
            threadName,
            threadId);
        ...
        {
          int pollNum = 1;
          while (!responseMap.containsKey(ps.epoc)
              && (pollNum < maxWait || ps.longRunning)
              && !killAll
              && !cancelledEpocs.contains(ps.epoc)) {
            classLogger.debug(
                "Thread {} waiting for response to epoc: {} (poll #{})",
