...
public class SocketServerHandler implements Runnable {
  ...
  private boolean blocking =
      false; // processes one payload and moves to the next one. This is how it currently behaves
  ...
  private Map<String, AbstractRJavaTranslator> rtMap =
      new HashMap<String, AbstractRJavaTranslator>();
  ...
  public PayloadStruct getFinalOutput(PayloadStruct ps) {
    try {
      // System.err.println("Received For Processing " + ps.methodName +  "  bytes : " + totalBytes
      // + " Epoc " + ps.epoc);
      // classLogger.info("Received For Processing " + ps.methodName +  "  bytes : " + totalBytes +
      // " Epoc " + ps.epoc);
      ...
      if (ps.operation == PayloadStruct.OPERATION.R) {
        ...
        try {
          ...
          if (output != null) classLogger.info("Output is not null - CHROME");
          if (output instanceof ChromeDriver) output = new Object();
          if (output instanceof String) classLogger.info("Output is >>>>>>>>>>>>>>>  " + output);
          Object[] retObject = new Object[1];
          retObject[0] = output;
          ps.payload = retObject;
          ps.processed = true;
        ...
        try {
          ...
          if (contextProjectId != null) {
            reactor =
                getProjectReactor(contextProjectId, insight.getContextProjectName(), reactorName);
          }
          if (reactor == null && insight.getProjectId() != null) {
            reactor =
                getProjectReactor(insight.getProjectId(), insight.getProjectName(), reactorName);
          }
          if (reactor == null) {
        ...
        try {
          ...
          if (project == null) project = makeProject(ps.projectId, ps.projectName);
  ...
  public PayloadStruct writeResponse(PayloadStruct ps) {
    ...
    try {
      ...
      if (psBytes == null) {
        ...
        ps.payload =
            new String[] {"Output is not serializable. Forcing stringify <" + ps.payload[0] + ">"};
        psBytes = FstUtil.packBytes(ps);
      }
    }

    // send it
    // System.out.println("  Sending bytes " + psBytes.length + " >> " + ps.methodName + "  " +
    // ps.epoc + " >> ");
    classLogger.info(
        "  Sending bytes " + psBytes.length + " >> " + ps.methodName + "  " + ps.epoc + " >> ");
    try {
      os.write(psBytes);
  ...
  public void releaseAll() {
    ...
    while (keys.hasNext()) {
      ...
      if (ps != null) {
        ...
        if (ps.payload != null && ps.payload.length >= 1) message = message + ps.payload[0];
  ...
  public Object runMethodR(AbstractRJavaTranslator rt2, Method method, Object[] arguments)
      throws Exception {
    try {
      Object retObject = null;
  ...
  private AbstractRJavaTranslator getTranslator(String env) {
    if (!rtMap.containsKey(env)) {
      boolean JRI =
          DIHelper.getInstance().getProperty(Constants.R_CONNECTION_JRI) == null
              || DIHelper.getInstance()
                  .getProperty(Constants.R_CONNECTION_JRI)
  ...
  public void run() {
    ...
    // #1 SEMOSS Core sends a request and this responds - In this case the packet comes with
    // response = false to say this is a request
    ...
    while (!done) {
      try {
        ...
        if (lenBytes != null
            && lenBytesReadSoFar == lenBytes.length) // only get in here if you have read everything
        {
          bytesToRead = ByteBuffer.wrap(lenBytes).getInt();
          if (curBytes == null) curBytes = new byte[bytesToRead]; // block it

          int bytesRead =
              is.read(curBytes, bytesReadSoFar, (curBytes.length - bytesReadSoFar)); // block
          ...
          if (bytesReadSoFar == curBytes.length) // we got what we need.. let us go
          {
            ...
            // #2 - Response to an operation being performed by core semoss
              ...
              classLogger.info(
                  "Have response with epoc " + outgoing.containsKey(responseStruct.epoc));
              incoming.put(responseStruct.epoc, responseStruct);
              if (requestStruct != null) {
                synchronized (requestStruct) {
                  requestStruct
                      .notifyAll(); // this will give the thread back what it was looking for
                }
              }
              lenBytes = null;
              curBytes = null;
          ...
          if (lenBytes == null) lenBytes = new byte[bytesToRead]; // block it
          int bytesRead =
              is.read(lenBytes, lenBytesReadSoFar, (lenBytes.length - lenBytesReadSoFar)); // block
