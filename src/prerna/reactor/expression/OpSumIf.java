...
public class OpSumIf extends OpBasic {
	...
	public OpSumIf() {
		...
		this.keysToGet = new String[]{ReactorKeysEnum.VALUES.getKey(), ReactorKeysEnum.CRITERIA.getKey(),
				ReactorKeysEnum.SUM_RANGE.getKey()};
	}

	@Override
	protected NounMetadata evaluate(Object[] values) {
		/*
		 * The first input is the range of values to be evaluated by the criteria The
		 * second input is the criteria - this is a number, expression, or string, if
		 * this is a string, it can also be regex! The third input is an optional sum
		 * range, if this is empty, it will add the first input
		 */
			...
			FILTER_TEST : for (int i = 0; i < possibleFilters.length; i++) {
				...
				if (criteriaExpression.startsWith(filterToTest)) {
					...
					try {
						...
						filterNum = new BigDecimal(
								criteriaExpression.substring(filterToTest.length() - 1, filterToTest.length()));
	 ...
	 * @return
	 */
	private static BigDecimal evalNumericalExpression(Number rangeVal, String filterPrefix, Number filterNum,
			Number sumVal) {
