...
public class OpSumIfs extends OpBasic {
  ...
  protected NounMetadata evaluate(Object[] values) {
    ...
    if (isValidInput) {
      ...
      for (int i = 0; i < criteriaObjList.size(); i++) {
        ...
        if (isInteger) {
          indexMatchListFinal =
              getIndexListForIntegers(
                  criteriaRangeList,
                  criteria.replace("=", " ").trim(),
                  indexMatchList,
                  indexMatchListFinal,
                  isExpression,
                  expressionType);
      ...
      Map<Object, Long> finalIndicesMap =
          indexMatchListFinal.stream()
              .collect(Collectors.groupingBy(obj -> obj, Collectors.counting()));

      List<Entry<Object, Long>> finalIndicesList =
          finalIndicesMap.entrySet().stream()
      ...
      for (int i = 0; i < finalIndicesList.size(); i++) {
        if (finalIndicesList.get(i).getValue() == criteriaObjList.size()) {
          sumIfsVal +=
              ((Number) sumRangeList.get(((Number) finalIndicesList.get(i).getKey()).intValue()))
                  .doubleValue();
        }
      }
    }
  ...
  private List<Integer> getIndexListForIntegers(
      List<Object> criteriaRangeList,
      String criteria,
      List<Integer> indexMatchList,
      List<Integer> indexMatchListFinal,
      boolean isExpression,
      String expressionType) {

    List<Integer> intArrlist2 =
        criteriaRangeList.stream()
            .map(object -> (Integer.parseInt(Objects.toString(object, null))))
            .collect(Collectors.toList());

    for (int j = 0; j < intArrlist2.size(); j++) {
      if (!isExpression
          && !indexMatchListFinal.isEmpty()
          && (intArrlist2.get(j).equals(Integer.valueOf(criteria)))) {
        indexMatchList.add(j);
      } else if (isExpression) {
        indexMatchList =
            getIndexListForExpressions(
                intArrlist2.get(j),
  ...
  public List<Integer> getIndexListForString(
      List<Object> criteriaRangeList,
      String criteria,
      List<Integer> indexMatchList,
      List<Integer> indexMatchListFinal) {
  ...
  public List<Integer> getIndexListForExpressions(
      int intArrlist2,
      int criteria,
      List<Integer> indexMatchList,
      boolean isExpression,
