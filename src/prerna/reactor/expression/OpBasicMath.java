...
public abstract class OpBasicMath extends OpReactor {
  ...
  protected double evaluateString(String operation, NounMetadata frameColNoun) {
    ...
    if (dataMaker instanceof ITableDataFrame) {
      ...
      if (headers == null) {
        throw new IllegalArgumentException(
            "Cannot find variable or column with value " + frameColName);
      }
  ...
  private double evaluateString(ITableDataFrame frame, String operation, String frameColName) {
    ...
    try {
      ...
      if (it.hasNext()) {
        ...
        throw new IllegalArgumentException(
            "Data query did not return any results for the operation");
      }
    } catch (Exception e) {
  ...
  /**
   * Convert the object array to a double array This should only be used when running a required
   * sort
   *
   * @param values
   * @return
  ...
  /**
   * Convert the object array to a double array This is done based on the start/end index passed in
   *
   * @param values
   * @param startIndex
   * @param lastIndex
  ...
  public String getJavaSignature() {
    StringBuilder javaSignature =
        new StringBuilder(this.getClass().getName() + ".eval(new double[]{");
