...
public abstract class OpBasicMath extends OpReactor {
	...
	/*
	 * This class is to be extended for basic math operations To deal with string
	 * inputs that need to be evaluated on the frame
	 *
	 * TODO: stop casting everything to h2frame make generic expression class that
		...
		for (int i = 0; i < nouns.length; i++) {
			...
			if (valType == PixelDataType.CONST_DECIMAL) {
				...
				if (valueStr.toUpperCase().trim().equals("NONE")) {
					...
					throw new IllegalArgumentException("Invalid input for " + this.operation
							+ ". Require all values to be numeric or column names");
				}
			} else {
				throw new IllegalArgumentException(
						"Invalid input for " + this.operation + ". Require all values to be numeric or column names");
			}
		}
	...
	protected double evaluateString(String operation, NounMetadata frameColNoun) {
		...
		if (dataMaker instanceof ITableDataFrame) {
			...
			throw new IllegalArgumentException(
					"Cannot execute " + operation + " of " + frameColName + " with the given frame");
		}
	}
	 ...
	 *
	 * @param operation
	 *            The operation to execute - i.e. sum/min/max
	 * @param val
	 *            The name of the column to execute on
	 * @return
	 */
	...
	/**
	 * Convert the object array to a double array This should only be used when
	 * running a required sort
	 *
	 * @param values
	 * @return
	 */
	...
	/**
	 * Convert the object array to a double array This is done based on the
	 * start/end index passed in
	 *
	 * @param values
	 * @param startIndex
	 * @param lastIndex
