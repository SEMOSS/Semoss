...
public class NativeImporter extends AbstractImporter {
	...
	public void insertData() {
		...
		if (this.qs.getQsType() == QUERY_STRUCT_TYPE.RAW_ENGINE_QUERY && database instanceof IRDBMSEngine) {
			...
			// try {
			// StringBuilder newQueryB = new StringBuilder(" select * from (" + query + ")
			// as
			// customQuery ");
			// newQueryB = ((IRDBMSEngine) database).getQueryUtil().getFirstRow(newQueryB);
			// this.it = WrapperManager.getInstance().getRawWrapper(database,
			// newQueryB.toString());
			// } catch (Exception e) {
			// logger.error(Constants.STACKTRACE, e);
			// // one more time
      ...
      ////			String query = ((HardSelectQueryStruct) this.qs).getQuery();
			...
			// cleanUpSelectors(this.qs.getEngineId(), this.qs.getSelectors(),
			// this.qs.getRelations());
			// } catch (Exception e) {
