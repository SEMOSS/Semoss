...
public class NativeImporter extends AbstractImporter {
  ...
  public void insertData() {
    ...
    if (this.qs.getQsType() == QUERY_STRUCT_TYPE.RAW_ENGINE_QUERY
      ...
      try {
        ...
        for (int i = 0; i < numCols; i++) {
          ...
          executedDataTypes[i] =
              SemossDataType.convertStringToDataType(rsMeta.getColumnTypeName(i + 1));
        }
      ...
      //				try {
      //					StringBuilder newQueryB = new StringBuilder(" select * from (" + query + ") as
      // customQuery ");
    ...
    ImportUtility.parseNativeQueryStructIntoMeta(
        this.dataframe, this.qs, ignore, executedDataTypes);
  ...
  /**
   * Need to add additional joins that are part of the merge Since we do not really have TABLE,
   * join, TABLE anymore in RDBMS Yet I need to know what the join is
   *
   * @param joins
   */
  public void appendNecessaryRels(List<Join> joins) {
    ...
    for (int i = 0; i < numJoins; i++) {
      Join j = joins.get(i);
      BasicRelationship jRel =
          new BasicRelationship(new String[] {j.getRColumn(), j.getJoinType(), j.getLColumn()});
  ...
  private ITableDataFrame generateNewFrame(List<Join> joins) throws Exception {
    ...
    RDataTable rFrame =
        new RDataTable(
            this.in.getRJavaTranslator(LogManager.getLogger(CLASS_NAME)),
  ...
  private void cleanUpSelectors(
      String engineId, List<IQuerySelector> selectors, Set<String[]> rels) {
    ...
    if (queryAll) {
      ...
      Collection<String> possibleSelectors =
