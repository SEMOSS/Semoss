...
// if(unionFrame == null)
// {
// throw new SemossPixelException("Union is currently not available for the
...
// }
// } else {
// throw new IllegalArgumentException("Could not find any data input to merge
// into the frame");
// }
// }
// */
...
//
//
// private ITableDataFrame unionNative(ITableDataFrame curFrame, ITableDataFrame
// frame,
...
// curQS = QSAliasToPhysicalConverter.getPhysicalQs(curQS,
// curFrame.getMetaData());
//
//
// qs = ((NativeFrame)qs.getFrame()).getQueryStruct();
// qs = QSAliasToPhysicalConverter.getPhysicalQs(qs,
// qs.getFrame().getMetaData());
//
...
// }
// else
// throw new SemossPixelException("Joining tables across databases is not
// possible, please
// consider converting to a materialized frame");
// }
// else
// throw new SemossPixelException("Joining to a native frame from a materialized
// frame not
...
// * @throws Exception
// */
// private ITableDataFrame mergeFromQs(ITableDataFrame frame, SelectQueryStruct
// qs, List<Join>
...
// // get the correct q
// if(selector == null) {
// throw new IllegalArgumentException("There is an error with the join. Please
// make sure the
...
// if(frame.isEmpty()) {
// noDataError = true;
// throw new IllegalArgumentException("Attempting to join new data with an empty
// frame. End
...
// SemossDataType sDataType =
// frame.getMetaData().getHeaderTypeAsEnum(leftColumnJoin);
...
// }
//
// qs.addImplicitFilter(SimpleQueryFilter.makeColToValFilter(rColumnJoin, "==",
// values,
// dataType));
// } catch(Exception e) {
// classLogger.error(Constants.STACKTRACE, e);
// throw new IllegalArgumentException("Trying to merge on a column that does not
// exist within
// the frame!");
// }
// }
...
// new NounMetadata("Frame size is too large, please limit the data size before
// proceeding",
...
// * @throws Exception
// */
// private ITableDataFrame mergeFromTask(ITableDataFrame frame, ITask task,
// List<Join> joins) throws
// Exception {
...
//
// /**
// * Convert the frame join name from the alias to the physical table__column
// name
// * @param joins
// * @param meta
// * @return
...
// Join newJ = new Join(newLCol, j.getJoinType(), j.getRColumn(),
// j.getComparator(),
// j.getJoinRelName());
...
// }
//
// List<NounMetadata> frameCur =
// this.curRow.getNounsOfType(PixelDataType.FRAME);
...
// this.store.makeNoun(ReactorKeysEnum.FRAME.getKey()).add(new
// NounMetadata(defaultFrame,
// PixelDataType.FRAME));
// return defaultFrame;
// }
//
