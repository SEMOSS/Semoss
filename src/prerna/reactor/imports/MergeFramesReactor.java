...
public class MergeFramesReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (sourceFrame.getFrameType() == targetFrame.getFrameType()) {
      ...
      if (sourceFrame instanceof RDataTable) {
        ...
        // a) need to rename the columns if they exist in both source and target and not part of the
        // join
        ...
        Map<String, SemossDataType> rightTableTypes =
            sourceFrame.getMetaData().getHeaderToTypeMap();
        ...
        for (String leftTableHeader : leftTableHeaders) {
          ...
          String dupRightTableHeader =
              AbstractImporter.setIgnoreCaseMatch(
                  leftTableHeader, rightTableHeaders, rightTableJoinCols);
        ...
        if (sourceNFrame.getEngineId().equals(targetNFrame.getEngineId())) {
          ...
          if (sourceQs.getCustomFrom() == null && targetQs.getCustomFrom() == null) {
            ...
            NativeImporter importer =
                (NativeImporter) ImportFactory.getImporter(targetNFrame, targetQs, null);
    ...
    if (!optimized) {
      ...
      if (targetFrame instanceof NativeFrame) {
        try {
          ...
          // the qs's frame on the target frame is the sourceFrame. Later on during the import
          // process for
          // taking this native frame to an R frame, we read the qs then get qs.getFrame to generate
          // metadata
   ...
   * @return
   */
  private List<Join> convertJoins(
      List<Join> joins, OwlTemporalEngineMeta sourceMeta, OwlTemporalEngineMeta targetMeta) {
    ...
    for (Join j : joins) {
      ...
      // and add it to the list
        ...
        Join newJ =
            new Join(
                newLCol, j.getJoinType(), j.getRColumn(), j.getComparator(), j.getJoinRelName());
        convertedJoins.add(newJ);
      } else if (newLCol == null && newRCol != null) {
        Join newJ =
            new Join(
                j.getLColumn(), j.getJoinType(), newRCol, j.getComparator(), j.getJoinRelName());
        convertedJoins.add(newJ);
      } else {
        Join newJ =
            new Join(newLCol, j.getJoinType(), newRCol, j.getComparator(), j.getJoinRelName());
        convertedJoins.add(newJ);
      }
    }
