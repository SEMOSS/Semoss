...
public abstract class AbstractUnion implements UnionRoutine {
	...
	/*
	 * TODO: Below methods needs to be generalised for all frame types.
	 */
	 ...
	 * @param frameB
	 */
	protected void checkRBaseCases(ITableDataFrame frameA, ITableDataFrame frameB, List<String> aCols,
			List<String> bCols) {
		...
		// if(!isLenSame)
		// throw new IllegalArgumentException("Number of columns not same in both the
		// datasets. Please
	 ...
	 * @param frameB
	 */
	protected void checkPyBaseCases(ITableDataFrame frameA, ITableDataFrame frameB, List<String> aCols,
			List<String> bCols) {
	...
	public void checkColNames(List<String> aCols, List<String> bCols) {
		...
		for (String col : aCols) {
			if (!colsB.contains(col)) {
				throw new IllegalArgumentException("Mismatch detected for column " + col + " Please make sure col "
						+ col + " is present in both the datasets.");
			}
		}
	}
