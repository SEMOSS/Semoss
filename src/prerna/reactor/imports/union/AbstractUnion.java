...
public abstract class AbstractUnion implements UnionRoutine {
   ...
   * @param frameB
   */
  protected void checkRBaseCases(
      ITableDataFrame frameA, ITableDataFrame frameB, List<String> aCols, List<String> bCols) {
    ...
    if (!frameAType)
      throw new IllegalArgumentException(
          "Please convert " + frameA.getName() + " to a R frame before union.");
    if (!frameBType)
      throw new IllegalArgumentException(
          "Please convert " + frameB.getName() + " to a R frame before union.");
    //		if(!isLenSame)
    //			throw new IllegalArgumentException("Number of columns not same in both the datasets. Please
    // ensure that both the datasets have the same number of columns.");
   ...
   * @param frameB
   */
  protected void checkPyBaseCases(
      ITableDataFrame frameA, ITableDataFrame frameB, List<String> aCols, List<String> bCols) {
    ...
    if (!frameAType)
      throw new IllegalArgumentException(
          "Please convert " + frameA.getName() + " to a Py frame before union.");
    if (!frameBType)
      throw new IllegalArgumentException(
          "Please convert " + frameB.getName() + " to a Py frame before union.");
    //		if(!isLenSame)
    //			throw new IllegalArgumentException("Number of columns not same in both the datasets. Please
    // ensure that both the datasets have the same number of columns.");
    checkColNames(aCols, bCols);
  ...
  public void checkColNames(List<String> aCols, List<String> bCols) {
    ...
    for (String col : aCols) {
      if (!colsB.contains(col)) {
        throw new IllegalArgumentException(
            "Mismatch detected for column "
                + col
                + " Please make sure col "
                + col
                + " is present in both the datasets.");
      }
    }
  }
