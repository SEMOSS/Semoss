...
public class RUnion extends AbstractUnion {
  ...
  /**
   * This method takes both the frames and runs the union script in the R interpretor.
   *
   * @param unionType
   * @param frameA
   * @param frameB
  ...
  private ITableDataFrame[] matchColMetadata(
    ...
    Set<String> colBSet =
        new HashSet<>(colMappings.values().stream().map(String::trim).collect(Collectors.toList()));
