...
public class RUnion extends AbstractUnion {
	...
	/**
	 * This method takes both the frames and runs the union script in the R
	 * interpretor.
	 *
	 * @param unionType
	 * @param frameA
	...
	private void unionR(String unionType, String frameA, String frameB, Insight insight) {
		...
		script.append(frameA).append("<-").append(uType).append("(").append(frameA).append(",").append(frameB)
				.append(")");
	...
	/********************************************
	 *
	 * Helper methods to flush R data into new variables.
	 *
	 * ******************************************
	 */
	...
	private ITableDataFrame[] matchColMetadata(Insight insight, ITableDataFrame a, ITableDataFrame b,
		...
		Set<String> colBSet = new HashSet<>(
				colMappings.values().stream().map(String::trim).collect(Collectors.toList()));
	...
	private void realignCols(ITableDataFrame a, ITableDataFrame b, List<String> aCols, List<String> bCols) {
		...
		String script = new StringBuilder().append(dfName).append("<-").append(dfName).append("[, c(").append(aCols)
				.append(")]").toString();
		logger.info(script);
		rJavaTranslator.runR(script);
		dfName = b.getName();
		script = new StringBuilder().append(dfName).append("<-").append(dfName).append("[, c(").append(bCols)
				.append(")]").toString();
		logger.info(script);
		rJavaTranslator.runR(script);
	}
