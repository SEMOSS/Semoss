...
public class AddDefaultInsightsReactor extends AbstractReactor {
  ...
  public AddDefaultInsightsReactor() {
    this.keysToGet =
        new String[] {
          ReactorKeysEnum.PROJECT.getKey(), ReactorKeysEnum.DATABASE.getKey(), INSIGHT_KEYS
        };
  }
  ...
  public NounMetadata execute() {
    ...
    if (!SecurityProjectUtils.userCanEditProject(this.insight.getUser(), projectId)) {
      throw new IllegalArgumentException(
          "User does not have permission to add insights in the project");
    }
    ...
    if (addAll) {
      ...
      Map<String, Object> retMap =
          UploadUtilities.addExploreInstanceInsight(
              projectId, projectName, databaseId, databaseName, insightEngine);
      ...
      registerInsightAndMetadata(
          projectId,
          newInsightId,
          insightName,
          UploadUtilities.EXPLORE_INSIGHT_LAYOUT,
          cacheable,
          cacheMinutes,
          cacheCron,
          cachedOn,
      ...
      registerInsightAndMetadata(
          ...
          newInsightId,
          insightName,
          UploadUtilities.INSIGHT_USAGE_STATS_LAYOUT,
          cacheable,
          cacheMinutes,
          cacheCron,
          cachedOn,
      ...
      if (dbType == IDatabaseEngine.DATABASE_TYPE.RDBMS) {
        retMap =
            UploadUtilities.addGridDeltaInsight(
                projectId, projectName, databaseId, databaseName, insightEngine);
        newInsightId = (String) retMap.get(UploadUtilities.INSIGHT_ID_KEY);
        recipe = (List<String>) retMap.get(UploadUtilities.RECIPE_ID_KEY);
        insightName = (String) retMap.get(UploadUtilities.INSIGHT_NAME_KEY);
        ...
        registerInsightAndMetadata(
            ...
            newInsightId,
            insightName,
            UploadUtilities.GRID_DELTA_LAYOUT,
            cacheable,
            cacheMinutes,
            cacheCron,
            cachedOn,
        ...
        retMap =
            UploadUtilities.addAuditModificationView(
                projectId, projectName, databaseId, databaseName, insightEngine);
        if (retMap != null) {
          newInsightId = (String) retMap.get(UploadUtilities.INSIGHT_ID_KEY);
          recipe = (List<String>) retMap.get(UploadUtilities.RECIPE_ID_KEY);
          ...
          if (newInsightId != null) {
            registerInsightAndMetadata(
                ...
                newInsightId,
                insightName,
                UploadUtilities.AUDIT_MODIFICATION_VIEW_LAYOUT,
                cacheable,
                cacheMinutes,
                cacheCron,
                cachedOn,
            ...
            additionalNouns.add(
                NounMetadata.getWarningNounMessage("Unable to add audit modification view"));
          }
        } else {
          additionalNouns.add(
              NounMetadata.getWarningNounMessage("Unable to add audit timeline view"));
        }

        retMap =
            UploadUtilities.addAuditTimelineView(
                projectId, projectName, databaseId, databaseName, insightEngine);
        if (retMap != null) {
          newInsightId = (String) retMap.get(UploadUtilities.INSIGHT_ID_KEY);
          recipe = (List<String>) retMap.get(UploadUtilities.RECIPE_ID_KEY);
          ...
          if (newInsightId != null) {
            registerInsightAndMetadata(
                ...
                newInsightId,
                insightName,
                UploadUtilities.AUDIT_TIMELINE_LAYOUT,
                cacheable,
                cacheMinutes,
                cacheCron,
                cachedOn,
      ...
      if (insightsToAdd.contains(INSIGHT_STATS)) {
        Map<String, Object> retMap =
            UploadUtilities.addInsightUsageStats(projectId, projectName, insightEngine);
        String newInsightId = (String) retMap.get(UploadUtilities.INSIGHT_ID_KEY);
        List<String> recipe = (List<String>) retMap.get(UploadUtilities.RECIPE_ID_KEY);
        String insightName = (String) retMap.get(UploadUtilities.INSIGHT_NAME_KEY);
        String schemaName = (String) retMap.get(UploadUtilities.SCHEMA_NAME_KEY);
      ...
      if (insightsToAdd.contains(INSERT_FORM)) {
        if (dbType == IDatabaseEngine.DATABASE_TYPE.RDBMS) {
          Map<String, Object> retMap =
              UploadUtilities.addInsertFormInsight(
                  projectId, projectName, databaseId, databaseName, insightEngine);
          if (retMap != null) {
            String newInsightId = (String) retMap.get(UploadUtilities.INSIGHT_ID_KEY);
            List<String> recipe = (List<String>) retMap.get(UploadUtilities.RECIPE_ID_KEY);
            ...
            if (newInsightId != null) {
              ...
              registerInsightAndMetadata(
                  ...
                  newInsightId,
                  insightName,
                  UploadUtilities.INSERT_FORM_LAYOUT,
                  cacheable,
                  cacheMinutes,
                  cacheCron,
                  cachedOn,
              ...
              additionalNouns.add(
                  NounMetadata.getWarningNounMessage("Unable to add insert form insight"));
            }
          } else {
            additionalNouns.add(
                NounMetadata.getWarningNounMessage("Unable to add insert form insight"));
    ...
    if (addedInsight) {
      additionalNouns.add(
          NounMetadata.getSuccessNounMessage("Successfully added default insights"));
    }
    noun.addAllAdditionalReturn(additionalNouns);
    return noun;
  }

  private void registerInsightAndMetadata(
      String projectId,
      String insightIdToSave,
      String insightName,
      String layout,
      boolean cacheable,
      int cacheMinutes,
    ...
    if (this.insight.getUser() != null) {
      SecurityInsightUtils.addUserInsightCreator(
          this.insight.getUser(), projectId, insightIdToSave);
    }
  }
  ...
  private List<String> getDefaultInsights() {
    ...
    if (this.store.getNoun(this.keysToGet[0]) == null
        || this.store.getNoun(this.keysToGet[0]).isEmpty()) {
  ...
  protected String getDescriptionForKey(String key) {
    if (key.equals(INSIGHT_KEYS)) {
      return "Determine which default insights to append. "
          ...
          + EXPLORE_INSTANCE
          + "' = '"
          + UploadUtilities.EXPLORE_INSIGHT_INSIGHT_NAME
          + "', "
          + "Value '"
          + AUDIT_MODIFICATION
          + "' = '"
          + UploadUtilities.AUDIT_MODIFICATION_VIEW_INSIGHT_NAME
          + "', "
          + "Value '"
          + AUDIT_TIMELINE
          + "' = '"
          + UploadUtilities.AUDIT_TIMELINE_INSIGHT_NAME
          + "', "
          + "value '"
          + GRID_DELTA_INSTANCE
          + "' = '"
          + UploadUtilities.GRID_DELTA_INSIGHT_NAME
          + "'."
          + UploadUtilities.GRID_DELTA_INSIGHT_NAME
