 ...
 */
public class ProjectGitCreateBranchReactor extends AbstractReactor {

	private static final Logger classLogger = LogManager.getLogger(ProjectGitCreateBranchReactor.class);
	private static final String BRANCH_KEY = "branch";
	private static final String START_POINT_KEY = "startPoint";

	public ProjectGitCreateBranchReactor() {
		this.keysToGet = new String[] { ReactorKeysEnum.PROJECT.getKey(), BRANCH_KEY, START_POINT_KEY };
	...
	public NounMetadata execute() {
		...
		try (Git thisGit = Git.open(new File(versionFolder))) {
			...
			resultMap.put("status",
					ProjectGitReactorUtils.buildStatusMap(projectId, ProjectGitStatusUtils.computeStatus(repo)));
		} catch (SemossPixelException e) {
			throw e;
		} catch (Exception e) {
			classLogger.error("Error occurred creating branch for project {}", projectId, e);
	...
	public String getReactorDescription() {
		return "This reactor creates and checks out a new local branch in a project's git repository";
	}
