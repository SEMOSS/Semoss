...
public class RequestProjectReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (user == null) {
      NounMetadata noun =
          new NounMetadata(
              "User must be signed into an account in order to request a project",
    ...
    Integer currentUserPermission =
        SecurityProjectUtils.getUserProjectPermission(userId, projectId);
    ...
    Integer currentPendingUserPermission =
        SecurityProjectUtils.getUserAccessRequestProjectPermission(userId, projectId);
    ...
    boolean canRequest =
        SecurityProjectUtils.canRequestProject(projectId)
            || SecurityProjectUtils.userHasExplicitAccess(user, projectId);
    if (canRequest) {
      String userType = token.getProvider().toString();
      SecurityProjectUtils.setUserAccessRequest(
          userId, userType, projectId, requestComment, requestPermission, user);
    ...
    if (template != null && !template.isEmpty()) {
      ...
      if (projectOwners != null && !projectOwners.isEmpty()) {
        ...
        EmailUtility.sendEmail(
            emailSession,
            projectOwners.toArray(new String[0]),
            null,
            null,
