...
public class ReplaceInaccessibleEnginesReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		try (Stream<Path> stream = Files.walk(Paths.get(projectF.getAbsolutePath()))) {
			stream.filter(Files::isRegularFile).filter(path -> Arrays.stream(DEPENDENCIES_FILE_EXTENSIONS)
					.anyMatch(ext -> path.toString().toLowerCase().endsWith(ext))).forEach(path -> {
						try {
							...
							for (String line : lines) {
								...
								for (Map.Entry<String, String> entry : replacementMap.entrySet()) {
									...
									if (updatedLine.contains(inaccessibleEngineId)) {
										...
										Set<String> files = (Set<String>) uuidSuccessFiles.get(inaccessibleEngineId)
												.get("files");
							...
							for (Map.Entry<String, Boolean> entry : replacedInThisFile.entrySet()) {
								...
								if (!wasReplaced) {
									...
									if (uuidWasPresent) {
										...
										classLogger.warn("Engine ID {} was not replaced in file: {}", uuid,
												path.toString());
									}
								}
							}
