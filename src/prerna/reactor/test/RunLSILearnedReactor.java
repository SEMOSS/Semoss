...
public class RunLSILearnedReactor extends AbstractRFrameReactor {
	...
	public NounMetadata execute() {
		...
		String alterFrameSpace = returnTable + "$" + frameJoinCol + "<-gsub(\" \",\"_\"," + returnTable + "[,"
				+ frameJoinCol + "]);";
		...
		rsb.append(returnTable + "$LSA_Score<-NULL;" + returnTable + "$LSA_Category<-NULL;" + returnTable
				+ "$Match<-NULL;");
		...
		// execute r command
		String mergeString = RSyntaxHelper.getMergeSyntax(returnTable, leftTableName, rightTableName, joinType,
				joinCols);
