...
public class BuildAndPublishAppReactor extends AbstractReactor {
    ...
    public NounMetadata execute() {
        ...
        String buildSvcUrl = DIHelper.getInstance().getProperty(NODE_SERVER_ENDPOINT);
        String endpointBase = normalizeBuildServiceUrl(buildSvcUrl);

        Path projectAssetsDir = Paths.get(
        ...
        try {
            ...
            String endpoint = endpointBase
                    + "/build"
                    + "?buildCmd=" + URLEncoder.encode(BUILD_CMD, "UTF-8")
            ...
            return new NounMetadata(
                    "App [" + projectId + "] built and published successfully.",
    ...
    /**
     * Unzip the portals archive; strips a leading "portals/" prefix if the
     * service wraps entries under that folder name.
     */
