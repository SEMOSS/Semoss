...
/**
 * Callback bound to {@code TriggerWorkflowReactor.executeNodes}, used by
 * {@code SubWorkflowNodeExecutor} to run a target project's entire workflow graph to completion
 ...
 * row/active-run claim) and intentionally stays owned by {@code TriggerWorkflowReactor} rather
 * than being reimplemented by the node-executor layer - see the "what does NOT change" scoping
 * note on ticket #2746.
 */
@FunctionalInterface
public interface ChildWorkflowRunner {

	/**
	 * Runs an already-claimed, already-topo-sorted child workflow run to completion and returns
	 * its final run result (the same shape {@code buildRunResult} produces for a top-level run).
	 *
	 * @param childRunId          the child run's id (already inserted into {@code WORKFLOW_RUNS}
	 *                             and already holding the active-run claim for {@code targetProjectId})
	 * @param targetProjectId     the project id whose workflow is being run
	 * @param orderedNodes        the child workflow's nodes, already topologically sorted
	 * @param configMap           the child project's {@code workflow-config.json} key/value pairs
	 * @param priorOutputs        prior node outputs to skip on resume - empty for a fresh
	 *                             sub-workflow call, never itself resumable independently
