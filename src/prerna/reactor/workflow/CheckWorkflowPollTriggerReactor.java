 ...
 * <pre>
 *   CheckWorkflowPollTrigger(project=["projectId"], type=["storage-poll"]);
 * </pre>
 * or
 * <pre>
 *   CheckWorkflowPollTrigger(project=["projectId"], type=["db-poll"]);
 * </pre>
 *
 * <p>The poll configuration (engineId, path/query) is read from the workflow's
 * trigger node config inside {@code workflow.json}. The "last seen" state hash
 * is persisted in {@code workflow-poll-state.json} in the portals folder.
 *
 * <p>State file format:
 * <pre>
 ...
 * </pre>
 */
public class CheckWorkflowPollTriggerReactor extends AbstractReactor {

    private static final Logger classLogger = LogManager.getLogger(CheckWorkflowPollTriggerReactor.class);
    private static final Gson GSON = new GsonBuilder().disableHtmlEscaping().create();

    private static final String POLL_STATE_FILE = "workflow-poll-state.json";
    private static final String WORKFLOW_FILE = WorkflowConstants.WORKFLOW_FILE_NAME;

    public CheckWorkflowPollTriggerReactor() {
    ...
    public NounMetadata execute() {
        ...
        // Load trigger config from workflow.json
        ...
        if ("storage-poll".equals(pollType)) {
            ...
            triggerType = WorkflowConstants.TRIGGER_STORAGE_POLL;
        } else if ("db-poll".equals(pollType)) {
            currentResult = executeDbCheck(triggerConfig);
            triggerType = WorkflowConstants.TRIGGER_DB_POLL;
        } else {
        ...
        // Fire TriggerWorkflow with the appropriate trigger type
        String pixel = "TriggerWorkflow(project=[\"" + projectId + "\"], triggerType=[\"" + triggerType + "\"]);";
        try {
            this.insight.runPixel(pixel);
        } catch (Exception e) {
            classLogger.error("Failed to fire workflow for project {} after change detected: {}", projectId, e.getMessage(), e);
            throw new IllegalStateException("Change detected but workflow trigger failed: " + e.getMessage(), e);
        }
    ...
    private Map<String, Object> loadTriggerConfig(String projectId) {
        try {
            ...
            File wfFile = new File(portalsFolder + "/" + WORKFLOW_FILE);
            if (!wfFile.exists()) return null;

            String json = Files.readString(wfFile.toPath(), StandardCharsets.UTF_8);
