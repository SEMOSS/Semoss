 ...
 * {@code SMSS_MCP_EXECUTION} is {@code "ask"} or {@code "yesno"}. The harness
 * creates rows when it transitions a run to {@code INPUT_REQUIRED};
 * {@code RunMCPToolReactor} updates them when the user decides;
 * {@code GetAgentRunReactor} reads them to surface pending actions to the UI.
 */
