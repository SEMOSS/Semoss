 ...
 *
 * <p>
 * Config keys (all read from {@code RDF_Map.prop} via
 * {@link Utility#getDIHelperProperty(String)}):
 * <table>
 * <tr>
 * <th>Key</th>
 * <th>Meaning</th>
 ...
 * </tr>
 * <tr>
 * <td>{@code AGENT_SANDBOX_ENABLE}</td>
 * <td>{@code true} to enable sandboxing; {@code false} (default) to skip it
 * entirely</td>
 * <td>{@code false}</td>
 * </tr>
 * <tr>
 * <td>{@code AGENT_SANDBOX_DEFAULT_READS}</td>
 * <td>Comma-separated baseline RO paths added to every policy</td>
 * <td>OS-dependent (TLS cert dir, JRE, /usr/lib)</td>
 * </tr>
 * <tr>
 * <td>{@code AGENT_SANDBOX_LOOPBACK_NETWORK}</td>
 * <td>{@code true} to allow localhost MCP callbacks</td>
 * <td>{@code true}</td>
 * </tr>
 * <tr>
 * <td>{@code AGENT_SANDBOX_BLOCK_READS}</td>
 * <td>Comma-separated additional paths to deny reads for (appended to the
 * built-in blocklist)</td>
 * <td>(empty)</td>
 * </tr>
 * </table>
 */
public final class AgentSandboxConfig {
	...
	private static final Logger classLogger = LogManager.getLogger(AgentSandboxConfig.class);
	...
	/**
	 * OS-aware baseline read-only paths the agent binary always needs: loader / TLS
	 * certs / system libraries. Overridable via {@link #CFG_DEFAULT_READS} (CSV of
	 * absolute paths).
	 */
	public static List<String> resolveDefaultReads() {
		...
		if (p == Platform.LINUX) {
			return Arrays.asList("/lib", "/lib64", "/usr/lib", "/usr/lib64", "/usr/bin", "/bin", "/etc/ssl",
					"/etc/resolv.conf", System.getProperty("java.home"));
		}
		if (p == Platform.MACOS) {
			return Arrays.asList("/usr/lib", "/usr/bin", "/System", "/Library", "/private/etc",
	 ...
	 * Always applied, not overridable via RDF config (the custom key only adds to
	 * this list).
	 *
	 * <p>
	 * Covers credential stores and cloud-provider secret directories that no agent
	 * run should ever access regardless of the task:
	 * <ul>
	...
	public static List<String> defaultSensitiveBlockList() {
		...
		return Arrays.asList(home + "/.ssh", home + "/.gnupg", home + "/.aws", home + "/.kube",
				home + "/.config/gcloud", home + "/.azure", home + "/.netrc", home + "/.npmrc", home + "/.pypirc",
	 ...
	 *
	 * <p>
	 * Reads are broadly allowed; sensitive credential paths and the SEMOSS home
	 * directory are explicitly blocked. The room folder, working directory, and
	 * binary parent are carved out as explicit RW/RO entries, which override any
	 * block that covers them as an ancestor.
	 *
	 * @param roomFolderPath   the per-room scratch/session directory (RW)
	 * @param workingDirectory the filePath/project slice the agent is editing (RW);
	 *                         nullable
	 * @param targetBinaryPath absolute path to the agent binary being wrapped (its
	 *                         parent dir is RO); nullable - callers may pass
	 *                         {@code null} when the path isn't known yet
	 */
	public static SandboxPolicy defaultPolicy(String roomFolderPath, String workingDirectory, String targetBinaryPath) {
		SandboxPolicyBuilder b = SandboxPolicy.builder().withEnforcement(resolveEnforcement())
		...
		if (semossBase != null && !semossBase.trim().isEmpty()) {
			addPathsSafely(b, "block", List.of(semossBase), "Utility.getBaseFolder", SandboxPolicyBuilder::withBlock);
		}
		addPathsSafely(b, "block", resolveCustomBlockList(), CFG_BLOCK_READS, SandboxPolicyBuilder::withBlock);

		return b.build();
	}

	private static void addPathsSafely(SandboxPolicyBuilder b, String kind, Iterable<String> paths, String source,
			BiConsumer<SandboxPolicyBuilder, String> add) {
		for (String p : paths) {
			try {
				add.accept(b, p);
			} catch (IllegalArgumentException e) {
				classLogger.warn("skipping invalid sandbox {} path '{}' (from {}): {}", kind, p, source,
						e.getMessage());
			}
		}
	}
	...
	/**
	 * Build the sandbox policy that should actually be applied to a run: always
	 * start from {@link #defaultPolicy(String, String, String)} (room folder +
	 * working dir + binary parent + DIHelper default reads), and overlay any
	 * pixel-supplied {@code override} on top - paths are unioned (RW wins over RO
	 * via the builder's dedupe), and the override's enforcement mode wins when
	 * present.
	 *
	 * <p>
	 * This is the entry point harnesses should call instead of branching on
	 * {@code ctx.getSandboxPolicy()} directly - otherwise pixel-level
	 * {@code sandbox_writes}/{@code sandbox_reads} would replace the defaults and
	 * leave the agent binary unreadable inside the sandbox.
	 */
	public static SandboxPolicy buildEffectivePolicy(String roomFolderPath, String workingDirectory,
			String targetBinaryPath, SandboxPolicy override) {
