...
public class MakePlaywrightMCPReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		Path recordingsDir = ReplayFromFileReactor.initRecordingsDir(this.insight);
	...
	private JSONObject createToolFromRecording(File file) throws IOException {
		...
		for (List<Step> stepGroup : envelope.steps()) {
			for (Step step : stepGroup) {
				if (step.type() == StepType.TYPE && step.storeValue()) {
	...
	private String sanitizePropertyName(String label) {
		...
		String sanitized = label.replaceAll("[^a-zA-Z0-9\\s]", "")
				.trim()
				.replaceAll("\\s+", "_")
				.toLowerCase();
