...
public final class MCPUtility {
   ...
   * @return
   */
  public static String runPythonTool(
      IProject project,
      Insight insight,
      String functionName,
      JSONObject functionProperties,
    ...
    classLogger.info(
        "Running python tool '" + runMethod + "' from project " + project.getProjectId());
   ...
   * @return
   */
  public static String runPixelTool(
      IProject project,
      Insight insight,
      String functionName,
      JSONObject functionProperties,
    ...
    if (project != null) {
      classLogger.info(
          "Running pixel tool '" + runMethod + "' from project " + project.getProjectId());
    } else {
  ...
  private static String formatArrayValue(Object arrayValue, String propType) {
    ...
    if (arrayValue instanceof List) {
      ...
      for (int i = 0; i < list.size(); i++) {
        ...
        if (propType.toUpperCase().contains("STR")
            && item != null
            && !item.toString().equals("None")) {
   ...
   * @return
   */
  public static JSONObject appendProjectIdToTooslMethodName(
      String projectId, JSONObject jsonToolsMap) {
  ...
  public static JSONObject appendProjectIdToToolsArgs(String projectId, JSONObject jsonToolsMap) {
    ...
    for (int i = 0; i < toolsArray.length(); i++) {
      ...
      smssProjectId.put(
          "description",
          "This is a required id field. Always return the enum's single value of '"
              + projectId
              + "'");
  ...
  /**
   * Converts camelCase, PascalCase, or snake_case strings to title case with spaces Useful for
   * pretty version of name -> title in MCP Tool schema
   *
   * @param input
   * @return
   */
