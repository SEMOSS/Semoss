...
public class MakePythonMCPReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (!pyFolder.exists() || !pyFolder.isDirectory()) {
      String errorOutput =
          "There is no py/main.py that exists. Please create this file and then try. "
    ...
    if (!mcpPyFile.exists() || !mcpPyFile.isFile()) {
      String errorOutput =
          "There is no py/smss_driver.py that exists. Please create this file and then try. "
              + "File main.py is the main driver which is utilized in terms of creating the MCP tools.";
      throw new IllegalArgumentException(errorOutput);
    }
    ...
    String[] script =
        new String[] {
          "smssutil.gen_mcp(src_file='" + mcpPyFileLoc + "', dest_file='" + outputFileLoc + "')"
        };
