 ...
 *   <li>{@code content} - matching lines in {@code file:line:content} format, with optional
 *       context lines via {@code after_context} / {@code before_context} / {@code context}
 *   <li>{@code count} - match count per file in {@code file:N} format
 * </ul>
 *
 * <p>Use {@code glob} to restrict which file types are searched (e.g. {@code *.java}).
 * Use {@code case_insensitive=true} for case-insensitive matching.
 * Use {@code head_limit} to cap result volume (default 200).
 *
 * <p><b>Note:</b> argument names are valid Pixel identifiers (no leading dashes) because
 * MCPUtility constructs a Pixel call literal from these keys; dash-prefixed names break
...
public class GrepFilesReactor extends AbstractAgentToolReactor {
    ...
    protected NounMetadata doExecute() throws Exception {
        ...
        if (outputMode == null || outputMode.trim().isEmpty()) outputMode = "files_with_matches";
        int after  = parseIntOr(afterStr, 0);
        int before = parseIntOr(beforeStr, 0);
        int ctx    = parseIntOr(contextStr, 0);
        if (ctx > 0) { after = ctx; before = ctx; }
        ...
        for (Path p : paths) {
            ...
            try {
                ...
                if (isFilesMode) {
                    ...
                    if (count > 0) results.add(relPath + ":" + count);
                } else {
