 ...
 *
 * <p>
 * Called from the semoss harness when a tool name matches either a named
 ...
 *
 * <p>
 * All entry points return a string suitable for handing directly back to the
...
public final class SubAgentDispatcher {
	...
	/**
	 * Spawn a named subagent (alias resolved via {@code spec.workspaceId}). Returns
	 * a JSON string
	 * {@code {"jobId":..., "runId":..., "roomId":..., "status":..., "alias":...}}
	 * for the LLM.
	 */
	public static String spawnNamed(SubAgentSpec spec, Map<String, Object> args, Room parentRoom,
			Insight callerInsight) {
		return spawnNamed(spec, args, parentRoom, callerInsight, null, parentRoom.getRoomOrWorkspaceSystemPrompt());
	}
	...
	/**
	 * Harness-safe named spawn. The caller supplies the parent's authored system
	 * prompt because the live room may temporarily contain a composed runtime
	 * prompt.
	 */
	public static String spawnNamed(SubAgentSpec spec, Map<String, Object> args, Room parentRoom, Insight callerInsight,
		...
		if (prompt == null) {
			return GSON
					.toJson(error("Missing required argument 'prompt' for named subagent '" + spec.getAlias() + "'"));
		}
		// Named subagents own their system prompt through workspace configuration. Fold
		// legacy callers' context value into the ordinary task input instead of
		// allowing
	...
	public static String spawnAnonymous(Map<String, Object> args, Room parentRoom, Insight callerInsight) {
		return spawnAnonymous(args, parentRoom, callerInsight, null, parentRoom.getRoomOrWorkspaceSystemPrompt());
	}
	...
	/**
	 * Harness-safe anonymous spawn. The caller supplies the parent's authored
	 * system prompt because the live room may temporarily contain a composed
	 * runtime prompt.
	 */
	public static String spawnAnonymous(Map<String, Object> args, Room parentRoom, Insight callerInsight,
			String parentJobId, String parentAuthoredSystemPrompt) {
	 ...
	 * only when the caller didn't pass one - preserves callers that aren't yet on
	 * the explicit-jobId path.
	 */
	...
	// envelope.
	// status=RUNNING when the child is still working; otherwise a terminal value
	// with result/error filled in.
	public static String check(String jobId, Insight callerInsight) {
		...
		try {
			Map<String, Object> run = AgentRunService.get().getRun(jobId, callerInsight);
	...
	// Block until the child AgentRun reaches a terminal status or wait-side
	// timeoutSec elapses.
	// On wait-side timeout the envelope reports status=RUNNING; the child is
	// unaffected.
	public static String wait(String jobId, Insight callerInsight, int timeoutSec) {
		...
		try {
			Map<String, Object> run = AgentRunService.get().waitForRun(jobId, callerInsight, timeoutSec * 1000L);
	...
	/**
	 * Reads a boolean tool-call arg. Accepts JSON boolean true/false or string
	 * "true"/"false".
	 */
