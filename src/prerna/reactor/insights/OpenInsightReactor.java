...
public class OpenInsightReactor extends AbstractInsightReactor {
	...
	public OpenInsightReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.PROJECT.getKey(), ReactorKeysEnum.ID.getKey(),
				ReactorKeysEnum.PARAM_KEY.getKey(), ReactorKeysEnum.ADDITIONAL_PIXELS.getKey(),
				ReactorKeysEnum.PARAM_VALUES_MAP.getKey(), CACHEABLE, USE_EXISTING_OPEN,
	...
	public NounMetadata execute() {
		 ...
		 *
		 * 1) Permission checks / pulling the recipe from the insights database 2)
		 * Legacy insight check - not really important for most developers 3) Do we want
		 * to use an existing insight that the user has opened? ******* If yes and it is
		 * opened - redirect to the UI State 4) Running the base insight recipe or
		 * pulling the cached insight 5) Parameter insight ******* Params are set in
		 * THIS current insight (not the new insight being created from the reactor)
		 * ******* If they are set, we then look for Panel 0 and make sure it has a
		 * param view ******* If yes - then we do the replacement and run that pixel 6)
		 * Run any additional pixels that are also passed in ******* This is things like
		 * scheduling an export post insight execution (but post the parameter filled
		 * recipe)
		 *
		 */
		...
		// we have to pull the insight assets in case those changed since we last opened
		// the insight
		ClusterUtil.pullProjectFolder(project,
				AssetUtility.getProjectVersionFolder(project.getProjectName(), projectId), rdbmsId);
		...
		try {
			...
			try {
				...
				NounMetadata noun = new NounMetadata(e2.getMessage(), PixelDataType.CONST_STRING,
						PixelOperationType.ERROR);
				SemossPixelException err = new SemossPixelException(noun);
				err.setContinueThreadOfExecution(false);
				throw err;
			} catch (ArrayIndexOutOfBoundsException e2) {
				NounMetadata noun = new NounMetadata("Insight does not exist", PixelDataType.CONST_STRING,
						PixelOperationType.ERROR);
				SemossPixelException err = new SemossPixelException(noun);
				err.setContinueThreadOfExecution(false);
				throw err;
		...
		// boolean isParam = cacheable && (params != null ||
		// PixelUtility.isNotCacheable(newInsight.getPixelList().getPixelRecipe()));
		...
		if (cacheable && hasCache && cachedInsight == null) {
			...
			InsightCacheUtility.deleteCache(newInsight.getProjectId(), newInsight.getProjectName(), rdbmsId,
					paramValues, true);
			additionalMetas.add(getWarning(
	 ...
	 * @return
	 */
	protected Insight getCachedInsight(Insight existingInsight, Map<String, Object> paramValues)
	 ...
	 * @return
	 */
	protected PixelRunner getCachedInsightData(Insight cachedInsight, Map<String, Object> paramValues)
			throws IOException, JsonSyntaxException {
		...
		try {
			...
			if (cachedInsight.getCachedDateTime() != null) {
				runner.addResult("GetInsightCachedDateTime();",
						new NounMetadata(cachedInsight.getCachedDateTime().toString(), PixelDataType.CONST_STRING),
						true);
			} else {
				runner.addResult("GetInsightCachedDateTime();", new NounMetadata(
						"Could not determine cached timestamp for insight", PixelDataType.CONST_STRING), true);
			}
				...
				for (String k : keys) {
					...
					if (type == PixelDataType.FRAME) {
						try {
							...
							runner.addResult(
									"CACHED_FRAME_HEADERS", new NounMetadata(frame.getFrameHeadersObject(),
	 ...
	 * @return
	 */
	protected PixelRunner runParameters(Insight newInsight, PixelRunner runner, Map<String, Object> paramValues,
			List<NounMetadata> additionalMetas, Logger logger) {
		if (paramValues != null && !paramValues.isEmpty()) {
			...
			if (paramPixel == null) {
				additionalMetas.add(getWarning(
						"Could not properly parse the insight recipe to generate the parameterized pixel expression"));
			} else {
				...
				String executionPixel = "META | RunParameterRecipe(recipe=[\"<encode>" + paramPixel
						+ "</encode>\"], fill=true);";
	...
	/**
	 * For legacy insights Do not use other than to handle that case
	 *
	 * @param oldInsight
	 * @return
	 */
