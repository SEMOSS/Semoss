...
public class OpenInsightReactor extends AbstractInsightReactor {
  ...
  public NounMetadata execute() {
    ...
    try {
      ...
      try {
        ...
        NounMetadata noun =
            new NounMetadata(e2.getMessage(), PixelDataType.CONST_STRING, PixelOperationType.ERROR);
        SemossPixelException err = new SemossPixelException(noun);
        err.setContinueThreadOfExecution(false);
        throw err;
        ...
        NounMetadata noun =
            new NounMetadata(
                "Insight does not exist", PixelDataType.CONST_STRING, PixelOperationType.ERROR);
        SemossPixelException err = new SemossPixelException(noun);
        err.setContinueThreadOfExecution(false);
        throw err;
      }
    ...
    if (user != null && useExistingInsightIfOpen()) {
      ...
      if (userOpen != null && !userOpen.isEmpty()) {
        ...
        if (alreadyOpenedInsight == null) {
          ...
          NounMetadata noun =
              new NounMetadata(
                  runnerWraper, PixelDataType.PIXEL_RUNNER, PixelOperationType.OPEN_SAVED_INSIGHT);
          return noun;
        }
      }
    }
    ...
    boolean isDashoard =
        cacheable && PixelUtility.isDashboard(newInsight.getPixelList().getPixelRecipe());
    ...
    if (cacheable && hasCache && cachedInsight == null) {
      ...
      InsightCacheUtility.deleteCache(
          newInsight.getProjectId(), newInsight.getProjectName(), rdbmsId, paramValues, true);
      additionalMetas.add(
          getWarning(
      ...
      try {
        ...
        logger.info(
            "Error occurred pulling cached insight. Deleting cache and executing original recipe.");
        InsightCacheUtility.deleteCache(
            newInsight.getProjectId(), newInsight.getProjectName(), rdbmsId, paramValues, true);
        additionalMetas.add(
            getWarning(
    ...
    if (runner == null) {
      ...
      if (cacheable && !isDashoard) {
        ...
        try {
          InsightCacheUtility.cacheInsight(
              newInsight, getCachedRecipeVariableExclusion(runner), paramValues);
          Path projectFolder =
              Paths.get(
          ...
          String cacheFolder =
              InsightCacheUtility.getInsightCacheFolderPath(newInsight, paramValues);
    ...
    try {
      ...
      if (cachedInsight.getCachedDateTime() != null) {
        runner.addResult(
            ...
            new NounMetadata(
                cachedInsight.getCachedDateTime().toString(), PixelDataType.CONST_STRING),
            true);
      } else {
        runner.addResult(
            "GetInsightCachedDateTime();",
            new NounMetadata(
                "Could not determine cached timestamp for insight", PixelDataType.CONST_STRING),
            true);
      }
      ...
      Map<String, Object> viewData =
          InsightCacheUtility.getCachedInsightViewData(cachedInsight, paramValues);
      List<Object> pixelReturn = (List<Object>) viewData.get("pixelReturn");
      if (!pixelReturn.isEmpty()) {
        runner.addResult(
            "CACHED_DATA", new NounMetadata(pixelReturn, PixelDataType.CACHED_PIXEL_RUNNER), true);
      }

      // get the insight config for layout
      NounMetadata insightConfig =
          cachedInsight.getVarStore().get(SetInsightConfigReactor.INSIGHT_CONFIG);
   ...
   * @return
   */
  protected PixelRunner runParameters(
      Insight newInsight,
      PixelRunner runner,
      Map<String, Object> paramValues,
      List<NounMetadata> additionalMetas,
      Logger logger) {
    if (paramValues != null && !paramValues.isEmpty()) {
      ...
      if (panel0 != null) {
        ...
        if ("param".equals(panel0View)) {
          ...
          if (paramMap != null) {
            List<Map<String, Object>> jsonMapArray =
                (List<Map<String, Object>>) paramMap.get("json");
      ...
      if (paramPixel == null) {
        ...
        String executionPixel =
            "META | RunParameterRecipe(recipe=[\"<encode>"
                + paramPixel
                + "</encode>\"], fill=true);";
        ...
        Map<String, Object> innerRunnerMap =
            (Map<String, Object>) innerRunner.getResults().get(0).getValue();
