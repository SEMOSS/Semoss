...
public abstract class AbstractInsightReactor extends AbstractReactor {
  ...
  protected String getProject() {
    ...
    if (projectId == null) {
      ...
      throw new IllegalArgumentException(
          "Need to define the project where the insight currently exists");
    }
  ...
  /**
   * This can either be passed specifically using the insightName key Or it is the second input in a
   * list of values Save(engineName, insightName)
   *
   * @return
   */
  ...
  /**
   * Get the description for the insight Assume it is passed by the key or it is the last string
   * passed into the curRow
   *
   * @return
   */
  ...
  /**
   * Set the new file as the image for the insight Semoss/images/engineName_insightID.png
   *
   * @param base64Image
   * @param insightId
   * @param engineName
   ...
   */
  protected void storeImageFromFile(
    ...
    final String saveImageFileAs =
        basePath + DIR_SEP + "image." + FilenameUtils.getExtension(fileName);
   ...
   * @throws FileNotFoundException
   */
  protected void storeImageFromPng(
      String base64Image, String insightId, String projectId, String projectName) {
    // set up path to save image to file
    final String DIR_SEP = java.nio.file.FileSystems.getDefault().getSeparator();
   ...
   * @return
   */
  protected boolean saveFilesInInsight(
      PixelList insightPixelList,
      String projectId,
      String projectName,
      String newInsightId,
      boolean deleteOrigFile,
      PixelList originalInsightPixelList) {
    ...
    for (Pixel p : insightPixelList) {
      ...
      if (p.isFileRead()) {
        ...
        List<Map<String, Object>> datasources =
            PixelUtility.getDatasourcesMetadata(this.insight.getUser(), p.getPixelString());
        for (int i = 0; i < datasources.size(); i++) {
          ...
          if (datasourceType.equals("FILEREAD")) {
            ...
            String filePixelPortion =
                ((Map<String, List<String>>) datasourceMap.get("params")).get("filePath").get(0);
            String space = null;
            ...
            boolean isUserSpace =
                space != null && space.toUpperCase().equals(AssetUtility.USER_SPACE_KEY);
   ...
   * @return
   */
  protected String getUserSchemaNameOrExistingSchemaName(
      String projectId, String existingInsightId) {
    if (projectId == null) {
      return null;
    }
    GenRowStruct genericEngineGrs = this.store.getNoun(SCHEMA_NAME);
