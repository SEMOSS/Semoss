...
public abstract class AbstractInsightReactor extends AbstractReactor {
	...
	/**
	 * This can either be passed specifically using the insightName key Or it is the
	 * second input in a list of values Save(engineName, insightName)
	 * 
	 * @return
	 */
			...
			if (cronExpression != null) {
				...
				if (!CronExpression.isValidExpression(cronExpression)) {
					throw new IllegalArgumentException(
							"Cron expression '" + cronExpression + "' is not of a valid format");
				}
	...
	protected List<String> getRecipe() {
		...
		if (genericRecipeGrs != null && !genericRecipeGrs.isEmpty()) {
			...
			List<String> recipe = new ArrayList<>(size);
	...
	/**
	 * Get the description for the insight Assume it is passed by the key or it is
	 * the last string passed into the curRow
	 * 
	 * @return
	 */
	 ...
	 */
	protected void storeImageFromFile(final String fileName, final String insightId, final String projectId,
			final String projectName) {
		// set up path to save image to file
		final String DIR_SEP = java.nio.file.FileSystems.getDefault().getSeparator();
		final String basePath = EngineUtility.getSpecificEngineVersionFolder(IEngine.CATALOG_TYPE.PROJECT, projectId,
				projectName) + DIR_SEP + insightId;
	...
	/**
	 * Save base64 encoded image to file Semoss/images/engineName_insightID.png
	 * 
	 * @param base64Image
	 * @param insightId
	 * @param engineName
	...
	protected void storeImageFromPng(String base64Image, String insightId, String projectId, String projectName) {
		// set up path to save image to file
		final String DIR_SEP = java.nio.file.FileSystems.getDefault().getSeparator();
		final String imagePath = EngineUtility.getSpecificEngineVersionFolder(IEngine.CATALOG_TYPE.PROJECT, projectId,
				projectName) + DIR_SEP + insightId + DIR_SEP + "image.png";
	 ...
	 * @return
	 */
	protected boolean saveFilesInInsight(PixelList insightPixelList, String projectId, String projectName,
			String newInsightId, boolean deleteOrigFile, PixelList originalInsightPixelList) {
		...
		for (Pixel p : insightPixelList) {
			...
			if (p.isFileRead()) {
				...
				List<Map<String, Object>> datasources = PixelUtility.getDatasourcesMetadata(this.insight.getUser(),
						p.getPixelString());
				for (int i = 0; i < datasources.size(); i++) {
					...
					if (datasourceType.equals("FILEREAD")) {
						...
						String filePixelPortion = ((Map<String, List<String>>) datasourceMap.get("params"))
								.get("filePath").get(0);
						String space = null;
						...
						if (space == null || space.isEmpty() || isUserSpace) {
							...
							String newFileLoc = AssetUtility.getProjectVersionFolder(projectName, projectId)
									+ DIR_SEPARATOR + newInsightId + DIR_SEPARATOR + "data";
							...
							if (origF.getParentFile().getAbsolutePath()
									.equals(newF.getParentFile().getAbsolutePath())) {
								continue;
							}
	...
	protected File getPipelineFileLocation(String projectId, String projectName, String rdbmsID) {
		String pipelinePath = EngineUtility.getSpecificEngineVersionFolder(IEngine.CATALOG_TYPE.PROJECT, projectId,
				projectName) + DIR_SEPARATOR + rdbmsID;
