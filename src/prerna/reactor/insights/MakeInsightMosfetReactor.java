...
public class MakeInsightMosfetReactor extends AbstractInsightReactor {
  ...
  private MosfetFile generateMosfetFromInsight(IProject project, String insightId) {
    String query =
        "SELECT DISTINCT ID, QUESTION_NAME, QUESTION_LAYOUT, CACHEABLE, QUESTION_PKQL FROM QUESTION_ID WHERE ID IN ('"
            + insightId
            + "');";
    ...
    query =
        "SELECT DISTINCT INSIGHTID, METAKEY, METAVALUE, METAORDER FROM INSIGHTMETA WHERE INSIGHTID='"
            + insightId
            + "' ORDER BY METAORDER";
    try {
      wrapper = WrapperManager.getInstance().getRawWrapper(insightRdbms, query);
