...
public class RequestInsightReactor extends AbstractReactor {
  ...
  public RequestInsightReactor() {
    this.keysToGet =
        new String[] {
          ...
          ReactorKeysEnum.PERMISSION.getKey(),
  ...
  public NounMetadata execute() {
    ...
    if (user == null) {
      NounMetadata noun =
          new NounMetadata(
              "User must be signed into an account in order to request a insight",
    ...
    Integer currentUserPermission =
        SecurityInsightUtils.getUserInsightPermission(userId, projectId, insightId);
    ...
    Integer currentPendingUserPermission =
        SecurityInsightUtils.getUserAccessRequestInsightPermission(userId, projectId, insightId);
    ...
    SecurityInsightUtils.setUserAccessRequest(
        userId, userType, projectId, requestComment, insightId, requestPermission, user);
  ...
  private void sendEmail(
      User user, String projectId, String insightId, String permission, String requestComment) {
    ...
    if (template != null && !template.isEmpty()) {
      ...
      if (insightOwners != null && !insightOwners.isEmpty()) {
        ...
        Map<String, String> emailReplacements =
            SocialPropertiesUtil.getInstance().getEmailStaticProps();
        ...
        EmailUtility.sendEmail(
            emailSession,
            insightOwners.toArray(new String[0]),
            null,
            null,
            SocialPropertiesUtil.getInstance().getSmtpSender(),
            "SEMOSS - Insight Access Request",
            message,
            true,
            null);
      }
