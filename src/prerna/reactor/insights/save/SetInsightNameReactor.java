...
public class SetInsightNameReactor extends AbstractInsightReactor {
  ...
  public NounMetadata execute() {
    ...
    updateRecipeFile(
        logger, project.getProjectId(), project.getProjectName(), existingId, insightName);
    logger.info("3) Done");
   ...
   * @param recipeToSave
   */
  protected void updateRecipeFile(
      Logger logger, String projectId, String projectName, String rdbmsID, String insightName) {
    ...
    if (mosfet.exists()) {
      try {
        ...
        GitRepoUtils.commitAddedFiles(
            gitFolder, GitUtils.getDateMessage("Changed " + insightName + " insight name"));
      } catch (Exception e) {
