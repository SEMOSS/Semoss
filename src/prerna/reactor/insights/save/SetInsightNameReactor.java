...
public class SetInsightNameReactor extends AbstractInsightReactor {
	 ...
	 * @param recipeToSave
	 */
	protected void updateRecipeFile(Logger logger, String projectId, String projectName, String rdbmsID,
			String insightName) {
		String recipeLocation = AssetUtility.getProjectVersionFolder(projectName, projectId) + DIR_SEPARATOR + rdbmsID
				+ DIR_SEPARATOR + MosfetFile.RECIPE_FILE;
		...
		if (mosfet.exists()) {
			try {
				...
				GitRepoUtils.commitAddedFiles(gitFolder,
						GitUtils.getDateMessage("Changed " + insightName + " insight name"));
			} catch (Exception e) {
