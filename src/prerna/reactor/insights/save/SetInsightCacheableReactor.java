...
public class SetInsightCacheableReactor extends AbstractInsightReactor {
	...
	public SetInsightCacheableReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.PROJECT.getKey(), ReactorKeysEnum.ID.getKey(), CACHEABLE,
				CACHE_MINUTES, CACHE_CRON, CACHE_ENCRYPT};
	}
	...
	public NounMetadata execute() {
		...
		Map<String, Object> currentInsightDetails = SecurityInsightUtils.getSpecificInsightCacheDetails(projectId,
				existingId);
		...
		if (cacheEncrypt && SecretsFactory.getSecretConnector() == null) {
			...
			additionalMetas.add(NounMetadata.getWarningNounMessage(
		...
		SecurityInsightUtils.updateInsightCache(projectId, existingId, cache, cacheMinutes, cacheCron, cachedOn,
				cacheEncrypt);
		logger.info("2) Done");
