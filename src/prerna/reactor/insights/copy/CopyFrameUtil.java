...
public class CopyFrameUtil {
	...
	/**
	 * Copy the frame into a new frame Will also account for R/Py to rename the
	 * variable since the insight copy will share the same space
	 *
	 ...
	 * @throws Exception
	 */
	public static ITableDataFrame copyFrame(Insight insightContext, ITableDataFrame frameToCopy, int limit)
			throws Exception {
	...
	public static ITableDataFrame renameFrame(ITableDataFrame frame, String newName) {
		...
		if (frame instanceof PandasFrame) {
			...
			((PandasFrame) frame)
					.runScript(PandasSyntaxHelper.makeWrapper(((PandasFrame) frame).getWrapperName(), newName));
			...
			if (ifilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
				...
				if (simpleF.getSimpleFilterType() == SimpleQueryFilter.FILTER_TYPE.COL_TO_COL) {
					...
					if (newLhs != null && newRhs != null) {
						...
						SimpleQueryFilter newFilter = new SimpleQueryFilter(newLhs, simpleF.getComparator(),
								simpleF.getRComparison());
						newFrame.addFilter(newFilter);
					} else if (newRhs != null) {
						SimpleQueryFilter newFilter = new SimpleQueryFilter(simpleF.getLComparison(),
								simpleF.getComparator(), newRhs);
						newFrame.addFilter(newFilter);
					} else {
