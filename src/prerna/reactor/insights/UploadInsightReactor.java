  ...
  public NounMetadata execute() {
    ...
    if (!SecurityEngineUtils.userCanEditEngine(user, projectId)) {
      throw new IllegalArgumentException(
          "User does not have permission to add insights in the proejct");
    }
    ...
    try {
        ...
        SemossPixelException exception =
            new SemossPixelException(
                NounMetadata.getErrorNounMessage(
                    "Unable to find " + MosfetFile.RECIPE_FILE + " file."));
      ...
      SemossPixelException exception =
          new SemossPixelException(NounMetadata.getErrorNounMessage("Unable to unzip files."));
      exception.setContinueThreadOfExecution(false);
      throw exception;
    }
    ...
    int step = 1;
    logger.info(
        step + ") Add insight " + Utility.cleanLogString(insightName) + " to rdbms store...");
