...
public class RunParameterRecipeReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
      ...
      for (String paramPixel : recipeInput) {
        ...
        for (String paramKey : paramInputs.keySet()) {
          ...
          String paramReplacement =
              pStruct.getDetailsList().get(0).getPixelStringReplacement(pStruct.getDefaultValue());
          // for some reason
    ...
    // AND DIDN'T PUT AddPanel(0) INSIDE THE QUERY PORTION OF THE JSON
    {
      if (this.insight.getInsightPanels().containsKey("0")) {
        ...
        runnerResults.add(
            0,
            new NounMetadata(
                insight.getInsightPanel("0"), PixelDataType.PANEL, PixelOperationType.PANEL_OPEN));
      }
    }
    ...
    NounMetadata noun =
        new NounMetadata(
            runnerWraper, PixelDataType.PIXEL_RUNNER, PixelOperationType.PARAMETER_EXECUTION);
    return noun;
  }
