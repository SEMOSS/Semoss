...
public class GetConsolidatedCodeExecutionReactor extends AbstractReactor {
	...
	private static final Logger classLogger = LogManager.getLogger(GetConsolidatedCodeExecutionReactor.class);
			...
			for (int i = 0; i < size; i++) {
				...
				if (p.isCodeExecution()) {
					...
					if (prevLanguage == p.getLanguage()) {
						...
						curFile = new File(codeDir + "/file" + (counter++) + CODE_SUFFIX + "."
								+ Variable.getExtension(prevLanguage));
