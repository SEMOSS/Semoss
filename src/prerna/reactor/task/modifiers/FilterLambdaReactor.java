...
public class FilterLambdaReactor extends AbstractLambdaTaskReactor {
	...
	protected void buildTask() {
		...
		ClassMaker myClass = new ClassMaker(FilterTaskIterator.class.getPackage().getName(),
				Utility.getRandomString(12));
		...
		for (int i = 0; i < size; i++) {
			...
			if (type.equalsIgnoreCase("STRING")) {
				method.append("String ").append(varName).append(" = random_dont_conflict_values[").append(i)
						.append("].toString(); ");
			} else if (type.equalsIgnoreCase("NUMBER")) {
				method.append("Double ").append(varName).append(" = (Double) random_dont_conflict_values[").append(i)
						.append("]; ");
			}
		}
		method.append(code);
