...
public class SubstringLambda extends AbstractMapLambda {
	...
	public void init(List<Map<String, Object>> headerInfo, List<String> columns) {
		...
		if (inputCols == 0) {
			throw new SemossPixelException(new NounMetadata("No column input found in Substring",
					PixelDataType.CONST_STRING, PixelOperationType.ERROR));
		}

		if (inputCols > 1) {
			throw new SemossPixelException(new NounMetadata("Can only input 1 column into Substring",
					PixelDataType.CONST_STRING, PixelOperationType.ERROR));
		}
