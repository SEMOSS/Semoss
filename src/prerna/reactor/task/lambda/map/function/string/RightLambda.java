  ...
  public void init(List<Map<String, Object>> headerInfo, List<String> columns) {
    ...
    if (inputCols == 0) {
      throw new SemossPixelException(
          new NounMetadata(
              "No column input found in Right",
    ...
    if (inputCols > 1) {
      throw new SemossPixelException(
          new NounMetadata(
              "Can only input 1 column into Right",
              PixelDataType.CONST_STRING,
              PixelOperationType.ERROR));
    }
    ...
    if (this.rightColumn == null) {
      ...
      throw new SemossPixelException(
          new NounMetadata(
              "Could not find column " + valueToFind + " in Right routine",
              PixelDataType.CONST_STRING,
              PixelOperationType.ERROR));
    }
    ...
    if (this.params.containsKey(INDEX)) {
      ...
      throw new SemossPixelException(
          new NounMetadata(
              "Must provide the index value for the function",
              PixelDataType.CONST_STRING,
              PixelOperationType.ERROR));
    }
  }
