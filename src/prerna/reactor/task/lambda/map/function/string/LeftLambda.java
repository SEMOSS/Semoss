...
public class LeftLambda extends AbstractMapLambda {
	...
	public void init(List<Map<String, Object>> headerInfo, List<String> columns) {
		...
		if (inputCols == 0) {
			throw new SemossPixelException(new NounMetadata("No column input found in Left", PixelDataType.CONST_STRING,
					PixelOperationType.ERROR));
		}

		if (inputCols > 1) {
			throw new SemossPixelException(new NounMetadata("Can only input 1 column into Left",
		...
		if (this.leftColumn == null) {
			// throw an error
			throw new SemossPixelException(new NounMetadata("Could not find column " + valueToFind + " in Left routine",
					PixelDataType.CONST_STRING, PixelOperationType.ERROR));
		}
		...
		if (this.params.containsKey(INDEX)) {
			...
			// throw an error
			throw new SemossPixelException(new NounMetadata("Must provide the index value for the function",
					PixelDataType.CONST_STRING, PixelOperationType.ERROR));
		}
	}
}
