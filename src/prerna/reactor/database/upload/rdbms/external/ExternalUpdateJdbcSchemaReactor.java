...
public class ExternalUpdateJdbcSchemaReactor extends AbstractReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(ExternalUpdateJdbcSchemaReactor.class);
  ...
  public ExternalUpdateJdbcSchemaReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.DATABASE.getKey(), ReactorKeysEnum.FILTERS.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    if (!SecurityEngineUtils.userCanEditEngine(user, databaseId)) {
      throw new IllegalArgumentException(
          "User does not have permission to edit this database schema");
    }
    ...
    try {
      ...
      try {
        ...
        throw new SemossPixelException(
            new NounMetadata(
                "Unable to get the database metadata",
      ...
      CustomTableAndViewIterator tableViewIterator =
          new CustomTableAndViewIterator(
              connection, meta, catalogFilter, schemaFilter, driverEnum, tableAndViewFilters);
      ...
      try {
        while (tableViewIterator.hasNext()) {
          ...
          try {
            ...
            columnsRs =
                RdbmsConnectionHelper.getColumns(
                    meta, tableOrView, catalogFilter, schemaFilter, driverEnum);

            while (columnsRs.next()) {
              String cName = columnsRs.getString(COLUMN_NAME_STR);
              columnNames.add(cName);
              columnTypes.add(columnsRs.getString(COLUMN_TYPE_STR));
              cleanColumnTypes.add(
                  SemossDataType.convertStringToDataType(columnsRs.getString(COLUMN_TYPE_STR))
                      .toString());
      ...
      Map<String, Map<String, Double>> positions =
          GenerateMetamodelLayout.generateMetamodelLayoutForExternal(databaseTables, databaseJoins);
