...
public class RdbmsUploadTableDataReactor extends AbstractDatabaseUploadFileReactor {
  ...
  public void generateNewDatabase(User user, final String newDatabaseName, final String filePath)
    ...
    Map<String, String> additionalDataTypeMap =
        UploadInputUtility.getAdditionalCsvDataTypes(this.store);
    ...
    String uniqueRowId =
        uniqueColumnName.equalsIgnoreCase("true")
    ...
    Object[] headerTypesArr =
        UploadUtilities.getHeadersAndTypes(this.helper, dataTypesMap, additionalDataTypeMap);
    ...
    try {
      sqlTypes =
          RdbmsUploadReactorUtility.createNewTable(
              this.database, tableName, uniqueRowId, headers, types, replace);
    } catch (Exception e) {
  ...
  public void addToExistingDatabase(String filePath) throws Exception {
    ...
    Map<String, Map<String, String>> existingRDBMSStructure =
        RDBMSEngineCreationHelper.getExistingRDBMSStructure(this.database);
    ...
    String tableToInsertInto =
        determineExistingTableToInsert(existingRDBMSStructure, headers, types);
    if (tableToInsertInto == null) {
      ...
      try {
        sqlTypes =
            RdbmsUploadReactorUtility.createNewTable(
                this.database, tableToInsertInto, uniqueRowId, headers, types, replace);
      } catch (Exception e) {
        classLogger.error(Constants.STACKTRACE, e);
        throw new SemossPixelException(
            new NounMetadata(
      ...
      logger.info("Done create table");
      bulkInsertFile(
          this.database, this.helper, tableToInsertInto, headers, types, additionalTypes, clean);
      ...
      WriteOWLEngine owlEngine = this.database.getOWLEngineFactory().getWriteOWL();
      RdbmsUploadReactorUtility.generateTableMetadata(
          owlEngine, tableToInsertInto, uniqueRowId, headers, sqlTypes, additionalTypes);
      UploadUtilities.insertFlatOwlMetadata(
          owlEngine,
          tableToInsertInto,
          headers,
  ...
  private void bulkInsertFile(
    ...
    try {
        ...
        for (int colIndex = 0; colIndex < nextRow.length; colIndex++) {
          ...
          // int
            ...
            try {
              ...
              // parsing as Double
              int mult = 1;
              if (val.startsWith("(") || val.startsWith("-")) mult = -1;
          ...
          // timestamps
            ...
            // can I get a format?
            Long dTime =
                SemossDate.getTimeForTimestamp(nextRow[colIndex], additionalTypes[colIndex]);
            if (dTime != null) {
      ...
      if (nextRow == null) {
        ...
        errorMessage =
            "Error occurred while performing insert on csv data row:"
                + "\n"
  ...
  private String determineExistingTableToInsert(
      Map<String, Map<String, String>> existingRDBMSStructure,
      String[] headers,
      SemossDataType[] types) {
    ...
    for (String existingTableName : existingRDBMSStructure.keySet()) {
      ...
      for (int i = 0; i < headers.length; i++) {
        ...
        SemossDataType existingColDataType =
            SemossDataType.convertStringToDataType(existingColTypeMap.get(csvHeader.toUpperCase()));
  ...
  //			struct.add(new NounMetadata("ztest" + Utility.getRandomString(6),
  // PixelDataType.CONST_STRING));
