...
public class RdbmsUploadReactorUtility {
   ...
   * @param sqlTypes
   */
  public static void generateTableMetadata(
  ...
  /**
   * Create a new table given the table name, headers, and types Returns the sql types that were
   * generated
   *
   * @param tableName
   * @param headers
   ...
   * @throws IOException
   */
  public static void addIndex(IDatabaseEngine engine, String tableName, String columnName)
      throws Exception {
    String indexName = columnName.toUpperCase() + "_INDEX";
    String indexSql =
        "CREATE INDEX " + indexName + " ON " + tableName + "(" + columnName.toUpperCase() + ")";
    engine.insertData(indexSql);
  }

  /**
   * Delete all the row records from the database Only runs the operation on the tables that are
   * identified in the OWL file
   *
   * @param engine
   */
