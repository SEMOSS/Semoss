...
public class RDBMSEngineCreationHelper {
  ...
  public static Map<String, Map<String, String>> getExistingRDBMSStructure(
      IDatabaseEngine rdbmsEngine) {
    return getExistingRDBMSStructure(rdbmsEngine, null);
  }

  public static Map<String, Map<String, String>> getExistingRDBMSStructure(
      IDatabaseEngine rdbmsEngine, Set<String> tablesToRetrieve) {
    ...
    try {
      ...
      CustomTableAndViewIterator tableViewIterator =
          new CustomTableAndViewIterator(
              con, meta, catalogFilter, schemaFilter, driverEnum, tablesToRetrieve);
      try {
        while (tableViewIterator.hasNext()) {
          ...
          try {
            ...
            while (columnsRs.next()) {
              colDetails.put(
                  columnsRs.getString(COLUMN_NAME_STR), columnsRs.getString(COLUMN_TYPE_STR));
            }
          } catch (SQLException e) {
            classLogger.error(Constants.STACKTRACE, e);
          } finally {
  ...
  public static String cleanTableName(String s) {
    ...
    s =
        s.replaceAll(
            "[^a-zA-Z0-9\\_]", ""); // matches anything that is not alphanumeric or underscore
    while (s.contains("__")) {
      s = s.replace("__", "_");
    }
  ...
  public static boolean conceptExists(
      IDatabaseEngine engine, String tableName, String colName, Object instanceValue) {
    String query =
        "SELECT DISTINCT "
            + colName
            + " FROM "
