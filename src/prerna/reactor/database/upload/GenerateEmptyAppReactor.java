...
// if(user == null) {
// NounMetadata noun = new NounMetadata("User must be signed into an account in
// order to create a
...
// this.insight.getUser().isAnonymous()) {
...
//
// // need to make sure we are not overriding something that already exists in
// the file system
...
// if(appFolder.exists()) {
// throw new IllegalArgumentException("Database folder already contains an app
// directory with the
// same name. Please delete the existing app folder or provide a unique app
// name");
// }
//
...
// String appNames = (String)
// DIHelper.getInstance().getLocalProp(Constants.ENGINES);
// appNames = appNames + ";" + appId;
// DIHelper.getInstance().setLocalProperty(Constants.ENGINES, appNames);
//
// // and rename .temp to .smss
// File smssFile = new File(tempSmss.getAbsolutePath().replace(".temp",
// ".smss"));
// try {
