...
/**
 * 1) Rename the column in the DB 2) Update the physical name in the owl, this
 * does not change the conceptual name
 *
...
public class DatabaseRenameColumnReactor extends AbstractReactor {
	...
	public DatabaseRenameColumnReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.DATABASE.getKey(), ReactorKeysEnum.CONCEPT.getKey(),
				ReactorKeysEnum.COLUMN.getKey(), ReactorKeysEnum.NEW_VALUE.getKey()};
	...
	public NounMetadata execute() {
		...
		try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
			...
			// update owl
			try {
				owlEngine.renameProp(table, existingColumn, newColumn);
				owlEngine.commit();
				owlEngine.export();
