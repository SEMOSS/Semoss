...
public class RdfConvertLiteralTypeReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		String query = "select ?concept ?property where { "
				...
				+ concept + ">} " + "{?concept <" + propertyUri + "> ?property} " + "}";
		...
		for (Object[] modification : collection) {
			...
			if (object instanceof SemossDate) {
				...
				engine.doAction(ACTION_TYPE.REMOVE_STATEMENT, new Object[]{subject, propertyUri, object, false});
			}
			...
			try {
				...
				// add
				engine.doAction(ACTION_TYPE.ADD_STATEMENT, new Object[]{subject, propertyUri, newObject, false});
		...
		if (warning == null) {
			noun.addAdditionalReturn(getSuccess("Successfully modified the data type of " + property + " to "
					+ newDataType + ". Number of rows modifed = " + counter));
		} else {
	...
	private void writeExcel(String concept, String property, String datatype, List<Object[]> modifications) {
		...
		String filename = Utility
				.normalizePath(("Convert_" + concept + "_" + property + "_totype_" + datatype + "_" + modifiedDate)
