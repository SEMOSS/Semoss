...
public class RdfConvertLiteralTypeReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (!SecurityEngineUtils.userIsOwner(this.insight.getUser(), databaseId)) {
      throw new IllegalArgumentException(
          "Database " + databaseId + " does not exist or user is not an owner to the database");
    }
    ...
    String query =
        "select ?concept ?property where { "
            + "{?concept <http://www.w3.org/1999/02/22-rdf-syntax-ns#type> <http://semoss.org/ontologies/Concept/"
            + concept
            + ">} "
            + "{?concept <"
            + propertyUri
    ...
    for (Object[] modification : collection) {
      ...
      if (object instanceof SemossDate) {
        engine.doAction(
            ACTION_TYPE.REMOVE_STATEMENT,
            new Object[] {subject, propertyUri, ((SemossDate) object).getZonedDateTime(), false});
      } else {
        engine.doAction(
            ACTION_TYPE.REMOVE_STATEMENT, new Object[] {subject, propertyUri, object, false});
      }
      // now we try to convert the object
      try {
        Object newObject = null;
        if (newDataType == SemossDataType.STRING || newDataType == SemossDataType.FACTOR) {
          newObject = object + "";
        } else if (newDataType == SemossDataType.INT || newDataType == SemossDataType.DOUBLE) {
          newObject = Double.parseDouble(object + "");
        } else if (newDataType == SemossDataType.DATE || newDataType == SemossDataType.TIMESTAMP) {
        ...
        // add
        engine.doAction(
            ACTION_TYPE.ADD_STATEMENT, new Object[] {subject, propertyUri, newObject, false});
    ...
    if (warning == null) {
      noun.addAdditionalReturn(
          getSuccess(
              "Successfully modified the data type of "
                  + property
                  + " to "
                  + newDataType
                  + ". Number of rows modifed = "
                  + counter));
    } else {
  ...
  private void writeExcel(
      String concept, String property, String datatype, List<Object[]> modifications) {
    Date date = new Date();
    ...
    String filename =
        Utility.normalizePath(
                ("Convert_" + concept + "_" + property + "_totype_" + datatype + "_" + modifiedDate)
    ...
    String[] headers =
        new String[] {"Instance URI", "Original Value", "Original Value Type", "Modified Value"};
