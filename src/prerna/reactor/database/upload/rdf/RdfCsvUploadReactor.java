...
public class RdfCsvUploadReactor extends AbstractDatabaseUploadFileReactor {
  ...
  public void generateNewDatabase(User user, String newDatabaseName, String filePath)
    ...
    this.tempSmss =
        UploadUtilities.createTemporaryRdfSmss(
            this.database, this.databaseId, newDatabaseName, owlFile, baseUri);
    ...
    this.database.doAction(
        IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT, new Object[] {sub, typeOf, obj, true});
  ...
  private void processRelationships(
    ...
    while ((values = helper.getNextRow()) != null && count < endRow) {
      ...
      for (int relIndex = 0; relIndex < relationList.size(); relIndex++) {
        ...
        RdfUploadReactorUtility.createRelationship(
            owlEngine,
            customBaseURI,
            subject,
            object,
      ...
      for (int relIndex = 0; relIndex < nodePropList.size(); relIndex++) {
        ...
        RdfUploadReactorUtility.addNodeProperties(
            owlEngine, customBaseURI, subject, subjectValue, nodePropHash, allInsertStatements);
      }
