...
public class TinkerCsvUploadReactor extends AbstractDatabaseUploadFileReactor {
  ...
  public void generateNewDatabase(User user, String newDatabaseName, String filePath)
      throws Exception {
    ...
    this.tempSmss =
        UploadUtilities.generateTemporaryTinkerSmss(
            this.databaseId, newDatabaseName, owlFile, getTinkerDriverType());
    ...
    processRelationships(
        this.database, owlEngine, this.helper, Arrays.asList(headers), types, metamodelProps);
    ...
    Files.write(
        Paths.get(this.tempSmss.getAbsolutePath()), mapProp.getBytes(), StandardOpenOption.APPEND);
  ...
  private void processRelationships(
    ...
    while ((values = csvHelper.getNextRow()) != null && count < (endRow)) {
      ...
      for (int relIndex = 0; relIndex < relationList.size(); relIndex++) {
        ...
        createRelationship(
            database, subject, object, subjectValue, objectValue, predicate, propHash);
      }
  ...
  public void addNodeProperties(
    ...
    Vertex vert =
        (Vertex)
            database.doAction(
                IDatabaseEngine.ACTION_TYPE.VERTEX_UPSERT, new Object[] {nodeType, instanceName});
  ...
  /**
   * Gets the instance value for a given subject. The subject can be a concatenation. Note that we
   * do not care about the data type for this since a URI is always a string
   *
   ...
   * @param headers Map containing the header names to index within the values array
  ...
  private String createInstanceValue(String subject, String[] values, List<String> headers) {
    ...
    if (subject.contains("+")) {
      ...
      // that
      if (!retString.equals("")) retString = retString.substring(0, retString.length() - 1);
    } else {
  ...
  public void createRelationship(
    ...
    startV =
        (Vertex)
            database.doAction(
                IDatabaseEngine.ACTION_TYPE.VERTEX_UPSERT,
    ...
    database.doAction(
        IDatabaseEngine.ACTION_TYPE.EDGE_UPSERT,
        new Object[] {startV, subjectNodeType, endV, objectNodeType, propHash});
  }

  /**
   * Change the name of nodes that are concatenations of multiple CSV columns Example: changes the
   * string "Cat+Dog" into "CatDog"
   *
