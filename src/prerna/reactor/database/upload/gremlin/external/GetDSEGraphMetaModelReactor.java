	...
	public NounMetadata execute() {
		...
		if (host == null) {
			SemossPixelException exception = new SemossPixelException(new NounMetadata(
					"Requires host to get graph metamodel.", PixelDataType.CONST_STRING, PixelOperationType.ERROR));
		...
		if (port == null) {
			SemossPixelException exception = new SemossPixelException(new NounMetadata(
					"Requires port to get graph metamodel.", PixelDataType.CONST_STRING, PixelOperationType.ERROR));
			exception.setContinueThreadOfExecution(false);
			throw exception;
		}
		...
		if (graphName == null) {
			SemossPixelException exception = new SemossPixelException(
					new NounMetadata("Requires graph name to get graph metamodel.", PixelDataType.CONST_STRING,
		...
		if (!useLabel) {
			if (graphTypeId == null) {
				SemossPixelException exception = new SemossPixelException(
						new NounMetadata("Requires graph type id to get graph metamodel.", PixelDataType.CONST_STRING,
								PixelOperationType.ERROR));
				exception.setContinueThreadOfExecution(false);
				throw exception;
			}
