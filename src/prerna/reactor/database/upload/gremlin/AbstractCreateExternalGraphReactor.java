...
public abstract class AbstractCreateExternalGraphReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		try {
			...
			if (error) {
				...
				UploadUtilities.cleanUpCreateNewError(this.database, this.newDatabaseId, this.tempSmss, this.smssFile,
						this.databaseFolder);
			}
		}
	...
	private void generateNewDatabase(User user) throws Exception {
		...
		try {
			UploadUtilities.validateEngine(IEngine.CATALOG_TYPE.DATABASE, user, this.newDatabaseName,
					this.newDatabaseId);
		} catch (IOException e) {
		...
		this.databaseFolder = UploadUtilities.generateSpecificEngineFolder(IEngine.CATALOG_TYPE.DATABASE,
				this.newDatabaseId, this.newDatabaseName);
		classLogger.info("1. Complete");

		classLogger.info("Generate new database");
		classLogger.info("2. Create metadata for database...");
		File owlFile = UploadUtilities.generateOwlFile(IEngine.CATALOG_TYPE.DATABASE, this.newDatabaseId,
		...
		/*
		 * This is the portion where we validate user input And parse through the graph
		 * metadata
		 * 
		...
		if (!useLabel && nodeType == null) {
			SemossPixelException exception = new SemossPixelException(new NounMetadata(
					"Requires graph type id to save.", PixelDataType.CONST_STRING, PixelOperationType.ERROR));
			exception.setContinueThreadOfExecution(false);
			throw exception;
		}
		if (nodeName == null) {
			SemossPixelException exception = new SemossPixelException(new NounMetadata(
					"Requires graph name id to save.", PixelDataType.CONST_STRING, PixelOperationType.ERROR));
			exception.setContinueThreadOfExecution(false);
			throw exception;
		}
	...
	/*
	 * Execution methods This will be done by every implementation
	 */
