...
public abstract class AbstractCreateExternalGraphReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    this.newDatabaseName =
        this.keyValue.get(ReactorKeysEnum.DATABASE.getKey()).trim().replaceAll("\\s+", "_");
    ...
    try {
      ...
      DIHelper.getInstance()
          .setEngineProperty(
              this.newDatabaseId + "_" + Constants.STORE, this.smssFile.getAbsolutePath());
    ...
    Map<String, Object> retMap =
        UploadUtilities.getEngineReturnData(this.insight.getUser(), newDatabaseId);
  ...
  private void generateNewDatabase(User user) throws Exception {
    ...
    try {
      UploadUtilities.validateEngine(
          IEngine.CATALOG_TYPE.DATABASE, user, this.newDatabaseName, this.newDatabaseId);
    } catch (IOException e) {
    ...
    this.databaseFolder =
        UploadUtilities.generateSpecificEngineFolder(
            IEngine.CATALOG_TYPE.DATABASE, this.newDatabaseId, this.newDatabaseName);
    ...
    if (!useLabel && nodeType == null) {
      SemossPixelException exception =
          new SemossPixelException(
              new NounMetadata(
                  "Requires graph type id to save.",
                  PixelDataType.CONST_STRING,
                  PixelOperationType.ERROR));
    ...
    if (nodeName == null) {
      SemossPixelException exception =
          new SemossPixelException(
              new NounMetadata(
                  "Requires graph name id to save.",
                  PixelDataType.CONST_STRING,
                  PixelOperationType.ERROR));
      exception.setContinueThreadOfExecution(false);
      throw exception;
    ...
    try {
      ...
      DIHelper.getInstance()
          .setEngineProperty(
              this.newDatabaseId + "_" + Constants.STORE, tempSmss.getAbsolutePath());
    } catch (IOException e) {
      classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(e.getMessage());
    }
