...
public class CommandReactor extends GitBaseReactor {
  ...
  private static final Set<String> APPROVED_PROD_COMMANDS =
      new HashSet<String>(Arrays.asList("PULL", "CLONE", "RESET", "STATUS"));
  ...
  public NounMetadata execute() {
    ...
    if (disable_terminal != null && !disable_terminal.isEmpty()) {
      if (Boolean.parseBoolean(disable_terminal)) {
        throw new IllegalArgumentException(
            "Terminal/Shell and user code execution has been disabled.");
      }
    }
    ...
    if (git != null
        ...
        && gitCommand != null
        && gitCommand.equalsIgnoreCase("push")) {
    ...
    if (git != null
      ...
      if (isCloneAllowed != null && !isCloneAllowed) {
        preCloneMessage =
            "You are cloning into a folder that is already part of git. Tracking at this level will be disabled";
      }
    }
    ...
    if (git != null
      ...
      postCloneMessage =
          "If this is a java project, please make sure to adjust the target directory (XML Element build/directory) to ${classesDir}";
    }
      ...
      if (gitCommand.equalsIgnoreCase("git") && push.equalsIgnoreCase("push")) {
        ...
        if (url != null) // need something to say these are Oauth2'able
        {
          ...
          return new NounMetadata(
              "Pushing Git", PixelDataType.CONST_STRING, PixelOperationType.HELP);
        }
      }
    }
    return null;
  ...
  private void postProcessClone(String command, String workingDir, boolean cloneAllowed) {
    ...
    if (commands.countTokens() >= 2) {
      ...
      if (gitCommand.equalsIgnoreCase("git") && gitOperation.equalsIgnoreCase("clone")) {
        ...
        if (cloneAllowed
            && appBaseFolder.endsWith("app_root")
            && new File(appBaseFolder + "/version").exists()
          ...
          try (FileOutputStream fos = new FileOutputStream(repoFile)) {
            ...
            this.insight
                .getCmdUtil()
                .executeCommand("git commit -m \"adding repos\" "); // dont know if we
            // need to add
            // the author
            // here else it
            // complains on
          ...
          File gitFolder =
              new File(
                  Utility.normalizePath(workingDir)
                      + File.separator
                      + dirName
                      + File.separator
                      + ".git");
