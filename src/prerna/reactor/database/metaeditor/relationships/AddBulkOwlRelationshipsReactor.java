...
public class AddBulkOwlRelationshipsReactor extends AbstractMetaEditorReactor {
	...
	public AddBulkOwlRelationshipsReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.DATABASE.getKey(), ReactorKeysEnum.FRAME.getKey(), PROP_MAX,
				STORE_VALUES_FRAME};
	}
	...
	public NounMetadata execute() {
		...
		noun.addAdditionalReturn(new NounMetadata("Successfully adding relationships", PixelDataType.CONST_STRING,
  ...
  ////////////////////////////////////////////////////////////////////////////
	...
	private void optimzieStore(RDataTable frame, double distance, RDataTable storeFrame, Logger logger) {
		...
		if (storeFrame.isEmpty()) {
			...
			rJavaTranslator.runR(defineVars + storeFrameName + "<-funion(" + positiveVarName + "," + negativeVarName
					+ ");rm(" + positiveVarName + "," + negativeVarName + ");");
			rJavaTranslator.runR("names(" + storeFrameName + ")<-" + RSyntaxHelper.createStringRColVec(
			...
			String storeFrameNameString = storeFrameName
					+ "[, c('sourceTable','sourceCol','targetTable','targetCol','action')]";
			String positiveVarNameString = positiveVarName
					+ "[, c('sourceTable','sourceCol','targetTable','targetCol','action')]";
			String negVarNameString = negativeVarName
					+ "[, c('sourceTable','sourceCol','targetTable','targetCol','action')]";
			rJavaTranslator.runR(defineVars + storeFrameName + "<-funion(" + storeFrameNameString + ","
					+ positiveVarNameString + ");" + storeFrameName + "<-funion(" + storeFrameNameString + ","
					+ negVarNameString + ");rm(" + positiveVarName + "," + negativeVarName + ");");
		}
	}

	/** Store the results by looping through the data */
	private void storeResults(ITableDataFrame frame, double distance, SelectQueryStruct qs, RDataTable storeFrame,
			Logger logger) {
