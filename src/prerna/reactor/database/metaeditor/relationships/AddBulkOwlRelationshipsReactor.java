...
public class AddBulkOwlRelationshipsReactor extends AbstractMetaEditorReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(AddBulkOwlRelationshipsReactor.class);
  ...
  public NounMetadata execute() {
    ...
    try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
      ...
      NounMetadata lComparison =
          new NounMetadata(new QueryColumnSelector("distance"), PixelDataType.COLUMN);
    ...
    noun.addAdditionalReturn(
        new NounMetadata(
            "Successfully adding relationships",
            PixelDataType.CONST_STRING,
   ...
   * @param logger
   */
  private void optimzieStore(
      RDataTable frame, double distance, RDataTable storeFrame, Logger logger) {
    ...
    if (storeFrame.isEmpty()) {
      ...
      String storeFrameNameString =
          storeFrameName + "[, c('sourceTable','sourceCol','targetTable','targetCol','action')]";
      String positiveVarNameString =
          positiveVarName + "[, c('sourceTable','sourceCol','targetTable','targetCol','action')]";
      String negVarNameString =
          negativeVarName + "[, c('sourceTable','sourceCol','targetTable','targetCol','action')]";
      rJavaTranslator.runR(
          defineVars
              + storeFrameName
              + "<-funion("
              + storeFrameNameString
              + ","
              + positiveVarNameString
              + ");"
              + storeFrameName
              + "<-funion("
              + storeFrameNameString
  ...
  private ITableDataFrame getFrame() {
    ...
    throw new IllegalArgumentException(
        "Need to define the frame which contains the inforamtion to bulk insert");
  }
}
