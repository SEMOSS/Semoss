...
public class RemoveOwlRelationshipReactor extends AbstractMetaEditorReactor {
	...
	private static final Logger classLogger = LogManager.getLogger(RemoveOwlRelationshipReactor.class);
	...
	public RemoveOwlRelationshipReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.DATABASE.getKey(), "startT", "endT", "startC", "endC",
				STORE_VALUES_FRAME};
	}
	...
	public NounMetadata execute() {
		...
		try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
			...
			for (int i = 0; i < size; i++) {
				...
				owlEngine.removeRelation(startT, endT, rel);
			}
			...
			try {
				...
				noun.addAdditionalReturn(
						new NounMetadata("An error occurred attempting to add the relationships defined",
