...
public class GetOwlDictionaryReactor extends AbstractMetaEditorReactor {
	...
	public NounMetadata execute() {
		...
		String query = "SELECT DISTINCT " + "?URI " + "(COALESCE(?DESCRIPTION, '') AS ?desc) "
				+ "(COALESCE(?LOGICAL_NAME, '') AS ?logical) " + "(COALESCE(?TYPE, 'STRING') AS ?type) " + "WHERE {"
				+ "{" + "{?URI <" + RDFS.SUBCLASSOF + "> <http://semoss.org/ontologies/Concept> } " + "OPTIONAL{?URI <"
				+ RDFS.COMMENT.toString() + "> ?DESCRIPTION} " + "OPTIONAL{?URI <" + OWL.sameAs.toString()
				+ "> ?LOGICAL_NAME} " + "OPTIONAL{?URI <" + RDFS.CLASS.toString() + "> ?TYPE} "
				+ "Filter(?URI != <http://semoss.org/ontologies/Concept>)"
				// currently not returning the table names in RDBMS/R/etc. data source
				// structures
				+ "MINUS{?URI <" + RDFS.DOMAIN.toString() + "> \"noData\" }" + "}" + "UNION " + "{" + "{?URI <"
				+ RDF.TYPE + "> <http://semoss.org/ontologies/Relation/Contains> } " + "OPTIONAL{?URI <"
				+ RDFS.COMMENT.toString() + "> ?DESCRIPTION} " + "OPTIONAL{?URI <" + OWL.sameAs.toString()
				+ "> ?LOGICAL_NAME} " + "OPTIONAL{?URI <" + RDFS.CLASS.toString() + "> ?TYPE} " + "}"
