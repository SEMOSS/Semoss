  ...
  public NounMetadata execute() {
    ...
    if (concept == null || concept.isEmpty()) {
      throw new IllegalArgumentException(
          "Must define the concept being added to the database metadata");
    }
    ...
    if ((column == null || column.isEmpty()) && database.getDatabaseType() == DATABASE_TYPE.RDBMS) {
      throw new IllegalArgumentException(
          "Must define the column for the concept being added to the database metadata");
    }
    String dataType = this.keyValue.get(this.keysToGet[3]);
    if (dataType == null || dataType.isEmpty()) {
      throw new IllegalArgumentException(
          "Must define the data type for the concept being added to the database metadata");
    }
    ...
    for (String conceptUri : concepts) {
      ...
      if (table.equalsIgnoreCase(concept)) {
        throw new IllegalArgumentException(
            "A concept already exists with this name. "
    ...
    try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
      ...
      try {
        ...
        noun.addAdditionalReturn(
            new NounMetadata(
                "An error occurred attempting to add the desired concept",
    ...
    noun.addAdditionalReturn(
        new NounMetadata(
            "Successfully added new concept",
            PixelDataType.CONST_STRING,
