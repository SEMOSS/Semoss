...
public class EditOwlConceptDataTypeReactor extends AbstractMetaEditorReactor {
	...
	public NounMetadata execute() {
		...
		try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
			...
			if (currentDataType != null) {
				owlEngine.removeFromBaseEngine(
						new Object[]{conceptPhysicalURI, RDFS.CLASS.stringValue(), currentDataType, true});
			}
			// add the new data type
			owlEngine.addToBaseEngine(
					new Object[]{conceptPhysicalURI, RDFS.CLASS.stringValue(), "TYPE:" + newDataType, true});
			...
			if (newAdditionalDataType != null && !newAdditionalDataType.isEmpty()) {
				...
				if (currentAdditionalDataType != null) {
					...
					owlEngine.removeFromBaseEngine(new Object[]{conceptPhysicalURI,
							AbstractOWLEngine.ADDITIONAL_DATATYPE_RELATION_URI, currentAdditionalDataType, false});
				}
				owlEngine.addToBaseEngine(new Object[]{conceptPhysicalURI,
		...
		noun.addAdditionalReturn(new NounMetadata("Successfully edited data type of " + concept + " to " + newDataType,
