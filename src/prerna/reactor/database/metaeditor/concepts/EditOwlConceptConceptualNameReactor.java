	...
	public NounMetadata execute() {
		...
		try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
			...
			for (String propertyPhysicalUri : properties) {
				String propertyPixelUri = database.getPropertyPixelUriFromPhysicalUri(conceptPhysicalURI,
						propertyPhysicalUri);
				String propPixelName = Utility.getClassName(propertyPixelUri);
				String newPropertyPixelUri = "http://semoss.org/ontologies/Relation/Contains/" + propPixelName + "/"
						+ newPixelName;

				// remove the current relationship
				owlEngine.removeFromBaseEngine(new Object[]{propertyPhysicalUri, AbstractOWLEngine.PIXEL_RELATION_URI,
						propertyPixelUri, true});
				// add the new relationship
				owlEngine.addToBaseEngine(new Object[]{propertyPhysicalUri, AbstractOWLEngine.PIXEL_RELATION_URI,
						newPropertyPixelUri, true});
			}

			// now update the actual concept
			// remove the current relationship
			owlEngine.removeFromBaseEngine(
					new Object[]{conceptPhysicalURI, AbstractOWLEngine.PIXEL_RELATION_URI, existingPixelURI, true});
			// add the new relationship
			owlEngine.addToBaseEngine(
					new Object[]{conceptPhysicalURI, AbstractOWLEngine.PIXEL_RELATION_URI, newPixelURI, true});
			...
			try {
				...
				noun.addAdditionalReturn(new NounMetadata("An error occurred attempting to commit modifications",
		...
		noun.addAdditionalReturn(
				new NounMetadata("Successfully edited concept name from " + concept + " to " + newPixelName,
