...
public class FindSemanticColumnOwlRelationshipsReactor extends AbstractMetaEditorReactor {
	 ...
	 * allTables <-
	 * c('city','city','city','city','city','countrylanguage','countrylanguage','countrylanguage','countrylanguage','country','country','country','country','country','country','country','country','country','country','country','country','country','country','country');
	 * allColumns <-
	 * c('ID','Name','CountryCode','District','Population','CountryCode','Language','IsOfficial','Percentage','Code','Name','Continent','Region','SurfaceArea','IndepYear','Population','LifeExpectancy','GNP','GNPOld','LocalName','GovernmentForm','HeadOfState','Capital','Code2');
	...
	public NounMetadata execute() {
		...
		for (int i = 0; i < size; i++) {
			...
			String description = database
					.getDescription("http://semoss.org/ontologies/Relation/Contains/" + cName + "/" + tName);
			if (description != null && !description.isEmpty()) {
				if (existingDefinitions == null) {
					existingDefinitions = new StringBuilder("list(");
					existingDefinitions.append("c(\"").append(tName).append("\",\"").append(cName)
							// escape quotes
		...
		if (existingDefinitions == null) {
			script.append(matchDataFrame).append("<- getDocumentCosineSimilarity(").append(allTablesVar).append(",")
					.append(allColumnsVar).append(");");
		} else {
			// close the list
			existingDefinitions.append(")");
			script.append(matchDataFrame).append("<- getDocumentCosineSimilarity(").append(allTablesVar).append(",")
					.append(allColumnsVar).append(",").append(existingDefinitions.toString()).append(");");
		}
		// execute!
