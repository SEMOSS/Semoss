...
public class PredictOwlDescriptionReactor extends AbstractMetaEditorReactor {
	 ...
	 * predictDescriptions(instanceValues_aIPO75a);
	 */
	...
	public NounMetadata execute() {
		...
		if (numDescriptions == 0) {
			throw new SemossPixelException(new NounMetadata("Unable to generate a description for the input column",
		...
		// "\\R\\OwlMatchRoutines\\OwlPredictDescriptions.R";
		// rScriptPath = rScriptPath.replace("\\", "/");
		// script.append("source(\"" + rScriptPath + "\");");
		//
		// String descriptionsFrameVar = "descriptionsFrame_" +
		...
		// rJavaTranslator.getStringArray(descriptionsFrameVar);
		//
		...
		// noun.addAdditionalReturn(new NounMetadata("An error occurred attempting to
		// add
		// descriptions",
		// PixelDataType.CONST_STRING, PixelOperationType.ERROR));
		// return noun;
		...
		noun.addAdditionalReturn(new NounMetadata("Predicted descriptions for review", PixelDataType.CONST_STRING,
