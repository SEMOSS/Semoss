...
public class PredictOwlLogicalNamesReactor extends AbstractMetaEditorReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(PredictOwlLogicalNamesReactor.class);
  private static final String CLASS_NAME = PredictOwlLogicalNamesReactor.class.getName();

  /**
   * Example script
   *
   * <p>source("C:/workspace/Semoss_Dev/R/OwlMatchRoutines/OwlPredictLogicalNames.R");
  ...
  public NounMetadata execute() {
    ...
    if (prop == null || prop.isEmpty()) {
      ...
      dataType =
          SemossDataType.convertStringToDataType(
              MasterDatabaseUtility.getBasicDataType(databaseId, qsName, null));
    } else {
      values.add(prop);
      qsName = concept + "__" + prop;
      dataType =
          SemossDataType.convertStringToDataType(
              MasterDatabaseUtility.getBasicDataType(databaseId, prop, concept));
    }
    ...
    if (topLogicalNames.isEmpty()) {
      throw new SemossPixelException(
          new NounMetadata(
              "Unable to generate logicla names for the input column",
    ...
    noun.addAdditionalReturn(
        new NounMetadata(
            "Predicted and logical names for review",
            PixelDataType.CONST_STRING,
