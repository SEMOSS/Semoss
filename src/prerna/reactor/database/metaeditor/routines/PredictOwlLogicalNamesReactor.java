...
public class PredictOwlLogicalNamesReactor extends AbstractMetaEditorReactor {
	 ...
	 * instanceValues_aIPO75a <- c("Classic
	 * Cars","Motorcycles","Planes","Ships","Trains"); descriptionsFrame_a9L7hr1 <-
	 * predictLogicalNames(instanceValues_aIPO75a);
	 */
	...
	public NounMetadata execute() {
		...
		// this.insight.getRJavaTranslator(logger);
		...
		if (prop == null || prop.isEmpty()) {
			values.add(concept);
			qsName = concept;
			dataType = SemossDataType
					.convertStringToDataType(MasterDatabaseUtility.getBasicDataType(databaseId, qsName, null));
		} else {
			values.add(prop);
			qsName = concept + "__" + prop;
			dataType = SemossDataType
					.convertStringToDataType(MasterDatabaseUtility.getBasicDataType(databaseId, prop, concept));
		}
		...
		if (topLogicalNames.isEmpty()) {
			throw new SemossPixelException(new NounMetadata("Unable to generate logicla names for the input column",
					PixelDataType.CONST_STRING, PixelOperationType.ERROR));
		}

		// StringBuilder script = new StringBuilder();
		// // first source the file where we have the main method for running
		// String rScriptPath = getBaseFolder() +
		// "\\R\\OwlMatchRoutines\\OwlPredictLogicalNames.R";
		...
		// script.append(logicalNamesVar).append(" <-
		// predictLogicalNames(").append(instanceVectorVar).append(");");
		//
		// // execute!
		...
		noun.addAdditionalReturn(new NounMetadata("Predicted and logical names for review", PixelDataType.CONST_STRING,
