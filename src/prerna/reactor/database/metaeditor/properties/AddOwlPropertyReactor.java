...
public class AddOwlPropertyReactor extends AbstractMetaEditorReactor {
	...
	public NounMetadata execute() {
		...
		// throw new IllegalArgumentException("Conceptual name must contain only
		// letters, numbers,
		// and underscores");
		// }
		...
		if (conceptPhysicalUri == null) {
			throw new IllegalArgumentException(
					"Could not find the concept. Please define the concept first before adding properties");
		}
		...
		try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
			...
			// String colName = null;
			// if(engine.getEngineType() == ENGINE_TYPE.RDBMS || engine.getEngineType() ==
			// ENGINE_TYPE.IMPALA) {
			...
			try {
				...
				noun.addAdditionalReturn(new NounMetadata("An error occurred attempting to add the desired property",
		...
		noun.addAdditionalReturn(new NounMetadata("Successfully added new property", PixelDataType.CONST_STRING,
