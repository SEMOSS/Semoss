...
public class EditOwlPropertyDataTypeReactor extends AbstractMetaEditorReactor {
	...
	public NounMetadata execute() {
		...
		try (WriteOWLEngine owlEngine = database.getOWLEngineFactory().getWriteOWL()) {
			...
			if (propertyPhysicalURI == null) {
				throw new IllegalArgumentException(
						"Could not find the property. Please define the property first before modifying the conceptual name");
			}
			...
			if (currentDataType != null) {
				owlEngine.removeFromBaseEngine(
						new Object[]{propertyPhysicalURI, RDFS.CLASS.stringValue(), currentDataType, true});
			}
			owlEngine.addToBaseEngine(
					new Object[]{propertyPhysicalURI, RDFS.CLASS.stringValue(), "TYPE:" + newDataType, true});
			...
			if (newAdditionalDataType != null && !newAdditionalDataType.isEmpty()) {
				...
				if (currentAdditionalDataType != null) {
					...
					owlEngine.removeFromBaseEngine(new Object[]{propertyPhysicalURI,
							AbstractOWLEngine.ADDITIONAL_DATATYPE_RELATION_URI, currentAdditionalDataType, false});
				}
				owlEngine.addToBaseEngine(new Object[]{propertyPhysicalURI,
						AbstractOWLEngine.ADDITIONAL_DATATYPE_RELATION_URI, adtlTypeObject, false});
			}
		...
		noun.addAdditionalReturn(new NounMetadata("Successfully edited data type of " + property + " to " + newDataType,
