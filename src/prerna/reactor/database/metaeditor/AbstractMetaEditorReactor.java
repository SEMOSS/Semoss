...
public abstract class AbstractMetaEditorReactor extends AbstractReactor {
  ...
  static {
    ...
    literalPreds.add(
        AbstractOWLEngine.SEMOSS_URI_PREFIX + AbstractOWLEngine.DEFAULT_PROP_CLASS + "/UNIQUE");
  ...
  /**
   * Get an array of lists The first list contains the tables The second list contains the column
   * But the first list table will repeat for each column so that they match based on index
   */
  protected List<String>[] getTablesAndColumnsList(
      IDatabaseEngine database, List<String> tableFilters) {
    // store 2 lists
    // of all table names
    // and column names
    // matched by index
  ...
  protected SelectQueryStruct getMostOccuringSingleColumnNonEmptyQs(String qsName, int limit) {
...

    {
      NounMetadata lComparison =
          new NounMetadata(new QueryColumnSelector(qsName), PixelDataType.COLUMN);
   ...
   * @param endCList
   */
  protected void storeUserInputs(
      Logger logger,
      List<String> startTList,
      List<String> startCList,
      List<String> endTList,
      List<String> endCList,
      String action) {
   ...
   * @param randomVar
   */
  private void execQueryStore(
      RDataTable storeFrame, IRJavaTranslator rJavaTranslator, StringBuilder newValuesBuilder) {
   ...
   * @param logger
   */
  protected void removeStoredValues(
      String resultsFrame, Object[] storeTypesToRemove, Logger logger) {
    RDataTable storeFrame = getStore();
    boolean storeResults = (storeFrame != null);
    if (storeResults) {
  ...
  protected static List<String> getTopNResults(List<String> results, int n) {
    ...
    List<String> sortedTopN =
        topN.entrySet().stream()
