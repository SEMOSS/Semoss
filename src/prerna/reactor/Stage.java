...
public class Stage extends Hashtable<String, Hashtable> {
  ...
  StringBuffer queryStructString =
      new StringBuffer(
  ...
  public void synchronizeInput(String operationName) {
    ...
    Hashtable<String, String> inputTypes =
        (Hashtable<String, String>) codeHash.get(Stage.INPUT_TYPE);
    String inputString = "{";
    ...
    for (int inputIndex = 0; inputIndex < inputs.size(); inputIndex++) {
      if (inputIndex == 0) inputString = inputString + "\"" + inputs.elementAt(inputIndex) + "\"";
      else inputString = inputString + ", \"" + inputs.elementAt(inputIndex) + "\"";
    ...
    // adds the property
    // and sets the frame
    if (codeHash.containsKey("FRAME")) stageStore.put("FRAME", codeHash.get("FRAME"));
    ...
    */
    ...
    if (inputString.equalsIgnoreCase("{};")) inputString = "";
  ...
  public Hashtable addOperation(String operationName) {
    if (containsKey(operationName)) return getOperation(operationName);
    else {
  ...
  public void setDependency(String fromOperationName, String toOperationName) {
    ...
    if (depNum > fromIndex) lowestDependency.put(toOperationName, fromIndex);
  }
    ...
    if (stageInputs.indexOf(input) < 0) {
      ...
      if (column.length == 1)
        ...
        queryStructString.append(
            "qs.addSelector(\"" + column[0] + "\",  \"" + column[1] + "\");\n");
    }
  }
    ...
    if (values.size() > 0) {
      ...
      if (type == PixelDataType.CONST_STRING) {
        ...
        for (int valIndex = 1; valIndex < values.size(); valIndex++)
          filterVectorString.append(
              "strVector.addElement(" + pad + values.get(valIndex) + pad + ");\n");
        filterVectorString.append(
            "qs.addFilter(\""
                + filter.getLComparison().get(0).toString()
                + "\", \""
        ...
        for (int valIndex = 1; valIndex < values.size(); valIndex++) {
          ...
          filterVectorString.append(
              "decVector.addElement(new Double(" + values.get(valIndex) + "));\n");
        }
        filterVectorString.append(
            "qs.addFilter(\""
                + filter.getLComparison().get(0).toString()
    ...
    for (int opIndex = 0; opIndex < operationSequence.size(); opIndex++) {
      ...
      if (containsKey(thisOperation)) {
        ...
        if (codeHash.containsKey("REACTOR")) {
          executeBlock.append(
              "thisReactor = (IReactor)store.get(" + codeHash.get("SIGNATURE") + ");\n");
  ...
  public Vector<String> getOperationsInStage() {
    if (opVector == null) {
      ...
      while (keys.hasMoreElements()) retVector.addElement(keys.nextElement());
    }
    return opVector;
  }
}
