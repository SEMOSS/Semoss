...
public class Stage extends Hashtable<String, Hashtable> {
	...
	StringBuffer queryStructString = new StringBuffer(
	...
	// should keep another hash of lowest dependency just in case things come in
	// later
	...
	public void synchronizeInput(String operationName) {
		...
		for (int inputIndex = 0; inputIndex < inputs.size(); inputIndex++) {
			...
			if (inputTypes.containsKey(inputs.elementAt(inputIndex))
					&& inputTypes.get(inputs.elementAt(inputIndex)).equalsIgnoreCase("QUERY"))
		...
		/*
		 * for(int filterIndex = 0;filters != null && filterIndex <
		 * filters.size();filterIndex++) { Object thisFilter =
		 * filters.elementAt(filterIndex); if(thisFilter instanceof Join)
		 * System.out.println("Instance of join.. "); else
		 ...
		 * { Object thisJoin = joins.elementAt(joinIndex); if(thisJoin instanceof Join)
		 * System.out.println("Instance of join.. "); else
		 * System.out.println("Weird.. ");;
		 * 
		...
		/*
		 * // need to add the derived inputs here as well inputs =
		 * (Vector<String>)codeHash.get(this.DERIVED_INPUTS); for(int inputIndex =
		 * 0;inputIndex < inputs.size();inputIndex++) { if(inputIndex == 0) inputString
		 * = "\"" + inputs.elementAt(inputIndex) + "\""; else inputString = inputString
		 * + ", \"" + inputs.elementAt(inputIndex) + "\"";
	...
	// need to ask maher.. if I need to do the same for the filters i.e. add
	...
	public void addStageFilter(Filter filter) {
		...
		if (values.size() > 0) {
			...
			if (type == PixelDataType.CONST_STRING) {
				...
				filterVectorString.append("qs.addFilter(\"" + filter.getLComparison().get(0).toString() + "\", \""
						+ filter.getComparator() + "\" ,strVector);\n");
			} else {
				...
				filterVectorString.append("qs.addFilter(\"" + filter.getLComparison().get(0).toString() + "\", \""
						+ filter.getComparator() + "\",decVector);\n");
			}
	...
	public void addStageRelation(Join filter) {
		...
		relationString.append("qs.addRelation(\"" + filter.getLColumn() + "\", \"" + filter.getRColumn() + "\" ,\""
				+ filter.getJoinType() + "\");\n");
	...
	public String getCode() {
		...
		StringBuffer executeBlock = new StringBuffer(
				"public IHeadersDataRow executeCode(IHeadersDataRow curRow) \n{\n IReactor thisReactor = null;");
		...
		// no longer valid
		/*
		 * // print all the inputs as a query String selectors = ""; // Fill the query
		 * Struct
		 * 
		 * 
		 * for(int inputIndex =0;inputIndex < stageInputs.size();inputIndex++) {
		 * if(selectors.length() == 0) selectors = stageInputs.elementAt(inputIndex);
		 * else selectors = selectors + "," + stageInputs.elementAt(inputIndex); }
		 * 
		 * retString = retString.append("\n"); retString =
		 * retString.append("// QUERY SELECTORS NEEDED for this STAGE " + selectors +
		...
		// thisClass.addMethod(queryStructString.toString()); <-- I dont need this
		// anymore
		...
		runner.test();
		// thisClass.writeClass("C:\\Users\\pkapaleeswaran\\workspacej3\\SemossDev\\Codagen\\Stage"
		// +
		// stageNum + ".java");
