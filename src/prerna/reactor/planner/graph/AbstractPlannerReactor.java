...
public abstract class AbstractPlannerReactor extends AbstractReactor {
  ...
  /**
   * Runs through and get all downstream vertices based on the execution order and will continue to
   * traverse down and get downstream vertices that it can execute
   *
   * @param vertsToRun The starting set of vertices to execute
   * @param downstreamVertIds
   */
  protected void getAllDownstreamVertsBasedOnTraverseOrder(
      Set<Vertex> vertsToRun, List<String> downstreamVertIds) {
   ...
   * @return
   */
  protected List<String> orderVertsAndGetPksls(
      PixelPlanner planner, List<String> downstreamVertIds) {
    List<String> returnPksls = new Vector<String>();

    GraphTraversal<Vertex, Vertex> newRootsTraversal =
        planner
            .g
            .traversal()
            .V()
            .has(PixelPlanner.TINKER_ID, P.within(downstreamVertIds))
            .order()
            .by(PixelPlanner.ORDER, Order.asc);
  ...
  protected void addOrderToNonExistentVerts(PixelPlanner planner) {
    GraphTraversal<Vertex, Vertex> newRootsTraversal =
        planner.g.traversal().V().hasNot(PixelPlanner.ORDER);
    while (newRootsTraversal.hasNext()) {
      Vertex vert = newRootsTraversal.next();
  ...
  protected void addOpToMainMap(PixelPlanner planner, Vertex opVertex) {
    HashMap<String, String> mainMap =
        (HashMap<String, String>) planner.getProperty("MAIN_MAP", "MAIN_MAP");
    Map<String, String> baseMap = null;
    if (planner.hasProperty("BASE_MAP", "BASE_MAP")) {
      baseMap = (HashMap<String, String>) planner.getProperty("BASE_MAP", "BASE_MAP");
    }
  ...
  protected Set<Vertex> getRootPksls(PixelPlanner planner) {
    ...
    GraphTraversal<Vertex, Long> getAllRootOpsCount =
        planner.g.traversal().V().has(PixelPlanner.TINKER_TYPE, PixelPlanner.OPERATION).count();
    ...
    GraphTraversal<Vertex, Vertex> getAllRootOps =
        planner.g.traversal().V().has(PixelPlanner.TINKER_TYPE, PixelPlanner.OPERATION);
    ...
    while (getAllRootOps.hasNext()) {
      ...
      if (isRoot) {
        ...
        pkslOperation =
            getPksl(
                nextOp); // nextOp.property(PKSLPlanner.TINKER_ID).value().toString().substring(3)
        // + ";";

        // we ignore this stupid thing....
        if (!pkslOperation.isEmpty()
            && !pkslOperation.equals("FRAME")
            && !pkslOperation.equals("MAIN_MAP")) {
  ...
  /**
   * Given a set of roots Find the downstream nouns Within the planner, find the ops that have the
   * downstream nouns as inputs Return those as the new roots
   *
   * @param roots
   * @param planner
   * @return
      ...
      while (outNounsIt.hasNext()) {
        ...
        GraphTraversal<Vertex, Vertex> newRootsTraversal =
            planner
                ...
                .traversal()
                .V()
                .has(PixelPlanner.TINKER_ID, outNounId)
                .out()
                .has(PixelPlanner.PROCESSED, false) // make
                // sure
                // it
                // hasn't
                // been
  ...
  /**
   * Reset the processed boolean to be false This is required to properly determine which nodes/how
  ...
  protected void resetProcessedBoolean(PixelPlanner planner) {
    // grab all op nodes
    GraphTraversal<Vertex, Vertex> allOpsIt =
        planner.g.traversal().V().has(PixelPlanner.TINKER_TYPE, PixelPlanner.OPERATION);
  ...
  protected String getPksl(Vertex vert) {
    String pkslOperation =
        vert.property(PixelPlanner.TINKER_ID).value().toString().substring(3) + ";";
