...
public class RunPlanReactor extends AbstractPlannerReactor {
  ...
  public NounMetadata execute() {
    ...
    LOGGER.info(
        "****************    Build Super Clases with Fields "
            + (endTime - startTime)
            + "ms      *************************");
  ...
  private Class<BaseJavaRuntime> buildSuperClassWithOnlyFields(
      List<String> fieldsList, Class<BaseJavaRuntime> superClass) {
  ...
  private List<String> buildFields(
      Map<String, String> mainMap, PixelPlanner planner, RuntimeJavaClassBuilder builder) {
  ...
  /**
   * We want to override the pksl that is extracted from the vertex so that we grab the java string
   * instead of the pksl string
   */
  @Override
