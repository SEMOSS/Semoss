...
public class GraphPlanReactor extends AbstractReactor {
  ...
  private Map createVertStores2(PixelPlanner planner) {
    ...
    GraphTraversal<Edge, Edge> edgesIt =
        planner
            .g
            .traversal()
            .E(); // .not(__.or(__.has(TINKER_TYPE, TINKER_FILTER), __.bothV().in().has(TINKER_TYPE,
    // TINKER_FILTER), __.V().has(PRIM_KEY, true)));
    ...
    GraphTraversal<Vertex, Vertex> vertIt =
        planner
            .g
            .traversal()
            .V(); // .not(__.or(__.has(TINKER_TYPE, TINKER_FILTER), __.in().has(TINKER_TYPE,
    // TINKER_FILTER), __.has(PRIM_KEY, true)));
  ...
  private SEMOSSVertex getSEMOSSVertex(Map<String, SEMOSSVertex> vertStore, Vertex tinkerVert) {
    ...
    if (semossVert == null) {
      ...
      //				String propName = prop.key();
      //				if(!propName.equals(TINKER_ID) && !propName.equals(TINKER_NAME) &&
      // !propName.equals(TINKER_TYPE)) {
