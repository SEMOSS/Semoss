...
public class GraphPlanReactor extends AbstractReactor {
	...
	private Map createVertStores2(PixelPlanner planner) {
		...
		// get all edges
		GraphTraversal<Edge, Edge> edgesIt = planner.g.traversal().E(); // .not(__.or(__.has(TINKER_TYPE,
																		// TINKER_FILTER),
																		// __.bothV().in().has(TINKER_TYPE,
		...
		GraphTraversal<Vertex, Vertex> vertIt = planner.g.traversal().V(); // .not(__.or(__.has(TINKER_TYPE,
																			// TINKER_FILTER), __.in().has(TINKER_TYPE,
