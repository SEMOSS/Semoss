...
public abstract class AbstractLoadClient extends AbstractReactor {
  ...
  /**
   * @return This abtract method is defined by the child class to have control as to what type of
   *     planner to create, all other operations are the same
   */
   ...
   * @param assignment
   * @param value
   *     <p>This method will directly add the variable to the planner instead of adding an
   *     assignment pksl
   */
   ...
   * @param assignment
   * @param value
   * @return returns a pksl query given an assignment variable and the value
   */
  ...
  protected Iterator getIterator() throws Exception {
    ...
    BasicIteratorTask task =
        (BasicIteratorTask) this.store.getNoun(PixelDataType.TASK.getKey()).get(0);
    return task.getIterator();
  }
  ...
  protected int[] getAssignmentIndices(String[] iteratorHeaders) {
    ...
    for (int index = 0; index < numAssignmentCols; index++) {
      ...
      assignmentIndices[index] =
          ArrayUtilityMethods.arrayContainsValueAtIndex(iteratorHeaders, assignmentName);
    }
  ...
  protected int getTypeIndex(String[] iteratorHeaders) {
    ...
    int typeIndex =
        ArrayUtilityMethods.arrayContainsValueAtIndexIgnoreCase(iteratorHeaders, typeName);
    return typeIndex;
  }
