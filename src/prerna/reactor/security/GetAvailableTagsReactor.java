        ...
        if (!projectFilters.contains(inputAppFilter)) {
          warningNouns.add(
              NounMetadata.getWarningNounMessage(
                  inputAppFilter + " does not exist or user does not have access to project."));
        } else {
    ...
    if (appliedProjectFilters != null && appliedProjectFilters.isEmpty()) {
      if (inputFilters.isEmpty()) {
        ...
        return NounMetadata.getErrorNounMessage(
            "Input project filters do not exist or user does not have access to the projects");
      }
    }

    List<Map<String, Object>> ret =
        SecurityInsightUtils.getAvailableInsightTagsAndCounts(appliedProjectFilters);
