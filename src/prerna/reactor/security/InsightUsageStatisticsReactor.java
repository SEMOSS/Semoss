...
public class InsightUsageStatisticsReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		// retNouns.add(new NounMetadata(newFrame, PixelDataType.FRAME,
		...
		// BasicIteratorTask task = new BasicIteratorTask(loadedDataQs,
		// loadedDataIterator);
		//
		// if(panelId != null) {
		// Map<String, Object> optMap = task.getFormatter().getOptionsMap();
		// TaskOptions tOptions = AudoTaskOptionsHelper.getAutoOptions(qs, panelId,
		// "GRID", optMap);
		// if(tOptions != null) {
		...
		// } catch (Exception e) {
		// classLogger.error(Constants.STACKTRACE, e);
		// throw new IllegalArgumentException("There was an error in querying the data
		// frame with the
		...
		// noun.addAdditionalReturn(getSuccess("Successfully generated new frame with
		// insight usage
		// statistics"));
		// return noun;
	}
