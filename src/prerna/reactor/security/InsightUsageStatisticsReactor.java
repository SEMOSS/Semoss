...
public class InsightUsageStatisticsReactor extends AbstractReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(InsightUsageStatisticsReactor.class);
  ...
  public NounMetadata execute() {
    ...
    if (projectGrsFilters != null && !projectGrsFilters.isEmpty()) {
      ...
      for (int i = 0; i < projectGrsFilters.size(); i++) {
        ...
        engineFilter =
            SecurityProjectUtils.testUserProjectIdForAlias(this.insight.getUser(), engineFilter);
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error occurred trying to create frame of the default type", e);
    }
    ...
    SelectQueryStruct qs =
        SecurityInsightUtils.searchUserInsightsUsage(
            this.insight.getUser(), pFilters, searchTerm, tagFilters);
    ;
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "There was an error in executing the retrieving and loading the insight query statistics",
          e);
    } finally {
      if (wrapper != null) {
        try {
    ...
    //
    //			retNouns.add(new NounMetadata(task, PixelDataType.FORMATTED_DATA_SET,
    // PixelOperationType.TASK_DATA));
    //		} catch (Exception e) {
    //			classLogger.error(Constants.STACKTRACE, e);
    //			throw new IllegalArgumentException("There was an error in querying the data frame with the
    // loaded insight query statistics", e);
    //		}
    //
    //		NounMetadata noun = new NounMetadata(retNouns, PixelDataType.VECTOR,
    // PixelOperationType.VECTOR);
    //		noun.addAdditionalReturn(getSuccess("Successfully generated new frame with insight usage
    // statistics"));
    //		return noun;
  }
