...
public class GetDatabaseMetadataReactor extends AbstractReactor {
  ...
  public GetDatabaseMetadataReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.DATABASE.getKey(), ReactorKeysEnum.META_KEYS.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    databaseInfo.putAll(
        SecurityEngineUtils.getAggregateEngineMetadata(databaseId, getMetaKeys(), false));
    // append last engine update
    databaseInfo.put("last_updated", MasterDatabaseUtility.getEngineDate(databaseId));
    return new NounMetadata(
        databaseInfo, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.DATABASE_INFO);
  }
