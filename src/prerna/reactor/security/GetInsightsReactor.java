...
public class GetInsightsReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
			...
			for (int i = 0; i < projectFilterGrs.size(); i++) {
				...
				if (SecurityProjectUtils.userCanViewProject(this.insight.getUser(), pFilter)) {
					...
					warningNouns.add(NounMetadata.getWarningNounMessage(
							pFilter + " does not exist or user does not have access to project."));
				}
			}
		}
		...
		List<Map<String, Object>> results = SecurityInsightUtils.searchUserInsights(this.insight.getUser(),
				projectFilters, searchTerm, favoritesOnly, sortBy, insightMetadataFilter, limit, offset);
		...
		if (!noMeta) {
			...
			// grab the wrapper for all the meta information for all the project + insights
			// in one go
			if (!projectIdsToInsight.isEmpty()) {
				IRawSelectWrapper wrapper = SecurityInsightUtils.getInsightMetadataWrapper(projectIdsToInsight,
						META_KEYS_LIST);
		...
		NounMetadata retNoun = new NounMetadata(results, PixelDataType.CUSTOM_DATA_STRUCTURE,
				PixelOperationType.APP_INSIGHTS);
