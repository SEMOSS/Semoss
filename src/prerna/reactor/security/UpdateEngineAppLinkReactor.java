...
public class UpdateEngineAppLinkReactor extends AbstractReactor {
  ...
  public UpdateEngineAppLinkReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.ENGINE.getKey(), ReactorKeysEnum.PROJECT.getKey()};
  }
      ...
      try {
        ...
        noun.addAdditionalReturn(
            NounMetadata.getSuccessNounMessage(
                "Successfully set the new tool_app for the engine " + engineId));
      } catch (Exception e) {
      ...
      noun.addAdditionalReturn(
          NounMetadata.getErrorNounMessage(
              "Access Denied to user: "
                  + userName
                  + " - Only Admin or Author can update tool_app"));
    }
    return noun;
  }
  ...
  private void hasAccessToAppAndEngine(User user, String engineId, String projectId) {
    if (!canAccessEngine(user, engineId)) {
      throw new IllegalArgumentException(
          "Engine does not exist or user does not have access to the project");
    }
  ...
  private boolean isUserAuthor(User user, String engineId) {
    ...
    Map<String, Object> permissionMap =
        SecurityQueryUtils.getEnginePermission(userDetails.getValue0(), engineId);
