  ...
  public NounMetadata execute() {
    ...
    if (insightSheets.containsKey(possibleSheetId)) {
      return new NounMetadata(
          possibleSheetId, PixelDataType.CONST_STRING, PixelOperationType.MOVE_SHEET);
    }
      ...
      if (sheetLabels.contains(sheetLabel)) {
        if (sheetLabel.equals(possibleSheetId)) {
          ...
          throw new IllegalArgumentException(
              "Please make sure the sheets have unique identifiers.");
        }
      } else {
    ...
    if (actualSheetId != null) {
      return new NounMetadata(
          actualSheetId, PixelDataType.CONST_STRING, PixelOperationType.MOVE_SHEET);
    }

    throw new IllegalArgumentException(
        "Count not find sheet with id or label = " + possibleSheetId);
  }
}
