...
public class MgmtUtil {
  ...
  private static final String Temp_FOLDER =
      (DIHelper.getInstance().getProperty(Constants.BASE_FOLDER) + "/" + "R" + "/" + "Temp" + "/")
          .replace('\\', '/');

  public static void diskUtilizationPerProcess(int pid) {
    /**
     * ByteRead : Returns the number of bytes the process has read from disk. ByteWritten : Returns
  ...
  public static void cpuUtilizationPerProcess(int processId) {
    /**
     * User Time : Returns the number of milliseconds the process has executed in user mode. Kernel
     * Time : Returns the number of milliseconds the process has executed in kernel/system mode.
     */
  ...
  public static void printChild(int pid) {
    ...
    for (int childIndex = 0; childIndex < childProcesses.size(); childIndex++) {
      logger.info(
          "Process id "
              + childProcesses.get(childIndex).getProcessID()
              + " <> "
  ...
  public static long getFreeMemory() {
    if (freeMemory.longValue() == -1) {
      ...
      if (reservedJavaMem != null && !(reservedJavaMem = reservedJavaMem.trim()).isEmpty()) {
        ...
        logger.info(
            "Total available memory minus RESERVED_JAVA_MEM = " + freeMemory.longValue() + " GB");
      }
    }
