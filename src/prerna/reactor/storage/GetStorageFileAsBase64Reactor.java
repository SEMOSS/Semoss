...
public class GetStorageFileAsBase64Reactor extends AbstractReactor {
     ...
     * Returns original bytes unchanged for PDF, TXT, PPTX (skip), or unknown types.
     */
    ...
    /**
     * Converts DOCX bytes to an HTML string using Apache POI XWPFDocument.
     * Handles paragraphs (with heading styles), inline formatting (bold/italic/underline),
     * and tables.
     */
    private String docxToHtml(byte[] docxBytes) throws Exception {
        try (ByteArrayInputStream bis = new ByteArrayInputStream(docxBytes);
            ...
            html.append("<!DOCTYPE html><html><head>")
                ...
                .append("h1,h2,h3,h4,h5,h6 { margin: 16px 0 8px 0; }")
    ...
    /**
     * Converts a legacy binary .doc file to HTML using Apache POI WordToHtmlConverter.
    ...
    /**
     * Converts XLSX bytes to an HTML string with one table per sheet.
     * Uses DataFormatter so formulas and numbers render as their displayed values.
     */
    private String xlsxToHtml(byte[] xlsxBytes) throws Exception {
        try (ByteArrayInputStream bis = new ByteArrayInputStream(xlsxBytes);
            ...
            html.append("<!DOCTYPE html><html><head>")
                ...
                .append("th { background-color: #e0e0e0; font-weight: bold; }")
                .append("</style></head><body>");
    ...
    private String paragraphToHtml(XWPFParagraph para) {
        ...
        if (styleId != null) {
            if (styleId.startsWith("Heading1") || styleId.equals("1")) return "<h1>" + text + "</h1>";
            if (styleId.startsWith("Heading2") || styleId.equals("2")) return "<h2>" + text + "</h2>";
            if (styleId.startsWith("Heading3") || styleId.equals("3")) return "<h3>" + text + "</h3>";
            if (styleId.startsWith("Heading4") || styleId.equals("4")) return "<h4>" + text + "</h4>";
        }

        // Alignment
        String align = "";
        ParagraphAlignment alignment = para.getAlignment();
        if (alignment == ParagraphAlignment.CENTER) align = " style='text-align:center'";
        else if (alignment == ParagraphAlignment.RIGHT) align = " style='text-align:right'";
        else if (alignment == ParagraphAlignment.BOTH)  align = " style='text-align:justify'";
