...
public class FrameToGraphReactor extends AbstractRFrameReactor {
    ...
    public NounMetadata execute() {
        ...
        // String assetsDir =
        // AssetUtility.getProjectAssetFolder(projectID).replace("\\", "/");

        String userInput = "";
        try {
            userInput = URLDecoder.decode((String) this.keyValue.get(this.keysToGet[2]),
        ...
        String dataString = "";
        try {
            dataString = URLDecoder.decode((String) this.keyValue.get(this.keysToGet[4]),
                    StandardCharsets.UTF_8.toString());
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        if (userInput != null) {
            CONTEXT += " \n"
                    ...
                    // + "- If the user requests showing or comparing only specific data columns,
                    // data types, or subsets of the data, ensure the final graph reflects exactly
                    // those selections.\n"
                    // + "- Interpret any specific user instructions carefully. For example, if the
                    // user says: �Show me a graph comparing column A and column B only,� or �Pl
                    // 
                    //
                    //
                    // t a bar chart showing sales over time with blue tones,� incorporate these i
                    // structions fully.\n"
        ...
        PROMPT += "Please generate a valid Vega-Lite chart specification in JSON format that accurately and clearly visualizes the given data by selecting the graph template that best fits the data and user's needs.\"\n"
                + " Your task is to add the data to the values section of the JSON Spec. Additionally, add values to the placeholder values.  "
                + " \n"
                // + " Your output must include:\n"
                // + " 1. \"**The complete Vega-Lite JSON spec** only � do not include any e
                // planation, commentary, irregular quotation marks in data values, or code b
                // ocks.\"\n"
                ...
                // + " \"- Optional: tooltips, color, and other enhancements to improve
                // clarity\"\n"
        ...
        // if (userInput != null) {
        // PROMPT += "5. \"The most meaningful and suprising patterns, insights, or
        // anomalies possible in the dataset.\"\n";
        // }
        PROMPT += "            Guidelines:\n"
                + "            \"- Avoid complex transforms unless specified in the user's prompt\"\n"
                // + " \"- Choose the chart type from the appropriate chart family (temporal,
                // categorical, hierarchical, relational, spatial) that best fits the data and
                // user instructions.\"\n"
                + "            \"- Add axis titles based on the field names.\"\n"
                + " \n"
                + " Here are the templates you can choose from: \n"
                + getVegaBarChartTemplate() + "\n"
                + getVegaStackedBarChartTemplate() + "\n"
        ...
        ///////// MODEL ///////////
        String QUESTION = "Here is the relevant data. Include only the relevant data in the JSON. "
                + dataString + PROMPT
                + ". Remember ONLY return the Vega JSON. Do not include any text, explanation, markdown, or notes.\\n\"";
        // String QUESTION = PROMPT + "Here is the relevant data that needs mapping,
        // Include only the relevant data in the JSON. " + dataString ;
        ...
        if (modelResponse != null) {
            ...
            // Process the response as needed (e.g., parse steps, adjust graph
            // configuration, etc.)
        } else {
    ...
    private String buildVegaPrompt(ITableDataFrame sourceFrame) {
        ...
        // TODO: Make this work for frames greater than size 20. Test how many rows we
        // can insert into prompt.
        int sampleRows = Math.min(rowCount, 20);

        // Partition headers into numerical and categorical based on the first non-null
        // cell in row 0
        ...
        // Filter headers by data type (Numerical, Categorical, or Temporal).
        // Categorical is default
        ...
        // \"").append(categoricalRowData[r]).append("\"");
        ...
        for (int r = 0; r < sampleRows; r++) {
            ...
            for (int i = 0; i < headers.length; i++) {
                ...
                if (cell == null) {
                    ...
                    promptBuilder.append("\"").append(header).append("\": \"")
                            .append(cell.toString().replace("\"", "'")).append("\"");
                }
        ...
        // Minimal encoding: for demonstration use first categorical as x and first
        // numerical as y (if available)
    ...
    /*
     * Determines whether the row data in a column are categorical, numerical, or
     * temporal
     */
    ...
    private String getVegaPieChartTemplate() {
        return " \n" +
                ...
                "      \"encode\": {\n" +
                "        \"enter\": {\n" +
                "          \"fill\": {\"scale\": \"color\", \"field\": \"placeholder\"},\n" +
                "          \"x\": {\"signal\": \"width / 2\"},\n" +
                "          \"y\": {\"signal\": \"height / 2\"},\n" +
                "          \"tooltip\": {\"signal\": \"'placeholder: ' + datum.placeholder + 'placeholder: ' + datum.placeholder\"}\n"
                +
                "        },\n" +
                "        \"update\": {\n" +
    ...
    private String getVegaStackedBarChartTemplate() {
        return " \n" +
