	...
	public NounMetadata execute() {
		...
		if (overrideColumn) {
			...
			try {
				...
				classLogger.error("Failed to extract numeric characters in-place for columns {} on table {}", columns,
						table, e);
			}
		}
		// create new columns
			...
			for (int i = 0; i < columns.size(); i++) {
				...
				try {
					...
					classLogger.error("Failed to extract numeric characters from column {} into {} on table {}", column,
							newColumn, table, e);
				}
