...
public abstract class AbstractFilterReactor extends AbstractFrameReactor {
			...
			if (addFilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
				...
				for (int filterIndex = 0; filterIndex < existingFilters.getFilters().size(); filterIndex++) {
					...
					if (currentFilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
						...
						IQueryFilter requireNewFilter = processSimpleQueryFilter(simpleAdd, addFilterIdx, curFilter,
								filterIndex, indicesToRemove, addFiltersToIgnore, indicesRemovedByDirectConflict);
	 ...
	 */
	protected IQueryFilter processSimpleQueryFilter(SimpleQueryFilter simpleAdd, int addFilterIndex,
			SimpleQueryFilter curFilter, int currentFilterIndex, List<Integer> indicesToRemove,
			List<Integer> addFiltersToIgnore, List<Integer> indicesRemovedByDirectConflict) {
		...
		if (curFilter.equivalentColumnModifcation(simpleAdd, false)) {
			...
			else if (IQueryFilter.newComparatorOvershadowsExisting(curFilter.getComparator(),
					simpleAdd.getComparator())) {
