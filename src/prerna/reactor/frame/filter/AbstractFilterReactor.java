...
public abstract class AbstractFilterReactor extends AbstractFrameReactor {
  ...
  protected void mergeFilters(GenRowFilters newFiltersToAdd, GenRowFilters existingFilters) {
    ...
    for (int addFilterIdx = 0; addFilterIdx < addFiltersList.size(); addFilterIdx++) {
      ...
      if (addFilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
        ...
        if ((selectAll = SimpleQueryFilter.isSelectAll(simpleAdd))
            || SimpleQueryFilter.isUnselectAll(simpleAdd)) {
        ...
        for (int filterIndex = 0;
            filterIndex < existingFilters.getFilters().size();
            filterIndex++) {
          ...
          if (currentFilter.getQueryFilterType() == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
            ...
            IQueryFilter requireNewFilter =
                processSimpleQueryFilter(
                    simpleAdd,
                    addFilterIdx,
                    curFilter,
                    filterIndex,
                    indicesToRemove,
                    addFiltersToIgnore,
                    indicesRemovedByDirectConflict);
    ...
    if (!indicesToRemove.isEmpty()) {
      ...
      for (int i = indicesToRemove.size(); i > 0; i--) {
        ...
        IQueryFilter removedFilter =
            existingFilters.removeFilter(indicesToRemove.get(i - 1).intValue());
  ...
  protected IQueryFilter processSimpleQueryFilter(
      SimpleQueryFilter simpleAdd,
      int addFilterIndex,
      SimpleQueryFilter curFilter,
    ...
    if (curFilter.equivalentColumnModifcation(simpleAdd, false)) {
      ...
      if (IQueryFilter.comparatorsDirectlyConflicting(
          curFilter.getComparator(), simpleAdd.getComparator())) {
      ...
      else if (IQueryFilter.comparatorsRegexConflicting(
          curFilter.getComparator(), simpleAdd.getComparator())) {
      ...
      else if (IQueryFilter.newComparatorOvershadowsExisting(
          curFilter.getComparator(), simpleAdd.getComparator())) {
      ...
      else if (IQueryFilter.comparatorsRequireOrStatement(
          curFilter.getComparator(), simpleAdd.getComparator())) {
        // remove this filter
        // do not add the simple filter
        // because we will add the OR filter
      ...
      else if (IQueryFilter.comparatorsCanCombine(
          curFilter.getComparator(), simpleAdd.getComparator())) {
