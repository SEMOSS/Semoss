...
public class CacheNativeFrame extends AbstractFrameReactor {
  ...
  public CacheNativeFrame() {
    this.keysToGet =
        new String[] {
          ReactorKeysEnum.FRAME.getKey(), ReactorKeysEnum.FRAME_TYPE.getKey(), SYNCHRONIZE
        };
  }
  ...
  public NounMetadata execute() {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error occurred trying to create frame of type " + frameType, e);
    }
    ...
    if (inThread() || this.insight.isSavedInsightMode()) {
      ...
      newFrameNoun.addAdditionalOpTypes(
      ...
      return new NounMetadata(
          "Swapping frame in parallel thread",
          PixelDataType.CONST_STRING,
          PixelOperationType.FRAME_SWAP);
    }
  }
  ...
  /**
   * True means run in the current thread False means run in its own thread
   *
   * @return
   */
  ...
  class CachingThread implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        this.insight.addDelayedMessage(
            getError("Error occurred while caching live query. " + e.getMessage()));
        return;
      }
      ...
      this.insight.addDelayedMessage(
          NounMetadata.getSuccessNounMessage(
              "Successfully cached data. "
                  + "Operation took "
                  + (end - start)
                  + "ms to execute the query and "
                  + (end2 - start2)
                  + "ms to generate the cache"));
    }
  }
}
