...
public class CacheNativeFrame extends AbstractFrameReactor {
	...
	public NounMetadata execute() {
		...
		if (inThread() || this.insight.isSavedInsightMode()) {
			...
			newFrameNoun.addAdditionalOpTypes(PixelOperationType.FRAME, PixelOperationType.FRAME_DATA_CHANGE,
			...
			t.start();
			return new NounMetadata("Swapping frame in parallel thread", PixelDataType.CONST_STRING,
	...
	class CachingThread implements Runnable {
		...
		public void run() {
			...
			this.insight.addDelayedMessage(
					NounMetadata.getSuccessNounMessage("Successfully cached data. " + "Operation took " + (end - start)
							+ "ms to execute the query and " + (end2 - start2) + "ms to generate the cache"));
		}
	}
}
