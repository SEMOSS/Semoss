...
public class NodeDetailsReactor extends AbstractRFrameReactor {
	...
	public NodeDetailsReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.FRAME.getKey(), ReactorKeysEnum.COLUMN.getKey(),
				ReactorKeysEnum.VALUE.getKey(), ReactorKeysEnum.PANEL.getKey()};
	}
	...
	public NounMetadata execute() {
		...
		if (graphName != null) {
			...
			for (int vertIndex = 0; vertIndex < vertices.length; vertIndex++) {
				String thisNodeId = this.rJavaTranslator.getString("vertex_attr(" + graphName + ", \""
						+ TinkerFrame.TINKER_ID + "\", " + vertices[vertIndex] + ")");
		...
		ConstantDataTask taskData = ConstantTaskCreationHelper.getGridData(panelId, new String[]{"Metric", "Value"},
				gridData);
