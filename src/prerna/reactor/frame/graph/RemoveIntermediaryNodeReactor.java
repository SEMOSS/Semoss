...
public class RemoveIntermediaryNodeReactor extends AbstractFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    GraphTraversal<Vertex, Vertex> traversal =
        tinker.g.traversal().V().has(TinkerFrame.TINKER_TYPE, physicalTypeToRemove);
    while (traversal.hasNext()) {
      ...
      for (int i = 0; i < numSource; i++) {
        ...
        for (int j = 0; j < numTarget; j++) {
          ...
          GraphTraversal<Edge, Edge> gt =
              tinker.g.traversal().E().has(TinkerFrame.TINKER_ID, edgeID);
          if (gt.hasNext()) {
            ...
            source.addEdge(
                type, target, TinkerFrame.TINKER_ID, edgeID, TinkerFrame.TINKER_COUNT, 1);
          }
        }
      }
