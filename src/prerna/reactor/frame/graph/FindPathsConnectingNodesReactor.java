...
public class FindPathsConnectingNodesReactor extends AbstractFrameReactor {
	...
	public static void findSharedVertices(TinkerFrame tf, String columnType, List<String> instances,
			int numTraversals) {
		...
		for (int index = 0; index < numInstances - 1; index++) {
			...
			List<String> instancesToBind = new ArrayList<String>(instances.subList(index + 1, numInstances));

			GraphTraversal t1 = tf.g.traversal().V().has(TinkerFrame.TINKER_TYPE, nodeType)
					.has(TinkerFrame.TINKER_NAME, instance).as("start");
