...
public class AutoCleanColumnReactor extends AbstractRFrameReactor {
  ...
  public AutoCleanColumnReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.COLUMN.getKey(), ReactorKeysEnum.OVERRIDE.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    if (!override) {
      ...
      metaData.setDataTypeToProperty(
          tableName + "__" + newHeaderName, SemossDataType.STRING.toString());
    } else {
      ...
      this.rJavaTranslator.runR(
          tableName + "$" + column + " <- master_col_data(" + tableName + "$" + column + ");");
    }
    frame.syncHeaders();
