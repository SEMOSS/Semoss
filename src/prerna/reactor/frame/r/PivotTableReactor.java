...
public class PivotTableReactor extends AbstractRFrameReactor {
  ...
  /**
   * This reactor creates a pivot table based on various thing Row Groups - what do you want your
   * row to be - basically columns Column Groups - What are the columns on the pivot table Values -
   * The calculation columns that is needed Following are the calculations possible Sum mean max min
   * n() - just count standard deviation - sd ignoring na - max(SchedSpeedMPH, na.rm=TRUE) - this
  ...
  public NounMetadata execute() {
    ...
    // -	pt <- qpvt(df, c("gender", "frame"), c("location"), calculations=c("TOTAL Chol" =
    ...
    // create the html
    genPivot =
        genPivot + "; " + pivotName + "$saveHtml(paste(ROOT" + ",\"/" + htmlName + "\", sep=\"\"))";
    ...
    */
    ...
    return new NounMetadata(
        "ROOT/" + htmlName, PixelDataType.CONST_STRING, PixelOperationType.OPERATION);
  }
