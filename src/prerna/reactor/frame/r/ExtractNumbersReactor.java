...
public class ExtractNumbersReactor extends AbstractRFrameReactor {
  ...
  public ExtractNumbersReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.COLUMNS.getKey(), ReactorKeysEnum.OVERRIDE.getKey()};
  }
    ...
    if (overrideColumn) {
      for (int i = 0; i < columns.size(); i++) {
        ...
        if (Utility.isStringType(dataType.toString())) {
          String script =
              table + "$" + column + " <- gsub('[^\\\\.0-9]', '', " + table + "$" + column + ");";
          frame.executeRScript(script);
          this.addExecutedCode(script);
          frame
              .getMetaData()
              .modifyDataTypeToProperty(
                  table + "__" + column, table, SemossDataType.STRING.toString());
        } else {
      ...
      for (int i = 0; i < columns.size(); i++) {
        ...
        if (Utility.isStringType(dataType.toString())) {
          ...
          metaData.setDataTypeToProperty(
              table + "__" + newColumn, SemossDataType.STRING.toString());
        } else {
          throw new IllegalArgumentException("Column type must be string");
        }
      }
