...
public class SemanticBlendingReactor extends AbstractRFrameReactor {
  ...
  /**
   * This reactor runs the semantic blending routine on a given column and flushes the results out
   * as a table The inputs to the reactor are: 1) the columns 2) the number of results to be
   * displayed (defaults to 3 if none is entered) 3) the number of random values to use in the
   * routine (defaults to 20 if none is entered) 4) boolean indicator if we want to create an r data
   * table, otherwise just return table of results; true indicates widget is used; defaults to false
  ...
  public SemanticBlendingReactor() {
    this.keysToGet =
        new String[] {
          ...
          ReactorKeysEnum.NUM_DISPLAY.getKey(),
          ReactorKeysEnum.RANDOM_VALS.getKey(),
          GENERATE_FRAME,
          FRAME_NAME
        };
  }
  ...
  public NounMetadata execute() {
    ...
    // build a query struct so that we can query and limit the number of values being passed into
    // the method
    ...
    // this will define the column numbers that we are selecting from our frame to run through the
    // routine
    ...
    String baseRScriptPath =
        getBaseFolder() + "\\" + Constants.R_BASE_FOLDER + "\\" + "AnalyticsRoutineScripts";
    ...
    logger.info(
        "This process may take a few minutes depending on the type of data and internet speed");
    ...
    if (!generateFrameIndicator) {
      ...
      return new NounMetadata(
          table, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.WIKI_LOGICAL_NAMES);
    } else {
