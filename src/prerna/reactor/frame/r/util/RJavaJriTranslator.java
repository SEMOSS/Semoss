...
public class RJavaJriTranslator extends AbstractRJavaTranslator {
  ...
  private static ConcurrentMap<String, ReentrantLock> genEngineLock =
  ...
  /**
   * Constructor only accessible through the package Please use the insight object or the
   * RJavaTranslatorFactory to get the correct instance
   */
  ...
  /**
   * This will start R, only if it has not already been started In this case we are starting an
   * engine for JRI
   */
  @Override
  public void startR() {
    ...
    if (retEngine == null) {
      try {
        ...
        if (this.insight != null) {
          this.insight
              .getVarStore()
              .put(IRJavaTranslator.R_ENGINE, new NounMetadata(retEngine, PixelDataType.R_ENGINE));
        }
        ...
        throw new IllegalArgumentException(
            "Could not connect to R JRI.  Please make sure paths are accurate");
  ...
  /**
   * This method is used to get the column types of a frame Need to account for R where factors come
   * in as ordered factors... Have portions in other places to pick up "ordered" and "factor" as
   * both factor
   *
   * @param frameName
   */
