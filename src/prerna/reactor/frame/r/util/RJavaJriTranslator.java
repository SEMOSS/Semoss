...
public class RJavaJriTranslator extends AbstractRJavaTranslator {
	...
	/**
	 * Constructor only accessible through the package Please use the insight object
	 * or the RJavaTranslatorFactory to get the correct instance
	 */
	...
	/**
	 * This will start R, only if it has not already been started In this case we
	 * are starting an engine for JRI
	 */
	@Override
	public void startR() {
		Rengine retEngine = Rengine.getMainEngine();
		...
		if (retEngine == null) {
			try {
				...
				if (this.insight != null) {
					this.insight.getVarStore().put(IRJavaTranslator.R_ENGINE,
							new NounMetadata(retEngine, PixelDataType.R_ENGINE));
				}
	...
	/**
	 * This method is used to get the column types of a frame Need to account for R
	 * where factors come in as ordered factors... Have portions in other places to
	 * pick up "ordered" and "factor" as both factor
	 *
	 * @param frameName
	 */
	@Override
	...
	// rjt.executeR("mv <-
	// fread(\"c:/users/pkapaleeswaran/workspacej3/datasets/Movie3.csv\")");
	//
	// try
	// {
	// Method meth = rjt.getClass().getSuperclass().getDeclaredMethod("getString",
	// String.class);
