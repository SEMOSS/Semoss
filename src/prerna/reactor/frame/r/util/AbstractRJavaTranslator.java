...
public abstract class AbstractRJavaTranslator implements IRJavaTranslator {
  ...
  /**
   * Execute an R Script without a return FOR INTERNAL USE ONLY - IF YOU TRY TO CALL IT WONT WORK
   *
   * @param rScript
   */
   ...
   * breaks for a histogram as double[] counts for a histogram as int[] Map keys are breaks and
   * counts
   *
   * @param script
   */
  ...
  protected void removeRFunctions() {
    ...
    String rScript =
        "getenv <- function() {};Sys.chmod<-getenv;Sys.date<-getenv;Sys.getenv<-getenv;Sys.getlocate<-getenv;"
  ...
  /**
   * Remove the existing R environment This should only be called when you drop the entire insight
   */
   ...
   * @param rScript
   */
  public static void loadDataBetweenEnv(
      AbstractRJavaTranslator trans1,
      String assignVar,
      AbstractRJavaTranslator trans2,
      String rScript) {
    String combinedScript =
        trans1.encapsulateForEnv(assignVar + "<- " + trans2.encapsulateForEnv(rScript));
  ...
  public void changeColumnType(
    ...
    if (newType.equalsIgnoreCase("string")) {
      script =
          frameName + " <- " + frameName + "[, " + colName + " := as.character(" + colName + ")]";
      ...
      script =
          frameName + " <- " + frameName + "[, " + colName + " := as.numeric(" + colName + ")]";
      this.executeEmptyR(script);
  ...
  /**
   * This method is used initialize an empty matrix with the appropriate number of rows and columns
   *
   * @param matrix
   * @param numRows
   * @param numColumns
  ...
  /**
   * This method is used generate a r data.table from a given query Returns the r variable name that
   * references the created data.table
   *
   * @param frame
   * @param qs
   * @return rFrame name
  ...
  public String generateRDataTableVariable(ITableDataFrame frame, SelectQueryStruct qs) {
    ...
    StringBuilder instanceValuesBuilder =
        new StringBuilder(); // this puts values into the frame we are constructing
    ...
    // now that we have the instance values, we can use them to build a string to populate the table
    // that we will make in r
    ...
    for (int i = 0; i < numSelectors; i++) {
      ...
      for (int j = 0; j < instanceList.size(); j++) {
        ...
        if (instanceList.get(j) == null) {
          ...
          instanceValuesBuilder.append(
              "\"" + instanceList.get(j)[i].toString().replaceAll("_", " ") + "\"");
        }
        instanceValuesBuilder.append(";");
      }
    }
    ...
    // scriptSb format: frame<-data.frame(Title= character(), stringsAsFactors = FALSE); + the
    // instances
    StringBuilder scriptSb = new StringBuilder(); // this builds the dataframe
    scriptSb
        .append(dfName)
        .append("<-data.frame(")
        .append(colNameString)
  ...
  public void checkPackages(String[] packages) {
     ...
     */
    //		String packageError = "";
    //		int[] confirmedPackages = this.getIntArray("which(as.logical(lapply(list('" +
    // StringUtils.join(packages, "','")
    ...
    //			String errorMessage = "\nMake sure you have all the following R libraries installed:\n" +
    // packageError;
    //			throw new SemossPixelException(new NounMetadata(errorMessage, PixelDataType.CONST_STRING,
    ...
    if (this.insight != null) {
      ...
      try {
        ...
        if (this.insight.getUser() != null && !this.insight.getUser().isAnonymous()) {
          userRootPath =
              AssetUtility.getRootFolderPath(this.insight, AssetUtility.USER_SPACE_KEY, false);
  ...
  public String runRAndReturnOutput(String script) {
    ...
    script =
        randomVariable
            ...
            + encapsulateForEnv(
                insightRootAssignment + appRootAssignment + userRootAssignment + script)
            + "sink();";
      ...
      try {
        if (!outputFile.exists()) {
          throw new IllegalArgumentException(
              "Failed to run R script. No output could be generated");
        }
  ...
  private static String cleanErrorOutput(String output) {
    output =
        output
            .replaceAll("Error in eval\\(expr, envir, enclos\\) : ", "")
