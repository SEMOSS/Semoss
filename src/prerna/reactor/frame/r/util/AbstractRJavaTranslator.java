...
public abstract class AbstractRJavaTranslator implements IRJavaTranslator {
	...
	/*
	 * abstract internal methods should not be used outside of the translator
	 */

	/**
	 * Execute an R Script without a return FOR INTERNAL USE ONLY - IF YOU TRY TO
	 * CALL IT WONT WORK
	 * 
	 * @param rScript
	 */
	...
	/**
	 * Execute an R Script FOR INTERNAL USE ONLY - IF YOU TRY TO CALL IT WONT WORK
	 * 
	 * @param rScript
	 */
	...
	/**
	 * This method uses specific Rserve or JRI methods to get the breaks and counts
	 * for a histogram breaks for a histogram as double[] counts for a histogram as
	 * int[] Map keys are breaks and counts
	 * 
	 * @param script
	 */
	...
	/**
	 * This method uses specific Rserve or JRI methods to get a table in the form
	 * Map<String, Object>
	 * 
	 * @param framename
	 * @param colNames
	...
	/**
	 * Remove the existing R environment This should only be called when you drop
	 * the entire insight
	 */
	...
	/**
	 * Function to run a script from one environment and assign the output into
	 * another environment
	 * 
	 * @param trans1
	 * @param assignVar
	 ...
	 * @param rScript
	 */
	public static void loadDataBetweenEnv(AbstractRJavaTranslator trans1, String assignVar,
			AbstractRJavaTranslator trans2, String rScript) {
	...
	public boolean isEmpty(String frameName) {
		String script = "(!exists(\"" + frameName + "\") || (is.data.table(" + frameName + ") && nrow(" + frameName
				+ ") == 0))";
	...
	/**
	 * This method is used initialize an empty matrix with the appropriate number of
	 * rows and columns
	 * 
	 * @param matrix
	 * @param numRows
	...
	/**
	 * This method is used generate a r data.table from a given query Returns the r
	 * variable name that references the created data.table
	 * 
	 * @param frame
	 * @param qs
	 * @return rFrame name
	...
	public String generateRDataTableVariable(ITableDataFrame frame, SelectQueryStruct qs) {
		...
		StringBuilder instanceValuesBuilder = new StringBuilder(); // this puts values into the frame we are
																	// constructing
		...
		try {
			...
			classLogger.error("Failed to query frame while generating R data.table variable", e);
		} finally {
			if (it != null) {
				try {
					it.close();
				} catch (IOException e) {
					classLogger.error("Failed to close query iterator while generating R data.table variable", e);
				}
			}
		}

		// now that we have the instance values, we can use them to build a string to
		// populate the table that we will make in r
		// colNameString will keep track of the columns we are creating in our new r
		// data frame
		...
		for (int i = 0; i < numSelectors; i++) {
			...
			for (int j = 0; j < instanceList.size(); j++) {
				...
				if (instanceList.get(j) == null) {
					...
					instanceValuesBuilder.append("\"" + instanceList.get(j)[i].toString().replace("_", " ") + "\"");
				}
				instanceValuesBuilder.append(";");
			}
		}
		...
		// scriptSb format: frame<-data.frame(Title= character(), stringsAsFactors =
		// FALSE); + the instances
	 ...
	 * 
	 * @param packages
	 * @throws IllegalArgumentException error if an r package is missing
	 */
	@Override
...
//		}
	 ...
	 * 
	 * @param packages
	 * @param logger   log missing packages
	...
	public void runR(String script) {
		...
		script = encapsulateForEnv(insightRootAssignment + appRootAssignment + userRootAssignment + script);
//		logger.info("#### Rscript ####: " +scriptFile.getName() + ": " + script);
		...
		try {
			...
			classLogger.error("Failed to write or source R script file", e);
		} finally {
			try {
	...
	public String runRAndReturnOutput(String script) {
		...
		// attempt to put it into environment
		script = randomVariable + "<- file(\"" + outputPath + "\"); " + "sink(" + randomVariable
		...
		try {
			...
			try {
				...
				logger.error("Failed to read R script output", e);
			...
			classLogger.error("Failed to write R script file for execution", e);
	...
	private static String cleanErrorOutput(String output) {
		output = output.replace("Error in eval(expr, envir, enclos) : ", "").replace("Error in eval(ei, envir) : ", "")
				.replace("\r", "").replace("\n", " ");
