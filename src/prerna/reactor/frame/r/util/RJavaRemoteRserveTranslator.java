...
public class RJavaRemoteRserveTranslator extends RJavaRserveTranslator {
    ...
    if (this.retCon == null) {
      try {
        ...
        if (this.insight != null) {
          this.insight
              .getVarStore()
              .put(AbstractBaseRClass.R_CONN, new NounMetadata(retCon, PixelDataType.R_CONNECTION));
          this.insight
              .getVarStore()
              .put(AbstractBaseRClass.R_PORT, new NounMetadata(port, PixelDataType.CONST_STRING));
        }
  ...
  public String runRAndReturnOutput(String script) {
    ...
    if (remoteR) {
      ...
      try {
        String finalScript =
            "print(source(\"" + tempFileLocation + "\", print.eval=TRUE, local=TRUE)); ";
  ...
  public RConnection getRcon() {
    ...
    // if there is no user or insight associated, just send back a new RCon that will be a fresh
    // rserve space
    else {
      logger.info("R Connection has not been defined yet...");
      logger.info("Starting R Connection... ");
