...
public class RunAssociatedLearningReactor extends AbstractRFrameReactor {
  ...
  /**
   * RunAssociatedLearning(ruleSide=["Outcome"], column=["Nominated"] , values=["Y"] ,
   * attributes=["Genre" , "Nominated" ,"Rating"], conf = [0.5],support = [0.005],lift=[1],
   * panel=[0]);
   */
  ...
  public NounMetadata execute() {
    ...
    String resultsFrame =
        runAssociatedRulesRScript(targetDt, ruleSide, column, values, attrs, supp, conf, lift);
    ...
    if (ruleslength == 0) {
      noun.addAdditionalReturn(
          NounMetadata.getErrorNounMessage(
              "Assocation Learning Algorithm ran successfully, but no results were found"));
    } else {
      noun.addAdditionalReturn(
          NounMetadata.getSuccessNounMessage("Associated Learning ran successfully!"));
    }

    return noun;
  }

  private String runAssociatedRulesRScript(
      String frameName,
      String ruleSide,
      String column,
      List<String> values,
  ...
  private List<String> getInputList(String input) {
    ...
    GenRowStruct columnGrs =
        input.equals("0") ? this.store.getNoun(keysToGet[0]) : this.store.getNoun(input);
    if (columnGrs != null) {
      ...
      if (input.equals("0")) {
        throw new IllegalArgumentException(
            "Attribute(s) that make up a transaction must be specified.");
      }
    }
