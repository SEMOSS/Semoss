...
public class RunNumericalModelReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    if (!dataFrame.isNumeric(instanceColumn)) {
      ...
      NounMetadata noun =
          new NounMetadata(
              "Numerical Similarity can only be run on a numerical column",
    ...
    if (success) {
      ...
      ITask taskData =
          ConstantTaskCreationHelper.getScatterPlotData(
              panelId, "ROW_ID", instanceColumn, newColName, bulkRowDataPoints);
      ...
      NounMetadata noun =
          new NounMetadata(dataFrame, PixelDataType.FRAME, PixelOperationType.FRAME_DATA_CHANGE);
      noun.addAdditionalReturn(
          new NounMetadata(
              taskData, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA));
      noun.addAdditionalReturn(
          new NounMetadata(
              "Numerical Similarity ran successfully!",
              PixelDataType.CONST_STRING,
      ...
      NounMetadata noun =
          new NounMetadata(
              "Numerical Similarity did not run successfully. No model found.",
              PixelDataType.CONST_STRING,
              PixelOperationType.ERROR);
      return noun;
    }
  ...
  private boolean runAlgorithm(String frameName, String instanceColumn, String newColName) {
    ...
    String NumSimScriptFilePath =
        getBaseFolder() + "\\R\\AnalyticsRoutineScripts\\NumericalModel.R";
    ...
    rsb.append(
        "setnames(" + resultFrame + ",old = c(\"NN_Output\"), new=c(\"" + newColName + "\"));");
