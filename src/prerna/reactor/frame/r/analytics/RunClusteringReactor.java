...
public class RunClusteringReactor extends AbstractRFrameReactor {
  ...
  /**
   * with specific cluster # RunClustering ( algorithm = [kmeans], multiOption = [false], instance =
   * [ Species ] , attributes = [ "SepalLength" , "SepalWidth" , "PetalLength" , "PetalWidth" ],
   * numClusters = [ 3 ], uniqInstPerRow = [Yes] ) ;
   *
   * <p>with min-max range of cluster #s RunClustering ( algorithm = [kmeans], multiOption = [true],
   * instance = [ Species ] , attributes = [ "SepalLength" , "SepalWidth" , "PetalLength" ,
   * "PetalWidth" ], minNumClusters = [2], maxNumClusters = [10] , uniqInstPerRow = [Yes]) ;
   *
   * <p>Input keys: 1. algorithm (optional) - kmeans (numerical data only), pam (numerical data
   * only), pamGower (categorical/numerical data). default = kmeans for numerical only data or
   * pamGower for numerical and/or categorical data 2. multiOption (required) - boolean (true or
   * false) if true, then multiclustering (minNumClusters/maxnNumClusters can be specified) if
   * false, then single clustering (numClusters can be specified) 3. instance (required) 4.
   * attributes (required) 5. numClusters (optional) - can be specified if multioption = false.
   * default = 5 6. minNumClusters (optional) - can be specified if multioption = true. default = 2
   * 7. maxnNumClusters (optional) - can be specified if multioption = true. default = 20 8.
   * uniqInstPerRow (optional; if not passed in, assumes no) - if yes, then will treat each row in
   * the frame as a unique instance/record; if no, then will aggregate the data in the attributes
   * columns by the instance column first
   */
  ...
  public RunClusteringReactor() {
    this.keysToGet =
        new String[] {
          ...
          ReactorKeysEnum.CLUSTER_KEY.getKey(),
          MIN_NUM_CLUSTERS,
          MAX_NUM_CLUSTERS,
          UNIQUE_INSTANCE_PER_ROW
        };
  }
  ...
  public NounMetadata execute() {
    ...
    rsb.append(
        attrNamesList_R + "<- " + RSyntaxHelper.createStringRColVec(attrNamesList.toArray()) + ";");
    String clusteringScriptFilePath =
        getBaseFolder() + "\\R\\AnalyticsRoutineScripts\\Clustering.R";
    ...
    if (multiOption == false) {
      ...
      if ((minNumClusters > 0 && minNumClusters >= nrows)
          || (maxNumClusters > 0 && maxNumClusters >= nrows)) {
        meta.dropProperty(dtName + "__" + tempKeyCol, dtName);
        this.rJavaTranslator.runR(
            "rm("
                + scaleUniqueData_R
                ...
                + ",getDtClusterTable,getNewColumnNam,scaleUniqueData);gc();");
        throw new IllegalArgumentException(
            "Number of min/max clusters requested should be less than the "
                + "number of unique instances, "
                + nrows
                + ".");
      }
    ...
    if (!updatedDfCols.isEmpty()) {
      // if implicitFilter == true, then need to join the resulting column to the whole frame
      // (dtName var)
      if (implicitFilter) {
        this.rJavaTranslator.runR(
            dtName
                ...
                + "',"
                + "'"
                + StringUtils.join(updatedDfCols, "','")
                + "'"
                + "), with=FALSE],by ='"
                + tempKeyCol
                + "', all.x=TRUE);"
  ...
  private void addUUIDColumnToOrigFrame(
      String frameName, OwlTemporalEngineMeta meta, String tempKeyCol) {
    this.rJavaTranslator.executeEmptyR(
        frameName + "$" + tempKeyCol + "<- seq.int(nrow(" + frameName + "));");
  ...
  private String getAlgorithm() {
    ...
    if (algorithmGrs != null) {
      ...
      // default to kmeans; if categorical data is detected in attributes cols, then will default to
      // pamGower
      algorithm = "kmeans";
    }
    return algorithm;
  ...
  private boolean getMultiOption() {
    ...
    if (multiOptionGrs != null) {
      ...
      throw new IllegalArgumentException(
          "Specify whether single or multiple clustering is being requested");
    }
  }
