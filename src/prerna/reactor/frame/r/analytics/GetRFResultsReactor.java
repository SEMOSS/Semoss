...
public class GetRFResultsReactor extends AbstractRFrameReactor {
  ...
  /**
   * GetRFResults(requestItem = [VarImp], panel = [99]) GetRFResults(requestItem = [CONFMATRIX],
   * panel = [99])
   *
   * <p>This reactor will only run if the RRandomForestAlgorithmReactor created a variable
   * RF_VARIABLE_999988888877777 as it extracts/processes this variable. Input keys: 1. sortBy
   * (optional) - for classification results, sorts the variable importance by either
   * MeanDecreaseAccuracy (1) or MeanDecreaseGini (2). for regression results, sorts the variable
   * importance by either %IncMSE (1) or IncNodePurity {2}. defaults = 1 2. requestItem (required) -
   * return object - must be either varimp (variable importance) or confmatrix (confusion matrix) 3.
   * panelID (required)
   */
  ...
  public NounMetadata execute() {
    ...
    if (!new ArrayList<String>(Arrays.asList("varimp", "confmatrix"))
        .contains(requestItem.toLowerCase())) {
    ...
    switch (requestItem.toLowerCase()) {
      case "varimp":
        sb.append(
            temp_R
                + " <- getRFResults( RF_VARIABLE_999988888877777, 'varimp', sortBy="
                + sortBy
                + ");");
        this.rJavaTranslator.runR(sb.toString());

        String[] varImpCols = this.rJavaTranslator.getColumns(temp_R + "$returnObject");
        List<Object[]> varImpData =
            this.rJavaTranslator.getBulkDataRow(temp_R + "$returnObject", varImpCols);
        // label,x,y,z,series
        String[] varImpAlignment = this.rJavaTranslator.getStringArray(temp_R + "$alignmentInfo");

        taskData =
            ConstantTaskCreationHelper.getScatterPlotData(
                panelId,
                varImpCols,
                varImpData,
                varImpAlignment[0],
      ...
      case "confmatrix":
        ...
        sb.append(
            temp_R
                + " <- getRFResults( RF_VARIABLE_999988888877777, 'confmatrix', sortBy="
                + sortBy
                + ");");
        this.rJavaTranslator.runR(sb.toString());
    ...
    noun.addAdditionalReturn(
        new NounMetadata(
            "Random Forest ran successfully!",
  ...
  private String getStringInput(String keyName) {
    ...
    if (columnGrs != null) {
      ...
      if (keyName == REQUESTITEM) {
        throw new IllegalArgumentException(
            "RequestItem of either 'varimp' or 'confmatrix' must be specified.");
      }
    }
    return null;
  }
