...
public class GetRFResultsReactor extends AbstractRFrameReactor {
	...
	/**
	 * GetRFResults(requestItem = [VarImp], panel = [99]) GetRFResults(requestItem =
	 * [CONFMATRIX], panel = [99])
	 *
	 * <p>
	 * This reactor will only run if the RRandomForestAlgorithmReactor created a
	 * variable RF_VARIABLE_999988888877777 as it extracts/processes this variable.
	 * Input keys: 1. sortBy (optional) - for classification results, sorts the
	 * variable importance by either MeanDecreaseAccuracy (1) or MeanDecreaseGini
	 * (2). for regression results, sorts the variable importance by either %IncMSE
	 * (1) or IncNodePurity {2}. defaults = 1 2. requestItem (required) - return
	 * object - must be either varimp (variable importance) or confmatrix (confusion
	 * matrix) 3. panelID (required)
	 */
		...
		switch (requestItem.toLowerCase()) {
			case "varimp" :
				...
				taskData = ConstantTaskCreationHelper.getScatterPlotData(panelId, varImpCols, varImpData,
						varImpAlignment[0], varImpAlignment[1], varImpAlignment[2], varImpAlignment[3],
						varImpAlignment[0], null);
