...
/**
 * This reactor updates determines the outlier rows based on the selected attribute columns The
 * result is a new column added to the data frame with values of TRUE (is an outlier) or FALSE (not
 * an outlier) The inputs to the reactor are: 1) the attribute columns 2) alpha
 *
 * <p>RunOutlier(attributes=["Revenue_International","Revenue_Domestic","MovieBudget"],alpha=[0.05],nullHandleType=["As_Is"])
...
public class RunOutlierReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    if (nullHandlingType.equals("as_is")) {
      for (String columnName : attributeNamesList) {
        ...
        int countNulls =
            this.rJavaTranslator.getInt("sum(is.na(" + frameName + "$" + columnName + "))");
        if (countNulls > 0) {
          String msg =
              columnName
                  + " has "
    ...
    String newFrameName =
        runROutlierAlgorithm(frameName, attributeNames, alpha, nullHandlingType, newColName);
   ...
   * @param newColName
   */
  private String runROutlierAlgorithm(
      String frameName,
      String[] attributeNames,
      String alpha,
      String nullHandlingType,
      String newColName) {
