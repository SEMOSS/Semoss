...
public class RunLOFReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    sb.append(
        attrList_R
            + "<- "
            + RSyntaxHelper.createStringRColVec(this.attributeNamesList.toArray())
            + ";");
    ...
    if (nrows == 1) {
      ...
      this.rJavaTranslator.runR(
          "rm("
              ...
              + ","
              + dtNameIF
              + ",scaleUniqueData,runLOF,getLOP,getNewColumnName);gc();");
      throw new IllegalArgumentException("Instance column contains only 1 unique record.");
    }

    String kStr = getK();
    boolean intK =
        this.rJavaTranslator.getBoolean(
            "all(" + "c(" + kStr + ") == floor( " + "c(" + kStr + ")))");
    ...
    if (!intK || this.kArray == null || this.kArray.length == 0) {
      ...
      throw new IllegalArgumentException(
          "K must be either a single integer or a list of integers only.");
    }
    ...
    this.rJavaTranslator.runR(
        "rm("
            ...
            + ","
            + k_R
            + ",scaleUniqueData,runLOF,getLOP,getNewColumnNam);gc();");
