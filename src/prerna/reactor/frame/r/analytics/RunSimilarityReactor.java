...
/**
 * This reactor updates determines the similarity between values in an instance column based on the
 * selected attribute columns The result is a new column added to the data frame with values between
 * 0 and 1 for each row Higher values indicate stronger similarity The inputs to the reactor are: 1)
 * the instance column 2) the attribute columns
 */
public class RunSimilarityReactor extends AbstractRFrameReactor {
  ...
  public RunSimilarityReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.INSTANCE_KEY.getKey(), ReactorKeysEnum.ATTRIBUTES.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    // we are returning the name of our table that sits in R; it is structured as a list of entries:
    // x,y,cor
    NounMetadata noun =
        new NounMetadata(
            dataFrame,
    ...
    noun.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage(
            "Similarity ran succesfully! See new \"" + newColName + "\" column in the grid."));
    return noun;
  }
   ...
   * @param newColName
   */
  private void runRSimilarityAlgorithm(
      String frameName, String[] attributeNames, String instanceColumn, String newColName) {
    ...
    String correlationScriptFilePath =
        getBaseFolder() + "\\R\\AnalyticsRoutineScripts\\Similarity.R";
    correlationScriptFilePath = correlationScriptFilePath.replace("\\", "/");
    rsb.append("source(\"" + correlationScriptFilePath + "\");");
    // R syntax for the routine: ResultsTableName <- getCorrelationTable(frameName, "Title",
    // c("col1", "col2"), "simColumnName")
    rsb.append(
        resultsFrameName
            + "<- GenerateSimilarityTable("
            + frameName
            + ", "
            + "\""
            + instanceColumn
    ...
    rsb.append(
        "rm("
            + resultsFrameName
            + ",CalculateSimilarity, CSimilarity,DefineRatios,"
