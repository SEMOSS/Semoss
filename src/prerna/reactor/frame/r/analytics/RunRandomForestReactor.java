...
public class RunRandomForestReactor extends AbstractRFrameReactor {
	...
	/**
	 * FRAME | RunRandomForest ( classify = [ Nominated ] , attributes = [
	 * "Director", "Genre" , "RottenTomatoesAudience" , "Studio" , "Title" ] , space
	 * = [ "insight" ] , panel = [ 1 ] , size = [ 1000 ] , blocks = [ 100 ] , mode =
	 * [ "build" ] , treeDepth = [0] ) ; FRAME | RunRandomForest ( classify = [
	 * Nominated ] , attributes = [ "Director", "Genre" , "RottenTomatoesAudience" ,
	 * "Studio" , "Title" ] , space = [ "insight" ] , panel = [ 1 ] , size = [ 1000
	 * ] , blocks = [ 100 ] , mode = [ "predict" ] , treeDepth = [0] ) ;
	 */
	...
	public RunRandomForestReactor() {
		this.keysToGet = new String[] { CLASSIFICATION_COLUMN, ReactorKeysEnum.ATTRIBUTES.getKey(),
				ReactorKeysEnum.SPACE.getKey(), ReactorKeysEnum.PANEL.getKey(), SETTINGS, MODE, FILENAME, TREE_SETTINGS,
				NULL_HANDLING };
	}
	...
	public NounMetadata execute() {
		...
		NounMetadata noun = new NounMetadata(barAndGridTask, PixelDataType.FORMATTED_DATA_SET,
		...
		if (mode.equals("build")) { // check if the model needs to be built
			...
			sb.append(RF_VARIABLE + " <- rfmodel_mgr( ind_vars=" + indVars + ", dep_var=\"" + depVar + "\", trainFrame="
					+ targetDt + ", model_fn=\"" + assetFolder + "\", mis_data=\"" + nullHandlerType
					+ "\", sample_size=" + sampleSize + ", sample_blocks=" + sampleBlocks + ", depth=" + treeDepth
					+ ");");
			...
			if (predError != null) {
				...
				List<Object[]> retBarPlotOutput = this.rJavaTranslator.getBulkDataRow(RF_VARIABLE + "[[3]]",
						headerOrdering);
				...
				barAndGridTask = ConstantTaskCreationHelper.getBarChartInfo(barChartPanelId, "Variables",
						"Importance Values", retBarPlotOutput);
				...
				noun.addAdditionalReturn(
						NounMetadata.getSuccessNounMessage("Random Forest model was built and saved to APP assets"));
				NounMetadata noun2 = new NounMetadata(dendrogramTask, PixelDataType.FORMATTED_DATA_SET,
						PixelOperationType.TASK_DATA);
				...
				SemossPixelException exception = new SemossPixelException(
						NounMetadata.getErrorNounMessage("Model build was unsuccessful"));
			...
			// set call to R function
			sb.append(RF_VARIABLE + " <- predict_rfmodel( newFrame=" + targetDt + ", model_fn=\"" + assetFolder
			...
			if (runResult[0] == 0) {
				...
				noun = new NounMetadata(barAndGridTask, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA);
				noun.addAdditionalReturn(
						NounMetadata.getSuccessNounMessage("Prediction from Random Forest model successful"));
			} else {
				...
				SemossPixelException exception = new SemossPixelException(NounMetadata.getErrorNounMessage(
						"Missing data or columns are missing: " + Arrays.toString(columnsNeeded.toArray())));
				exception.setContinueThreadOfExecution(false);
				throw exception;
			}
			...
			if (treeSettings == null || treeSettings.isEmpty()) {
				...
				sb.append(TREE + " <- get_tree(model_fn=\"" + assetFolder + "\", model_nbr=" + treeSettings.get(1)
						+ ", tree_nbr=" + treeSettings.get(2) + ");");
				// execute R
				this.rJavaTranslator.runR(sb.toString());
			}
			String[] dendrogramHeaders = this.rJavaTranslator.getStringArray("colnames(" + TREE + ");");
	...
	private ITask getDendrogramInfo(String panelId, String[] headers, List<Object[]> dataValues) {
		...
		// taskOptions={0={layout=Column, alignment={tooltip=[], labels=[col1],
		// value=[col2]}}}
