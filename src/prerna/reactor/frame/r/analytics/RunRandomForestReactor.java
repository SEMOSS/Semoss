...
public class RunRandomForestReactor extends AbstractRFrameReactor {
  /**
   * FRAME | RunRandomForest ( classify = [ Nominated ] , attributes = [ "Director", "Genre" ,
   * "RottenTomatoesAudience" , "Studio" , "Title" ] , space = [ "insight" ] , panel = [ 1 ] , size
   * = [ 1000 ] , blocks = [ 100 ] , mode = [ "build" ] , treeDepth = [0] ) ; FRAME |
   * RunRandomForest ( classify = [ Nominated ] , attributes = [ "Director", "Genre" ,
   * "RottenTomatoesAudience" , "Studio" , "Title" ] , space = [ "insight" ] , panel = [ 1 ] , size
   * = [ 1000 ] , blocks = [ 100 ] , mode = [ "predict" ] , treeDepth = [0] ) ;
   */
  ...
  public NounMetadata execute() {
    ...
    NounMetadata noun =
        new NounMetadata(
            barAndGridTask, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA);
    ...
    if (mode.equals("build")) { // check if the model needs to be built
      ...
      if (predError != null) {
        ...
        List<Object[]> retBarPlotOutput =
            this.rJavaTranslator.getBulkDataRow(RF_VARIABLE + "[[3]]", headerOrdering);
        ...
        barAndGridTask =
            ConstantTaskCreationHelper.getBarChartInfo(
                barChartPanelId, "Variables", "Importance Values", retBarPlotOutput);
        this.insight.getTaskStore().addTask(barAndGridTask);

        dendrogramTask =
            getDendrogramInfo(dendrogramChartPanelId, dendrogramHeaders, retDendrogramOutput);
        ...
        noun.addAdditionalReturn(
            NounMetadata.getSuccessNounMessage(
                "Random Forest model was built and saved to APP assets"));
        NounMetadata noun2 =
            new NounMetadata(
                dendrogramTask, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA);
        ...
        "predict")) { // check if values need to be predicted from an existing model
      // get predict inputs
      nullHandlerType = getNullHandleType();
        ...
        SemossPixelException exception =
            new SemossPixelException(
                NounMetadata.getErrorNounMessage(
                    "Missing data or columns are missing: "
                        + Arrays.toString(columnsNeeded.toArray())));
        exception.setContinueThreadOfExecution(false);
        throw exception;
      }
