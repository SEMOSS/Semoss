...
public class RunNumericalColumnSimilarityReactor extends AbstractRFrameReactor {
   ...
   * "RottenTomatoes_Critics", "Revenue_Domestic" , "Revenue_International", "MovieBudget" ] , panel
   * = [ "0" ] , sampleSize = ["100"], significance = ["0.05"], showAll = ["TRUE"] );
   */
  ...
  public NounMetadata execute() {
    ...
    ITask taskData =
        ConstantTaskCreationHelper.getHeatMapData(
            panelId, "New", "Existing", "Similarity", retOutput);
    ...
    // User Tracking
    //		UserTrackerFactory.getInstance().trackAnalyticsWidget(this.insight, dataFrame,
    // "ColSimilarity",
    ...
    if (!significance.equals("0.1")
        && !significance.equals("0.05")
        && !significance.equals("0.02")
        && !significance.equals("0.01")) {
      noun.addAdditionalReturn(
          NounMetadata.getWarningNounMessage("Significance level was reset to 0.05"));
    }
    noun.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage("Numerical Column Similarity ran successfully!"));
    return noun;
  }

  private String runAlgorithm(
      String frameName,
      List<String> retHeaders,
      String sampleSize,
      String significance,
      String showAll) {
