...
public class RunNumericalColumnSimilarityReactor extends AbstractRFrameReactor {
	...
	// value to determine whether we also want script to retun non-similar columns,
	// default is true
	protected static final String SHOW_ALL = "showAll";

	/**
	 * RunNumericalColumnSimilarity ( columns = [ "RottenTomatoes_Audience" ,
	 * "RottenTomatoes_Critics", "Revenue_Domestic" , "Revenue_International",
	 * "MovieBudget" ] , panel = [ "0" ] , sampleSize = ["100"], significance =
	 * ["0.05"], showAll = ["TRUE"] );
	 */
	public RunNumericalColumnSimilarityReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.COLUMNS.getKey(), ReactorKeysEnum.PANEL.getKey(), SAMPLE_SIZE,
				SIGNIFICANCE, SHOW_ALL};
	}
	...
	public NounMetadata execute() {
		...
		// User Tracking
		// UserTrackerFactory.getInstance().trackAnalyticsWidget(this.insight,
		// dataFrame,
		// "ColSimilarity",
		...
		// significance is defaulted to 0.05, if something other than 0.1, 0.05, 0.02,
		// or 0.01 is chosen
		...
		if (!significance.equals("0.1") && !significance.equals("0.05") && !significance.equals("0.02")
	...
	private String runAlgorithm(String frameName, List<String> retHeaders, String sampleSize, String significance,
			String showAll) {
		...
		rsb.append(
				resultFrame + "$Similarity <- (" + resultFrame + "$Identical_CDF + " + resultFrame + "$Close_CDF );");
