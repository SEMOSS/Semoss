...
public class RunClassificationReactor extends AbstractRFrameReactor {
  ...
  public RunClassificationReactor() {
    this.keysToGet =
        new String[] {
          CLASSIFICATION_COLUMN, ReactorKeysEnum.ATTRIBUTES.getKey(), ReactorKeysEnum.PANEL.getKey()
        };
  }
  ...
  public NounMetadata execute() {
    ...
    rsb.append(
        attributes_R + "<- " + RSyntaxHelper.createStringRColVec(attributes.toArray()) + ";");
    ...
    // validate that the count of unique values in the instance column != number of rows in the
    // frame
    int nrows = frame.getNumRows(targetDt);
    int uniqInstCount =
        this.rJavaTranslator.getInt(
            "if (is.factor("
                + targetDt
                + "$"
                + predictionCol
    ...
    // clustering r script
    String classificationScriptFilePath =
        getBaseFolder() + "\\R\\AnalyticsRoutineScripts\\Classification.R";
    ...
    if (ctreeArray == null || ctreeArray.length == 0) {
      ...
      NounMetadata noun =
          new NounMetadata(
              vizData, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.VIZ_OUTPUT);
    ...
    NounMetadata noun =
        new NounMetadata(
            ...
            PixelOperationType.FORCE_SAVE_VISUALIZATION);
    noun.addAdditionalReturn(
        new NounMetadata(
  ...
  private void generateTreeEndingWithParenthesis(
      Map<String, Map> rootMap,
      String startKey,
      int subTreeIndex,
      String[] treeStringArr,
      Integer index) {
    String endRegex = ".*\\(n\\s=.*\\)";
    String lastRegex = "\\(n\\s=.*\\)";
    ...
    for (; index < treeStringArr.length; index++) {
      ...
      if (!row.startsWith("|")) {
        ...
        // either done with the currtree - need to pull back out and assess whether rootMap needs to
        // be grown out more
        return;
