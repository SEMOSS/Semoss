...
public class RunMatrixRegressionReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    List<Object[]> bulkRow =
        this.rJavaTranslator.getBulkDataRow(queryCoefficients, coefficientTableHeaders);
    ...
    ITask gridTaskData =
        ConstantTaskCreationHelper.getGridData(panelIds.get(1), labels, retCoefficientOutput);
    NounMetadata noun1 =
        new NounMetadata(
            gridTaskData, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA);
    ...
    this.rJavaTranslator.executeEmptyR(
        queryDataPoints + "$ROW_ID <- seq.int(nrow(" + queryDataPoints + "))");
    ...
    // move to java var
    List<Object[]> bulkRowDataPoints =
        this.rJavaTranslator.getBulkDataRow(queryDataPoints, dataTableHeaders);

    // create and return a task for the Actuals vs Fitted scatterplot
    ITask scatterTaskData =
        ConstantTaskCreationHelper.getScatterPlotData(
            panelIds.get(0), "ROW_ID", "Actual", "Fitted", bulkRowDataPoints);
    ...
    NounMetadata noun2 =
        new NounMetadata(
            scatterTaskData, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA);
    noun2.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage("Matrix regression ran successfully!"));
  ...
  private String runRLinearRegression(
      String frameName, String predictionCol, String[] retHeaders, Logger logger) {
    ...
    String regressionScriptFilePath =
        getBaseFolder() + "\\R\\AnalyticsRoutineScripts\\MatrixRegression.R";
    ...
    // R syntax for the routine: getRegressionCoefficientsFromScript("lm(y~x, data = frameName)",
    // frameName$PredictionCol)
    rsb.append(
        resultsListName
            + "<- fit_lm("
            + dataFrameTable
