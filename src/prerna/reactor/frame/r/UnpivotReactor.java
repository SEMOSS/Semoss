...
public class UnpivotReactor extends AbstractRFrameReactor {
  ...
  /**
   * This reactor unpivots columns so that the columns selected will be removed and combined to
   * generate 2 new columns "variable" and "value" "variable" - original column headers "value" -
   * value for original column header The inputs to the reactor are: 1) the columns to unpivot
   */
  ...
  public NounMetadata execute() {
    ...
    if (numColsToUnPivot > 0) {
      ...
      for (int colIndex = 0; colIndex < numColsToUnPivot; colIndex++) {
        ...
        if (colIndex + 1 < numColsToUnPivot) concatString = concatString + ", ";
      }
    ...
    String[] newColumns =
        headerChecker.cleanAndMatchColumnNumbers("variable_1", "value_1", allColumns);
