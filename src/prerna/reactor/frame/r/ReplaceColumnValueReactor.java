...
public class ReplaceColumnValueReactor extends AbstractRFrameReactor {
	...
	public NounMetadata execute() {
		...
		for (String column : columnNames) {
			...
			if (sType == SemossDataType.INT || sType == SemossDataType.DOUBLE) {
				...
				if (oldValue.isEmpty() || oldValue.equalsIgnoreCase("null") || oldValue.equalsIgnoreCase("na")
					...
					script.append(columnSelect + "[is.na(" + columnSelect + ")] <- as.Date(\""
							+ newD.getFormatted("yyyy-MM-dd") + "\");");
				} else {
					...
					script.append(columnSelect + "[" + columnSelect + " == " + QUOTE + oldD.getFormatted("yyyy-MM-dd")
							+ QUOTE + "] <- as.Date(" + QUOTE + newD.getFormatted("yyyy-MM-dd") + QUOTE + ");");
				}
				...
				if (oldValue.isEmpty() || oldValue.equalsIgnoreCase("null") || oldValue.equalsIgnoreCase("na")
					...
					script.append(columnSelect + "[is.na(" + columnSelect + ")] <- as.POSIXct(\""
							+ newD.getFormatted("yyyy-MM-dd HH:mm:ss") + "\");");
				} else {
					...
					script.append(columnSelect + "[" + columnSelect + " == " + QUOTE + oldD.getFormatted("yyyy-MM-dd")
							+ QUOTE + "] <- as.POSIXct(" + QUOTE + newD.getFormatted("yyyy-MM-dd HH:mm:ss") + QUOTE
							+ ");");
				}
				...
				if (oldValue.equalsIgnoreCase("null")) {
					...
					script.append(
							columnSelect + "[is.na(" + columnSelect + ")] <- " + QUOTE + escapedNewValue + QUOTE + ";");
				} else {
					// escape and update
					String escapedOldValue = oldValue.replace("\"", "\\\"");
					String escapedNewValue = newValue.replace("\"", "\\\"");
					script.append(columnSelect + "[" + columnSelect + " == " + QUOTE + escapedOldValue + QUOTE + "] <- "
		...
		// NEW TRACKING
		UserTrackerFactory.getInstance().trackAnalyticsWidget(this.insight, frame, "ReplaceColumnValue",
