...
public class CollectPivotReactor extends TaskBuilderReactor {
  ...
  public NounMetadata execute() {
    ...
    if (PyUtils.pyEnabled()) {
      // move this guy to py
      prerna.reactor.frame.py.CollectPivotReactor pyCollect =
      ...
      //  cubed <- data.table::cube(mv, .(budget=sum(MovieBudget), revenue=mean(RevenueDomestic)),
      // by=c('Genre', 'Studio'))
      ...
      pivoter.append(
          pivotName + " <- data.table::cube(" + fileName + "," + calcs + "," + rows + ");");
