...
public class ColumnCountReactor extends AbstractRFrameReactor {
  ...
  /**
   * This reactor counts the number of columns and unique columns it stores these values in a matrix
   * 1) column to use 2) boolean indicator (optional) if true (default), sort by descending
   * frequency of items in a column if false, sort ascending 3) panelId (defaults to zero if nothing
   * is entered)
   */
  ...
  public ColumnCountReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.COLUMN.getKey(), TOP, ReactorKeysEnum.PANEL.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    if (colType.equals("int") || colType.equals("num") || colType.equals("numeric")) {
      // this accounts for the case that the values are numeric - just make a histogram using the
      // histogram reactor
    ...
    // this script will create a table with one column of col vals and one column of the
    ...
    if (top) {
      ...
      script =
          tempName + " <-  head(" + table + "[, .N, by=\"" + column + "\"][order(rank(N)),] , 25);";
    }
    ...
    if (colType.equalsIgnoreCase("date")) {
      ...
      uniqueColumns =
          this.rJavaTranslator.getStringArray(
              "format(" + script + ", format='" + dateFormat + "')");
    } else {
    ...
    ITask taskData =
        ConstantTaskCreationHelper.getBarChartInfo(panelId, column, "Frequency", retOutput);
