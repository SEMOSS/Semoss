...
public class DiscretizeReactor extends AbstractRFrameReactor {
  /**
   * Discretize([{"column":"PetalLength"}, {"column":"SepalLength", "breaks":"(4.3, 5.5, 6.7, 7.9)",
   * "labels":"(Short,Medium,Long)"}, {"column":"PetalWidth", "breaks":"0:5*.5"}])
   * Discretize({"column":"MovieBudget", "numDigits":"10"}) Input keys: 1. column (required) 2.
   * breaks (conditionally required - req only if labels specified) - can be one of 3 types:
   * integer, breakpoints as a list, mathematical notation of range - breakpoints as a list or
   * mathematical notation of range need to be specified in ascending order 3. labels (optional) 4.
   * numDigits (optional) specifies number of digits used in formatting the break/range numbers
   *
   * <p>Return format: if labels is not specified, then the discretized ranges will be wrapped with
   * [ ] (inclusive) or () or a combination and contain the lower and upper range, comma separated:
   * ex. [0, 100) aka 0 =< x < 100
   */
  ...
  public NounMetadata execute() {
    ...
    for (int i = 0; i < reqList.size(); i++) {
      ...
      if (!Utility.isNumericType(dataType)) {
        throw new IllegalArgumentException(
            "Specified column name, " + name + ", must be a numeric type");
      }
      ...
      if (breaks == null || breaks.isEmpty()) {
        ...
        if (breaks != null && !breaks.isEmpty() && breaks.matches(".*[a-zA-z]+.*") == true) {
          throw new IllegalArgumentException(
              "Breaks should be either a numerical integer or a "
      ...
      if (labels != null && !labels.isEmpty()) {
        if (breaks == null || breaks.isEmpty() || breaks.matches(".*[a-zA-z]+.*") == true) {
          throw new IllegalArgumentException(
              "Please specify breaks (cannot contain "
          ...
          for (int j = 0; j < labelsList.size(); j++) {
            String jLabel =
                "'" + labelsList.get(j).replaceAll("\"", "").trim().replaceAll("\\s", "_") + "'";
      ...
      // validate that if numDigits specified AND labels is absent, then numDigits is a positive
      // integer > 0
      if (numDigitsStr == null || numDigitsStr.isEmpty() || isValidLabels == true) {
        ...
        try {
          ...
          if (numDigitsStr.replaceAll("[\\D]", "").matches("^[0-9]*[1-9][0-9]*$")) {
            ...
            throw new IllegalArgumentException(
                "Number of digits specified must be a positive integer.");
          }
    ...
    List<String> updatedDtColumns =
        new ArrayList<String>(Arrays.asList(this.rJavaTranslator.getColumns(dtName)));
        ...
        this.rJavaTranslator.runR(
            new StringBuilder()
                .append(
                    colLevels_R
                        + "<-"
                        + RSyntaxHelper.getOrderedLevelsFromRFactorCol(dtName, newColName))
                .toString());
    ...
    cleanUpScript.append(
        "rm("
            ...
            + ","
            + colLevels_R
            + ",discretizeColumnsDt, discretize, getNewColumnName);");
    ...
    noun.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage(
            "Successfully added discretized column: " + updatedDtColumns.get(0)));
    return noun;
  }
}
