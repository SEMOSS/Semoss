  ...
  public NounMetadata execute() {
    ...
    if (where == null || (!where.equals(LEFT) && !where.equals(RIGHT))) {
      throw new IllegalArgumentException(
          "Need to pass in 'where' to keep or remove characters (right or left)");
    }
    ...
    if (!replace) {
      ...
      if (!colExists) {
        NounMetadata error =
            NounMetadata.getErrorNounMessage("Unable to perform string extraction");
      ...
      metaData.setDataTypeToProperty(
          frameName + "__" + newColName, SemossDataType.STRING.toString());
    ...
    retNoun.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage("Successfully performed string extraction!"));
    return retNoun;
  }
