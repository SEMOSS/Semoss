...
/**
 * This class lets user profile the data in a frame. It analyzes the data in a frame and then
 * provides a comprehensive summary on Min, Max, Sum, count of blanks, count of unique values, Avg
 * and count of null values in each column.
 *
 * <p>The input to this routine will be the current frame and a new frame will be the output with
 * the profiling data.
 */
public class FrameProfileReactor extends AbstractFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error occurred trying to create frame of type "
              + DataFrameTypeEnum.GRID.getTypeAsString(),
          e);
    }
    ...
    ImportUtility.parseTableColumnsAndTypesToFlatTable(
        newFrameMetadata, newHeaders, dataTypes, newTableName);
    ...
    classLogger.info(
        "New Frame created with alias and has been set in the insight**********" + newFrameAlias);
   ...
   * @return
   */
  private String[] getNumericalProfileDataOnFrame(
      ITableDataFrame frame, String tableName, String colName) {
    // ----FORMAT------
    // "table_name", "column_name", "numOfBlanks", "numOfUniqueValues", "minValue", "averageValue",
    // "maxValue", "sumValue", "numOfNullValues"
   ...
   * @return
   */
  private String[] getStringProfileDataOnFrame(
      ITableDataFrame frame, String tableName, String colName) {
    // ----FORMAT------
    // "table_name", "column_name", "numOfBlanks", "numOfUniqueValues", "minValue", "averageValue",
    // "maxValue", "sumValue", "numOfNullValues"
  ...
  private String getNumOfNullValues(ITableDataFrame frame, String colName) {
    ...
    SimpleQueryFilter stringNullFilter =
        new SimpleQueryFilter(
            new NounMetadata(col, PixelDataType.COLUMN),
            DOUBLE_EQUALS,
            new NounMetadata("null", PixelDataType.NULL_VALUE));
  ...
  private void computeNumericalProfilingData(String[] row, ITableDataFrame frame, String colName) {
    ...
    try {
      ...
      if (resultWrapper.hasNext()) {
        ...
        classLogger.info(
            "Wrapper for calculating unique/min/max/avg/sum is empty for column name " + colName);
      }
    } catch (Exception e) {
      classLogger.warn("Exception during execution of query." + e.getMessage());
      classLogger.error(Constants.STACKTRACE, e);
   ...
   * @return
   */
  private static QueryFunctionSelector getUniqueCountFunctionSelector(
      QueryColumnSelector innerSelector, String colName) {
   ...
   * @return
   */
  private static QueryFunctionSelector getMinFunctionSelector(
      QueryColumnSelector innerSelector, String colName) {
   ...
   * @return
   */
  private static QueryFunctionSelector getAvgFunctionSelector(
      QueryColumnSelector innerSelector, String colName) {
   ...
   * @return
   */
  private static QueryFunctionSelector getMaxFunctionSelector(
      QueryColumnSelector innerSelector, String colName) {
   ...
   * @return
   */
  private static QueryFunctionSelector getSumFunctionSelector(
      QueryColumnSelector innerSelector, String colName) {
