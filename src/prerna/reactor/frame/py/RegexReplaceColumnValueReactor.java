...
public class RegexReplaceColumnValueReactor extends AbstractPyFrameReactor {
  ...
  /**
   * This reactor updates row values based on a regex It replaces all portions of the current cell
   * value that is an exact match to the input value The inputs to the reactor are: 1) the column to
   * update 2) the regex to look for 3) value to replace the regex with
   */
  ...
  public NounMetadata execute() {
    ...
    for (int i = 0; i < numColumns; i++) {
      ...
      if (sType == SemossDataType.INT || sType == SemossDataType.DOUBLE) {
        ...
        //				if(!NUMERIC_PATTERN.matcher(newValue).matches()) {
        //					throw new IllegalArgumentException("Cannot update a date field to non-numeric
        // values");
        //				}
