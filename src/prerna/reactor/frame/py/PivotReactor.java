...
public class PivotReactor extends AbstractPyFrameReactor {
  ...
  /**
   * This reactor pivots a column so that the unique values will be transformed into new headers The
   * inputs to the reactor are: 1) the column to pivot 2) the column to turn into values for the
   * selected pivot column 3) the aggregate function 4) the other columns to maintain 5) the
   * optional no value replace
   */
  ...
  public PivotReactor() {
    this.keysToGet =
        new String[] {
          PIVOT_COLUMN_KEY,
          VALUE_COLUMN_KEY,
          ReactorKeysEnum.MAINTAIN_COLUMNS.getKey(),
  ...
  public NounMetadata execute() {
    ...
    boolean canAgg =
        (boolean) frame.runScript(frame.getWrapperName() + ".is_numeric('" + valuesCol + "')");
    if (canAgg) {
      ...
      List<String> headerList =
          (List<String>) frame.runScript(PandasSyntaxHelper.getColumns(table));
  ...
  private String getValuesCol() {
    ...
    throw new IllegalArgumentException(
        "Need to define column to turn into values for the selected pivot column");
  }
