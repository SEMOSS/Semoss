...
public class DateExpanderReactor extends AbstractPyFrameReactor {
  ...
  public DateExpanderReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.COLUMN.getKey(), ReactorKeysEnum.OPTIONS.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    // also re add the options again for a list where we will remove to make sure they were properly
    // added
    ...
    if (operationsNotAdded.size() == numColumnsToAdd) {
      ...
      warning =
          NounMetadata.getWarningNounMessage(
              "The following operations were not appended: " + options.toString());
    }
    ...
    for (int i = 0; i < endColumnsList.size(); i++) {
      ...
      metaData.setDataTypeToProperty(
          table + "__" + newColName, addedColumnDataType.get(newColName));
