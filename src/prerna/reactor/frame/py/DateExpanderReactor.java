...
public class DateExpanderReactor extends AbstractPyFrameReactor {
	...
	public NounMetadata execute() {
		...
		// also re add the options again for a list where we will remove to make sure
		// they were properly
		// added
		...
		if (operationsNotAdded.size() == numColumnsToAdd) {
			...
			warning = NounMetadata
					.getWarningNounMessage("The following operations were not appended: " + options.toString());
		}
		...
		if (warning != null) {
			...
			retNoun.addAdditionalReturn(
					NounMetadata.getSuccessNounMessage("Successfully extracted details from " + srcCol));
		}
