...
public class RankReactor extends AbstractPyFrameReactor {
  ...
  /**
   * This reactor ranks the data based on a given column(s) and sort direction. The inputs to the
   * reactor are: 1) the column(s) to be used for rank 2) the name of the rank column 3) the sorting
   * order for each column 4) the partition column's to be used for rank
   */
  ...
  public NounMetadata execute() {
    ...
    if (!partitionbyCols.isEmpty()) {
      ...
      tempRankScript
          ...
          .append("['TempRank'] =")
          .append(frame.getName())
          .append(".sort_values(cols,")
          .append(" ascending=")
          .append(sortValues)
          .append(")")
          .append(".groupby(")
          .append("cols,")
          .append("sort=False, dropna=True).ngroup()");
      ...
      finalRankScript
          ...
          .append(frame.getName())
          .append(".groupby(")
          .append("[" + partitionByScript + "]")
          .append(")['TempRank'].transform('min') + 1");
      ...
      for (int i = 0; i < columns.size(); i++) {
        ...
        rankScript
            ...
            .append(getSortOrder(i, ReactorKeysEnum.SORT.getKey()))
            .append(")");
      ...
      finalRankScript
          ...
          .append(frame.getName())
          .append(
              ".sort_values(cols, ascending=True).groupby(cols, sort=False,dropna=True).ngroup() + 1");
  ...
  private String getSortOrder(int index, String key) {
    ...
    if (grs == null || grs.isEmpty() || index >= grs.size()) {
      ...
      if (!grs.get(index).toString().isEmpty()
          && grs.get(index).toString() != null
        ...
        // will be descending
        if (grs.get(index).toString().equalsIgnoreCase(ASC)
            || grs.get(index).toString().isEmpty()) {
          return "True";
        } else {
          return "False";
        }
