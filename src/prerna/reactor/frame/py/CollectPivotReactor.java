...
public class CollectPivotReactor extends TaskBuilderReactor {
  ...
  public CollectPivotReactor() {
    this.keysToGet =
        new String[] {
          ...
          ReactorKeysEnum.SUBTOTALS.getKey(),
          "json",
          "margins",
          "sections",
          "optional"
  ...
  public NounMetadata execute() {
    ...
    // Frame ( frame = [ FRAME890385 ] ) | Select ( Genre , Studio, MovieBudget ) .as ( [ Genre ,
    // Studio, MovieBudget ] ) | CollectPivot( rowGroups=["Genre"], columns=["Studio"],
    // values=["sum(MovieBudget)"] ) ;
    // pandas format is - pd.pivot_table(mv, index=['Genre', 'Nominated'], values=['MovieBudget',
    // 'RevenueDomestic'], aggfunc={'MovieBudget':np.sum, 'RevenueDomestic':np.mean},
    // columns='Studio')
    ...
    if (task instanceof BasicIteratorTask && queryFrame instanceof PandasFrame) {
      ...
      // generate the script
      makePivotFrame =
          PandasSyntaxHelper.getCsvFileRead(
              PandasFrame.PANDAS_IMPORT_VAR,
              PandasFrame.NUMPY_IMPORT_VAR,
              outputFile,
              pivotFrameName,
              ",",
              "\"",
    ...
    for (int valIndex = 0; valIndex < values.size(); valIndex++) {
      ...
      if (curValue.contains("(")) {
        ...
        String operand =
            curValue.substring(curValue.indexOf("(") + 1, curValue.length() - 1).trim();
    ...
    if (sections == null) {
      ...
      String sectionNames =
          pivotFrameName + "[['" + sections.get(0) + "']].drop_duplicates().to_dict('list')";
  ...
  private NounMetadata checkPivotLimits(
      String frameName, List<String> colGroups, List<String> pivotTableNames) {
    getPivotLimits();
  ...
  private String getQuote(String columnName) {
    ...
    for (Map<String, Object> headerMap : headersInfo) {
      ...
      SemossDataType type =
          SemossDataType.convertStringToDataType(headerMap.get("type").toString());
      if (name.equalsIgnoreCase(columnName)) {
        if (type == SemossDataType.INT || type == SemossDataType.DOUBLE) quote = "";
        break;
      }
    }
    return quote;
  ...
  public String genSections(
    ...
    // mv[['Genre']].drop_duplicates().to_dict('list') - Values of this list is an array and for
    ...
    if (!sections.get(0).equalsIgnoreCase(ALL_SECTIONS)) {
      for (int sectionIndex = 0; sectionIndex < sections.size(); sectionIndex++) {
        ...
        allSections
            ...
            .append("'] == ")
            .append(quote)
            .append(thisSectionValue)
            .append(quote)
            .append("]")
            .append(NEW_LINE)
            .append(
                genPivot(
                    sectionSpecificFrame,
                    rows,
                    columns,
                    subtotalColumns,
                    values,
  ...
  public String genPivot(
    ...
    StringBuilder colString =
        new StringBuilder(""); // we dont need colstring anymore for pivot. We append it to rows
    ...
    if (funString.length() > 0) {
      ...
      crosstabAgg =
          new StringBuilder("aggfunc = ")
              .append(crosstabAgg)
              .append(", "); // need to put margins finally
    }
    ...
    if (rows.size() == 1) labelsCheat = "Row Total";
    ...
    // 2 decimal places
    retString
        .append("pd.set_option('display.float_format', lambda x: '%.2f' % x)")
        .append(NEW_LINE);
    ...
    retString
        ...
        .append(outputFormat)
        .append(")")
        .append(NEW_LINE); // .append("del(").append(pivotName).append(")");
  ...
  private String generateTotals(
    ...
    */
    ...
    if (rows.size() > 1) {
      ...
      // aftgto = ahF34A.append(sapwSx8.assign(gender='', location= 'zzzzzz').set_index(['gender',
      ...
      totalString.append(
          rowTotal
              ...
              + ".sum(level=('"
              + rows.get(0)
              + "'))"); // this is multiple levels - you have to always do 1 less
      ...
      totalString.append(
          rowTotal + ".assign(" + rowSumColumnAdderString + ")"); // create a new column
      totalString.append(
          ".set_index("
              + rowSumColumnIndexString
              + ", append=True).sort_index())"); // add this column as index and sort it

      String grandTotal = "g" + Utility.getRandomString(5);
      totalString
          .append(NEW_LINE)
          .append(grandTotal + " = " + pivotName + "[" + valueString + "].sum()");
      ...
      for (int valIndex = 0; valIndex < values.size(); valIndex++) {
        ...
        if (valIndex == 0) totalValueIndex.append("'zzzzpp'");
        else totalValueIndex.append("''");
      ...
      // aDzHlB = aDzHlB.append(pd.DataFrame([gapHkVh.values], columns=gapHkVh.index, index =
      // pd.MultiIndex.from_tuples([('zzzzpp', '')], names = ['id', 'age']))).fillna('')
      if (values.size() > 1) {
      ...
      rowString.append("]");
      totalString
          .append(finalPivotName)
          .append(" = ")
          .append(finalPivotName)
          .append(".sort_index(level=")
          .append(rowString)
          .append(")");

      // now replace the indices
      totalString.append(NEW_LINE);
      // fpiv1.index = pd.MultiIndex.from_tuples([(x[0].replace('zzzzpp', 'Row Total'),
      // x[1].replace('zzzzzzzz', 'Total')) for x in fpiv1.index], names=fpiv1.index.names)

      totalString.append(finalPivotName).append(".index = ");
      totalString
          .append("pd.MultiIndex.from_tuples([")
          .append(rowTotalReplacer)
          .append(" for x in ")
          .append(finalPivotName)
          .append(".index], names = ")
          .append(finalPivotName)
  ...
  private long getCount(String frameName, List<String> items) {
    ...
    for (int itemIndex = 0; itemIndex < items.size(); itemIndex++) {
      StringBuilder sb =
          new StringBuilder(frameName)
              .append("['")
              .append(items.get(itemIndex))
              .append("'].nunique()");
  ...
  public static String getJson2HTML(JSONObject mainObj, List<String> rows) {
    ...
    for (int dataIndex = 0; dataIndex < data.length(); dataIndex++) {
      ...
      for (int recIndex = 0; recIndex < record.length(); recIndex++) {
        ...
        else dataArray[dataIndex][recIndex] = 0;
      }
    }
    ...
    for (int columnIndex = 0; columnIndex < colArray.length(); columnIndex++) {
      if (colArray.get(columnIndex) instanceof JSONArray) // this is a multi level
      {
        ...
        for (int colLevelIndex = 0; colLevelIndex < thisLevel.length(); colLevelIndex++) {
          ...
          if (parent.length() > 0) key = parent + ":" + key;
    ...
    String[][] rowDataArray =
        new String[index.length()]
            [rows.size()
                + values.length
                + 1]; // this keeyps track of actual parent etc. required when we print
    ...
    for (int tdIndex = 0; tdIndex < rows.size(); tdIndex++)
      ...
      outputString.append(
          "<th style=\"width:200px;background-color:#F6F6F6;color:#1E1E1E;\">"
              + rows.get(tdIndex)
              + "</th>");
    // fill other tds
    for (int tdIndex = rows.size(); tdIndex < columns[0].length; tdIndex++)
      ...
      outputString.append(
          "<th style=\"width:200px;background-color:#F6F6F6;color:#1E1E1E;\"></th>");
    ...
    for (int trIndex = 0; trIndex < rowDataArrayOutput.length; trIndex++) {
      ...
      for (int tdIndex = 0; tdIndex < thisRow.length; tdIndex++) {
        ...
        if (tdIndex == 0
            && curLevelItem != null
            && !dataItem.equalsIgnoreCase(curLevelItem)) // logic for doing totals
        {
          newItem = true;
          // add the total for this column
          // System.err.println("<td colspan=" + (thisRow.length - (values.length + 1)) +">");
          outputString.append(
              "<th style=\"background-color:#F6F6F6;color:#1E1E1E;\" colspan="
          ...
          if (totals != null) {
            for (int totalIndex = 0; totalIndex < totals.length; totalIndex++) {
              ...
              outputString.append(
                  "<td style=\"font-weight:bold;\">" + totals[totalIndex] + "</td>");
            }
          }
        ...
        if (thisItem != null && thisItem.length() > 0) {
          ...
          // System.err.println("</td>");
          outputString.append(thisItem);
          if (tdIndex < rows.size()) outputString.append("</th>");
          else outputString.append("</td>");
        }
        parent = key;
      }
    ...
    if (curLevelItem != null) {
      ...
      // System.err.println(curLevelItem + " -- TOTAL </td>");
      outputString.append(
          "<th style=\"width:200px;background-color:#F6F6F6;color:#1E1E1E;\" colspan="
    ...
    // ">All Total</td>");
    outputString.append(
        "<th style=\"background-color:#F6F6F6;color:#1E1E1E;font-weight:bold;\" colspan="
            + (rowDataArrayOutput[0].length - (values.length + 1))
            + ">All Total</th>");
