...
public class PyPlotReactor extends AbstractPyFrameReactor implements ICodeExecution {
	...
	public NounMetadata handlePyPlot(String command) {
		...
		// there is a third scenario that needs to be addressed i..e when the frame type
		// is R
		...
		if (bin.length() > 0) {
			...
			if (newWindow) {
				...
				retList.add(noun);
				retList.add(new NounMetadata(cdt, PixelDataType.FORMATTED_DATA_SET, PixelOperationType.TASK_DATA,
