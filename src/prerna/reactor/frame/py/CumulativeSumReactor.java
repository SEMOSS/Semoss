...
public class CumulativeSumReactor extends AbstractPyFrameReactor {
  ...
  /**
   * This reactor performs the cumulative sum by grouping data in a columns Input Keys are as
   * follows: 0) newCol = Name of the new column being created 1) value = The instance value in a
   * column, or the numeric or string value used in a operation 2) groupByCols = List of columns
   * used to groupBy cumulative sum 3) sortCols = List of Columns used to sort data 4) sort = Sort
  ...
  public NounMetadata execute() {
    ...
    // TODO determine if the value column datatype is int or double this will define the new column
    // datatype
    ...
    String script =
        newColumnSelector + "= " + frameName + groupBySyntax + "['" + value + "'].cumsum()";
    frame.runScript(script);
    this.addExecutedCode(script);

    // check if operation was successful
    boolean success =
        this.insight.getPyTranslator().getBoolean("'" + newColName + "' in " + frameName);
    if (!success) {
    ...
    retNoun.addAdditionalReturn(
        new AddHeaderNounMetadata(newColName)); // need this to show newly added column
    retNoun.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage("Successfully performed Cumulative Sum."));
    return retNoun;
  }
