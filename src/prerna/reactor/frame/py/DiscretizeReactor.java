...
public class DiscretizeReactor extends AbstractPyFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    List<String> colNames =
        Arrays.asList(
            frame.getColumnHeaders()); // check if this is the same as R's frame.getColumnNames()
    ...
    if (!updatedDtColumns.isEmpty()) {
      for (String newColName : updatedDtColumns) {
        ...
        // R's DiscretizeReactor calls getOrderedLevelsFromRFactorCol(), not bothering with that
        // here
      }
    } else {
      // no results
