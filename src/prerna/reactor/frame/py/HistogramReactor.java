...
public class HistogramReactor extends AbstractFrameReactor {
  ...
  /**
   * This reactor gets a histogram The inputs to the reactor are: 1) the column to base the
   * histogram on 2) the number of breaks 3) the panel id - defaults to zero if nothing is entered
   */
  public HistogramReactor() {
    this.keysToGet =
        new String[] {
          ReactorKeysEnum.COLUMN.getKey(),
          ReactorKeysEnum.BREAKS.getKey(),
  ...
  protected NounMetadata getHistogram(
      PandasFrame frame, String table, String column, String panelId, int numBreaks) {
    ...
    if (numBreaks > 1) {
      ...
      //			np.histogram( FRAME940921[~np.isnan(FRAME940921['MovieBudget'])]['MovieBudget'],
      // bins='auto')
      script
          .append("np.histogram(")
          .append(table)
