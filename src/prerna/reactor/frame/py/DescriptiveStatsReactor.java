...
public class DescriptiveStatsReactor extends AbstractFrameReactor {
  ...
  /**
   * This reactor gets statistics for a column 1) column to get stats on 2) panelId (defaults to
   * zero if no panel id is entered)
   */
  ...
  public NounMetadata execute() {
    ...
    ArrayList sum_median =
        (ArrayList) frame.runScript(wrapperFrameName + ".sum_median('" + column + "')");
    ...
    ITask taskData =
        ConstantTaskCreationHelper.getBarChartInfo(panelId, column, "StatOutput", retOutput);
