...
public class NLPQueryReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		if (Utility.getDIHelperProperty("HF_SEQ_2_SQL") != null) {
			// force the model
			model = Utility.getDIHelperProperty("HF_SEQ_2_SQL");
		}

		String modelVarName = Utility.cleanString(model, true);
		modelVarName = modelVarName.replaceAll("-", "_");

		String pipeVar = modelVarName; // setting up so that I dont need to load same model multiple times during the
										// same session

		boolean modelLoaded = this.insight.getPyTranslator().getBoolean("'" + modelVarName + "' in globals()");
		if (!modelLoaded) // load the model
		{
			// load the model
			// pipeVar = Utility.getRandomString(5);
			String cacheFolder = Utility.getDIHelperProperty(Settings.HF_CACHE_DIR);
			if (cacheFolder == null) {
				return NounMetadata
						.getErrorNounMessage("Hugging Face Cache is not set. Please set it RDF Map HF_CACHE_DIR");
			}
			this.insight.getPyTranslator().runScript(modelVarName + " = smssutil.load_hugging_face_model('" + model
					+ "', 'text2text-generation', '" + cacheFolder + "')");
		...
		// create the string
		// set into the frames
		// format - [question] | [db_id] | [table] : [column] ( [content] , [content] )
		// , [column] ( ... ) , [...] | [table] : ... | ...
		// example - concert_singer | stadium : stadium_id, location, name, capacity,
		// highest, lowest, average | singer : singer_id, name, country,
		...
		while (frameIterator.hasNext()) {
			...
			if (thisFrame instanceof PandasFrame) {
				Object output = this.insight.getPyTranslator()
						.runScript(thisFrame.getName() + "w.get_categorical_values()");
				if (output instanceof HashMap) {
					columnValues = (HashMap) this.insight.getPyTranslator()
							.runDirectPy(thisFrame.getName() + "w.get_categorical_values()");
				}
			}
		...
		boolean frameCreated = insight.getPyTranslator().getBoolean("'" + frameName + "' in globals()");
		...
		if (frameCreated) {
			...
			outputs.add(new NounMetadata("Query Generated : " + sqlDFQuery + " Data : " + frameName,
					PixelDataType.CONST_STRING));
			outputs.add(new NounMetadata(this.insight.getPyTranslator().runDirectPy(frameName + ".head(20)"),
					PixelDataType.CONST_STRING));
			outputs.add(new NounMetadata(
					"To start working with this frame  GenerateFrameFrom" + frameType + "Variable('" + frameName + "')",
					PixelDataType.CONST_STRING));

			return new NounMetadata(outputs, PixelDataType.CODE, PixelOperationType.CODE_EXECUTION);
		} else {
			return new NounMetadata("Could not compute the result / query invalid -- " + sqlDFQuery,
					PixelDataType.CONST_STRING);
		}
	}
