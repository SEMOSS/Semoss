...
public class NLPQueryReactor extends AbstractReactor {
  ...
  public NLPQueryReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.COMMAND.getKey(), ReactorKeysEnum.MODEL.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    if (keyValue.containsKey(keysToGet[1])) model = keyValue.get(keysToGet[1]);
    ...
    String pipeVar =
        modelVarName; // setting up so that I dont need to load same model multiple times during the
    // same session

    boolean modelLoaded =
        this.insight.getPyTranslator().getBoolean("'" + modelVarName + "' in globals()");
    if (!modelLoaded) // load the model
    {
      ...
      if (cacheFolder == null)
        return NounMetadata.getErrorNounMessage(
            "Hugging Face Cache is not set. Please set it RDF Map HF_CACHE_DIR");
      this.insight
          .getPyTranslator()
          .runScript(
              modelVarName
                  + " = smssutil.load_hugging_face_model('"
                  + model
    ...
    // create the string
    // set into the frames
    // format - [question] | [db_id] | [table] : [column] ( [content] , [content] ) , [column] ( ...
    // ) , [...] | [table] : ... | ...
    // example - concert_singer | stadium : stadium_id, location, name, capacity, highest, lowest,
    ...
    if (frameCreated) {
      ...
      outputs.add(
          new NounMetadata(
              "Query Generated : " + sqlDFQuery + " Data : " + frameName,
              PixelDataType.CONST_STRING));
      outputs.add(
          new NounMetadata(
              this.insight.getPyTranslator().runDirectPy(frameName + ".head(20)"),
              PixelDataType.CONST_STRING));
      outputs.add(
          new NounMetadata(
              "To start working with this frame  GenerateFrameFrom"
                  + frameType
                  + "Variable('"
                  + frameName
                  + "')",
      ...
      return new NounMetadata(
          "Could not compute the result / query invalid -- " + sqlDFQuery,
          PixelDataType.CONST_STRING);
    }
  }
}
