...
public class MooseChatReactor extends AbstractGaasBaseReactor {
	...
	public MooseChatReactor() {
		this.keysToGet = new String[] { ReactorKeysEnum.COMMAND.getKey(), ReactorKeysEnum.PROJECT.getKey(),
				ReactorKeysEnum.MODEL.getKey(), Constants.MOOSE_ENDPOINT, ReactorKeysEnum.CONTEXT.getKey(),
				ReactorKeysEnum.CONTENT_LENGTH.getKey(), ReactorKeysEnum.PARAM_VALUES_MAP.getKey() };
	...
	public NounMetadata execute() {
		...
		organizeKeys();
		String model = Utility.getDIHelperProperty(Constants.MOOSE_MODEL);
		...
		if (projectId == null) {
			if (model.equalsIgnoreCase("alpaca")) {
				...
				String answer = (String) insight.getPyTranslator().runScript(
						"smssutil.run_alpaca(\"" + template + "\", " + maxTokens + " ,\" " + endpoint.trim() + "\")");
			...
			if (model.equalsIgnoreCase("guanaco")) {
				...
				String pyCommand = function + "(context= " + context + ", " + "question=\"" + template + "\", "
						+ "max_new_tokens=" + maxTokens + "," + "client=" + client_name + paramString + ")";
