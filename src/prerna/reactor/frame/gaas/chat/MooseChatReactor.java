...
public class MooseChatReactor extends AbstractGaasBaseReactor {
  ...
  public NounMetadata execute() {
    ...
    if (keyValue.containsKey(keysToGet[2])) model = keyValue.get(keysToGet[2]);

    String context = "\"\"";
    if (keyValue.containsKey(keysToGet[4])) context = "\"" + keyValue.get(keysToGet[4]) + "\"";
    ...
    if (paramMap != null) paramString = ", " + processMapToString(paramMap);
    ...
    if (projectId == null) {
      ...
      if (model.equalsIgnoreCase("guanaco")) {
        ...
        insight
            .getPyTranslator()
            .runScript(client_name + " = Client('" + endpoint + "', timeout=60)");
        ...
        if (answer instanceof HashMap) answer = ((HashMap) answer).get("response");
