...
public class NLPQuery3Reactor extends AbstractFrameReactor {
	...
	public NounMetadata execute() {
		...
		for (ITableDataFrame thisFrame : theseFrames) {
			...
			// finalDbString.append("CREATE TABLE
			// ").append(thisFrame.getName()).append("(");
			...
			while (columns.hasNext()) {
				...
				// finalDbString.append(thisColumn).append("
				// ").append(colTypeString).append(",");
			...
			// finalDbString.append(". Be Concise. Provide as markdown. Output should start
			// and end with
			// ``` markdown.");
			//
			finalDbString2.append(")\n\n");
