...
public class NLPQuery2Reactor extends AbstractFrameReactor {
	...
	public NounMetadata execute() {
		...
		String model = Utility.getDIHelperProperty(Constants.SQL_MOOSE_MODEL);
		...
		/*
		 * 
		 * ### SQL tables, with their properties: # # Employee(id, name, department_id)
		 * # Department(id, name, address) # Salary_Payments(id, employee_id, amount,
		 * date) # ### A query to list the names of employees in department sales living
		 * in virginia who make more than 10000
		 * 
		 */
		// use \n to separate
		...
		for (ITableDataFrame thisFrame : theseFrames) {
			...
			if (model.equalsIgnoreCase("alpaca")) {
				...
				// finalDbString.append("Write SQLite query to answer the following question
			...
			Object output = null;
			if (model.equalsIgnoreCase("alpaca")) {
				String endpoint = Utility.getDIHelperProperty(Constants.MOOSE_ENDPOINT);
				if (endpoint == null || endpoint.trim().isEmpty()) {
					throw new IllegalArgumentException("Must define endpoint to run custom models");
				}
				output = insight.getPyTranslator().runScript("smssutil.run_alpaca(\"" + finalDbString + "\", "
						+ maxTokens + " ,\"" + endpoint.trim() + "\")");
			} else if (model.equalsIgnoreCase("run_guanaco")) {
				// create the client first

				// create instruction and get response back
				String endpoint = Utility.getDIHelperProperty(Constants.GUANACO_ENDPOINT);
				if (endpoint == null || endpoint.trim().isEmpty()) {
					throw new IllegalArgumentException("Must define endpoint to run custom models");
				}
				output = insight.getPyTranslator().runScript("smssutil.run_guanaco(\"" + finalDbString + "\", "
						+ maxTokens + " ,\"" + endpoint.trim() + "\")");
			} else if (model.equalsIgnoreCase("chat_guanaco")) {
				String endpoint = Utility.getDIHelperProperty(Constants.GUANACO_ENDPOINT);
				if (endpoint == null || endpoint.trim().isEmpty()) {
				...
				String command = "smssutil.chat_guanaco_code(context=\"" + finalDbString + "\", " + "question=\""
						+ finalQuery + "\"," + "max_new_tokens=" + maxTokens + "," + "client=" + clientName + ")";
				...
				output = insight.getPyTranslator()
						.runScript("smssutil.run_gpt_3(\"" + finalDbString + "\", " + maxTokens + ")");
			} else {
				Map params = new HashMap();
				params.put("temperature", 0.66);
				Map<String, Object> modelOutput = engine.ask(finalDbString + "", null, this.insight, params).toMap();
	...
	private void processSQL(String sql) {
		sql = "SELECT actor_name, title, gender" + " FROM actor" + " WHERE title = 'a' and gender > avg(age) "
				+ " and name > a+b and " + "title IN (SELECT title" + "                FROM mv"
		...
		try {
			...
			// need to walk the gen expression and figure out if there are subqueries and
			// start assimilating
			...
			// if it is an IN filter then the right hand side could possibly be a query
			// struct (like above)
	...
	private Object processFilter(GenExpression filter, IQueryFilter parentFilter) {
		...
		if (filter != null) {
			...
			if (filter.operation.equalsIgnoreCase("=") || filter.operation.equalsIgnoreCase(">")
					...
					|| filter.operation.equalsIgnoreCase("<=") || filter.operation.equalsIgnoreCase("AND")
				...
				if (parentFilter == null) {
					if (filter.operation.equalsIgnoreCase("AND")) {
						...
						parentFilter = new OrQueryFilter();
							...
							|| filter.operation.equalsIgnoreCase("<") || filter.operation.equalsIgnoreCase(">=")
							|| filter.operation.equalsIgnoreCase("<=")) || filter.operation.equalsIgnoreCase("IN")) {
						NounMetadata replacer = new NounMetadata("replacer", PixelDataType.CONST_STRING);
						parentFilter = new SimpleQueryFilter(replacer, "***", replacer); // I will replace these values
																							// shortly.
					}
				...
				if (filter.paranthesis && !root) // start of a new world
				{
					if (filter.operation.equalsIgnoreCase("AND")) {
						...
						newParentFilter = new OrQueryFilter();
							...
							|| filter.operation.equalsIgnoreCase("<=")) || filter.operation.equalsIgnoreCase("IN")) {
						NounMetadata replacer = new NounMetadata("replacer", PixelDataType.CONST_STRING);
						newParentFilter = new SimpleQueryFilter(replacer, "***", replacer); // I will replace these
																							// values shortly.
					}
				...
				// also address the regular case where this is not an and
						...
						|| filter.operation.equalsIgnoreCase("<=")) || filter.operation.equalsIgnoreCase("IN")) {
					NounMetadata replacer = new NounMetadata("replacer", PixelDataType.CONST_STRING);
					newParentFilter = new SimpleQueryFilter(replacer, "***", replacer); // I will replace these values
																						// shortly.
