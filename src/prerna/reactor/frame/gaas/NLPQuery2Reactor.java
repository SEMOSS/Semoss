...
public class NLPQuery2Reactor extends AbstractFrameReactor {
    ...
    */
    ...
    for (ITableDataFrame thisFrame : theseFrames) {
      ...
      if (model.equalsIgnoreCase("alpaca")) {
        ...
        // finalDbString.append("Write SQLite query to answer the following question given the
        // database schema. Please wrap your code answer using ```: Schema: ");
        ...
        while (columns.hasNext()) {
          ...
          if (colType == SemossDataType.STRING) colTypeString = "TEXT";
      ...
      if (model.equalsIgnoreCase("alpaca")) {
        ...
        hasClientName =
            insight.getPyTranslator().runScript("print('" + clientName + "' in locals())") + "";
        ...
        String command =
            "smssutil.chat_guanaco_code(context=\""
                + finalDbString
                + "\", "
                + "question=\""
                + finalQuery
        ...
        if (output instanceof HashMap) output = ((HashMap) output).get("response");
        ...
        output =
            insight
                .getPyTranslator()
                .runScript("smssutil.run_gpt_3(\"" + finalDbString + "\", " + maxTokens + ")");
      } else {
        Map params = new HashMap();
        params.put("temperature", 0.66);
        Map<String, Object> modelOutput =
            engine.ask(finalDbString + "", null, this.insight, params).toMap();
        ...
        if (start >= 0) response = response.substring(start + markdown.length());
        // get the select also
      ...
      // b. See the constants and change the value based on the appropriate value the column has -
      // you can circumvent this by giving value in quotes
      ...
      if (thisFrame instanceof PandasFrame) {
        ...
        String frameMaker =
            frameName + " = pd.read_sql(\"" + sqlDFQuery + "\", " + sqliteName + ")";
        classLogger.info("Creating frame with query..  " + sqlDFQuery + " <<>> " + frameMaker);
        insight.getPyTranslator().runEmptyPy(frameMaker);
        String sampleOut =
            insight.getPyTranslator().runScript(frameName + ".head(20)") + ""; // load the sql df
        ...
        if (sampleOut != null && sampleOut.length() > 0) {
          if (json) {
            outputMap.put(
                ReactorKeysEnum.FRAME_TYPE.getKey(), DataFrameTypeEnum.PYTHON.getTypeAsString());
            ...
            outputString.append("\n");
            retListForFrames.add(
                new NounMetadata(outputString.toString(), PixelDataType.CONST_STRING));
          }
        } else {
          if (json) {
            outputMap.put("Query", sqlDFQuery);
        ...
        boolean frameCreated =
            rt.runRAndReturnOutput("exists('" + frameName + "')").toUpperCase().contains("TRUE");

        if (frameCreated) {
          String sampleOut = rt.runRAndReturnOutput("head(" + frameName + ", 20)");
          if (json) {
            outputMap.put(
                ReactorKeysEnum.FRAME_TYPE.getKey(), DataFrameTypeEnum.R.getTypeAsString());
            outputMap.put("Query", sqlDFQuery);
            outputMap.put(ReactorKeysEnum.FRAME.getKey(), frameName);
            outputMap.put("SAMPLE", sampleOut);
            ...
            outputString.append("\n");
            outputString.append(
                "To start working with this frame  GenerateFrameFromRVariable('"
                    + frameName
                    + "')");
            retListForFrames.add(
                new NounMetadata(outputString.toString(), PixelDataType.CONST_STRING));
        ...
        String newQuery =
            sqlDFQuery.replace(
                thisFrame.getName(), "(" + baseQuery + ") as " + thisFrame.getName());
        ...
        try {
          ...
          if (json) {
            outputMap.put(
                ReactorKeysEnum.FRAME_TYPE.getKey(), DataFrameTypeEnum.NATIVE.getTypeAsString());
        ...
        try {
          ...
          if (json) {
            outputMap.put(
                ReactorKeysEnum.FRAME_TYPE.getKey(), DataFrameTypeEnum.GRID.getTypeAsString());
            outputMap.put("Query", sqlDFQuery);
            outputMap.put(ReactorKeysEnum.FRAME.getKey(), frameName);
            outputMap.put("SAMPLE", sampleOut.toString());
        ...
        retListForFrames.add(
            getError(
                "NLP Query 2 has only been implemented for python, r, grid, and native frame at this point, please convert your frames to python,r and try again"));
      }
    }
    ...
    try {
      ...
      if (!allColumns) {
        for (int selectorIndex = 0;
            selectorIndex < columnHeaders.length && sameColumns;
            selectorIndex++) // going to run a dual for loop here
        {
          ...
          for (int newColumnIndex = 0; newColumnIndex < selects.size(); newColumnIndex++) {
            ...
            if (alias == null) alias = thisSelector.getLeftExpr();
            if (thisColumn.equalsIgnoreCase(alias)) foundThisColumn = true;
          }
  ...
  private void processSQL(String sql) {
    ...
    try {
      ...
      // need to walk the gen expression and figure out if there are subqueries and start
      // assimilating
      ...
      // if it is an IN filter then the right hand side could possibly be a query struct (like
      // above)
  ...
  private Object processFilter(GenExpression filter, IQueryFilter parentFilter) {
    ...
    if (filter != null) {
      ...
      if (filter.operation.equalsIgnoreCase("=")
          || filter.operation.equalsIgnoreCase(">")
          || filter.operation.equalsIgnoreCase("<")
          || filter.operation.equalsIgnoreCase(">=")
          || filter.operation.equalsIgnoreCase("<=")
          || filter.operation.equalsIgnoreCase("AND")
          || filter.operation.equalsIgnoreCase("OR")
        ...
        boolean root = false;
        if (parentFilter == null) {
          if (filter.operation.equalsIgnoreCase("AND")) parentFilter = new AndQueryFilter();
          else if (filter.operation.equalsIgnoreCase("OR")) parentFilter = new OrQueryFilter();
          else if ((filter.operation.equalsIgnoreCase("=")
                  || filter.operation.equalsIgnoreCase(">")
                  || filter.operation.equalsIgnoreCase("<")
                  || filter.operation.equalsIgnoreCase(">=")
                  || filter.operation.equalsIgnoreCase("<="))
            ...
            parentFilter =
                new SimpleQueryFilter(
                    replacer, "***", replacer); // I will replace these values shortly.
          }
        ...
        {
          if (filter.operation.equalsIgnoreCase("AND")) newParentFilter = new AndQueryFilter();
          else if (filter.operation.equalsIgnoreCase("OR")) newParentFilter = new OrQueryFilter();
          else if ((filter.operation.equalsIgnoreCase("=")
                  || filter.operation.equalsIgnoreCase(">")
                  || filter.operation.equalsIgnoreCase("<")
                  || filter.operation.equalsIgnoreCase(">=")
