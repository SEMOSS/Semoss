...
public class ConvertFrameToVegaVizReactor extends AbstractFrameReactor {
    ...
    public NounMetadata execute() {
        ...
        String instructions = buildRequestInstructions(getVegaBarChartTemplate(), getVegaLineChartTemplate(), getVegaPieChartTemplate());
	...
	private ITableDataFrame getSelectedFrame() {
	    ...
	    // Check for a specifically selected frame name
	    ...
	    if (selectedFrame != null && !selectedFrame.trim().isEmpty()) {
	        ...
	        for (String k : varStore.getKeys()) {
	            ...
	            if (noun.getNounType() == PixelDataType.FRAME) {
	                ...
	                if (selectedFrame.equals(frame.getOriginalName())) {
	                    return frame;
	                }
	            }
	        }
        ...
        logger.warn("Frame ID '{}' not provided, attempting default frame.", selectedFrame);
    ...
    private String detectType(Object cell) {
        if (cell instanceof Integer || cell instanceof Double) return NUMERICAL;
        if (cell instanceof SemossDate) return TEMPORAL;
        return CATEGORICAL;
    }
