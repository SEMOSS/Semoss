...
public class MakePortalReactor extends AbstractReactor {
	...
	// specifies whether to reconfig only or create a new portal from
	// scratch
	private static final String RECONF = "reconf";

	// Inputs - portalName - Artifact ID, Client name - Group ID, Version (0.0.1
	// Default) - Version not given, Archetype (Simple Portal Default) - If not
	// given

	public MakePortalReactor() {
		this.keysToGet = new String[] { ReactorKeysEnum.PROJECT.getKey(), PORTAL_NAME, CLIENT, VERSION, ARCHETYPE,
				RECONF };
	...
	private void writePortalProperties(String projectFolder, Properties projectValues, String portalName)
			throws Exception {
	...
	private void convertAllTemplates(String projectFolder, Properties projectValues, String portalName)
			throws Exception {
	...
	private void convertTemplateToProject(Properties projectValues, String archetype, String templateDir,
		...
		try (FileWriter outputWriter = new FileWriter(new File(outputFileName))) {
