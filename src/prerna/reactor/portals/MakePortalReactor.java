...
public class MakePortalReactor extends AbstractReactor {
  ...
  private static final String RECONF =
      "reconf"; // specifies whether to reconfig only or create a new portal from scratch

  // Inputs - portalName - Artifact ID, Client name - Group ID, Version (0.0.1 Default) - Version
  // not given, Archetype (Simple Portal Default) - If not given

  public MakePortalReactor() {
    this.keysToGet =
        new String[] {
          ReactorKeysEnum.PROJECT.getKey(), PORTAL_NAME, CLIENT, VERSION, ARCHETYPE, RECONF
        };
  ...
  public NounMetadata execute() {
    ...
    if (keyValue.containsKey(keysToGet[3])) version = keyValue.get(keysToGet[3]);

    String archetype = "https://github.com/prabhuk12/JART.git";
    if (keyValue.containsKey(keysToGet[4])) archetype = keyValue.get(keysToGet[4]);
    ...
    return NounMetadata.getSuccessNounMessage("Successfully created portal " + portalName);
  }
  ...
  private void pullGit(String projectFolder, String archetype, String portalName) {
    ...
    if (!portalFolderFile.exists()) portalFolderFile.mkdir();

    CmdExecUtil util =
        new CmdExecUtil(this.insight.getUser(), this.insight.getInsightId(), portalFolder);
  ...
  private void writePortalProperties(
      String projectFolder, Properties projectValues, String portalName) throws Exception {
  ...
  private void convertAllTemplates(
      String projectFolder, Properties projectValues, String portalName) throws Exception {
  ...
  private void convertTemplateToProject(
      Properties projectValues,
      String archetype,
      String templateDir,
