...
/**
 * Assigns one or more users to a version bucket within an app.
 *
 * Pixel: SetUserAppVersion(app="SEMOSS", flagId="uuid",
 * users=["alice","bob","carol"], version=1)
 *
 * Requires app owner or admin.
 */
public class SetUserAppVersionReactor extends AbstractReactor {
	...
	public SetUserAppVersionReactor() {
		this.keysToGet = new String[] { APP_KEY, FLAG_ID_PARAM, USERS_KEY, VERSION_KEY };
	}
	...
	public NounMetadata execute() {
		...
		if (userIds == null || userIds.isEmpty()) {
			throw new IllegalArgumentException("'users' is required and must contain at least one user id");
		}

		if (!AppFeatureFlagUtils.canManageFlags(user, appId)) {
			throw new IllegalArgumentException("User does not have permission to set user versions for this app");
		}
		...
		AppFeatureFlagUtils.setUserVersions(appId, flagId, userIds, version);

		return new NounMetadata(true, PixelDataType.BOOLEAN, PixelOperationType.OPERATION);
	}

	private List<String> getUserIds() {
		GenRowStruct grs = this.store.getGenRowStruct(USERS_KEY);
