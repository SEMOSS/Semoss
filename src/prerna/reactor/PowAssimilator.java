...
public class PowAssimilator extends Assimilator {
	...
	public NounMetadata execute() {
		...
		for (String formula : formulas) {
			this.lSignature = StringUtils.replaceOnce(this.lSignature, formula,
					"( 1.0 * " + formula.substring(1, formula.length()));
			this.rSignature = StringUtils.replaceOnce(this.rSignature, formula,
					"( 1.0 * " + formula.substring(1, formula.length()));
		}
		...
		expressionBuilder.append(
				"return new java.math.BigDecimal(Math.pow( 1.0 * " + lSignature + ", 1.0 * " + rSignature + "));}");
		...
		try {
			...
			if (noun == null) {
				throw new IllegalArgumentException(
						"Error!!! Could not properly evaluate expression = " + this.signature);
			}
		...
		for (int i = 0; i < lambdas.size(); i++) {
			...
			if (n.getValue() instanceof EmbeddedScriptReactor) {
				...
				if (dataType == PixelDataType.CONST_DECIMAL || dataType == PixelDataType.CONST_INT) {
					this.lSignature = StringUtils.replaceOnce(this.lSignature,
							((EmbeddedScriptReactor) n.getValue()).getOriginalSignature(),
							output.getValue().toString());
					this.rSignature = StringUtils.replaceOnce(this.rSignature,
							((EmbeddedScriptReactor) n.getValue()).getOriginalSignature(),
							output.getValue().toString());
	...
	private void appendVariables(StringBuilder expressionBuilder) {
		...
		for (String input : uniqueInputs) {
			...
			// TODO: this should no longer be needed since getVariableValue will evaluate a
			// lambda
				...
				if (rVal instanceof IReactor) {
					...
					if (newDataType == PixelDataType.CONST_DECIMAL) {
						expressionBuilder.append("double ").append(input).append(" = ").append(newNoun.getValue())
								.append(";");
					} else if (newDataType == PixelDataType.CONST_INT) {
						expressionBuilder.append("int ").append(input).append(" = ").append(newNoun.getValue())
								.append(";");
					}
				} else {
