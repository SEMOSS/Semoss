...
public class PowAssimilator extends Assimilator {
  ...
  public NounMetadata execute() {
    ...
    for (String formula : formulas) {
      this.lSignature =
          StringUtils.replaceOnce(
              this.lSignature, formula, "( 1.0 * " + formula.substring(1, formula.length()));
      this.rSignature =
          StringUtils.replaceOnce(
              this.rSignature, formula, "( 1.0 * " + formula.substring(1, formula.length()));
    }
    ...
    expressionBuilder.append(
        "return new java.math.BigDecimal(Math.pow( 1.0 * "
            + lSignature
            + ", 1.0 * "
            + rSignature
            + "));}");
  ...
  private void appendVariables(StringBuilder expressionBuilder) {
    ...
    for (String input : uniqueInputs) {
      ...
      // TODO: this should no longer be needed since getVariableValue will evaluate a lambda
        ...
        throw new IllegalArgumentException(
            "Assimilator can currently only handle outputs of scalar variables");
      }
    }
  }
