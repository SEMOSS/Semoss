...
public class RunClusteringReactor extends AbstractFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    while (go) {
      ...
      Iterator<List<Object[]>> it =
          dataFrame.scaledUniqueIterator(instanceColumn, attributeNamesList);
      while (it.hasNext()) {
        List<Object[]> instance = it.next();
        Object instanceName = instance.get(0)[instanceIndex];
        int bestCluster =
            findBestClusterForInstance(
                instance, attributeNames, isNumeric, instanceIndex, clusters);
        boolean instanceChangeCluster =
            isInstanceChangedCluster(results, instanceName, bestCluster);
        ...
        if (counter % 100 == 0) {
          ...
          logger.info(
              "Finished execution for loop number = "
                  + (currIt + 1)
      ...
      if (currIt > maxIt) {
        Configurator.setLevel(logger.getName(), Level.INFO);
        logger.info(
            "Convergence Error ::: clustering routine did not converge after "
                + maxIt
                + " iterations");
        go = false;
      }
    ...
    if (addToFrame) {
      ...
      AlgorithmMergeHelper.mergeSimpleAlgResult(
          dataFrame, this.instanceColumn, newColName, "NUMBER", this.results);
    }
  ...
  public boolean isInstanceChangedCluster(
      AlgorithmSingleColStore<Integer> results, Object instanceName, int bestCluster) {
  ...
  public void updateInstanceIndex(
      List<Object[]> instance, String[] attributeNames, boolean[] isNumeric, Cluster clusterToAdd) {
    clusterToAdd.addToCluster(instance, attributeNames, isNumeric);
  }

  public void removeInstanceIndex(
  ...
  private void initializeClusters(
      ITableDataFrame dataFrame, List<String> attributeNamesList, Logger logger) {
    Configurator.setLevel(logger.getName(), Level.OFF);
    Iterator<List<Object[]>> it =
        dataFrame.scaledUniqueIterator(instanceColumn, attributeNamesList);
    ...
    for (int i = 1; i < numClusters; i++) {
      ...
      while (it.hasNext()) {
        ...
        double val =
            combinedInstances.getSimilarityForInstance(
                instance, attributeNames, isNumeric, instanceIndex);
        if (val < simVal) {
        ...
        if (counter % 100 == 0) {
          Configurator.setLevel(logger.getName(), Level.INFO);
          logger.info(
              "Trying to determine intial point for cluster # "
                  + i
                  + ". Looped through "
                  + counter
      ...
      logger.info(
          "Found new initial instance for cluster # "
              + i
              + " with instance = "
  ...
  public int findBestClusterForInstance(
      List<Object[]> instance,
      String[] attributeNames,
      boolean[] isNumeric,
      int instanceIndex,
    ...
    for (; i < numClusters; i++) {
      double newSimVal =
          clusters
              .get(i)
              .getSimilarityForInstance(instance, attributeNames, isNumeric, instanceIndex);
      if (newSimVal > simVal) {
        bestIndex = i;
        simVal = newSimVal;
      }
  ...
  private int getNumClusters() {
    ...
    if (numClustersGrs != null) {
      ...
      // else, we assume it is the first index in the current row --> RunClustering(instanceIndex,
      // numClusters, selectors);
