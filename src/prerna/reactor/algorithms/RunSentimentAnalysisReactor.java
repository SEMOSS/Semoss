...
public class RunSentimentAnalysisReactor extends AbstractRFrameReactor {
  ...
  /**
   * User enters a string column and the reactor will run sentiment analysis on that column and also
   * give an advanced (more meaningful) aggregate sentiment of a grouping User can also add
   * miscellaneous emotion columns if desired
   */
  ...
  public RunSentimentAnalysisReactor() {
    this.keysToGet =
        new String[] {
          SENTIMENT_COLUMN_KEY, GROUP_COLUMN_KEY, EMOTIONS_KEY, ReactorKeysEnum.PANEL.getKey()
        };
  }
  ...
  public NounMetadata execute() {
    ...
    // source the files
    rsb.append(
        "source(\"" + baseFolder.replace("\\", "/") + "/R/AnalyticsRoutineScripts/sentiment.R\");");
    ...
    noun.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage("Sentiment analysis ran successfully!"));
  ...
  private void removeDuplicateColumns(
      String frameName, List<String> currentColHeaders, String[] newColHeaders) {
