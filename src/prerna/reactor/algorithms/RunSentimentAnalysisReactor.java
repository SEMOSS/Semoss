...
public class RunSentimentAnalysisReactor extends AbstractRFrameReactor {
	...
	/**
	 * User enters a string column and the reactor will run sentiment analysis on
	 * that column and also give an advanced (more meaningful) aggregate sentiment
	 * of a grouping User can also add miscellaneous emotion columns if desired
	 */
	...
	public RunSentimentAnalysisReactor() {
		this.keysToGet = new String[] { SENTIMENT_COLUMN_KEY, GROUP_COLUMN_KEY, EMOTIONS_KEY,
	...
	public NounMetadata execute() {
		...
		if (!hasAgg) {
			rsb.append(
					sentimentFrame + " <- data.frame(" + sentimentCol + " = " + frameName + "$" + sentimentCol + ");");
		} else {
		...
		rsb.append("colnames(" + sentimentFrame + ")[colnames(" + sentimentFrame + ")==\"" + sentimentCol + "_of_"
				+ sentimentCol + "\"] <- \"" + sentimentCol + "\";");
		if (!hasAgg) {
			rsb.append(
					frameName + " <- merge(" + frameName + "," + sentimentFrame + ",by=c(\"" + sentimentCol + "\"));");
		} else {
			rsb.append("colnames(" + sentimentFrame + ")[colnames(" + sentimentFrame + ")==\"" + groupCol + "_of_"
					+ sentimentCol + "\"] <- \"" + groupCol + "\";");
			rsb.append(frameName + " <- merge(" + frameName + "," + sentimentFrame + ",by=c(\"" + groupCol + "\",\""
					+ sentimentCol + "\"));");
		}
