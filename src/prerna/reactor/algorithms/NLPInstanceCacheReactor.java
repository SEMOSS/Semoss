...
public class NLPInstanceCacheReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    if (this.store.getNoun(this.keysToGet[2]) == null
        || this.store.getNoun(this.keysToGet[2]).isEmpty()) {
      allValues = true;
    }
    ...
    if (rdsExists) {
      ...
      if (!updateExistingValues) {
        if (allValues) {
          ...
          String uniqueColsTables =
              rdsFrameTrim + " <- unique(" + rdsFrame + "[c(\"Table\", \"Column\", \"AppID\")])";
          this.rJavaTranslator.runR(uniqueColsTables);
          String existingColsScript =
              rdsFrameTrim + "[" + rdsFrameTrim + "$AppID == \"" + databaseId + "\",]$Column;";
          String existingTablesScript =
              rdsFrameTrim + "[" + rdsFrameTrim + "$AppID == \"" + databaseId + "\",]$Table;";
          ...
          for (int x = 0; x < existingCols.length; x++) {
            ...
            if (existingTableCols == null
                || !existingTableCols.contains(existingTable + existingCol)) {
    ...
    setupBuilder.append(
        "setwd(\"" + baseFolder.replace("\\", "/") + "/R/AnalyticsRoutineScripts/\");");
    setupBuilder.append(
        retFrame
    ...
    if (allValues) {
      ...
      for (Object[] tableCol : allTableCols) {
        ...
        processSingleColumn(
            database, colQs, databaseId, appIdVector, tableVector, colVector, valueVector);
      }
    ...
    noun.addAdditionalReturn(
        new NounMetadata(
            "Successfully cached instances for NLP searching",
            PixelDataType.CONST_STRING,
            PixelOperationType.SUCCESS));
    return noun;
  }

  private void processSingleColumn(
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(colSelector, "!=", null, PixelDataType.NULL_VALUE));
  ...
  private void addTable(
    ...
    String addTableScript =
        tempFrame
            + " <- data.frame(AppID = "
            + RSyntaxHelper.createStringRColVec(databaseIdVector)
            + " , Table = "
            + RSyntaxHelper.createStringRColVec(tableVector)
            + " , Column = "
            + RSyntaxHelper.createStringRColVec(colVector)
            + " , Value = "
            + valueVectorName
