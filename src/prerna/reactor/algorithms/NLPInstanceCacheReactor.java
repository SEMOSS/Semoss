...
public class NLPInstanceCacheReactor extends AbstractRFrameReactor {
	...
	/**
	 * Reads in the Databases and Creates an RDS file of all unique instances in the
	 * AnalyticsRoutineScripts Folder
	 */
	...
	public NLPInstanceCacheReactor() {
		this.keysToGet = new String[] { ReactorKeysEnum.DATABASE.getKey(), ReactorKeysEnum.TABLE.getKey(),
				ReactorKeysEnum.COLUMNS.getKey(), UPDATE_EXISTING_VALUES };
	}
	...
	public NounMetadata execute() {
		...
		if (this.store.getGenRowStruct(this.keysToGet[2]) == null
				|| this.store.getGenRowStruct(this.keysToGet[2]).isEmpty()) {
			allValues = true;
		}
		...
		if (rdsExists) {
			...
			if (!updateExistingValues) {
				if (allValues) {
					...
					String uniqueColsTables = rdsFrameTrim + " <- unique(" + rdsFrame
							+ "[c(\"Table\", \"Column\", \"AppID\")])";
					this.rJavaTranslator.runR(uniqueColsTables);
					String existingColsScript = rdsFrameTrim + "[" + rdsFrameTrim + "$AppID == \"" + databaseId
							+ "\",]$Column;";
					String existingTablesScript = rdsFrameTrim + "[" + rdsFrameTrim + "$AppID == \"" + databaseId
							+ "\",]$Table;";
					...
					// in this case, we know the table of the columns and columnsToUpdate is not
					// null
					// so just remove those columns that match by app id, table, and column
					String existingColsScript = "unique(" + rdsFrame + "[" + rdsFrame + "$AppID == \"" + databaseId
							+ "\" & " + rdsFrame + "$Table == \"" + table + "\" ,]$Column);";
					String[] existingCols = this.rJavaTranslator.getStringArray(existingColsScript);
				...
				if (allValues) {
					...
					for (String col : columnsToUpdate) {
						removeScript += amp;
						amp = " & ";
						removeScript += "!(" + rdsFrame + "$AppID==\"" + databaseId + "\" & " + rdsFrame + "$Column==\""
								+ col + "\" & " + rdsFrame + "$Table==\"" + table + "\")";
					}
					removeScript += " ),];";
				}
		...
		noun.addAdditionalReturn(new NounMetadata("Successfully cached instances for NLP searching",
				PixelDataType.CONST_STRING, PixelOperationType.SUCCESS));
		return noun;
	}

	private void processSingleColumn(IDatabaseEngine database, String colQs, String databaseId,
			List<String> databaseIdVector, List<String> tableVector, List<String> colVector, List<String> valueVector) {
	...
	private void addTable(String rdsFrame, String tempFrame, List<String> databaseIdVector, List<String> tableVector,
		...
		String addTableScript = tempFrame + " <- data.frame(AppID = "
				+ RSyntaxHelper.createStringRColVec(databaseIdVector) + " , Table = "
				+ RSyntaxHelper.createStringRColVec(tableVector) + " , Column = "
