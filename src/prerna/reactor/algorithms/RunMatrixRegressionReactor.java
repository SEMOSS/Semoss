...
public class RunMatrixRegressionReactor extends AbstractFrameReactor {
	...
	public RunMatrixRegressionReactor() {
		this.keysToGet = new String[]{Y_COLUMN, X_COLUMNS, ReactorKeysEnum.DEFAULT_VALUE_KEY.getKey(),
	...
	public NounMetadata execute() {
		...
		// NEW TRACKING
		UserTrackerFactory.getInstance().trackAnalyticsWidget(this.insight, dataFrame, "MatrixRegression",
				AnalyticsTrackerHelper.getHashInputs(this.store, this.keysToGet));

		return new NounMetadata(vizData, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.VIZ_OUTPUT);
	}

	private double[][] setValuesInOlsAndCorr(OLSMultipleLinearRegression ols, Iterator<IHeadersDataRow> it, int numCols,
			int numRows, double defaultVal, Logger logger) {
...
/**
 * Need an inner class that extends OLS from Apache Commons to get some of the
 * specific data out since methods are protected and not public
 */
