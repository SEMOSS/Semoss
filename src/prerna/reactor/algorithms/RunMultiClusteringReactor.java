...
public class RunMultiClusteringReactor extends AbstractFrameReactor {
  ...
  /**
   * RunMultiClustering(instance = column, minNumClusters = min#, maxNumCluster = max#, col1,
   * col2....);
   */
  ...
  public NounMetadata execute() {
    ...
    if (minNumClusters != maxNumClusters) {
      results =
          runGoldenSelectionForNumberOfClusters(dataFrame, minNumClusters, maxNumClusters, logger);
    } else {
      // usually occurs when there is too few data points
      results =
          runClusteringRoutine(
              dataFrame,
              minNumClusters,
    ...
    AlgorithmMergeHelper.mergeSimpleAlgResult(
        dataFrame, instanceColumn, newColName, "NUMBER", results);
  ...
  private AlgorithmSingleColStore<Integer> runGoldenSelectionForNumberOfClusters(
      ITableDataFrame data, int start, int end, Logger logger) {
    ...
    try {
      startClusterResult =
          runClusteringRoutine(data, x1, startClusterList, previousResults, logger);
      startVal =
          computeClusteringScore(
              data, startClusterResult, startClusterList, previousResults, x1, logger);
    ...
    try {
      ...
      endVal =
          computeClusteringScore(
              data, endClusterResult, endClusterList, previousResults, x2, logger);
  ...
  private double computeClusteringScore(
    ...
    while (it.hasNext()) {
      ...
      double simVal =
          clusters
              .get(clusterIndex)
              .getSimilarityForInstance(
                  instance, this.attributeNames, isNumeric, this.instanceIndex);
      innerClusterSimilairty += simVal / clusters.get(clusterIndex).getNumInstances();
    }
    Configurator.setLevel(logger.getName(), Level.INFO);
    logger.info(
        "For cluster # " + numClusters + ", inner cluster score = " + innerClusterSimilairty);
    ...
    logger.info(
        "For cluster # "
            + numClusters
            + ", cluster-to-cluster similatiry score = "
  ...
  private AlgorithmSingleColStore<Integer> runClusteringRoutine(
    ...
    prerna.reactor.algorithms.RunClusteringReactor alg =
        new prerna.reactor.algorithms.RunClusteringReactor();
