...
public class RunMultiClusteringReactor extends AbstractFrameReactor {
	...
	/**
	 * RunMultiClustering(instance = column, minNumClusters = min#, maxNumCluster =
	 * max#, col1, col2....);
	 */
	public RunMultiClusteringReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.INSTANCE_KEY.getKey(), MIN_NUM_CLUSTERS, MAX_NUM_CLUSTERS,
	...
	public NounMetadata execute() {
		...
		if (minNumClusters != maxNumClusters) {
			...
			results = runClusteringRoutine(dataFrame, minNumClusters, new ArrayList<Cluster>(),
					new HashMap<Integer, Double>(), logger);
		}
		...
		// track GA data
		// UserTrackerFactory.getInstance().trackAnalyticsPixel(this.insight,
		// "ClusterOptimization");

		// NEW TRACKING
		UserTrackerFactory.getInstance().trackAnalyticsWidget(this.insight, dataFrame, "ClusterOptimization",
	...
	private AlgorithmSingleColStore<Integer> runGoldenSelectionForNumberOfClusters(ITableDataFrame data, int start,
			int end, Logger logger) {
	...
	private double computeClusteringScore(ITableDataFrame data, AlgorithmSingleColStore<Integer> startClusterResult,
			List<Cluster> clusters, Map<Integer, Double> previousResults, int numClusters, Logger logger) {
		...
		while (it.hasNext()) {
			...
			double simVal = clusters.get(clusterIndex).getSimilarityForInstance(instance, this.attributeNames,
					isNumeric, this.instanceIndex);
		...
		logger.info("For cluster # " + numClusters + ", cluster-to-cluster similatiry score = "
				+ clusterToClusterSimilarity);

		return innerClusterSimilairty / numClusters
				+ clusterToClusterSimilarity / ((double) (numClusters * (numClusters - 1) / 2));
	}

	private AlgorithmSingleColStore<Integer> runClusteringRoutine(ITableDataFrame dataframe, int numClusters,
			List<Cluster> clusters, Map<Integer, Double> previousResults, Logger logger) {
		if (previousResults.containsKey(numClusters)) {
			return null;
		}
		...
		// RunClustering(instance = column, numClusters = numCluters, columns =
		// attributeNamesList);
