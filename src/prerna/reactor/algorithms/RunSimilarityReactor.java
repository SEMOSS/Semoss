...
public class RunSimilarityReactor extends AbstractFrameReactor {
  ...
  private void generateClusterCenters(
      ITableDataFrame dataFrame, Cluster cluster, boolean[] isNumeric) {
    Iterator<List<Object[]>> it =
        dataFrame.scaledUniqueIterator(attributeNames[instanceIndex], attributeNamesList);
    while (it.hasNext()) {
  ...
  public void getSimilarityValuesForInstances(
    ...
    while (it.hasNext()) {
      ...
      double sim =
          cluster.getSimilarityForInstance(instance, attributeNames, isNumeric, instanceIndex);
