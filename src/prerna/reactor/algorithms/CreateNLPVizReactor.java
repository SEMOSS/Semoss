...
public class CreateNLPVizReactor extends AbstractRFrameReactor {
  ...
  /**
   * Reads in the Columns and Database IDs and returns the visualization string, which is then
   * appended to the NLP search
   */
  ...
  public NounMetadata execute() {
        ...
        new Comparator<String>() {
          public int compare(String firstCol, String secondCol) {
            if (isAggregate(firstCol)) {
              ...
              return frame.getUniqueInstanceCount(firstCol)
                  - frame.getUniqueInstanceCount(secondCol);
            }
          }
        });
    ...
    if (vizSelection.equals("Recommended")) {
      ...
      rsb.append(
          recommend + "<-viz_recom_mgr(\"" + fileroot + "\", " + inputFrame + ", \"Grid\", 1);");
    ...
    this.rJavaTranslator.executeEmptyR(
        "rm(" + wd + "," + outputJson + "," + inputFrame + "," + recommend + "); gc();");

    // return
    String returnPixel =
        generatePixelFromRec(recommendations, cols, frameName, metadata, panelId, sortPixel);
    PixelRunner runner = this.insight.runPixel(returnPixel);
    return runner.getResults().get(0);
  }

  private boolean isAggregate(String col) {
    return (col.contains("UniqueCount_")
        || col.contains("Count_")
        || col.contains("Min_")
        || col.contains("Max_")
        || col.contains("Average_")
        || col.contains("Sum_"));
  }
  ...
  private String generatePixelFromRec(
      Map<String, HashMap<String, String>> recommendations,
      List<String> columns,
      String frameName,
    ...
    for (String col : columns) {
      ...
      if (!label.equals(col)
          && (Utility.isIntegerType(dataType) || Utility.isDoubleType(dataType))) {
