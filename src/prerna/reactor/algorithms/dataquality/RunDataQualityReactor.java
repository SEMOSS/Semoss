...
public class RunDataQualityReactor extends AbstractRFrameReactor {
	...
	// Define rules because ReactorKeyEnum doesn't have default values for our
	// purposes
	...
	// This gets the basic format of the data. The information put in here will be
	 ...
	 * 
	 * This function creates Meta Data on the variable. It takes in the value and
	 * also takes the pixel and assigns a type. We use this to create the variables
	 * to pass to R
	 */
	public NounMetadata execute() {
		...
		if (inputTable == null) {
			rScript.append(retRVariableName).append(
					" <- data.table(Columns=character(), Errors=integer(), Valid=integer(), Total=integer(), Rules=character(), Description=character(), toColor = character());");
		}
		rScript.append(inputString.toString());

		rScript.append(retRVariableName)
				.append(" <- missionControl(" + rFrameVariable + ", " + inputVariable + ", " + retRVariableName + ");");
