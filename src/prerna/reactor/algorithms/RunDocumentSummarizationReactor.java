...
public class RunDocumentSummarizationReactor extends AbstractRFrameReactor {
  ...
  /** Generates a dashboard summarizing a chosen document that is entered by the user */

  // CreateFrame ( R ) .as ( [ 'FRAME950929' ] ) | RunDocumentSummarization(desiredOutput =
  ...
  public RunDocumentSummarizationReactor() {
    this.keysToGet =
        new String[] {FILE_ORIGIN, USER_INPUT, NUM_SENTENCES, NUM_TOPICS, NUM_TOPIC_TERMS};
  }
  ...
  public NounMetadata execute() {
    ...
    rsb.append(
        frameName + "<- bind_rows(as.data.frame(" + summaryFrame + ")," + topicsFrame + ");");
    ...
    rsb.append(
        frameName + "$keyword_frequency <- as.numeric(" + frameName + "$keyword_frequency);");
