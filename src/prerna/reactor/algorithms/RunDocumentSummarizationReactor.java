...
public class RunDocumentSummarizationReactor extends AbstractRFrameReactor {
	...
	// CreateFrame ( R ) .as ( [ 'FRAME950929' ] ) |
	// RunDocumentSummarization(desiredOutput =
	// ["Summarize Text"], fileOrigin = ["File Path"], userInput =
	// ["C:/Users/chrilong/Desktop/GoTAppeal.doc"],numSentences = ["5"],numTopics =
	// ["5"],numTopicTerms=["6"])
	...
	public NounMetadata execute() {
		...
		rsb.append(wd + "<- getwd();");
		rsb.append(("setwd(\"" + baseFolder + DIR_SEPARATOR + "R" + DIR_SEPARATOR + "AnalyticsRoutineScripts\");")
				.replace("\\", "/"));
		...
		rsb.append(summaryFrame + " <- summarize_text(" + fileOrigin + " = \"" + userInput + "\", topN = "
				+ numSentences + ");");
		rsb.append(topicsFrame + " <- summarize_topics_text(" + fileOrigin + " = \"" + userInput + "\" , topTopics = "
				+ numTopics + ", " + "topTerms = " + numTopicTerms + ");");
		rsb.append(keywordsFrame + " <- text_keywords(" + fileOrigin + " = \"" + userInput + "\");");

		// adjust the keyword and freq column naming issue
		rsb.append("setnames(" + topicsFrame
				+ ", old=c(\"freq\",\"keyword\", \"text\"), new=c(\"topic_frequency\", \"topic_keywords\", \"topic_summary\"));");
	...
	private boolean checkRVersion(int majorReq, int minorReq) {
		// relies on the following R format (seems like its been this format for > 6
		// years
