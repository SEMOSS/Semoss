...
public class GenerateXRayHashingReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    for (String dbId : databaseIds) {
      if (!SecurityEngineUtils.userCanViewEngine(this.insight.getUser(), dbId)) {
        throw new IllegalArgumentException(
            "User does not have permission to view this database or database does not exist");
      }
    }
    ...
    boolean rowComparison =
        Boolean.parseBoolean(this.keyValue.get(GenerateXRayMatchingReactor.ROW_MATCHING) + "");
    ...
    this.rJavaTranslator.executeEmptyR(
        RSyntaxHelper.loadPackages(reqPackages)
            + "; source(\""
    ...
    for (String databaseId : databaseIds) {
      ...
      Collection<String> pixelSelectors =
          MasterDatabaseUtility.getSelectorsWithinDatabaseRDBMS(databaseId);
      ...
      if (rowComparison) {
        ...
        for (String selector : pixelSelectors) {
          ...
          if (selectorFilters != null
              && !selectorFilters.isEmpty()
              && !selectorFilters.contains(selector)) {
            // ignore the selector
            continue;
          }
        ...
        for (String table : tableToQs.keySet()) {
          ...
          try {
            ...
            try {
              ...
              this.rJavaTranslator.executeEmptyR(
                  RSyntaxHelper.getFReadSyntax(randomFrame, f.getAbsolutePath(), "\t"));
              ...
              this.rJavaTranslator.executeEmptyR(
                  "encode_instances(" + randomFrame + ", \"" + outputFile + "\")");
        ...
        for (String selector : pixelSelectors) {
          ...
          try {
            wrapper = WrapperManager.getInstance().getRawWrapper(engine, qs);
            File f =
                new File(this.folderPath + "/" + Utility.normalizePath(selector) + "_base.tsv");
            try {
              // write file
    ...
    noun.addAdditionalReturn(
        NounMetadata.getSuccessNounMessage("Successfully stored hash files for databases"));
    return noun;
  }
