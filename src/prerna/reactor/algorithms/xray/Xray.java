...
//
// public Xray(AbstractRJavaTranslator rJavaTranslator, String baseFolder,
// Logger logger) {
...
// String[] packages = new String[] { "textreuse", "RcppProgress", "withr",
// "NLP", "tidyr",
// "devtools", "memoise", "digest", "tidyselect", "purrr"};
// this.rJavaTranslator.checkPackages(packages);
// // packages to write corpus files
// String[] encodePackages = new String[] {"WikidataR", "WikipediR", "httr",
// "curl", "jsonlite"};
...
// String dataFolder = this.baseFolder +
// "\\R\\XrayCompatibility\\Temp\\MatchingRepository";
// dataFolder = dataFolder.replace("\\", "/");
// // output folder for semantic data to be written to
// String semanticFolder = this.baseFolder +
// "\\R\\XrayCompatibility\\Temp\\SemanticRepository";
...
// Map<String, Object> connectorData = (Map<String, Object>)
// connection.get("connectorData");
// Map<String, Object> dataSelection = (Map<String, Object>)
// connection.get("dataSelection");
// if (connectorType.equalsIgnoreCase("LOCAL")) {
// writeLocalEngineToFile(connectorData, dataSelection, dataMode, dataFolder,
// semanticMode,
// semanticFolder);
...
// writeExternalToFile(connectorData, dataSelection, dataMode, dataFolder,
// semanticMode,
// semanticFolder);
...
// writeExcelToFile(sourceFile, dataSelection, connectorData, dataMode,
// dataFolder,
// semanticMode,
// semanticFolder);
// }
...
// + RSyntaxHelper.createStringRColVec(propColumn.toArray()) + ", count=" +
// RSyntaxHelper
...
// rsb.append(lookupFrame + " <- data.frame(engineId = character(), engineName =
// character(),
// stringsAsFactors=FALSE);");
// int row = 1;
// for(String key: engineNameLookup.keySet()){
// rsb.append(lookupFrame + "[" + row + ", ]<-c(\"" + key + "\",\"" +
// engineNameLookup.get(key) +
// "\");");
// row++;
// }
// if (dataMode) {
// rsb.append(rFrameName + " <- " + Constants.R_LSH_MATCHING_FUN + "(\"" +
// dataFolder + "\", " +
// nMinhash
...
// by.x=\"Source_Database_Id\", by.y=\"engineId\");colnames(" + rFrameName +
// ")[13] <-
// \"Source_Database\";");
// rsb.append(rFrameName + " <-merge(" + rFrameName + "," + lookupFrame +",
// by.x=\"Target_Database_Id\", by.y=\"engineId\");colnames(" + rFrameName +
// ")[14] <-
// \"Target_Database\";");
// }
...
// "(\"" +
// semanticFolder + "\", "
// + nMinhash + ", " + nBands + ", " + similarityThreshold + ", " +
// instancesThreshold + ", \""
// + ENGINE_CONCEPT_PROPERTY_DELIMETER + "\", " +
// matchSameDB.toString().toUpperCase()
// + ", \"" + semanticOutputFolder + "\");");
// rsb.append(semanticComparisonFrame + " <-merge(" + semanticComparisonFrame +
// "," + lookupFrame
// + ", by.x=\"Source_Database_Id\", by.y=\"engineId\");colnames(" +
// semanticComparisonFrame +
// ")[13] <- \"Source_Database\";");
// rsb.append(semanticComparisonFrame + " <-merge(" + semanticComparisonFrame +
// "," + lookupFrame
// +", by.x=\"Target_Database_Id\", by.y=\"engineId\");colnames(" +
// semanticComparisonFrame + ")[14]
// <- \"Target_Database\";");
...
// rsb.append(rFrameName + " <- xray_merge(" + rFrameName + ", " +
// semanticComparisonFrame +
// ");");
// } else {
...
//
// }
// this.logger.info("Comparing data from datasources for X-ray semantic
// mode...");
// this.rJavaTranslator.runR(rsb.toString());
//
// }
...
// */
// private void writeExcelToFile(String excelFile, Map<String, Object>
// dataSelection,
// Map<String, Object> connectorData, boolean dataComparison, String dataFolder,
// boolean
// semanticMode,
...
// this.logger.info("Getting " + Utility.cleanLogString(col) + " from excel
// sheet: " + sheetName
// + " for X-ray comparison...");
// // encode and write to file
// encodeInstances(instances, dataComparison, filePath, semanticMode,
// semanticFolder);
// }
// }
// }
...
// * the location where data from semantic mode is written to
// */
// private void writeCsvToFile(String csvFile, Map<String, Object>
// dataSelection, boolean dataMode,
// String dataFolder,
// boolean semanticMode, String semanticFolder) {
// String[] csvFileNameArray = csvFile.split("\\\\");
// String csvName = csvFileNameArray[csvFileNameArray.length -
// 1].replace(".csv", "");
...
// this.logger.info("Getting " + Utility.cleanLogString(col) + " from " +
// csvName + " csv to run
// xray comparison... ");
...
// * the location where data from semantic mode is written to
// */
// private void writeExternalToFile(Map<String, Object> connectorData,
// Map<String, Object>
// dataSelection,
// boolean dataMode, String dataFolder, boolean semanticMode, String
// semanticFolder) {
...
// Connection con = null;
// try {
// con = RdbmsConnectionHelper.buildConnection(type, host, port, username,
// password, schema,
// null);
// } catch (SQLException e1) {
// throw new IllegalArgumentException("Invalid connection");
// }
// this.logger.info("Querying data from external database " + newDBName + " for
// X-ray
// comparison...");
// // loop through tables within a database
// for (String table : dataSelection.keySet()) {
// Map<String, Object> allColumns = (Map<String, Object>)
// dataSelection.get(table);
...
// String fileName = dataFolder + "\\" + newDBName + ";" + table + ";" + column
// + ".txt";
// fileName = fileName.replace("\\", "/");
// // get instances
...
// }
// // encode and write to file
// this.logger.info("Querying " + column + " from " +
// Utility.cleanLogString(table) + " in " +
...
// * the location where data from semantic mode is written to
// */
// private void writeLocalEngineToFile(Map<String, Object> connectorData,
// Map<String, Object>
// dataSelection,
// boolean dataMode, String dataFolder, boolean semanticMode, String
// semanticFolder) {
...
// this.logger.info("Querying data from local database for X-ray comparison : "
// + engineName);
// // loop through tables within a database
// for (String table : dataSelection.keySet()) {
// Map<String, Object> allColumns = (Map<String, Object>)
...
// getLocalEngineInstanceCount(engine, table, null, QueryFunctionHelper.COUNT,
// false);
...
// }
// }
// this.logger.info("Querying table " + Utility.cleanLogString(table) + " from "
// + engineName
// + " for X-ray comparison");
// // encode and write to file
// encodeInstances(instances, dataMode, fileName, semanticMode, semanticFolder);
...
// else {
// // get total instance count
// getLocalEngineInstanceCount(engine, table, column, QueryFunctionHelper.COUNT,
// false);
// // write txt file path of where data will be written
// // to dataFolder/engineName;table;column.txt
//
// String fileName = dataFolder + "\\" + engineID + ";" + table + ";" + column +
// ".txt";
// fileName = fileName.replace("\\", "/");
...
// */
// private void encodeInstances(List<Object> instances, boolean dataMode, String
// filePath, boolean
// semanticMode,
// String semanticFolder) {
// if (instances.size() > 1) {
...
// String minHashFilePath = this.baseFolder +
// "\\R\\AnalyticsRoutineScripts\\encode_instances.R";
...
// rsb.append(dfName + "<-data.frame(instances=character(), stringsAsFactors =
// FALSE);");
...
// rsb.append("\"" + instances.get(j).toString().replaceAll("[^A-Za-z0-9 ]",
// "_") + "\"");
// }
// rsb.append(";");
//
...
// int randomVals = 20;
//
// rsb.append(semanticResults + "<- concept_xray(" + dfName + "," +
// colSelectString + "," +
// numDisplay
// + "," + randomVals + ");");
// String fileName = filePath.substring(filePath.lastIndexOf('/') + 1);
// String fileCon = Utility.getRandomString(8);
// writeSemanticResultsToFile.append(fileCon +" <- file(\"" + semanticFolder +
// "/" + fileName +
// "\");");
// writeSemanticResultsToFile.append("writeLines(" + semanticResults +
// "$Predicted_Concept,
// "+fileCon+");");
...
// rsb.append("rm(" + dfName + ",concept_mgr, concept_xray, encode_instances,
// get_claims,
...
// * output location of wiki header data to compare
// */
// private void encodeInstances(HashSet<Object> instances, boolean dataMode,
// String filePath,
...
// String fileName = filePath.substring(filePath.lastIndexOf('/') + 1);
// String fileCon = Utility.getRandomString(8);
// writeFrameResultsToFile.append(fileCon + " <- file(\"" + semanticFolder + "/"
// + fileName +
// "\");");
// writeFrameResultsToFile.append("writeLines(" + semanticResults +
// "$Predicted_Concept, " +
// fileCon + ");");
...
// * @return
// */
// private void getLocalEngineInstanceCount(IEngine engine, String concept,
// String prop, String
