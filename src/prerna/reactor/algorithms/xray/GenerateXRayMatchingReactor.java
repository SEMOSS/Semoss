...
public class GenerateXRayMatchingReactor extends AbstractRFrameReactor {
	...
	public NounMetadata execute() {
		...
		this.rJavaTranslator.executeEmptyR(
				"source(\"" + DIHelper.getInstance().getProperty(Constants.BASE_FOLDER).replace('\\', '/')
						+ "/R/XRay/matching.R\", local=TRUE);");

		logger.info("Running matching routine");
		String rFrameName = "xray_" + Utility.getRandomString(4);
		String script = rFrameName + " <- run_lsh_matching(" + RSyntaxHelper.createStringRColVec(filePaths) + ", "
		...
		if (replaceIds) {
			...
			for (int i = 0; i < databaseIds.size(); i++) {
				...
				replaceSyntax.append(
						sourceDbId + "[" + sourceDbId + " == \"" + databaseId + "\"] <- \"" + databaseName + "\";");
				replaceSyntax.append(
						targetDbId + "[" + targetDbId + " == \"" + databaseId + "\"] <- \"" + databaseName + "\";");
			}
			this.rJavaTranslator.executeEmptyR(replaceSyntax.toString());
		}

		RDataTable matchingFrame = createNewFrameFromVariable(rFrameName);
		NounMetadata noun = new NounMetadata(matchingFrame, PixelDataType.FRAME, PixelOperationType.FRAME,
		...
		if (addSameDbWarn) {
			noun.addAdditionalReturn(NounMetadata.getWarningNounMessage(
					"Since only one database was selected, altered input value to perform same database matching"));
		}
