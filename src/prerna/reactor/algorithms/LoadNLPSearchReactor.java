...
public class LoadNLPSearchReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    if (assetId != null && !(assetId.isEmpty())) {
      ...
      savePath =
          AssetUtility.getUserAssetAndWorkspaceVersionFolder("Asset", assetId)
              + DIR_SEPARATOR
              + "assets";
    ...
    if (!nldrDb.exists()
        || !nldrJoins.exists()
        || !nldrMembership.exists()
        || nldrMembership.lastModified() < replaceTime) {
      createRdsFiles(rFolderPath);
    } else {
      script
          .append(NLDR_MEMBERSHIP + " <- readRDS(\"nldr_membership.rds\");")
  ...
  private void createRdsFiles(String rFolderPath) {
    ...
    sessionTableBuilder.append(
        "source(\"" + rFolderPath.replace("\\", "/") + "data_inquiry_guide.R\");");
    sessionTableBuilder.append(
        "source(\"" + rFolderPath.replace("\\", "/") + "data_inquiry_assembly.R\");");
    ...
    sessionTableBuilder
        .append(NLDR_MEMBERSHIP + " <- cluster_tables(" + db + "," + joins + ");")
