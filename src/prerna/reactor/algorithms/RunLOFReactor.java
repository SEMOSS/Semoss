...
public class RunLOFReactor extends AbstractFrameReactor {
	...
	public RunLOFReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.INSTANCE_KEY.getKey(), K_NEIGHBORS,
	...
	public NounMetadata execute() {
		...
		if (k > numInstances) {
			throw new IllegalArgumentException("Number of unqiue instances: " + this.numInstances
					+ ", is less than the selected K value: " + k + ".");
		}
		...
		// This code flattens out instances, incase there are repeat appearances of an
		// identifier
		...
		// NEW TRACKING
		UserTrackerFactory.getInstance().trackAnalyticsWidget(this.insight, dataFrame, "LOF",
	...
	public AlgorithmSingleColStore<Double> score(int k, Logger logger) {
		...
		// For a point A, LRD equals k divided by the sum of kdistances of point A's
		// nearest k
		// neighbors.
