...
public class RunLOFReactor extends AbstractFrameReactor {
  ...
  private double[][]
      reachDistance; // This stores the reachDistance between 2 points. Not symmetrical!
  ...
  public RunLOFReactor() {
    this.keysToGet =
        new String[] {
          ReactorKeysEnum.INSTANCE_KEY.getKey(), K_NEIGHBORS, ReactorKeysEnum.ATTRIBUTES.getKey()
        };
  }
  ...
  public NounMetadata execute() {
    ...
    if (k > numInstances) {
      throw new IllegalArgumentException(
          "Number of unqiue instances: "
              + this.numInstances
              + ", is less than the selected K value: "
              + k
              + ".");
    }
    ...
    AlgorithmMergeHelper.mergeSimpleAlgResult(
        dataFrame, this.instanceColumn, newColName, "NUMBER", results);
  ...
  public AlgorithmSingleColStore<Double> score(int k, Logger logger) {
    ...
    for (int i = 0; i < numInstances; i++) {
      ...
      kDistance[i] =
          dist(dataFormatted[i], dataFormatted[(int) neighborIDs[neighborIDs.length - 1]]);
      if (kDistance[i] == 0) {
        kDistance[i] = .0000001;
      }
    ...
    //    For a point A, LRD equals k divided by the sum of kdistances of point A's nearest k
    // neighbors.
