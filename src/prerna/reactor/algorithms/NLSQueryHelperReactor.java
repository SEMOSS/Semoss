...
public class NLSQueryHelperReactor extends AbstractRFrameReactor {
  ...
  public NLSQueryHelperReactor() {
    this.keysToGet =
        new String[] {
          ReactorKeysEnum.QUERY_KEY.getKey(), ReactorKeysEnum.DATABASE.getKey(), HELP_ON, GLOBAL
        };
  }
  ...
  private Object[] getDropdownItems(String queryTable, String colHeadersAndTypesFrame) {
    ...
    if (getGlobal()) {
      ...
      rsb.append(
          retList + " <- analyze_request(" + colHeadersAndTypesFrame + "," + queryTable + ")");
    }
    this.rJavaTranslator.runR(rsb.toString());
    ...
    this.rJavaTranslator.executeEmptyR(
        "rm( " + retList + "," + colHeadersAndTypesFrame + " ); gc();");
