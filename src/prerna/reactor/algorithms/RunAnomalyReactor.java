...
public class RunAnomalyReactor extends AbstractRFrameReactor {
  ...
  public RunAnomalyReactor() {
    this.keysToGet =
        new String[] {TIME_COLUMN, SERIES_COLUMN, AGG_FUNC, MAX_ANOMS, DIRECTION, ALPHA, PERIOD};
  }
  ...
  public NounMetadata execute() {
    ...
    String argsScript =
        "args <- list('"
            ...
            + maxAnoms
            + ", '"
            + anomDirection.toString().toLowerCase()
            + "', "
            + alpha
            + ", "
            + period
    ...
    String script =
        "source(\"" + getBaseFolder() + "\\R\\AnalyticsRoutineScripts\\AnomalyDetection.R" + "\");";
    ...
    metaData.setAliasToProperty(
        table + "__" + aggregateFunction + "_" + seriesColumn,
    ...
    metaData.setAliasToProperty(
        table + "__" + "anom_" + aggregateFunction + "_" + seriesColumn,
        "anom_" + aggregateFunction + "_" + seriesColumn);
    metaData.setDataTypeToProperty(
        table + "__" + "anom_" + aggregateFunction + "_" + seriesColumn, "NUMBER");
