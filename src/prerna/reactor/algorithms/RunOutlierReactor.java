...
public class RunOutlierReactor extends AbstractFrameReactor {
  ...
  /**
   * RunOutlier(instance = column, subsetSize = [numSubsetSize], numRuns = [numRuns], columns =
   * attributeNamesList);
   */
  ...
  public NounMetadata execute() {
    ...
    if (dups == null) {
      ...
      for (int i = 0; i < numAttributes; i++) {
        if (isNumeric[i])
          dups.put(
              attributeNamesList.get(i), new DuplicationReconciliation(ReconciliationMode.MEAN));
      }
    }
      ...
      while (it.hasNext()) {
        ...
        for (int i = 0; i < numSubsetSize; i++) {
          ...
          if (subsetObj != null && instanceName != null) {
            ...
            double sim =
                InstanceSimilarity.getInstanceSimilarity(
                    instance, subsetInstance, isNumeric, attributeNames, dups);
            if (maxSim < sim) {
              maxSim = sim;
            }
          }
        ...
        if (counter % 100 == 0) {
          Configurator.setLevel(logger.getName(), Level.INFO);
          logger.info(
              "Finished execution for run number = " + k + ", unique instance number = " + counter);
          Configurator.setLevel(logger.getName(), Level.OFF);
        }
        counter++;
      }
