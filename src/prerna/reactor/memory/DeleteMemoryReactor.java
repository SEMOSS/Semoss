...
/**
 * Soft-deletes a memory. Only the memory's creator may delete it (see
 * MemoryUtils#deleteMemory); a MEMORY_AUDIT row is recorded with the prior
 * content/metadata.
 */
public class DeleteMemoryReactor extends AbstractReactor {
	...
	public String getReactorDescription() {
		return """
				Soft-deletes a memory. Only the memory's creator may delete it; the memory \
				is hidden from normal listings but preserved in the audit trail, not \
				permanently removed.\
				""";
	}
	...
	public NounMetadata execute() {
		...
		boolean deleted = MemoryUtils.deleteMemory(memoryId, userId);
	...
	protected String getDescriptionForKey(String key) {
		if (key.equals(ReactorKeysEnum.MEMORY_ID.getKey())) {
			return "Id of the memory to delete";
		}
