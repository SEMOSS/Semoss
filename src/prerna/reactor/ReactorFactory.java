...
public class ReactorFactory {
  ...
  public static Map<String, Class<? extends IReactor>> reactors =
      new HashMap<String, Class<? extends IReactor>>();

  public static List<String> nmList = new ArrayList<String>();
  public static List<Class<? extends IReactor>> classList =
      new ArrayList<Class<? extends IReactor>>();
  ...
  static {
    ...
    try {
      ...
      if (additionalPackages != null
          && !(additionalPackages = additionalPackages.trim()).isEmpty()) {
  ...
  public static void printInconsistency(String mapname, Map map) {
    ...
    while (keys.hasNext()) {
      ...
      if (!name.equals(key)) System.out.println(key + " <<>> " + name);
    }
    System.out.println("-----------");
  }
      ...
      for (int classIndex = 0; classIndex < classes.size(); classIndex++) {
        ...
        if (!Modifier.isAbstract(actualClass.getModifiers())
          ...
          if (frame) {
            ...
            } else if (packageName.equalsIgnoreCase("tinker")
                || packageName.equalsIgnoreCase("graph")) {
  ...
  */
  ...
  private static void createReactorHash(Map<String, Class<? extends IReactor>> reactorHash) {
    ...
    reactorHash.put(
        "FindSemanticColumnOwlRelationships", FindSemanticColumnOwlRelationshipsReactor.class);
    reactorHash.put(
        "FindSemanticInstanceOwlRelationships", FindSemanticInstanceOwlRelationshipsReactor.class);
    ...
    reactorHash.put(
        "RunPlaysheetMethod", prerna.reactor.legacy.playsheets.RunPlaysheetMethodReactor.class);
    reactorHash.put("RunPlaysheet", prerna.reactor.legacy.playsheets.RunPlaysheetReactor.class);
    reactorHash.put(
        "GetPlaysheetParams", prerna.reactor.legacy.playsheets.GetPlaysheetParamsReactor.class);
  ...
  private static void populateNativeFrameHash(
      Map<String, Class<? extends IReactor>> nativeFrameHash) {}
  ...
  private static void populateRFrameHash(Map<String, Class<? extends IReactor>> rFrameHash) {
    ...
    rFrameHash.put(
        "RegexReplaceColumnValue", prerna.reactor.frame.r.RegexReplaceColumnValueReactor.class);
    ...
    rFrameHash.put(
        ...
        prerna.reactor.frame.r.analytics.RunAssociatedLearningReactor.class);
    rFrameHash.put(
        "RunClassification", prerna.reactor.frame.r.analytics.RunClassificationReactor.class);
    rFrameHash.put("RunClustering", prerna.reactor.frame.r.analytics.RunClusteringReactor.class);
    rFrameHash.put(
        "RunDescriptionGenerator",
        prerna.reactor.frame.r.analytics.RunDescriptionGeneratorReactor.class);
    rFrameHash.put(
        "RunDocCosSimilarity", prerna.reactor.frame.r.analytics.RunDocCosSimilarityReactor.class);
    rFrameHash.put(
        "RunDocumentSummarization",
        prerna.reactor.algorithms.RunDocumentSummarizationReactor.class);
    rFrameHash.put("RunLOF", prerna.reactor.frame.r.analytics.RunLOFReactor.class);
    rFrameHash.put(
        "RunMatrixRegression", prerna.reactor.frame.r.analytics.RunMatrixRegressionReactor.class);
    rFrameHash.put(
        "RunNumericalColumnSimilarity",
        prerna.reactor.frame.r.analytics.RunNumericalColumnSimilarityReactor.class);
    rFrameHash.put(
        "RunNumericalCorrelation",
        prerna.reactor.frame.r.analytics.RunNumericalCorrelationReactor.class);
    rFrameHash.put(
        "RunNumericalModel", prerna.reactor.frame.r.analytics.RunNumericalModelReactor.class);
    rFrameHash.put("RunOutlier", prerna.reactor.frame.r.analytics.RunOutlierReactor.class);
    rFrameHash.put(
        "RunRandomForest", prerna.reactor.frame.r.analytics.RunRandomForestReactor.class);
    ...
    rFrameHash.put(
        "RunSimilarityHeat", prerna.reactor.frame.r.analytics.RunSimilarityHeatReactor.class);
    rFrameHash.put("MatchColumnValues", prerna.reactor.frame.r.MatchColumnValuesReactor.class);
    rFrameHash.put(
        "UpdateMatchColumnValues", prerna.reactor.frame.r.UpdateMatchColumnValuesReactor.class);
    //		rFrameHash.put("MetaSemanticSimilarity",
    // prerna.sablecc2.reactor.frame.r.MetaSemanticSimilarityReactor.class);
  ...
  private static void populateTinkerFrameHash(
      Map<String, Class<? extends IReactor>> tinkerFrameHash) {
  ...
  private static void populatePandasFrameHash(
      Map<String, Class<? extends IReactor>> pandasFrameHash) {
    ...
    pandasFrameHash.put(
        "ReplaceColumnValue", prerna.reactor.frame.py.ReplaceColumnValueReactor.class);
    ...
    pandasFrameHash.put(
        "MatchColumnValues", prerna.reactor.frame.py.MatchColumnValuesReactor.class);
  ...
  public static IReactor getReactor(
      Insight insight, String reactorId, IReactor parentReactor, ITableDataFrame frame) {
    if (insight != null) {
   ...
   * @return This will simply return the IReactor responsible for execution based on the reactorId
   *     <p>Special case: if we are dealing with an expression, we determine if this expression is
   *     part of a select query or should be reduced If it is a reducing expression we 1. create an
   *     expr reactor 2. grab the reducing expression reactor from the frame 3. set that reactor to
   *     the expr reactor and return the expr reactor The expr reactor when executed will use that
   *     reducing expression reactor to evaluate
   ...
   */
  public static IReactor getReactor(
      String reactorId, ITableDataFrame frame, IReactor parentReactor) {
      ...
      if (frame != null) {
        ...
        if (frame instanceof AbstractRdbmsFrame) {
          ...
          if (tinkerFrameHash.containsKey(reactorId)) {
            reactor =
                (IReactor) tinkerFrameHash.get(reactorId).getDeclaredConstructor().newInstance();
          }
        } else if (frame instanceof NativeFrame) {
          if (nativeFrameHash.containsKey(reactorId)) {
            reactor =
                (IReactor) nativeFrameHash.get(reactorId).getDeclaredConstructor().newInstance();
          }
        } else if (frame instanceof PandasFrame) {
          if (pandasFrameHash.containsKey(reactorId)) {
            reactor =
                (IReactor) pandasFrameHash.get(reactorId).getDeclaredConstructor().newInstance();
          }
        }
  ...
  public static boolean hasReactor(String reactorId) {
    return reactorHash.containsKey(reactorId)
        || expressionHash.containsKey(reactorId.toUpperCase());
  }

  /**
   * This method takes in a prop file to build the reactorHash
   *
   * @param propFile - the path to the prop file with the reactor names and classes
   * @param reactorHash - the specific reactor hash object that we are building
   */
  public static void buildReactorHashFromPropertyFile(
      Map<String, Class<? extends IReactor>> hash, String propFile) {
  ...
  /**
   * Loads the JSON for additional reactors IF NAMES COLLIDE, THE PROP FILE WILL TAKE PRECEDENCE
   *
   * @param jsonFile
   */
