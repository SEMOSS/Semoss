...
public class MyEngineProjectReactor extends AbstractReactor {
  ...
  public MyEngineProjectReactor() {
    this.keysToGet =
        new String[] {
          ...
          ReactorKeysEnum.SUB_TYPE.getKey(),
  ...
  public NounMetadata execute() {
    ...
    // Common filters
    Boolean noMeta =
        Boolean.parseBoolean(String.valueOf(this.keyValue.get(ReactorKeysEnum.NO_META.getKey())));
    Boolean portalsOnly =
        Boolean.parseBoolean(
            String.valueOf(this.keyValue.get(ReactorKeysEnum.ONLY_PORTALS.getKey())));
    Boolean favoritesOnly =
        Boolean.parseBoolean(
            String.valueOf(this.keyValue.get(ReactorKeysEnum.ONLY_FAVORITES.getKey())));
    ...
    for (int i = 0; i < typesToGet.size(); i++) {
      ...
      if ("PROJECT".equalsIgnoreCase(currentType)) {
        result =
            getProjects(
                // List.of("CODE", "BLOCKS"),
                // for right now, do not apply filter on project type since it is not properly in
                // some smss files
                null,
                favoritesOnly,
                portalsOnly,
    ...
    return new NounMetadata(
        combinedInfo, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.DATABASE_INFO);
  }
  ...
  private List<Map<String, Object>> getProjects(
      ...
      boolean favoritesOnly,
      boolean portalsOnly,
      Map<String, Object> engineProjectMetadataFilter,
   ...
   * @return
   */
  private List<Map<String, Object>> getEngines(
