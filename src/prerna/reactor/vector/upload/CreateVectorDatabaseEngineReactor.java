...
public class CreateVectorDatabaseEngineReactor extends AbstractReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(CreateVectorDatabaseEngineReactor.class);
  ...
  public NounMetadata execute() {
    ...
    if (user == null) {
      NounMetadata noun =
          new NounMetadata(
              "User must be signed into an account in order to create a model engine",
    ...
    if (vectorDbType != VectorDatabaseTypeEnum.PROXY
      ...
      String embedderEngineId =
          (String) vectorDbDetails.getOrDefault(Constants.EMBEDDER_ENGINE_ID, null);
    ...
    try {
      ...
      specificEngineFolder =
          UploadUtilities.generateSpecificEngineFolder(
              IEngine.CATALOG_TYPE.VECTOR, vectorDbId, vectorDbName);
      ...
      tempSmss =
          UploadUtilities.createTemporaryVectorDatabaseSmss(
              vectorDbId, vectorDbName, vectorDbClass, vectorDbDetails);

      // store in DIHelper so that when we move temp smss to smss it doesn't try to reload again
      DIHelper.getInstance()
          .setEngineProperty(vectorDbId + "_" + Constants.STORE, tempSmss.getAbsolutePath());
    ...
    Map<String, Object> retMap =
        UploadUtilities.getEngineReturnData(this.insight.getUser(), vectorDbId);
  ...
  private void cleanUpCreateNewError(
      IVectorDatabaseEngine vectorEngine,
      String modelId,
      File tempSmss,
      File smssFile,
