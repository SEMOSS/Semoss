...
public class RerankReactor extends AbstractReactor {
  ...
  public RerankReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.FUNCTION.getKey(), ReactorKeysEnum.LIMIT.getKey()};
  ...
  public NounMetadata execute() {
    ...
    if (!SecurityEngineUtils.userCanViewEngine(this.insight.getUser(), engineId)) {
      throw new IllegalArgumentException(
          "Function " + engineId + " does not exist or user does not have access to it.");
    }

    IFunctionEngine functionEngine = Utility.getFunctionEngine(engineId);

    Object question =
        this.insight.getVar(AbstractVectorDatabaseEngine.LATEST_VECTOR_SEARCH_STATEMENT);
  ...
  /**
   * Determine the input limit for the number of results returned from the reranker. The default
   * value is 5.
   *
   * @return
   */
