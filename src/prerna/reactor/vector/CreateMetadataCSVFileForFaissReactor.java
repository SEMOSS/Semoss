...
public class CreateMetadataCSVFileForFaissReactor extends AbstractReactor {

	private static final Logger classLogger = LogManager.getLogger(CreateMetadataCSVFileForFaissReactor.class);
	
	private final String PATH_TO_UNZIP_FILES = "zipFileExtractFolder";
	private final String FILE_PATHS_KEY = "filePaths";
	
	public CreateMetadataCSVFileForFaissReactor() {
		this.keysToGet = new String[] {ReactorKeysEnum.ENGINE.getKey(), FILE_PATHS_KEY, 
				ReactorKeysEnum.PARAM_VALUES_MAP.getKey(), ReactorKeysEnum.SPACE.getKey() };
	...
	public NounMetadata execute() {
		...
		if(paramMap == null || paramMap.isEmpty()) {
			throw new IllegalArgumentException("Metadata Map is missing in paramValues");
		}
		...
		List<Map<String, String>> metadataStatusList = null;
		try {
			...
			FaissDatabaseEngine faissEngine = (FaissDatabaseEngine) vectorDatabase;
			metadataStatusList = faissEngine.addMetadataFaiss(validFiles, this.insight, paramMap);
		...
		NounMetadata noun = new NounMetadata(metadataStatusList, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.OPERATION);
	...
	/**
	 * Get the metadata map from the paramValues noun store
	 */
	...
	public String getReactorDescription() {
		return "Generates metadata CSV file for documents uploaded to Faiss vector database engine.";
	}

	@Override
	protected String getDescriptionForKey(String key) {
		if (key.equals(ReactorKeysEnum.ENGINE.getKey())) {
			return "Gets the engine Id of the faiss vector database engine.";
		} else if (key.equals(ReactorKeysEnum.PARAM_VALUES_MAP.getKey())) {
			return "This is a required field which contains the key-value map of metadata.";
		}
