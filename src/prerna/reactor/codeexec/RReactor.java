...
public final class RReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    if (smartSync) {
      ...
      if (insight.getProperty("FIRST_SYNC") == null) {
        ...
        insight
            .getVarStore()
            .put("FIRST_SYNC", new NounMetadata("True", PixelDataType.CONST_STRING));
      }
    }
  ...
  public NounMetadata handleGGPlot(String ggplotCommand) {
    ...
    if (!animate) ggplotter.append("ggsave(" + ggsaveFile + ");");
    else ggplotter.append("anim_save(" + ggsaveFile + ", " + plotString + ");");
    ...
    String ggremove =
        "rm("
            + ggsaveFile
            + ", txt);detach(\"package:ggplot2\", unload=FALSE);"; // detach(\"package:RCurl\",
    // unload=FALSE)";
    if (animate) ggremove = ggremove + "detach(\"package:gganimate\", unload=FALSE);";
    ...
    // {0={layout=GGPlot, alignment={ggplot=[], selectors=[MovieBudget,
    // RevenueDomestic]}}}
    ...
    if (newWindow) panelMap.put(panelId + panelNum, optionMap);
    else panelMap.put(this.insight.getLastPanelId(), optionMap);
    ...
    if (animate) ggplotCommand = ggplotCommand + " + animate";
    ...
    if (bin.length() > 0) {
      ...
      if (newWindow) {
        ...
        NounMetadata noun =
            new NounMetadata(daPanel, PixelDataType.PANEL, PixelOperationType.PANEL_OPEN);
        retList.add(noun);
        retList.add(
            new NounMetadata(
                cdt,
                PixelDataType.FORMATTED_DATA_SET,
