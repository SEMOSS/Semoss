...
public class Lambda implements Iterator {
  ...
  public void makeQuery() {
    ...
    String tableName = "";
    if (this.frame != null) tableName = frame.getName();
    ...
    if (cols != null) {
      for (int colIndex = 0; cols != null && colIndex < cols.size(); colIndex++) {
        ...
        if (column.length == 1) qs.addSelector(tableName, input);
        else qs.addSelector(tableName, input);
      }
    }
    if (filters != null) {
      for (int filterIndex = 0; filterIndex < filters.size(); filterIndex++) {
        ...
        if (values.size() > 0) {
          ...
          if (type == PixelDataType.CONST_STRING) {
            ...
            qs.addFilter(
                filter.getLComparison().get(0).toString(), filter.getComparator(), strVector);
          } else {
            ...
            qs.addFilter(
                filter.getLComparison().get(0).toString(), filter.getComparator(), decVector);
          }
        }
      }
    }
  ...
  public Object[] getRow(String inputStringRef, IHeadersDataRow curRow) {
    ...
    for (int inputIndex = 0; inputIndex < inputStrings.length; inputIndex++) {
      ...
      if (retRow[inputIndex] == null) retRow[inputIndex] = store.get(inputStrings[inputIndex]);
    }
