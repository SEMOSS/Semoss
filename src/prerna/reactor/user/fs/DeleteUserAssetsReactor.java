...
public class DeleteUserAssetsReactor extends AbstractReactor {

	private static final Logger classLogger = LogManager.getLogger(DeleteUserAssetsReactor.class);

	public DeleteUserAssetsReactor() {
	...
	public NounMetadata execute() {
		...
		String versionGitFolder = AssetUtility.getUserAssetVersionFolder(project.getProjectName(),
		...
		try (Git git = Git.open(new File(versionGitFolder))) {
			...
			classLogger.error("Error staging deleted files in git for user asset project {}", project.getProjectId(),
					e);
		}
		// commit it
	...
	public String getReactorDescription() {
		return "Delete a single or multiple files in the user's assets folder";
	}
	...
	protected String getDescriptionForKey(String key) {
		if (key.equals(ReactorKeysEnum.FILE_PATH.getKey())) {
			...
			return "Comment to add while removing the files within the git repository for the user's asset project";
		}
