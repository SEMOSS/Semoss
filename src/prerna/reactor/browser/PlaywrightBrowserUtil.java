...
public class PlaywrightBrowserUtil {
  ...
  /**
   * functions we need open close click xy get all forms get all input fields get all the links get
   * screenshot click link upload - wow this will be interesting..
   */
  public void open(JSONObject obj) {
    if (ctx == null) initPlaywright();
  ...
  public void initPlaywright() {
    ...
    if (this.playwright == null) this.playwright = Playwright.create();
  ...
  public void mimicUser() {
    try {
      ...
      while (!done) {
        ...
        if (obj.getString("actor").equalsIgnoreCase("qq")) break;
    ...
    try {
      for (int keyIndex = 0; keyIndex < keys.length; keyIndex++) {
        ...
        if (key.equalsIgnoreCase("website")) {
          if (baseUrl == null) value = br.readLine();
          else continue;
          kv.put(key, value);
  ...
  private boolean processAction(JSONObject action, String actionId) {
    ...
    String actor = action.getString("actor");
    boolean critical =
        action.has("critical") && action.getString("critical").equalsIgnoreCase("True");
    ...
    if (actor.equalsIgnoreCase("system")) {
      try {
        ...
        // TODO Auto-generated catch block
        e.printStackTrace();
        if (!critical) System.err.println("Failed " + actionId);
        else {
          go = false;
        }
      }
    ...
    if (action.has("debug")
        && action.getString("debug").equalsIgnoreCase("true")
        && locators.containsKey(actionId)) System.err.println(locators.get(actionId).innerHTML());
  ...
  public void getByPlaceHolder(JSONObject payload, String actionName) {
    ...
    if (payload.has("event")) {
      ...
      if (event.equalsIgnoreCase("click")) loc.click();
  ...
  public void mouse_xy(JSONObject payload, String actionName) {
    ...
    if (event.equalsIgnoreCase("click")) {
      ...
      if (payload.has("options")) {
        ...
        if (option.equalsIgnoreCase("left")) op.setButton(MouseButton.LEFT);
        if (option.equalsIgnoreCase("right")) op.setButton(MouseButton.RIGHT);
        if (option.equalsIgnoreCase("middle")) op.setButton(MouseButton.MIDDLE);
      }
  ...
  public void getByLabel(JSONObject payload, String actionName) {
    ...
    Locator loc = null;
    int sleep = 400;
    if (payload.has("sleep_after")) sleep = payload.getInt("sleep_after");
  ...
  public void getUserInput(JSONObject payload, String actionId) {
    ...
    if (payload.has("parent_action")) {
      ...
      boolean search =
          payload.has("search") && payload.getString("search").equalsIgnoreCase("true");
      ...
      if (loc != null) {
        do {
          ...
          if (payload.has("message")) message = payload.getString("message");
          try {
            System.err.println(message);
            data = br.readLine();
            if (data.startsWith("!")
                && search) // this is a search we found our value come out of it
            break;
            ...
            if (sync) trace(actionId, "- User - " + data);
          } catch (IOException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
          }
  ...
  public String getScreenShot() {
    ...
    try {
      ...
      String filePath =
          Paths.get(outputDir + "/" + outputName + ".png")
              .toString(); // Change this to the path of your image file
  ...
  private GetByRoleOptions createByRoleOptions(JSONObject payload) {
    ...
    if (payload.has("options")) {
      ...
      for (int optionIndex = 0; optionIndex < optionArray.length(); optionIndex++) {
        ...
        if (optionName.equalsIgnoreCase("setName")) options.setName(optionValue);
      }
    }

    options.setExact(
        payload.has("options_exact")
            && payload.getString("options_exact").equalsIgnoreCase("True"));
    return options;
  }
  ...
  private FilterOptions createFilterOptions(JSONObject payload) {
    ...
    if (payload.has("options")) {
      ...
      for (int optionIndex = 0; optionIndex < optionArray.length(); optionIndex++) {
        String optionName = optionArray.getString(optionIndex);
        String optionValue = optionValues.getString(optionIndex);
        if (optionName.equalsIgnoreCase("setHasText")) options.setHasText(optionValue);
      }
    }
    return options;
  }
  ...
  private Locator resolveLocator(List<Locator> allLoc, JSONObject payload) {
    ...
    Locator loc = null;
    if (allLoc.size() == 1) loc = allLoc.get(0);
    ...
    } else if (allLoc.size() > 0) loc = allLoc.get(0);
    ...
    if (payload.has("super_navigate") && loc != null) {
      ...
      for (int navIndex = 0; navIndex < navLevels; navIndex++) loc = loc.locator("..");
    }
    ...
    while (!success) {
      try {
        ...
        if (this.tracePage) {
          ...
          FileUtils.write(
              new File(outputDir + "/" + outputName + ".html"),
              htmlContent,
        ...
        if (this.traceScreenshot) {
          cur_page.screenshot(
              new Page.ScreenshotOptions()
                  .setPath(Paths.get(outputDir + "/" + outputName + ".png")));
        }
        success = true;
      } catch (IOException e) {
        // TODO Auto-generated catch block
        ...
        if (message.contains("navigating") && attempt < 4) attempt++;
        else success = true;
  ...
  public String convertRelativeToAbsoluteLinks(String baseUrl, String page) {
    ...
    try {
      ...
      for (int formIndex = 0; formIndex < forms.size(); formIndex++) {
        ...
        // then get the button.. the button is what we need to click once the user fills basically -
        // dont need the button since it will be a click event
  ...
  private List makeObservable(Elements links, String name, String event) {
    ...
    for (int linkIndex = 0; linkIndex < links.size(); linkIndex++) {
      ...
      // link.attr(event, "alert(document.querySelector('[" + css_id + "=" + name + linkIndex +
      // "]').value)");
      ...
      if (link.hasAttr("id")) inputLinks.put(link.attr("id"), link);
      if (link.hasAttr("name")) inputLinks.put(link.attr("name"), link);
    }
