...
public class DatabaseMetadataToPdfReactor extends AbstractReactor {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(DatabaseMetadataToPdfReactor.class);
  ...
  public NounMetadata execute() {
    ...
    Map<String, Object> databaseInfo =
        SecurityEngineUtils.getUserEngineList(this.insight.getUser(), databaseId, null).get(0);
    ...
    if (databaseInfo.containsKey("description")
        && !((String) databaseInfo.get("description")).isEmpty()) {
      htmlBuilder.append("<h3>Description: " + databaseInfo.get("description") + "</h3>");
    }
    if (databaseInfo.containsKey("tags")
        && !((Collection<String>) databaseInfo.get("tags")).isEmpty()) {
    ...
    for (Object nodeObject : nodes) {
      ...
      for (String prop : propNames) {
        ...
        String physicalDataType =
            ((Map<String, String>) metamodelObject.get("physicalTypes")).get(uid);
