...
public class CheckRPackagesReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    return new NounMetadata(
        returnMap, PixelDataType.CUSTOM_DATA_STRUCTURE, PixelOperationType.CHECK_R_PACKAGES);
  }
  ...
  private void setPackages(boolean reload) {
    if (reload || pkgs == null) {
      ...
      try {
        ...
        CheckRPackagesReactor.pkgs =
            rJavaTranslator.getStringArray(
                "as.character(unique(data.frame(installed.packages())$Package))");
