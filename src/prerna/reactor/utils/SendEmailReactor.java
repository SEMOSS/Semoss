...
public class SendEmailReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
        ...
        if (messageFile.exists() && messageFile.isFile()) {
          try {
            ...
            throw new IllegalArgumentException(
                "Error reading message file. Check logs for details.");
          }
        }
      }
    ...
    //		if (message == null || (message=message.trim()).isEmpty()) {
    //			throw new IllegalArgumentException("Need to define the email message as " + EMAIL_MESSAGE +
    // " or passing in file location with message body");
    //		}
    ...
    if (Boolean.parseBoolean(this.keyValue.get(ReactorKeysEnum.MUSTACHE.getKey()) + "")) {
      ...
      try {
        ...
        throw new IllegalArgumentException(
            "Invalid mustache template or variables. Detailed error message = " + e.getMessage(),
            e);
      }
    ...
    if ((smtpHost == null || smtpHost.isEmpty()) && (smtpPort == null || smtpPort.isEmpty())) {
      ...
      if (emailSession == null) {
        ...
        if (!SocialPropertiesUtil.getInstance().smtpEmailEnabled()) {
          throw new IllegalArgumentException(
              "Need to define an smtp server to utilize this function");
        }
    ...
    boolean success =
        EmailUtility.sendEmail(
            emailSession, to, cc, bcc, sender, subject, message, isHtml, attachments);
  ...
  private Session getEmailSessionFromCall() {
    ...
    List<NounMetadata> mapInputs =
        emailSessionGrs.getNounsOfType(PixelDataType.PROJECT_EMAIL_SESSION);
   ...
   * @return
   */
  private Session contrustOneTimeEmailSession(
