...
public class ImageCaptureReactor extends AbstractReactor {
    ...
    try {
      wrapper =
          WrapperManager.getInstance()
              .getRawWrapper(insightsEng, "select distinct id from question_id");
  ...
  public static void runImageCapture(
      String feUrl,
      String projectId,
      String insightId,
      String params,
   ...
   * @param sessionId
   */
  private static void runHeadlessChrome(
      String feUrl, Insight insight, String params, String sessionId, Integer waitTime) {
    feUrl = feUrl.trim();
    ...
    if (params == null) {
      ...
      //				DIR_SEPARATOR + id;
      imageDirStr =
          AssetUtility.getProjectVersionFolder(projectName, projectId) + DIR_SEPARATOR + id;
    } else {
    ...
    insight
        .getChromeDriver()
        .captureImage(feUrl, url, imageDirStr + DIR_SEPARATOR + "image.png", sessionId, waitTime);
  }
  ...
  // "#!/insight?type=single&engine=" + engineId + "&id=" + id + "&parameters=" + params +
