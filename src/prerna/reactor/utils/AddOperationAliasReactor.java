...
public class AddOperationAliasReactor extends AbstractReactor {
  ...
  /**
   * This reactor takes in the name of a reactor and allows the user to assign it an alias The
   * inputs to the reactor are: 1) the name of the reactor 2) the alias for the reactor
   */
  public AddOperationAliasReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.REACTOR.getKey(), ReactorKeysEnum.ALIAS.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    //		Properties prop = addReactorAliasToProperties(ReactorFactory.REACTOR_PROP_PATH, reactorName,
    // alias);
    ...
    //			}
    //
    //			prop = addReactorAliasToProperties(ReactorFactory.NATIVE_FRAME_PROP_PATH, reactorName,
    // alias);
    //			if (prop != null && prop.size() > 0) {
    ...
    //			}
    //
    //			prop = addReactorAliasToProperties(ReactorFactory.TINKER_FRAME_PROP_PATH, reactorName,
    // alias);
    //			if (prop != null && prop.size() > 0) {
