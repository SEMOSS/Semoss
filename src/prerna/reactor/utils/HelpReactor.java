...
public class HelpReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (isAdmin) {
      ...
      return new NounMetadata(
          HelpReactor.adminHelpString, PixelDataType.CONST_STRING, PixelOperationType.HELP);
    } else {
      if (reset || helpString == null) {
        HelpReactor.helpString = createHelpString(false);
      }
      return new NounMetadata(
          HelpReactor.helpString, PixelDataType.CONST_STRING, PixelOperationType.HELP);
    }
  }
  ...
  private String createHelpString(boolean isAdmin) {
    ...
    if (expressionSet.size() > 0) {
      allReactors
          .append("Expression Set Reactors: \n")
