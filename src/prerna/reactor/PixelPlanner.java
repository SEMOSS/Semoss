...
public class PixelPlanner {
  ...
  public void addOutputs(
      String opName, List<String> outputs, List<String> outputTypes, IReactor.TYPE opType) {
    opName = opName.trim();
    // find the vertex for each of the input
    // wonder if this should be a full fledged block of java ?
    Vertex opVertex = upsertVertex(OPERATION, opName);
  ...
  public void addInputs(
      String opName, List<String> inputs, List<String> asInputs, IReactor.TYPE opType) {
    // find the vertex for each of the input
    // wonder if this should be a full fledged block of java ?
    Vertex opVertex = upsertVertex(OPERATION, opName);
  ...
  public void addInputs(
      String opName, CodeBlock codeBlock, List<String> inputs, IReactor.TYPE opType) {
  ...
  public void addInputs(
      ...
      CodeBlock codeBlock,
      List<String> inputs,
      List<String> asInputs,
      IReactor.TYPE opType) {
  ...
  public Iterator runMyPlan(String output) {
    ...
    // when there is a reduce which multiple thing depends on, I need to validatee so that it
    // doesn't do it twice
  ...
  private StageKeeper loadVertex3(StageKeeper keeper, List<Vertex> verticesToProcess) {
    ...
    if (keeper.lastStage != null) thisStage = keeper.lastStage;
    else {
      thisStage = new Stage();
    ...
    System.out.println(
        "Processing Vertex >>>>>>>>>>>>>>>>>>>> " + thisVertex.property(TINKER_NAME));
    ...
    while (inputs.hasNext()) {
      ...
      if (opVertices.hasNext()) // ok there is an operation
      {
        ...
        if (verticesToProcess.indexOf(thisOp) >= 0) verticesToProcess.remove(thisOp);
        ...
        if (codeBlock instanceof String) opInputs.add(nounName);
        else // this is a derived call
        deInputs.add(nounName);
      } else // this is the base input with no parent operation
      {
        if (opInputs.indexOf(nounName) < 0) opInputs.add(nounName);
      }
    }
  ...
  private String loadVertex2(Stack<Stage> codeStack, Vector<Vertex> verticesToProcess) {
    ...
    // I need to break this into stages i.e. the codeStack
    Stage thisStage = null;
    if (codeStack.size() > 0) thisStage = codeStack.pop();
    else {
      thisStage = new Stage();
      thisStage.stageNum = 0;
    }
  ...
  // whenever there is an existing piece of code it should take the order and reinsert it to top
  // i.e. vector
  // I also need
  private String loadVertex(Stack<Hashtable<String, Object>> codeStack) {
    ...
    String curCode = "}";
    // we will refine this code piece later
    if (curHash.containsKey("CODE")) curCode = (String) curHash.get("CODE");
    ...
    while (inputs.hasNext()) {
      ...
      if (opVertices.hasNext()) // ok there is an operation
      {
        ...
        // need to close this block after this is done
        codeBlock = thisOp.property("CODE").value();
        if (codeBlock instanceof String) inputCols.add(nounName);
        else // this is a derived call
    ...
    if (opType != IReactor.TYPE.REDUCE) {
      ...
      for (int inputIndex = 0; inputIndex < inputCols.size(); inputIndex++)
        if (allInputCols.indexOf(inputCols.elementAt(inputIndex)) < 0)
          allInputCols.add(inputCols.elementAt(inputIndex));
      ...
      String mapOp =
          "OPERATION NAME : "
              + thisVertex.property(TINKER_NAME).value()
              + " \n COLUMN SUBSET..  "
              + nounSoFar
              + "\n";
    ...
    } else // this is a reduce operation
    {
      ...
      String reduceLogic =
          "// here comes the reduce logic :) " + thisVertex.property(TINKER_NAME).value();
    ...
    if (verticesToProcess.size() > 0) {
      ...
      String foreach = "while(result.hasNext()) { \n";
      String mapLogic =
          "// here comes map logic :) " + thisVertex.property(TINKER_NAME).value() + "\n";
