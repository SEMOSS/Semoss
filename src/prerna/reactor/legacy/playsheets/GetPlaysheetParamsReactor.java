...
public class GetPlaysheetParamsReactor extends AbstractReactor {
	...
	// "SELECT DISTINCT PARAMETER_LABEL, PARAMETER_TYPE, PARAMETER_OPTIONS,
	// PARAMETER_QUERY,
	// PARAMETER_DEPENDENCY, PARAMETER_IS_DB_QUERY, PARAMETER_MULTI_SELECT,
	// PARAMETER_COMPONENT_FILTER_ID FROM PARAMETER_ID WHERE PARAMETER_ID = '" +
	// PARAMETER_ID_PARAM_KEY + "'";
	private static final String GET_INFO_FOR_PARAM = "SELECT DISTINCT " + "PARAMETER_LABEL, " + "PARAMETER_TYPE, "
			+ "PARAMETER_OPTIONS, " + "PARAMETER_QUERY, " + "PARAMETER_DEPENDENCY, " + "PARAMETER_IS_DB_QUERY, "
			+ "PARAMETER_MULTI_SELECT, " + "PARAMETER_COMPONENT_FILTER_ID " + "FROM PARAMETER_ID "
	...
	public List<Object> getParamOptions(IProject project, String parameterID) {
		...
		if (!retParam.isEmpty()) {
			SEMOSSParam ourParam = retParam.get(0); // there should only be one as we are getting the param from a
													// specific param URI
			...
			if (options != null && !options.isEmpty()) {
				...
				// if options are not defined, need to get uris either from custom sparql or
				// type
				...
				// RDBMS right now does type:type... need to get just the second type. This will
				// be fixed
				// once insights don't store generic query
				// TODO: fix this logic. need to decide how to store param type for rdbms
				if (paramQuery != null && !paramQuery.isEmpty()) {
					// TODO: rdbms has type as null... this is confusing given the other comments
					// here....
