...
public class GetPlaysheetParamsReactor extends AbstractReactor {
  ...
  // "SELECT DISTINCT PARAMETER_LABEL, PARAMETER_TYPE, PARAMETER_OPTIONS, PARAMETER_QUERY,
  ...
  private static final String GET_INFO_FOR_PARAM =
      "SELECT DISTINCT "
          + "PARAMETER_LABEL, "
          + "PARAMETER_TYPE, "
          + "PARAMETER_OPTIONS, "
  ...
  public List<Object> getParamOptions(IProject project, String parameterID) {
    ...
    if (!retParam.isEmpty()) {
      SEMOSSParam ourParam =
          retParam.get(
              0); // there should only be one as we are getting the param from a specific param URI
      ...
      if (options != null && !options.isEmpty()) {
        ...
        // RDBMS right now does type:type... need to get just the second type. This will be fixed
