...
public class LegacyInsightDatabaseUtility {
  ...
  private static final Logger classLogger =
      LogManager.getLogger(LegacyInsightDatabaseUtility.class);
  ...
  private static final String GET_ALL_PARAMS_FOR_QUESTION_ID =
      "SELECT DISTINCT PARAMETER_LABEL, PARAMETER_TYPE, PARAMETER_OPTIONS, PARAMETER_QUERY, PARAMETER_DEPENDENCY, PARAMETER_IS_DB_QUERY, PARAMETER_MULTI_SELECT, PARAMETER_COMPONENT_FILTER_ID, PARAMETER_ID FROM PARAMETER_ID WHERE QUESTION_ID_FK = "
          + QUESTION_ID_FK_PARAM_KEY;

  // get param information when i have the param id
  private static final String PARAMETER_ID_PARAM_KEY = "@PARAMETER_ID";
  private static final String GET_INFO_FOR_PARAMS =
      "SELECT DISTINCT PARAMETER_LABEL, PARAMETER_TYPE, PARAMETER_OPTIONS, PARAMETER_QUERY, PARAMETER_DEPENDENCY, PARAMETER_IS_DB_QUERY, PARAMETER_MULTI_SELECT, PARAMETER_COMPONENT_FILTER_ID FROM PARAMETER_ID WHERE PARAMETER_ID IN ("
          + PARAMETER_ID_PARAM_KEY
          + ")";
   ...
   * @return
   */
  public static List<SEMOSSParam> getParamsFromInsightId(
      IDatabaseEngine insightRdbms, String insightId) {
  ...
  public static Vector<SEMOSSParam> getParamsFromParamIds(
      IDatabaseEngine insightRdbms, String... paramIds) {
    String pIdString = "";
    ...
    while (wrap.hasNext()) {
      ...
      SEMOSSParam param = new SEMOSSParam();
      param.setName(label);
      if (ss.getVar(names[1]) != null) param.setType(ss.getVar(names[1]) + "");
      if (ss.getVar(names[2]) != null) param.setOptions(ss.getVar(names[2]) + "");
      if (ss.getVar(names[3]) != null) param.setQuery(ss.getVar(names[3]) + "");
      if (ss.getRawVar(names[4]) != null) param.addDependVar(ss.getRawVar(names[4]) + "");
      ...
      if (ss.getVar(names[0]) != null) param.setParamID(ss.getVar(names[0]) + "");
