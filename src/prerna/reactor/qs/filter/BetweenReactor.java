...
public class BetweenReactor extends FilterReactor {
  ...
  protected AbstractQueryStruct createQueryStruct() {
    ...
    if (oColumn instanceof QueryColumnSelector) bqf.setColumn((QueryColumnSelector) oColumn);

    if (filters.size() >= 1) bqf.setStart(filters.get(1));

    if (filters.size() >= 2) bqf.setEnd(filters.get(2));
