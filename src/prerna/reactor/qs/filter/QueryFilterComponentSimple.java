...
import prerna.reactor.qs.selectors.IfReactor;
...
public class QueryFilterComponentSimple extends FilterReactor {
	...
	private SimpleQueryFilter generateFilterObject() {
		...
		for (NounMetadata noun : this.curRow.vector) {
			...
			// this is a special case where we need to turn the rset into the selector of
			// the query struct
			// if I adjust this everything else gets adjusted
			if ((this.parentReactor instanceof IfReactor || this.parentReactor instanceof IfQueryFilterComponentAnd
					|| this.parentReactor instanceof IfQueryFilterComponentOr)
					&& noun.getNounType() == PixelDataType.QUERY_STRUCT)
				noun = new NounMetadata(((SelectQueryStruct) noun.getValue()).getSelectors().get(0),
		...
		if (nouns.size() == 0) {
			...
			for (int i = 0; i < nouns.size(); i++) {
				...
				if (subNoun.getNounType() == PixelDataType.TASK
						|| subNoun.getNounType() == PixelDataType.FORMATTED_DATA_SET) {
