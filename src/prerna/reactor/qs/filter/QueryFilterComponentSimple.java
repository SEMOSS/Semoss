  ...
  private SimpleQueryFilter generateFilterObject() {
    ...
    for (NounMetadata noun : this.curRow.vector) {
      ...
      if ((this.parentReactor instanceof IfReactor
              || this.parentReactor instanceof IfQueryFilterComponentAnd
              || this.parentReactor instanceof IfQueryFilterComponentOr)
          && noun.getNounType() == PixelDataType.QUERY_STRUCT)
        noun =
            new NounMetadata(
                ((SelectQueryStruct) noun.getValue()).getSelectors().get(0), PixelDataType.COLUMN);
    ...
    SimpleQueryFilter filter =
        new SimpleQueryFilter(getNounForFilter(lSet), comparator, getNounForFilter(rSet));
    return filter;
  }
