...
public class QuerySelectorExpressionAssimilator extends AbstractReactor {
	...
	/**
	 * Since query expression assimilators are embedded within each other We set an
	 * encapsulated boolean to account for logic with modifying for OOO
	 *
	 * @param encapsulated
	 */
	...
	/**
	 * Method to correctly set up left and right selectors based on order of
	 * operations
	 *
	 ...
	 * @return
	 */
	public static QueryArithmeticSelector combineSelectorsModingForOOO(String mathExpression,
			IQuerySelector leftSelector, IQuerySelector rightSelector) {
		...
		if (mathExpression.equals("*") || mathExpression.equals("/")) {
			...
			if (leftSelector.getSelectorType() == IQuerySelector.SELECTOR_TYPE.ARITHMETIC
				...
				if (leftSideMathExpression.equals("-") || leftSideMathExpression.equals("+")) {
					...
					// (leftOfLeftSelector + rightOfLeftSelector) * rightSelector ->
					// leftOfLeftSelector +
