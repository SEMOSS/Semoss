...
public class QuerySelectorExpressionAssimilator extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    QueryArithmeticSelector newSelector =
        combineSelectorsModingForOOO(this.mathExpr, leftSelector, rightSelector);
  ...
  /**
   * Since query expression assimilators are embedded within each other We set an encapsulated
   * boolean to account for logic with modifying for OOO
   *
   * @param encapsulated
   */
   ...
   * @return
   */
  public static QueryArithmeticSelector combineSelectorsModingForOOO(
      String mathExpression, IQuerySelector leftSelector, IQuerySelector rightSelector) {
    ...
    if (mathExpression.equals("*") || mathExpression.equals("/")) {
      ...
      if (leftSelector.getSelectorType() == IQuerySelector.SELECTOR_TYPE.ARITHMETIC
          && !((QueryArithmeticSelector) leftSelector).isEncapsulated()) {
        ...
        if (leftSideMathExpression.equals("-") || leftSideMathExpression.equals("+")) {
          ...
          // (leftOfLeftSelector + rightOfLeftSelector) * rightSelector -> leftOfLeftSelector +
          // (rightOfLeftSelector * rightSelector)
          ...
          && !((QueryArithmeticSelector) rightSelector).isEncapsulated()) {
        ...
        if (rightSideMathExpression.equals("-") || rightSideMathExpression.equals("+")) {
          ...
          // we will do a nice switch
          // leftSelector * (leftOfRightSelector + rightOfRightSelector) -> (leftSelector *
          // leftOfRightSelector) + rightOfRightSelector
