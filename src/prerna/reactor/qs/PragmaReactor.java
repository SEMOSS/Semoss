...
public class PragmaReactor extends AbstractQueryStructReactor {
  ...
  protected AbstractQueryStruct createQueryStruct() {
    ...
    if (mapOptions != null && mapOptions.size() > 0) {
      ...
      if (thisMap.containsKey("type")) type = (String) thisMap.get("type");

      if (type.equalsIgnoreCase("insight")
          && this.qs.getSelectors().size()
              == 0) // also it is replacing it as opposed to adding it // need a way to identify
      // when it was not set up that way. May be I just do the execute here instead of
      // abstract querystruct
      {
        if (thisMap.size() > 0 && this.insight.getPragmap() != null) // append it
        this.insight.getPragmap().putAll(thisMap);
        else if (this.insight.getPragmap() == null) this.insight.setPragmap(thisMap);
        else // the user is trying to clear it
        this.insight.clearPragmap();
      } else // this can only be used enroute in a query - we can change it so that you can change
        // it after the fact. I just dont feel that comfortable with it
