...
public class PragmaReactor extends AbstractQueryStructReactor {
	...
	protected AbstractQueryStruct createQueryStruct() {
		...
		if (mapOptions != null && mapOptions.size() > 0) {
			...
			if (type.equalsIgnoreCase("insight") && this.qs.getSelectors().size() == 0) // also it is replacing it as
																						// opposed to adding it // need
																						// a way to identify
			...
			} else // this can only be used enroute in a query - we can change it so that you can
					// change
