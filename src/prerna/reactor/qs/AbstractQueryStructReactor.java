 ...
 *
 * <p>
 * The design of this class is as such: This class holds a query struct,
 * responsible for grabbing input and passing output any subclass is only
 * responsible for building the query struct through method createQueryStruct()
 * Ex: SelectReactor overrides createQueryStruct() build on top of the existing
 * query struct with only Selectors JoinReactor overrides createQueryStruct()
 * builds on top of the existing query struct with only joins
 *
 * <p>
 * QueryStructReactor is responsible for grabbing any input querystructs and
 * passing the output
 */
public abstract class AbstractQueryStructReactor extends AbstractReactor {
	...
	protected static void setAlias(List<IQuerySelector> selectors, String[] selectorAlias, int startingPoint) {
		/*
		 * Since multiple select reactors can each have an alias we need to go back and
		 * add alias to the last selectors that were added based on the index of the
		 * alias we are adding
		 */
