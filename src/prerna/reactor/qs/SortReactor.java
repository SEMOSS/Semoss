		...
		for (int selectIndex = 0; selectIndex < colSize; selectIndex++) {
			...
			if (newSelector.contains("__")) {
				...
				if (sortDirSize > selectIndex) {
					((SelectQueryStruct) this.qs).addOrderBy(selectorSplit[0], selectorSplit[1],
							sortDirs.get(selectIndex));
				} else {
					((SelectQueryStruct) this.qs).addOrderBy(selectorSplit[0], selectorSplit[1], "ASC");
				}
			} else {
				if (sortDirSize > selectIndex) {
					((SelectQueryStruct) this.qs).addOrderBy(newSelector, SelectQueryStruct.PRIM_KEY_PLACEHOLDER,
							sortDirs.get(selectIndex));
				} else {
					((SelectQueryStruct) this.qs).addOrderBy(newSelector, SelectQueryStruct.PRIM_KEY_PLACEHOLDER,
							"ASC");
