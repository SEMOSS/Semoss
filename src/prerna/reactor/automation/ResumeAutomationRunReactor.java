...
/**
 * Resumes a failed or interrupted automation run from the first failed node.
 *
 * <p>Pixel: {@code ResumeAutomationRun(project=["appId"], runId=["failed-run-id"])}
 *
 * <p>Validates the target run exists and is in FAILED or INTERRUPTED status,
 * then delegates to {@link TriggerAutomationReactor} with the {@code resumeRunId}
...
public class ResumeAutomationRunReactor extends AbstractReactor {
	...
	private static final Logger classLogger = LogManager.getLogger(ResumeAutomationRunReactor.class);
	...
	public NounMetadata execute() {
		...
		if (!AutomationConstants.STATUS_FAILED.equals(status)
				&& !AutomationConstants.STATUS_INTERRUPTED.equals(status)) {
			throw new IllegalArgumentException(
					"Can only resume FAILED or INTERRUPTED runs. Current status: " + status);
		}

		// Verify the run belongs to this project
		String runProjectId = (String) runDetail.get(AutomationConstants.PROJECT_ID);
		if (!projectId.equals(runProjectId)) {
			throw new IllegalArgumentException("Run " + runId + " does not belong to project " + projectId);
		}

		classLogger.info("Resuming automation run {} for project {}", runId, projectId);

		// Both values come from validated/DB sources (projectId from testUserProjectIdForAlias,
		// runId from AUTOMATION_RUNS), so injection is not expected - guard defensively.
		...
		String pixel = "TriggerAutomation(project=[\"" + projectId + "\"], "
