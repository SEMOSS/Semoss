 ...
 * equivalent ({@code buildPixelPreview()} in {@code automation-utils.ts}) encodes. Any config field
...
final class EngineNodeSupport {
	...
	static String resolveEncoded(String rawTemplate, Map<String, String> scope, Map<String, String> configMap) {
		return encoded(AutomationExecutionUtils.resolve(rawTemplate, scope, configMap));
	}
	...
	static String resolveAndValidateJsonLiteral(String rawTemplate, Map<String, String> scope,
		...
		String resolved = AutomationExecutionUtils.resolve(rawTemplate, scope, configMap);
		try {
	 ...
	 * missing or blank - backend-side re-validation, since an automation.json can be edited
	 * directly (API call bypassing the FE, or a future FE bug) and must never be trusted implicitly.
	 */
