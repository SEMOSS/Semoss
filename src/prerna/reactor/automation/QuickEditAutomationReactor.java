 ...
 * this reactor chains {@code BuildAutomation} (edit mode) and {@code SaveAutomation} silently
...
public class QuickEditAutomationReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		String currentDocJson = AutomationExecutionUtils.loadAutomationDocOrEmpty(projectId);

		// BuildAutomation decodes base64 to prevent Pixel injection  - same pattern used by the FE
		...
		String generatePixel = String.format(
				"BuildAutomation(project=[\"%s\"], description=[\"%s\"], currentDoc=[\"%s\"]);",
				projectId, encodedDesc, encodedDoc);

		classLogger.info("QuickEditAutomationReactor: calling BuildAutomation for project {}", projectId);
		Object raw;
		try {
			raw = PixelExecutionUtils.runAndCollect(this.insight, generatePixel);
		} catch (PixelExecutionUtils.AutomationPixelException e) {
			classLogger.error("BuildAutomation pixel error for project {}", projectId, e);
			throw new IllegalArgumentException("AI generation failed: " + e.getMessage());
		}

		if (raw == null) {
			throw new IllegalArgumentException("BuildAutomation returned no result for project: " + projectId);
		}
		String generatedJson = raw instanceof String ? (String) raw : raw.toString();
		if (generatedJson.isBlank()) {
			throw new IllegalArgumentException("BuildAutomation returned an empty result for project: " + projectId);
		}
	...
	private Map<String, Object> parseAndValidate(String json, String projectId) {
		...
		try {
			...
			classLogger.error("BuildAutomation returned invalid JSON for project {}", projectId, e);
		...
		if (graph == null) {
			classLogger.error("BuildAutomation result missing 'graph' field for project {}", projectId);
		...
		if (nodes == null || nodes.isEmpty()) {
			classLogger.error("BuildAutomation result has no nodes for project {}", projectId);
		...
		if (!hasTrigger) {
			classLogger.error("BuildAutomation result has no trigger node for project {}", projectId);
	...
	public String getReactorDescription() {
		return "Modifies an automation using AI generation without opening any UI. "
				+ "Chains BuildAutomation (edit mode) and SaveAutomation silently. "
