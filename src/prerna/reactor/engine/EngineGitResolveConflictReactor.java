...
/**
 * Resolves a merge conflict for a single file in an engine's git repository.
 * Stages the resolved content but never commits.
 */
public class EngineGitResolveConflictReactor extends AbstractReactor {

	private static final Logger classLogger = LogManager.getLogger(EngineGitResolveConflictReactor.class);
	private static final String RESOLUTION_KEY = "resolution";

	public EngineGitResolveConflictReactor() {
	...
	public NounMetadata execute() {
		...
		try (Git thisGit = Git.open(new File(versionFolder))) {
			...
			resultMap.put("status", EngineGitReactorUtils.buildStatusMap(engineId,
					ProjectGitStatusUtils.computeStatus(thisGit.getRepository())));
		} catch (IllegalArgumentException e) {
			throw new SemossPixelException(e.getMessage());
		} catch (Exception e) {
			classLogger.error("Error occurred resolving conflict for engine {}", engineId, e);
	...
	public String getReactorDescription() {
		return "This reactor resolves a merge conflict for a file in an engine's git repository";
	}
