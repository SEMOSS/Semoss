...
public class RequestEngineReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (user == null) {
      NounMetadata noun =
          new NounMetadata(
              "User must be signed into an account in order to request an engine",
    ...
    Integer currentPendingUserPermission =
        SecurityEngineUtils.getUserAccessRequestEnginePermission(userId, engineId);
    ...
    if (canRequest) {
      ...
      SecurityEngineUtils.setUserAccessRequest(
          userId, userType, engineId, requestComment, requestPermission, user);
      sendEmail(user, engineId, permission, requestComment);
      return NounMetadata.getSuccessNounMessage(
          "Successfully requested access to engine '" + engineId + "'");
    }
    ...
    if (template != null && !template.isEmpty()) {
      ...
      if (databaseOwners != null && !databaseOwners.isEmpty()) {
        ...
        EmailUtility.sendEmail(
            emailSession,
            databaseOwners.toArray(new String[0]),
            null,
            null,
            SocialPropertiesUtil.getInstance().getSmtpSender(),
            "SEMOSS - Database Access Request",
            message,
            true,
            null);
      }
