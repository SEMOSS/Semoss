...
public class SchedulerFactorySingleton {
	...
	private void init() {
		...
		if (schedulerDb instanceof H2EmbeddedServerEngine) {
			quartzProperties = setUpQuartzProperties(((H2EmbeddedServerEngine) schedulerDb).getServerUrl(), username,
					password, rdbmsType);
		} else { // instanceof RDBMSNativeEngine
			quartzProperties = setUpQuartzProperties(schedulerDb.getConnectionUrl(), username, password, rdbmsType);
		}
	...
	public static Properties setUpQuartzProperties(String serverUrl, String username, String password,
