...
public class ResumeJobTriggerReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		if (jobTags != null && !jobTags.isEmpty()) {
			...
			// Covers scenario where job id is not passed in but job tags is and it finds
			// multiple jobs to resume
			// Also covers scenario where job id is sent in & job tags is also sent in, but
			// no jobs are returned
			// that fall under the job tag, therefore only the job id passed in was resumed,
			// and we do not want
