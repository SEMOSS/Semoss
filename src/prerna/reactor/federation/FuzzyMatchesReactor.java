...
public class FuzzyMatchesReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    if (frameCol != null) {
      ...
      if (frame instanceof RDataTable) {
        ...
        String loadFileRScript =
            RSyntaxHelper.getFReadSyntax(rCol2, newFile.getAbsolutePath(), "\t");
        script.append(loadFileRScript);
        cleanUpFiles.add(newFile);
      }
    } else {
    ...
    script.append(
        "source(\""
            + baseFolder.replace("\\", "/")
            + "/R/Recommendations/advanced_federation_blend.r\");");
    // create the matches frame using the best_match method
    if (optimized) {
      script.append(
          matchesFrame + " <- best_match(" + rCol1 + "[[names(" + rCol1 + ")[1]]]," + rCol2 + ");");
    } else {
      script.append(
          matchesFrame
              + " <- best_match("
    ...
    script.append(
        matchesFrame + "<-" + matchesFrame + "[order(unique(" + matchesFrame + ")$distance),];");
    ...
    String exactMatchCount =
        this.rJavaTranslator.getString(
            "as.character(nrow(" + matchesFrame + "[" + matchesFrame + "$distance == 0,]))");
