...
public class FuzzyMergeReactor extends AbstractRFrameReactor {
  ...
  public NounMetadata execute() {
    ...
    // using the combined lookup column, we will be able to just rbind those results with the
    // current linkframe
    if (requireMatches) {
      script.append(
          linkFrame
              ...
              + matchesFrame
              + "$combined %in% "
              + RSyntaxHelper.createStringRColVec(allMatches)
              + ",]);");
    }
    ...
    if (!(frame instanceof RDataTable)) {
      ...
      InsightUtility.replaceNounValue(
          this.insight.getVarStore(), frame, new NounMetadata(newFrame, PixelDataType.FRAME));
      // reasign reference
      frame = newFrame;
    }
    ...
    // append the merge
    script.append(
        RSyntaxHelper.getMergeSyntax(rFrameVar, rFrameVar, linkFrame, "inner.join", joinsList)
            + ";");
    ...
    // at the end, i will run a routine to actually set this data type as expected instead of being
    // a string
    OwlTemporalEngineMeta metaData = frame.getMetaData();
    SemossDataType frameColDataType =
        metaData.getHeaderTypeAsEnum(metaData.getUniqueNameFromAlias(frameCol));
    ...
    metaData.setQueryStructNameToProperty(
        rFrameVar + "__" + fuzzyColName, "FuzzyMatching", "FuzzyMatching");
    ...
    this.rJavaTranslator.runR(
        RSyntaxHelper.alterColumnType(rFrameVar, fuzzyColName, frameColDataType));
    return noun;
  }
