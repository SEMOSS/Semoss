...
public class FuzzyMergeReactor extends AbstractRFrameReactor {
	...
	public FuzzyMergeReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.QUERY_STRUCT.getKey(), ReactorKeysEnum.JOINS.getKey(),
				ReactorKeysEnum.FRAME.getKey(), FED_FRAME, MATCHES, NONMATCHES, PROP_MAX};
	}
	...
	public NounMetadata execute() {
		...
		/*
		 * The logic for this is to determine the fuzzy matches to maintain within the
		 * frame and then use those new values to join additional columns
		 *
		 * First, we will take the fedFrame which contains all the matches. We will take
		 * a subset of this that is above the propagation value. Then, we will add the
		 * user defined matches and remove the user defined nonMatches.
		 *
		 * Second, drop the unnecessary columns. And then join fuzzy column names to the
		 * existing column names We now have the headers we need
		 *
		 */
		...
		// using the combined lookup column, we will be able to just rbind those results
		// with the
		// current linkframe
		if (requireMatches) {
			script.append(linkFrame + "<- rbind(" + linkFrame + ", " + matchesFrame + "[" + matchesFrame
					+ "$combined %in% " + RSyntaxHelper.createStringRColVec(allMatches) + ",]);");
		}
		// grab the lists to remove
		// we will use this list to just remove from the existing linkFrame
		if (requireNonmatches) {
			script.append(linkFrame + "<- " + linkFrame + "[!(" + linkFrame + "$combined %in% "
					+ RSyntaxHelper.createStringRColVec(nonMatches) + "),];");
		}
		...
		if (!(frame instanceof RDataTable)) {
			...
			InsightUtility.replaceNounValue(this.insight.getVarStore(), frame,
					new NounMetadata(newFrame, PixelDataType.FRAME));
			// reasign reference
			frame = newFrame;
		}
		...
		// at the end, i will run a routine to actually set this data type as expected
		// instead of being
		// a string
		...
		for (Join j : joins) {
			...
			if (lCol.contains("__")) {
				j.setLColumn(lCol.split("__")[1]);
			} ;
			String rCol = j.getRColumn();
			if (rCol.contains("__")) {
				j.setRColumn(rCol.split("__")[1]);
			} ;
		}
