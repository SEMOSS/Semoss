...
public class CollectReactor extends TaskBuilderReactor {
	...
	public NounMetadata execute() {
		...
		if (this.task instanceof BasicIteratorTask) {
			try {
				...
				if (this.task.getTaskOptions() != null && !this.task.getTaskOptions().isEmpty()) {
					UserTrackerFactory.getInstance().trackVizWidget(this.insight, this.task.getTaskOptions(),
							((BasicIteratorTask) task).getQueryStruct());
				} else {
					UserTrackerFactory.getInstance().trackQueryData(this.insight,
							((BasicIteratorTask) task).getQueryStruct());
				}
			} catch (Exception e) {
				classLogger.error(Constants.STACKTRACE, e);
		...
		// TaskOptions newTOptions = AutoTaskOptionsHelper.getAutoOptions(qs, panelId,
		// layout);
