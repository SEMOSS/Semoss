...
public class GraphFormatter extends AbstractFormatter {
	...
	/*
	 * These are the options that the FE can define for graph output 1)
	 * connectionsMap Example : {upstreamNode1 -> [downstreamNode1 ,
	 * downstreamnode2, .. etc] This defines the up node to its list of downnodes
	 * The same node can be an up node in some situations and down nodes in orders
	 *
	 * 2) nodePropertiesMap Example : {node -> [prop1, prop2, ... propN] This
	 * defines the properties to add into the prop hash for a given node
	 *
	 * 3) edgePropertiesMap Example: {upNode1.downNode1 -> [prop1, prop2, ... propN]
	 * This defines the properties to put inbetween a given relationship that must
	 * already be defined in teh connectionsMap
	 *
	 */
						...
						for (String edgeLabel : edgePropertiesMap.keySet()) {
							...
							if (edgeLabel.contains(".")) {
								...
								if (split.length > 0) {
									...
									int startNodeIndex = ArrayUtilityMethods.arrayContainsValueAtIndex(headers,
											startNode);
									...
									if (validEdgeLabel(startNodeIndex, endNodeIndex)) {
										...
										for (String property : properties) {
											int propertyIndex = ArrayUtilityMethods.arrayContainsValueAtIndex(headers,
													property);
											propHash.put(property, values[propertyIndex]);
										}
									}
	...
	/** Gets graph metadata "col" : unique instance count */
