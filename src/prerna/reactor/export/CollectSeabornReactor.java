...
public class CollectSeabornReactor extends TaskBuilderReactor {
  ...
  public NounMetadata execute() {
    ...
    if (keyValue.containsKey(keysToGet[1])) format = keyValue.get(keysToGet[1]);
    String assigner = "";
    ...
    if (!type.equalsIgnoreCase("py")) // && !insight.getVarStore().containsKey("PY_SYNCHRONIZED"))
    {
      ...
      insight
          .getVarStore()
          .put(Insight.CUR_FRAME_KEY, new NounMetadata(thisFrame, PixelDataType.FRAME));
    ...
    try {
      prerna.engine.api.IRawSelectWrapper taskItearator =
          (((BasicIteratorTask) (task)).getIterator());
      String[] headers = taskItearator.getHeaders();
      for (int headerIndex = 0; headerIndex < headers.length; headerIndex++) {
        if (headerIndex != 0) columns.append(",");
    ...
    // ].iloc[0:][['MovieBudget', 'Genre', 'Nominated', 'RevenueDomestic']].to_dict('split'),
    // orient='index')
