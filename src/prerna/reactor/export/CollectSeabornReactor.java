...
public class CollectSeabornReactor extends TaskBuilderReactor {
	...
	public NounMetadata execute() {
		...
		if (!type.equalsIgnoreCase("py")) // && !insight.getVarStore().containsKey("PY_SYNCHRONIZED"))
		{
			...
			/*
			 * ITableDataFrame newFrame = null; try { newFrame =
			 * FrameFactory.getFrame(this.insight, "R", frame.get); } catch (Exception e) {
			 * throw new
			 * IllegalArgumentException("Error occurred trying to create frame of type " +
			 * frameType, e); } // insert the data for the new frame IImporter importer =
			 * ImportFactory.getImporter(newFrame, qs, it); try { importer.insertData(); }
			 * catch (Exception e) { classLogger.error(Constants.STACKTRACE, e); throw new
			 * SemossPixelException(e.getMessage()); }
			 */
	...
	/*
	 * task.getMetaMap(); SemossDataType[] sTypes = null; String[] headers = null;
	 * try { IRawSelectWrapper taskItearator =
	 * (((BasicIteratorTask)(task)).getIterator()); sTypes =
	 * taskItearator.getTypes(); headers = taskItearator.getHeaders(); } catch
	 * (Exception e) { classLogger.error(Constants.STACKTRACE, e); throw new
	 * SemossPixelException(e.getMessage()); } Map<String, SemossDataType> typeMap =
	 * new HashMap<String, SemossDataType>(); for(int i = 0; i < headers.length;
	 * i++) { typeMap.put(headers[i],sTypes[i]); } // I need to see how to get this
	 * to temp if(headers != null) // if they are using this natively, go with it {
	 * String dir = insight.getUserFolder() + "/Temp"; dir =
	 * dir.replaceAll("\\\\", "/"); File tempDir = new File(dir);
	 * if(!tempDir.exists()) tempDir.mkdir(); String outputFile = dir + "/" +
	 * fileName + ".csv"; Utility.writeResultToFile(outputFile, this.task, ","); //
	 * need something here to adjust the types // need to move this to utilities //
	 * will move it once we have figured it out // at some point the encoding etc.
	 * needs to be fixed loadDT = fileName + " = pd.read_csv(\"" + outputFile +
	 * "\");"; // adjust the types adjustTypes = Utility.adjustTypePy(fileName,
	 * headers, typeMap); // run the job //pyt.runEmptyPy(loadDT, adjustTypes); //
	 * now comes the building part // I need to ask kunal if he mauls the path so I
	 * cannot load seaborn anymore assignPlotter = "plotterframe = " + fileName; }
	 * else { // give it the full frame name ITableDataFrame frame =
	 * (ITableDataFrame)insight.getVarStore().get("$CUR_FRAME_KEY").getValue();
	 * headers = frame.getColumnHeaders(); assignPlotter = "plotterframe = " +
	 * frame.getName(); }
	 */
