...
public class ToLoaderSheetReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		String databaseId = MasterDatabaseUtility.testDatabaseIdIfAlias(engineId);
		IDatabaseEngine engine = Utility.getDatabase(databaseId);
		if (engine == null) {
			throw new IllegalArgumentException("Cannot find the specified engine");
		}

		Date date = new Date();
		String modifiedDate = new SimpleDateFormat("yyyy_MM_dd_HH_mm_ss").format(date);
		String fileLoc = this.insight.getInsightFolder() + DIR_SEPARATOR
				+ SmssUtilities.getUniqueName(engine.getEngineName(), engine.getEngineId()) + "_" + modifiedDate
				+ "_Loader_Sheet_Export.xlsx";
