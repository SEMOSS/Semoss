...
public class ToDatabaseReactor extends TaskBuilderReactor {
  ...
  public NounMetadata execute() {
    try {
      ...
      this.engineId =
          SecurityQueryUtils.testUserEngineIdForAlias(this.insight.getUser(), this.engineId);
      ...
      this.override =
          Boolean.parseBoolean(this.keyValue.get(ReactorKeysEnum.OVERRIDE.getKey()) + "");
      // checks if targetTable doesn't exist in the engine
      this.newTable =
          !MasterDatabaseUtility.getConceptsWithinDatabaseRDBMS(this.engineId)
  ...
  protected void buildTask() {
    ...
    if (!(targetDatabase instanceof IRDBMSEngine)) {
      throw new SemossPixelException(
          new NounMetadata(
              "Can only persist data to a relational database at the moment",
              PixelDataType.CONST_STRING,
    ...
    if (this.override) {
      ...
      try {
        ...
        throw new SemossPixelException(
            new NounMetadata(
                "Error occurred trying to delete the existing table with message = "
                    + e.getMessage(),
                PixelDataType.CONST_STRING,
                PixelOperationType.ERROR));
      }
      ...
      if (this.override || this.newTable) {
        ...
        try (WriteOWLEngine owlEngine = targetEngine.getOWLEngineFactory().getWriteOWL()) {
          ...
          if (this.override) {
            ...
            logger.info(
                "Finished removing concept from the OWL. Total time = "
                    + ((end2 - start2) / 1000)
                    + " seconds");
          }
        ...
        logger.info(
            "Finished adding concept to the OWL. Total time = "
                + ((end - start) / 1000)
                + " seconds");
      }
    }
