...
public class ToDatabaseReactor extends TaskBuilderReactor {
	...
	public ToDatabaseReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.TASK.getKey(), TARGET_DATABASE, TARGET_TABLE,
				ReactorKeysEnum.OVERRIDE.getKey(), INSERT_ID_KEY, IGNORE_OWL_KEY, BATCH_SIZE_KEY};
	}
	...
	public NounMetadata execute() {
		try {
			...
			if (!SecurityEngineUtils.userCanEditEngine(this.insight.getUser(), this.engineId)) {
				throw new IllegalArgumentException("Database " + this.engineId
			...
			this.newTable = !MasterDatabaseUtility.getConceptsWithinDatabaseRDBMS(this.engineId)
					.contains(this.targetTable);
			...
			if (this.newTable && this.override) {
				...
				throw new SemossPixelException(new NounMetadata("Table " + this.targetTable
						+ " cannot be found to override. Please enter an existing table name or turn override to false",
		...
		return new NounMetadata(true, PixelDataType.BOOLEAN, PixelOperationType.FORCE_SAVE_DATA_EXPORT);
	}
	...
	protected void buildTask() {
		...
		if (!(targetDatabase instanceof IRDBMSEngine)) {
			throw new SemossPixelException(
					new NounMetadata("Can only persist data to a relational database at the moment",
		...
		if (this.override) {
			...
			try {
				...
				throw new SemossPixelException(new NounMetadata(
						"Error occurred trying to delete the existing table with message = " + e.getMessage(),
						PixelDataType.CONST_STRING, PixelOperationType.ERROR));
			}
			...
			try {
				...
				classLogger.error(Constants.STACKTRACE, e);
				throw new SemossPixelException(new NounMetadata(
						"Error occurred trying to create the new table with message = " + e.getMessage(),
						PixelDataType.CONST_STRING, PixelOperationType.ERROR));
			}
			...
			if (cols.size() != size) {
				throw new SemossPixelException(new NounMetadata(
						"Header size does not match for existing table. Please create a new table or have override=true",
						PixelDataType.CONST_STRING, PixelOperationType.ERROR));
			}
		}
		...
		try {
			...
			throw new IllegalArgumentException(
					"An error occurred creating the prepared statement with message = " + e.getMessage());
		}
		try {
			...
			classLogger.error(Constants.STACKTRACE, e);
			throw new IllegalArgumentException(
					"An error occurred persisting data into the database with message = " + e.getMessage());
		} finally {
			if (ps != null) {
				try {
					ps.close();
