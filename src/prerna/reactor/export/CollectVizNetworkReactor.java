...
public class CollectVizNetworkReactor extends TaskBuilderReactor {
  ...
  public NounMetadata execute() {
    ...
    builder.append(
        "library('igraph');library('visNetwork');library('htmlwidgets');library('withr');");
    ...
    builder
        ...
        .append(filePath)
        .append("\", saveWidget(")
        .append(randomGraphVar)
        .append(", file=\"")
        .append(randomHtmlFile)
        .append("\"));");
    ...
    if (!filePathF.exists() || !filePathF.isFile()) {
      throw new IllegalArgumentException(
          "Error occurred generating the network visualization. "
    ...
    try {
      outputMap.put(
          "values", new String[] {FileUtils.readFileToString(filePathF, StandardCharsets.UTF_8)});
    } catch (IOException e) {
      // classLogger.error(Constants.STACKTRACE, e);
      throw new IllegalArgumentException(
          "Error occurred reading the html file with full message: " + e.getMessage());
    }
