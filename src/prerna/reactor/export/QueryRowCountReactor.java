...
public class QueryRowCountReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    } else if (qsType == QUERY_STRUCT_TYPE.FRAME || qsType == QUERY_STRUCT_TYPE.RAW_FRAME_QUERY) {
      return rowsForFrame(qs, logger);
    }

    throw new IllegalArgumentException(
        "Can not determine row count for Query Struct of type " + qsType);
  }
  ...
  private NounMetadata rowsForFrame(SelectQueryStruct qs, Logger logger) {
    ...
    if (frame == null) {
      throw new IllegalArgumentException(
          "Query Struct is of type " + qs.getQsType() + " but no frame is set");
    }
  ...
  private NounMetadata rowsForEngine(SelectQueryStruct qs, Logger logger) {
    ...
    if (engine == null) {
      throw new IllegalArgumentException(
          "Query Struct is of type " + qs.getQsType() + " but no engine is set");
    }
    IRawSelectWrapper iterator = null;
    try {
