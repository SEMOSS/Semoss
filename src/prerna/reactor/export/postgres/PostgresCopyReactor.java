...
public class PostgresCopyReactor extends AbstractReactor {

	private static final Logger classLogger = LogManager.getLogger(PostgresCopyReactor.class);

	private static final String FORMAT = "format";
	private static final String NULL = "nullValue";

	public PostgresCopyReactor() {
		this.keysToGet = new String[]{ReactorKeysEnum.DATABASE.getKey(), ReactorKeysEnum.TABLE.getKey(), FORMAT, NULL,
				ReactorKeysEnum.DELIMITER.getKey(), ReactorKeysEnum.FILE_PATH.getKey(), ReactorKeysEnum.SPACE.getKey()};
	}
	...
	public NounMetadata execute() {
		...
		if (!SecurityEngineUtils.userCanEditEngine(user, engineId)) {
			throw new IllegalArgumentException(
					"Database " + engineId + " does not exist or user does not have edit access to the database");
		}
		...
		try {
			...
			classLogger.info(User.getSingleLogginName(user) + " is running copy command on " + engineId + " with sql: "
					+ copySyntax + " on file " + filePath);
			rowsInserted = new CopyManager((BaseConnection) postgresConn).copyIn(copySyntax,
					new BufferedReader(new FileReader(filePath)));
			classLogger.info(User.getSingleLogginName(user) + " is ran copy command on " + engineId + " and added "
					+ rowsInserted + " rows");
			...
			throw new IllegalArgumentException(
					"Error executing the COPY command. Detailed message = " + e.getMessage());
		} finally {
			if (database.isConnectionPooling()) {
