...
public class PostgresCopyReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    String filePath =
        Utility.normalizePath(UploadInputUtility.getExtendedFilePath(this.store, this.insight));
    ...
    try {
      ...
      classLogger.info(
          User.getSingleLogginName(user)
              ...
              + filePath);
      rowsInserted =
          new CopyManager((BaseConnection) postgresConn)
              .copyIn(copySyntax, new BufferedReader(new FileReader(filePath)));
      classLogger.info(
          User.getSingleLogginName(user)
              + " is ran copy command on "
              + engineId
              + " and added "
              + rowsInserted
              + " rows");
