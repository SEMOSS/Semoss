...
public class CollectGGPlotReactor extends TaskBuilderReactor {
  ...
  public CollectGGPlotReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.GGPLOT.getKey(), ReactorKeysEnum.FORMAT.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    for (int tokenIndex = 0; tokenIndex < comTokens.length; tokenIndex++) {
      ...
      if (tokenIndex + 1 < comTokens.length) nextCommand = comTokens[tokenIndex + 1];
      if (!command.contains("animate")) {
        newCommand.append(command);
        if (nextCommand != null && !nextCommand.contains("animate")) newCommand.append(" + ");
      } else {
        animate = true;
      }
    }
