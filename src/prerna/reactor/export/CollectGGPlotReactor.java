...
public class CollectGGPlotReactor extends TaskBuilderReactor {
	...
	public NounMetadata execute() {
		...
		if (!type.equalsIgnoreCase("R") && !insight.getVarStore().containsKey("R_SYNCHRONIZED")) {
			...
			// insight.getVarStore().put("R_SYNCHRONIZED", new NounMetadata(true,
			// PixelDataType.BOOLEAN));
			// // need replace to the frame back
			// insight.getVarStore().put(Insight.CUR_FRAME_KEY, new NounMetadata(thisFrame,
			// PixelDataType.FRAME));
		}
	 ...
	 * i++) { typeMap.put(headers[i],sTypes[i]); }
	 * 
	 * // I need to see how to get this to temp String fileName =
	 * Utility.getRandomString(6); String dir = insight.getUserFolder() + "/Temp";
	 * dir = dir.replaceAll("\\\\", "/"); File tempDir = new File(dir);
	 * if(!tempDir.exists()) tempDir.mkdir(); String outputFile = dir + "/" +
	 * fileName + ".csv"; Utility.writeResultToFile(outputFile, this.task, typeMap,
	 * ",");
	 * 
	 * 
	 * 
	 * // need something here to adjust the types // need to move this to utilities
	 * // will move it once we have figured it out String loadDT = fileName +
	 * " <- fread(\"" + outputFile + "\");"; // adjust the types String adjustTypes
	 * = Utility.adjustTypeR(fileName, headers, typeMap); // run the job
