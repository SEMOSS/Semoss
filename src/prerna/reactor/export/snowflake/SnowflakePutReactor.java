...
public class SnowflakePutReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    Boolean compress =
        Boolean.parseBoolean(this.keyValue.getOrDefault(this.keysToGet[6], "true") + "");
    String sourceCompression = this.keyValue.get(this.keysToGet[7]);
    String parallelStr = this.keyValue.get(this.keysToGet[8]);
    Boolean overwrite =
        Boolean.parseBoolean(this.keyValue.getOrDefault(this.keysToGet[9], "false") + "");
    ...
    String sql =
        "PUT 'file://"
            ...
            + stageDestination
            + " AUTO_COMPRESS = "
            + compress.toString().toUpperCase()
            + " OVERWRITE = "
            + overwrite.toString().toUpperCase();
