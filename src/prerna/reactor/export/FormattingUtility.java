...
public class FormattingUtility {
	 ...
	 *            -panel format data value ex {round=0, delimiter=,, type=thousand}
	...
	public static Object formatDataValues(Object value, String dataType, String metamodelAdditionalDataType,
		...
		if (Objects.nonNull(dataType) && Objects.nonNull(panelFormatting)) {
			...
			if (semossDataType == SemossDataType.STRING && formatted instanceof String) {
				...
				Integer round = panelFormatting.containsKey(ROUND)
	...
	/**
	 * @param colorRuleMap
	 *            -contains the colorbyvalue rule object as key and valuesToColor as
	 *            value key- {comparator={display=is Equal
	 *            To,value===},color=#4FA4DE,colorOn=Director} valuesColumn=Genre},
	 *            value-[Adam_McKay, Adam_Shankman, Alexander_Payne,
	 *            Alfonso_Cuarón,etc]
	 ...
	 *            data frame
	 * @param colIdx-
	 *            index of the cell index with respect to columns
	...
	/**
	 * @param valuesToColorList
	 *            - Filtered Rule Result
	 * @param cell
	 *            - Actual Cell Value
	 ...
	 * value.replace("\\B(?=(\\d{3})+(?!\\d))/g", ","); System.out.println("text::"
	 * + text);
	 *
	 * }
	 */
