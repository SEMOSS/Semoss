...
public class FormattingUtility {
	 ...
	 * @param panelFormatting -panel format data value ex {round=0, delimiter=,,
	 *                        type=thousand}
	 * @return it will return the cell value after applying the format
	 */
	public static Object formatDataValues(Object value, String dataType, String metamodelAdditionalDataType,
		...
		if (metamodelAdditionalDataType != null && !metamodelAdditionalDataType.isEmpty()) {
			...
			try {
				...
				if (customDataFormat != null && !customDataFormat.isEmpty() && customDataFormat.containsKey(ROUND)
				...
				classLogger.error(
						"Unable to parse metamodel additional data type formatting configuration {} for data type {}.",
						metamodelAdditionalDataType, dataType, e);
			}
	...
	public static String getDate(Object value, String currFormat, String targetFormat) {
		...
		try {
			...
			// ignore
			classLogger.error("Unable to parse date value {} using input format {} and target format {}.", value,
					currFormat, targetFormat, e);
		}
	...
	/**
	 * @param colorRuleMap   -contains the colorbyvalue rule object as key and
	 *                       valuesToColor as value key- {comparator={display=is
	 *                       Equal To,value===},color=#4FA4DE,colorOn=Director}
	 *                       valuesColumn=Genre}, value-[Adam_McKay, Adam_Shankman,
	 *                       Alexander_Payne, Alfonso_Cuarón,etc]
	...
	public static Object getBackgroundColor(Map<ColorByValueRule, List<Object>> colorRuleMap, String[] headers,
		...
		for (Entry<ColorByValueRule, List<Object>> cbvRuleValues : colorRuleMap.entrySet()) {
			...
			boolean restrict = optionsMap.containsKey("restrict")
	...
	public static boolean passedSimpleFilterCheck(Object cellData, Object selectedValue, String comparator) {
		...
		else if (cellData instanceof String && comparator.equals("?like")) {
	...
	public static boolean isColoringValid(ColorByValueRule rule, Object[] rowData, String[] headers) {
		...
		for (IQueryFilter iQueryFilter : filterVec) {
			...
			if (filterType == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
				NounMetadata leftComp = ((SimpleQueryFilter) iQueryFilter).getLComparison(); // this is the column, I am
																								// not going to bother ?
				...
				for (IQueryFilter andQueryFilter : ((AndQueryFilter) iQueryFilter).getFilterList()) {
					NounMetadata leftComp = ((SimpleQueryFilter) andQueryFilter).getLComparison(); // this is the
																									// column,I am not
																									// going to bother ?
																									// //
