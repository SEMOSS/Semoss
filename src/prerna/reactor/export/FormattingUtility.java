...
public class FormattingUtility {
   ...
   * @param panelFormatting -panel format data value ex {round=0, delimiter=,, type=thousand}
   * @return it will return the cell value after applying the format
   */
  public static Object formatDataValues(
      Object value,
      String dataType,
      String metamodelAdditionalDataType,
    ...
    if (Objects.nonNull(dataType) && Objects.nonNull(panelFormatting)) {
      ...
      if (semossDataType == SemossDataType.STRING && formatted instanceof String) {
        ...
        Integer round =
            panelFormatting.containsKey(ROUND)
                ? Integer.parseInt(panelFormatting.get(ROUND))
                : null;
        String delimiter = panelFormatting.get(DELIMITER);
        Pattern betweenDigitsPattern = Pattern.compile(BETWEEN_DIGITS);

        if (null != formatType
            && !"".equalsIgnoreCase(formatType)
            && !formatType.equalsIgnoreCase("Default")) {

          if (formatType.equalsIgnoreCase(THOUSAND)
              || formatType.equalsIgnoreCase(MILLION)
              || formatType.equalsIgnoreCase(BILLION)
              || formatType.equalsIgnoreCase(TRILLION)) {
            ...
            if (!Double.isNaN(numericValue)) {
              ...
              if (round != null) {
                ...
                if (!Double.isNaN(numericValue)) {
                  formatted =
                      new BigDecimal(Math.round(shift * numericValue) / shift)
        ...
        if (delimiter != null && !delimiter.equalsIgnoreCase("Default")) {
          ...
          || semossDataType == SemossDataType.TIMESTAMP) {
    ...
    if (metamodelAdditionalDataType != null && !metamodelAdditionalDataType.isEmpty()) {
      ...
      try {
        ...
        if (customDataFormat != null
            && !customDataFormat.isEmpty()
            && customDataFormat.containsKey(ROUND)
            && !customDataFormat.get(ROUND).equals("")) {
      ...
      if (semossDataType == SemossDataType.STRING && formatted instanceof String) {
        ...
        if (date != null) {
          ...
          formatted =
              getDate(formatted.toString(), "yyyy-MM-dd hh:mm:ss", metamodelAdditionalDataType);
        }
        // the input is a date format itself
      }
  ...
  /**
   * @param type
   * @return <double>-exponential value this will return exponential value for given format type
   */
  ...
  /**
   * @param value - cell date value
   * @param currFormat -current date format
   * @param targetFormat- target date format
   * @return converts cell value from current date format to target date format
   */
   ...
   *     key- {comparator={display=is Equal To,value===},color=#4FA4DE,colorOn=Director}
   *     valuesColumn=Genre}, value-[Adam_McKay, Adam_Shankman, Alexander_Payne, Alfonso_Cuarón,etc]
  ...
  public static Object getBackgroundColor(
      Map<ColorByValueRule, List<Object>> colorRuleMap,
      String[] headers,
      Object[] rowData,
      int colIdx) {
    ...
    for (Entry<ColorByValueRule, List<Object>> cbvRuleValues : colorRuleMap.entrySet()) {
      ...
      boolean restrict =
          optionsMap.containsKey("restrict")
              ? Boolean.parseBoolean(optionsMap.get("restrict") + "")
              : false;
      // restrict check
      ...
      boolean isRowColor =
          highlightRow
              && valuesToColorList != null
              && valuesToColorList.size() > 0
              && isCellEligibleForColor(
                  valuesToColorList, rowData[Arrays.asList(headers).indexOf(colorOn)]);
   ...
   * @param selectedValue-Selected column value
   * @param comparator-comparator
   * @return true or false if criteria matches
   */
  public static boolean passedSimpleFilterCheck(
      Object cellData, Object selectedValue, String comparator) {
    boolean isValid = true;
    boolean selectedValueType = selectedValue instanceof Collection;
    List selectedListValue =
        selectedValueType ? (List) selectedValue : Arrays.asList(selectedValue);
    ...
    // Number type and Date type comparator check
      ...
      double formattedCellData =
          NumberUtils.isCreatable(selectedValue.toString())
              ? Double.parseDouble(cellData.toString())
              : ((SemossDate) cellData).getDate().getTime();
      double formattedSelectedValue =
          NumberUtils.isCreatable(selectedValue.toString())
              ? Double.parseDouble(selectedValue.toString())
              : new SemossDate(selectedValue.toString(), ((SemossDate) cellData).getPattern())
                  .getDate()
                  .getTime();
      if (comparator.equals(">")) {
  ...
  /**
   * @param rule ColorbyValue Rule Filter
    ...
    for (IQueryFilter iQueryFilter : filterVec) {
      ...
      if (filterType == IQueryFilter.QUERY_FILTER_TYPE.SIMPLE) {
        NounMetadata leftComp =
            ((SimpleQueryFilter) iQueryFilter)
                .getLComparison(); // this is the column, I am not going to bother ?
        ...
        for (IQueryFilter andQueryFilter : ((AndQueryFilter) iQueryFilter).getFilterList()) {
          NounMetadata leftComp =
              ((SimpleQueryFilter) andQueryFilter)
                  .getLComparison(); // this is the column,I am not going to bother ?
          //										//
  ...
  public static boolean isCellEligibleForColor(List<Object> valuesToColorList, Object cell) {
    return valuesToColorList.stream()
        .anyMatch(
            valuesColorObj ->
                ((cell instanceof SemossDate && valuesColorObj instanceof SemossDate)
                    ? (((SemossDate) valuesColorObj).getDate().getTime()
                        == ((SemossDate) cell).getDate().getTime())
   ...
   */
  public static Map<String, Object> mapFormatOpts(
      String metamodelAdditionalDataType, String dataType) {
    Map<String, Object> format = new HashMap<String, Object>();

    String formatString = metamodelAdditionalDataType,
        delimiter = "",
        prepend = "",
        append = "",
        type = "";
