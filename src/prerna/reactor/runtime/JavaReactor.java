...
public final class JavaReactor extends AbstractReactor implements ICodeExecution {
  ...
  public NounMetadata execute() {
    String retString =
        "Java console is now discontinued. Please use mvn directly for performing all java operations";
    retString =
        retString
  ...
  public NounMetadata execute_old() {
    ...
    try {
      ...
      CtClass cc =
          pool.makeClass(
              uniqueName); // the only reason I do this is if the user wants to do something
      // else
      ...
      jR.setLogger(logger);
      jR.setRJavaTranslator(
          this.insight.getRJavaTranslator(this.getLogger(jR.getClass().getName())));
      ...
      if (outputs == null || outputs.isEmpty()) {
        ...
        if (jR.System.out.output != null) {
          outputs.add(
              new NounMetadata(jR.System.out.output.toString(), PixelDataType.CONST_STRING));
        } else if (jR.System.err.output != null) {
          outputs.add(
              new NounMetadata(jR.System.err.output.toString(), PixelDataType.CONST_STRING));
        }
      }
    ...
    return new NounMetadata(
        "no output", PixelDataType.CONST_STRING, PixelOperationType.CODE_EXECUTION);
  }
