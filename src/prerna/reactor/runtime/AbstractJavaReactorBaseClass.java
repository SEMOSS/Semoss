...
public abstract class AbstractJavaReactorBaseClass extends AbstractReactor {
  ...
  protected void filterNode(String columnHeader, String[] instances) {
    ...
    this.nounMetaOutput.add(
        new NounMetadata(this.dataframe, PixelDataType.FRAME, PixelOperationType.FRAME_FILTER));
  }
  ...
  //	 */
  //	protected void runAnomaly(String timeColumn, String seriesColumn, String aggregateFunction,
  // double maxAnoms,
  ...
  //		AnomalyDetector anomalyDetector = new AnomalyDetector(dataframe, pkql, timeColumn,
  // seriesColumn,
  ...
  protected void removeNode(String type) {
    ...
    if (this.dataframe instanceof TinkerFrame) {
      ...
      this.nounMetaOutput.add(
          new NounMetadata(
              this.dataframe, PixelDataType.FRAME, PixelOperationType.FRAME_DATA_CHANGE));
    }
    //		java.lang.System.setSecurityManager(reactorManager);
  }
  ...
  protected void degree(String type, String data) {
    ...
    if (this.dataframe instanceof TinkerFrame) {
      ...
      // set the output so the FE knows what occurred
      this.nounMetaOutput.add(
          new NounMetadata(output, PixelDataType.CONST_STRING, PixelOperationType.OPERATION));
    }
    //		java.lang.System.setSecurityManager(reactorManager);
  }
  ...
  //		if(this.dataframe instanceof TinkerFrame)
  //		{
  //			TinkerFrame newDataFrame = DataFrameHelper.generateNewGraph((TinkerFrame) this.dataframe,
  // edgeHashStr, traversalHashStr);
  //			if(newDataFrame == null) {
  //				System.out.println("ERROR: Generated graph is empty! Please modify inputs to get a frame
  // which has data");
  //			} else {
  //				// set the output so the FE knows what occurred
  //				this.nounMetaOutput.add(new NounMetadata(newDataFrame, PixelDataType.FRAME,
  ...
  //		if(this.dataframe instanceof TinkerFrame)
  //		{
  //			TinkerFrame newDataFrame = DataFrameHelper.findSharedVertices((TinkerFrame) this.dataframe,
  // type, instances, numTraversals);
  //			// set the output so the FE knows what occurred
  //			this.nounMetaOutput.add(new NounMetadata(newDataFrame, PixelDataType.FRAME,
  // PixelOperationType.FRAME_DATA_CHANGE));
  //			System.out.println("Generated new graph data frame");
  ...
  //			System.out.println("Filtered to keep only vertices which are shared between defined
  // instances");
  //		}
  //		java.lang.System.setSecurityManager(reactorManager);
  //	}
  ...
  //	 * @param traversal
  //	 */
  //	protected void shiftToNodeProperty(String conceptName, String propertyName, Map<String,
  // Set<String>> traversal) {
  //		java.lang.System.setSecurityManager(curManager);
  //		if(this.dataframe instanceof TinkerFrame)
  //		{
  //			DataFrameHelper.shiftToNodeProperty((TinkerFrame) this.dataframe, conceptName, propertyName,
  // traversal);
  //			this.dataframe.updateDataId();
  ...
  //	 * @param traversal
  //	 */
  //	protected void shiftToEdgeProperty(String[] relationship, String propertyName, Map<String,
  // Set<String>> traversal) {
  //		java.lang.System.setSecurityManager(curManager);
  //		if(this.dataframe instanceof TinkerFrame)
  //		{
  //			DataFrameHelper.shiftToEdgeProperty((TinkerFrame) this.dataframe, relationship, propertyName,
  // traversal);
  //			this.dataframe.updateDataId();
  //			System.out.println("Modified graph data frame");
  //
  ...
  protected void runLoopIdentifer(int cycleSize) {
    ...
    if (this.dataframe instanceof TinkerFrame) {
      String loops =
          TinkerAlgorithmUtility.runLoopIdentifer((TinkerFrame) this.dataframe, cycleSize);
      System.out.println(loops);

      // set the output so the FE knows what occurred
      this.nounMetaOutput.add(
          new NounMetadata(loops, PixelDataType.CONST_STRING, PixelOperationType.OPERATION));
    }
    //		java.lang.System.setSecurityManager(reactorManager);
  }
  ...
  protected void runDisconnectedNodesIdentifier(String type, String instance) {
    //		java.lang.System.setSecurityManager(curManager);
    if (this.dataframe instanceof TinkerFrame) {
      String disconnectedNodes =
          TinkerAlgorithmUtility.runDisconnectedNodesIdentifier(
              (TinkerFrame) this.dataframe, type, instance);
      System.out.println(disconnectedNodes);

      // set the output so the FE knows what occurred
      this.nounMetaOutput.add(
          new NounMetadata(
              disconnectedNodes, PixelDataType.CONST_STRING, PixelOperationType.OPERATION));
    }
    //		java.lang.System.setSecurityManager(reactorManager);
  }
  ...
  protected String getBaseFolder() {
    ...
    if (baseFolder == null) baseFolder = "C:/users/pkapaleeswaran/workspacej3/SemossWeb";
