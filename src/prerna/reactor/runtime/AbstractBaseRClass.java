...
public abstract class AbstractBaseRClass extends AbstractJavaReactorBaseClass {
  ...
  protected void recreateMetadata(String frameName) {
    ...
    ImportUtility.parseTableColumnsAndTypesToFlatTable(
        newTable.getMetaData(), colNames, colTypes, frameName);
    this.nounMetaOutput.add(
        new NounMetadata(newTable, PixelDataType.FRAME, PixelOperationType.FRAME_DATA_CHANGE));
  ...
  private void synchronizeGridToR(String frameName) {
    ...
    this.rJavaTranslator.executeEmptyR(
        RSyntaxHelper.getFReadSyntax(frameName, outputLocation, "\\t"));
  ...
  private void recalculateTypes(
      String tableName,
      Map<String, SemossDataType> origDataTypes,
      String[] colSelectors,
      String[] types) {
  ...
  protected RDataTable synchronizeGridToRDataTable(
      String rVarName, boolean replaceDefaultInsightFrame) {
    ...
    for (String colName : types.keySet()) {
      ...
      if (smssType == SemossDataType.STRING || smssType == SemossDataType.DATE) {
        dataTypeConversion.append(
            RSyntaxHelper.alterColumnTypeToCharacter(frameName, colName) + ";");
      }
    }
  ...
  protected void synchronizeGridFromR(String frameName, boolean overrideExistingTable) {
    ...
    this.rJavaTranslator.executeEmptyR(
        "fwrite(" + frameName + ", file='" + tempFileLocation + "')");
  ...
  //				// implementation
  //				this.rJavaTranslator.executeEmptyR(graphName + " <- delete_vertices(" + graphName + ", V(" +
  // graphName + ")[vertex_attr(" + graphName
  ...
  public void key() {
    ...
    for (int vertIndex = 0; vertIndex < vertices.length; vertIndex++) {
      ...
      String typeOutput = "";
      if (dataHash.containsKey(typeData[0])) typeOutput = dataHash.get(typeData[0]);
