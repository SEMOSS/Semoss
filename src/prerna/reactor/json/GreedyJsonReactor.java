...
public class GreedyJsonReactor extends AbstractReactor {
	...
	private static final Logger classLogger = LogManager.getLogger(GreedyJsonReactor.class);
	...
	// I add a new hashtable with that entry and record it there as part of curnoun
	// call
	// which basically means, I also need some way to say set this value somewhere
	// in that hierarchy
	// question is what happens if I have 2 "different" values set up at different
	// levels
	// The only way to resolve this problem is introduce an intermediary reactor
	// which will set the specific value
	// which I think is not a bad idea

	// I also need a way to look up stuff i.e. get this to me in parent or child
	// type stuff
	// getFromParent("keyname", boolean levels) / getFromChild("keyname", boolean
	// levels)
	...
	// and the out will be where you either get the results back or you get the
	// errors back
	...
	// When there is an error, the error will have a key which says ERROR to be
	// tracked
	...
	// the <item which errored>__ERROR and value - this is the one to be displayed
	// if we so chose
	...
	// this is doing breadth first.. we could change it to depth first if we so
	// chose to
...
//			return hasError;
		...
		for (int nodeIndex = 0; nodeIndex < remainingHash.size(); nodeIndex++) {
			Hashtable childHash = remainingHash.get(0);
					...
					for (int childIndex = 0; childIndex < childs.size(); childIndex++) {
						...
						if (thisHash.containsKey(childs.get(childIndex))) {
							...
							if (thisHash.get(childs.get(childIndex)) instanceof Hashtable) {
								...
								classLogger.debug("Find out what this is is then ?!");
	...
	public StringBuffer getJsonOutput() {
		 ...
		 * retOutput.append("{");
		 * 
		 * Vector<Hashtable> nextLevelHash = new Vector<Hashtable>(); // I need to do a
		 * couple of things here // get the data hash // remove the childs, all // add
		 * the childs to the next vector // print the key and the value for(int
		 * nodeIndex = 0;nodeIndex < childVector.size();nodeIndex++) { Hashtable
		 * childHash = childVector.get(nodeIndex); childHash.remove("all");
		 * if(childHash.containsKey(CHILDS)) { Vector <String> childs =
		 * (Vector<String>)childHash.get(CHILDS); for(int childIndex = 0;childIndex <
		 * childs.size();childIndex++) {
		 * nextLevelHash.add((Hashtable)childHash.get(childs.get(childIndex)));
		 * childHash.remove(childs.get(childIndex)); } }
		 * 
		 * // now that I have the pure hashtable print it out Gson gson = new
		 * GsonBuilder() .disableHtmlEscaping()
		 * .excludeFieldsWithModifiers(Modifier.STATIC, Modifier.TRANSIENT)
		 * //.registerTypeAdapter(Double.class, new NumberAdaptor()) .create(); String
		 * propString = gson.toJson(childHash);
		 * 
		 * // send this for output again retOutput.append(propString); }
		 * 
		 * retOutput.append("}");
		 */
		return retOutput;
	...
	// searchAllparent will telescopically search each of the parent and see if it
	// can get the value
	...
	public Object getValue(String key, boolean searchAllParent) {
		...
		while (!nextSet.isEmpty()) {
			...
			// if continuing with parent.. add all the child to the next list so we can
			// continue to search
			if (searchAllParent) {
	...
	public boolean validatePhone(String phone) {
		// accomodates for
		/*
		 * 1234567890 : true 123-456-7890 : true 123.456.7890 : true 123 456 7890 : true
		 * (123) 456 7890 : true
		 */
