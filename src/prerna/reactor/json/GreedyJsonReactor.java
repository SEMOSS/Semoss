...
public class GreedyJsonReactor extends AbstractReactor {
  ...
  // The only way to resolve this problem is introduce an intermediary reactor which will set the
  // specific value
  ...
  public Hashtable getOutput() {
    ...
    if (!store.getNoun(NounStore.all).isEmpty()) {
      ...
      if (reactorName == null || reactorName.isEmpty()) reactorName = DATA;
  ...
  public void addError(String element, String errorDescription) {
    ...
    if (outputHash.containsKey(ERROR)) errorVector = (Vector<String>) outputHash.get(ERROR);
    else errorVector = new Vector<>();
  ...
  public boolean hasError(Vector<Hashtable> remainingHash) {
    ...
    // no more to go.. yay !!
    if (!nextLevelHash.isEmpty() && !error) hasError(nextLevelHash);
        ...
        if (thisHash.containsKey(CHILDS)) {
          ...
          for (int childIndex = 0; childIndex < childs.size(); childIndex++) {
            ...
            if (thisHash.containsKey(childs.get(childIndex))) {
              ...
              else logger.debug("Find out what this is is then ?!");
  ...
  public Object getValue(String key, boolean searchAllParent) {
    ...
    while (!nextSet.isEmpty()) {
      ...
      if (output != null) {
        ...
        if (realValueList.size() == 1) realValue = realValueList.get(0);
      }
