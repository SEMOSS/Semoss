...
public class LazyJsonTranslation extends DepthFirstAdapter {
  ...
  public void inAIdWordOrId(AIdWordOrId node) {
    ...
    // something could be a variable but not be loaded as a variable yet...i.e. loadclient when
  ...
  public void inAMapEntry(AMapEntry node) {
    ...
    if (curReactor != null) curReactor.curNoun(processNode);
    // if the curReactor is null - make one here
      ...
      if (newReactor.hasProp("CHILDS")) childVector = (Vector<String>) newReactor.getProp("CHILDS");
      else childVector = new Vector<String>();
  ...
  public void inANestedMapValues(ANestedMapValues node) {
    ...
    if (thisReactor == null) {
      ...
      newReactor.setPixel(processNode, node.toString().trim());
      Vector<String> childVector = null;
      if (curReactor.hasProp("CHILDS")) childVector = (Vector<String>) curReactor.getProp("CHILDS");
      else childVector = new Vector<String>();
      childVector.add(processNode);
      curReactor.setProp("CHILDS", childVector);
   ...
   * @return Return the reactor based on the reactorId Sets the PKSL operations in the reactor
   */
  private IReactor getReactor(String reactorId, String nodeString) {
    IReactor newReactor =
        ReactorFactory.getReactor(
            this.insight, reactorId, this.curReactor, (ITableDataFrame) getDataMaker());
