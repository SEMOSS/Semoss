...
public class LazyJsonTranslation extends DepthFirstAdapter {
  ...
  //////////////////////////////////////////
	...
	public void inAIdWordOrId(AIdWordOrId node) {
		...
		// something could be a column but not loaded into a frame yet...i.e. select
		// pksl in import
		// something could be a variable but not be loaded as a variable yet...i.e.
		// loadclient when
		// loading pksls one by one into the graph in any order
		if (curReactor != null) {
	...
	// fraction = "0";
	// }
	// Number retNum = new BigDecimal(whole + "." + fraction).divide(new
	// BigDecimal(100));
	// // determine if it is a negative
	// // in which case, multiply by -1
	////    	if(node.getPosOrNeg() != null) {
  ...
  ////    		}
  ////    	}
	// NounMetadata noun = new NounMetadata(retNum.doubleValue(),
	// PkslDataTypes.CONST_DECIMAL);
	// if(curReactor != null) {
	// curReactor.getCurRow().add(noun);
	// // modify the parent such that the signature has the correct
	// // value of the numerical without any extra spaces
	// // plain string will always modify to a integer even if we return double
	// value
	// curReactor.modifySignature(node.toString().trim(), new
	// BigDecimal(retNum.doubleValue()).toPlainString());
	// } else {
	// // looks like you just have a number...
	// // i guess i will return this?
	// this.planner.addVariable("$RESULT", noun);
	...
	public void inANestedMapValues(ANestedMapValues node) {
		...
		if (thisReactor != null) {
			...
			if (curReactor != null) {
				...
				// the idea behind this is to take the store and add the child automatically and
				// parent
				...
				/*
				 * NounStore store = curReactor.getNounStore(); NounStore newStore =
				 * thisReactor.getNounStore();
				 * 
				 * GenRowStruct newStoreStruct = new GenRowStruct();
				 * newStoreStruct.add(newStore, PkslDataType.MAP); store.addNoun(IReactor.CHILD,
				 * newStoreStruct);
				 * 
				 * GenRowStruct curStoreStruct = new GenRowStruct(); curStoreStruct.add(store,
				 * PkslDataType.MAP); newStore.addNoun(IReactor.PARENT, newStoreStruct);
				 */
  ...
  //////////////////////////////////////////////////////////////////
	 ...
	 * @param nodeString
	 *            - full operation
	 * @return Return the reactor based on the reactorId Sets the PKSL operations in
	 *         the reactor
	 */
	private IReactor getReactor(String reactorId, String nodeString) {
		IReactor newReactor = ReactorFactory.getReactor(this.insight, reactorId, this.curReactor,
				(ITableDataFrame) getDataMaker());
