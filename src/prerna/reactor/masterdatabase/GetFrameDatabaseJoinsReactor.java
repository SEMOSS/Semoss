...
public class GetFrameDatabaseJoinsReactor extends AbstractFrameReactor {
  ...
  public GetFrameDatabaseJoinsReactor() {
    this.keysToGet =
        new String[] {ReactorKeysEnum.FRAME.getKey(), ReactorKeysEnum.DATABASE.getKey()};
  }
  ...
  public NounMetadata execute() {
    ...
    if (specificDbFilter != null) {
      if (!SecurityEngineUtils.userCanViewEngine(this.insight.getUser(), specificDbFilter)) {
        throw new IllegalArgumentException(
            "Database "
                + specificDbFilter
                + " does not exist or user does not have access to database");
      }
    ...
    for (String uniqueFrameHeader : dbInfo.keySet()) {
      ...
      for (String[] info : qsData) {
        if (info.length == 2) {
          String physicalNameId =
              MasterDatabaseUtility.getPhysicalConceptIdFromPixelName(info[0], info[1]);
      ...
      List<String> conceptualNames =
          MasterDatabaseUtility.getConceptualNamesFromPhysicalIds(physicalNameIds);
      List<String[]> headerConnections =
          MasterDatabaseUtility.getConceptualConnections(conceptualNames, dbFilters);
