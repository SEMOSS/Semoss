...
public class GetFrameDatabaseJoinsReactor extends AbstractFrameReactor {
	...
	public NounMetadata execute() {
		...
		if (specificDbFilter != null) {
			if (!SecurityEngineUtils.userCanViewEngine(this.insight.getUser(), specificDbFilter)) {
				throw new IllegalArgumentException(
						"Database " + specificDbFilter + " does not exist or user does not have access to database");
			}
		...
		for (String uniqueFrameHeader : dbInfo.keySet()) {
			...
			List<String[]> headerConnections = MasterDatabaseUtility.getConceptualConnections(conceptualNames,
					dbFilters);
		...
		return new NounMetadata(connections, PixelDataType.CUSTOM_DATA_STRUCTURE,
