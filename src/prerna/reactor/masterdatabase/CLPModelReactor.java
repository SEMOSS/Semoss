...
public class CLPModelReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    List<String[]> values =
        MasterDatabaseUtility.getConceptualToLogicalToPhysicalModel(cNames, engineFilters);
    if (values == null) {
      throw new IllegalArgumentException(
          "Query returned no results using the conceptual names passed in");
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error occurred attempting to insert CLP model into a frame");
    } finally {
      if (ps != null) {
        try {
          ps.close();
