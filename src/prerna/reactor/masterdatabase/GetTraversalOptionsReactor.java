...
public class GetTraversalOptionsReactor extends AbstractReactor {
  ...
  public NounMetadata execute() {
    ...
    if (logicals != null && !logicals.isEmpty()) {
      ...
      Map connectedConcepts =
          MasterDatabaseUtility.getConnectedConceptsRDBMS(logicals, engineFilters);
      Map<String, Object[]> conceptProperties =
          MasterDatabaseUtility.getConceptProperties(logicals, engineFilters);

      // so if this is a property from a node.. then I need to find what that node is
      // and then from that node traverse to other things.. although.. I have no idea why I would do
      // that
