...
public class GetTraversalOptionsReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		if (logicals != null && !logicals.isEmpty()) {
			...
			Map<String, Object[]> conceptProperties = MasterDatabaseUtility.getConceptProperties(logicals,
					engineFilters);

			// so if this is a property from a node.. then I need to find what that node is
			// and then from that node traverse to other things.. although.. I have no idea
			// why I would do
			// that
			// alternately.. I need to account for the flag which could be properties
			// instead of nodes

			traversalOptions.put("connectedConcepts", connectedConcepts);
			traversalOptions.put("connectedProperties", conceptProperties);
			return new NounMetadata(traversalOptions, PixelDataType.CUSTOM_DATA_STRUCTURE,
					PixelOperationType.TRAVERSAL_OPTIONS);
		}

		// if there are no traversal options
		// return empty map
		return new NounMetadata(new HashMap<String, Object>(), PixelDataType.CUSTOM_DATA_STRUCTURE,
				PixelOperationType.TRAVERSAL_OPTIONS);
	}
