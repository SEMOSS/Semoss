...
/**
 * Reactor to upload the images in the database and project cluster folders to CouchDB. There are
 * optional input keys to indicate if the remote folders should be re-synced, if images should be
 * filtered to existing databases and projects, or if we should check the whether the image is
 * already in CouchDB before upload.
 */
public class MigrateImagesToCouchReactor extends AbstractReactor {
  ...
  private static final String SELECTOR_TEMPLATE =
      "{\"selector\": {\"%s\": {\"$eq\": \"%s\"}, \"_attachments\": {\"%s\": {\"digest\": {\"$eq\": \"%s\"}}}}}";
  ...
  public NounMetadata execute() {
    ...
    uploadRemoteToCouch(
        ClusterUtil.IMAGES_FOLDER_PATH + DIR_SEPARATOR + "databases",
        SecurityEngineUtils.getAllEngineIds(),
        CouchUtil.DATABASE,
        outcomes);
    uploadRemoteToCouch(
        ClusterUtil.IMAGES_FOLDER_PATH + DIR_SEPARATOR + "storages",
        SecurityEngineUtils.getAllEngineIds(),
        CouchUtil.STORAGE,
        outcomes);
    uploadRemoteToCouch(
        ClusterUtil.IMAGES_FOLDER_PATH + DIR_SEPARATOR + "models",
        SecurityEngineUtils.getAllEngineIds(),
        CouchUtil.MODEL,
        outcomes);
    uploadRemoteToCouch(
        ClusterUtil.IMAGES_FOLDER_PATH + DIR_SEPARATOR + "projects",
        SecurityProjectUtils.getAllProjectIds(),
        CouchUtil.PROJECT,
        outcomes);
  ...
  private void loadInputs() {
    ...
    if (!SecurityAdminUtils.userIsAdmin(user)) {
      SemossPixelException err =
          new SemossPixelException(
              new NounMetadata(
                  "User must be an admin to perform this operation",
    ...
    if (!CouchUtil.COUCH_ENABLED || !ClusterUtil.IS_CLUSTER) {
      SemossPixelException err =
          new SemossPixelException(
              new NounMetadata(
                  "CouchDB and Clustering need to be enabled to use this reactor",
                  PixelDataType.CONST_STRING,
                  PixelOperationType.ERROR));
      err.setContinueThreadOfExecution(false);
      throw err;
    ...
    if (repull) {
      try {
        ...
        SemossPixelException err =
            new SemossPixelException(
                new NounMetadata(
                    "Failed to pull images from cloud storage",
                    PixelDataType.CONST_STRING,
                    PixelOperationType.ERROR));
        err.setContinueThreadOfExecution(false);
        throw err;
  ...
  private void uploadRemoteToCouch(
      String imageFolderPath, List<String> ids, String partition, List<String> outcomes) {
    File[] images =
        new File(imageFolderPath)
            .listFiles(
                new FileFilter() {
                  @Override
        ...
        if (verifyMissing) {
          ...
          try {
            ...
            String digest =
                "md5-"
                    + new String(
                        Base64.encodeBase64(md5Instance.digest(Files.readAllBytes(f.toPath()))));
            String searchSelector =
                String.format(SELECTOR_TEMPLATE, partition, identifier, attachmentName, digest);
