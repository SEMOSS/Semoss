...
public class Assimilator extends AbstractReactor implements JavaExecutable {
	...
	public NounMetadata execute() {
		...
		for (String formula : formulas) {
			this.signature = StringUtils.replaceOnce(this.signature, formula,
					"( 1.0 * " + formula.substring(1, formula.length()));
		}
		...
		// noun = new NounMetadata( ((Number) retVal).intValue(),
		// PkslDataTypes.CONST_INT);
		// } else {
		// // not a valid integer
		// // return as a double
		// noun = new NounMetadata( ((Number) retVal).doubleValue(),
	...
	/**
	 * @param stringMethod
	 * @return method responsible for building a new assimilator class from a
	 *         stringified method
	 */
	...
	private void appendVariables(StringBuilder expressionBuilder, Map<String, Object> vars) {
		...
		for (String input : uniqueInputs) {
			...
			if (dataType == PixelDataType.CONST_DECIMAL) {
				expressionBuilder.append("double ").append(input).append(" = ")
						.append("((Number)super.vars.get(" + "\"" + input + "\")).doubleValue()").append(";");
				vars.put(input, value);
			} else if (dataType == PixelDataType.CONST_INT) {
				expressionBuilder.append("int ").append(input).append(" = ")
						.append("((Number)super.vars.get(" + "\"" + input + "\")).intValue()").append(";");
				vars.put(input, value);
			} else if (dataType == PixelDataType.CONST_STRING) {
				this.containsStringValue = true;
				expressionBuilder.append("String ").append(input).append(" = ")
						.append("(String)super.vars.get(" + "\"" + input + "\")").append(";");
				vars.put(input, value);
		...
		for (int i = 0; i < lambdas.size(); i++) {
			...
			if (n.getValue() instanceof EmbeddedScriptReactor) {
				...
				if (dataType == PixelDataType.CONST_DECIMAL || dataType == PixelDataType.CONST_INT) {
					this.signature = StringUtils.replaceOnce(this.signature,
							((EmbeddedScriptReactor) n.getValue()).getOriginalSignature(),
					...
					if (formatData == null) {
						...
						if (newDataType == PixelDataType.CONST_DECIMAL || newDataType == PixelDataType.CONST_INT) {
							this.signature = StringUtils.replaceOnce(this.signature,
									((EmbeddedScriptReactor) n.getValue()).getOriginalSignature(), newValue.toString());
						} else {
							this.signature = StringUtils.replaceOnce(this.signature,
									((EmbeddedScriptReactor) n.getValue()).getOriginalSignature(), newValue.toString());
							this.containsStringValue = true;
	...
	public String getJavaSignature() {
		...
		for (int i = 0; i < curRow.size(); i++) {
			...
			// NounMetadata result = thisReactor.execute();// this might further trigger
			// other things
			...
			// replaceValue = new BigDecimal( ((Number)
			// replaceValue).doubleValue()).toPlainString();
			// } else {
