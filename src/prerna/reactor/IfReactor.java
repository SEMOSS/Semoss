...
public class IfReactor extends AbstractReactor implements JavaExecutable {
  ...
  public boolean getBooleanEvaluation() {
    ...
    if (ifEvaluatorType == PixelDataType.BOOLEAN) {
      ...
      this.caseEvaluation =
          (boolean) this.planner.getVariableValue(ifEvaluatorObject.toString()).getValue();
  ...
  private NounMetadata evaluateStatement(NounMetadata evalNoun) {
    ...
    if (evalNoun.getValue() instanceof AbstractReactor) {
      ...
      NounMetadata evalutedNoun =
          new NounMetadata(evalutedValues, PixelDataType.VECTOR, evalNoun.getOpType());
  ...
  private String getReturnType(NounMetadata returnNoun) {
    ...
    } else if (returnNoun.getNounType() == PixelDataType.CONST_DECIMAL
        || returnNoun.getNounType() == PixelDataType.CONST_INT) {
      returnType = "double";
