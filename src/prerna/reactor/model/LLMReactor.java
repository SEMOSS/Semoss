...
public class LLMReactor extends AbstractReactor {
	...
	public NounMetadata execute() {
		...
		// default is true
		Boolean useHistoryParam = getBoolean(ReactorKeysEnum.USE_HISTORY.getKey(), true);

		String question = Utility.decodeURIComponent(getString(ReactorKeysEnum.COMMAND.getKey()));
		String context = getString(ReactorKeysEnum.CONTEXT.getKey());
		if (context != null) {
	...
	protected String getDescriptionForKey(String key) {
		if (key.equals(ReactorKeysEnum.COMMAND.getKey())) {
			...
			return """
					Boolean true/false to determine if we should incorporate the user's chat history based on the previous chats in this insight id.
					Default is true.
					This is the same as passing 'use_history' in the <replacement> key-value map
					"""
					.replace("<replacement>", ReactorKeysEnum.PARAM_VALUES_MAP.getKey());
