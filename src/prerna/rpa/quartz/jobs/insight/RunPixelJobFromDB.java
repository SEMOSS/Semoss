...
public class RunPixelJobFromDB implements InterruptableJob {
  ...
  public void execute(JobExecutionContext context) throws JobExecutionException {
    ...
    String userAccess =
        RPAProps.getInstance().decrypt(dataMap.getString(JobConfigKeys.USER_ACCESS));
    ...
    try {
      ...
      if (url == null) {
        throw new IllegalArgumentException(
            "Must define the scheduler endpoint to run scheduled jobs");
      }
      url = url.trim();
      ...
      CloseableHttpClient httpclient =
          HttpHelperUtility.getCustomClient(httpCookieStore, keyStore, keyStorePass, keyPass);
      if (FETCH_CSRF) {
        String fetchUrl = url;
      ...
      if (pixelParameters != null && !(pixelParameters = pixelParameters.trim()).isEmpty()) {
        ...
        if (!pixelParameters.isEmpty()) {
          hasParam = true;
          paramList.add(
              new BasicNameValuePair(JobConfigKeys.PIXEL, pixelParameters + " | " + pixel));
        }
      }
      if (!hasParam) {
      ...
      //			try {
      //				logger.info("##SCHEDULED JOB: Json return = " +
      ...
      SchedulerDatabaseUtility.insertIntoAuditTrailTable(
          jobId, jobGroup, start, end, success, schedulerOutput);
  ...
  public void interrupt() throws UnableToInterruptJobException {
    logger.warn(
        "Received request to interrupt the "
            + jobId
