...
public final class FormBuilder {
  ...
  public static void commitFormData(
      IDatabaseEngine engine, Map<String, Object> engineHash, String user) throws IOException {
    if (engine == null) {
    ...
    String auditLogTableName =
        AbstractSqlQueryUtil.escapeForSQLStatement(
                    RDBMSEngineCreationHelper.cleanTableName(engine.getEngineId()))
                .toUpperCase()
            + AUDIT_FORM_SUFFIX;
    ...
    String checkTableQuery =
        "SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='" + auditLogTableName + "'";
   ...
   * @param nodes
   * @param relationships
   *     <p>Save data from the form to a RDF Database
   * @param auditLogTableName
   * @param formEng
   * @param user
   */
  private static void saveRDFFormData(
    ...
    for (int i = 0; i < removeRelationships.size(); i++) {
      ...
      instanceSubjectURI =
          baseURI + "/Concept/" + Utility.getInstanceName(subject) + "/" + startNode;
    ...
    for (int i = 0; i < removeNodes.size(); i++) {
      ...
      instanceConceptURI =
          baseURI + "/Concept/" + Utility.getInstanceName(conceptType) + "/" + conceptValue;
      ...
      if (removeConcept) {
        ...
        deleteAllRDFConnectionsToConcept(
            engine, uriBindingList, formEng, auditLogTableName, user, DATE_DF);
        ...
        List<HashMap<String, Object>> properties =
            (List<HashMap<String, Object>>) deleteConcept.get("properties");
        ...
        for (int j = 0; j < properties.size(); j++) {
          ...
          engine.doAction(
              IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
              new Object[] {instanceConceptURI, propertyURI, propertyValue, false});
          // ugh... we need to push forms
          // values being passed are not properly keeping track of things that have underscores and
          // things that don't
          ...
          engine.doAction(
              ...
              new Object[] {
                instanceConceptURI,
                propertyURI,
                Utility.cleanString(propertyValue.toString(), true, false, true),
                false
              });
    ...
    for (int i = 0; i < relationships.size(); i++) {
      ...
      engine.doAction(
          IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
    ...
    for (int i = 0; i < nodes.size(); i++) {
      ...
      if (concept.containsKey("properties")) {
        List<HashMap<String, Object>> properties =
            (List<HashMap<String, Object>>) concept.get("properties");
  ...
  //	private static void removeRDFNodeProp(IDatabase engine, String instanceConceptURI, String
  ...
  // instanceConceptURI + "> AS ?instance) {?instance <" + propertyURI + "> ?propVal} }";
  //
  ...
  //			engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{instanceConceptURI,
  ...
  /**
   * Deletes the relationship and relationship properties that exist between an instance and all
   * other instances of a specified type
   *
   * @param engine
   ...
   * @param deleteUnconnectedConcepts
   */
  private static void overrideRDFRelationship(
      IDatabaseEngine engine,
      String instanceSubjectURI,
      ...
      String baseRelationshipURI,
      boolean deleteDownstream,
      boolean deleteUnconnectedConcepts,
      boolean removeNode,
      IDatabaseEngine formEng,
      String auditLogTableName,
      String user,
      final DateFormat DATE_DF) {
    StringBuilder query =
        new StringBuilder("SELECT DISTINCT ?SUB ?PRED ?OBJ ?LABEL ?PROP ?VAL WHERE { ");
    //		if(deleteDownstream) {
    ...
    query
        .append("{?PROP <")
        .append(RDF.TYPE)
        .append("> <http://semoss.org/ontologies/Relation/Contains>} ");
  ...
  private static void deleteAllRDFConnectionsToConcept(
      IDatabaseEngine engine,
      Set<String> uriBindingList,
      IDatabaseEngine formEng,
      String auditLogTableName,
      String user,
      final DateFormat DATE_DF) {
    ...
    for (String query : queries) {
      ...
      while (wrapper.hasNext()) {
        ...
        String currTime = DATE_DF.format(cal.getTime());
        addAuditLog(
            formEng, auditLogTableName, user, REMOVE, subURI, predURI, objURI, "", "", currTime);
        if (label != null && label.toString().isEmpty()) {
          engine.doAction(
              IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
  ...
  //	private static void deleteAllRDFConnectionsToConcept(IDatabase engine, String conceptURI) {
  ...
  //				Object propVal = ss.getVar(names[5]);
  //
  //				engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{subURI, predURI,
  // objURI, true});
  //				if(!label.isEmpty()) {
  //					engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{predURI, RDFS.LABEL,
  // label, false});
  //				}
  //				if(!propURI.isEmpty()) {
  //					engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{predURI, propURI,
  // propVal, false});
  //				}
  //			}
  //		}
  ...
  private static String generateDeleteAllRDFConnectionsToConceptQuery(
      Set<String> conceptURI, boolean downstream) {
    ...
    query
        .append("{?PRED <")
        .append(RDFS.SUBPROPERTYOF)
        .append("> <http://semoss.org/ontologies/Relation>} ");
    query.append("{?SUB ?PRED ?OBJ} ");
    query.append("OPTIONAL{ ?PRED <").append(RDFS.LABEL).append("> ?LABEL} ");
    query.append("} UNION { ");
    query
  ...
  //	private static String generateDeleteAllRDFConnectionsToConceptQuery(String conceptURI, boolean
  // downstream) {
  //		StringBuilder query = new StringBuilder("SELECT DISTINCT ?SUB ?PRED ?OBJ ?LABEL ?PROP ?VAL
  // WHERE { ");
  //		if(downstream) {
  ...
  private static void removeUnconnectedRDFNodes(
      IDatabaseEngine engine,
      Set<String> uriBindingList,
      IDatabaseEngine formEng,
      String auditLogTableName,
    ...
    StringBuilder query =
        new StringBuilder("SELECT DISTINCT ?CONCEPT (COUNT(?REL) AS ?C_RELS) WHERE {");
    query.append(
  ...
  //	public static void removeUnconnectedRDFNodes(IDatabase engine, String conceptURI) {
  //		// check relationships in one direction
  ...
  private static void removeRDFNodeAndAllProps(
      IDatabaseEngine engine,
      Set<String> uriBindingList,
      IDatabaseEngine formEng,
      String auditLogTableName,
  ...
  //	private static void removeRDFNodeAndAllProps(IDatabase engine, String conceptURI) {
  //		StringBuilder query = new StringBuilder("SELECT DISTINCT ?NODE ?PROP ?VAL WHERE { ");
  //		query.append("BIND(<" + conceptURI + "> AS ?NODE) ");
  //		query.append("{?PROP <").append(RDF.TYPE).append(">
  // <http://semoss.org/ontologies/Relation/Contains>} ");
  ...
  //			Object propVal = ss.getVar(names[2]);
  //
  //			engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{nodeURI, propURI,
  // propVal, false});
  //		}
  //		String typeURI = "http://semoss.org/ontologies/Concept/" + Utility.getClassName(conceptURI);
  //		engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{conceptURI, RDF.TYPE,
  // typeURI, true});
  //		engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{conceptURI, RDFS.LABEL,
   ...
   * @param nodes
   * @param relationships
   *     <p>Save form data to a RDBMS database
   * @param user
   * @param auditLogTableName
   * @param formEng
   */
  private static void saveRDBMSFormData(
      IDatabaseEngine engine,
      String baseURI,
      String relationURI,
    ...
    for (int j = 0; j < nodes.size(); j++) {
      ...
      if (override
          && RDBMSEngineCreationHelper.conceptExists(engine, tableName, tableColumn, tableValue)) {
        String updateQuery =
            createUpdateStatement(
                ...
                GENERIC_DF,
                SIMPLE_DATE_DF);
        // TODO: need to enable modifying the actual instance name as opposed to only its
        // properties.. this would set the updateQuery to never return back an empty string
    ...
    for (int r = 0; r < relationships.size(); r++) {
      ...
      String[] relVals =
          Utility.getInstanceName(relationship.get("relType").toString()).split("\\.");
      // use the relationship to get the information
      // format is Title.Title.Studio.Title_FK (OTHER_TABLE_NAME, OTHER_COL_NAME, TABLE_NAME,
      // FOREIGN_KEY)
      ...
      if (override) {
        ...
        if (relationship.get(DELETE_UNCONNECTED_CONCEPTS) != null) {
          deleteUnconnectedConcepts =
              Boolean.parseBoolean(relationship.get(DELETE_UNCONNECTED_CONCEPTS).toString());
        }
        boolean removeNode = false;
        ...
        if (type.equalsIgnoreCase(UPSTREAM)) {
          if (!deletedRels.contains(table + foreignKeyCol + conceptVal)) {
            overrideUpstreamRDBMSRelationship(
                engine,
                table,
                conceptCol,
                conceptVal,
          ...
          if (!deletedRels.contains(table + conceptCol + foreignKeyVal)) {
            overrideDownstreamRDBMSRelationship(
                engine,
                table,
                conceptCol,
                conceptVal,
        ...
        addRDBMSRelationship(
            engine, table, conceptCol, conceptVal, foreignKeyCol, foreignKeyVal, tableColTypesHash);
      }
    }
  ...
  private static void overrideUpstreamRDBMSRelationship(
      IDatabaseEngine engine,
      String table,
      String conceptCol,
      String conceptVal,
  ...
  private static void overrideDownstreamRDBMSRelationship(
      IDatabaseEngine engine,
      String table,
      String conceptCol,
      String conceptVal,
    ...
    queryBuilder
        ...
        .append(" AS (SELECT DISTINCT * FROM ")
        .append(table)
        .append(" WHERE ")
        .append(conceptCol)
        .append("='")
  ...
  private static void addRDBMSRelationship(
      IDatabaseEngine engine,
      String table,
      String conceptCol,
      String conceptVal,
      ...
      String foreignKeyVal,
      Map<String, Map<String, String>> tableColTypesHash) {
    // if concept already exists, need to add in manner to preserve many-to-many relationship
    // structure
    if (RDBMSEngineCreationHelper.conceptExists(engine, table, conceptCol, conceptVal)) {
      appendRDBMSRelationship(
          engine, table, conceptCol, conceptVal, foreignKeyCol, foreignKeyVal, tableColTypesHash);
    } else {
  ...
  private static void appendRDBMSRelationship(
      IDatabaseEngine engine,
      String table,
      String conceptCol,
      String conceptVal,
      ...
      Map<String, Map<String, String>> tableColTypesHash) {
    StringBuilder queryBuilder = new StringBuilder();
    // it exists, now need to find all unique values given the instance except the foreign key and
    ...
    // create a new temp table that is the unique set of all columns for the specific instance
  ...
  private static void removeDuplicates(
      IDatabaseEngine engine,
      List<String> tablesToRemoveDuplicates,
  ...
  private static String createInsertStatement(
      ...
      List<String> types,
      final DateFormat DATE_DF,
      final DateFormat GENERIC_DF,
      final DateFormat SIMPLE_DATE_DF) {
  ...
  private static void addAuditLog(
    ...
    try {
      ...
      queryUtil.handleInsertionOfClob(
          ps.getConnection(), ps, propValue, parameterIndex++, new Gson());
  ...
  public static Map<String, Object> getAuditDataForEngine(String engineName) {
    ...
    String auditLogTableName =
        AbstractSqlQueryUtil.escapeForSQLStatement(
                    RDBMSEngineCreationHelper.cleanTableName(engineName))
                .toUpperCase()
            + AUDIT_FORM_SUFFIX;
    IDatabaseEngine formEng = Utility.getDatabase(FORM_BUILDER_ENGINE_NAME);
