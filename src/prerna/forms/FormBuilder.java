...
public final class FormBuilder {
	 ...
	 * @throws IOException
	 */
	public static void commitFormData(IDatabaseEngine engine, Map<String, Object> engineHash, String user)
			throws IOException {
		if (engine == null) {
			throw new IOException("Engine cannot be found");
		}

		String auditLogTableName = AbstractSqlQueryUtil
				.escapeForSQLStatement(RDBMSEngineCreationHelper.cleanTableName(engine.getEngineId())).toUpperCase()
				+ AUDIT_FORM_SUFFIX;
		...
		if (!auditTableExists) {
			String createAuditTable = "CREATE TABLE " + auditLogTableName + " (" + "ID IDENTITY, "
					+ "USER VARCHAR(255), " + "ACTION VARCHAR(100), " + "START_NODE VARCHAR(255), "
					+ "REL_NAME VARCHAR(255), " + "END_NODE VARCHAR(255), " + "PROP_NAME VARCHAR(255), "
					+ "PROP_VALUE CLOB, " + "TIME TIMESTAMP)";
			try {
		...
		if (engine.getDatabaseType() == IDatabaseEngine.DATABASE_TYPE.JENA
			...
			saveRDFFormData(engine, baseURI, relationBaseURI, propertyBaseURI, nodes, relationships, removeNodes,
					removeRelationships, formEng, auditLogTableName, user);
			...
			saveRDBMSFormData(engine, baseURI, relationBaseURI, conceptBaseURI, propertyBaseURI, nodes, relationships,
					formEng, auditLogTableName, user);
		} else {
	 ...
	 * @param user
	 */
	private static void saveRDFFormData(IDatabaseEngine engine, String baseURI, String relationBaseURI,
			String propertyBaseURI, List<HashMap<String, Object>> nodes, List<HashMap<String, Object>> relationships,
			List<HashMap<String, Object>> removeNodes, List<HashMap<String, Object>> removeRelationships,
			IDatabaseEngine formEng, String auditLogTableName, String user) {
		...
		// final DateFormat GENERIC_DF = new
		...
		for (int i = 0; i < removeRelationships.size(); i++) {
			...
			overrideRDFRelationship(engine, instanceSubjectURI, subject, instanceObjectURI, object, baseRelationshipURI,
					true, deleteUnconnectedConcepts, removeNode, formEng, auditLogTableName, user, DATE_DF);
		}
		...
		for (int i = 0; i < removeNodes.size(); i++) {
			...
			if (removeConcept) {
				...
				List<HashMap<String, Object>> properties = (List<HashMap<String, Object>>) deleteConcept
						.get("properties");
				...
				for (int j = 0; j < properties.size(); j++) {
					...
					engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
							new Object[]{instanceConceptURI, propertyURI, propertyValue, false});
					// ugh... we need to push forms
					// values being passed are not properly keeping track of things that have
					// underscores and
					// things that don't
					// just going to try both versions
					engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{instanceConceptURI,
							propertyURI, Utility.cleanString(propertyValue.toString(), true, false, true), false});

					// add audit log statement
					Calendar cal = Calendar.getInstance();
					String currTime = DATE_DF.format(cal.getTime());
					addAuditLog(formEng, auditLogTableName, user, REMOVE, instanceConceptURI, "", "", propertyURI,
							propertyValue + "", currTime);
					engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
							new Object[]{propertyURI, RDF.TYPE, propertyBaseURI, true});
				}
			}
		}
		...
		for (int i = 0; i < relationships.size(); i++) {
			...
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceSubjectURI, RDF.TYPE, subject, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceObjectURI, RDF.TYPE, object, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceSubjectURI, relationBaseURI, instanceObjectURI, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceSubjectURI, baseRelationshipURI, instanceObjectURI, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceSubjectURI, instanceRelationshipURI, instanceObjectURI, true});
			// add audit log statement
			Calendar cal = Calendar.getInstance();
			String currTime = DATE_DF.format(cal.getTime());
			addAuditLog(formEng, auditLogTableName, user, ADD, instanceSubjectURI, baseRelationshipURI,
					instanceObjectURI, "", "", currTime);
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceRelationshipURI, RDFS.SUBPROPERTYOF, baseRelationshipURI, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceRelationshipURI, RDFS.SUBPROPERTYOF, relationBaseURI, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceRelationshipURI, RDF.TYPE, RDF.PROPERTY, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceRelationshipURI, RDFS.LABEL, instanceRel, false});
		}
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			instanceConceptURI = baseURI + "/Concept/" + Utility.getInstanceName(conceptType) + "/" + conceptValue;
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceConceptURI, RDF.TYPE, conceptType, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
					new Object[]{instanceConceptURI, RDFS.LABEL, conceptValue, false});
			if (concept.containsKey("properties")) {
				...
				for (int j = 0; j < properties.size(); j++) {
					...
					Calendar cal = Calendar.getInstance();
					String currTime = DATE_DF.format(cal.getTime());
					addAuditLog(formEng, auditLogTableName, user, ADD, instanceConceptURI, "", "", propertyURI,
							propertyValue + "", currTime);
					engine.doAction(IDatabaseEngine.ACTION_TYPE.ADD_STATEMENT,
							new Object[]{propertyURI, RDF.TYPE, propertyBaseURI, true});
				}
	...
	// private static void removeRDFNodeProp(IDatabase engine, String
	// instanceConceptURI, String
	// propertyURI, IDatabase formEng, String auditLogTableName, String user) {
	// String getOldNodePropValuesQuery = "SELECT DISTINCT ?propVal WHERE { BIND(<"
	// +
	// instanceConceptURI + "> AS ?instance) {?instance <" + propertyURI + ">
	// ?propVal} }";
	//
	// List<Object> propVals = new ArrayList<Object>();
	// ISelectWrapper wrapper = WrapperManager.getInstance().getSWrapper(engine,
	// getOldNodePropValuesQuery);
	...
	// engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new
	// Object[]{instanceConceptURI,
	// propertyURI, propertyValue, false});
	// // add audit log statement
	// Calendar cal = Calendar.getInstance();
	// String currTime = DATE_DF.format(cal.getTime());
	// addAuditLog(formEng, auditLogTableName, user, REMOVE, instanceConceptURI, "",
	// "",
	// propertyURI, propertyValue + "", currTime);
	// }
	// }

	/**
	 * Deletes the relationship and relationship properties that exist between an
	 * instance and all other instances of a specified type
	 *
	 * @param engine
	 ...
	 * @param deleteUnconnectedConcepts
	 */
	private static void overrideRDFRelationship(IDatabaseEngine engine, String instanceSubjectURI,
			String subjectTypeURI, String instanceObjectURI, String objectTypeURI, String baseRelationshipURI,
			boolean deleteDownstream, boolean deleteUnconnectedConcepts, boolean removeNode, IDatabaseEngine formEng,
			String auditLogTableName, String user, final DateFormat DATE_DF) {
		...
		while (wrapper.hasNext()) {
			...
			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{subURI, baseRelationshipURI, objURI, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{subURI, baseRelationURI, objURI, true});
			// add audit log statement
			Calendar cal = Calendar.getInstance();
			String currTime = DATE_DF.format(cal.getTime());
			addAuditLog(formEng, auditLogTableName, user, REMOVE, subURI, baseRelationshipURI, objURI, "", "",
					currTime);

			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{predURI, RDFS.SUBPROPERTYOF, baseRelationshipURI, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{predURI, RDFS.SUBPROPERTYOF, baseRelationURI, true});
			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{predURI, RDF.TYPE, RDF.PROPERTY, true});
			if (label != null && !label.toString().isEmpty()) {
				engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
						new Object[]{predURI, RDFS.LABEL, label.toString(), false});
			}
			if (propURI != null && !propURI.toString().isEmpty()) {
				engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
						new Object[]{predURI, propURI.toString(), propVal, false});
				// add audit log statement
				currTime = DATE_DF.format(cal.getTime());
				addAuditLog(formEng, auditLogTableName, user, REMOVE, "", predURI, "", propURI.toString(), propVal + "",
						currTime);
			}
	...
	private static void deleteAllRDFConnectionsToConcept(IDatabaseEngine engine, Set<String> uriBindingList,
			IDatabaseEngine formEng, String auditLogTableName, String user, final DateFormat DATE_DF) {
		String[] queries = new String[]{generateDeleteAllRDFConnectionsToConceptQuery(uriBindingList, true),
		...
		for (String query : queries) {
			...
			while (wrapper.hasNext()) {
				...
				engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
						new Object[]{subURI, predURI, objURI, true});
				// add audit log statement
				Calendar cal = Calendar.getInstance();
				String currTime = DATE_DF.format(cal.getTime());
				...
				if (label != null && label.toString().isEmpty()) {
					engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
							new Object[]{predURI, RDFS.LABEL, label, false});
				}
				if (propURI != null && !propURI.toString().isEmpty()) {
					engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
							new Object[]{predURI, propURI, propVal, false});
					// add audit log statement
					cal = Calendar.getInstance();
					currTime = DATE_DF.format(cal.getTime());
					addAuditLog(formEng, auditLogTableName, user, REMOVE, "", predURI, "", propURI.toString(),
							propVal + "", currTime);
				}
			}
		}
	...
	// private static void deleteAllRDFConnectionsToConcept(IDatabase engine, String
	// conceptURI) {
	...
	// Object propVal = ss.getVar(names[5]);
	//
	// engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{subURI,
	// predURI,
	// objURI, true});
	// if(!label.isEmpty()) {
	// engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{predURI,
	// RDFS.LABEL,
	// label, false});
	// }
	// if(!propURI.isEmpty()) {
	...
	// private static String generateDeleteAllRDFConnectionsToConceptQuery(String
	// conceptURI, boolean
	// downstream) {
	// StringBuilder query = new StringBuilder("SELECT DISTINCT ?SUB ?PRED ?OBJ
	// ?LABEL ?PROP ?VAL
	// WHERE { ");
	// if(downstream) {
	...
	private static void removeUnconnectedRDFNodes(IDatabaseEngine engine, Set<String> uriBindingList,
			IDatabaseEngine formEng, String auditLogTableName, String user, final DateFormat DATE_DF) {
	...
	// public static void removeUnconnectedRDFNodes(IDatabase engine, String
	// conceptURI) {
	// // check relationships in one direction
	// StringBuilder query = new StringBuilder("SELECT DISTINCT (COUNT(?REL) AS
	// ?C_RELS) WHERE {");
	// query.append("BIND(<" + conceptURI + "> AS ?NODE) ");
	// query.append("{?REL <http://www.w3.org/2000/01/rdf-schema#subPropertyOf>
	// <http://semoss.org/ontologies/Relation>} ");
	// query.append("{?CONCEPT a <http://semoss.org/ontologies/Concept>} ");
	// query.append("{?NODE ?REL ?CONCEPT} }");
	//
	...
	//
	// if(!isConnected) {
	// // made sure has no upstream or downstream, so delete it and all its
	// properties
	// removeRDFNodeAndAllProps(engine, conceptURI);
	// }
	// }
	...
	private static void removeRDFNodeAndAllProps(IDatabaseEngine engine, Set<String> uriBindingList,
			IDatabaseEngine formEng, String auditLogTableName, String user, final DateFormat DATE_DF) {
		if (uriBindingList.isEmpty()) {
			return;
		}
		...
		while (wrapper.hasNext()) {
			...
			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{nodeURI, propURI, propVal, false});
			// add audit log statement
			Calendar cal = Calendar.getInstance();
			String currTime = DATE_DF.format(cal.getTime());
		...
		for (String nodeURI : uriBindingList) {
			...
			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{nodeURI, RDF.TYPE, typeURI, true});
			// add audit log statement
			Calendar cal = Calendar.getInstance();
			String currTime = DATE_DF.format(cal.getTime());
			addAuditLog(formEng, auditLogTableName, user, REMOVE, nodeURI, "", "", "", "", currTime);
			engine.doAction(IDatabaseEngine.ACTION_TYPE.REMOVE_STATEMENT,
					new Object[]{nodeURI, RDFS.LABEL, Utility.getInstanceName(nodeURI), false});
		}
	}

	// private static void removeRDFNodeAndAllProps(IDatabase engine, String
	// conceptURI) {
	// StringBuilder query = new StringBuilder("SELECT DISTINCT ?NODE ?PROP ?VAL
	// WHERE { ");
	// query.append("BIND(<" + conceptURI + "> AS ?NODE) ");
	// query.append("{?PROP <").append(RDF.TYPE).append(">
	// <http://semoss.org/ontologies/Relation/Contains>} ");
	...
	// Object propVal = ss.getVar(names[2]);
	//
	// engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new Object[]{nodeURI,
	// propURI,
	// propVal, false});
	// }
	// String typeURI = "http://semoss.org/ontologies/Concept/" +
	// Utility.getClassName(conceptURI);
	// engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new
	// Object[]{conceptURI, RDF.TYPE,
	// typeURI, true});
	// engine.doAction(IDatabase.ACTION_TYPE.REMOVE_STATEMENT, new
	// Object[]{conceptURI, RDFS.LABEL,
	// Utility.getInstanceName(conceptURI), false});
	// }
	 ...
	 * @param formEng
	 */
	private static void saveRDBMSFormData(IDatabaseEngine engine, String baseURI, String relationURI,
			String conceptBaseURI, String propertyBaseURI, List<HashMap<String, Object>> nodes,
			List<HashMap<String, Object>> relationships, IDatabaseEngine formEng, String auditLogTableName,
			String user) {
		...
		Map<String, Map<String, String>> tableColTypesHash = RDBMSEngineCreationHelper
		...
		for (int j = 0; j < nodes.size(); j++) {
			...
			if (!colNamesAndType.containsKey(tableColumn.toUpperCase())) {
				throw new IllegalArgumentException(
						"Table column, " + tableColumn + ", within table name, " + tableName + ", cannot be found.");
			}
			...
			for (int k = 0; k < properties.size(); k++) {
				...
				if (!colNamesAndType.containsKey(propName.toUpperCase())) {
					throw new IllegalArgumentException(
							"Table column, " + propName + ", within table name, " + tableName + ", cannot be found.");
				}
			...
			if (override && RDBMSEngineCreationHelper.conceptExists(engine, tableName, tableColumn, tableValue)) {
				String updateQuery = createUpdateStatement(tableName, tableColumn, tableValue, propNames, propValues,
						types, DATE_DF, GENERIC_DF, SIMPLE_DATE_DF);
				// TODO: need to enable modifying the actual instance name as opposed to only
				// its
				// properties.. this would set the updateQuery to never return back an empty
				// string
				...
				String insertQuery = createInsertStatement(tableName, tableColumn, tableValue, propNames, propValues,
						types, DATE_DF, GENERIC_DF, SIMPLE_DATE_DF);
				try {
		...
		for (int r = 0; r < relationships.size(); r++) {
			...
			if (!colNamesAndType.containsKey(startCol.toUpperCase())) {
				throw new IllegalArgumentException(
						"Table column, " + startCol + ", within table name, " + startTable + ", cannot be found.");
			}
			...
			if (!colNamesAndType.containsKey(endCol.toUpperCase())) {
				throw new IllegalArgumentException(
						"Table column, " + endCol + ", within table name, " + endTable + ", cannot be found.");
			}
			...
			// format is Title.Title.Studio.Title_FK (OTHER_TABLE_NAME, OTHER_COL_NAME,
			// TABLE_NAME,
			// FOREIGN_KEY)
			...
			if (!colNamesAndType.containsKey(conceptCol.toUpperCase())) {
				throw new IllegalArgumentException(
						"Table column, " + conceptCol + ", within table name, " + table + ", cannot be found.");
			}
			if (!colNamesAndType.containsKey(foreignKeyCol.toUpperCase())) {
				throw new IllegalArgumentException(
						"Table column, " + foreignKeyCol + ", within table name, " + table + ", cannot be found.");
			}
			...
			if (override) {
				...
				if (relationship.get(DELETE_UNCONNECTED_CONCEPTS) != null) {
					deleteUnconnectedConcepts = Boolean
							.parseBoolean(relationship.get(DELETE_UNCONNECTED_CONCEPTS).toString());
				}
				boolean removeNode = false;
				...
				if (type.equalsIgnoreCase(UPSTREAM)) {
					if (!deletedRels.contains(table + foreignKeyCol + conceptVal)) {
						overrideUpstreamRDBMSRelationship(engine, table, conceptCol, conceptVal, foreignKeyCol,
								foreignKeyVal, tableColTypesHash, deleteUnconnectedConcepts, removeNode);
						deletedRels.add(table + foreignKeyCol + conceptVal);
					} else {
						// already did the override once, now just insert
						addRDBMSRelationship(engine, table, conceptCol, conceptVal, foreignKeyCol, foreignKeyVal,
								tableColTypesHash);
					}
				} else if (type.equalsIgnoreCase(DOWNSTREAM)) {
					if (!deletedRels.contains(table + conceptCol + foreignKeyVal)) {
						overrideDownstreamRDBMSRelationship(engine, table, conceptCol, conceptVal, foreignKeyCol,
								foreignKeyVal, tableColTypesHash, deleteUnconnectedConcepts, removeNode);
	...
	private static void overrideUpstreamRDBMSRelationship(IDatabaseEngine engine, String table, String conceptCol,
		...
		// override all current values that exist for the relationship to the foreign
		// key
		queryBuilder.append("DELETE FROM ").append(table).append(" WHERE ").append(foreignKeyCol).append("='")
				.append(foreignKeyVal).append("'");
		try {
	...
	private static void overrideDownstreamRDBMSRelationship(IDatabaseEngine engine, String table, String conceptCol,
		...
		queryBuilder.append("UPDATE ").append(table).append(TEMP_EXTENSION).append(" SET ").append(foreignKeyCol)
				.append("='").append(foreignKeyVal).append("' WHERE ").append(conceptCol).append("='")
				.append(conceptVal).append("';");
		try {
			engine.insertData(queryBuilder.toString());
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
		...
		queryBuilder.append("INSERT INTO ").append(table).append(" SELECT * FROM ").append(table)
				.append(TEMP_EXTENSION);
		try {
			engine.insertData(queryBuilder.toString());
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
	...
	private static void addRDBMSRelationship(IDatabaseEngine engine, String table, String conceptCol, String conceptVal,
		...
		// if concept already exists, need to add in manner to preserve many-to-many
		// relationship
		// structure
		if (RDBMSEngineCreationHelper.conceptExists(engine, table, conceptCol, conceptVal)) {
			appendRDBMSRelationship(engine, table, conceptCol, conceptVal, foreignKeyCol, foreignKeyVal,
					tableColTypesHash);
		} else {
			// just perform an insert statement
			StringBuilder queryBuilder = new StringBuilder();
			queryBuilder.append("INSERT INTO ").append(table.toUpperCase()).append(" (").append(conceptCol).append(", ")
					.append(foreignKeyCol).append(") VALUES ('").append(conceptVal).append("', '").append(foreignKeyCol)
					.append("')");
			try {
				engine.insertData(queryBuilder.toString());
			} catch (Exception e) {
	...
	private static void appendRDBMSRelationship(IDatabaseEngine engine, String table, String conceptCol,
			String conceptVal, String foreignKeyCol, String foreignKeyVal,
			Map<String, Map<String, String>> tableColTypesHash) {
		StringBuilder queryBuilder = new StringBuilder();
		// it exists, now need to find all unique values given the instance except the
		// foreign key and
		// append that to the table
		final String TEMP_EXTENSION = "____TEMP";
		...
		// create a new temp table that is the unique set of all columns for the
		// specific instance
		// excluding other foreign key values
		queryBuilder.append("CREATE TABLE ").append(table).append(TEMP_EXTENSION).append(" AS (SELECT DISTINCT ")
				.append(cols.toString()).append(" FROM ").append(table).append(" WHERE ").append(conceptCol)
				.append("='").append(conceptVal).append("')");
		try {
			engine.insertData(queryBuilder.toString());
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
		...
		queryBuilder.append("ALTER TABLE ").append(table).append(TEMP_EXTENSION).append(" ADD ").append(foreignKeyCol)
				.append(" ").append(foreignKeyType);
		try {
			engine.insertData(queryBuilder.toString());
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
		...
		removeDuplicates(engine, table + TEMP_EXTENSION, conceptCol);
		// add all the values from the temp table into the table we care about
		queryBuilder.append("INSERT INTO ").append(table).append("(").append(cols).append(", ").append(foreignKeyCol)
				.append(")").append(" SELECT * FROM ").append(table).append(TEMP_EXTENSION);
		try {
			engine.insertData(queryBuilder.toString());
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
	...
	private static void removeDuplicates(IDatabaseEngine engine, List<String> tablesToRemoveDuplicates,
			List<String> colsForTablesToRemoveDuplicates) {
	...
	private static void removeDuplicates(IDatabaseEngine engine, String tableName, String colName) {
		...
		// remove the duplicated
		StringBuilder queryBuilder = new StringBuilder();
		queryBuilder.append("CREATE TABLE ").append(tableName).append(TEMP_EXTENSION)
				.append(" AS (SELECT DISTINCT * FROM ").append(tableName).append(" WHERE ").append(colName)
				.append(" IS NOT NULL AND TRIM(").append(colName).append(") <> '' )");
		try {
			engine.insertData(queryBuilder.toString());
		} catch (Exception e) {
			classLogger.error(Constants.STACKTRACE, e);
		...
		queryBuilder.setLength(0);
		queryBuilder.append("ALTER TABLE ").append(tableName).append(TEMP_EXTENSION).append(" RENAME TO ")
				.append(tableName);
		try {
			engine.insertData(queryBuilder.toString());
		} catch (Exception e) {
	...
	private static String createInsertStatement(String tableName, String tableColumn, String tableValue,
			List<String> propNames, List<Object> propValues, List<String> types, final DateFormat DATE_DF,
			final DateFormat GENERIC_DF, final DateFormat SIMPLE_DATE_DF) {
		...
		if (propNames.size() > 0) {
			...
			for (int i = 0; i < propValues.size(); i++) {
				...
				if (type.contains("VARCHAR")) {
					...
					insertQuery.append("'");
				} else if (type.contains("INT") || type.contains("DECIMAL") || type.contains("DOUBLE")
						|| type.contains("LONG") || type.contains("BIGINT") || type.contains("TINYINT")
					...
					try {
						...
						classLogger.error(Constants.STACKTRACE, e);
						throw new IllegalArgumentException("Input value, " + propertyValue + " for column "
								+ propNames.get(i) + " cannot be parsed as a date.");
					}
	...
	private static String createUpdateStatement(String tableName, String tableColumn, String tableValue,
			List<String> propNames, List<Object> propValues, List<String> types, final DateFormat DATE_DF,
			final DateFormat GENERIC_DF, final DateFormat SIMPLE_DATE_DF) {
		if (propNames.size() == 0) {
			return "";
	...
	private static void addAuditLog(IDatabaseEngine formEng, String auditLogTableName, String user, String action,
			String startNode, String relName, String endNode, String propName, String propValue, String timeStamp) {
		...
		String queryString = "INSERT INTO " + auditLogTableName
				+ "(USER, ACTION, START_NODE, REL_NAME, END_NODE, PROP_NAME, PROP_VALUE, TIME) VALUES(?, ?, ?, ?, ?, ?, ?, ?)";
		try {
	...
	public static Map<String, Object> getAuditDataForEngine(String engineName) {
		...
		String auditLogTableName = AbstractSqlQueryUtil
				.escapeForSQLStatement(RDBMSEngineCreationHelper.cleanTableName(engineName)).toUpperCase()
				+ AUDIT_FORM_SUFFIX;
		IDatabaseEngine formEng = Utility.getDatabase(FORM_BUILDER_ENGINE_NAME);
