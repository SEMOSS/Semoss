...
public abstract class AbstractFormBuilder {
	...
	protected AbstractFormBuilder(IDatabaseEngine engine) {
		...
		this.engine = engine;
		this.auditLogTableName = AbstractSqlQueryUtil
				.escapeForSQLStatement(RDBMSEngineCreationHelper.cleanTableName(this.engine.getEngineId()))
				.toUpperCase() + FormBuilder.AUDIT_FORM_SUFFIX;
	...
	public void commitFormData(Map<String, Object> engineHash, String user) throws IOException {
		...
		saveFormData(baseURI, conceptBaseURI, relationBaseURI, propertyBaseURI, nodes, relationships, removeNodes,
				removeRelationships);
	...
	/**
	 * Utility method to generate a form user access table if it doesn't currently
	 * exist
	 *
	 * @param formEng
	 */
	...
	protected void generateEngineAuditLog(String auditLogTableName) {
		...
		if (!auditTableExists) {
			try (WriteOWLEngine owlEngine = formEng.getOWLEngineFactory().getWriteOWL()) {
				...
				createAuditTable.append(auditLogTableName)
						.append("(ID IDENTITY, USER VARCHAR(255), ACTION VARCHAR(100), START_NODE VARCHAR(255), "
			...
			// since there can be multiple forms with different tags for searching on the
			// same engine
			...
			try (WriteOWLEngine owlEngine = formEng.getOWLEngineFactory().getWriteOWL()) {
				...
				String allColsPresent = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '"
						+ auditLogTableName.toUpperCase() + "'";
				try {
				...
				if (colsToAdd.size() > 0) {
					...
					Connection conn = null;
					Statement stmt = null;
					String alterQuery = rdbmsEng.getQueryUtil().alterTableAddColumns(auditLogTableName,
							colsToAdd.toArray(new String[]{}), colsToAddTypes.toArray(new String[]{}));
	 ...
	 */
	protected abstract void saveFormData(String baseURI, String conceptBaseURI, String relationBaseURI,
	 ...
	 * @param timeStamp
	 */
	protected void addAuditLog(String action, String startNode, String relName, String endNode, String propName,
			String propValue, String timeStamp) {
		...
		try {
			ps = rdbmsEng.getPreparedStatement("INSERT INTO " + this.auditLogTableName
