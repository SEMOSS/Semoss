...
public abstract class AbstractFormBuilder {
  ...
  protected AbstractFormBuilder(IDatabaseEngine engine) {
    ...
    this.engine = engine;
    this.auditLogTableName =
        AbstractSqlQueryUtil.escapeForSQLStatement(
                    RDBMSEngineCreationHelper.cleanTableName(this.engine.getEngineId()))
                .toUpperCase()
            + FormBuilder.AUDIT_FORM_SUFFIX;
  ...
  public static void generateFormPermissionTable(IDatabaseEngine formEng) {
    ...
    String checkTableQuery =
        "SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME='FORMS_USER_ACCESS'";
    ...
    if (!permissionTableExists) {
      try (WriteOWLEngine owlEngine = formEng.getOWLEngineFactory().getWriteOWL()) {
        ...
        String query =
            "CREATE TABLE FORM_USER_ACCESS (USER_ID VARCHAR(100), INSTANCE_NAME VARCHAR(255), IS_SYS_ADMIN BOOLEAN)";
  ...
  protected void generateEngineAuditLog(String auditLogTableName) {
    ...
    if (!auditTableExists) {
      try (WriteOWLEngine owlEngine = formEng.getOWLEngineFactory().getWriteOWL()) {
        ...
        createAuditTable
            .append(auditLogTableName)
            .append(
                "(ID IDENTITY, USER VARCHAR(255), ACTION VARCHAR(100), START_NODE VARCHAR(255), "
          ...
          String alterQuery =
              rdbmsEng
                  ...
                  .alterTableAddColumns(
                      auditLogTableName,
                      colsToAdd.toArray(new String[] {}),
                      colsToAddTypes.toArray(new String[] {}));
   ...
   */
  public abstract void modifyInstanceValue(
      String origName, String newName, boolean deleteInstanceBoolean);
