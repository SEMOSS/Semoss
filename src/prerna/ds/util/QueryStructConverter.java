	...
	public static SelectQueryStruct convertOldQueryStruct(QueryStruct qs) {
		...
		for (String colname : oldFilters.keySet()) {
			...
			for (String comparator : compHash.keySet()) {
				...
				SimpleQueryFilter newFilter = new SimpleQueryFilter(new NounMetadata(colSelector, PixelDataType.COLUMN),
						comparator, new NounMetadata(compHash.get(comparator), type));
