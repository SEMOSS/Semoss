...
public final class TinkerAlgorithmUtility {
	...
	public static String runLoopIdentifer(TinkerFrame tf, int cycleSize) {
		GraphTraversal<Vertex, Path> traversal = tf.g.traversal().V().as("a").repeat(__.out().simplePath())
				.times(cycleSize).where(__.out().as("a")).path().dedup()
				.by(__.unfold().order().by(TinkerFrame.TINKER_ID, new Comparator<String>() {
	...
	public static String runDisconnectedNodesIdentifier(TinkerFrame tf, String type, String instance) {
		GraphTraversal<Vertex, Object> traversal = tf.g.traversal().V()
				.or(__.not(__.V().has(TinkerFrame.TINKER_ID, type + ":" + instance).repeat(__.out())
