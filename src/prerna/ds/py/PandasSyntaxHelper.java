...
public class PandasSyntaxHelper {
  ...
  private static final String NAN_VALUES =
      "[\"\",\"#N/A\",\"#N/A N/A\",\"#NA\",\"-1.#IND\",\"<NA>\",\"N/A\",\"NULL\""
   ...
   * @return
   */
  public static String getCsvFileRead(
      String pandasImportVar, String numpyImportVar, String fileLocation, String tableName) {
    return getCsvFileRead(
        pandasImportVar, numpyImportVar, fileLocation, tableName, null, null, null);
  }
   ...
   */
  public static String getCsvFileRead(
      String pandasImportVar,
      String numpyImportVar,
      String fileLocation,
      String tableName,
      String sep,
      String quoteChar,
    ...
    return getCsvFileRead(
        pandasImportVar, numpyImportVar, fileLocation, tableName, sep, quoteChar, escapeChar, null);
  }
  ...
  /**
   * Builds the script to read in a CSV file to a pandas data frame. Code preserves leading zeros of
   * integers. Returned data frame can have mixed columns of floats and ints, but if there is a
   * string present, the column type is returned as an object.
   *
   * @param pandasImportVar
   * @param numpyImportVar
   * @param fileLocation
   ...
   * @return
   */
  public static StringBuffer buildReadCsv(
      String pandasImportVar,
      String numpyImportVar,
      String fileLocation,
      String tableName,
    ...
    converter
        .append(numeric)
        .append(",")
        .append(numpyImportVar)
        .append(".integer) and x.strip()[0] == '0' and len(x.strip()) > 1) else ")
        .append(numeric)
    ...
    script
        .append(tableName)
        .append(" =")
        .append(pandasImportVar)
        .append(".read_csv('")
        .append(fileLocation.replaceAll("\\\\+", "/"))
        .append("',sep='" + sep + "',encoding='" + encoding + "',")
        .append(converter + ")")
        .append(replace)
        .append(".apply(lambda x: x.astype(str) if (any(x.map(type) == str)) else x)")
        .append(replace);
   ...
   * @return
   */
  private static StringBuffer buildReadCsv(
      String pandasImportVar,
      String numpyImportVar,
      String fileLocation,
      String tableName,
    ...
    for (String column : dataTypeMaps.keySet()) {
      ...
      if (dataType == SemossDataType.DATE || dataType == SemossDataType.TIMESTAMP) {
        ...
        dataMap
            ...
            .append(column)
            .append("':")
            .append(convertSemossDataType(numpyImportVar, dataType));
      }
    }
   ...
   * @return
   */
  public static String getJsonFileRead(
      String pandasImportVar,
      String numpyImportVar,
      String fileLocation,
      String tableName,
    ...
    if (dataTypeMaps == null || dataTypeMaps.isEmpty()) {
      ...
      script =
          buildReadJson(pandasImportVar, numpyImportVar, fileLocation, tableName, dataTypeMaps);
    }
    return script.toString();
  }
   ...
   * @return
   */
  public static StringBuffer buildReadJson(
      String pandasImportVar,
      String numpyImportVar,
      String fileLocation,
      String tableName,
    ...
    sb.append(tableName)
        ...
        .append(".read_json('")
        .append(fileLocation.replaceAll("\\\\+", "/"))
        .append("',dtype=" + dataMap.toString())
        .append(",convert_dates=")
        .append(dateList)
        .append(")");
   ...
   * @return
   */
  public static String getWriteCsvFile(
      String tableName, String fileLocation, String sep, String encoding) {
    if (encoding == null || encoding.isEmpty()) {
      encoding = "utf-8";
    }
    String readCsv =
  ...
  public static String loadExcelSheet(
    ...
    // add column range
    sb.append(
        "sheet_name='" + sheetName + "', usecols=range(" + (startCol - 1) + ", " + endCol + "), ");
    // add row range
  ...
  public static String getWritePandasToPickle(
      String pickleVarName, String tableName, String fileLocation) {
    return pickleVarName
        + ".dump("
        + tableName
        + ", open(\""
        + fileLocation.replaceAll("\\\\+", "/")
  ...
  public static String getReadPickleToPandas(
      String pandasImportVar, String fileLocation, String tableName) {
    return tableName
        + " = "
        + pandasImportVar
        + ".read_pickle(\""
   ...
   * @return
   */
  public static String getParquetFileRead(
      String pandasImportVar, String numpyImportVar, String fileLocation, String tableName) {
    StringBuffer script = new StringBuffer();
    script
        .append(tableName)
  ...
  /**
   * Merges two dataframes. if nonEqui is true, implicitly performs a join by taking the cross
   * product.
   *
   * @param leftTableName
  ...
  public static String getMergeFilterSyntax(
      String tableName, List<Map<String, String>> joinCols, List<String> joinComparators) {
    StringBuffer builder = new StringBuffer();
  ...
  /**
   * Get the correct py syntax for the table join columns This uses the join information keys since
  ...
  public static void getMergeColsSyntax(
      StringBuffer builder, List<Map<String, String>> colNames, boolean grabKeys) {
  ...
  public static String removeDuplicateColumns(String tableName, String newTable) {
    String script =
        newTable + " = " + tableName + ".loc[:,~" + tableName + ".columns.duplicated()]";
    return script;
  }
   ...
   * @return
   */
  public static String filterRowBySlice(
      String tableName, String newTableName, Object indexEnd, Object indexStart) {
    ...
    try {
      ...
      String script =
          newTableName + " = " + tableName + ".iloc[" + indexStart + ":" + indexEnd + "]";
      return script;
      ...
      String script =
          newTableName + " = " + tableName + ".loc['" + indexStart + "':'" + indexEnd + "']";
      return script;
    }
  }
  ...
  /**
   * Filter pandas data frame by specified value and operator. Handles both numeric and object value
   * types.
   *
   * @param tableName
   * @param newTableName
   ...
   * @return
   */
  public static String filterBySingleValue(
      String tableName, String newTableName, String columnName, String operator, Object value) {
    try {
      ...
      if (!OPERATOR_LIST.contains(operator.trim())) {
        throw new IllegalArgumentException(
            "Operator " + operator.trim() + " is not a valid operator.");
      }
   ...
   * @return
   */
  public static String filterByMultipleValues(
      String tableName,
      String newTableName,
      String columnName,
      List<String> values,
  ...
  /**
   * Filters a data frame by whether a specified column contains an expression. Requirements are
   * that columnType is object.
   *
   * @param tableName
   * @param newTableName
   * @param columnName
   ...
   * @return
   */
  public static String filterByExpression(
      String tableName,
      String newTableName,
      String columnName,
      Object expression,
   ...
   * @return
   */
  public static String filterBetweenNumbers(
      String tableName,
      String newTableName,
      String columnName,
      Object floor,
