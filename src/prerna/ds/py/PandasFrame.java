...
public class PandasFrame extends AbstractTableDataFrame {
   ...
   * @param dataTypeMap
   */
  public void addRowsViaIterator(
      Iterator<IHeadersDataRow> it, String tableName, Map<String, SemossDataType> dataTypeMap) {
    ...
    if (!loaded) {
      ...
      String loadS =
          PandasSyntaxHelper.getJsonFileRead(
              PANDAS_IMPORT_VAR, NUMPY_IMPORT_VAR, fileLocation, tableName, dataTypeMap);
      ...
      String makeWrapper =
          PandasSyntaxHelper.makeWrapper(
              PandasSyntaxHelper.createFrameWrapperName(tableName), tableName);
      // execute the script
  ...
  private void addRowsViaCsvIterator(CsvFileIterator it, String tableName) {
    ...
    String selectedColumns =
        PandasSyntaxHelper.filterByColumn(tableName, tableName, Arrays.asList(cleanNewHeaders));
  ...
  private void addRowsViaExcelIterator(ExcelSheetFileIterator it, String tableName) {
    ...
    String loadS =
        PandasSyntaxHelper.loadExcelSheet(
            PANDAS_IMPORT_VAR, filePath, tableName, sheetName, sheetRange);
    long limit = qs.getLimit();
    if (limit > -1) {
      String rowLimits = String.valueOf(limit);
      loadS = loadS + "[:" + rowLimits + "]";
  ...
  private void addRowsViaParquetIterator(ParquetFileIterator it, String tableName) {
    ...
    String fileLocation = it.getFileLocation();
    String loadS =
        PandasSyntaxHelper.getParquetFileRead(
            PANDAS_IMPORT_VAR, NUMPY_IMPORT_VAR, fileLocation, tableName);
    // apply limit for import
    long limit = qs.getLimit();
    if (limit > -1) {
      String rowLimits = String.valueOf(limit);
  ...
  /**
   * Merge the pandas frame with another frame. If a non equi join, performs a cross product and
   * then filters the results. For the non equi joins, if the left and right join column names are
   * equal, changes the right column name so that it can be dropped later.
   *
   * @param returnTable
   * @param leftTableName
  ...
  public void merge(
      ...
      boolean nonEqui) {
    String mergeString =
        PandasSyntaxHelper.getMergeSyntax(
            PANDAS_IMPORT_VAR,
            returnTable,
            leftTableName,
            rightTableName,
    ...
    if (!nonEqui) {
      ...
      for (int i = 0; i < joinCols.size(); i++) {
        ...
        for (String lColumn : joinMap.keySet()) {
          if (lColumn.equals(joinMap.get(lColumn))) {
            ...
            pyTranslator.runScript(
                PandasSyntaxHelper.alterColumnName(
                    rightTableName, joinMap.get(lColumn), newColumn));
      ...
      String filterSyntax =
          PandasSyntaxHelper.getMergeFilterSyntax(returnTable, joinCols, joinComparators);
      pyTranslator.runScript(mergeString);
  ...
  private void adjustDataTypes(String tableName, Map<String, SemossDataType> dataTypeMap) {
    ...
    for (int colIndex = 0; colIndex < headers.length; colIndex++) {
      ...
      if (proposedType != null
          && pyproposedType != null
          && !pyproposedType.equalsIgnoreCase(colType)) {
  ...
  // tries to see if the order in which pandas is giving is valid with the order that is being
  // requested
  ...
  private IRawSelectWrapper processInterpreter(PandasInterpreter interp, SelectQueryStruct qs) {
    ...
    // ascszng =
    // d[['bp.1d','bp.1s','bp.2d','bp.2s','chol','Drug','frame','gender','glyhb','hdl','height','hip','id','location','ratio','stab.glu','time.ppn','waist','weight']]
  ...
  /**
   * Run the script By default return the first script passed in use the Executor to grab the
   * specific code portion if running multiple
   *
   * @param script
   * @return
   */
  ...
  public void close() {
    ...
    if (sqliteConnectionName != null) pyTranslator.runScript("del " + sqliteConnectionName);
  ...
  public void open(CachePropFileFrameObject cf, Cipher cipher) {
    ...
    String[] commands =
        new String[] {
          PANDAS_IMPORT_STRING,
          "import pickle",
          "import smssutil",
          PandasSyntaxHelper.getReadPickleToPandas(
              PANDAS_IMPORT_VAR, cf.getFrameCacheLocation(), this.frameName),
  ...
  public Object querySQL(String sql) {
    ...
    if (sql.trim().toUpperCase().startsWith("SELECT")) {
      ...
      String deleteAll =
          "del " + tempFrameName + ", " + tempFrameName + "_types, " + tempFrameName + "_dict";
  ...
  public Object queryCSV(String sql) {
    ...
    if (sql.toUpperCase().startsWith("SELECT")) {
      ...
      try {
        ...
        if (fileName.exists()) {
          // retObject = new String(Files.readAllBytes(fileName.toPath())); // get the dictionary
          // back
  ...
  public String getSQLite() {
    ...
    if (sqliteConnectionName == null) {
      ...
      commands[1] =
          sqliteConnectionName + " = sqlite3.connect(':memory:', check_same_thread=False)";
      commands[2] =
          frameName
              + ".to_sql('"
              + frameName
  ...
  private String getSQLFromNLP(String query) {
    ...
    for (int columnIndex = 0; columnIndex < columns.length; columnIndex++) {
      if (columnIndex == 0) finalDbString.append(columns[columnIndex]);
      else finalDbString.append(" , ").append(columns[columnIndex]);
    }
    ...
    Object output =
        this.pyTranslator.runScript("smssutil.run_gpt_3(\"" + finalDbString + "\", " + 150 + ")");
    // get the string
    // make a frame
  ...
  //	private List<String> checkColumn(String tableName, String colName, SemossDataType curType,
  // SemossDataType newType, List <String> colChanger) {
  //		if(curType == newType) {
  //			return colChanger;
  //		} else {
  //			// get the pytype
