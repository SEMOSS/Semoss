...
public class PandasFrame extends AbstractTableDataFrame {
	...
	private void addRowsViaCsvIterator(CsvFileIterator it, String tableName) {
		...
		String loadS = PandasSyntaxHelper.getCsvFileRead(PANDAS_IMPORT_VAR, NUMPY_IMPORT_VAR, fileLocation, tableName,
				qs.getDelimiter() + "", "\"", "\\\\", null, qs.getColumnTypes(), limit);
		...
		String selectedColumns = PandasSyntaxHelper.filterByColumn(tableName, tableName,
				Arrays.asList(cleanNewHeaders));
	...
	private void addRowsViaParquetIterator(ParquetFileIterator it, String tableName) {
		...
		String importNumpyS = new StringBuilder(NUMPY_IMPORT_STRING).toString();
		String fileLocation = it.getFileLocation();
		String loadS = PandasSyntaxHelper.getParquetFileRead(PANDAS_IMPORT_VAR, NUMPY_IMPORT_VAR, fileLocation,
				tableName);
		// apply limit for import
		long limit = qs.getLimit();
		if (limit > -1) {
	...
	/**
	 * Merge the pandas frame with another frame. If a non equi join, performs a
	 * cross product and then filters the results. For the non equi joins, if the
	 * left and right join column names are equal, changes the right column name so
	 * that it can be dropped later.
	 *
	 * @param returnTable
	 * @param leftTableName
	 ...
	 * @param joinCols
	 */
	public void merge(String returnTable, String leftTableName, String rightTableName, String joinType,
			List<Map<String, String>> joinCols, List<String> joinComparators, boolean nonEqui) {
		String mergeString = PandasSyntaxHelper.getMergeSyntax(PANDAS_IMPORT_VAR, returnTable, leftTableName,
				rightTableName, joinType, joinCols, nonEqui);
				...
				for (String lColumn : joinMap.keySet()) {
					if (lColumn.equals(joinMap.get(lColumn))) {
						...
						pyTranslator.runScript(
								PandasSyntaxHelper.alterColumnName(rightTableName, joinMap.get(lColumn), newColumn));
	...
	// tries to see if the order in which pandas is giving is valid with the order
	// that is being
	// requested
	...
	public IRawSelectWrapper query(SelectQueryStruct qs) {
		...
		// at this point try to see if the cache already has it and if so pass that
		// iterator instead
	...
	private IRawSelectWrapper processInterpreter(PandasInterpreter interp, SelectQueryStruct qs) {
		...
		// the command that is coming in has the sort values and other things attached
		// to it
	...
	public boolean isEmpty(String tableName) {
		...
		String command = "( ('" + wrapperName + "' in vars() or '" + wrapperName + "' in globals()) and len("
				+ wrapperName + ".cache['data'])>=0 )";
  ...
  ///////////////////////////////////////////////////////////////////////////////
	...
	public String getSQLite() {
		...
		if (sqliteConnectionName == null) {
			...
			commands[2] = frameName + ".to_sql('" + frameName + "', " + sqliteConnectionName
  ...
  ///////////////////////////////////////////////////////////////////////////////
	...
	// private void alignColumns(String tableName, Map<String, SemossDataType>
	// dataTypeMap) {
	...
	//
	// colChanger = checkColumn(tableName, colName, pysColType, sColType,
	// colChanger);
	// }
	//
	...
	// private List<String> checkColumn(String tableName, String colName,
	...
	//
	// // column change
	// String script = PandasSyntaxHelper.getColumnChange(tableName, colName,
	// asType);
