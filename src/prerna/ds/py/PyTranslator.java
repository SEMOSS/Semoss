...
public class PyTranslator {
   ...
   * This does not append any variables (ROOT, APP_ROOT, USER_ROOT) with the execution
   *
   * @param executionInsight If we have a User invoking an engine python process The engine python
   *     process has its own unique insight for variable encapsulation However, we need to know from
   *     what insight is the user invoking this request So that if the engine is making a call
   *     back/reactor request It knows which User invoked for security permissions
   * @param script
   * @return
   */
  ...
  private String[] getDefaultPaths(Insight insight) {
    ...
    if (insight.getContextProjectId() != null) {
      appPath =
          AssetUtility.getProjectAssetsFolder(
              insight.getContextProjectName(), insight.getContextProjectId());
