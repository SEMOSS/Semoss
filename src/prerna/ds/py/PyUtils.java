...
public class PyUtils {
  ...
  public static String constructPyDictFromMap(Map<String, Object> theMap) {
    ...
    for (Entry<String, Object> entry : theMap.entrySet()) {
      ...
      theDict
          .append(determineStringType(entry.getKey()))
          .append(":")
          .append(determineStringType(entry.getValue()));
    ...
    if (SystemUtils.IS_OS_WINDOWS) {
      ...
      if (libDir.exists() && libDir.isDirectory()) {
        ...
        if (libSubDirs != null && libSubDirs.length > 0) {
          ...
          File pythonVersionDir =
              Arrays.stream(libSubDirs)
  ...
  public static String[] applyUlimit(String[] commands) {
    ...
    if (!SystemUtils.IS_OS_WINDOWS
      ...
      commands =
          new String[] {"/bin/bash", "-c", "\"ulimit -v " + ulimit + " && " + sb.toString() + "\""};
    }
    return commands;
  }
}
