...
public class QueryStruct {
  ...
  // Hashtable <String, Hashtable<String, Vector>> orfilters = new Hashtable<String,
  ...
  public static final int COUNT_CELLS =
      1; // use this when we want to count the number of total rows*selectors in the query
  public static final int COUNT_DISTINCT_SELECTORS =
      2; // use this when we want to count the number of distinct values for a selector in a query
  ...
  public void addSelector(String concept, String property) {
    if (property == null) property = PRIM_KEY_PLACEHOLDER;
  ...
  public void addFilter(String fromCol, String comparator, List filterData) {
    ...
    if (andfilters.containsKey(fromCol)) compHash = andfilters.get(fromCol);

    List curData = new Vector();
    // next piece is to see if we have the comparator
    if (compHash.containsKey(comparator)) curData = compHash.get(comparator);
  ...
  public void addRelation(String fromConcept, String toConcept, String comparator) {
    ...
    Map<String, List> compHash = new Hashtable<String, List>();
    if (relations.containsKey(fromConcept)) compHash = relations.get(fromConcept);
  ...
  private void addToHash(String concept, String property, Map<String, List<String>> hash) {
    ...
    if (hash.containsKey(concept)) propList = hash.get(concept);
   ...
   *
   * <p>Will return like this: Title --> [Title__Budget, Studio] Studio --> [StudioOwner] etc.
   *
   * @return
   */
  public Map<String, Set<String>> getReturnConnectionsHash() {
    ...
    for (String selectorKey : this.selectors.keySet()) {
      ...
      // if the props doesn't contain a prim_key_placeholder... then it is actually just a property
      // and not a concept
      if (!props.contains(PRIM_KEY_PLACEHOLDER)) {
        ...
        for (String prop : props) {
          ...
          // mergeQSEdgeHash needs this to be the concept__property... plus need to keep it
  ...
  /**
   * Logic to process the relationship This takes into consideration intermediary nodes that should
   * not be added to the return hash e.g. i have concepts a -> b -> c -> d but I only want to return
   * a-> d
   *
  ...
  private void processRelationship(
      String startNode, Map<String, List> relMap, Map<String, Set<String>> edgeHash) {
  ...
  /**
   * Recursive method to find the shortest path to all the nearest concepts that are being returned
   * as selectors
   *
   * @param endNode The endNode that is node a selector which we are trying to find the shortest
   *     path to
   * @param edgeHash The edgeHash to find the current selectors
   * @param newEndNodeList The list of endNodes that have been found using the logic to find the
   *     shortest path for connected nodes
   */
  private void determineShortestEndNodePath(
      String endNode, Map<String, Set<String>> edgeHash, List<String> newEndNodeList) {
    // this endNode is a node which is not a selector
    // need to find the shortest path to nodes which this endNode is connected to which is also a
    // selector
    ...
    if (this.relations.containsKey(endNode)) {
      ...
      for (List<String> endNodeList : connections) {
        for (String possibleNewEndNode : endNodeList) {
          // if this connection is a selector (i.e. key in the edgeHash), then we need to add it to
          // the newEndNodeList
  ...
  public QueryStruct deepCopy() {
    ...
    copy.selectors =
        gson.fromJson(
            stringified_selectors, new TypeToken<Hashtable<String, Vector<String>>>() {}.getType());

    String stringified_relations = gson.toJson(this.relations);
    copy.relations =
        gson.fromJson(
            stringified_relations,
            new TypeToken<Hashtable<String, Hashtable<String, Vector>>>() {}.getType());
