...
public class QueryStruct {
	...
	// so it would go as
	// | Movie Budget | > | Vector(200) | // or this could be a whole object /
	// vector
	// | < | Vector(300) |
	public Map<String, Map<String, List>> andfilters = new Hashtable<String, Map<String, List>>();

	// Hashtable <String, Hashtable<String, Vector>> orfilters = new
	// Hashtable<String,
	...
	public static final int NO_COUNT = 0; // use this when we don't want to do a count
	public static final int COUNT_CELLS = 1; // use this when we want to count the number of total rows*selectors in the
												// query
	public static final int COUNT_DISTINCT_SELECTORS = 2; // use this when we want to count the number of distinct
															// values for a selector in a query
	...
	public void addRelation(String fromConcept, String toConcept, String comparator) {
		// I need pick the keys from the table based on relationship and then add that
		// to the relation
	...
	public Map<String, Set<String>> getReturnConnectionsHash() {
		...
		for (String selectorKey : this.selectors.keySet()) {
			...
			// if the props doesn't contain a prim_key_placeholder... then it is actually
			// just a property
			// and not a concept
			if (!props.contains(PRIM_KEY_PLACEHOLDER)) {
				...
				for (String prop : props) {
					...
					// mergeQSEdgeHash needs this to be the concept__property... plus need to keep
					// it
		...
		if (this.relations != null) {
			...
			for (String startNode : this.relations.keySet()) {
				// the relMap contains the joinType pointing to a list of columns to be joined
				// to
	...
	/**
	 * Logic to process the relationship This takes into consideration intermediary
	 * nodes that should not be added to the return hash e.g. i have concepts a -> b
	 * -> c -> d but I only want to return a-> d
	 *
	 * @param startNode
	 *            The startNode of the relationship
	 * @param relMap
	 *            The relationships being observed for the startNode
	 * @param edgeHash
	 *            The existing edge hash to determine what the current selectors are
	 */
	...
	/**
	 * Recursive method to find the shortest path to all the nearest concepts that
	 * are being returned as selectors
	 *
	 * @param endNode
	 *            The endNode that is node a selector which we are trying to find
	 *            the shortest path to
	 * @param edgeHash
	 *            The edgeHash to find the current selectors
	 * @param newEndNodeList
	 *            The list of endNodes that have been found using the logic to find
	 *            the shortest path for connected nodes
	 */
	private void determineShortestEndNodePath(String endNode, Map<String, Set<String>> edgeHash,
			List<String> newEndNodeList) {
		// this endNode is a node which is not a selector
		// need to find the shortest path to nodes which this endNode is connected to
		// which is also a
		// selector
		...
		if (this.relations.containsKey(endNode)) {
			...
			for (List<String> endNodeList : connections) {
				for (String possibleNewEndNode : endNodeList) {
					// if this connection is a selector (i.e. key in the edgeHash), then we need to
					// add it to
					// the newEndNodeList
					if (edgeHash.containsKey(possibleNewEndNode)) {
						...
						if (possibleNewEndNode.contains("__")) {
							...
							if (edgeHash.containsKey(concept)) {
								...
								// then we need to recursively go down the path and see if it has a
								// possibleNewEndNode
	...
	public QueryStruct deepCopy() {
		...
		copy.selectors = gson.fromJson(stringified_selectors, new TypeToken<Hashtable<String, Vector<String>>>() {
		}.getType());

		String stringified_relations = gson.toJson(this.relations);
		copy.relations = gson.fromJson(stringified_relations,
				new TypeToken<Hashtable<String, Hashtable<String, Vector>>>() {
				}.getType());

		String stringified_filters = gson.toJson(this.andfilters);
		copy.andfilters = gson.fromJson(stringified_filters,
				new TypeToken<Hashtable<String, Hashtable<String, Vector>>>() {
				}.getType());
	...
	// {
	// // test code for getting proper edge hash when there are intermediary nodes
	// that
	// // i.e. the query requires a specific node that you do not want in your
	// selectors
  ...
  ////		System.out.println(query);
	...
	// String fileName =
	// "C:\\Users\\bisutton\\workspace\\SEMOSSDev\\db\\UpdatedRDBMSMovies.smss";
