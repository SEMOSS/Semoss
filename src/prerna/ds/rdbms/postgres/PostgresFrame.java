...
public class PostgresFrame extends AbstractRdbmsFrame {
  ...
  protected void initConnAndBuilder() throws Exception {
    ...
    try (InputStream input =
        new FileInputStream(Utility.getBaseFolder() + DIR_SEPARATOR + CONFIGURATION_FILE)) {
      ...
      this.conn =
          AbstractSqlQueryUtil.makeConnection(
              RdbmsTypeEnum.POSTGRES, connectionUrl, username, password);
  ...
  // FileInputStream(DIHelper.getInstance().getProperty(Constants.BASE_FOLDER) + DIR_SEPARATOR +
  // CONFIGURATION_FILE)) {
  //			Properties prop = new Properties();
  //
  //			prop.load(input);
