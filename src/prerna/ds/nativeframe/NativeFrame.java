...
public class NativeFrame extends AbstractTableDataFrame {
  ...
  public IRawSelectWrapper query(String query) throws Exception {
    ...
    IRawSelectWrapper it =
        WrapperManager.getInstance().getRawWrapper(this.queryQs.retrieveQueryStructEngine(), query);
  ...
  public IRawSelectWrapper query(SelectQueryStruct qs) throws Exception {
    ...
    IRawSelectWrapper it = null;
    if (cache) {
      if (NativeFrame.cacheEngines.contains(
          this.queryQs.retrieveQueryStructEngine().getDatabaseType())) {
        // this is an engine whose results can be cached
        IQueryInterpreter interpreter =
            this.queryQs.retrieveQueryStructEngine().getQueryInterpreter();
  ...
  public boolean engineQueryCacheable() {
    return NativeFrame.cacheEngines.contains(
        this.queryQs.retrieveQueryStructEngine().getDatabaseType());
  }
        ...
        writer =
            new BufferedWriter(
                new OutputStreamWriter(
                    new CipherOutputStream(new FileOutputStream(frameFile), cipher)));
      } else {
        ...
        reader =
            new BufferedReader(
                new InputStreamReader(
                    new CipherInputStream(new FileInputStream(frameFile), cipher)));
      } else {
