...
public class OwlTemporalEngineMeta {
  ...
  private static final String SEMOSS_PROPERTY_PREFIX =
  ...
  private static final String IS_PRIM_KEY_PRED =
      "http://semoss.org/ontologies/Relation/Contains/IsPrimKey";
  private static final String IS_DERIVED_PRED =
      "http://semoss.org/ontologies/Relation/Contains/IsDerived";
  private static final String QUERY_STRUCT_PRED =
      "http://semoss.org/ontologies/Relation/Contains/QueryStructName";
  private static final String ALIAS_PRED = "http://semoss.org/ontologies/Relation/Contains/Alias";
  private static final String ORDERING_PRED =
      "http://semoss.org/ontologies/Relation/Contains/Ordering";
  private static final String ADDTL_DATATYPE_PRED =
      "http://semoss.org/ontologies/Relation/Contains/AddtlDataType";

  // if opaque selector, need to store it
  private static final String QUERY_SELECTOR_COMPLEX_PRED =
      "http://semoss.org/ontologies/Relation/Contains/IsComplex";
  private static final String QUERY_SELECTOR_TYPE_PRED =
      "http://semoss.org/ontologies/Relation/Contains/QuerySelectorType";
  private static final String QUERY_SELECTOR_AS_STRING_PRED =
      "http://semoss.org/ontologies/Relation/Contains/QuerySelector";

  // specific for tinker
  private static final String PHYSICAL_PRED =
      "http://semoss.org/ontologies/Relation/Contains/Physical";
    ...
    try {
      ...
      if (cipher != null) {
        try (InputStreamReader is =
            new InputStreamReader(new CipherInputStream(new FileInputStream(file), cipher))) {
  ...
  /**
   * Currently, this is only used by tinker when on the meta we create a System_2 node but the
   * actual tinker vertices are type System
   *
   * @param vertexName
   * @param physical
   */
  ...
  /**
   * Need to validate if a unique name for a table or property is valid Return of true means it is
   * valid Return of false means it is not valid
   *
   * @param uniqueName
   * @return
   */
  ...
  public Map<String, String> getUniqueNameToAlias() {
    String query =
        "select distinct ?header ?alias where {"
            + "{"
            + "{?header <"
            + RDFS.SUBCLASSOF
            + "> <"
  ...
  public Object[] getComplexSelector(String uniqueName) {
    String query =
        "select distinct "
            ...
            + "> as ?header)"
            + "{?header <"
            + QUERY_SELECTOR_COMPLEX_PRED
            + "> \"true\"^^xsd:boolean}"
            + "{?header <"
            + QUERY_SELECTOR_TYPE_PRED
            + "> ?queryType}"
            + "{?header <"
            + QUERY_SELECTOR_AS_STRING_PRED
            + "> ?queryJson}"
            + "} UNION {"
            + "bind(<"
            + SEMOSS_PROPERTY_PREFIX
  ...
  public List<String[]> getAllRelationships() {
    String query =
        "select ?fromNode ?toNode ?rel where {"
            + "{?fromNode <"
            + RDFS.SUBCLASSOF
            + "> <"
            + SEMOSS_CONCEPT_PREFIX
    ...
    try {
      ...
      while (it.hasNext()) {
        Object[] row = it.next().getValues();
        relationships.add(
            new String[] {row[0].toString(), row[1].toString(), row[2].toString().split(":")[2]});
      }
    } catch (Exception e) {
      classLogger.error(Constants.STACKTRACE, e);
    } finally {
  ...
  /**
   * Get all upstream relationships to the node This means the input node is the target
   *
   * @param node
   * @return
   */
  ...
  /**
   * Get all downstream relationships to the node This means the input node is the source
   *
   * @param node
   * @return
   */
  ...
  public Map<String, SemossDataType> getHeaderToTypeMap() {
    String query =
        "select distinct ?header ?datatype where {"
            + "{"
            + "{?header <"
            + RDFS.SUBCLASSOF
            + "> <"
  ...
  public Map<String, String> getHeaderToAdtlTypeMap() {
    String query =
        "select distinct ?header ?adtlDataType where {"
            + "{"
            + "{?header <"
            + RDFS.SUBCLASSOF
            + "> <"
  ...
  public Map<String, List<String>> getHeaderToSources() {
    String query =
        "select distinct ?header ?qsInfo where {"
            + "{"
            + "{?header <"
            + RDFS.SUBCLASSOF
            + "> <"
  ...
  public String getOrderingAsString(String uniqueName, String parentUniqueName) {
    // we have a property
    String query =
        "select distinct ?header ?orderedLevels where {"
            + "bind(<"
            + SEMOSS_PROPERTY_PREFIX
            + "/"
            + uniqueName
      ...
      for (int i = 0; i < dataTypes.length; i++) {
        ...
        if (dataTypes[i].toUpperCase().equals("NUMBER")) {
          ...
          if (type == SemossDataType.STRING) {
            // add factor as well
            filter.append(" || ");
            filter.append("?dt = \"").append(SemossDataType.FACTOR.toString()).append("\"");
          }
        }
      }
      ...
      while (it.hasNext()) {
        ...
        if (!append) {
          ...
          for (int i = 0; i < numReturns; i++) {
            ...
            if (headers[i].equals("qsName") && !values[i].toString().equals("unknown")) {
              Map<String, List<String>> engineQsMap =
                  (Map<String, List<String>>) rowMap.get("qsName");
              String[] split = values[i].toString().split(":::");
  ...
  public Map<String, Object> getComplexMetamodel() {
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        Map<String, Map<String, Object>> propertiesMap =
            (Map<String, Map<String, Object>>) node.get("properties");
    ...
    for (String key : nodesMap.keySet()) {
      Map<String, Map<String, Object>> properties =
          (Map<String, Map<String, Object>>) nodesMap.get(key).get("properties");
  ...
  /**
   * Get the metamodel view of the data The payload should match: {@link
   * prerna.nameserver.utility.MasterDatabaseUtility#getMetamodelRDBMS(String, boolean)}
   *
   * @return
   */
  ...
  /**
   * Get a list of arrays containing [table, column, type] for a given database The payload should
   * match: {@link prerna.nameserver.utility.MasterDatabaseUtility#getAllTablesAndColumns(String)}
   *
   * @param engineId
   * @return
   */
  public List<Object[]> getAllTablesAndColumns() {
    ...
    String query =
        "select distinct ?header ?datatype (lcase(?header) as ?loweralias) "
            + "where {"
            + "{"
            + "{?header <"
            + RDFS.SUBCLASSOF
  ...
  public void modifyAdditionalDataTypeToProperty(
      String uniqueName, String parent, String newAdditionalDataType) {
    // need to get the current value
  ...
  public void modifyPropertyName(String propertyName, String tableName, String newPropertyName) {
    ...
    for (Object[] headerPropRow : propertyInfo) {
      ...
      this.myEng.removeStatement(
          new Object[] {tableUri, SEMOSS_PROPERTY_PREFIX, propertyUri, true});
      this.myEng.removeStatement(
          new Object[] {propertyUri, RDF.TYPE, SEMOSS_PROPERTY_PREFIX, true});

      this.myEng.addStatement(
          new Object[] {tableUri, SEMOSS_PROPERTY_PREFIX, newPropertyUri, true});
      this.myEng.addStatement(
          new Object[] {newPropertyUri, RDF.TYPE, SEMOSS_PROPERTY_PREFIX, true});

      if (!propertyIsPrimKey.equals(EMPTY_VALUE)) {
        this.myEng.removeStatement(
            new Object[] {propertyUri, IS_PRIM_KEY_PRED, propertyIsPrimKey, false});
        this.myEng.addStatement(
            new Object[] {newPropertyUri, IS_PRIM_KEY_PRED, propertyIsPrimKey, false});
      }
      if (!propertyDataType.equals(EMPTY_VALUE)) {
        this.myEng.removeStatement(
            new Object[] {propertyUri, OWL.DATATYPEPROPERTY, propertyDataType, false});
        this.myEng.addStatement(
            new Object[] {newPropertyUri, OWL.DATATYPEPROPERTY, propertyDataType, false});
      }
      if (!propertyQsInfo.equals(EMPTY_VALUE)) {
        this.myEng.removeStatement(
            new Object[] {propertyUri, QUERY_STRUCT_PRED, propertyQsInfo, false});
        this.myEng.addStatement(
            new Object[] {newPropertyUri, QUERY_STRUCT_PRED, propertyQsInfo, false});
      }
      ...
      this.myEng.addStatement(
          new Object[] {newPropertyUri, ALIAS_PRED, newPropertyName.split("__")[1], false});
      if (!propertyDerived.equals(EMPTY_VALUE)) {
        this.myEng.removeStatement(
            new Object[] {propertyUri, IS_DERIVED_PRED, propertyDerived, false});
        this.myEng.addStatement(
            new Object[] {newPropertyUri, IS_DERIVED_PRED, propertyDerived, false});
      }
    }
  }
  ...
  public void modifyVertexName(String oldTableName, String newTableName) {
    ...
    for (Object[] headerRow : headerInfo) {
      ...
      this.myEng.removeStatement(
          new Object[] {tableUri, RDFS.SUBCLASSOF, SEMOSS_CONCEPT_PREFIX, true});
      this.myEng.addStatement(
          new Object[] {newTableUri, RDFS.SUBCLASSOF, SEMOSS_CONCEPT_PREFIX, true});
    ...
    for (Object[] headerPropRow : headerProperties) {
      ...
      String newPropertyUri =
          SEMOSS_PROPERTY_PREFIX
              ...
              + newTableName
              + "__"
              + headerPropRow[1].toString().split("__")[1];
      ...
      this.myEng.addStatement(
          new Object[] {newTableUri, SEMOSS_PROPERTY_PREFIX, newPropertyUri, true});
      this.myEng.addStatement(
          new Object[] {newPropertyUri, RDF.TYPE, SEMOSS_PROPERTY_PREFIX, true});
    ...
    for (Object[] upRel : upstreamRels) {
      ...
      this.myEng.removeStatement(
          new Object[] {relUri, RDFS.SUBPROPERTYOF, SEMOSS_RELATION_PREFIX, true});
      this.myEng.addStatement(
          new Object[] {newRelUri, RDFS.SUBPROPERTYOF, SEMOSS_RELATION_PREFIX, true});
  ...
  public OwlTemporalEngineMeta copy() {
    ...
    try {
      ...
      while (it.hasNext()) {
        ...
        if (predUri.equals(IS_PRIM_KEY_PRED)
            || predUri.equals(IS_DERIVED_PRED)
            || predUri.equals(QUERY_STRUCT_PRED)
            || predUri.equals(ALIAS_PRED)
            || predUri.equals(OWL.DATATYPEPROPERTY.toString())
            || predUri.equals(ADDTL_DATATYPE_PRED)) {
