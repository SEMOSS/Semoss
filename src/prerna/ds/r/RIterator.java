	...
	private void init() {
		...
		if (this.qs != null && this.numRows > 0) {
			...
			if (offset > numRows) {
				...
				try {
					updatedTempVarQuery = RSyntaxHelper.determineLimitOffsetSyntax(this.tempVarName, this.numRows,
							limit, offset);
					...
					updatedTempVarQuery = RSyntaxHelper.determineLimitOffsetSyntax(this.tempVarName, this.numRows, 1,
							0);
					this.builder.evalR(updatedTempVarQuery);
					// and then update the number of rows
					this.numRows = 0;
