...
public class RIterator implements Iterator<IHeadersDataRow> {
			...
			if (!init) {
				...
				if (this.qs != null && this.numRows > 0) {
					...
					if (offset > numRows) {
						...
						try {
							updatedTempVarQuery = RSyntaxHelper.determineLimitOffsetSyntax(this.tempVarName,
									this.numRows, limit, offset);
				...
				this.colTypes = builder
						.getColumnTypes(tempVarName + "[," + RSyntaxHelper.createStringRColVec(headers) + "]");
	...
	public IHeadersDataRow next() {
		...
		// if the data is null or the data position has reached the total size of the
		// data
	...
	public String getJsonOfResults() {
		...
		// write(toJSON(dt, dataframe=c("values"), pretty=TRUE),
		// "c:/workspace/testing.json")
		return this.builder.getRJavaTranslator()
				.getString("jsonlite:::toJSON( " + this.tempVarName + "[," + RSyntaxHelper.createStringRColVec(headers)
