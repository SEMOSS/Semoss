...
public class RSyntaxHelper {
  ...
  static {
    ...
    //		javaRDatTimeTranslationMap.put("z", "%Z"); 		//tz (Pacific Standard Time; PST; GMT-08:00)
   ...
   * @return String containing the equivalent r column vector
   */
  public static String createRColVec(
      List<Object> row, SemossDataType dataType, String additionalParameter) {
    ...
    for (; i < size; i++) {
      if (SemossDataType.STRING == dataType || SemossDataType.FACTOR == dataType) {
        ...
        if (additionalParameter != null && !additionalParameter.isEmpty()) {
          str.append("as.POSIXct(\"")
              .append(row.get(i).toString())
  ...
  public static String createOrderedRFactor(Object[] row, String orderedLevels) {
    ...
    if (orderedLevelsSplit.length < 2) {
      ...
      str.append(
          "factor(" + vectorParameter + ", ordered = TRUE, levels = " + orderLevelsVector + ");");
    }
    return str.toString();
  }
   ...
   * @return String containing the equivalent r column vector
   */
  public static String createOrderedRFactor(
      Object[] row, String orderedLevels, String orderedLevelLabels) {
    StringBuilder str = new StringBuilder();
    String vectorParameter = createStringRColVec(row);
    Object[] orderedLevelsSplit = orderedLevels.split("+++");
    ...
    if (orderedLevelsSplit.length != orderedLevelLabelsSplit.length) {
      throw new RuntimeException(
          "Counts of ordered levels and the corresponding labels must be equal.");
  ...
  public static String alterColumnNames(
      String tableName, String[] oldHeaders, String[] newHeaders) {
    StringBuilder str = new StringBuilder();
    str.append(
        "setnames("
            + tableName
            + ",old="
            + createStringRColVec(oldHeaders)
            + ",new="
  ...
  public static String alterColumnTypeToCharacter(String tableName, List<String> colName) {
    ...
    builder.append(
        tableName
            + "[,(c('"
            + StringUtils.join(colName, "','")
            + "')) := lapply(.SD, as.character), .SDcols = c('"
            + StringUtils.join(colName, "','")
            + "')]");
    return builder.toString();
  }
  ...
  public static String alterColumnTypeToNumeric(String tableName, String colName) {
    ...
    builder
        ...
        .append(colName)
        .append(" <- ")
        .append("as.numeric(as.character(")
        .append(tableName)
        .append("$")
        .append(colName)
        .append("))");
  ...
  public static String alterColumnTypeToInteger(String tableName, String colName) {
    ...
    builder
        ...
        .append(colName)
        .append(" <- ")
        .append("as.integer(as.character(")
        .append(tableName)
        .append("$")
        .append(colName)
        .append("))");
  ...
  public static String alterColumnTypeToDate(String tableName, String format, List<String> cols) {
    ...
    builder.append(
        tableName
            ...
            + "')) := "
            + "lapply(.SD, function(x) as.Date(x, format='"
            + parsedFormat[0]
            + "')), "
            + ".SDcols = c('"
            + StringUtils.join(cols, "','")
  ...
  public static List<String> alterColumnTypeToDate_Excel(String tableName, List<String> cols) {
    ...
    // append the function that will handle dates that have been translated into a numerical value
    // during read.xlsx2 upload
    builder.append(
        "clean_convertToDate_function <- function(x){	cleanCol <- gsub('^\\\\s*$', 'NA', "
            + tableName
            + "[[x]]) "
            + "%>% .[!is.na(.) & . != 'NA' & . != 'null' & . != 'NULL'];"
            + "if (all(!is.na(as.numeric(cleanCol)))) as.Date('1900-01-01') + as.numeric("
            + tableName
            + "[[x]]) - 2 else 'NOTHING' };");
  ...
  public static String alterColumnTypeToDateTime(
      String tableName, String format, List<String> cols) {
    if (format == null) {
      format = "%Y-%m-%d %H:%M:%OS|NULL";
    }
    // parse out the milliseconds options
    ...
    builder.append(
        tableName
            ...
            + StringUtils.join(cols, "','")
            + "')) := "
            + "lapply(.SD, function(x) as.POSIXct(fast_strptime(x, format='"
            + parsedFormat[0]
            + "'))), "
            + ".SDcols = c('"
            + StringUtils.join(cols, "','")
  ...
  public static List<String> alterColumnTypeToDateTime_Excel(String tableName, List<String> cols) {
    ...
    // append the function that will handle datetimes that have been translated into a numerical
    // value during read.xlsx2 upload
    builder.append(
        "clean_convertToPOSIXct_function <- function(x){ cleanCol <- gsub('^\\\\s*$', 'NA', "
            + tableName
            + "[[x]]) "
            + "%>% .[!is.na(.) & . != 'NA' & . != 'null' & . != 'NULL'];"
            + "if (all(!is.na(as.numeric(cleanCol)))) as.POSIXct(as.numeric("
            + tableName
   ...
   * @return
   */
  public static String getMergeSyntax(
      String returnTable,
      String leftTableName,
      String rightTableName,
      String joinType,
      List<Map<String, String>> joinCols) {
    /*
  ...
  /**
   * Get the correct r syntax for the table join columns This uses the join information keys since
   * it is {leftCol -> rightCol}
   *
   * @param builder
   * @param colNames
   */
  public static void getMergeColsSyntax(
      StringBuilder builder, List<Map<String, String>> colNames, boolean grabKeys) {
  ...
  /**
   * Generate an alter statement to add new columns, taking into consideration joins and new column
   * alias's
   *
   * @param tableName
   ...
   * @return
   */
  public static String alterMissingColumns(
      String tableName,
  ...
  public static String getFReadSyntax(String tableName, String absolutePath, String delimiter) {
    ...
    builder
        .append(tableName)
        .append(" <- fread(\"")
        .append(absolutePath.replace("\\", "/"))
        .append("\", sep=\"")
        .append(delimiter)
        .append(
            "\", encoding=\"UTF-8\", blank.lines.skip=TRUE, fill=TRUE, na.strings=NULL, keepLeadingZeros=TRUE, integer64='numeric');");
    return builder.toString();
  }
  ...
  public static String getExcelReadSheetSyntax(
      String tableName,
      String absolutePath,
      int sheetIndex,
      List<Integer> colIndices,
      boolean uploadSubset) {
    StringBuilder builder = new StringBuilder();
    builder
        .append(tableName)
        .append(" <- as.data.table(read.xlsx2(\"")
        .append(absolutePath.replace("\\", "/"))
        .append("\", sheetIndex=")
        .append(sheetIndex)
        .append(", colClasses = c(rep('character',")
        .append(colIndices.size())
   ...
   * @return
   */
  public static String loadExcelSheet(
      String filePath, String frameName, String sheetName, String sheetRange) {
    StringBuilder rsb = new StringBuilder();
  ...
  public static String loadParquetFile(String filePath, String frameName) {
    ...
    rsb.append(
        frameName
            + " <- read_parquet(file = \""
            + filePath
            + "\", col_select = NULL, as_data_frame = TRUE);");
    rsb.append(frameName + " <- as.data.table(" + frameName + ");");
    return rsb.toString();
  }
   ...
   * @return
   */
  public static String formatFilterValue(
      Object value, SemossDataType dataType, String additionalParameter) {
    DecimalFormat decimalFormat = null;
    if (SemossDataType.STRING == dataType || SemossDataType.FACTOR == dataType) {
      return "\"" + value + "\"";
    } else if (SemossDataType.INT == dataType || SemossDataType.DOUBLE == dataType) {
  ...
  public static String readRDS(String var, String filePath) {
    return new StringBuilder(var + " <-readRDS(\"" + filePath.replace("\\", "/") + "\");")
        .toString();
  }
  ...
  public static String translateJavaRDateTimeFormat(String javaFormat) {
    ...
    for (int i = 0; i < javaFormat.length(); i++) {
      String ch =
          (i == javaFormat.length() - 1) ? javaFormat.substring(i) : javaFormat.substring(i, i + 1);
      if (!ch.matches(datetimeRegex) && !ch.equals("'")) {
        ...
        if (ch.equals("'")) {
          // if character is a single quote (SQ), then need to properly parse to the nearest closing
          // single quote
          if (substr.equals("''")) {
          ...
          if (ch.equalsIgnoreCase("y") || ch.equals("M") || ch.equals("E")) {
            // for these ch, it needs to be concatenated with the length of the substr to identify
   ...
   * @return
   */
  public static String determineLimitOffsetSyntax(
      String tableName, long numRows, long limit, long offset) {
  ...
  public static void main(String[] args) {
    ...
    //		//		joinCols.add(join2);
    //
    //		System.out.println(getMergeSyntax(returnTable, leftTableName, rightTableName, joinType,
    // joinCols));
    //
