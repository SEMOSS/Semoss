...
public class RSyntaxHelper {
	...
	static {
		...
		// javaRDatTimeTranslationMap.put("z", "%Z"); //tz (Pacific Standard Time; PST;
		// GMT-08:00)
		// ???? - TODO needs to be handled via the tz param
	 ...
	 * @param dataType
	 *            The data type for each entry in the object[]
	 ...
	 *            Useful on timestamp where we need to define the timezone, tz()
	 * @return String containing the equivalent r column vector
	 */
		...
		for (; i < size; i++) {
			if (SemossDataType.STRING == dataType || SemossDataType.FACTOR == dataType) {
				...
				if (additionalParameter != null && !additionalParameter.isEmpty()) {
					str.append("as.POSIXct(\"").append(row.get(i).toString())
							.append("\", format='%Y-%m-%d %H:%M:%OS', ").append(additionalParameter).append(")");
				} else {
					str.append("as.POSIXct(\"").append(row.get(i).toString())
							.append("\", format='%Y-%m-%d %H:%M:%OS')");
				}
			} else {
	 ...
	 * @param orderedLevels
	 *            The ordering of the factor
	 * @param orderedLevelLabels
	 *            Corresponding labels of the ordered levels of the factor
	 * @return String containing the equivalent r column vector
	 */
	public static String createOrderedRFactor(Object[] row, String orderedLevels, String orderedLevelLabels) {
		...
		if (orderedLevelsSplit.length != orderedLevelLabelsSplit.length) {
			...
			String orderLevelsVector = createStringRColVec(orderedLevelsSplit);
			str.append("factor(" + vectorParameter + ", ordered = TRUE, levels = " + orderLevelsVector + "), labels = "
					+ orderedLevelLabelsSplit + ");");
		}
		return str.toString();
	}
	...
	public static String alterColumnName(String tableName, String oldHeader, String newHeader) {
		StringBuilder str = new StringBuilder();
		str.append("colnames(" + tableName + ")[which(names(" + tableName + ") == \"" + oldHeader + "\")] <- \""
				+ newHeader + "\";");
		return str.toString();
	}
	...
	public static String alterColumnNames(String tableName, String[] oldHeaders, String[] newHeaders) {
		StringBuilder str = new StringBuilder();
		str.append("setnames(" + tableName + ",old=" + createStringRColVec(oldHeaders) + ",new="
				+ createStringRColVec(newHeaders) + ");");
		return str.toString();
	}
	...
	public static String alterColumnTypeToCharacter(String tableName, String colName) {
		...
		// "datatable$Revenue_International <-
		// as.numeric(as.character(datatable$Revenue_International))"
		StringBuilder builder = new StringBuilder();
		builder.append(tableName).append("$").append(colName).append(" <- ").append("as.character(").append(tableName)
	...
	public static String alterColumnTypeToCharacter(String tableName, List<String> colName) {
		StringBuilder builder = new StringBuilder();
		builder.append(tableName + "[,(c('" + StringUtils.join(colName, "','")
				+ "')) := lapply(.SD, as.character), .SDcols = c('" + StringUtils.join(colName, "','") + "')]");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToFactor(String tableName, String colName) {
		// will generate a string similar to
		// "datatable$Revenue_International <-
		// as.factor(datatable$Revenue_International)"
		StringBuilder builder = new StringBuilder();
		builder.append(tableName).append("$").append(colName).append(" <- ").append("as.factor(").append(tableName)
				.append("$").append(colName).append(")");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToFactor(String tableName, List<String> colName) {
		StringBuilder builder = new StringBuilder();
		builder.append(tableName + "[,(c('" + StringUtils.join(colName, "','")
				+ "')) := lapply(.SD, as.factor), .SDcols = c('" + StringUtils.join(colName, "','") + "')]");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToNumeric(String tableName, List<String> colName) {
		StringBuilder builder = new StringBuilder();
		builder.append(tableName + "[,(c('" + StringUtils.join(colName, "','")
				+ "')) := lapply(.SD, as.numeric), .SDcols = c('" + StringUtils.join(colName, "','") + "')]");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToBoolean(String tableName, String colName) {
		// will generate a string similar to
		// "datatable$Revenue_International <-
		// as.boolean(datatable$Revenue_International)"
		StringBuilder builder = new StringBuilder();
		builder.append(tableName).append("$").append(colName).append(" <- ").append("as.logical(").append(tableName)
				.append("$").append(colName).append(")");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToBoolean(String tableName, List<String> colName) {
		StringBuilder builder = new StringBuilder();
		builder.append(tableName + "[,(c('" + StringUtils.join(colName, "','")
				+ "')) := lapply(.SD, as.logical), .SDcols = c('" + StringUtils.join(colName, "','") + "')]");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToInteger(String tableName, String colName) {
		// will generate a string similar to
		// "datatable$Revenue_International <-
		// as.integer(as.character(datatable$Revenue_International))"
		StringBuilder builder = new StringBuilder();
	...
	public static String alterColumnTypeToInteger(String tableName, List<String> colName) {
		StringBuilder builder = new StringBuilder();
		builder.append(tableName + "[,(c('" + StringUtils.join(colName, "','")
				+ "')) := lapply(.SD, as.integer), .SDcols = c('" + StringUtils.join(colName, "','") + "')]");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToDate(String tableName, String format, String colName) {
		...
		// will generate a string similar to
		// "datatable$Birthday <- as.Date(as.character(datatable$Birthday), format =
		// '%m/%d/%Y')"
		StringBuilder builder = new StringBuilder();
		builder.append(tableName + "$" + colName + "<- as.Date(" + tableName + "$" + colName + ", format = '"
				+ parsedFormat[0] + "')");
		return builder.toString();
	}
	...
	public static String alterColumnTypeToDate(String tableName, String format, List<String> cols) {
		...
		builder.append(tableName + "[,(c('" + StringUtils.join(cols, "','") + "')) := "
				+ "lapply(.SD, function(x) as.Date(x, format='" + parsedFormat[0] + "')), " + ".SDcols = c('"
	...
	public static List<String> alterColumnTypeToDate_Excel(String tableName, List<String> cols) {
		...
		// append the function that will handle dates that have been translated into a
		// numerical value
		// during read.xlsx2 upload
		builder.append("clean_convertToDate_function <- function(x){	cleanCol <- gsub('^\\\\s*$', 'NA', " + tableName
				+ "[[x]]) " + "%>% .[!is.na(.) & . != 'NA' & . != 'null' & . != 'NULL'];"
				+ "if (all(!is.na(as.numeric(cleanCol)))) as.Date('1900-01-01') + as.numeric(" + tableName
				+ "[[x]]) - 2 else 'NOTHING' };");
		// call the function above and sort columns by whether they were handled by the
		// function
		builder.append(convertedDateCols_R + " <- lapply(c('" + StringUtils.join(cols, "','")
				+ "'), clean_convertToDate_function);");
		builder.append(convertedDateColsList_R + " <- c('" + StringUtils.join(cols, "','") + "')[-grep('NOTHING',"
				+ convertedDateCols_R + ")];");
		// update columns via the outputs of the function
		builder.append(
				"if (length(" + convertedDateColsList_R + ") > 0) " + tableName + "[, (" + convertedDateColsList_R
						+ ") := lapply(" + "setdiff(seq(1,length(" + convertedDateCols_R + ")), grep('NOTHING',"
						+ convertedDateCols_R + ")), " + "function(x) unlist(" + convertedDateCols_R + "[[x]] ))];");
	...
	public static String alterColumnTypeToDateTime(String tableName, String format, String colName) {
		...
		builder.append(tableName + "$" + colName + "<- as.POSIXct(fast_strptime(" + tableName + "$" + colName
				+ ", format = '" + parsedFormat[0] + "'))");
		return builder.toString();
	}
	...
	public static List<String> alterColumnTypeToDateTime_Excel(String tableName, List<String> cols) {
		...
		// append the function that will handle datetimes that have been translated into
		// a numerical
		// value during read.xlsx2 upload
		builder.append("clean_convertToPOSIXct_function <- function(x){ cleanCol <- gsub('^\\\\s*$', 'NA', " + tableName
				+ "[[x]]) " + "%>% .[!is.na(.) & . != 'NA' & . != 'null' & . != 'NULL'];"
				+ "if (all(!is.na(as.numeric(cleanCol)))) as.POSIXct(as.numeric(" + tableName
				+ "[[x]])*86400, origin = '1899-12-30', tz='UTC') else 'NOTHING' };");
		// call the function above and sort columns by whether they were handled by the
		// function
		builder.append(convertedDTCols_R + " <- lapply(c('" + StringUtils.join(cols, "','")
				+ "'), clean_convertToPOSIXct_function);");
		builder.append(convertedDTColsList_R + " <- c('" + StringUtils.join(cols, "','") + "')[-grep('NOTHING',"
				+ convertedDTCols_R + ")];");
		// update columns via the outputs of the function
		builder.append("if (length(" + convertedDTColsList_R + ") > 0) " + tableName + "[, (" + convertedDTColsList_R
				+ ") := lapply(" + "setdiff(seq(1,length(" + convertedDTCols_R + ")), grep('NOTHING',"
				+ convertedDTCols_R + ")), " + "function(x) unlist(" + convertedDTCols_R + "[[x]] ))];");
	...
	public static String alterEmptyTableColumnTypeToDateTime(String tableName, List<String> cols) {
		...
		StringBuilder builder = new StringBuilder();
		builder.append(tableName + "[,(c('" + StringUtils.join(cols, "','") + "')) := "
				+ "lapply(.SD, function(x) as.POSIXct(x)), " + ".SDcols = c('" + StringUtils.join(cols, "','") + "')]");
		return builder.toString();
	}
	 ...
	 * @return
	 */
	public static String getMergeSyntax(String returnTable, String leftTableName, String rightTableName,
			String joinType, List<Map<String, String>> joinCols) {
		/*
		...
		StringBuilder builder = new StringBuilder();
		builder.append(returnTable).append(" <- merge(").append(leftTableName).append(", ").append(rightTableName)
				.append(", by.x = c(");
	...
	/**
	 * Get the correct r syntax for the table join columns This uses the join
	 ...
	 * @return
	 */
	public static String alterMissingColumns(String tableName, Map<String, SemossDataType> newColumnsToTypeMap,
			List<Join> joins, Map<String, String> newColumnAlias) {
	...
	public static String getFReadSyntax(String tableName, String absolutePath, String delimiter) {
		StringBuilder builder = new StringBuilder();
		builder.append(tableName).append(" <- fread(\"").append(absolutePath.replace("\\", "/")).append("\", sep=\"")
				.append(delimiter)
	...
	public static String getExcelReadSheetSyntax(String tableName, String absolutePath, int sheetIndex,
			List<Integer> colIndices, boolean uploadSubset) {
		StringBuilder builder = new StringBuilder();
		builder.append(tableName).append(" <- as.data.table(read.xlsx2(\"").append(absolutePath.replace("\\", "/"))
				.append("\", sheetIndex=").append(sheetIndex).append(", colClasses = c(rep('character',")
				.append(colIndices.size()).append(")), stringsAsFactors=FALSE");
		if (uploadSubset) {
	 ...
	 * @param frameName
	 *            The name of the frame to load excel sheet
	 * @param sheetName
	 *            The name of the sheet in the excel workbook
	 * @param sheetRange
	 *            The desired range to load
	 * @return
	 */
	public static String loadExcelSheet(String filePath, String frameName, String sheetName, String sheetRange) {
		...
		rsb.append(frameName + " <- read_excel(path = \"" + filePath + "\", col_names = TRUE, sheet = \"" + sheetName
				+ "\", range='" + sheetRange + "');");
	...
	public static String loadParquetFile(String filePath, String frameName) {
		...
		filePath = filePath.replace("\\", "/");
		rsb.append(
				frameName + " <- read_parquet(file = \"" + filePath + "\", col_select = NULL, as_data_frame = TRUE);");
		rsb.append(frameName + " <- as.data.table(" + frameName + ");");
		return rsb.toString();
	}
			...
			if (!ch.matches(datetimeRegex) && !ch.equals("'")) {
				...
				if (ch.equals("'")) {
					...
					if (ch.equalsIgnoreCase("y") || ch.equals("M") || ch.equals("E")) {
						// for these ch, it needs to be concatenated with the length of the substr to
						// identify
						// the appropriate R syntax
	...
	public static String replaceNAString(String tableName, List<String> colName) {
		// dt[,c("FRAME","AGE")] <- replace(dt[,c("FRAME","AGE")],
		// is.na(dt[,c("FRAME","AGE")]), "NA");
		String colVector = createStringRColVec(colName);
		String subsetTable = tableName + "[," + colVector + "]";
		StringBuilder rsb = new StringBuilder(subsetTable).append(" <- replace(").append(subsetTable).append(",is.na(")
	...
	public static void main(String[] args) {
		...
		// String joinType = "inner.join"; // left.outer.join, right.outer.join,
		// outer.join
		...
		// // joinCols.add(join2);
		//
		// System.out.println(getMergeSyntax(returnTable, leftTableName, rightTableName,
		// joinType,
		// joinCols));
		//
		...
		// joinType = "left.outer.join"; // left.outer.join, right.outer.join,
		// outer.join
		//
		// System.out.println(getMergeSyntax(returnTable, leftTableName, rightTableName,
		// joinType,
