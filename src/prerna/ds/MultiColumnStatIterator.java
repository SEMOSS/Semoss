...
public class MultiColumnStatIterator implements Iterator<Object[]>{
	Iterator<List<Object[]>> iterator;
	DuplicationReconciliation[] statArray;
	int[] indexes;
	Queue<Object[]> returnData;
	
	public MultiColumnStatIterator(TreeNode root, DuplicationReconciliation[] stats) {
...
//		}
	...
	public MultiColumnStatIterator(TreeNode root, DuplicationReconciliation[] stats, List<String> skipColumns) {
		iterator = new UniqueBTreeIterator(root, false, skipColumns);
		statArray = stats;
		returnData = new LinkedList<Object[]>();
	}
	
	public MultiColumnStatIterator(TreeNode root, DuplicationReconciliation[] stats, List<String> skipColumns, String[] columnHeaders) {
		iterator = new UniqueBTreeIterator(root, false, skipColumns);
		statArray = stats;
		indexes = getColumnHeaderIndexes(stats, columnHeaders.length);
		returnData = new LinkedList<Object[]>();
	}
