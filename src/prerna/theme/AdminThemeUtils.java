...
public class AdminThemeUtils extends AbstractThemeUtils {
  ...
  public static Object getActiveAdminTheme() {
    ...
    qs.addExplicitFilter(
        SimpleQueryFilter.makeColToValFilter(
            ADMIN_THEME_PREFIX + "IS_ACTIVE", "==", true, PixelDataType.BOOLEAN));
  ...
  /**
   * Set the active admin level theme This means setting the is_active boolean to true for 1 theme
   * while the previous true value to false;
   *
   * @param themeId
   * @return
   */
  public boolean setActiveTheme(String themeId) {
    String query =
        "UPDATE ADMIN_THEME SET IS_ACTIVE=FALSE WHERE IS_ACTIVE=TRUE; UPDATE ADMIN_THEME SET IS_ACTIVE=TRUE WHERE ID='"
            + themeId
            + "'";
    try {
  ...
  public String createAdminTheme(String themeName, String themeMap, boolean isActive) {
    ...
    try {
      ...
      themeDb
          .getQueryUtil()
          .handleInsertionOfClob(ps.getConnection(), ps, themeMap, parameterIndex++, new Gson());
   ...
   * @return
   */
  public boolean editAdminTheme(
      String themeId, String themeName, String themeMap, boolean isActive) {
    PreparedStatement ps = null;
    try {
      ps =
          themeDb.getPreparedStatement(
              "UPDATE ADMIN_THEME SET THEME_NAME=?, THEME_MAP=?, IS_ACTIVE=? WHERE ID=?");
      int parameterIndex = 1;
      ps.setString(parameterIndex++, themeName);
      themeDb
          .getQueryUtil()
