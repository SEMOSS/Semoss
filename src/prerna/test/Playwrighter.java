...
public class Playwrighter implements Runnable {
  ...
  public static final String starter =
      "You are a natural language to HTML element mapper giving output in the following format as a valid JSON array \n\n [{'pw_id of the element': 'element_value from the prompt provided '}, {'pw_id of the element': 'element_value from the prompt provided '} ] \n\n";
  // by their pw_id attribute that need to be filled. Provide element pw_id and the potential value
  // in the format {'pw_id': 'field pw_id value'}. For html elements provide pw_id attribute of the
  // html element and the potential value from the prompt. Identify all relevant fields that can be
  // pre-filled. Dont identify un-necessary fields. If more than one element provide as an array.
  public static final String json_format =
      "Identify the HTML elements that can satisfy the prompt << ";
  // USE THE CONTEXT PROVIDED ONLY. Do NOT manufacture facts. If this page does not allow me to
  // accomplish my goal please respond with {result:NA}.
  public static final String no_disclaimer =
      "Be Concise. Do not provide as json markdown. Please provide only VALID. Do not provide introductions, explanations and summary. ";
  ...
  public void initJRC() {
    ...
    jrc.login(
        "https://workshop.cfg.deloitte.com/cfg-ai-demo/Monolith/api",
        "488c161f-ba17-45ba-97a1-22915fb17f15",
  ...
  public void run() {
    ...
    prompt =
        "I would like to travel to newyork for a meeting on december 1 2024 for a day. I am traveling alone. ";
  ...
  public String callLLM(String finalPrompt, String engineId) {
    ...
    String llm =
        "LLM(engine=[\"" + engineId + "\"], command=[\"<encode>" + finalPrompt + "</encode>\"]);";
  ...
  private Map getUserChoices(Map llmMap) {
    ...
    while (keys.hasNext()) itemList.append(keys.next()).append("<>");
    ...
    try {
      do {
        ...
        } else System.err.println("No value proposed.. please enter value");
        value = br.readLine(); // type i for ignore
        if (!value.equalsIgnoreCase("i")) newMap.put(elemSelected, value);
  ...
  private void makeObservable(Elements links, String name, String event) {
    ...
    for (int linkIndex = 0; linkIndex < links.size(); linkIndex++) {
      ...
      link.attr(
          event,
          "alert(document.querySelector('[" + css_id + "=" + name + linkIndex + "]').value)");
        ...
        if (key.contains(css_id)) {
          if (elemId == null) {
            if (inputLinks.containsKey(key)) {
              elemId = key;
              if (elemValue == null) elemValue = value;
            } else if (inputLinks.containsKey(value)) elemId = value;
          }
        } else elemValue = value;
      }
  ...
  public void runPW() {
    try (Playwright playwright = Playwright.create()) {
      ...
      page.screenshot(
          new Page.ScreenshotOptions().setPath(Paths.get("c:/temp/example" + count + ".png")));
    }
  }
  ...
  public void runExpedia() {
    ...
    try (Playwright playwright = Playwright.create()) {
      ...
      do {
        ...
        Locator search =
            page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Find a hotel"));
      ...
      /*List<ElementHandle> locList = page.querySelectorAll(" ");
      ...
      */
      ...
      //List <Locator> locators = page.getByRole(AriaRole.FORM).all();
