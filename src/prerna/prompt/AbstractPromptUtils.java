...
public class AbstractPromptUtils {
  ...
  private static void initialize() throws SQLException {
    ...
    if (allowIfExistsIndexs) {
      String sql =
          queryUtil.createIndexIfNotExists("PROMPTMETA_PROMPT_ID_INDEX", "PROMPTMETA", "PROMPT_ID");
      classLogger.info("Running sql " + sql);
      promptDb.insertData(sql);
    } else {
      // see if index exists
      if (!queryUtil.indexExists(
          promptDb, "PROMPTMETA_PROMPT_ID_INDEX", "PROMPTMETA", database, schema)) {
    ...
    for (String tableName : metaKeyTableNames) {
      ...
      {
        List<String> allCols =
            queryUtil.getTableColumns(promptDb.getConnection(), tableName, database, schema);
        ...
        try {
          wrapper =
              WrapperManager.getInstance()
                  .getRawWrapper(promptDb, "select count(*) from " + tableName);
          if (wrapper.hasNext()) {
