...
public class PromptOwlCreator extends AbstractOwlCreator {

	public PromptOwlCreator(AbstractSqlQueryUtil queryUtil) {
	...
	public void createColumnsAndTypes(AbstractSqlQueryUtil queryUtil) {
		...
		final String INTEGER_DATATYPE_NAMEA = queryUtil.getIntegerDataTypeName();
		final String VARCHAR_255 = "VARCHAR(255)";

		this.allSchemas = new ArrayList<>();

		// @formatter:off
		addTable("PROMPT", Arrays.asList(
				Pair.with("ID", VARCHAR_255),
				Pair.with("TITLE", VARCHAR_255),
				Pair.with("CONTEXT", CLOB_DATATYPE_NAME),
				Pair.with("VERSION", INTEGER_DATATYPE_NAMEA),
				Pair.with("INTENT", VARCHAR_255),
		...
		addTable("PROMPTMETA", Arrays.asList(
				Pair.with("PROMPT_ID", VARCHAR_255),
				Pair.with("METAKEY", VARCHAR_255),
				Pair.with("METAVALUE", CLOB_DATATYPE_NAME),
				Pair.with("METAORDER", INTEGER_DATATYPE_NAMEA)));
