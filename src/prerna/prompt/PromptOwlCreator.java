...
public class PromptOwlCreator {
	...
	private void createColumnsAndTypes(AbstractSqlQueryUtil queryUtil) {
		...
		this.promptColumns = Arrays.asList(Pair.with("ID", "VARCHAR(255)"), Pair.with("TITLE", "VARCHAR(255)"),
				Pair.with("CONTEXT", CLOB_DATATYPE_NAME), Pair.with("VERSION", "INT"),
				Pair.with("INTENT", "VARCHAR(255)"), Pair.with("CREATED_BY", "VARCHAR(255)"),
				Pair.with("DATE_CREATED", "TIMESTAMP"), Pair.with("IS_LATEST", BOOLEAN_DATATYPE_NAME));

		this.promptMetaColumns = Arrays.asList(Pair.with("PROMPT_ID", "VARCHAR(255)"),
				Pair.with("METAKEY", "VARCHAR(255)"), Pair.with("METAVALUE", CLOB_DATATYPE_NAME),
				Pair.with("METAORDER", "INT"));

		this.promptMetaKeysColumns = Arrays.asList(Pair.with("METAKEY", "VARCHAR(255)"),
				Pair.with("SINGLEMULTI", "VARCHAR(255)"), Pair.with("DISPLAYORDER", "VARCHAR(255)"),
				Pair.with("DISPLAYOPTIONS", "INT"), Pair.with("DEFAULTVALUES", "VARCHAR(255)"));

		this.allSchemas = Arrays.asList(Pair.with("PROMPT", this.promptColumns),
