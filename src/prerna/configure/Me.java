...
public class Me {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args == null || args.length < 5) {
			System.out.println(
					"Usage: java prerna.configure.Me <semoss home dir> <r home> <r library> <Location to R Library dll/so> <Location to JRI library dll/so>");
			System.exit(0);
		} else if (args != null && args.length == 5) {
			// System.out.println("CAUTION!!! MULTIPLE ARGUEMENTS BEING PASSED, MIGHT BE
			// ERROR.. WILL TRY TO RUN WITH FIRST ARGUMENT\n"
...
//		cm.changeJS(homePath, port);
		...
		System.out.println("------------------------");
		System.out.println("SEMOSS configured! Run startSEMOSS.bat and point your browser to http://localhost:" + port
				+ "/SemossWeb/ to access SEMOSS!");
		System.out.println("------------------------");
	}
	...
	public void writePath(String semossHome, String rHome, String rDll, String jriDll, String rLib) {
		...
		try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName)))) {
			...
			classLogger.error("Failed to write path configuration file: {}", e.getMessage(), e);
		}
	}

	public void writeTomcatEnv(String semossHome, String rHome, String jriHome) {
		// -Djava.library.path=C:\Users\pkapaleeswaran\Documents\R\win-library\3.1\rJava\jri\x64;"C:\Program
		// Files\R\R-3.2.4revised\bin\x64"
		...
		try (BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName)))) {
			...
			classLogger.error("Failed to write Tomcat environment file: {}", e.getMessage(), e);
		}

	}

	public void writeConfigureFile(String homePath, String port) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(homePath + "/configured.txt"))) {
			writer.write("Port = " + port);
			writer.write("SEMOSS Web = " + "http://localhost:" + port + "/SemossWeb/");
			writer.flush();
		} catch (IOException e) {
			classLogger.error("Failed to write SEMOSS configure marker file: {}", e.getMessage(), e);
		}
	}
...
//		appFile = "C:/Users/pkapaleeswaran/Desktop/From C Drive Root/apache-tomcat-8.0.15/conf/server2.xml";
	...
	public String findOpenPort() {
		...
		try (InputStreamReader is = new InputStreamReader(System.in); BufferedReader br = new BufferedReader(is)) {
			...
			classLogger.error("Failed to read fallback port from console: {}", e.getMessage(), e);
		}
		return portStr;
	}
	...
	public void changeRDFMap(String homePath, String port, String rdfHome) {
		...
		String[] stringToReplace = { "BaseFolder", "LOG4J", "SMSSWebWatcher_DIR", "SMSSWatcher_DIR",
				"SMSSStorageWatcher_DIR", "SMSSModelWatcher_DIR", "SMSSVectorWatcher_DIR", "SMSSFunctionWatcher_DIR",
				"SMSSGuardrailWatcher_DIR", "SMSSVenvWatcher_DIR", "ProjectWatcher_DIR", "CSVInsightsWebWatcher_DIR",
				"INSIGHT_CACHE_DIR", "SOLR_BASE_URL", "ADDITIONAL_REACTORS", "SOCIAL", "JobSchedulerWatcher_DIR",
				"rpa.config.directory", "EMAIL_TEMPLATES" };
		String[] stringToReplaceWith = { homePath, homePath + "/log4j.prop", homePath + "/db", homePath + "/db",
				homePath + "/storage", homePath + "/model", homePath + "/vector", homePath + "/function",
				homePath + "/guardrail", homePath + "/venv", homePath + "/project",
				homePath + "/InsightCache/CSV_Insights", homePath + "/InsightCache",
				"http://localhost:" + port + "/solr", homePath + "/reactors.json", homePath + "/social.properties",
				homePath + "/rpa/json", homePath + "/rpa", homePath + "/emailTemplates/" };
...
//		String altPath = homePath + "/Webcontent/dev/app/app.config2.js";
		...
		updatePortInJsConfig(appPath, altPath, port);
...
//		altPath = homePath + "/Webcontent/dev/olddev/app/scripts/config2.js";
	...
	private void updatePortInJsConfig(String appPath, String altPath, String port) {
		try (FileInputStream fis = new FileInputStream(appPath);
				InputStreamReader isr = new InputStreamReader(fis);
				BufferedReader br = new BufferedReader(isr);
				FileOutputStream fos = new FileOutputStream(altPath);
			...
			classLogger.error("Failed to update port in JavaScript config file: {}", e.getMessage(), e);
		}
	}
	...
	private void replaceProp(String fileName, String[] keysForThingsToReplace, String[] newValuesForReplacement) {
		...
		try (FileReader fr = new FileReader(file); BufferedReader reader = new BufferedReader(fr)) {
			...
			classLogger.error("Failed to update properties file values: {}", e.getMessage(), e);
			return;
		}
	...
	public void changeWebXML(String homePath) throws Exception {
		...
		Map<String, String> thingsToWatch = new HashMap<>();
		...
		for (int nodeIndex = 0; nodeIndex < nl.getLength(); nodeIndex++) {
			...
			for (int childIndex = 0; childIndex < cl.getLength(); childIndex++) {
				...
				// System.out.println("child is.. " + c.getNodeName() + ">>" +
				// c.getTextContent());
