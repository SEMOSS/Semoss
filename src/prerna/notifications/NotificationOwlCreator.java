...
public class NotificationOwlCreator {
   ...
   // each column name paired to its type in a var
	...
	private void createColumnsAndTypes(AbstractSqlQueryUtil queryUtil) {
		...
		this.notificationColumns = Arrays.asList(
				Pair.with("NOTIFICATIONID", "VARCHAR(255)"),
				Pair.with("RECIPIENTID", "VARCHAR(255)"),
				Pair.with("RECIPIENTTYPE", "VARCHAR(255)"),
				Pair.with("NOTIFICATIONTITLE", "VARCHAR(255)"),
				Pair.with("MESSAGE", CLOB_DATATYPE_NAME),
				Pair.with("ACTIONTYPE", "VARCHAR(50)"),
				Pair.with("ACTIONTARGET", "VARCHAR(255)"),
				Pair.with("ISREAD", BOOLEAN_DATATYPE_NAME),
				Pair.with("PRIORITY", "VARCHAR(20)"),
				Pair.with("NOTIFICATIONTYPE", "VARCHAR(255)"),
				Pair.with("CATALOGID", "VARCHAR(255)"),
				Pair.with("CREATEDBY", "VARCHAR(255)"),
		...
		this.allSchemas = Arrays.asList(
				Pair.with("NOTIFICATION", this.notificationColumns)
			);
	}
