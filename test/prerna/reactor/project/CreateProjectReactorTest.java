 ...
 *
 * <p>WORKSPACE and SKILL projects have additional persistence requirements
 * (inference-tracking WORKSPACE row + WORKSPACE_RESOURCE links for
 * workspaces; skill-metadata wiring for skills) that {@code CreateProject}
 * does not perform. Leaving those paths open let callers create
 * half-materialized projects that downstream readers couldn't see — see
 * the AgentPicker bug that motivated this guard.
 */
class CreateProjectReactorTest {
    ...
    private void setKeys(String name, String projectType, boolean hasPortal) {
        ...
        reactor.keyValue.put(ReactorKeysEnum.PORTAL.getKey(), String.valueOf(hasPortal));
    }
    ...
    void rejectsWorkspaceType() {
        setKeys("MyProject", "WORKSPACE", true);
    ...
    void rejectsUnknownProjectTypeWithAllowedListInMessage() {
        setKeys("MyProject", "NOT_A_REAL_TYPE", true);
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class, () -> reactor.execute());
    ...
    void invalidNameStillRejectedBeforeTypeCheck() {
        ...
        setKeys("123 bad name starts with digit", "WORKSPACE", true);
        IllegalArgumentException ex = assertThrows(IllegalArgumentException.class, () -> reactor.execute());
