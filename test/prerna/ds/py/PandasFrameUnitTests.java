...
class PandasFrameUnitTests {
    ...
    class ConstructorTests {
        ...
        @Test
        void constructor_initializesCommandsList() {
            PandasFrame frame = new PandasFrame("test", mockTranslator);
            assertNotNull(frame.commands);
    ...
    class IsEmptyTests {
        ...
        void isEmpty_withTableName_whenTranslatorReturnsTrue_returnsFalse() {
            ...
            String wn = "myFramew";
            String cmd = "( ('" + wn + "' in vars() or '" + wn
                    + "' in globals()) and len(" + wn + ".cache['data'])>=0 )";
    ...
    class GetSQLiteTests {
        ...
        void getSQLite_firstCall_createsConnectionViaRunEmptyPy() {
            PandasFrame frame = new PandasFrame("myFrame", mockTranslator);
            frame.getSQLite();
            verify(mockTranslator).runEmptyPy(
                    "import sqlite3",
    ...
    class MergeTests {
        ...
        void merge_nonEqui_multipleMatchingCols_renamesBoth() {
            ...
            verify(mockTranslator).runScript(
                    PandasSyntaxHelper.alterColumnName("right", "x", "x_CTD"));
            verify(mockTranslator).runScript(
                    PandasSyntaxHelper.alterColumnName("right", "y", "y_CTD"));
    ...
    class EmptyStubTests {
        ...
        void addRow_doesNotThrow() {
            PandasFrame frame = new PandasFrame("test", mockTranslator);
            assertDoesNotThrow(() -> frame.addRow(
                    new Object[]{"a", 1}, new String[]{"col1", "col2"}));
        }
    ...
    class PublicFieldTests {
        ...
        @Test
        void commands_canAddItems() {
            PandasFrame frame = new PandasFrame("test", mockTranslator);
            frame.commands.add("some_command");
            assertEquals(1, frame.commands.size());
            assertEquals("some_command", frame.commands.get(0));
        }
    }
    ...
    class GetHeaderAndTypesTests {
        ...
        void getHeaderAndTypes_returnsTypesAndHeaders() {
            ...
            when(mockTranslator.getList(colScript))
                    .thenReturn(Arrays.asList("col1", "col2", "col3"));
            when(mockTranslator.getList(typeScript))
                    .thenReturn(Arrays.asList("object", "int64", "float64"));
        ...
        void getHeaderAndTypes_withDatetime64() {
            ...
            when(mockTranslator.getList(colScript))
                    .thenReturn(Arrays.asList("dateCol"));
            when(mockTranslator.getList(typeScript))
                    .thenReturn(Arrays.asList("datetime64"));
        ...
        void getHeaderAndTypes_withBoolType() {
            ...
            String typeScript = PandasSyntaxHelper.getTypes("df");
            when(mockTranslator.getList(colScript))
                    .thenReturn(Arrays.asList("flag"));
            when(mockTranslator.getList(typeScript))
                    .thenReturn(Arrays.asList("bool"));
        ...
        void getHeaderAndTypes_withUnknownType_returnsNull() {
            ...
            when(mockTranslator.getList(typeScript))
                    .thenReturn(Arrays.asList("complex128"));
        ...
        void getHeaderAndTypes_resultStructure() {
            ...
            when(mockTranslator.getList(typeScript))
                    .thenReturn(Arrays.asList("object", "object"));
        ...
        void getHeaderAndTypes_usesDifferentTargetFrame() {
            ...
            when(mockTranslator.getList(typeScript))
                    .thenReturn(Arrays.asList("float64"));
        ...
        void querySQL_selectCommand_runsPyCommands() {
            ...
            try (MockedStatic<Utility> utilMock = mockStatic(Utility.class)) {
                ...
                verify(mockTranslator).runEmptyPy(
                        ...
                        contains("dtypes.to_dict"),
                        contains("to_dict('split')"),
                        contains("_dict['types']")
                );
                // Verify cleanup
    ...
    class SaveTests {
        ...
        void save_frameFilePathHasForwardSlashes() throws Exception {
            ...
            try {
                ...
                verify(mockTranslator).runEmptyPy(
                        eq("import pickle"),
                        argThat((String cmd) -> !cmd.contains("\\"))
                );
            } finally {
                Files.walk(tempDir)
                        .sorted(Comparator.reverseOrder())
