...
public class AbstractRdbmsFrameUnitTests {
  ...
  class InnerAbstractRdbmsFrameUnitTests {
    ...
    void constuctor() throws Exception {
      try (MockedStatic<UUID> uuidStatic = Mockito.mockStatic(UUID.class)) {
        ...
        frame =
            new AbstractRdbmsFrame() {
              @Override
              public CachePropFileFrameObject save(String folderDir, Cipher cipher)
                  throws IOException {
                return null;
              }
    ...
    void constuctorTableName() throws Exception {
      try (MockedStatic<UUID> uuidStatic = Mockito.mockStatic(UUID.class)) {
        ...
        frame =
            new AbstractRdbmsFrame("tableName") {
              @Override
              public CachePropFileFrameObject save(String folderDir, Cipher cipher)
                  throws IOException {
                return null;
    ...
    void constuctorHeaders() throws Exception {
      ...
      try (MockedStatic<UUID> uuidStatic = Mockito.mockStatic(UUID.class)) {
        ...
        doNothing()
            .when(builder)
            .alterTableNewColumns(eq("RDBMSFRAME_UUID"), eq(headers), any(String[].class));

        frame =
            new AbstractRdbmsFrame(headers) {
              @Override
              public CachePropFileFrameObject save(String folderDir, Cipher cipher)
                  throws IOException {
                return null;
    ...
    void constuctorHeadersTypes() throws Exception {
      ...
      try (MockedStatic<UUID> uuidStatic = Mockito.mockStatic(UUID.class);
        ...
        doNothing()
            .when(builder)
            .alterTableNewColumns(eq("RDBMSFRAME_UUID"), eq(headers), eq(types));

        frame =
            new AbstractRdbmsFrame(headers, types) {
              @Override
              public CachePropFileFrameObject save(String folderDir, Cipher cipher)
                  throws IOException {
                return null;
  ...
  void removeColumn() throws Exception {
    ...
    when(util.alterTableDropColumn(anyString(), eq(headers)))
        .thenReturn("DROP COL1 FROM TABLENAME");
  ...
  void queryString() throws Exception {
    ...
    try (MockedStatic<RawRDBMSSelectWrapper> wrapperStatic =
        Mockito.mockStatic(RawRDBMSSelectWrapper.class)) {
      wrapperStatic
          .when(
              () -> RawRDBMSSelectWrapper.directExecutionViaConnection(frame.conn, "query", false))
          .thenReturn(wrapper);
  ...
  void querySelectQueryStruct() throws Exception {
    ...
    try (MockedStatic<QSAliasToPhysicalConverter> qsAliasStatic =
            Mockito.mockStatic(QSAliasToPhysicalConverter.class);
        MockedStatic<QSRenameTableConverter> qsRemaeStatic =
            Mockito.mockStatic(QSRenameTableConverter.class);
        MockedStatic<RawRDBMSSelectWrapper> wrapperStatic =
            Mockito.mockStatic(RawRDBMSSelectWrapper.class)) {

      qsAliasStatic
          .when(
              () ->
                  QSAliasToPhysicalConverter.getPhysicalQs(
                      eq(qs), any(OwlTemporalEngineMeta.class)))
          .thenReturn(qs);
      qsRemaeStatic
          .when(() -> QSRenameTableConverter.convertQs(eq(qs), any(Map.class), eq(true)))
          .thenReturn(qs);
  ...
  void querySql() {
    ...
    try (MockedStatic<QSAliasToPhysicalConverter> qsAliasStatic =
      ...
      IllegalArgumentException e =
          assertThrows(IllegalArgumentException.class, () -> frame.querySQL(query));
