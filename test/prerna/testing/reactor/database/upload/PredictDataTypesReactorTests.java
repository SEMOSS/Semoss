...
public class PredictDataTypesReactorTests extends AbstractBaseSemossApiTests {
  ...
  public void testPredictTypes() {
    ...
    Map<String, Object> predictedTypes =
        UploadTestUtility.predictDataTypes(
            ApiTestsSemossConstants.MOVIE_CSV_FILE_NAME, ApiTestsSemossConstants.DELIMITER);
    ...
    assertEquals(
        SemossDataType.STRING.toString(), dataTypes.get(ApiTestsSemossConstants.NOMINATED));

    // int cols
    assertEquals(
        SemossDataType.INT.toString(), dataTypes.get(ApiTestsSemossConstants.REVENUE_DOMESTIC));
    assertEquals(
        SemossDataType.INT.toString(),
        dataTypes.get(ApiTestsSemossConstants.REVENUE_INTERNATIONAL));
    assertEquals(
        SemossDataType.INT.toString(), dataTypes.get(ApiTestsSemossConstants.MOVIE_BUDGET));

    // double cols
    assertEquals(
        SemossDataType.DOUBLE.toString(),
        dataTypes.get(ApiTestsSemossConstants.ROTTEN_TOMATOES_CRITICS));
    assertEquals(
        SemossDataType.DOUBLE.toString(),
        dataTypes.get(ApiTestsSemossConstants.ROTTEN_TOMATOES_AUDIENCE));
  }
}
