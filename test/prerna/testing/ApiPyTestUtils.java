...
public class ApiPyTestUtils {
	...
	private static final Logger LOGGER = LogManager.getLogger(ApiPyTestUtils.class);

	public static void setupAndCheck() throws Exception {
		LOGGER.info("Checking and setting up Python");
		Path pyFolderInSemoss = Paths.get(ApiTestsSemossConstants.BASE_DIRECTORY, "py");
		if (Files.exists(pyFolderInSemoss)) {
			LOGGER.info("Py folder in base directory exists, setting up Python.");
			copyPyDirectory();

			LOGGER.info("Python pixel check");
			basicPythonPixelCheck();

			LOGGER.info("Teardown py server");
			teardownPyServer();
		} else {
			LOGGER.warn("Py folder in base directory does not exist, Python will NOT be setup.");
		}
	...
	private static void teardownPyServer() {
		try {
			...
			LOGGER.info("Killing python process for user");
			...
			if (cpw != null) {
				...
				LOGGER.info("Closing CPW");
				if (cpw.getProcess() != null) {
					LOGGER.info("Destroying cpw process");
			...
			LOGGER.fatal("COULD NOT KILL PYTHON PROCESS <<<<< LOOK AT THIS PLEASE", e);
		}
	}

	private static void basicPythonPixelCheck() {
		String pixel = "Py(\"<encode>1+1</encode>\");";
		LOGGER.info("Running basic python pixel");
		...
		LOGGER.info("basic python pixel check passed");
	}
