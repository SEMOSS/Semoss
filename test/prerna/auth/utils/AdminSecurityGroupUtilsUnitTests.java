...
public class AdminSecurityGroupUtilsUnitTests extends AbstractSecurityUtilsUnitTestsSetup {
    ...
    void testDeleteGroupAndPropogate_CustomGroup() throws Exception {
        ...
        List<String> userCustomGroups = AdminSecurityGroupUtils.getUserCustomGroups(adminUser.getAccessToken(AuthProvider.NATIVE));
    ...
    void testGetNonGroupMembers_NoSearchTerm() throws Exception {
        ...
        assertEquals(0, nonGroupMembers.stream()
                .map(s -> s.get("id"))
                .filter(s -> s.equals("adminid"))
                .count());
    ...
    void testGetProjectsForGroup_searchLimitAndOffset() throws Exception {
        ...
        // make sure that the only value we get back is the one for the group id, that starts with pid,
        // that is offset by 1
    ...
    void testUserExists_yes() {
        assertTrue(instance.userExists(adminUser.getPrimaryLoginToken().getId(),
                adminUser.getPrimaryLogin().getLabel()));
    }
