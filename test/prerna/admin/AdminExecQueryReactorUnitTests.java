...
public class AdminExecQueryReactorUnitTests {
  ...
  //
  //			qs.getQsType() == QUERY_STRUCT_TYPE.ENGINE || qs.getQsType() ==
  // QUERY_STRUCT_TYPE.RAW_ENGINE_QUERY;
  //
  //			IllegalArgumentException e = assertThrows(IllegalArgumentException.class, reactor::execute);
  //			assertEquals("Input to admin exec query requires a query struct on an engine",
  // e.getMessage());
  //		}
  //	}
  ...
  void testQueryStructNotEngine() {
    ...
    try (MockedStatic<SecurityAdminUtils> sau = Mockito.mockStatic(SecurityAdminUtils.class)) {
      ...
      IllegalArgumentException e = assertThrows(IllegalArgumentException.class, reactor::execute);
      assertEquals(
          "Input to admin exec query requires a query struct on an engine", e.getMessage());
    }
  }

  @ParameterizedTest
  @EnumSource(
      value = QUERY_STRUCT_TYPE.class,
      names = {"ENGINE", "RAW_ENGINE_QUERY"})
  ...
  //	@Test
  //	void testNullInsertDataIntoEngine(QUERY_STRUCT_TYPE qsType, IDatabaseEngine.DATABASE_TYPE
  // dbType) {
  //		NounStore ns = mock(NounStore.class);
  //		reactor.setNounStore(ns);
  //		GenRowStruct grs = mock(GenRowStruct.class);
  ...
  //	        assertEquals("An error occurred trying to execute the query in the database",
  // e.getMessage());
  //		}
  //	}
  ...
  void testInvalidInsertDataIntoEngine() {
    ...
    try {
      doThrow(new SemossPixelException("Database error"))
          .when(engine)
    ...
    try (MockedStatic<SecurityAdminUtils> sau = Mockito.mockStatic(SecurityAdminUtils.class)) {
      ...
      SemossPixelException e = assertThrows(SemossPixelException.class, reactor::execute);
      assertEquals(
          "An error occurred trying to execute the query in the database: Database error",
          e.getMessage());
    }
  }
  ...
  void testSelectQueryStruct() {
    ...
    try {
      doThrow(new SemossPixelException("Database error"))
          .when(engine)
          .insertData(interp.composeQuery());
    } catch (Exception e) {

    }

    try (MockedStatic<SecurityAdminUtils> sau = Mockito.mockStatic(SecurityAdminUtils.class);
        MockedStatic<DeleteSqlInterpreter> deleteSqlInterpreterMockedStatic =
            Mockito.mockStatic(DeleteSqlInterpreter.class)) {
      SecurityAdminUtils s = mock(SecurityAdminUtils.class);
      sau.when(() -> SecurityAdminUtils.getInstance(user)).thenReturn(s);
  ...
  void testUpdateQueryStruct() {
    ...
    try (MockedStatic<SecurityAdminUtils> sau = Mockito.mockStatic(SecurityAdminUtils.class);
        MockedStatic<UpdateSqlInterpreter> updateSqlInterpreterMockedStatic =
            Mockito.mockStatic(UpdateSqlInterpreter.class)) {
      SecurityAdminUtils s = mock(SecurityAdminUtils.class);
      sau.when(() -> SecurityAdminUtils.getInstance(user)).thenReturn(s);
