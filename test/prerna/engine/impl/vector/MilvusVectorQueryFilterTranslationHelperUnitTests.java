...
public class MilvusVectorQueryFilterTranslationHelperUnitTests {
	...
	void testProcessMilvusFilter() {
		...
		assertEquals(tableName + "__IS_LATEST == true AND " + tableName + "__IS_DELETED == false", filter);

		SimpleQueryFilter simpleFilter = SimpleQueryFilter.makeColToColFilter(
				new QueryColumnSelector(tableName + "__COL1"), ">", new QueryColumnSelector(tableName + "__COL2"));
	...
	void testAddSelectorToValuesMilvusFilter() {
		NounMetadata lnm = new NounMetadata(new QueryColumnSelector("MILVUS_TABLE_NAME__COL1"), PixelDataType.COLUMN);
