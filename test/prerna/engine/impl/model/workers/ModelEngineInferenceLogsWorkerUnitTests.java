...
public class ModelEngineInferenceLogsWorkerUnitTests {
	...
	void test() {
		...
		reactor = new ModelEngineInferenceLogsWorker("id", "method", engine, insight.getInsightId(),
				insight.getContextProjectId(), insight.getProjectId(), insight.getUser(), "sessionId", "roomId",
				"context", "", new ArrayList() {
					{
						add("full prompt");
					}
				}, 0, ZonedDateTime.of(LocalDate.now(), LocalTime.now(), ZoneId.systemDefault()), "response", 1,
		...
		try (MockedStatic<Utility> util = Mockito.mockStatic(Utility.class);
			...
			milUtils.when(() -> ModelInferenceLogsUtils.doCreateNewAgent(engineId, engineName, null, "", "tokenId"))
					.thenAnswer((Answer<Void>) invocation -> null);

			milUtils.when(() -> ModelInferenceLogsUtils.doCheckRoomExists(insightId)).thenReturn(false);
			milUtils.when(() -> ModelInferenceLogsUtils.doCreateNewConversation(insightId, "full prompt", null,
					"userId", userEmail, "tokenUsername", "", engineId, true, projectId, "porjectName"))
					.thenAnswer((Answer<Void>) invocation -> null);

			milUtils.when(() -> ModelInferenceLogsUtils.setRoomContext(insightId, "userId", "userUsername"))
					.thenAnswer((Answer<Void>) invocation -> null);

			when(((AbstractModelEngine) engine).keepInputOutput()).thenReturn(true);

			milUtils.when(() -> ModelInferenceLogsUtils.doRecordMessage(eq("messageId"), eq("INPUT"), eq("full prompt"),
					eq("method"), eq(0), any(Double.class), any(ZonedDateTime.class), eq(engineId), eq(insightId),
					eq(sessionId), eq(insightId), // room id
					eq("userId"), eq("userUserName"), eq(userEmail))).thenAnswer((Answer<Void>) invocation -> null);
			milUtils.when(() -> ModelInferenceLogsUtils.doRecordMessage(eq("messageId"), eq("RESPONSE"),
					eq("full prompt"), eq("method"), eq(0), any(Double.class), any(ZonedDateTime.class), eq(engineId),
					eq(insightId), eq(sessionId), eq(insightId), // room id
					eq("userId"), eq("userUserName"), eq(userEmail))).thenAnswer((Answer<Void>) invocation -> null);
      ...
      /////////////////////
			...
			milUtils.verify(() -> {
				...
				ModelInferenceLogsUtils.doCreateNewConversation(insightId, "full prompt", null, "userId", userEmail,
						"tokenUsername", "", engineId, true, projectId, "porjectName");

				ModelInferenceLogsUtils.setRoomContext(insightId, "userId", "userUsername");

				ModelInferenceLogsUtils.doRecordMessage(eq("messageId"), eq("INPUT"), eq("full prompt"), eq("method"),
						eq(0), any(Double.class), any(ZonedDateTime.class), eq(engineId), eq(insightId), eq(sessionId),
						eq(insightId), // room id
						eq("userId"), eq("userUserName"), eq(userEmail));

				ModelInferenceLogsUtils.doRecordMessage(eq("messageId"), eq("RESPONSE"), eq("full prompt"),
						eq("method"), eq(0), any(Double.class), any(ZonedDateTime.class), eq(engineId), eq(insightId),
						eq(sessionId), eq(insightId), // room id
						eq("userId"), eq("userUserName"), eq(userEmail));
			});
	...
	void testAbstractVectorDatabaseEngine() {
      ...
      /////////////////////
			...
			milUtils.verify(() -> {
				...
				ModelInferenceLogsUtils.doCreateNewConversation(insightId, "full prompt", null, "userId",
						"tokenUsername", userEmail, "", engineId, true, projectId, "porjectName");
	...
	void testPGVectorDatabaseEngine() {
		...
		reactor = new ModelEngineInferenceLogsWorker("id", "method", engine, insight.getInsightId(),
				...
				ZonedDateTime.of(LocalDate.now(), LocalTime.now(), ZoneId.systemDefault()), "response", 1,
				ZonedDateTime.of(LocalDate.now(), LocalTime.now(), ZoneId.systemDefault()));
