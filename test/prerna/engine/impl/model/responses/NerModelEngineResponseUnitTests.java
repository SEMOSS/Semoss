...
public class NerModelEngineResponseUnitTests {
  ...
  void fromJson() {
    ...
    assertNotNull(ans);
    assertEquals(
        "{mask_values={key=value}, output=output, input=input, entities=[string], raw_output=[{score=5.0, start=0, end=1, text=text, label=label}], message=message, status=status}",
        ans.getResponse().toString());
  ...
  void fromJsonNullObj() {
    ...
    assertNotNull(ans);
    assertEquals(
        "{message=Null response from model request, status=error}", ans.getResponse().toString());
    assertEquals(0, (int) ans.getNumberOfTokensInPrompt());
    assertEquals(0, (int) ans.getNumberOfTokensInResponse());
  }
}
