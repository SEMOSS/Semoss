...
public class RDFFileJenaEngineUnitTests {
	...
	void testExecQueryRS() {
		...
		try (MockedStatic<QueryFactory> qfMockedStatic = Mockito.mockStatic(QueryFactory.class);
				MockedStatic<QueryExecutionFactory> qefMockedStatic = Mockito
	...
	static Stream<Arguments> determineLangSupplier() {
		return Stream.of(arguments("RDF/XML", Lang.RDFXML), arguments("TURTLE", Lang.TURTLE), arguments("N3", Lang.N3),
				arguments("NTRIPLES", Lang.NTRIPLES), arguments("TRIG", Lang.TRIG), arguments("TRIX", Lang.TRIX));
	}
