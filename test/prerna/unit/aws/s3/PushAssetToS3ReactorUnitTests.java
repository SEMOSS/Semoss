...
public class PushAssetToS3ReactorUnitTests {
    ...
    public void testReactorDescription() {
        assertEquals("Upload an asset file to an S3 bucket. Credentials can be set via a profile path/name or with an explicit access key and secret",
    ...
    public void testExecuteAssetIsDirectory() {
        ...
        try (MockedStatic<AssetUtility> au = mockStatic(AssetUtility.class);
            ...
            when(tm.uploadDirectory("bucket", "fp", null, true))
                    .thenReturn(multiFileUpload);
